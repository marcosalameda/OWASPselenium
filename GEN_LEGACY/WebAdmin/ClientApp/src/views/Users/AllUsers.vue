<template>
	...
	<qtable 
		...
		<template #QWB="props">
			<q-badge
				v-for="userRole in props.row.UserRoles.filter(role => role.Module === 'QWB' && Modules[role.Module].active)"
				class="q-table__cell--multiple"
				:key="userRole.Role"
				variant="bold" >
				{{ Resources[userRole.Designation] }}
	...
	export default {
		...
		data() {
			...
			return {
				...
				userId: '',
				Modules: {
					QWB : { active: true, Cod: "QWB", Description: 'QWEB31116'},
		...
		computed: {
			selectModules: {
                get() {
					...
					return                     this.Modules.QWB.active &&
                    this.Modules.XRS.active ;
                },
                set(value) {
                    //If we explicitly declare the variables, vue will bind them
                    this.Modules.QWB.active = value;
		...
		methods: {
			...
			userColumns() {
				...
				{
					label: () => vm.$t(vm.Modules.QWB.Description),
					name: "QWB",
					slot_name: "QWB",
					sort: false,
					visibility: vm.Modules.QWB.active && vm.columnModules
				},
				{
