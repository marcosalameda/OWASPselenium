...
namespace CSGenio.business
{
	...
	public class CSGenioAperio : DbArea
	{
		...
		public CSGenioAperio(User user) : this(user, user.CurrentModule)
		{
		}
...
#pragma warning restore CS0168, S1481 // Variable is declared but never used
			//- - - - - - - - - - - - - - - - - - -
			Qfield = new Field(info.Alias, "codperio", FieldType.KEY_INT);
		...
		private static void InicializaFormulas(AreaInfo info)
		{
			...
			// emptyD([PERIO->PERISTAR])==1 || emptyD([PERIO->PERIEND])==1 || ComparaDatas([PERIO->PERISTAR],[PERIO->PERIEND])<=0
			{
		...
		private static AreaInfo InicializaAreaInfo()
		{
			...
			info.QSystem="GQT";
			info.TableName="gqtperiod";
			info.ShadowTabName="";
			info.ShadowTabKeyName="";

			info.PrimaryKeyName="codperio";
			info.HumanKeyName="periodstart,".TrimEnd(',');
			info.Alias="perio";
			info.IsDomain = true;
		...
		public static FieldRef FldCodperio { get { return m_fldCodperio; } }
		private static FieldRef m_fldCodperio = new FieldRef("perio", "codperio");

		/// <summary>Field : "" Tipo: "+" Formula:  ""</summary>
		public string ValCodperio
		{
			get { return (string)returnValueField(FldCodperio); }
		...
		private static FieldRef m_fldZzstate = new FieldRef("perio", "zzstate");
        ...
        public static CSGenioAperio search(PersistentSupport sp, string key, User user, string[] fields = null, bool forUpdate = false)
        {
			if (string.IsNullOrEmpty(key))
				return null;

		    CSGenioAperio area = new CSGenioAperio(user, user.CurrentModule);
        ...
        /// <remarks>Persistence operations should not be used on a partially positioned register</remarks>
        public static List<CSGenioAperio> searchList(PersistentSupport sp, User user, CriteriaSet where, string[] fields = null, bool distinct = false, bool noLock = false)
        {
				return sp.searchListWhere<CSGenioAperio>(where, user, fields, distinct, noLock);
        }
        ...
        /// <returns>A list of area records with all fields populated</returns>
        /// <remarks>Persistence operations should not be used on a partially positioned register</remarks>
        public static void searchListAdvancedWhere(PersistentSupport sp, User user, CriteriaSet where, ListingMVC<CSGenioAperio> listing)
        {
			sp.searchListAdvancedWhere<CSGenioAperio>(where, listing);
        }
