...
namespace CSGenio.business
{
	...
	public class CSGenioAstake : DbArea
	{
		...
		public CSGenioAstake(User user, string module)
		{
            this.user = user;
            this.module = module;
			// USE /[MANUAL GQT CONSTRUTOR STAKE]/
		}

		public CSGenioAstake(User user) : this(user, user.CurrentModule)
		{
		}
...
#pragma warning restore CS0168, S1481 // Variable is declared but never used
			...
			Qfield.FieldDescription = "Designação";
			Qfield.FieldSize =  85;
			...
			//- - - - - - - - - - - - - - - - - - -
			Qfield = new Field(info.Alias, "nif", FieldType.TEXT);
			...
			//- - - - - - - - - - - - - - - - - - -
			Qfield = new Field(info.Alias, "logotipo", FieldType.IMAGE);
		...
		private static void InicializaRelacoes(AreaInfo info)
		{
			...
			info.ChildTable[0]= new ChildRelation("pesso", new String[] {"codparte"}, DeleteProc.NA);
		...
		private static AreaInfo InicializaAreaInfo()
		{
			...
			info.QSystem="GQT";
			info.TableName="gqtstake";
			info.ShadowTabName="";
			info.ShadowTabKeyName="";

			info.PrimaryKeyName="codparte";
			info.HumanKeyName="designat,".TrimEnd(',');
			info.Alias="stake";
			info.IsDomain = true;
			info.PersistenceType = PersistenceType.Database;
			info.AreaDesignation="Interested party";
			info.AreaPluralDesignation="Stakeholders";
			info.DescriptionCav="INTERESTED_PARTY36855";
		...
		private static FieldRef m_fldCodparte = new FieldRef("stake", "codparte");
		...
		private static FieldRef m_fldSigla = new FieldRef("stake", "sigla");
		...
		private static FieldRef m_fldTelephon = new FieldRef("stake", "telephon");
		...
		private static FieldRef m_fldZzstate = new FieldRef("stake", "zzstate");
        ...
        public static CSGenioAstake search(PersistentSupport sp, string key, User user, string[] fields = null, bool forUpdate = false)
        {
			if (string.IsNullOrEmpty(key))
				return null;

		    CSGenioAstake area = new CSGenioAstake(user, user.CurrentModule);
        ...
        /// <remarks>Persistence operations should not be used on a partially positioned register</remarks>
        public static List<CSGenioAstake> searchList(PersistentSupport sp, User user, CriteriaSet where, string[] fields = null, bool distinct = false, bool noLock = false)
        {
				return sp.searchListWhere<CSGenioAstake>(where, user, fields, distinct, noLock);
        }
        ...
        /// <returns>A list of area records with all fields populated</returns>
        /// <remarks>Persistence operations should not be used on a partially positioned register</remarks>
        public static void searchListAdvancedWhere(PersistentSupport sp, User user, CriteriaSet where, ListingMVC<CSGenioAstake> listing)
        {
			sp.searchListAdvancedWhere<CSGenioAstake>(where, listing);
        }
