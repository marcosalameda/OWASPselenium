...
namespace CSGenio.business
{
	...
	public class CSGenioAwareh : DbArea
	{
		...
		public CSGenioAwareh(User user, string module)
		{
            this.user = user;
            this.module = module;
			// USE /[MANUAL GQT CONSTRUTOR WAREH]/
		}

		public CSGenioAwareh(User user) : this(user, user.CurrentModule)
		{
		}
...
#pragma warning restore CS0168, S1481 // Variable is declared but never used
			//- - - - - - - - - - - - - - - - - - -
			Qfield = new Field(info.Alias, "codwareh", FieldType.KEY_INT);
		...
		private static FieldRef m_fldCodwareh = new FieldRef("wareh", "codwareh");
		...
		private static FieldRef m_fldWarehcod = new FieldRef("wareh", "warehcod");
		...
		private static FieldRef m_fldShowreco = new FieldRef("wareh", "showreco");
		...
		private static FieldRef m_fldZzstate = new FieldRef("wareh", "zzstate");
        ...
        public static CSGenioAwareh search(PersistentSupport sp, string key, User user, string[] fields = null, bool forUpdate = false)
        {
			if (string.IsNullOrEmpty(key))
				return null;

		    CSGenioAwareh area = new CSGenioAwareh(user, user.CurrentModule);
        ...
        /// <remarks>Persistence operations should not be used on a partially positioned register</remarks>
        public static List<CSGenioAwareh> searchList(PersistentSupport sp, User user, CriteriaSet where, string[] fields = null, bool distinct = false, bool noLock = false)
        {
				return sp.searchListWhere<CSGenioAwareh>(where, user, fields, distinct, noLock);
        }
        ...
        /// <returns>A list of area records with all fields populated</returns>
        /// <remarks>Persistence operations should not be used on a partially positioned register</remarks>
        public static void searchListAdvancedWhere(PersistentSupport sp, User user, CriteriaSet where, ListingMVC<CSGenioAwareh> listing)
        {
			sp.searchListAdvancedWhere<CSGenioAwareh>(where, listing);
        }
