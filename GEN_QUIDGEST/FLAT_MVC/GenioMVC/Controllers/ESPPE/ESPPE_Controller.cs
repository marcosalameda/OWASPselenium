...
// USE /[MANUAL GQT MANUAL_CONTROLLER ESPPE]/
        ...
        public JsonResult ReloadDBEdit(string Identifier, IDictionary<string, object> Values)
        {
            ...
            try { row = Models.Esppe.Find(navigation.GetStrValue("esppe")); }
            catch (Exception)
            {
					...
					case "ESPPE___PESSONAME____":	// Field (DB)
                        {
						    ...
						    var model = new Esppe_ViewModel(navigation) { editable = false };
        ...
        public JsonResult GetDependants(string Identifier, string Selected, IDictionary<string, object> Limits)
        {
            ...
            try
            {
				...
				{
					case "ESPPE___PESSONAME____":	// Field (DB)
						values = Esppe_ViewModel.GetDependant_EsppeTablePessoName(Selected, navigation);
						break;
					case "ESPPE___SPECIESPECIAL":	// Field (DB)
						values = Esppe_ViewModel.GetDependant_EsppeTableSpeciEspecial(Selected, navigation);
						break;
					default: break;
				}
		...
		[AuthorizeForUsers]
        [ActionSessionState(System.Web.SessionState.SessionStateBehavior.ReadOnly)]
        public JsonResult RecalculateFormulas_Esppe(Esppe_ViewModel form_data)
        {
            return GenericRecalculateFormulas(form_data, "esppe",
                (primaryKey) => Models.Esppe.Find(primaryKey, "FESPPE"),
