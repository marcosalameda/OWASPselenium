...
// USE /[MANUAL GQT MANUAL_CONTROLLER EVCAT]/
        ...
        public JsonResult ReloadDBEdit(string Identifier, IDictionary<string, object> Values)
        {
            ...
            try { row = Models.Evcat.Find(navigation.GetStrValue("evcat")); }
            catch (Exception)
            {
					...
					case "EVCAT___PESSONAME____":	// Field (DB)
                        {
						    ...
						    var model = new Evcat_ViewModel(navigation) { editable = false };
        ...
        public JsonResult GetDependants(string Identifier, string Selected, IDictionary<string, object> Limits)
        {
            ...
            try
            {
				...
				{
					case "EVCAT___PESSONAME____":	// Field (DB)
						values = Evcat_ViewModel.GetDependant_EvcatTablePessoName(Selected, navigation);
						break;
					case "EVCAT___CATE1CATEGORY":	// Field (DB)
						values = Evcat_ViewModel.GetDependant_EvcatTableCate1Category(Selected, navigation);
						break;
					default: break;
				}
		...
		[AuthorizeForUsers]
        [ActionSessionState(System.Web.SessionState.SessionStateBehavior.ReadOnly)]
        public JsonResult RecalculateFormulas_Evcat(Evcat_ViewModel form_data)
        {
            return GenericRecalculateFormulas(form_data, "evcat",
                (primaryKey) => Models.Evcat.Find(primaryKey, "FEVCAT"),
