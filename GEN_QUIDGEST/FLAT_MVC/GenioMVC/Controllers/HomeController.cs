...
// USE /[MANUAL GQT HOME_CONTROLLER_INDEX]/
        ...
        public ActionResult GetProfileImg()
        {
            ...
            GlobalFunctions funcoes = new GlobalFunctions(user, user.CurrentModule, sp);

            try
            {
                // Gets image from user profile
                List<object> userProfile = funcoes.GetUserProfile();
                Resource rec = (Resource)userProfile[1];

                if (rec != null)
                {
                    byte[] userImage = rec.GetContent(sp);
                    return new FileContentResult(userImage, "image/jpeg");
                }
                else
                {
                    // Tries to get an image for the module
                    var module = user.CurrentModule;
                    var menuModule = GenioMVC.Helpers.Menus.Menus.AvailableModules(user).FirstOrDefault(m => m.ID.Equals(user.CurrentModule));
                    string image = null;
                    if (module == "Public")
                    {
                        menuModule = GenioMVC.Helpers.Menus.Menus.AvailableModules(GenioMVC.Models.Navigation.UserContext.Current.User).FirstOrDefault();
                    }
                    if (menuModule != null)
                    {
                        image = menuModule.Image;
                    }
                    if (!string.IsNullOrEmpty(image))
                        return File(HttpContext.Server.MapPath(image), "image/png");
                    else
                        return View("Error");
                }
            }
