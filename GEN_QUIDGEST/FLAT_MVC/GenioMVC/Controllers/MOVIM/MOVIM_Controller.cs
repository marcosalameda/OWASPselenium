...
// USE /[MANUAL GQT MANUAL_CONTROLLER MOVIM]/
        ...
        public JsonResult ReloadDBEdit(string Identifier, IDictionary<string, object> Values)
        {
            ...
            try { row = Models.Movim.Find(navigation.GetStrValue("movim")); }
            catch (Exception)
            {
					...
					case "MOVIM___EQUIPREGISTNR":	// Field (DB)
                        {
						    ...
						    var model = new Movim_ViewModel(navigation) { editable = false };
        ...
        public JsonResult GetDependants(string Identifier, string Selected, IDictionary<string, object> Limits)
        {
            ...
            try
            {
				...
				{
					case "MOVIM___EQUIPREGISTNR":	// Field (DB)
						values = Movim_ViewModel.GetDependant_MovimTableEquipRegistnr(Selected, navigation);
						break;
					case "MOVIM___ROOMSROOMNR__":	// Field (DB)
						values = Movim_ViewModel.GetDependant_MovimTableRoomsRoomnr(Selected, navigation);
						break;
					default: break;
				}
		...
		[AuthorizeForUsers]
        [ActionSessionState(System.Web.SessionState.SessionStateBehavior.ReadOnly)]
        public JsonResult RecalculateFormulas_Movim(Movim_ViewModel form_data)
        {
            return GenericRecalculateFormulas(form_data, "movim",
                (primaryKey) => Models.Movim.Find(primaryKey, "FMOVIM"),
