...
// USE /[MANUAL GQT MANUAL_CONTROLLER PWORG]/
        ...
        public JsonResult ReloadDBEdit(string Identifier, IDictionary<string, object> Values)
        {
            ...
            try { row = Models.Pworg.Find(navigation.GetStrValue("pworg")); }
            catch (Exception)
            {
					...
					case "PWORG___PSW__NOME____":	// Field (DB)
                        {
						    ...
						    var model = new Pworg_ViewModel(navigation) { editable = false };
        ...
        public JsonResult GetDependants(string Identifier, string Selected, IDictionary<string, object> Limits)
        {
            ...
            try
            {
				...
				{
					case "PWORG___PSW__NOME____":	// Field (DB)
						values = Pworg_ViewModel.GetDependant_PworgTablePswNome(Selected, navigation);
						break;
					case "PWORG___ORGANORGANIZA":	// Field (DB)
						values = Pworg_ViewModel.GetDependant_PworgTableOrganOrganiza(Selected, navigation);
						break;
					default: break;
				}
		...
		[AuthorizeForUsers]
        [ActionSessionState(System.Web.SessionState.SessionStateBehavior.ReadOnly)]
        public JsonResult RecalculateFormulas_Pworg(Pworg_ViewModel form_data)
        {
            return GenericRecalculateFormulas(form_data, "pworg",
                (primaryKey) => Models.Pworg.Find(primaryKey, "FPWORG"),
