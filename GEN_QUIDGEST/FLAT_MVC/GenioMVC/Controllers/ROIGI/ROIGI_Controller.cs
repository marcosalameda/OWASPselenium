...
// USE /[MANUAL GQT MANUAL_CONTROLLER ROIGI]/
        ...
        public JsonResult ReloadDBEdit(string Identifier, IDictionary<string, object> Values)
        {
            ...
            try { row = Models.Roigi.Find(navigation.GetStrValue("roigi")); }
            catch (Exception)
            {
					...
					case "ROIGI___ROGL1TITLE___":	// Field (DB)
                        {
						    ...
						    var model = new Roigi_ViewModel(navigation) { editable = false };
        ...
        public JsonResult GetDependants(string Identifier, string Selected, IDictionary<string, object> Limits)
        {
            ...
            try
            {
				...
				{
					case "ROIGI___ROGL1TITLE___":	// Field (DB)
						values = Roigi_ViewModel.GetDependant_RoigiTableRogl1Title(Selected, navigation);
						break;
					default: break;
				}
		...
		[AuthorizeForUsers]
        [ActionSessionState(System.Web.SessionState.SessionStateBehavior.ReadOnly)]
        public JsonResult RecalculateFormulas_Roigi(Roigi_ViewModel form_data)
        {
            return GenericRecalculateFormulas(form_data, "roigi",
                (primaryKey) => Models.Roigi.Find(primaryKey, "FROIGI"),
