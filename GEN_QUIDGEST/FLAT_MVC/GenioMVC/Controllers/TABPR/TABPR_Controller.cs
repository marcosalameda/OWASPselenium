...
// USE /[MANUAL GQT MANUAL_CONTROLLER TABPR]/
        ...
        public JsonResult ReloadDBEdit(string Identifier, IDictionary<string, object> Values)
        {
            ...
            try { row = Models.Tabpr.Find(navigation.GetStrValue("tabpr")); }
            catch (Exception)
            {
					...
					case "TABPR___TPEQUTIPOEQUI":	// Field (DB)
                        {
						    ...
						    var model = new Tabpr_ViewModel(navigation) { editable = false };
        ...
        public JsonResult GetDependants(string Identifier, string Selected, IDictionary<string, object> Limits)
        {
            ...
            try
            {
				...
				{
					case "TABPR___TPEQUTIPOEQUI":	// Field (DB)
						values = Tabpr_ViewModel.GetDependant_TabprTableTpequTipoequi(Selected, navigation);
						break;
					default: break;
				}
		...
		[AuthorizeForUsers]
        [ActionSessionState(System.Web.SessionState.SessionStateBehavior.ReadOnly)]
        public JsonResult RecalculateFormulas_Tabpr(Tabpr_ViewModel form_data)
        {
            return GenericRecalculateFormulas(form_data, "tabpr",
                (primaryKey) => Models.Tabpr.Find(primaryKey, "FTABPR"),
