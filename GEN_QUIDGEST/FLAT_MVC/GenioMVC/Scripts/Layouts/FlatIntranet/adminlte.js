...
var PushMenu = function ($) {
  ...
  var PushMenu = function () {
    ...
    PushMenu.prototype.show = function show() {
      ...
      window.dispatchEvent(new CustomEvent(Event.SHOWN));
    };
    ...
    PushMenu.prototype.collapse = function collapse() {
      ...
      window.dispatchEvent(new CustomEvent(Event.COLLAPSED));
    };
    ...
    PushMenu.prototype.toggle = function toggle() {
      ...
      if ($(window).width() >= this._options.screenCollapseSize) {
        ...
        if ($(window).width() >= this._options.screenCollapseSize) {
            ...
            if (isShown) {
                this.collapse();
                menuMode = "sidebar-collapse";
            } else {
                this.show();
                //menuMode = "sidebar-open";
            }
            $.post(quidgestGlobals.UrlAction.sidebarMode, { mode: menuMode });
        }
        else {
            if (isShown) {
                this.collapse();
...
var Treeview = function ($) {
  ...
  var Selector = {
    ...
    PUSH_BUTTON: '[data-identifier="sidemenu_push_button"]',
    MAIN_CONTAINER: '[data-identifier="sidemenu_container"]'
  };
  ...
  var Treeview = function () {
    ...
    Treeview.prototype.init = function init() {
      ...
      var curQLS = QLocalStorage.getLocalStorage('sidebar_selectedMenu');
      if (!$.isEmptyObject(curQLS)) {            
          $('li#' + curQLS).addClass("active menu-open");
          $('ul#' + curQLS).css("display", "block");

          $('#' + curQLS).parentsUntil('ul.nav-sidebar', 'li').addClass("menu-open");
          $('#' + curQLS).parentsUntil('ul.nav-sidebar', 'ul').css("display", "block");
      }

      var curQLSelect = QLocalStorage.getLocalStorage('sidebar_selectedItemMenu');
      if (!$.isEmptyObject(curQLSelect)) {
          var menuItem = $('#' + curQLSelect),
              parentDiv = menuItem.closest(Selector.MAIN_CONTAINER);

          menuItem.addClass("selected");

          // Scroll to the last selected menu
          try {
            if (!QUtils.isElementInView(parentDiv, false)) {
                $(Selector.PUSH_BUTTON).one('click', { parentDiv, menuItem }, function (event) {
                    var parentDiv = event.data.parentDiv,
                        menuItem = event.data.menuItem;
                    parentDiv.scrollTop(menuItem.position().top);
                });
            } else {
                parentDiv.scrollTop(menuItem.position().top);
            }
        }
        catch (e) { /* MH - Errors are irrelevant. It is not a feature that prevents the normal operation of the system.
                  The try catch was placed due to a variation of combinations of menu display options. */ }
      }
    };
    ...
    Treeview.prototype.toggle = function toggle(event) {
      var $relativeTarget = $(event.currentTarget);
        ...
        if (!treeviewMenu.is(Selector.TREEVIEW_MENU)) {
            //$(event.currentTarget.parent).addClass('activo teste');
            var parentLi = $relativeTarget.parents(Selector.LI).first();
            if (parentLi) {
                QLocalStorage.setLocalStorage('sidebar_selectedItemMenu', parentLi.attr("id"));                
            }            
        return;
      }
      ...
      if (isOpen) {
          ...
          QLocalStorage.setLocalStorage('sidebar_selectedMenu', parentLi.parent().attr("id"));
      } else {
          this.expand($(treeviewMenu), parentLi);
          QLocalStorage.setLocalStorage('sidebar_selectedMenu', parentLi.attr("id"));
      }
    };
    ...
    Treeview.prototype._setupListeners = function _setupListeners() {
      ...
      $(document).on('click', this._config.trigger, function (event) {
          _this3.toggle(event);
          //$(this._config.trigger).addClass('menuactiv');
      });
    };
