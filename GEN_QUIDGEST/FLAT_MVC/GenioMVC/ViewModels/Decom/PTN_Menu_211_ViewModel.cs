...
FieldRef[] fields = new FieldRef[] { CSGenioAdecom.FldCoddeco, CSGenioAdecom.FldZzstate, CSGenioAdecom.FldDtdeco, CSGenioAdecom.FldDecomnr, CSGenioAdecom.FldNote, CSGenioAdecom.FldCreatdat, CSGenioAdecom.FldCreatope, CSGenioAdecom.FldChngdate, CSGenioAdecom.FldOperchng };
        ...
        public PTN_Menu_211_ViewModel(NavigationContext currentNavigation) : base(currentNavigation)
        {
...
FieldRef[] fields = new FieldRef[] { CSGenioAdecom.FldCoddeco, CSGenioAdecom.FldZzstate, CSGenioAdecom.FldDtdeco, CSGenioAdecom.FldDecomnr, CSGenioAdecom.FldNote, CSGenioAdecom.FldCreatdat, CSGenioAdecom.FldCreatope, CSGenioAdecom.FldChngdate, CSGenioAdecom.FldOperchng };
			...
			CriteriaSet ptn_menu_211Conds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
            tableReload &= hasAllRequiredLimits;
			
// USE /[MANUAL PTN OVERRQ 211]/

            // This will happen in case there is an error
            if(ptn_menu_211Conds == null)
                return;
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAdecom>(false, ptn_menu_211Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "ML211", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL PTN OVERRQLSTEXP 211]/

                conditions = ptn_menu_211Conds;
                return;
			}
...
// USE /[MANUAL PTN OVERRQLIST 211]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = UserContext.Current.PersistentSupport.getPagingPos(CSGenioAdecom.GetInformation(), QMVC_POS_RECORD, sorts, ptn_menu_211Conds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioAdecom> listing = Models.ModelBase.Where<CSGenioAdecom>(false, ptn_menu_211Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "ML211", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
