...
{
    public class Ldsai_OudocValNrdocsda_ViewModel : ListViewModel
    {
        /// <summary>
        /// Gets or sets the object that represents the table and its elements.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("Table")]
        public TablePartial<GenioMVC.Models.Oudoc> Menu { get; set; }

        /// <inheritdoc/>
        public override string TableAlias { get => "oudoc"; }

        /// <inheritdoc/>
        public override string Uuid { get => "Ldsai_OudocValNrdocsda"; }
        ...
        public Ldsai_OudocValNrdocsda_ViewModel(NavigationContext currentNavigation) : base(currentNavigation)
        {
        ...
        public void LoadToExport(out ListingMVC<CSGenioAoudoc> listing, out CriteriaSet conditions, out List<Exports.QColumn> columns, NameValueCollection requestValues, bool ajaxRequest = false)
        {
            ...
            if (ajaxRequest && userColumns!=null)
            {
                ...
                foreach (CSGenioAlstcol column in userColumns)
                {
                    ...
                    string areabase = column.ValTabela.ToLower() != "oudoc" ? CultureInfo.InvariantCulture.TextInfo.ToTitleCase(column.ValTabela) + "." : "";
		...
		public override CriteriaSet BuildCriteriaSet(NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			if(Menu == null)
				Menu = new TablePartial<GenioMVC.Models.Oudoc>();
		...
		public void Load(int numberListItems, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAoudoc> Qlisting, ref CriteriaSet conditions)
		{
			...
			if (ajaxRequest)
				this.Navigation.SetValue("requestValues" + "Ldsai_OudocValNrdocsda", requestValues);
			else if (!ajaxRequest && this.Navigation.CheckKey("requestValues" + "Ldsai_OudocValNrdocsda"))
				requestValues = this.Navigation.GetValue<NameValueCollection>("requestValues" + "Ldsai_OudocValNrdocsda");
			...
			var pageNumber = (ajaxRequest && !String.IsNullOrEmpty(requestValues["pLdsai_OudocValNrdocsda"])) ? int.Parse(requestValues["pLdsai_OudocValNrdocsda"]) : 1;

			// Added to avoid 0 or -1 pages when setting number of records to -1 to disable pagination
			if (pageNumber < 1)
				pageNumber = 1;

			List<ColumnSort> sorts = GetRequestSorts(this.Menu, "sLdsai_OudocValNrdocsda", "dLdsai_OudocValNrdocsda", requestValues, "oudoc", allSortOrders);
...
FieldRef[] fields = new FieldRef[] { CSGenioAoudoc.FldCoddocsd, CSGenioAoudoc.FldZzstate, CSGenioAoudoc.FldNrdocsda, CSGenioAoudoc.FldDtdocsda, CSGenioAoudoc.FldTitle };
			...
			//Tooltip for EPHs affecting this viewmodel list
			{
				...
				CSGenioAoudoc model_limit_area = new CSGenioAoudoc(UserContext.Current.User);
			...
			CriteriaSet ldsai___oudocnrdocsdaConds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
            tableReload &= hasAllRequiredLimits;

// USE /[MANUAL GQT OVERRQ LDSAI_OUDOCNRDOCSDA]/

            // This will happen in case there is an error
            if(ldsai___oudocnrdocsdaConds == null)
                return;
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAoudoc>(false, ldsai___oudocnrdocsdaConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_LDSAI___OUDOCNRDOCSDA", true, firstVisibleColumn: firstVisibleColumn);
...
// USE /[MANUAL GQT OVERRQLIST LDSAI_OUDOCNRDOCSDA]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = UserContext.Current.PersistentSupport.getPagingPos(CSGenioAoudoc.GetInformation(), QMVC_POS_RECORD, sorts, ldsai___oudocnrdocsdaConds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioAoudoc> listing = Models.ModelBase.Where<CSGenioAoudoc>(false, ldsai___oudocnrdocsdaConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_LDSAI___OUDOCNRDOCSDA", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
        ...
        {
            var Elements = new List<Models.Oudoc>();
            int i = 0;
        ...
        /// to a Models.Oudoc object.
        /// </summary>
        /// <param name="row">The row.</param>
        private Models.Oudoc MapLdsai_OudocValNrdocsda(CSGenioAoudoc row)
        {
            var model = new Models.Oudoc(true, _fieldsToSerialize);
            if (row == null)
                return model;
