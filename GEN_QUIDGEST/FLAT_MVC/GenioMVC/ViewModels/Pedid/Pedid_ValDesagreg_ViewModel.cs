...
FieldRef[] fields = new FieldRef[] { CSGenioAlnhde.FldCodlnhde, CSGenioAlnhde.FldZzstate, CSGenioAlnhde.FldOrdem, CSGenioAlnhde.FldCodtpequ, CSGenioAtpeq1.FldCodtpequ, CSGenioAtpeq1.FldTipoequi, CSGenioAlnhde.FldQuantida };
			...
			CriteriaSet pedid___pseuddesagregConds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
            tableReload &= hasAllRequiredLimits;

// USE /[MANUAL GQT OVERRQ PEDID_PSEUDDESAGREG]/

            // This will happen in case there is an error
            if(pedid___pseuddesagregConds == null)
                return;
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAlnhde>(false, pedid___pseuddesagregConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_PEDID___PSEUDDESAGREG", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL GQT OVERRQLSTEXP PEDID_PSEUDDESAGREG]/

                conditions = pedid___pseuddesagregConds;
                return;
			}
...
// USE /[MANUAL GQT OVERRQLIST PEDID_PSEUDDESAGREG]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = UserContext.Current.PersistentSupport.getPagingPos(CSGenioAlnhde.GetInformation(), QMVC_POS_RECORD, sorts, pedid___pseuddesagregConds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioAlnhde> listing = Models.ModelBase.Where<CSGenioAlnhde>(false, pedid___pseuddesagregConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_PEDID___PSEUDDESAGREG", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
