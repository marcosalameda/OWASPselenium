...
{
    public class Pesso_ValEspecitl_ViewModel : ListViewModel
    {
        /// <summary>
        /// Gets or sets the object that represents the table and its elements.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("Table")]
        public TablePartial<GenioMVC.Models.Esppe> Menu { get; set; }

        /// <inheritdoc/>
        public override string TableAlias { get => "esppe"; }

        /// <inheritdoc/>
        public override string Uuid { get => "Pesso_ValEspecitl"; }
        ...
        public Pesso_ValEspecitl_ViewModel(NavigationContext currentNavigation) : base(currentNavigation)
        {
        ...
        public void LoadToExport(out ListingMVC<CSGenioAesppe> listing, out CriteriaSet conditions, out List<Exports.QColumn> columns, NameValueCollection requestValues, bool ajaxRequest = false)
        {
            ...
            if (ajaxRequest && userColumns!=null)
            {
                ...
                foreach (CSGenioAlstcol column in userColumns)
                {
                    ...
                    string areabase = column.ValTabela.ToLower() != "esppe" ? CultureInfo.InvariantCulture.TextInfo.ToTitleCase(column.ValTabela) + "." : "";
		...
		public override CriteriaSet BuildCriteriaSet(NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			if(Menu == null)
				Menu = new TablePartial<GenioMVC.Models.Esppe>();
			Menu.SetFilters(bool.Parse(requestValues["ValEspecitl_tableFilters"] ?? "false"), false);
		...
		public void Load(int numberListItems, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAesppe> Qlisting, ref CriteriaSet conditions)
		{
			...
			if (ajaxRequest)
				this.Navigation.SetValue("requestValues" + "Pesso_ValEspecitl", requestValues);
			else if (!ajaxRequest && this.Navigation.CheckKey("requestValues" + "Pesso_ValEspecitl"))
				requestValues = this.Navigation.GetValue<NameValueCollection>("requestValues" + "Pesso_ValEspecitl");
			...
			var pageNumber = (ajaxRequest && !String.IsNullOrEmpty(requestValues["pValEspecitl"])) ? int.Parse(requestValues["pValEspecitl"]) : 1;

			// Added to avoid 0 or -1 pages when setting number of records to -1 to disable pagination
			if (pageNumber < 1)
				pageNumber = 1;

			List<ColumnSort> sorts = GetRequestSorts(this.Menu, "sValEspecitl", "dValEspecitl", requestValues, "esppe", allSortOrders);
...
FieldRef[] fields = new FieldRef[] { CSGenioAesppe.FldCodesppe, CSGenioAesppe.FldZzstate, CSGenioAesppe.FldCodespec, CSGenioAspeci.FldCodespec, CSGenioAspeci.FldEspecial, CSGenioAspeci.FldAreatecn };
			...
			//Tooltip for EPHs affecting this viewmodel list
			{
				...
				CSGenioAesppe model_limit_area = new CSGenioAesppe(UserContext.Current.User);
			...
			CriteriaSet pesso___pseudespecitlConds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
            tableReload &= hasAllRequiredLimits;

// USE /[MANUAL GQT OVERRQ PESSO_PSEUDESPECITL]/

            // This will happen in case there is an error
            if(pesso___pseudespecitlConds == null)
                return;
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAesppe>(false, pesso___pseudespecitlConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_PESSO___PSEUDESPECITL", true, firstVisibleColumn: firstVisibleColumn);
...
// USE /[MANUAL GQT OVERRQLIST PESSO_PSEUDESPECITL]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = UserContext.Current.PersistentSupport.getPagingPos(CSGenioAesppe.GetInformation(), QMVC_POS_RECORD, sorts, pesso___pseudespecitlConds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioAesppe> listing = Models.ModelBase.Where<CSGenioAesppe>(false, pesso___pseudespecitlConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_PESSO___PSEUDESPECITL", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
        ...
        private List<Models.Esppe> MapPesso_ValEspecitl(ListingMVC<CSGenioAesppe> Qlisting)
        {
            var Elements = new List<Models.Esppe>();
            int i = 0;
        ...
        /// to a Models.Esppe object.
        /// </summary>
        /// <param name="row">The row.</param>
        private Models.Esppe MapPesso_ValEspecitl(CSGenioAesppe row)
        {
            var model = new Models.Esppe(true, _fieldsToSerialize);
            if (row == null)
                return model;
