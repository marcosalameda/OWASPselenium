...
namespace GenioMVC.ViewModels.Propr
{
    public class Proprall_RegioValRegiao_ViewModel : ListViewModel
    {
        ...
        public Proprall_RegioValRegiao_ViewModel(NavigationContext currentNavigation) : base(currentNavigation)
        {
		...
		public override CriteriaSet BuildCriteriaSet(NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			Menu.SetFilters(bool.Parse(requestValues["Proprall_RegioValRegiao_tableFilters"] ?? "false"), false);
		...
		public void Load(int numberListItems, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAregio> Qlisting, ref CriteriaSet conditions)
		{
			...
			if (ajaxRequest)
				this.Navigation.SetValue("requestValues" + "Proprall_RegioValRegiao", requestValues);
			else if (!ajaxRequest && this.Navigation.CheckKey("requestValues" + "Proprall_RegioValRegiao"))
				requestValues = this.Navigation.GetValue<NameValueCollection>("requestValues" + "Proprall_RegioValRegiao");
			...
			var pageNumber = (ajaxRequest && !String.IsNullOrEmpty(requestValues["pProprall_RegioValRegiao"])) ? int.Parse(requestValues["pProprall_RegioValRegiao"]) : 1;

			// Added to avoid 0 or -1 pages when setting number of records to -1 to disable pagination
			if (pageNumber < 1)
				pageNumber = 1;

			List<ColumnSort> sorts = GetRequestSorts(this.Menu, "sProprall_RegioValRegiao", "dProprall_RegioValRegiao", requestValues, "regio", allSortOrders);
...
FieldRef[] fields = new FieldRef[] { CSGenioAregio.FldCodregia, CSGenioAregio.FldZzstate, CSGenioAregio.FldRegiao };
			...
			CriteriaSet proprallregioregiao__Conds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
            tableReload &= hasAllRequiredLimits;

// USE /[MANUAL GQT OVERRQ PROPRALL_REGIOREGIAO]/

            // This will happen in case there is an error
            if(proprallregioregiao__Conds == null)
                return;
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAregio>(false, proprallregioregiao__Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_PROPRALLREGIOREGIAO__", true, firstVisibleColumn: firstVisibleColumn);
...
// USE /[MANUAL GQT OVERRQLIST PROPRALL_REGIOREGIAO]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = UserContext.Current.PersistentSupport.getPagingPos(CSGenioAregio.GetInformation(), QMVC_POS_RECORD, sorts, proprallregioregiao__Conds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioAregio> listing = Models.ModelBase.Where<CSGenioAregio>(false, proprallregioregiao__Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_PROPRALLREGIOREGIAO__", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
        ...
        private List<Models.Regio> MapProprall_RegioValRegiao(ListingMVC<CSGenioAregio> Qlisting)
        {
        ...
        private Models.Regio MapProprall_RegioValRegiao(CSGenioAregio row)
        {
