...
{
    public class Regia_on_ValImoveisl_ViewModel : ListViewModel
    {
        /// <summary>
        ...
        /// <inheritdoc/>
        public override string Uuid { get => "Regia_on_ValImoveisl"; }
        ...
        public Regia_on_ValImoveisl_ViewModel(NavigationContext currentNavigation) : base(currentNavigation)
        {
		...
		public void Load(int numberListItems, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioApropr> Qlisting, ref CriteriaSet conditions)
		{
			...
			if (ajaxRequest)
				this.Navigation.SetValue("requestValues" + "Regia_on_ValImoveisl", requestValues);
			else if (!ajaxRequest && this.Navigation.CheckKey("requestValues" + "Regia_on_ValImoveisl"))
				requestValues = this.Navigation.GetValue<NameValueCollection>("requestValues" + "Regia_on_ValImoveisl");
...
FieldRef[] fields = new FieldRef[] { CSGenioApropr.FldCodpropr, CSGenioApropr.FldZzstate, CSGenioApropr.FldName, CSGenioApropr.FldPrecoest, CSGenioApropr.FldPhotogra, CSGenioApropr.FldDescript, CSGenioApropr.FldCoordgeo, CSGenioApropr.FldCodpais1, CSGenioApais1.FldCodcntry, CSGenioApais1.FldCountry, CSGenioApropr.FldCodcntry, CSGenioAcntry.FldCodcntry, CSGenioAcntry.FldCountry };
			...
			CriteriaSet regia_onpseudimoveislConds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
            tableReload &= hasAllRequiredLimits;

// USE /[MANUAL GQT OVERRQ REGIA_ON_PSEUDIMOVEISL]/

            // This will happen in case there is an error
            if(regia_onpseudimoveislConds == null)
                return;
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioApropr>(false, regia_onpseudimoveislConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_REGIA_ONPSEUDIMOVEISL", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL GQT OVERRQLSTEXP REGIA_ON_PSEUDIMOVEISL]/

                conditions = regia_onpseudimoveislConds;
                return;
			}
...
// USE /[MANUAL GQT OVERRQLIST REGIA_ON_PSEUDIMOVEISL]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = UserContext.Current.PersistentSupport.getPagingPos(CSGenioApropr.GetInformation(), QMVC_POS_RECORD, sorts, regia_onpseudimoveislConds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioApropr> listing = Models.ModelBase.Where<CSGenioApropr>(false, regia_onpseudimoveislConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_REGIA_ONPSEUDIMOVEISL", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
        ...
        private Models.Propr MapRegia_on_ValImoveisl(CSGenioApropr row)
        {
