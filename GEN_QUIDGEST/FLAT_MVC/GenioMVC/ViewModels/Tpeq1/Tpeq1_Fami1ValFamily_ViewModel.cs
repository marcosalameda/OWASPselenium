...
namespace GenioMVC.ViewModels.Tpeq1
{
    public class Tpeq1_Fami1ValFamily_ViewModel : ListViewModel
    {
        ...
        public Tpeq1_Fami1ValFamily_ViewModel(NavigationContext currentNavigation) : base(currentNavigation)
        {
            ValCodtpequ = currentNavigation.CurrentLevel.GetEntry("tpeq1")?.ToString();
        }
...
FieldRef[] fields = new FieldRef[] { CSGenioAfami1.FldCodfamil, CSGenioAfami1.FldZzstate, CSGenioAfami1.FldFamily };
			...
			CriteriaSet tpeq1___fami1family__Conds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
            tableReload &= hasAllRequiredLimits;

// USE /[MANUAL GQT OVERRQ TPEQ1_FAMI1FAMILY]/

            // This will happen in case there is an error
            if(tpeq1___fami1family__Conds == null)
                return;
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAfami1>(false, tpeq1___fami1family__Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_TPEQ1___FAMI1FAMILY__", true, firstVisibleColumn: firstVisibleColumn);
...
// USE /[MANUAL GQT OVERRQLIST TPEQ1_FAMI1FAMILY]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = UserContext.Current.PersistentSupport.getPagingPos(CSGenioAfami1.GetInformation(), QMVC_POS_RECORD, sorts, tpeq1___fami1family__Conds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioAfami1> listing = Models.ModelBase.Where<CSGenioAfami1>(false, tpeq1___fami1family__Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_TPEQ1___FAMI1FAMILY__", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
