...
namespace GenioMVC.ViewModels.Tpequ
{
    public class Tpequ_ValComponen_ViewModel : ListViewModel
    {
        ...
        public Tpequ_ValComponen_ViewModel(NavigationContext currentNavigation) : base(currentNavigation)
        {
		...
		public void Load(int numberListItems, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAcmpki> Qlisting, ref CriteriaSet conditions)
		{
			...
			if (ajaxRequest)
				this.Navigation.SetValue("requestValues" + "Tpequ_ValComponen", requestValues);
			else if (!ajaxRequest && this.Navigation.CheckKey("requestValues" + "Tpequ_ValComponen"))
				requestValues = this.Navigation.GetValue<NameValueCollection>("requestValues" + "Tpequ_ValComponen");
			...
			var pageNumber = (ajaxRequest && !String.IsNullOrEmpty(requestValues["pValComponen"])) ? int.Parse(requestValues["pValComponen"]) : 1;

			// Added to avoid 0 or -1 pages when setting number of records to -1 to disable pagination
			if (pageNumber < 1)
				pageNumber = 1;

			List<ColumnSort> sorts = GetRequestSorts(this.Menu, "sValComponen", "dValComponen", requestValues, "cmpki", allSortOrders);
...
FieldRef[] fields = new FieldRef[] { CSGenioAcmpki.FldCodcmpki, CSGenioAcmpki.FldZzstate, CSGenioAcmpki.FldOrder, CSGenioAcmpki.FldCodtpeq1, CSGenioAtpeq1.FldCodtpequ, CSGenioAtpeq1.FldTipoequi, CSGenioAcmpki.FldQuantida, CSGenioAcmpki.FldDescript, CSGenioAcmpki.FldCode, CSGenioAcmpki.FldUrl };
			...
			CriteriaSet tpequ___pseudcomponenConds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
            tableReload &= hasAllRequiredLimits;

// USE /[MANUAL GQT OVERRQ TPEQU_PSEUDCOMPONEN]/

            // This will happen in case there is an error
            if(tpequ___pseudcomponenConds == null)
                return;
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAcmpki>(false, tpequ___pseudcomponenConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_TPEQU___PSEUDCOMPONEN", true, firstVisibleColumn: firstVisibleColumn);
...
// USE /[MANUAL GQT OVERRQLIST TPEQU_PSEUDCOMPONEN]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = UserContext.Current.PersistentSupport.getPagingPos(CSGenioAcmpki.GetInformation(), QMVC_POS_RECORD, sorts, tpequ___pseudcomponenConds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioAcmpki> listing = Models.ModelBase.Where<CSGenioAcmpki>(false, tpequ___pseudcomponenConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_TPEQU___PSEUDCOMPONEN", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
        ...
        private Models.Cmpki MapTpequ_ValComponen(CSGenioAcmpki row)
        {
