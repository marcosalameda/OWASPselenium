...
namespace GenioMVC.ViewModels.Tpequ
{
    public class Tpequ_ValUnico_ViewModel : ListViewModel
    {
		...
		public void Load(int numberListItems, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAtpequ> Qlisting, ref CriteriaSet conditions)
		{
			...
			if (ajaxRequest)
				this.Navigation.SetValue("requestValues" + "Tpequ_ValUnico", requestValues);
			else if (!ajaxRequest && this.Navigation.CheckKey("requestValues" + "Tpequ_ValUnico"))
				requestValues = this.Navigation.GetValue<NameValueCollection>("requestValues" + "Tpequ_ValUnico");
			...
			var pageNumber = (ajaxRequest && !String.IsNullOrEmpty(requestValues["pValUnico"])) ? int.Parse(requestValues["pValUnico"]) : 1;

			// Added to avoid 0 or -1 pages when setting number of records to -1 to disable pagination
			if (pageNumber < 1)
				pageNumber = 1;

			List<ColumnSort> sorts = GetRequestSorts(this.Menu, "sValUnico", "dValUnico", requestValues, "tpequ", allSortOrders);
...
FieldRef[] fields = new FieldRef[] { CSGenioAtpequ.FldCodtpequ, CSGenioAtpequ.FldZzstate };
			...
			CriteriaSet tpequ___pseudunico___Conds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
            tableReload &= hasAllRequiredLimits;
			
// USE /[MANUAL GQT OVERRQ TPEQU_PSEUDUNICO]/

            // This will happen in case there is an error
            if(tpequ___pseudunico___Conds == null)
                return;
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAtpequ>(false, tpequ___pseudunico___Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_TPEQU___PSEUDUNICO___", true, firstVisibleColumn: firstVisibleColumn);
...
// USE /[MANUAL GQT OVERRQLIST TPEQU_PSEUDUNICO]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = UserContext.Current.PersistentSupport.getPagingPos(CSGenioAtpequ.GetInformation(), QMVC_POS_RECORD, sorts, tpequ___pseudunico___Conds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioAtpequ> listing = Models.ModelBase.Where<CSGenioAtpequ>(false, tpequ___pseudunico___Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_TPEQU___PSEUDUNICO___", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
        ...
        private Models.Tpequ MapTpequ_ValUnico(CSGenioAtpequ row)
        {
