...
#pragma warning restore 618
	...
	manualQueries.Add("EQUIPORDEVOLV",new Q_equipordevolv());
	manualQueries.Add("CONTACORRENTE",new Q_contacorrente());
...
notifications.Add("NOTIF_1_EQUIPORDEVOLV",new Q_NOTIF_1_EQUIPORDEVOLV());

notifications.Add("NOTIF_2_DISPATCHALERT",new Q_NOTIF_2_DISPATCHALERT());
...
*/
        ...
        public static void upgradeSchema(string year, string username, string password, List<RdxScript> orderExec, string defPath = "", string confPath = "")
        {
            if(string.IsNullOrEmpty(defPath))
                defPath = System.IO.Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "bin", "Scripts", Configuration.Program, "_ReIdx", "Reindex");
                ...
                else //table in tree
                {
                    ...
                    if (!keyValue[1].Equals("''")) //exists a previous level
                    {
                        if (paiIsNivel)
                        {
                            ...
                            querySel.WhereCondition.Equal(areaLevel.Alias, level[1], (double)areaLevel.returnValueField(level[0] + "." + level[1]) + 1);
