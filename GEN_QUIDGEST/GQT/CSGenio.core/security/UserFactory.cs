...

{
    public class UserFactory
    {
		...
		/// <summary>
        /// Creates the access level to each model specified by the user registration that uses the PESS1 form.
        /// </summary>
        /// <param name="user">A userlogin (psw) record</param>
        public void CreateUser_PESS1(CSGenioApsw user)
        {
            CSGenioAuserauthorization ua;

            ua = new CSGenioAuserauthorization(user.User, "Public");
            ua.ValCodpsw = user.ValCodpsw;
			ua.ValSistema = "GQT";
			ua.ValModulo = "GQT";
            ua.ValRole = Role.ROLE_20.Id;
            // We have to mark this to keep compatibility with Backoffice shared applications.
            if (Role.ROLE_20.Type == RoleType.LEVEL)
                ua.ValNivel = Role.ROLE_20.GetLevelInt();
			ua.insert(sp);
		}
		...
		public static void MailSender(CSGenioApsw user, string ConfirmURL, string lang = "")
        {
            ...
            else
            {
                try
                {
                    ...
                    // USE /[MANUAL GQT CUSTOM_USER_REGISTRATION]/
