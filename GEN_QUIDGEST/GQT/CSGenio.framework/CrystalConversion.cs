            ...
            try
            {
                switch (formatting)
                {
                    ...
                    case FieldFormatting.FLOAT:
                        double tmpDbl;
                        if (!double.TryParse(Qvalue.ToString(), out tmpDbl))
							throw new FrameworkException(null, "CrystalConversion.FromInterno", "Error converting " + Qvalue.ToString() + " to double.");
