...
// USE /[MANUAL GQT MANUAL_CONTROLLER AGREG]/
        ...
        public JsonResult ReloadDBEdit(string Identifier, IDictionary<string, object> Values)
        {
            ...
            try { row = Models.Agreg.Find(navigation.GetStrValue("agreg")); }
            catch (Exception)
            {
					...
					case "AGREG___PROJEPROJECTO":	// Field (DB)
                        {
						    ...
						    var model = new Agreg_ViewModel(navigation) { editable = false };
        ...
        public JsonResult GetDependants(string Identifier, string Selected, IDictionary<string, object> Limits)
        {
            ...
            try
            {
				...
				{
					case "AGREG___PROJEPROJECTO":	// Field (DB)
						values = Agreg_ViewModel.GetDependant_AgregTableProjeProjecto(Selected, navigation);
						break;
					case "AGREG___YEAR_YEAR____":	// Field (DB)
						values = Agreg_ViewModel.GetDependant_AgregTableYearYear(Selected, navigation);
						break;
					default: break;
				}
		...
		[AuthorizeForUsers]
        [ActionSessionState(System.Web.SessionState.SessionStateBehavior.ReadOnly)]
        public JsonResult RecalculateFormulas_Agreg(Agreg_ViewModel form_data)
        {
            return GenericRecalculateFormulas(form_data, "agreg",
                (primaryKey) => Models.Agreg.Find(primaryKey, "FAGREG"),
