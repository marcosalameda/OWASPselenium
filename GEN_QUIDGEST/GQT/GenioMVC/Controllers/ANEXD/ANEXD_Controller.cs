...
// USE /[MANUAL GQT CONTROLLER_NAVIGATION ANEXD]/
        ...
        {
            CSGenio.business.Area area = CSGenio.business.Area.createArea<CSGenioAanexd>(UserContext.Current.User, UserContext.Current.User.CurrentModule);
...
// USE /[MANUAL GQT MANUAL_CONTROLLER ANEXD]/
        ...
        public JsonResult ReloadDBEdit(string Identifier, IDictionary<string, object> Values)
        {
            ...
            try { row = Models.Anexd.Find(navigation.GetStrValue("anexd")); }
            catch (Exception)
            {
                CSGenio.framework.Log.Error("ReloadDBEdit - " + Identifier + " Not found Model anexd");
            }
            if(row == null)
            {
                row = new Models.Anexd();
					...
					case "ANEXD___EQUIPREGISTNR":	// Field (DB)
                        {
						    ...
						    var model = new Anexd_ViewModel(navigation) { editable = false };
        ...
        public JsonResult GetDependants(string Identifier, string Selected, IDictionary<string, object> Limits)
        {
            ...
            try
            {
				...
				{
					case "ANEXD___EQUIPREGISTNR":	// Field (DB)
						values = Anexd_ViewModel.GetDependant_AnexdTableEquipRegistnr(Selected, navigation);
						break;
					case "ANEXD___LANGULANGUA__":	// Field (DB)
						values = Anexd_ViewModel.GetDependant_AnexdTableLanguLangua(Selected, navigation);
						break;
					default: break;
				}
		...
		[AuthorizeForUsers]
        [ActionSessionState(System.Web.SessionState.SessionStateBehavior.ReadOnly)]
        public JsonResult RecalculateFormulas_Anexd(Anexd_ViewModel form_data)
        {
            return GenericRecalculateFormulas(form_data, "anexd",
                (primaryKey) => Models.Anexd.Find(primaryKey, "FANEXD"),
                (model) => form_data.MapToModel(model as Models.Anexd)
            );
        }
		...
		[AuthorizeForUsers]
        public new ActionResult SubmitVersion(string ticket, string fieldSize = "", string dataIdentifier = "", bool isRequired = false)
        {
            return base.SubmitVersion(ticket, fieldSize, dataIdentifier, isRequired);
        }

		[AuthorizeForUsers]
        public new ActionResult CheckoutDocum(string ticket, bool usesTemplates, string fieldSize = "", string dataIdentifier = "", bool isRequired = false, DocumentViewTypeMode viewType = DocumentViewTypeMode.Print)
        {
            return base.CheckoutDocum(ticket, usesTemplates, fieldSize, dataIdentifier, isRequired, viewType);
        }

		[AuthorizeForUsers]
        public new ActionResult DeleteFile(string ticket, bool usesTemplates, ControllerBase.VersionDeleteAction action = VersionDeleteAction.All, string fieldSize = "", string dataIdentifier = "", bool isRequired = false)
        {
            return base.DeleteFile(ticket, usesTemplates, action, fieldSize, dataIdentifier, isRequired);
        }

		[AuthorizeForUsers]
        public new ActionResult SetFile(string ticket, bool usesTemplates, ControllerBase.VersionSubmitAction mode = VersionSubmitAction.Insert, string version = "1", string fieldSize = "", string dataIdentifier = "", bool isRequired = false, DocumentViewTypeMode viewType = DocumentViewTypeMode.Print)
        {
            return base.SetFile(ticket, usesTemplates, mode, version, fieldSize, dataIdentifier, isRequired, viewType);
        }
