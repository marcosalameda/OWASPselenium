...
// USE /[MANUAL GQT MANUAL_CONTROLLER CATTP]/
        ...
        public JsonResult ReloadDBEdit(string Identifier, IDictionary<string, object> Values)
        {
            ...
            try { row = Models.Cattp.Find(navigation.GetStrValue("cattp")); }
            catch (Exception)
            {
					...
					case "TPCAT___SBCATSUBCATEG":	// Field (DB)
                        {
						    ...
						    var model = new Tpcat_ViewModel(navigation) { editable = false };
        ...
        public JsonResult GetDependants(string Identifier, string Selected, IDictionary<string, object> Limits)
        {
            ...
            try
            {
				...
				{
					case "TPCAT___SBCATSUBCATEG":	// Field (DB)
						values = Tpcat_ViewModel.GetDependant_TpcatTableSbcatSubcateg(Selected, navigation);
						break;
					default: break;
				}
		...
		[AuthorizeForUsers]
        [ActionSessionState(System.Web.SessionState.SessionStateBehavior.ReadOnly)]
        public JsonResult RecalculateFormulas_Tpcat(Tpcat_ViewModel form_data)
        {
            return GenericRecalculateFormulas(form_data, "cattp",
                (primaryKey) => Models.Cattp.Find(primaryKey, "FTPCAT"),
