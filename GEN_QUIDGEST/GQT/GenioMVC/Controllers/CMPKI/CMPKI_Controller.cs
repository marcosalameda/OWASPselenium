...
// USE /[MANUAL GQT MANUAL_CONTROLLER CMPKI]/
        ...
        public JsonResult ReloadDBEdit(string Identifier, IDictionary<string, object> Values)
        {
            ...
            try { row = Models.Cmpki.Find(navigation.GetStrValue("cmpki")); }
            catch (Exception)
            {
					...
					case "CMPKI___TPEQUTIPOEQUI":	// Field (DB)
                        {
						    ...
						    var model = new Cmpki_ViewModel(navigation) { editable = false };
        ...
        public JsonResult GetDependants(string Identifier, string Selected, IDictionary<string, object> Limits)
        {
            ...
            try
            {
				...
				{
					case "CMPKI___TPEQUTIPOEQUI":	// Field (DB)
						values = Cmpki_ViewModel.GetDependant_CmpkiTableTpequTipoequi(Selected, navigation);
						break;
					case "CMPKI___TPEQ1TIPOEQUI":	// Field (DB)
						values = Cmpki_ViewModel.GetDependant_CmpkiTableTpeq1Tipoequi(Selected, navigation);
						break;
					default: break;
				}
		...
		[AuthorizeForUsers]
        [ActionSessionState(System.Web.SessionState.SessionStateBehavior.ReadOnly)]
        public JsonResult RecalculateFormulas_Cmpki(Cmpki_ViewModel form_data)
        {
            return GenericRecalculateFormulas(form_data, "cmpki",
                (primaryKey) => Models.Cmpki.Find(primaryKey, "FCMPKI"),
