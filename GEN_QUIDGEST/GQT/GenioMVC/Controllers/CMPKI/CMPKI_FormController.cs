...
namespace GenioMVC.Controllers
{
	public partial class CmpkiController : ControllerBase
	{
		...
		[AuthorizeForUsers]
		public ActionResult ChangeFormMode_Cmpki(string mode)
		{
			var _mode = Navigation.CurrentLevel.Location;
			var id = Navigation.GetStrValue("cmpki");
			...
			switch (mode)
			{
				case "show":
					if (m.Contains("v"))
						_mode = ACTION_CMPKI_SHOW;
					break;
				case "delete":
					if (m.Contains("a"))
						_mode = ACTION_CMPKI_DELETE;
					break;
				case "edit":
					if (m.Contains("e"))
						_mode = ACTION_CMPKI_EDIT;
					break;
				case "duplicate":
					if (m.Contains("d"))
						_mode = ACTION_CMPKI_DUPLICATE;
					break;
				case "new":
					if (m.Contains("i"))
						_mode = ACTION_CMPKI_NEW;
					break;
			}
...
// USE /[MANUAL GQT CONTROLLER_SHOW CMPKI]/
		//
		// GET: /Cmpki/Cmpki_Show
		[AuthorizeForUsers]
		public ActionResult Cmpki_Show(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";
			var model = new Cmpki_ViewModel(Navigation, nestedForm);
			...
			string partialView = qs["partialView"] ?? "Cmpki"; // MF send the patial view name

			var navigationLocationAction = ACTION_CMPKI_SHOW.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			...
			if (!isHomePage && (IsNewLocation(navigationLocationAction) || !string.Equals(Navigation.GetStrValue("cmpki"), id)))
			{
				Navigation.AddHistoryLevel(navigationLocationAction, FormMode.Show, nestedForm);
				CSGenio.framework.Audit.registAction(UserContext.Current.User, Resources.Resources.FORM54242 + " " + Navigation.CurrentLevel.Location.ShortDescription());
			}

			Navigation.SetValue("cmpki", id);
...
// USE /[MANUAL GQT BEFORE_LOAD_SHOW CMPKI]/
			...
			catch (ModelNotFoundException)
			{
				...
				CSGenio.framework.Log.Error("Cmpki_Show - GET " + id + " " + e.Message);
				throw;
			}
...
// USE /[MANUAL GQT AFTER_LOAD_SHOW CMPKI]/
			...
			if (isHomePage)
			{
				Navigation.SetValue("HomePage", "Cmpki");
				return PartialView(partialView, model);
			}
			else if (!Request.IsAjaxRequest())
				return View("Cmpki", model);
			else
				return PartialView(partialView, model);
		}
		...
		[ActionName("Cmpki_New_Insert")]
		[AuthorizeForUsers]
		[HttpGet]
		public ActionResult Cmpki_New_Insert()
		{
			...
			return RedirectToAction("Cmpki_New");
		}

// USE /[MANUAL GQT CONTROLLER_NEW_GET CMPKI]/
		//
		// GET: /Cmpki/Cmpki_New
		[ActionName("Cmpki_New")]
		[AuthorizeForUsers]
		[HttpGet]
		public ActionResult Cmpki_New()
		{
			...
			string partialView = qs["partialView"] ?? "Cmpki";

			var navigationLocationAction = ACTION_CMPKI_NEW.SetRoutedValues(new { m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);

			//FOR: OVERRIDE SKIP IF JUST ONE
			//Allow child form to use "Go Back" to menu list without "skip if only one"
			if (Navigation.OverrideSkipIfJustOne.ContainsKey("CMPKI"))
				Navigation.OverrideSkipIfJustOne["CMPKI"] = true;
...
// USE /[MANUAL GQT AFTER_LOAD_NEW CMPKI]/
			...
			catch (Exception e)
			{
				...
				CSGenio.framework.Log.Error("Cmpki_New - GET " + e.Message);
...
// USE /[MANUAL GQT CONTROLLER_NEW_POST CMPKI]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Cmpki_New(Cmpki_ViewModel model, bool redirect = true)
		{
...
// USE /[MANUAL GQT AFTER_LOAD_NEW_EX CMPKI]/
		...
		[AuthorizeForUsers]
		public ActionResult Cmpki_New_Redirect(bool internalRedirect = false)
		{
			IList<string> warningMsgs = TempData["NEW_WARNINGS_LIST"] as List<string>;
			string saveMsg = TempData["NEW_SAVE_LIST"] as string;

			if (!internalRedirect)
				SuccessMessage(String.IsNullOrEmpty(saveMsg) ? Resources.Resources.REGISTO_CRIADO_COM_S18746 + GetHumanKeyToQMessage("cmpki", Convert.ToString(Navigation.CurrentLevel.GetEntry("cmpki"))) : saveMsg);
			...
			if (Convert.ToBoolean(Request.QueryString.Get("repeatInsertion")))
				return RedirectToLocation(ACTION_CMPKI_NEW, new { repeatInsertion = true });
...
// USE /[MANUAL GQT CONTROLLER_EDIT_GET CMPKI]/
		//
		// GET: /Cmpki/Cmpki_Edit
		[AuthorizeForUsers]
		[HttpGet]
		[ActionName("Cmpki_Edit")]
		public ActionResult Cmpki_Edit(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";
			string partialView = qs["partialView"] ?? "Cmpki"; // MF send the partial view name

			//Useful to initialize a record that had to be created by the framework prior to edition (and method New doesnt support this). This way, if the form then is canceled by the user, the record will be automatically deleted
			var isNewEdit = Navigation.GetStrValue("NewEdit") == "true";

			var navigationLocationAction =  isNewEdit ? ACTION_CMPKI_NEW.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] }) : ACTION_CMPKI_EDIT.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);
...
// USE /[MANUAL GQT BEFORE_LOAD_EDIT CMPKI]/
			...
			catch (ModelNotFoundException)
			{
				...
				sp.closeConnection();
				//JFG 05-05-2017 This need to better handled, for now it's handled on webconfig's customErrors section.
				CSGenio.framework.Log.Error("Cmpki_Edit - " + id + " " + e.Message);
				throw;
			}
...
// USE /[MANUAL GQT CONTROLLER_EDIT_POST CMPKI]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Cmpki_Edit(Cmpki_ViewModel model, bool redirect = true)
		{
...
// USE /[MANUAL GQT AFTER_LOAD_EDIT_EX CMPKI]/
		...
		[AuthorizeForUsers]
		public ActionResult Cmpki_Edit_Redirect(bool internalRedirect = false)
		{
			IList<string> warningMsgs = TempData["EDIT_WARNINGS_LIST"] as List<string>;
			string saveMsg = TempData["NEW_SAVE_LIST"] as string;

			if (!internalRedirect || !Request.IsAjaxRequest())
				SuccessMessage(String.IsNullOrEmpty(saveMsg) ? Resources.Resources.ALTERACOES_EFETUADAS10166 + GetHumanKeyToQMessage("cmpki", Convert.ToString(Navigation.CurrentLevel.GetEntry("cmpki"))) : saveMsg);
...
// USE /[MANUAL GQT CONTROLLER_DELETE_GET CMPKI]/
		//
		// GET: /Cmpki/Cmpki_Delete
		[AuthorizeForUsers]
		[HttpGet]
		[ActionName("Cmpki_Delete")]
		public ActionResult Cmpki_Delete(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";

			var navigationLocationAction = ACTION_CMPKI_DELETE.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);
...
// USE /[MANUAL GQT BEFORE_LOAD_DELETE CMPKI]/
			...
			catch (ModelNotFoundException)
			{
				...
				//JFG 05-05-2017 This need to better handled, for now it's handled on webconfig's customErrors section.
				CSGenio.framework.Log.Error("Cmpki_Delete - " + id + " " + e.Message);
				throw;
			}
...
// USE /[MANUAL GQT AFTER_LOAD_DELETE CMPKI]/
			...
			if (!Request.IsAjaxRequest())
				...
				if (permission.Status.Equals(CSGenio.framework.Status.E))
					...
					return PartialView("Cmpki", model);
		}
...
// USE /[MANUAL GQT CONTROLLER_DELETE_POST CMPKI]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Cmpki_Delete(string id, FormCollection collection)
		{

			var model = new Cmpki_ViewModel (Navigation, id);
			model.MapFromModel();
...
// USE /[MANUAL GQT CONTROLLER_DUPLICATE_GET CMPKI]/
		//
		// GET: /Cmpki/Cmpki_Duplicate
		[AuthorizeForUsers]
		[HttpGet]
		public ActionResult Cmpki_Duplicate(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";
			var model = new Cmpki_ViewModel(Navigation, nestedForm);
			...
			var navigationLocationAction = ACTION_CMPKI_DUPLICATE.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);
...
// USE /[MANUAL GQT AFTER_LOAD_DUPLICATE CMPKI]/
					...
					sp.closeTransaction();
					Navigation.CurrentLevel.SetLocation(ACTION_CMPKI_DUPLICATE.SetRoutedValues(new { Id = model.ValCodcmpki }));
...
// USE /[MANUAL GQT CONTROLLER_DUPLICATE_POST CMPKI]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Cmpki_Duplicate(Cmpki_ViewModel model, bool redirect = true)
		{
			var eventSink = new EventSink()
			{
...
// USE /[MANUAL GQT AFTER_LOAD_DUPLICATE_EX CMPKI]/
		...
		[AuthorizeForUsers]
		public ActionResult Cmpki_Duplicate_Redirect(bool internalRedirect = false)
		{
...
// USE /[MANUAL GQT CONTROLLER_CANCEL_GET CMPKI]/
		...
		public ActionResult Cmpki_Cancel()
		{
			...
			if (Navigation.CurrentLevel.FormMode == FormMode.New || Navigation.CurrentLevel.FormMode == FormMode.Duplicate)
			{
				...
				try
				{
					var model = new GenioMVC.Models.Cmpki();
...
// USE /[MANUAL GQT AFTER_CANCEL CMPKI]/
			...
			else if (Navigation.PreviousLevel != null)
			{
				...
				string previousLevelArea = Navigation.PreviousLevel.Location.Controller?.ToLower() ?? "cmpki";
		...
		{
			var model = new Cmpki_ViewModel(Navigation, true);
			model.setModes(Request.QueryString["m"]);
			PersistentSupport sp = UserContext.Current.PersistentSupport;
			var navigationLocationAction = ACTION_CMPKI_NEW.SetRoutedValues(new { m = Request.QueryString["m"] });
			CheckLevels(navigationLocationAction);
			...
			return PartialView("MFCmpki", model);
		}
		...
		public ActionResult MFCmpki_Edit(string id)
		{
			return this.RedirectToAction("Cmpki_Edit", "Cmpki", new { id = id, partialView = "MFCmpki", nestedForm = "true", multiForm = "true" });
		}
		...
		public ActionResult MFCmpki_Cancel(string id)
		{
			//MH - Validate if current level is it correct. Remove all levels that is higher than the currently level.
			CheckLevels(NavigationLocation.Any);

			if (Navigation.CurrentLevel.IsNestedContext &&
				(Navigation.CurrentLevel.Location.Action == ACTION_CMPKI_NEW.Action || Navigation.CurrentLevel.Location.Action == ACTION_CMPKI_EDIT.Action))
			{
				if (Navigation.CurrentLevel.FormMode == FormMode.New)
				{
					...
					catch (Exception e)
					{
						...
						Log.Error("MFCmpki_Cancel - " + exceptionUserMessage);
						ErrorMessage(exceptionUserMessage);
					}
				}
			...
			return this.RedirectToAction("Cmpki_Show", "Cmpki", new { id = id, partialView = "MFCmpki", nestedForm = "true", multiForm = "true", nav });
		}
		...
		public JsonResult MFCmpki_Delete(string id)
		{
			var eventSink = new EventSink()
			{
		...
		public ActionResult Cmpki_TpequValTipoequi(string id, string partialView,  IDictionary<string, string> Limits)
		{
		...
		[ActionSessionState(System.Web.SessionState.SessionStateBehavior.ReadOnly)]
		public ActionResult Cmpki_Tpeq1ValTipoequi(string id, string partialView,  IDictionary<string, string> Limits)
		{
			int perPage = CSGenio.framework.Configuration.NrRegDBedit;
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Cmpki_SaveEdit(Cmpki_ViewModel model)
		{
			var eventSink = new EventSink()
			{
