...
// USE /[MANUAL GQT MANUAL_CONTROLLER CONTA]/
        ...
        public JsonResult ReloadDBEdit(string Identifier, IDictionary<string, object> Values)
        {
            ...
            try { row = Models.Conta.Find(navigation.GetStrValue("conta")); }
            catch (Exception)
            {
					...
					case "CONTA___PESSONAME____":	// Field (DB)
                        {
						    ...
						    var model = new Conta_ViewModel(navigation) { editable = false };
        ...
        public JsonResult GetDependants(string Identifier, string Selected, IDictionary<string, object> Limits)
        {
            ...
            try
            {
				...
				{
					case "CONTA___PESSONAME____":	// Field (DB)
						values = Conta_ViewModel.GetDependant_ContaTablePessoName(Selected, navigation);
						break;
					case "CONTA___GENREGENDER__":	// Field (DB)
						values = Conta_ViewModel.GetDependant_ContaTableGenreGender(Selected, navigation);
						break;
					case "CONTA___TPCONTIPOCONT":	// Field (DB)
						values = Conta_ViewModel.GetDependant_ContaTableTpconTipocont(Selected, navigation);
						break;
					default: break;
				}
		...
		[AuthorizeForUsers]
        [ActionSessionState(System.Web.SessionState.SessionStateBehavior.ReadOnly)]
        public JsonResult RecalculateFormulas_Conta(Conta_ViewModel form_data)
        {
            return GenericRecalculateFormulas(form_data, "conta",
                (primaryKey) => Models.Conta.Find(primaryKey, "FCONTA"),
