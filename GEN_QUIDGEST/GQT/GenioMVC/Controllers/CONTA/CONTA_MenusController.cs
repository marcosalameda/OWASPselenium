...
namespace GenioMVC.Controllers
{
    public partial class ContaController : ControllerBase
    {
        ...
        public ActionResult TBS_Menu_141(bool allSelected = false)
        {
			int perPage = CSGenio.framework.Configuration.NrRegDBedit;

            TBS_Menu_141_ViewModel model = new TBS_Menu_141_ViewModel(Navigation);
            ...
            {
                if (Navigation.CurrentLevel == null || !ACTION_TBS_MENU_141.IsSameAction(Navigation.CurrentLevel.Location))
                {
                    // reset the selections for this new navigation flow
                    // TODO: This change still requires more testing
                    Navigation.RemoveHistoryLevel(ACTION_TBS_MENU_141);
                    if (Navigation.CurrentLevel.Location.Action != ACTION_TBS_MENU_141.Action)
                    {
                        Navigation.AddHistoryLevel(ACTION_TBS_MENU_141, FormMode.List);
            ...
            if(isHomePage)
                return PartialView("TBS_Menu_141", model);
            else if (!Request.IsAjaxRequest())
                return View(model);
            else
                return PartialView("TBS_Menu_141_Partial", model);
        }
