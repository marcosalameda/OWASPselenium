...
namespace GenioMVC.Controllers
{
    ...
    {
        private static readonly NavigationLocation ACTION_GQT_Menu_TESTDS = new NavigationLocation("DASHBOARD27032", "GQT_Menu_TESTDS", "Dashboard");

        // GET: /Dashboard/GQT_Menu_TESTDS
		[AuthorizeForUsers]
        [ActionName("GQT_Menu_TESTDS")]
        public ActionResult GQT_Menu_TESTDS()
        {
            DashboardViewModel vm = new GQT_Menu_TESTDS_ViewModel(Navigation);
            vm.Load();
            ...
            if (isHomePage) Navigation.SetValue("HomePage", "GQT_Menu_TESTDS");
            ...
            {
                if (Navigation.CurrentLevel == null || !ACTION_GQT_Menu_TESTDS.IsSameAction(Navigation.CurrentLevel.Location))
                {
                    if (Navigation.ContainsAction(ACTION_GQT_Menu_TESTDS))
                        Navigation.RemoveHistoryLevel(ACTION_GQT_Menu_TESTDS);
                    if (Navigation.CurrentLevel.Location.Action != ACTION_GQT_Menu_TESTDS.Action)
                    {
                        Navigation.AddHistoryLevel(ACTION_GQT_Menu_TESTDS, FormMode.Show);
            ...
            {
                CSGenio.framework.Audit.registAction(UserContext.Current.User, Resources.Resources.MENU01948 + " " + ACTION_GQT_Menu_TESTDS.ShortDescription());
            ...
            if (isHomePage)
                return PartialView("GQT_Menu_TESTDS", vm);
        ...
        public ActionResult GQT_Menu_TESTDS_RenderMenuWidget(WidgetType type, string widgetId)
        {
            DashboardViewModel vm = new GQT_Menu_TESTDS_ViewModel(Navigation);
		...
		[HttpGet]
        [ActionSessionState(System.Web.SessionState.SessionStateBehavior.ReadOnly)]
        public ActionResult GQT_Menu_TESTDS_GetWidgetData(WidgetType widgetType, string widgetId)
        {
            DashboardViewModel vm = new GQT_Menu_TESTDS_ViewModel(Navigation);
