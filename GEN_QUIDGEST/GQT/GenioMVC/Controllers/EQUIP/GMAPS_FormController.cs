...
namespace GenioMVC.Controllers
{
	public partial class EquipController : ControllerBase
	{
		...
		[AuthorizeForUsers]
		public ActionResult ChangeFormMode_Gmaps(string mode)
		{
			...
			switch (mode)
			{
				case "show":
					if (m.Contains("v"))
						_mode = ACTION_GMAPS_SHOW;
					break;
				case "delete":
					if (m.Contains("a"))
						_mode = ACTION_GMAPS_DELETE;
					break;
				case "edit":
					if (m.Contains("e"))
						_mode = ACTION_GMAPS_EDIT;
					break;
				case "duplicate":
					if (m.Contains("d"))
						_mode = ACTION_GMAPS_DUPLICATE;
					break;
				case "new":
					if (m.Contains("i"))
						_mode = ACTION_GMAPS_NEW;
					break;
			}
		...
		[AuthorizeForUsers]
		public ActionResult Gmaps_ModalDBEdit(string partialView)
		{
			Gmaps_ViewModel model = new Gmaps_ViewModel();
...
// USE /[MANUAL GQT CONTROLLER_SHOW GMAPS]/
		//
		// GET: /Equip/Gmaps_Show
		[AuthorizeForUsers]
		public ActionResult Gmaps_Show(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";
			var model = new Gmaps_ViewModel(Navigation, nestedForm);
			model.setModes(Request.QueryString["m"]);
			...
			string partialView = qs["partialView"] ?? "Gmaps"; // MF send the patial view name

			var navigationLocationAction = ACTION_GMAPS_SHOW.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
...
// USE /[MANUAL GQT BEFORE_LOAD_SHOW GMAPS]/
			...
			catch (ModelNotFoundException)
			{
				...
				CSGenio.framework.Log.Error("Gmaps_Show - GET " + id + " " + e.Message);
				throw;
			}
...
// USE /[MANUAL GQT AFTER_LOAD_SHOW GMAPS]/
			...
			if (isHomePage)
			{
				Navigation.SetValue("HomePage", "Gmaps");
				return PartialView(partialView, model);
			}
			else if (!Request.IsAjaxRequest())
				return View("Gmaps", model);
			else
				return PartialView(partialView, model);
		}
		...
		[ActionName("Gmaps_New_Insert")]
		[AuthorizeForUsers]
		[HttpGet]
		public ActionResult Gmaps_New_Insert()
		{
			...
			return RedirectToAction("Gmaps_New");
		}

// USE /[MANUAL GQT CONTROLLER_NEW_GET GMAPS]/
		//
		// GET: /Equip/Gmaps_New
		[ActionName("Gmaps_New")]
		[AuthorizeForUsers]
		[HttpGet]
		public ActionResult Gmaps_New()
		{
			...
			string partialView = qs["partialView"] ?? "Gmaps";

			var navigationLocationAction = ACTION_GMAPS_NEW.SetRoutedValues(new { m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);
			...
			if (Navigation.OverrideSkipIfJustOne.ContainsKey("GMAPS"))
				Navigation.OverrideSkipIfJustOne["GMAPS"] = true;
...
// USE /[MANUAL GQT AFTER_LOAD_NEW GMAPS]/
			...
			catch (Exception e)
			{
				...
				CSGenio.framework.Log.Error("Gmaps_New - GET " + e.Message);
...
// USE /[MANUAL GQT CONTROLLER_NEW_POST GMAPS]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Gmaps_New(Gmaps_ViewModel model, bool redirect = true)
		{
...
// USE /[MANUAL GQT AFTER_LOAD_NEW_EX GMAPS]/
		...
		[AuthorizeForUsers]
		public ActionResult Gmaps_New_Redirect(bool internalRedirect = false)
		{
			...
			if (Convert.ToBoolean(Request.QueryString.Get("repeatInsertion")))
				return RedirectToLocation(ACTION_GMAPS_NEW, new { repeatInsertion = true });
...
// USE /[MANUAL GQT CONTROLLER_EDIT_GET GMAPS]/
		//
		// GET: /Equip/Gmaps_Edit
		[AuthorizeForUsers]
		[HttpGet]
		[ActionName("Gmaps_Edit")]
		public ActionResult Gmaps_Edit(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";
			string partialView = qs["partialView"] ?? "Gmaps"; // MF send the partial view name

			//Useful to initialize a record that had to be created by the framework prior to edition (and method New doesnt support this). This way, if the form then is canceled by the user, the record will be automatically deleted
			var isNewEdit = Navigation.GetStrValue("NewEdit") == "true";

			var navigationLocationAction =  isNewEdit ? ACTION_GMAPS_NEW.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] }) : ACTION_GMAPS_EDIT.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);
...
// USE /[MANUAL GQT BEFORE_LOAD_EDIT GMAPS]/
			...
			catch (ModelNotFoundException)
			{
				...
				sp.closeConnection();
				//JFG 05-05-2017 This need to better handled, for now it's handled on webconfig's customErrors section.
				CSGenio.framework.Log.Error("Gmaps_Edit - " + id + " " + e.Message);
				throw;
			}
...
// USE /[MANUAL GQT CONTROLLER_EDIT_POST GMAPS]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Gmaps_Edit(Gmaps_ViewModel model, bool redirect = true)
		{
...
// USE /[MANUAL GQT AFTER_LOAD_EDIT_EX GMAPS]/
		...
		[AuthorizeForUsers]
		public ActionResult Gmaps_Edit_Redirect(bool internalRedirect = false)
		{
...
// USE /[MANUAL GQT CONTROLLER_DELETE_GET GMAPS]/
		//
		// GET: /Equip/Gmaps_Delete
		[AuthorizeForUsers]
		[HttpGet]
		[ActionName("Gmaps_Delete")]
		public ActionResult Gmaps_Delete(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";

			var navigationLocationAction = ACTION_GMAPS_DELETE.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);
...
// USE /[MANUAL GQT BEFORE_LOAD_DELETE GMAPS]/
			...
			catch (ModelNotFoundException)
			{
				...
				//JFG 05-05-2017 This need to better handled, for now it's handled on webconfig's customErrors section.
				CSGenio.framework.Log.Error("Gmaps_Delete - " + id + " " + e.Message);
				throw;
			}
...
// USE /[MANUAL GQT AFTER_LOAD_DELETE GMAPS]/
			...
			if (!Request.IsAjaxRequest())
				...
				if (permission.Status.Equals(CSGenio.framework.Status.E))
					...
					return PartialView("Gmaps", model);
		}
...
// USE /[MANUAL GQT CONTROLLER_DELETE_POST GMAPS]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Gmaps_Delete(string id, FormCollection collection)
		{

			var model = new Gmaps_ViewModel (Navigation, id);
			model.MapFromModel();
...
// USE /[MANUAL GQT CONTROLLER_DUPLICATE_GET GMAPS]/
		//
		// GET: /Equip/Gmaps_Duplicate
		[AuthorizeForUsers]
		[HttpGet]
		public ActionResult Gmaps_Duplicate(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";
			var model = new Gmaps_ViewModel(Navigation, nestedForm);
			...
			var navigationLocationAction = ACTION_GMAPS_DUPLICATE.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);
...
// USE /[MANUAL GQT AFTER_LOAD_DUPLICATE GMAPS]/
					...
					sp.closeTransaction();
					Navigation.CurrentLevel.SetLocation(ACTION_GMAPS_DUPLICATE.SetRoutedValues(new { Id = model.ValCodequip }));
...
// USE /[MANUAL GQT CONTROLLER_DUPLICATE_POST GMAPS]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Gmaps_Duplicate(Gmaps_ViewModel model, bool redirect = true)
		{
			var eventSink = new EventSink()
			{
...
// USE /[MANUAL GQT AFTER_LOAD_DUPLICATE_EX GMAPS]/
		...
		[AuthorizeForUsers]
		public ActionResult Gmaps_Duplicate_Redirect(bool internalRedirect = false)
		{
...
// USE /[MANUAL GQT AFTER_CANCEL GMAPS]/
		...
		{
			var model = new Gmaps_ViewModel(Navigation, true);
			model.setModes(Request.QueryString["m"]);
			PersistentSupport sp = UserContext.Current.PersistentSupport;
			var navigationLocationAction = ACTION_GMAPS_NEW.SetRoutedValues(new { m = Request.QueryString["m"] });
			CheckLevels(navigationLocationAction);
			...
			return PartialView("MFGmaps", model);
		}
		...
		public ActionResult MFGmaps_Edit(string id)
		{
			return this.RedirectToAction("Gmaps_Edit", "Equip", new { id = id, partialView = "MFGmaps", nestedForm = "true", multiForm = "true" });
		}
		...
		public ActionResult MFGmaps_Cancel(string id)
		{
			//MH - Validate if current level is it correct. Remove all levels that is higher than the currently level.
			CheckLevels(NavigationLocation.Any);

			if (Navigation.CurrentLevel.IsNestedContext &&
				(Navigation.CurrentLevel.Location.Action == ACTION_GMAPS_NEW.Action || Navigation.CurrentLevel.Location.Action == ACTION_GMAPS_EDIT.Action))
			{
				if (Navigation.CurrentLevel.FormMode == FormMode.New)
				{
					...
					catch (Exception e)
					{
						...
						Log.Error("MFGmaps_Cancel - " + exceptionUserMessage);
						ErrorMessage(exceptionUserMessage);
					}
				}
			...
			return this.RedirectToAction("Gmaps_Show", "Equip", new { id = id, partialView = "MFGmaps", nestedForm = "true", multiForm = "true", nav });
		}
		...
		public JsonResult MFGmaps_Delete(string id)
		{
			var eventSink = new EventSink()
			{
		...
		public ActionResult Gmaps_ValInstalac(string id, string partialView)
		{
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Gmaps_SaveEdit(Gmaps_ViewModel model)
		{
			var eventSink = new EventSink()
			{
