...
namespace GenioMVC.Controllers
{
	public partial class FldsController : ControllerBase
	{
		...
		[AuthorizeForUsers]
		public ActionResult ChangeFormMode_Fieldhlp(string mode)
		{
			...
			switch (mode)
			{
				case "show":
					if (m.Contains("v"))
						_mode = ACTION_FIELDHLP_SHOW;
					break;
				case "delete":
					if (m.Contains("a"))
						_mode = ACTION_FIELDHLP_DELETE;
					break;
				case "edit":
					if (m.Contains("e"))
						_mode = ACTION_FIELDHLP_EDIT;
					break;
				case "duplicate":
					if (m.Contains("d"))
						_mode = ACTION_FIELDHLP_DUPLICATE;
					break;
				case "new":
					if (m.Contains("i"))
						_mode = ACTION_FIELDHLP_NEW;
					break;
			}
...
// USE /[MANUAL GQT CONTROLLER_SHOW FIELDHLP]/
		//
		// GET: /Flds/Fieldhlp_Show
		[AuthorizeForUsers]
		public ActionResult Fieldhlp_Show(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";
			var model = new Fieldhlp_ViewModel(Navigation, nestedForm);
			...
			string partialView = qs["partialView"] ?? "Fieldhlp"; // MF send the patial view name

			var navigationLocationAction = ACTION_FIELDHLP_SHOW.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
...
// USE /[MANUAL GQT BEFORE_LOAD_SHOW FIELDHLP]/
			...
			catch (ModelNotFoundException)
			{
				...
				CSGenio.framework.Log.Error("Fieldhlp_Show - GET " + id + " " + e.Message);
				throw;
			}
...
// USE /[MANUAL GQT AFTER_LOAD_SHOW FIELDHLP]/
			...
			if (isHomePage)
			{
				Navigation.SetValue("HomePage", "Fieldhlp");
				return PartialView(partialView, model);
			}
			else if (!Request.IsAjaxRequest())
				return View("Fieldhlp", model);
			else
				return PartialView(partialView, model);
		}
		...
		[ActionName("Fieldhlp_New_Insert")]
		[AuthorizeForUsers]
		[HttpGet]
		public ActionResult Fieldhlp_New_Insert()
		{
			...
			return RedirectToAction("Fieldhlp_New");
		}

// USE /[MANUAL GQT CONTROLLER_NEW_GET FIELDHLP]/
		//
		// GET: /Flds/Fieldhlp_New
		[ActionName("Fieldhlp_New")]
		[AuthorizeForUsers]
		[HttpGet]
		public ActionResult Fieldhlp_New()
		{
			...
			string partialView = qs["partialView"] ?? "Fieldhlp";

			var navigationLocationAction = ACTION_FIELDHLP_NEW.SetRoutedValues(new { m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);

			//FOR: OVERRIDE SKIP IF JUST ONE
			//Allow child form to use "Go Back" to menu list without "skip if only one"
			if (Navigation.OverrideSkipIfJustOne.ContainsKey("FIELDHLP"))
				Navigation.OverrideSkipIfJustOne["FIELDHLP"] = true;
...
// USE /[MANUAL GQT AFTER_LOAD_NEW FIELDHLP]/
			...
			catch (Exception e)
			{
				...
				CSGenio.framework.Log.Error("Fieldhlp_New - GET " + e.Message);
...
// USE /[MANUAL GQT CONTROLLER_NEW_POST FIELDHLP]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Fieldhlp_New(Fieldhlp_ViewModel model, bool redirect = true)
		{
...
// USE /[MANUAL GQT AFTER_LOAD_NEW_EX FIELDHLP]/
		...
		[AuthorizeForUsers]
		public ActionResult Fieldhlp_New_Redirect(bool internalRedirect = false)
		{
			...
			if (Convert.ToBoolean(Request.QueryString.Get("repeatInsertion")))
				return RedirectToLocation(ACTION_FIELDHLP_NEW, new { repeatInsertion = true });
...
// USE /[MANUAL GQT CONTROLLER_EDIT_GET FIELDHLP]/
		//
		// GET: /Flds/Fieldhlp_Edit
		[AuthorizeForUsers]
		[HttpGet]
		[ActionName("Fieldhlp_Edit")]
		public ActionResult Fieldhlp_Edit(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";
			string partialView = qs["partialView"] ?? "Fieldhlp"; // MF send the partial view name

			//Useful to initialize a record that had to be created by the framework prior to edition (and method New doesnt support this). This way, if the form then is canceled by the user, the record will be automatically deleted
			var isNewEdit = Navigation.GetStrValue("NewEdit") == "true";

			var navigationLocationAction =  isNewEdit ? ACTION_FIELDHLP_NEW.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] }) : ACTION_FIELDHLP_EDIT.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);
...
// USE /[MANUAL GQT BEFORE_LOAD_EDIT FIELDHLP]/
			...
			catch (ModelNotFoundException)
			{
				...
				sp.closeConnection();
				//JFG 05-05-2017 This need to better handled, for now it's handled on webconfig's customErrors section.
				CSGenio.framework.Log.Error("Fieldhlp_Edit - " + id + " " + e.Message);
				throw;
			}
...
// USE /[MANUAL GQT CONTROLLER_EDIT_POST FIELDHLP]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Fieldhlp_Edit(Fieldhlp_ViewModel model, bool redirect = true)
		{
...
// USE /[MANUAL GQT AFTER_LOAD_EDIT_EX FIELDHLP]/
		...
		[AuthorizeForUsers]
		public ActionResult Fieldhlp_Edit_Redirect(bool internalRedirect = false)
		{
...
// USE /[MANUAL GQT CONTROLLER_DELETE_GET FIELDHLP]/
		//
		// GET: /Flds/Fieldhlp_Delete
		[AuthorizeForUsers]
		[HttpGet]
		[ActionName("Fieldhlp_Delete")]
		public ActionResult Fieldhlp_Delete(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";

			var navigationLocationAction = ACTION_FIELDHLP_DELETE.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);
...
// USE /[MANUAL GQT BEFORE_LOAD_DELETE FIELDHLP]/
			...
			catch (ModelNotFoundException)
			{
				...
				//JFG 05-05-2017 This need to better handled, for now it's handled on webconfig's customErrors section.
				CSGenio.framework.Log.Error("Fieldhlp_Delete - " + id + " " + e.Message);
				throw;
			}
...
// USE /[MANUAL GQT AFTER_LOAD_DELETE FIELDHLP]/
			...
			if (!Request.IsAjaxRequest())
				...
				if (permission.Status.Equals(CSGenio.framework.Status.E))
					...
					return PartialView("Fieldhlp", model);
		}
...
// USE /[MANUAL GQT CONTROLLER_DELETE_POST FIELDHLP]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Fieldhlp_Delete(string id, FormCollection collection)
		{

			var model = new Fieldhlp_ViewModel (Navigation, id);
			model.MapFromModel();
...
// USE /[MANUAL GQT CONTROLLER_DUPLICATE_GET FIELDHLP]/
		//
		// GET: /Flds/Fieldhlp_Duplicate
		[AuthorizeForUsers]
		[HttpGet]
		public ActionResult Fieldhlp_Duplicate(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";
			var model = new Fieldhlp_ViewModel(Navigation, nestedForm);
			...
			var navigationLocationAction = ACTION_FIELDHLP_DUPLICATE.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);
...
// USE /[MANUAL GQT AFTER_LOAD_DUPLICATE FIELDHLP]/
					...
					sp.closeTransaction();
					Navigation.CurrentLevel.SetLocation(ACTION_FIELDHLP_DUPLICATE.SetRoutedValues(new { Id = model.ValCodflds }));
...
// USE /[MANUAL GQT CONTROLLER_DUPLICATE_POST FIELDHLP]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Fieldhlp_Duplicate(Fieldhlp_ViewModel model, bool redirect = true)
		{
			var eventSink = new EventSink()
			{
...
// USE /[MANUAL GQT AFTER_LOAD_DUPLICATE_EX FIELDHLP]/
		...
		[AuthorizeForUsers]
		public ActionResult Fieldhlp_Duplicate_Redirect(bool internalRedirect = false)
		{
...
// USE /[MANUAL GQT AFTER_CANCEL FIELDHLP]/
		...
		{
			var model = new Fieldhlp_ViewModel(Navigation, true);
			model.setModes(Request.QueryString["m"]);
			PersistentSupport sp = UserContext.Current.PersistentSupport;
			var navigationLocationAction = ACTION_FIELDHLP_NEW.SetRoutedValues(new { m = Request.QueryString["m"] });
			CheckLevels(navigationLocationAction);
			...
			return PartialView("MFFieldhlp", model);
		}
		...
		public ActionResult MFFieldhlp_Edit(string id)
		{
			return this.RedirectToAction("Fieldhlp_Edit", "Flds", new { id = id, partialView = "MFFieldhlp", nestedForm = "true", multiForm = "true" });
		}
		...
		public ActionResult MFFieldhlp_Cancel(string id)
		{
			//MH - Validate if current level is it correct. Remove all levels that is higher than the currently level.
			CheckLevels(NavigationLocation.Any);

			if (Navigation.CurrentLevel.IsNestedContext &&
				(Navigation.CurrentLevel.Location.Action == ACTION_FIELDHLP_NEW.Action || Navigation.CurrentLevel.Location.Action == ACTION_FIELDHLP_EDIT.Action))
			{
				if (Navigation.CurrentLevel.FormMode == FormMode.New)
				{
					...
					catch (Exception e)
					{
						...
						Log.Error("MFFieldhlp_Cancel - " + exceptionUserMessage);
						ErrorMessage(exceptionUserMessage);
					}
				}
			...
			return this.RedirectToAction("Fieldhlp_Show", "Flds", new { id = id, partialView = "MFFieldhlp", nestedForm = "true", multiForm = "true", nav });
		}
		...
		public JsonResult MFFieldhlp_Delete(string id)
		{
			var eventSink = new EventSink()
			{
		...
		public ActionResult Fieldhlp_AeroValName(string id, string partialView,  IDictionary<string, string> Limits)
		{
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Fieldhlp_SaveEdit(Fieldhlp_ViewModel model)
		{
			var eventSink = new EventSink()
			{
