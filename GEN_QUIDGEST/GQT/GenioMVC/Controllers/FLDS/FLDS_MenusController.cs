...
namespace GenioMVC.Controllers
{
    public partial class FldsController : ControllerBase
    {
        ...
        public ActionResult PTN_Menu_261(bool allSelected = false)
        {
			int perPage = CSGenio.framework.Configuration.NrRegDBedit;

            PTN_Menu_261_ViewModel model = new PTN_Menu_261_ViewModel(Navigation);
            ...
            {
                if (Navigation.CurrentLevel == null || !ACTION_PTN_MENU_261.IsSameAction(Navigation.CurrentLevel.Location))
                {
                    // reset the selections for this new navigation flow
                    // TODO: This change still requires more testing
                    Navigation.RemoveHistoryLevel(ACTION_PTN_MENU_261);
                    if (Navigation.CurrentLevel.Location.Action != ACTION_PTN_MENU_261.Action)
                    {
                        Navigation.AddHistoryLevel(ACTION_PTN_MENU_261, FormMode.List);
...
// USE /[MANUAL PTN MENU_GET 261]/
            ...
            if(isHomePage)
                return PartialView("PTN_Menu_261", model);
            else if (!Request.IsAjaxRequest())
                return View(model);
            else
                return PartialView("PTN_Menu_261_Partial", model);
        }
        ...
        public ActionResult PTN_Menu_271(bool allSelected = false)
        {
			int perPage = CSGenio.framework.Configuration.NrRegDBedit;

            PTN_Menu_271_ViewModel model = new PTN_Menu_271_ViewModel(Navigation);
            bool isHomePage = RouteData.Values.ContainsKey("isHomePage") ? (bool)RouteData.Values["isHomePage"] : false;
            if (isHomePage)
			...
			if (!isHomePage && !Request.IsAjaxRequest())
            {
                if (Navigation.CurrentLevel == null || !ACTION_PTN_MENU_271.IsSameAction(Navigation.CurrentLevel.Location))
                {
                    // reset the selections for this new navigation flow
                    // TODO: This change still requires more testing
                    Navigation.RemoveHistoryLevel(ACTION_PTN_MENU_271);
                    if (Navigation.CurrentLevel.Location.Action != ACTION_PTN_MENU_271.Action)
                    {
                        Navigation.AddHistoryLevel(ACTION_PTN_MENU_271, FormMode.List);
                        CSGenio.framework.Audit.registAction(UserContext.Current.User, Resources.Resources.MENU01948 + " " + Navigation.CurrentLevel.Location.ShortDescription());
                    }
				}
            }
...
// USE /[MANUAL PTN MENU_GET 271]/
            ...
            if(isHomePage)
                return PartialView("PTN_Menu_271", model);
            else if (!Request.IsAjaxRequest())
                return View(model);
            else
                return PartialView("PTN_Menu_271_Partial", model);
        }
        ...
        public ActionResult PTN_Menu_611(bool allSelected = false)
        {
			int perPage = CSGenio.framework.Configuration.NrRegDBedit;

            PTN_Menu_611_ViewModel model = new PTN_Menu_611_ViewModel(Navigation);
            bool isHomePage = RouteData.Values.ContainsKey("isHomePage") ? (bool)RouteData.Values["isHomePage"] : false;
            if (isHomePage)
			...
			if (!isHomePage && !Request.IsAjaxRequest())
            {
                if (Navigation.CurrentLevel == null || !ACTION_PTN_MENU_611.IsSameAction(Navigation.CurrentLevel.Location))
                {
                    // reset the selections for this new navigation flow
                    // TODO: This change still requires more testing
                    Navigation.RemoveHistoryLevel(ACTION_PTN_MENU_611);
                    if (Navigation.CurrentLevel.Location.Action != ACTION_PTN_MENU_611.Action)
                    {
                        Navigation.AddHistoryLevel(ACTION_PTN_MENU_611, FormMode.List);
                        CSGenio.framework.Audit.registAction(UserContext.Current.User, Resources.Resources.MENU01948 + " " + Navigation.CurrentLevel.Location.ShortDescription());
                    }
				}
            }
...
// USE /[MANUAL PTN MENU_GET 611]/
            ...
            if(isHomePage)
                return PartialView("PTN_Menu_611", model);
            else if (!Request.IsAjaxRequest())
                return View(model);
            else
                return PartialView("PTN_Menu_611_Partial", model);
        }
        ...
        public ActionResult TBS_Menu_1921(bool allSelected = false)
        {
			int perPage = CSGenio.framework.Configuration.NrRegDBedit;

            TBS_Menu_1921_ViewModel model = new TBS_Menu_1921_ViewModel(Navigation);
            bool isHomePage = RouteData.Values.ContainsKey("isHomePage") ? (bool)RouteData.Values["isHomePage"] : false;
            if (isHomePage)
			...
			if (!isHomePage && !Request.IsAjaxRequest())
            {
                if (Navigation.CurrentLevel == null || !ACTION_TBS_MENU_1921.IsSameAction(Navigation.CurrentLevel.Location))
                {
                    // reset the selections for this new navigation flow
                    // TODO: This change still requires more testing
                    Navigation.RemoveHistoryLevel(ACTION_TBS_MENU_1921);
                    if (Navigation.CurrentLevel.Location.Action != ACTION_TBS_MENU_1921.Action)
                    {
                        Navigation.AddHistoryLevel(ACTION_TBS_MENU_1921, FormMode.List);
                        CSGenio.framework.Audit.registAction(UserContext.Current.User, Resources.Resources.MENU01948 + " " + Navigation.CurrentLevel.Location.ShortDescription());
                    }
				}
            }
...
// USE /[MANUAL TBS MENU_GET 1921]/
            ...
            if(isHomePage)
                return PartialView("TBS_Menu_1921", model);
            else if (!Request.IsAjaxRequest())
                return View(model);
            else
                return PartialView("TBS_Menu_1921_Partial", model);
        }
