...
// USE /[MANUAL GQT MANUAL_CONTROLLER INDOC]/
        ...
        public JsonResult ReloadDBEdit(string Identifier, IDictionary<string, object> Values)
        {
            ...
            try { row = Models.Indoc.Find(navigation.GetStrValue("indoc")); }
            catch (Exception)
            {
					...
					case "DENTR___CNTRYCOUNTRY_":	// Field (DB)
                        {
						    ...
						    var model = new Dentr_ViewModel(navigation) { editable = false };
        ...
        public JsonResult GetDependants(string Identifier, string Selected, IDictionary<string, object> Limits)
        {
            ...
            try
            {
				...
				{
					case "DENTR___CNTRYCOUNTRY_":	// Field (DB)
						values = Dentr_ViewModel.GetDependant_DentrTableCntryCountry(Selected, navigation);
						break;
					case "DENTR___CMPNYDESIGNAT":	// Field (DB)
						values = Dentr_ViewModel.GetDependant_DentrTableCmpnyDesignat(Selected, navigation);
						break;
					case "DENTR___PESSONAME____":	// Field (DB)
						values = Dentr_ViewModel.GetDependant_DentrTablePessoName(Selected, navigation);
						break;
					case "DENTR___WARE1WAREHDES":	// Field (DB)
						values = Dentr_ViewModel.GetDependant_DentrTableWare1Warehdes(Selected, navigation);
						break;
					default: break;
				}
		...
		[AuthorizeForUsers]
        [ActionSessionState(System.Web.SessionState.SessionStateBehavior.ReadOnly)]
        public JsonResult RecalculateFormulas_Dentr(Dentr_ViewModel form_data)
        {
            return GenericRecalculateFormulas(form_data, "indoc",
                (primaryKey) => Models.Indoc.Find(primaryKey, "FDENTR"),
