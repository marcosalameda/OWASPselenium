...
// USE /[MANUAL GQT MANUAL_CONTROLLER LNHAG]/
        ...
        public JsonResult ReloadDBEdit(string Identifier, IDictionary<string, object> Values)
        {
            ...
            try { row = Models.Lnhag.Find(navigation.GetStrValue("lnhag")); }
            catch (Exception)
            {
					...
					case "LNHAG___PEDIDNRPEDIDO":	// Field (DB)
                        {
						    ...
						    var model = new Lnhag_ViewModel(navigation) { editable = false };
        ...
        public JsonResult GetDependants(string Identifier, string Selected, IDictionary<string, object> Limits)
        {
            ...
            try
            {
				...
				{
					case "LNHAG___PEDIDNRPEDIDO":	// Field (DB)
						values = Lnhag_ViewModel.GetDependant_LnhagTablePedidNrpedido(Selected, navigation);
						break;
					case "LNHAG___TPEQ1TIPOEQUI":	// Field (DB)
						values = Lnhag_ViewModel.GetDependant_LnhagTableTpeq1Tipoequi(Selected, navigation);
						break;
					default: break;
				}
		...
		[AuthorizeForUsers]
        [ActionSessionState(System.Web.SessionState.SessionStateBehavior.ReadOnly)]
        public JsonResult RecalculateFormulas_Lnhag(Lnhag_ViewModel form_data)
        {
            return GenericRecalculateFormulas(form_data, "lnhag",
                (primaryKey) => Models.Lnhag.Find(primaryKey, "FLNHAG"),
