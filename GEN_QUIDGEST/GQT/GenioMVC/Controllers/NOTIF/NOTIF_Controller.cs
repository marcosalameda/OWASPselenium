...
// USE /[MANUAL GQT MANUAL_CONTROLLER NOTIF]/
        ...
        public JsonResult ReloadDBEdit(string Identifier, IDictionary<string, object> Values)
        {
            ...
            try { row = Models.Notif.Find(navigation.GetStrValue("notif")); }
            catch (Exception)
            {
					...
					case "NOTIF___PESS2NAME____":	// Field (DB)
                        {
						    ...
						    var model = new Notif_ViewModel(navigation) { editable = false };
        ...
        public JsonResult GetDependants(string Identifier, string Selected, IDictionary<string, object> Limits)
        {
            ...
            try
            {
				...
				{
					case "NOTIF___PESS2NAME____":	// Field (DB)
						values = Notif_ViewModel.GetDependant_NotifTablePess2Name(Selected, navigation);
						break;
					default: break;
				}
		...
		[AuthorizeForUsers]
        [ActionSessionState(System.Web.SessionState.SessionStateBehavior.ReadOnly)]
        public JsonResult RecalculateFormulas_Notif(Notif_ViewModel form_data)
        {
            return GenericRecalculateFormulas(form_data, "notif",
                (primaryKey) => Models.Notif.Find(primaryKey, "FNOTIF"),
