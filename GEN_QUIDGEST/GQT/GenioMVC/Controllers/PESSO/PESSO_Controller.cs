...
// USE /[MANUAL GQT MANUAL_CONTROLLER PESSO]/
        ...
        public JsonResult ReloadDBEdit(string Identifier, IDictionary<string, object> Values)
        {
            ...
            try { row = Models.Pesso.Find(navigation.GetStrValue("pesso")); }
            catch (Exception)
            {
			...
			try
			{
				switch ((string.IsNullOrEmpty(Identifier) || Identifier.Length < 5) ? "" : Identifier.Substring(4)) // Substring(4) => to retirar o IFF_ e LED_
				{
					case "EXTERNO_CMPNYDESIGNAT":	// Field (DB)
                        {
						    ...
						    var model = new Externo_ViewModel(navigation) { editable = false };
					...
					case "PESSO___CATEGCATEGORY":	// Field (DB)
                        {
                            row.LoadKeysFormHistory(navigation, navigation.CurrentLevel.Level, false, true, true, true);
						    var model = new Pesso_ViewModel(navigation) { editable = false };
						    model.MapFromModel(row);
                            TryUpdateModel(model); // Map recived values to fields - The 'field' type limits
					...
					case "PESSO1__CATEGCATEGORY":	// Field (DB)
                        {
                            row.LoadKeysFormHistory(navigation, navigation.CurrentLevel.Level, false, true, true, true);
						    var model = new Pesso1_ViewModel(navigation) { editable = false };
						    model.MapFromModel(row);
                            TryUpdateModel(model); // Map recived values to fields - The 'field' type limits
					...
					case "PESSOSEPCATEGCATEGORY":	// Field (DB)
                        {
                            row.LoadKeysFormHistory(navigation, navigation.CurrentLevel.Level, false, true, true, true);
						    var model = new Pessosep_ViewModel(navigation) { editable = false };
						    model.MapFromModel(row);
                            TryUpdateModel(model); // Map recived values to fields - The 'field' type limits
        ...
        public JsonResult GetDependants(string Identifier, string Selected, IDictionary<string, object> Limits)
        {
            ...
            try
            {
				...
				{
					case "EXTERNO_CMPNYDESIGNAT":	// Field (DB)
						values = Externo_ViewModel.GetDependant_ExternoTableCmpnyDesignat(Selected, navigation);
						break;
					case "PESSO___CATEGCATEGORY":	// Field (DB)
						values = Pesso_ViewModel.GetDependant_PessoTableCategCategory(Selected, navigation);
						break;
					case "PESSO___PAIS1COUNTRY_":	// Field (DB)
						values = Pesso_ViewModel.GetDependant_PessoTablePais1Country(Selected, navigation);
						break;
					case "PESSO___CMPNYDESIGNAT":	// Field (DB)
						values = Pesso_ViewModel.GetDependant_PessoTableCmpnyDesignat(Selected, navigation);
						break;
					case "PESSO___REGI1REGIAO__":	// Field (DB)
						values = Pesso_ViewModel.GetDependant_PessoTableRegi1Regiao(Selected, navigation);
						break;
					case "PESSO1__CATEGCATEGORY":	// Field (DB)
						values = Pesso1_ViewModel.GetDependant_Pesso1TableCategCategory(Selected, navigation);
						break;
					case "PESSO1__CMPNYDESIGNAT":	// Field (DB)
						values = Pesso1_ViewModel.GetDependant_Pesso1TableCmpnyDesignat(Selected, navigation);
						break;
					case "PESSO1__REGI1REGIAO__":	// Field (DB)
						values = Pesso1_ViewModel.GetDependant_Pesso1TableRegi1Regiao(Selected, navigation);
						break;
					case "PESSOSEPCATEGCATEGORY":	// Field (DB)
						values = Pessosep_ViewModel.GetDependant_PessosepTableCategCategory(Selected, navigation);
						break;
					case "PESSOS00CMPNYDESIGNAT":	// Field (DB)
						values = Pessosep_ViewModel.GetDependant_Pessos00TableCmpnyDesignat(Selected, navigation);
						break;
					default: break;
				}
		...
		[AuthorizeForUsers]
        [ActionSessionState(System.Web.SessionState.SessionStateBehavior.ReadOnly)]
        public JsonResult RecalculateFormulas_Externo(Externo_ViewModel form_data)
        {
            return GenericRecalculateFormulas(form_data, "pesso",
                (primaryKey) => Models.Pesso.Find(primaryKey, "FEXTERNO"),
		...
		[AuthorizeForUsers]
        [ActionSessionState(System.Web.SessionState.SessionStateBehavior.ReadOnly)]
        public JsonResult RecalculateFormulas_Pesso(Pesso_ViewModel form_data)
        {
            return GenericRecalculateFormulas(form_data, "pesso",
                (primaryKey) => Models.Pesso.Find(primaryKey, "FPESSO"),
                (model) => form_data.MapToModel(model as Models.Pesso)
            );
        }
		...
		[AuthorizeForUsers]
        [ActionSessionState(System.Web.SessionState.SessionStateBehavior.ReadOnly)]
        public JsonResult RecalculateFormulas_Pesso1(Pesso1_ViewModel form_data)
        {
            return GenericRecalculateFormulas(form_data, "pesso",
                (primaryKey) => Models.Pesso.Find(primaryKey, "FPESSO1"),
                (model) => form_data.MapToModel(model as Models.Pesso)
            );
        }
		...
		[AuthorizeForUsers]
        [ActionSessionState(System.Web.SessionState.SessionStateBehavior.ReadOnly)]
        public JsonResult RecalculateFormulas_Pessosep(Pessosep_ViewModel form_data)
        {
            return GenericRecalculateFormulas(form_data, "pesso",
                (primaryKey) => Models.Pesso.Find(primaryKey, "FPESSOSEP"),
                (model) => form_data.MapToModel(model as Models.Pesso)
            );
        }
