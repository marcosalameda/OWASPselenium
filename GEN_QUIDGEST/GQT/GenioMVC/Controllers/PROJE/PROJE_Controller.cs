...
// USE /[MANUAL GQT MANUAL_CONTROLLER PROJE]/
        ...
        public JsonResult ReloadDBEdit(string Identifier, IDictionary<string, object> Values)
        {
            ...
            try { row = Models.Proje.Find(navigation.GetStrValue("proje")); }
            catch (Exception)
            {
					...
					case "PROJE___YEAR1YEAR____":	// Field (DB)
                        {
						    ...
						    var model = new Proje_ViewModel(navigation) { editable = false };
        ...
        public JsonResult GetDependants(string Identifier, string Selected, IDictionary<string, object> Limits)
        {
            ...
            try
            {
				...
				{
					case "PROJE___YEAR1YEAR____":	// Field (DB)
						values = Proje_ViewModel.GetDependant_ProjeTableYear1Year(Selected, navigation);
						break;
					default: break;
				}
		...
		[AuthorizeForUsers]
        [ActionSessionState(System.Web.SessionState.SessionStateBehavior.ReadOnly)]
        public JsonResult RecalculateFormulas_Proje(Proje_ViewModel form_data)
        {
            return GenericRecalculateFormulas(form_data, "proje",
                (primaryKey) => Models.Proje.Find(primaryKey, "FPROJE"),
