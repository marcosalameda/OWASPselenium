...
// USE /[MANUAL GQT MANUAL_CONTROLLER PWCOM]/
        ...
        public JsonResult ReloadDBEdit(string Identifier, IDictionary<string, object> Values)
        {
            ...
            try { row = Models.Pwcom.Find(navigation.GetStrValue("pwcom")); }
            catch (Exception)
            {
					...
					case "PWCOM___PSW__NOME____":	// Field (DB)
                        {
						    ...
						    var model = new Pwcom_ViewModel(navigation) { editable = false };
        ...
        public JsonResult GetDependants(string Identifier, string Selected, IDictionary<string, object> Limits)
        {
            ...
            try
            {
				...
				{
					case "PWCOM___PSW__NOME____":	// Field (DB)
						values = Pwcom_ViewModel.GetDependant_PwcomTablePswNome(Selected, navigation);
						break;
					case "PWCOM___PESS1NAME____":	// Field (DB)
						values = Pwcom_ViewModel.GetDependant_PwcomTablePess1Name(Selected, navigation);
						break;
					default: break;
				}
		...
		[AuthorizeForUsers]
        [ActionSessionState(System.Web.SessionState.SessionStateBehavior.ReadOnly)]
        public JsonResult RecalculateFormulas_Pwcom(Pwcom_ViewModel form_data)
        {
            return GenericRecalculateFormulas(form_data, "pwcom",
                (primaryKey) => Models.Pwcom.Find(primaryKey, "FPWCOM"),
