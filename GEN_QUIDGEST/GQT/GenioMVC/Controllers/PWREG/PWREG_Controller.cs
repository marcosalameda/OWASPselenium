...
// USE /[MANUAL GQT MANUAL_CONTROLLER PWREG]/
        ...
        public JsonResult ReloadDBEdit(string Identifier, IDictionary<string, object> Values)
        {
            ...
            try { row = Models.Pwreg.Find(navigation.GetStrValue("pwreg")); }
            catch (Exception)
            {
					...
					case "PWREG___PSW__NOME____":	// Field (DB)
                        {
						    ...
						    var model = new Pwreg_ViewModel(navigation) { editable = false };
        ...
        public JsonResult GetDependants(string Identifier, string Selected, IDictionary<string, object> Limits)
        {
            ...
            try
            {
				...
				{
					case "PWREG___PSW__NOME____":	// Field (DB)
						values = Pwreg_ViewModel.GetDependant_PwregTablePswNome(Selected, navigation);
						break;
					case "PWREG___REGIOREGIAO__":	// Field (DB)
						values = Pwreg_ViewModel.GetDependant_PwregTableRegioRegiao(Selected, navigation);
						break;
					default: break;
				}
		...
		[AuthorizeForUsers]
        [ActionSessionState(System.Web.SessionState.SessionStateBehavior.ReadOnly)]
        public JsonResult RecalculateFormulas_Pwreg(Pwreg_ViewModel form_data)
        {
            return GenericRecalculateFormulas(form_data, "pwreg",
                (primaryKey) => Models.Pwreg.Find(primaryKey, "FPWREG"),
