...
// USE /[MANUAL GQT MANUAL_CONTROLLER REGIO]/
        ...
        public JsonResult GetDependants(string Identifier, string Selected, IDictionary<string, object> Limits)
        {
            ...
            try
            {
				...
				{
					case "REGIA___CNTRYCOUNTRY_":	// Field (DB)
						values = Regia_ViewModel.GetDependant_RegiaTableCntryCountry(Selected, navigation);
						break;
					case "REGIA_MLCNTRYCOUNTRY_":	// Field (DB)
						values = Regia_ml_ViewModel.GetDependant_Regia_mlTableCntryCountry(Selected, navigation);
						break;
					case "REGIA_MLPAIS1COUNTRY_":	// Field (DB)
						values = Regia_ml_ViewModel.GetDependant_Regia_mlTablePais1Country(Selected, navigation);
						break;
					case "REGIA_ONCNTRYCOUNTRY_":	// Field (DB)
						values = Regia_on_ViewModel.GetDependant_Regia_onTableCntryCountry(Selected, navigation);
						break;
					case "REGIA_ONPAIS1COUNTRY_":	// Field (DB)
						values = Regia_on_ViewModel.GetDependant_Regia_onTablePais1Country(Selected, navigation);
						break;
					default: break;
				}
		...
		[AuthorizeForUsers]
        [ActionSessionState(System.Web.SessionState.SessionStateBehavior.ReadOnly)]
        public JsonResult RecalculateFormulas_Regia(Regia_ViewModel form_data)
        {
            return GenericRecalculateFormulas(form_data, "regio",
                (primaryKey) => Models.Regio.Find(primaryKey, "FREGIA"),
		...
		[AuthorizeForUsers]
        [ActionSessionState(System.Web.SessionState.SessionStateBehavior.ReadOnly)]
        public JsonResult RecalculateFormulas_Regia_ml(Regia_ml_ViewModel form_data)
        {
            return GenericRecalculateFormulas(form_data, "regio",
                (primaryKey) => Models.Regio.Find(primaryKey, "FREGIA_ML"),
                (model) => form_data.MapToModel(model as Models.Regio)
            );
        }
		...
		[AuthorizeForUsers]
        [ActionSessionState(System.Web.SessionState.SessionStateBehavior.ReadOnly)]
        public JsonResult RecalculateFormulas_Regia_on(Regia_on_ViewModel form_data)
        {
            return GenericRecalculateFormulas(form_data, "regio",
                (primaryKey) => Models.Regio.Find(primaryKey, "FREGIA_ON"),
                (model) => form_data.MapToModel(model as Models.Regio)
            );
        }
