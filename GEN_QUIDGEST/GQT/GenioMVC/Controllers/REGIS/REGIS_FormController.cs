...
namespace GenioMVC.Controllers
{
	public partial class RegisController : ControllerBase
	{
		...
		[AuthorizeForUsers]
		public ActionResult ChangeFormMode_Regis(string mode)
		{
			var _mode = Navigation.CurrentLevel.Location;
			var id = Navigation.GetStrValue("regis");
			...
			switch (mode)
			{
				case "show":
					if (m.Contains("v"))
						_mode = ACTION_REGIS_SHOW;
					break;
				case "delete":
					if (m.Contains("a"))
						_mode = ACTION_REGIS_DELETE;
					break;
				case "edit":
					if (m.Contains("e"))
						_mode = ACTION_REGIS_EDIT;
					break;
				case "duplicate":
					if (m.Contains("d"))
						_mode = ACTION_REGIS_DUPLICATE;
					break;
				case "new":
					if (m.Contains("i"))
						_mode = ACTION_REGIS_NEW;
					break;
			}
...
// USE /[MANUAL GQT CONTROLLER_SHOW REGIS]/
		//
		// GET: /Regis/Regis_Show
		[AuthorizeForUsers]
		public ActionResult Regis_Show(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";
			var model = new Regis_ViewModel(Navigation, nestedForm);
			...
			string partialView = qs["partialView"] ?? "Regis"; // MF send the patial view name

			var navigationLocationAction = ACTION_REGIS_SHOW.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			...
			if (!isHomePage && (IsNewLocation(navigationLocationAction) || !string.Equals(Navigation.GetStrValue("regis"), id)))
			{
				Navigation.AddHistoryLevel(navigationLocationAction, FormMode.Show, nestedForm);
				CSGenio.framework.Audit.registAction(UserContext.Current.User, Resources.Resources.FORM54242 + " " + Navigation.CurrentLevel.Location.ShortDescription());
			}

			Navigation.SetValue("regis", id);
...
// USE /[MANUAL GQT BEFORE_LOAD_SHOW REGIS]/
			...
			catch (ModelNotFoundException)
			{
				...
				CSGenio.framework.Log.Error("Regis_Show - GET " + id + " " + e.Message);
				throw;
			}
...
// USE /[MANUAL GQT AFTER_LOAD_SHOW REGIS]/
			...
			if (isHomePage)
			{
				Navigation.SetValue("HomePage", "Regis");
				return PartialView(partialView, model);
			}
			else if (!Request.IsAjaxRequest())
				return View("Regis", model);
			else
				return PartialView(partialView, model);
		}
		...
		[ActionName("Regis_New_Insert")]
		[AuthorizeForUsers]
		[HttpGet]
		public ActionResult Regis_New_Insert()
		{
			...
			return RedirectToAction("Regis_New");
		}

// USE /[MANUAL GQT CONTROLLER_NEW_GET REGIS]/
		//
		// GET: /Regis/Regis_New
		[ActionName("Regis_New")]
		[AuthorizeForUsers]
		[HttpGet]
		public ActionResult Regis_New()
		{
			...
			string partialView = qs["partialView"] ?? "Regis";

			var navigationLocationAction = ACTION_REGIS_NEW.SetRoutedValues(new { m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);

			//FOR: OVERRIDE SKIP IF JUST ONE
			//Allow child form to use "Go Back" to menu list without "skip if only one"
			if (Navigation.OverrideSkipIfJustOne.ContainsKey("REGIS"))
				Navigation.OverrideSkipIfJustOne["REGIS"] = true;
...
// USE /[MANUAL GQT CONTROLLER_NEW_POST REGIS]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Regis_New(Regis_ViewModel model, bool redirect = true)
		{
...
// USE /[MANUAL GQT AFTER_LOAD_NEW_EX REGIS]/
		...
		[AuthorizeForUsers]
		public ActionResult Regis_New_Redirect(bool internalRedirect = false)
		{
			IList<string> warningMsgs = TempData["NEW_WARNINGS_LIST"] as List<string>;
			string saveMsg = TempData["NEW_SAVE_LIST"] as string;

			if (!internalRedirect)
				SuccessMessage(String.IsNullOrEmpty(saveMsg) ? Resources.Resources.REGISTO_CRIADO_COM_S18746 + GetHumanKeyToQMessage("regis", Convert.ToString(Navigation.CurrentLevel.GetEntry("regis"))) : saveMsg);
			...
			if (Convert.ToBoolean(Request.QueryString.Get("repeatInsertion")))
				return RedirectToLocation(ACTION_REGIS_NEW, new { repeatInsertion = true });
...
// USE /[MANUAL GQT CONTROLLER_EDIT_GET REGIS]/
		//
		// GET: /Regis/Regis_Edit
		[AuthorizeForUsers]
		[HttpGet]
		[ActionName("Regis_Edit")]
		public ActionResult Regis_Edit(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";
			string partialView = qs["partialView"] ?? "Regis"; // MF send the partial view name

			//Useful to initialize a record that had to be created by the framework prior to edition (and method New doesnt support this). This way, if the form then is canceled by the user, the record will be automatically deleted
			var isNewEdit = Navigation.GetStrValue("NewEdit") == "true";

			var navigationLocationAction =  isNewEdit ? ACTION_REGIS_NEW.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] }) : ACTION_REGIS_EDIT.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);
...
// USE /[MANUAL GQT BEFORE_LOAD_EDIT REGIS]/
			...
			catch (ModelNotFoundException)
			{
				...
				sp.closeConnection();
				//JFG 05-05-2017 This need to better handled, for now it's handled on webconfig's customErrors section.
				CSGenio.framework.Log.Error("Regis_Edit - " + id + " " + e.Message);
				throw;
			}
...
// USE /[MANUAL GQT CONTROLLER_EDIT_POST REGIS]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Regis_Edit(Regis_ViewModel model, bool redirect = true)
		{
...
// USE /[MANUAL GQT AFTER_LOAD_EDIT_EX REGIS]/
		...
		[AuthorizeForUsers]
		public ActionResult Regis_Edit_Redirect(bool internalRedirect = false)
		{
			IList<string> warningMsgs = TempData["EDIT_WARNINGS_LIST"] as List<string>;
			string saveMsg = TempData["NEW_SAVE_LIST"] as string;

			if (!internalRedirect || !Request.IsAjaxRequest())
				SuccessMessage(String.IsNullOrEmpty(saveMsg) ? Resources.Resources.ALTERACOES_EFETUADAS10166 + GetHumanKeyToQMessage("regis", Convert.ToString(Navigation.CurrentLevel.GetEntry("regis"))) : saveMsg);
...
// USE /[MANUAL GQT CONTROLLER_DELETE_GET REGIS]/
		//
		// GET: /Regis/Regis_Delete
		[AuthorizeForUsers]
		[HttpGet]
		[ActionName("Regis_Delete")]
		public ActionResult Regis_Delete(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";

			var navigationLocationAction = ACTION_REGIS_DELETE.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);
...
// USE /[MANUAL GQT BEFORE_LOAD_DELETE REGIS]/
			...
			catch (ModelNotFoundException)
			{
				...
				//JFG 05-05-2017 This need to better handled, for now it's handled on webconfig's customErrors section.
				CSGenio.framework.Log.Error("Regis_Delete - " + id + " " + e.Message);
				throw;
			}
...
// USE /[MANUAL GQT AFTER_LOAD_DELETE REGIS]/
			...
			if (!Request.IsAjaxRequest())
				...
				if (permission.Status.Equals(CSGenio.framework.Status.E))
					...
					return PartialView("Regis", model);
		}
...
// USE /[MANUAL GQT CONTROLLER_DELETE_POST REGIS]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Regis_Delete(string id, FormCollection collection)
		{

			var model = new Regis_ViewModel (Navigation, id);
			model.MapFromModel();
...
// USE /[MANUAL GQT CONTROLLER_DUPLICATE_GET REGIS]/
		//
		// GET: /Regis/Regis_Duplicate
		[AuthorizeForUsers]
		[HttpGet]
		public ActionResult Regis_Duplicate(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";
			var model = new Regis_ViewModel(Navigation, nestedForm);
			...
			var navigationLocationAction = ACTION_REGIS_DUPLICATE.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);
...
// USE /[MANUAL GQT AFTER_LOAD_DUPLICATE REGIS]/
					...
					sp.closeTransaction();
					Navigation.CurrentLevel.SetLocation(ACTION_REGIS_DUPLICATE.SetRoutedValues(new { Id = model.ValCodregis }));
...
// USE /[MANUAL GQT CONTROLLER_DUPLICATE_POST REGIS]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Regis_Duplicate(Regis_ViewModel model, bool redirect = true)
		{
			var eventSink = new EventSink()
			{
...
// USE /[MANUAL GQT AFTER_LOAD_DUPLICATE_EX REGIS]/
		...
		[AuthorizeForUsers]
		public ActionResult Regis_Duplicate_Redirect(bool internalRedirect = false)
		{
...
// USE /[MANUAL GQT CONTROLLER_CANCEL_GET REGIS]/
		...
		public ActionResult Regis_Cancel()
		{
			...
			if (Navigation.CurrentLevel.FormMode == FormMode.New || Navigation.CurrentLevel.FormMode == FormMode.Duplicate)
			{
				...
				try
				{
					var model = new GenioMVC.Models.Regis();
...
// USE /[MANUAL GQT AFTER_CANCEL REGIS]/
			...
			else if (Navigation.PreviousLevel != null)
			{
				...
				string previousLevelArea = Navigation.PreviousLevel.Location.Controller?.ToLower() ?? "regis";
		...
		{
			var model = new Regis_ViewModel(Navigation, true);
			model.setModes(Request.QueryString["m"]);
			PersistentSupport sp = UserContext.Current.PersistentSupport;
			var navigationLocationAction = ACTION_REGIS_NEW.SetRoutedValues(new { m = Request.QueryString["m"] });
			CheckLevels(navigationLocationAction);
			...
			return PartialView("MFRegis", model);
		}
		...
		public ActionResult MFRegis_Edit(string id)
		{
			return this.RedirectToAction("Regis_Edit", "Regis", new { id = id, partialView = "MFRegis", nestedForm = "true", multiForm = "true" });
		}
		...
		public ActionResult MFRegis_Cancel(string id)
		{
			//MH - Validate if current level is it correct. Remove all levels that is higher than the currently level.
			CheckLevels(NavigationLocation.Any);

			if (Navigation.CurrentLevel.IsNestedContext &&
				(Navigation.CurrentLevel.Location.Action == ACTION_REGIS_NEW.Action || Navigation.CurrentLevel.Location.Action == ACTION_REGIS_EDIT.Action))
			{
				if (Navigation.CurrentLevel.FormMode == FormMode.New)
				{
					...
					catch (Exception e)
					{
						...
						Log.Error("MFRegis_Cancel - " + exceptionUserMessage);
						ErrorMessage(exceptionUserMessage);
					}
				}
			...
			return this.RedirectToAction("Regis_Show", "Regis", new { id = id, partialView = "MFRegis", nestedForm = "true", multiForm = "true", nav });
		}
		...
		public JsonResult MFRegis_Delete(string id)
		{
			var eventSink = new EventSink()
			{
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Regis_SaveEdit(Regis_ViewModel model)
		{
			var eventSink = new EventSink()
			{
