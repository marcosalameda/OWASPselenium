...
namespace GenioMVC.Controllers
{
	public partial class SaleController : ControllerBase
	{
		...
		[AuthorizeForUsers]
		public ActionResult ChangeFormMode_Vendaw03(string mode)
		{
			...
			switch (mode)
			{
				case "show":
					if (m.Contains("v"))
						_mode = ACTION_VENDAW03_SHOW;
					break;
				case "delete":
					if (m.Contains("a"))
						_mode = ACTION_VENDAW03_DELETE;
					break;
				case "edit":
					if (m.Contains("e"))
						_mode = ACTION_VENDAW03_EDIT;
					break;
				case "duplicate":
					if (m.Contains("d"))
						_mode = ACTION_VENDAW03_DUPLICATE;
					break;
				case "new":
					if (m.Contains("i"))
						_mode = ACTION_VENDAW03_NEW;
					break;
			}
		...
		[AuthorizeForUsers]
		public ActionResult Vendaw03_ModalDBEdit(string partialView)
		{
			Vendaw03_ViewModel model = new Vendaw03_ViewModel();
...
// USE /[MANUAL GQT CONTROLLER_SHOW VENDAW03]/
		//
		// GET: /Sale/Vendaw03_Show
		[AuthorizeForUsers]
		public ActionResult Vendaw03_Show(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";
			var model = new Vendaw03_ViewModel(Navigation, nestedForm);
			model.setModes(Request.QueryString["m"]);
			...
			string partialView = qs["partialView"] ?? "Vendaw03"; // MF send the patial view name

			var navigationLocationAction = ACTION_VENDAW03_SHOW.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
...
// USE /[MANUAL GQT BEFORE_LOAD_SHOW VENDAW03]/
			...
			catch (ModelNotFoundException)
			{
				...
				CSGenio.framework.Log.Error("Vendaw03_Show - GET " + id + " " + e.Message);
				throw;
			}
...
// USE /[MANUAL GQT AFTER_LOAD_SHOW VENDAW03]/
			...
			if (isHomePage)
			{
				Navigation.SetValue("HomePage", "Vendaw03");
				return PartialView(partialView, model);
			}
			else if (!Request.IsAjaxRequest())
				return View("Vendaw03", model);
			else
				return PartialView(partialView, model);
		}
		...
		[ActionName("Vendaw03_New_Insert")]
		[AuthorizeForUsers]
		[HttpGet]
		public ActionResult Vendaw03_New_Insert()
		{
			...
			return RedirectToAction("Vendaw03_New");
		}

// USE /[MANUAL GQT CONTROLLER_NEW_GET VENDAW03]/
		//
		// GET: /Sale/Vendaw03_New
		[ActionName("Vendaw03_New")]
		[AuthorizeForUsers]
		[HttpGet]
		public ActionResult Vendaw03_New()
		{
			...
			string partialView = qs["partialView"] ?? "Vendaw03";

			var navigationLocationAction = ACTION_VENDAW03_NEW.SetRoutedValues(new { m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);

			//FOR: OVERRIDE SKIP IF JUST ONE
			//Allow child form to use "Go Back" to menu list without "skip if only one"
			if (Navigation.OverrideSkipIfJustOne.ContainsKey("VENDAW03"))
				Navigation.OverrideSkipIfJustOne["VENDAW03"] = true;
...
// USE /[MANUAL GQT AFTER_LOAD_NEW VENDAW03]/
			...
			catch (Exception e)
			{
				...
				CSGenio.framework.Log.Error("Vendaw03_New - GET " + e.Message);
...
// USE /[MANUAL GQT CONTROLLER_NEW_POST VENDAW03]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Vendaw03_New(Vendaw03_ViewModel model, bool redirect = true)
		{
...
// USE /[MANUAL GQT AFTER_LOAD_NEW_EX VENDAW03]/
		...
		[AuthorizeForUsers]
		public ActionResult Vendaw03_New_Redirect(bool internalRedirect = false)
		{
			...
			if (Convert.ToBoolean(Request.QueryString.Get("repeatInsertion")))
				return RedirectToLocation(ACTION_VENDAW03_NEW, new { repeatInsertion = true });
...
// USE /[MANUAL GQT CONTROLLER_EDIT_GET VENDAW03]/
		//
		// GET: /Sale/Vendaw03_Edit
		[AuthorizeForUsers]
		[HttpGet]
		[ActionName("Vendaw03_Edit")]
		public ActionResult Vendaw03_Edit(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";
			string partialView = qs["partialView"] ?? "Vendaw03"; // MF send the partial view name

			//Useful to initialize a record that had to be created by the framework prior to edition (and method New doesnt support this). This way, if the form then is canceled by the user, the record will be automatically deleted
			var isNewEdit = Navigation.GetStrValue("NewEdit") == "true";

			var navigationLocationAction =  isNewEdit ? ACTION_VENDAW03_NEW.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] }) : ACTION_VENDAW03_EDIT.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);
...
// USE /[MANUAL GQT BEFORE_LOAD_EDIT VENDAW03]/
			...
			catch (ModelNotFoundException)
			{
				...
				sp.closeConnection();
				//JFG 05-05-2017 This need to better handled, for now it's handled on webconfig's customErrors section.
				CSGenio.framework.Log.Error("Vendaw03_Edit - " + id + " " + e.Message);
				throw;
			}
...
// USE /[MANUAL GQT CONTROLLER_EDIT_POST VENDAW03]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Vendaw03_Edit(Vendaw03_ViewModel model, bool redirect = true)
		{
...
// USE /[MANUAL GQT AFTER_LOAD_EDIT_EX VENDAW03]/
		...
		[AuthorizeForUsers]
		public ActionResult Vendaw03_Edit_Redirect(bool internalRedirect = false)
		{
...
// USE /[MANUAL GQT CONTROLLER_DELETE_GET VENDAW03]/
		//
		// GET: /Sale/Vendaw03_Delete
		[AuthorizeForUsers]
		[HttpGet]
		[ActionName("Vendaw03_Delete")]
		public ActionResult Vendaw03_Delete(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";

			var navigationLocationAction = ACTION_VENDAW03_DELETE.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);
...
// USE /[MANUAL GQT BEFORE_LOAD_DELETE VENDAW03]/
			...
			catch (ModelNotFoundException)
			{
				...
				//JFG 05-05-2017 This need to better handled, for now it's handled on webconfig's customErrors section.
				CSGenio.framework.Log.Error("Vendaw03_Delete - " + id + " " + e.Message);
				throw;
			}
...
// USE /[MANUAL GQT AFTER_LOAD_DELETE VENDAW03]/
			...
			if (!Request.IsAjaxRequest())
				...
				if (permission.Status.Equals(CSGenio.framework.Status.E))
					...
					return PartialView("Vendaw03", model);
		}
...
// USE /[MANUAL GQT CONTROLLER_DELETE_POST VENDAW03]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Vendaw03_Delete(string id, FormCollection collection)
		{

			var model = new Vendaw03_ViewModel (Navigation, id);
			model.MapFromModel();
...
// USE /[MANUAL GQT CONTROLLER_DUPLICATE_GET VENDAW03]/
		//
		// GET: /Sale/Vendaw03_Duplicate
		[AuthorizeForUsers]
		[HttpGet]
		public ActionResult Vendaw03_Duplicate(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";
			var model = new Vendaw03_ViewModel(Navigation, nestedForm);
			...
			var navigationLocationAction = ACTION_VENDAW03_DUPLICATE.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);
...
// USE /[MANUAL GQT AFTER_LOAD_DUPLICATE VENDAW03]/
					...
					sp.closeTransaction();
					Navigation.CurrentLevel.SetLocation(ACTION_VENDAW03_DUPLICATE.SetRoutedValues(new { Id = model.ValCodvenda }));
...
// USE /[MANUAL GQT CONTROLLER_DUPLICATE_POST VENDAW03]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Vendaw03_Duplicate(Vendaw03_ViewModel model, bool redirect = true)
		{
			var eventSink = new EventSink()
			{
...
// USE /[MANUAL GQT AFTER_LOAD_DUPLICATE_EX VENDAW03]/
		...
		[AuthorizeForUsers]
		public ActionResult Vendaw03_Duplicate_Redirect(bool internalRedirect = false)
		{
...
// USE /[MANUAL GQT AFTER_CANCEL VENDAW03]/
		...
		{
			var model = new Vendaw03_ViewModel(Navigation, true);
			model.setModes(Request.QueryString["m"]);
			PersistentSupport sp = UserContext.Current.PersistentSupport;
			var navigationLocationAction = ACTION_VENDAW03_NEW.SetRoutedValues(new { m = Request.QueryString["m"] });
			CheckLevels(navigationLocationAction);
			...
			return PartialView("MFVendaw03", model);
		}
		...
		public ActionResult MFVendaw03_Edit(string id)
		{
			return this.RedirectToAction("Vendaw03_Edit", "Sale", new { id = id, partialView = "MFVendaw03", nestedForm = "true", multiForm = "true" });
		}
		...
		public ActionResult MFVendaw03_Cancel(string id)
		{
			//MH - Validate if current level is it correct. Remove all levels that is higher than the currently level.
			CheckLevels(NavigationLocation.Any);

			if (Navigation.CurrentLevel.IsNestedContext &&
				(Navigation.CurrentLevel.Location.Action == ACTION_VENDAW03_NEW.Action || Navigation.CurrentLevel.Location.Action == ACTION_VENDAW03_EDIT.Action))
			{
				if (Navigation.CurrentLevel.FormMode == FormMode.New)
				{
					...
					catch (Exception e)
					{
						...
						Log.Error("MFVendaw03_Cancel - " + exceptionUserMessage);
						ErrorMessage(exceptionUserMessage);
					}
				}
			...
			return this.RedirectToAction("Vendaw03_Show", "Sale", new { id = id, partialView = "MFVendaw03", nestedForm = "true", multiForm = "true", nav });
		}
		...
		public JsonResult MFVendaw03_Delete(string id)
		{
			var eventSink = new EventSink()
			{
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Vendaw03_SaveEdit(Vendaw03_ViewModel model)
		{
			var eventSink = new EventSink()
			{
