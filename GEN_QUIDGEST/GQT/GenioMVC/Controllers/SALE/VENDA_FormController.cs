...
namespace GenioMVC.Controllers
{
	public partial class SaleController : ControllerBase
	{
		...
		[AuthorizeForUsers]
		public ActionResult ChangeFormMode_Venda(string mode)
		{
			...
			switch (mode)
			{
				case "show":
					if (m.Contains("v"))
						_mode = ACTION_VENDA_SHOW;
					break;
				case "delete":
					if (m.Contains("a"))
						_mode = ACTION_VENDA_DELETE;
					break;
				case "edit":
					if (m.Contains("e"))
						_mode = ACTION_VENDA_EDIT;
					break;
				case "duplicate":
					if (m.Contains("d"))
						_mode = ACTION_VENDA_DUPLICATE;
					break;
				case "new":
					if (m.Contains("i"))
						_mode = ACTION_VENDA_NEW;
					break;
			}
		...
		[AuthorizeForUsers]
		public ActionResult Venda_ModalDBEdit(string partialView)
		{
			Venda_ViewModel model = new Venda_ViewModel();
...
// USE /[MANUAL GQT CONTROLLER_SHOW VENDA]/
		...
		// GET: /Sale/Venda_Show
		[AuthorizeForUsers]
		public ActionResult Venda_Show(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";
			var model = new Venda_ViewModel(Navigation, nestedForm);
			model.setModes(Request.QueryString["m"]);
			...
			string partialView = qs["partialView"] ?? "Venda"; // MF send the patial view name

			var navigationLocationAction = ACTION_VENDA_SHOW.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
...
// USE /[MANUAL GQT BEFORE_LOAD_SHOW VENDA]/
			...
			catch (ModelNotFoundException)
			{
				...
				CSGenio.framework.Log.Error("Venda_Show - GET " + id + " " + e.Message);
				throw;
			}
...
// USE /[MANUAL GQT AFTER_LOAD_SHOW VENDA]/
			...
			if (isHomePage)
			{
				Navigation.SetValue("HomePage", "Venda");
				return PartialView(partialView, model);
			}
		...
		[ActionName("Venda_New_Insert")]
		[AuthorizeForUsers]
		[HttpGet]
		public ActionResult Venda_New_Insert()
		{
			...
			return RedirectToAction("Venda_New");
		}

// USE /[MANUAL GQT CONTROLLER_NEW_GET VENDA]/
		//
		// GET: /Sale/Venda_New
		[ActionName("Venda_New")]
		[AuthorizeForUsers]
		[HttpGet]
		public ActionResult Venda_New()
		{
			...
			string partialView = qs["partialView"] ?? "Venda";

			var navigationLocationAction = ACTION_VENDA_NEW.SetRoutedValues(new { m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);

			//FOR: OVERRIDE SKIP IF JUST ONE
			//Allow child form to use "Go Back" to menu list without "skip if only one"
			if (Navigation.OverrideSkipIfJustOne.ContainsKey("VENDA"))
				Navigation.OverrideSkipIfJustOne["VENDA"] = true;
...
// USE /[MANUAL GQT AFTER_LOAD_NEW VENDA]/
			...
			catch (Exception e)
			{
				...
				CSGenio.framework.Log.Error("Venda_New - GET " + e.Message);
...
// USE /[MANUAL GQT CONTROLLER_NEW_POST VENDA]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Venda_New(Venda_ViewModel model, bool redirect = true)
		{
...
// USE /[MANUAL GQT AFTER_LOAD_NEW_EX VENDA]/
		...
		[AuthorizeForUsers]
		public ActionResult Venda_New_Redirect(bool internalRedirect = false)
		{
			...
			if (Convert.ToBoolean(Request.QueryString.Get("repeatInsertion")))
				return RedirectToLocation(ACTION_VENDA_NEW, new { repeatInsertion = true });
...
// USE /[MANUAL GQT CONTROLLER_EDIT_GET VENDA]/
		...
		[AuthorizeForUsers]
		[HttpGet]
		[ActionName("Venda_Edit")]
		public ActionResult Venda_Edit(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";
			string partialView = qs["partialView"] ?? "Venda"; // MF send the partial view name

			//Useful to initialize a record that had to be created by the framework prior to edition (and method New doesnt support this). This way, if the form then is canceled by the user, the record will be automatically deleted
			var isNewEdit = Navigation.GetStrValue("NewEdit") == "true";

			var navigationLocationAction =  isNewEdit ? ACTION_VENDA_NEW.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] }) : ACTION_VENDA_EDIT.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);
...
// USE /[MANUAL GQT BEFORE_LOAD_EDIT VENDA]/
			...
			catch (ModelNotFoundException)
			{
				...
				sp.closeConnection();
				//JFG 05-05-2017 This need to better handled, for now it's handled on webconfig's customErrors section.
				CSGenio.framework.Log.Error("Venda_Edit - " + id + " " + e.Message);
				throw;
			}
...
// USE /[MANUAL GQT CONTROLLER_EDIT_POST VENDA]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Venda_Edit(Venda_ViewModel model, bool redirect = true)
		{
...
// USE /[MANUAL GQT AFTER_LOAD_EDIT_EX VENDA]/
		...
		[AuthorizeForUsers]
		public ActionResult Venda_Edit_Redirect(bool internalRedirect = false)
		{
...
// USE /[MANUAL GQT CONTROLLER_DELETE_GET VENDA]/
		//
		// GET: /Sale/Venda_Delete
		[AuthorizeForUsers]
		[HttpGet]
		[ActionName("Venda_Delete")]
		public ActionResult Venda_Delete(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";

			var navigationLocationAction = ACTION_VENDA_DELETE.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);
...
// USE /[MANUAL GQT BEFORE_LOAD_DELETE VENDA]/
			...
			catch (ModelNotFoundException)
			{
				...
				//JFG 05-05-2017 This need to better handled, for now it's handled on webconfig's customErrors section.
				CSGenio.framework.Log.Error("Venda_Delete - " + id + " " + e.Message);
				throw;
			}
...
// USE /[MANUAL GQT AFTER_LOAD_DELETE VENDA]/
			...
			if (!Request.IsAjaxRequest())
				...
				if (permission.Status.Equals(CSGenio.framework.Status.E))
					...
					return PartialView("Venda", model);
		}
...
// USE /[MANUAL GQT CONTROLLER_DELETE_POST VENDA]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Venda_Delete(string id, FormCollection collection)
		{

			var model = new Venda_ViewModel (Navigation, id);
			model.MapFromModel();
...
// USE /[MANUAL GQT CONTROLLER_DUPLICATE_GET VENDA]/
		//
		// GET: /Sale/Venda_Duplicate
		[AuthorizeForUsers]
		[HttpGet]
		public ActionResult Venda_Duplicate(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";
			var model = new Venda_ViewModel(Navigation, nestedForm);
			...
			var navigationLocationAction = ACTION_VENDA_DUPLICATE.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);
...
// USE /[MANUAL GQT AFTER_LOAD_DUPLICATE VENDA]/
					...
					sp.closeTransaction();
					Navigation.CurrentLevel.SetLocation(ACTION_VENDA_DUPLICATE.SetRoutedValues(new { Id = model.ValCodvenda }));
...
// USE /[MANUAL GQT CONTROLLER_DUPLICATE_POST VENDA]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Venda_Duplicate(Venda_ViewModel model, bool redirect = true)
		{
			var eventSink = new EventSink()
			{
...
// USE /[MANUAL GQT AFTER_LOAD_DUPLICATE_EX VENDA]/
		...
		[AuthorizeForUsers]
		public ActionResult Venda_Duplicate_Redirect(bool internalRedirect = false)
		{
...
// USE /[MANUAL GQT AFTER_CANCEL VENDA]/
		...
		{
			var model = new Venda_ViewModel(Navigation, true);
			model.setModes(Request.QueryString["m"]);
			PersistentSupport sp = UserContext.Current.PersistentSupport;
			var navigationLocationAction = ACTION_VENDA_NEW.SetRoutedValues(new { m = Request.QueryString["m"] });
			CheckLevels(navigationLocationAction);
			...
			return PartialView("MFVenda", model);
		}
		...
		public ActionResult MFVenda_Edit(string id)
		{
			return this.RedirectToAction("Venda_Edit", "Sale", new { id = id, partialView = "MFVenda", nestedForm = "true", multiForm = "true" });
		}
		...
		public ActionResult MFVenda_Cancel(string id)
		{
			//MH - Validate if current level is it correct. Remove all levels that is higher than the currently level.
			CheckLevels(NavigationLocation.Any);

			if (Navigation.CurrentLevel.IsNestedContext &&
				(Navigation.CurrentLevel.Location.Action == ACTION_VENDA_NEW.Action || Navigation.CurrentLevel.Location.Action == ACTION_VENDA_EDIT.Action))
			{
				if (Navigation.CurrentLevel.FormMode == FormMode.New)
				{
					...
					catch (Exception e)
					{
						...
						Log.Error("MFVenda_Cancel - " + exceptionUserMessage);
						ErrorMessage(exceptionUserMessage);
					}
				}
			...
			return this.RedirectToAction("Venda_Show", "Sale", new { id = id, partialView = "MFVenda", nestedForm = "true", multiForm = "true", nav });
		}
		...
		public JsonResult MFVenda_Delete(string id)
		{
			var eventSink = new EventSink()
			{
		...
		public ActionResult Venda_OrganValOrganiza(string id, string partialView,  IDictionary<string, string> Limits)
		{
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Venda_SaveEdit(Venda_ViewModel model)
		{
			var eventSink = new EventSink()
			{
