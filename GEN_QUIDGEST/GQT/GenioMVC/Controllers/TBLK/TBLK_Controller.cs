...
// USE /[MANUAL GQT MANUAL_CONTROLLER TBLK]/
        ...
        public JsonResult ReloadDBEdit(string Identifier, IDictionary<string, object> Values)
        {
            ...
            try { row = Models.Tblk.Find(navigation.GetStrValue("tblk")); }
            catch (Exception)
            {
					...
					case "TBLK____GRPB_NAME____":	// Field (DB)
                        {
						    ...
						    var model = new Tblk_ViewModel(navigation) { editable = false };
        ...
        public JsonResult GetDependants(string Identifier, string Selected, IDictionary<string, object> Limits)
        {
            ...
            try
            {
				...
				{
					case "TBLK____GRPB_NAME____":	// Field (DB)
						values = Tblk_ViewModel.GetDependant_TblkTableGrpbName(Selected, navigation);
						break;
					case "TBLK____TRSB_NAME____":	// Field (DB)
						values = Tblk_ViewModel.GetDependant_TblkTableTrsbName(Selected, navigation);
						break;
					default: break;
				}
		...
		[AuthorizeForUsers]
        [ActionSessionState(System.Web.SessionState.SessionStateBehavior.ReadOnly)]
        public JsonResult RecalculateFormulas_Tblk(Tblk_ViewModel form_data)
        {
            return GenericRecalculateFormulas(form_data, "tblk",
                (primaryKey) => Models.Tblk.Find(primaryKey, "FTBLK"),
