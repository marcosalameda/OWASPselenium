...
namespace GenioMVC.Controllers
{
    public partial class TblkController : ControllerBase
    {
        ...
        public ActionResult PTN_Menu_3161(bool allSelected = false)
        {
			int perPage = CSGenio.framework.Configuration.NrRegDBedit;

            PTN_Menu_3161_ViewModel model = new PTN_Menu_3161_ViewModel(Navigation);
            ...
            {
                if (Navigation.CurrentLevel == null || !ACTION_PTN_MENU_3161.IsSameAction(Navigation.CurrentLevel.Location))
                {
                    // reset the selections for this new navigation flow
                    // TODO: This change still requires more testing
                    Navigation.RemoveHistoryLevel(ACTION_PTN_MENU_3161);
                    if (Navigation.CurrentLevel.Location.Action != ACTION_PTN_MENU_3161.Action)
                    {
                        Navigation.AddHistoryLevel(ACTION_PTN_MENU_3161, FormMode.List);
...
// USE /[MANUAL PTN MENU_GET 3161]/
            ...
            if(isHomePage)
                return PartialView("PTN_Menu_3161", model);
            else if (!Request.IsAjaxRequest())
                return View(model);
            else
                return PartialView("PTN_Menu_3161_Partial", model);
        }
