...
// USE /[MANUAL GQT MANUAL_CONTROLLER TRADU]/
        ...
        public JsonResult ReloadDBEdit(string Identifier, IDictionary<string, object> Values)
        {
            ...
            try { row = Models.Tradu.Find(navigation.GetStrValue("tradu")); }
            catch (Exception)
            {
					...
					case "TRADU___LANG1LANGUA__":	// Field (DB)
                        {
						    ...
						    var model = new Tradu_ViewModel(navigation) { editable = false };
        ...
        public JsonResult GetDependants(string Identifier, string Selected, IDictionary<string, object> Limits)
        {
            ...
            try
            {
				...
				{
					case "TRADU___LANG1LANGUA__":	// Field (DB)
						values = Tradu_ViewModel.GetDependant_TraduTableLang1Langua(Selected, navigation);
						break;
					case "TRADU___LANG2LANGUA__":	// Field (DB)
						values = Tradu_ViewModel.GetDependant_TraduTableLang2Langua(Selected, navigation);
						break;
					default: break;
				}
		...
		[AuthorizeForUsers]
        [ActionSessionState(System.Web.SessionState.SessionStateBehavior.ReadOnly)]
        public JsonResult RecalculateFormulas_Tradu(Tradu_ViewModel form_data)
        {
            return GenericRecalculateFormulas(form_data, "tradu",
                (primaryKey) => Models.Tradu.Find(primaryKey, "FTRADU"),
