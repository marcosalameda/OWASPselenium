...
// USE /[MANUAL GQT MANUAL_CONTROLLER VISIT]/
        ...
        public JsonResult ReloadDBEdit(string Identifier, IDictionary<string, object> Values)
        {
            ...
            try { row = Models.Visit.Find(navigation.GetStrValue("visit")); }
            catch (Exception)
            {
                CSGenio.framework.Log.Error("ReloadDBEdit - " + Identifier + " Not found Model visit");
            }
            if(row == null)
            {
                row = new Models.Visit();
			...
			try
			{
				switch ((string.IsNullOrEmpty(Identifier) || Identifier.Length < 5) ? "" : Identifier.Substring(4)) // Substring(4) => to retirar o IFF_ e LED_
				{
					case "VISIT___EQUIPREGISTNR":	// Field (DB)
                        {
						    ...
						    var model = new Visit_ViewModel(navigation) { editable = false };
					...
					case "VISIT2__EQUIPREGISTNR":	// Field (DB)
                        {
                            row.LoadKeysFormHistory(navigation, navigation.CurrentLevel.Level, false, true, true, true);
						    var model = new Visit2_ViewModel(navigation) { editable = false };
						    model.MapFromModel(row);
                            TryUpdateModel(model); // Map recived values to fields - The 'field' type limits
        ...
        public JsonResult GetDependants(string Identifier, string Selected, IDictionary<string, object> Limits)
        {
            ...
            try
            {
				...
				{
					case "VISIT___EQUIPREGISTNR":	// Field (DB)
						values = Visit_ViewModel.GetDependant_VisitTableEquipRegistnr(Selected, navigation);
						break;
					case "VISIT2__EQUIPREGISTNR":	// Field (DB)
						values = Visit2_ViewModel.GetDependant_Visit2TableEquipRegistnr(Selected, navigation);
						break;
					default: break;
				}
		...
		[AuthorizeForUsers]
        [ActionSessionState(System.Web.SessionState.SessionStateBehavior.ReadOnly)]
        public JsonResult RecalculateFormulas_Visit(Visit_ViewModel form_data)
        {
            return GenericRecalculateFormulas(form_data, "visit",
                (primaryKey) => Models.Visit.Find(primaryKey, "FVISIT"),
                (model) => form_data.MapToModel(model as Models.Visit)
            );
        }
		...
		[AuthorizeForUsers]
        [ActionSessionState(System.Web.SessionState.SessionStateBehavior.ReadOnly)]
        public JsonResult RecalculateFormulas_Visit2(Visit2_ViewModel form_data)
        {
            return GenericRecalculateFormulas(form_data, "visit",
                (primaryKey) => Models.Visit.Find(primaryKey, "FVISIT2"),
                (model) => form_data.MapToModel(model as Models.Visit)
            );
        }
