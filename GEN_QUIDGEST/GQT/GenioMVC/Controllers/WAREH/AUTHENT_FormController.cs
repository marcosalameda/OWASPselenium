...
namespace GenioMVC.Controllers
{
	public partial class WarehController : ControllerBase
	{
		...
		[AuthorizeForUsers]
		public ActionResult ChangeFormMode_Authent(string mode)
		{
			...
			switch (mode)
			{
				case "show":
					if (m.Contains("v"))
						_mode = ACTION_AUTHENT_SHOW;
					break;
				case "delete":
					if (m.Contains("a"))
						_mode = ACTION_AUTHENT_DELETE;
					break;
				case "edit":
					if (m.Contains("e"))
						_mode = ACTION_AUTHENT_EDIT;
					break;
				case "duplicate":
					if (m.Contains("d"))
						_mode = ACTION_AUTHENT_DUPLICATE;
					break;
				case "new":
					if (m.Contains("i"))
						_mode = ACTION_AUTHENT_NEW;
					break;
			}
...
// USE /[MANUAL GQT CONTROLLER_SHOW AUTHENT]/
		//
		// GET: /Wareh/Authent_Show
		[AuthorizeForUsers]
		public ActionResult Authent_Show(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";
			var model = new Authent_ViewModel(Navigation, nestedForm);
			...
			string partialView = qs["partialView"] ?? "Authent"; // MF send the patial view name

			var navigationLocationAction = ACTION_AUTHENT_SHOW.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
...
// USE /[MANUAL GQT BEFORE_LOAD_SHOW AUTHENT]/
			...
			catch (ModelNotFoundException)
			{
				...
				CSGenio.framework.Log.Error("Authent_Show - GET " + id + " " + e.Message);
				throw;
			}
...
// USE /[MANUAL GQT AFTER_LOAD_SHOW AUTHENT]/
			...
			if (isHomePage)
			{
				Navigation.SetValue("HomePage", "Authent");
				return PartialView(partialView, model);
			}
			else if (!Request.IsAjaxRequest())
				return View("Authent", model);
			else
				return PartialView(partialView, model);
		}
		...
		[ActionName("Authent_New_Insert")]
		[AuthorizeForUsers]
		[HttpGet]
		public ActionResult Authent_New_Insert()
		{
			...
			return RedirectToAction("Authent_New");
		}

// USE /[MANUAL GQT CONTROLLER_NEW_GET AUTHENT]/
		//
		// GET: /Wareh/Authent_New
		[ActionName("Authent_New")]
		[AuthorizeForUsers]
		[HttpGet]
		public ActionResult Authent_New()
		{
			...
			string partialView = qs["partialView"] ?? "Authent";

			var navigationLocationAction = ACTION_AUTHENT_NEW.SetRoutedValues(new { m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);
			...
			if (Navigation.OverrideSkipIfJustOne.ContainsKey("AUTHENT"))
				Navigation.OverrideSkipIfJustOne["AUTHENT"] = true;
...
// USE /[MANUAL GQT AFTER_LOAD_NEW AUTHENT]/
			...
			catch (Exception e)
			{
				...
				CSGenio.framework.Log.Error("Authent_New - GET " + e.Message);
...
// USE /[MANUAL GQT CONTROLLER_NEW_POST AUTHENT]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Authent_New(Authent_ViewModel model, bool redirect = true)
		{
...
// USE /[MANUAL GQT AFTER_LOAD_NEW_EX AUTHENT]/
		...
		[AuthorizeForUsers]
		public ActionResult Authent_New_Redirect(bool internalRedirect = false)
		{
			...
			if (Convert.ToBoolean(Request.QueryString.Get("repeatInsertion")))
				return RedirectToLocation(ACTION_AUTHENT_NEW, new { repeatInsertion = true });
...
// USE /[MANUAL GQT CONTROLLER_EDIT_GET AUTHENT]/
		//
		// GET: /Wareh/Authent_Edit
		[AuthorizeForUsers]
		[HttpGet]
		[ActionName("Authent_Edit")]
		public ActionResult Authent_Edit(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";
			string partialView = qs["partialView"] ?? "Authent"; // MF send the partial view name

			//Useful to initialize a record that had to be created by the framework prior to edition (and method New doesnt support this). This way, if the form then is canceled by the user, the record will be automatically deleted
			var isNewEdit = Navigation.GetStrValue("NewEdit") == "true";

			var navigationLocationAction =  isNewEdit ? ACTION_AUTHENT_NEW.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] }) : ACTION_AUTHENT_EDIT.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);
...
// USE /[MANUAL GQT BEFORE_LOAD_EDIT AUTHENT]/
			...
			catch (ModelNotFoundException)
			{
				...
				sp.closeConnection();
				//JFG 05-05-2017 This need to better handled, for now it's handled on webconfig's customErrors section.
				CSGenio.framework.Log.Error("Authent_Edit - " + id + " " + e.Message);
				throw;
			}
...
// USE /[MANUAL GQT CONTROLLER_EDIT_POST AUTHENT]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Authent_Edit(Authent_ViewModel model, bool redirect = true)
		{
...
// USE /[MANUAL GQT AFTER_LOAD_EDIT_EX AUTHENT]/
		...
		[AuthorizeForUsers]
		public ActionResult Authent_Edit_Redirect(bool internalRedirect = false)
		{
...
// USE /[MANUAL GQT CONTROLLER_DELETE_GET AUTHENT]/
		//
		// GET: /Wareh/Authent_Delete
		[AuthorizeForUsers]
		[HttpGet]
		[ActionName("Authent_Delete")]
		public ActionResult Authent_Delete(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";

			var navigationLocationAction = ACTION_AUTHENT_DELETE.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);
...
// USE /[MANUAL GQT BEFORE_LOAD_DELETE AUTHENT]/
			...
			catch (ModelNotFoundException)
			{
				...
				//JFG 05-05-2017 This need to better handled, for now it's handled on webconfig's customErrors section.
				CSGenio.framework.Log.Error("Authent_Delete - " + id + " " + e.Message);
				throw;
			}
...
// USE /[MANUAL GQT AFTER_LOAD_DELETE AUTHENT]/
			...
			if (!Request.IsAjaxRequest())
				...
				if (permission.Status.Equals(CSGenio.framework.Status.E))
					...
					return PartialView("Authent", model);
		}
...
// USE /[MANUAL GQT CONTROLLER_DELETE_POST AUTHENT]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Authent_Delete(string id, FormCollection collection)
		{

			var model = new Authent_ViewModel (Navigation, id);
			model.MapFromModel();
...
// USE /[MANUAL GQT CONTROLLER_DUPLICATE_GET AUTHENT]/
		//
		// GET: /Wareh/Authent_Duplicate
		[AuthorizeForUsers]
		[HttpGet]
		public ActionResult Authent_Duplicate(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";
			var model = new Authent_ViewModel(Navigation, nestedForm);
			...
			var navigationLocationAction = ACTION_AUTHENT_DUPLICATE.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);
...
// USE /[MANUAL GQT AFTER_LOAD_DUPLICATE AUTHENT]/
					...
					sp.closeTransaction();
					Navigation.CurrentLevel.SetLocation(ACTION_AUTHENT_DUPLICATE.SetRoutedValues(new { Id = model.ValCodwareh }));
...
// USE /[MANUAL GQT CONTROLLER_DUPLICATE_POST AUTHENT]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Authent_Duplicate(Authent_ViewModel model, bool redirect = true)
		{
			var eventSink = new EventSink()
			{
...
// USE /[MANUAL GQT AFTER_LOAD_DUPLICATE_EX AUTHENT]/
		...
		[AuthorizeForUsers]
		public ActionResult Authent_Duplicate_Redirect(bool internalRedirect = false)
		{
...
// USE /[MANUAL GQT AFTER_CANCEL AUTHENT]/
		...
		{
			var model = new Authent_ViewModel(Navigation, true);
			model.setModes(Request.QueryString["m"]);
			PersistentSupport sp = UserContext.Current.PersistentSupport;
			var navigationLocationAction = ACTION_AUTHENT_NEW.SetRoutedValues(new { m = Request.QueryString["m"] });
			CheckLevels(navigationLocationAction);
			...
			return PartialView("MFAuthent", model);
		}
		...
		public ActionResult MFAuthent_Edit(string id)
		{
			return this.RedirectToAction("Authent_Edit", "Wareh", new { id = id, partialView = "MFAuthent", nestedForm = "true", multiForm = "true" });
		}
		...
		public ActionResult MFAuthent_Cancel(string id)
		{
			//MH - Validate if current level is it correct. Remove all levels that is higher than the currently level.
			CheckLevels(NavigationLocation.Any);

			if (Navigation.CurrentLevel.IsNestedContext &&
				(Navigation.CurrentLevel.Location.Action == ACTION_AUTHENT_NEW.Action || Navigation.CurrentLevel.Location.Action == ACTION_AUTHENT_EDIT.Action))
			{
				if (Navigation.CurrentLevel.FormMode == FormMode.New)
				{
					...
					catch (Exception e)
					{
						...
						Log.Error("MFAuthent_Cancel - " + exceptionUserMessage);
						ErrorMessage(exceptionUserMessage);
					}
				}
			...
			return this.RedirectToAction("Authent_Show", "Wareh", new { id = id, partialView = "MFAuthent", nestedForm = "true", multiForm = "true", nav });
		}
		...
		public JsonResult MFAuthent_Delete(string id)
		{
			var eventSink = new EventSink()
			{
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Authent_SaveEdit(Authent_ViewModel model)
		{
			var eventSink = new EventSink()
			{
