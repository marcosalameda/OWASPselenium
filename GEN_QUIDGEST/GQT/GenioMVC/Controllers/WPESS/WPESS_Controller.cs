...
// USE /[MANUAL GQT MANUAL_CONTROLLER WPESS]/
        ...
        public JsonResult GetDependants(string Identifier, string Selected, IDictionary<string, object> Limits)
        {
            ...
            try
            {
				...
				{
					case "ARMAPESSWAREHWAREHDES":	// Field (DB)
						values = Armapess_ViewModel.GetDependant_ArmapessTableWarehWarehdes(Selected, navigation);
						break;
					case "PESSPOP_WAREHWAREHDES":	// Field (DB)
						values = Pesspop_ViewModel.GetDependant_PesspopTableWarehWarehdes(Selected, navigation);
						break;
					default: break;
				}
		...
		[AuthorizeForUsers]
        [ActionSessionState(System.Web.SessionState.SessionStateBehavior.ReadOnly)]
        public JsonResult RecalculateFormulas_Armapess(Armapess_ViewModel form_data)
        {
            return GenericRecalculateFormulas(form_data, "wpess",
                (primaryKey) => Models.Wpess.Find(primaryKey, "FARMAPESS"),
		...
		[AuthorizeForUsers]
        [ActionSessionState(System.Web.SessionState.SessionStateBehavior.ReadOnly)]
        public JsonResult RecalculateFormulas_Imgmagn(Imgmagn_ViewModel form_data)
        {
            return GenericRecalculateFormulas(form_data, "wpess",
                (primaryKey) => Models.Wpess.Find(primaryKey, "FIMGMAGN"),
                (model) => form_data.MapToModel(model as Models.Wpess)
            );
        }
		...
		[AuthorizeForUsers]
        [ActionSessionState(System.Web.SessionState.SessionStateBehavior.ReadOnly)]
        public JsonResult RecalculateFormulas_Pesspop(Pesspop_ViewModel form_data)
        {
            return GenericRecalculateFormulas(form_data, "wpess",
                (primaryKey) => Models.Wpess.Find(primaryKey, "FPESSPOP"),
                (model) => form_data.MapToModel(model as Models.Wpess)
            );
        }
