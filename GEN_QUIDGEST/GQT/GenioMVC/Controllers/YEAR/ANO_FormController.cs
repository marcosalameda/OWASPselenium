...
namespace GenioMVC.Controllers
{
	public partial class YearController : ControllerBase
	{
		...
		[AuthorizeForUsers]
		public ActionResult ChangeFormMode_Ano(string mode)
		{
			var _mode = Navigation.CurrentLevel.Location;
			var id = Navigation.GetStrValue("year");
			...
			switch (mode)
			{
				case "show":
					if (m.Contains("v"))
						_mode = ACTION_ANO_SHOW;
					break;
				case "delete":
					if (m.Contains("a"))
						_mode = ACTION_ANO_DELETE;
					break;
				case "edit":
					if (m.Contains("e"))
						_mode = ACTION_ANO_EDIT;
					break;
				case "duplicate":
					if (m.Contains("d"))
						_mode = ACTION_ANO_DUPLICATE;
					break;
				case "new":
					if (m.Contains("i"))
						_mode = ACTION_ANO_NEW;
					break;
			}
...
// USE /[MANUAL GQT CONTROLLER_SHOW ANO]/
		...
		// GET: /Year/Ano_Show
		[AuthorizeForUsers]
		public ActionResult Ano_Show(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";
			var model = new Ano_ViewModel(Navigation, nestedForm);
			...
			string partialView = qs["partialView"] ?? "Ano"; // MF send the patial view name

			var navigationLocationAction = ACTION_ANO_SHOW.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			...
			if (!isHomePage && (IsNewLocation(navigationLocationAction) || !string.Equals(Navigation.GetStrValue("year"), id)))
			{
				Navigation.AddHistoryLevel(navigationLocationAction, FormMode.Show, nestedForm);
				CSGenio.framework.Audit.registAction(UserContext.Current.User, Resources.Resources.FORM54242 + " " + Navigation.CurrentLevel.Location.ShortDescription());
			}

			Navigation.SetValue("year", id);
...
// USE /[MANUAL GQT BEFORE_LOAD_SHOW ANO]/
			...
			catch (ModelNotFoundException)
			{
				...
				CSGenio.framework.Log.Error("Ano_Show - GET " + id + " " + e.Message);
				throw;
			}
...
// USE /[MANUAL GQT AFTER_LOAD_SHOW ANO]/
			...
			if (isHomePage)
			{
				Navigation.SetValue("HomePage", "Ano");
				return PartialView(partialView, model);
			}
			else if (!Request.IsAjaxRequest())
				return View("Ano", model);
			else
				return PartialView(partialView, model);
		}
		...
		[ActionName("Ano_New_Insert")]
		[AuthorizeForUsers]
		[HttpGet]
		public ActionResult Ano_New_Insert()
		{
			...
			return RedirectToAction("Ano_New");
		}
...
// USE /[MANUAL GQT CONTROLLER_NEW_GET ANO]/
		...
		public ActionResult Ano_New()
		{
			...
			string partialView = qs["partialView"] ?? "Ano";

			var navigationLocationAction = ACTION_ANO_NEW.SetRoutedValues(new { m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);

			//FOR: OVERRIDE SKIP IF JUST ONE
			//Allow child form to use "Go Back" to menu list without "skip if only one"
			if (Navigation.OverrideSkipIfJustOne.ContainsKey("ANO"))
				Navigation.OverrideSkipIfJustOne["ANO"] = true;
...
// USE /[MANUAL GQT AFTER_LOAD_NEW ANO]/
			...
			catch (Exception e)
			{
				...
				CSGenio.framework.Log.Error("Ano_New - GET " + e.Message);
...
// USE /[MANUAL GQT CONTROLLER_NEW_POST ANO]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Ano_New(Ano_ViewModel model, bool redirect = true)
		{
...
// USE /[MANUAL GQT AFTER_LOAD_NEW_EX ANO]/
		...
		[AuthorizeForUsers]
		public ActionResult Ano_New_Redirect(bool internalRedirect = false)
		{
			IList<string> warningMsgs = TempData["NEW_WARNINGS_LIST"] as List<string>;
			string saveMsg = TempData["NEW_SAVE_LIST"] as string;

			if (!internalRedirect)
				SuccessMessage(String.IsNullOrEmpty(saveMsg) ? Resources.Resources.REGISTO_CRIADO_COM_S18746 + GetHumanKeyToQMessage("year", Convert.ToString(Navigation.CurrentLevel.GetEntry("year"))) : saveMsg);
			...
			if (Convert.ToBoolean(Request.QueryString.Get("repeatInsertion")))
				return RedirectToLocation(ACTION_ANO_NEW, new { repeatInsertion = true });
...
// USE /[MANUAL GQT CONTROLLER_EDIT_GET ANO]/
		...
		[HttpGet]
		[ActionName("Ano_Edit")]
		public ActionResult Ano_Edit(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";
			string partialView = qs["partialView"] ?? "Ano"; // MF send the partial view name

			//Useful to initialize a record that had to be created by the framework prior to edition (and method New doesnt support this). This way, if the form then is canceled by the user, the record will be automatically deleted
			var isNewEdit = Navigation.GetStrValue("NewEdit") == "true";

			var navigationLocationAction =  isNewEdit ? ACTION_ANO_NEW.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] }) : ACTION_ANO_EDIT.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);
...
// USE /[MANUAL GQT BEFORE_LOAD_EDIT ANO]/
			...
			catch (ModelNotFoundException)
			{
				...
				sp.closeConnection();
				//JFG 05-05-2017 This need to better handled, for now it's handled on webconfig's customErrors section.
				CSGenio.framework.Log.Error("Ano_Edit - " + id + " " + e.Message);
				throw;
			}
...
// USE /[MANUAL GQT CONTROLLER_EDIT_POST ANO]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Ano_Edit(Ano_ViewModel model, bool redirect = true)
		{
...
// USE /[MANUAL GQT AFTER_LOAD_EDIT_EX ANO]/
		...
		[AuthorizeForUsers]
		public ActionResult Ano_Edit_Redirect(bool internalRedirect = false)
		{
			IList<string> warningMsgs = TempData["EDIT_WARNINGS_LIST"] as List<string>;
			string saveMsg = TempData["NEW_SAVE_LIST"] as string;

			if (!internalRedirect || !Request.IsAjaxRequest())
				SuccessMessage(String.IsNullOrEmpty(saveMsg) ? Resources.Resources.ALTERACOES_EFETUADAS10166 + GetHumanKeyToQMessage("year", Convert.ToString(Navigation.CurrentLevel.GetEntry("year"))) : saveMsg);
...
// USE /[MANUAL GQT CONTROLLER_DELETE_GET ANO]/
		...
		[AuthorizeForUsers]
		[HttpGet]
		[ActionName("Ano_Delete")]
		public ActionResult Ano_Delete(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";

			var navigationLocationAction = ACTION_ANO_DELETE.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);
...
// USE /[MANUAL GQT BEFORE_LOAD_DELETE ANO]/
			...
			catch (ModelNotFoundException)
			{
				...
				//JFG 05-05-2017 This need to better handled, for now it's handled on webconfig's customErrors section.
				CSGenio.framework.Log.Error("Ano_Delete - " + id + " " + e.Message);
				throw;
			}
...
// USE /[MANUAL GQT CONTROLLER_DELETE_POST ANO]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Ano_Delete(string id, FormCollection collection)
		{

			var model = new Ano_ViewModel (Navigation, id);
			model.MapFromModel();
...
// USE /[MANUAL GQT CONTROLLER_DUPLICATE_GET ANO]/
		//
		// GET: /Year/Ano_Duplicate
		[AuthorizeForUsers]
		[HttpGet]
		public ActionResult Ano_Duplicate(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";
			var model = new Ano_ViewModel(Navigation, nestedForm);
			...
			var navigationLocationAction = ACTION_ANO_DUPLICATE.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);
...
// USE /[MANUAL GQT AFTER_LOAD_DUPLICATE ANO]/
					...
					sp.closeTransaction();
					Navigation.CurrentLevel.SetLocation(ACTION_ANO_DUPLICATE.SetRoutedValues(new { Id = model.ValCodyear }));
...
// USE /[MANUAL GQT CONTROLLER_DUPLICATE_POST ANO]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Ano_Duplicate(Ano_ViewModel model, bool redirect = true)
		{
			var eventSink = new EventSink()
			{
...
// USE /[MANUAL GQT AFTER_LOAD_DUPLICATE_EX ANO]/
		...
		[AuthorizeForUsers]
		public ActionResult Ano_Duplicate_Redirect(bool internalRedirect = false)
		{
...
// USE /[MANUAL GQT CONTROLLER_CANCEL_GET ANO]/
		...
		public ActionResult Ano_Cancel()
		{
			...
			if (Navigation.CurrentLevel.FormMode == FormMode.New || Navigation.CurrentLevel.FormMode == FormMode.Duplicate)
			{
				...
				try
				{
					var model = new GenioMVC.Models.Year();
...
// USE /[MANUAL GQT AFTER_CANCEL ANO]/
			...
			else if (Navigation.PreviousLevel != null)
			{
				...
				string previousLevelArea = Navigation.PreviousLevel.Location.Controller?.ToLower() ?? "year";
		...
		{
			var model = new Ano_ViewModel(Navigation, true);
			model.setModes(Request.QueryString["m"]);
			PersistentSupport sp = UserContext.Current.PersistentSupport;
			var navigationLocationAction = ACTION_ANO_NEW.SetRoutedValues(new { m = Request.QueryString["m"] });
			CheckLevels(navigationLocationAction);
			...
			return PartialView("MFAno", model);
		}
		...
		public ActionResult MFAno_Edit(string id)
		{
			return this.RedirectToAction("Ano_Edit", "Year", new { id = id, partialView = "MFAno", nestedForm = "true", multiForm = "true" });
		}
		...
		public ActionResult MFAno_Cancel(string id)
		{
			//MH - Validate if current level is it correct. Remove all levels that is higher than the currently level.
			CheckLevels(NavigationLocation.Any);

			if (Navigation.CurrentLevel.IsNestedContext &&
				(Navigation.CurrentLevel.Location.Action == ACTION_ANO_NEW.Action || Navigation.CurrentLevel.Location.Action == ACTION_ANO_EDIT.Action))
			{
				if (Navigation.CurrentLevel.FormMode == FormMode.New)
				{
					...
					catch (Exception e)
					{
						...
						Log.Error("MFAno_Cancel - " + exceptionUserMessage);
						ErrorMessage(exceptionUserMessage);
					}
				}
			...
			return this.RedirectToAction("Ano_Show", "Year", new { id = id, partialView = "MFAno", nestedForm = "true", multiForm = "true", nav });
		}
		...
		public JsonResult MFAno_Delete(string id)
		{
			var eventSink = new EventSink()
			{
		...
		public ActionResult Ano_ValTodasdes(string id, string partialView)
		{
		...
		[ActionSessionState(System.Web.SessionState.SessionStateBehavior.ReadOnly)]
		public ActionResult Ano_ValAgregado(string id, string partialView)
		{
			int perPage = CSGenio.framework.Configuration.NrRegDBedit;
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Ano_SaveEdit(Ano_ViewModel model)
		{
			var eventSink = new EventSink()
			{
