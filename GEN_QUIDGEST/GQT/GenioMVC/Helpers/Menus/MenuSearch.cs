...
namespace GenioMVC.Helpers.Menus
{
	public class MenuSearch
	{
		...
		private void BuildMenuEntry(IndexWriter indexWriter, MenuResult result)
		{
			...
			Field idField = new Field(ID, entry.ID, Field.Store.YES, Field.Index.NO);
			Field textField = new Field(TEXT, result.Text, Field.Store.YES, Field.Index.ANALYZED);
			Field flatMenuField = new Field(FLAT, result.FlatMenu, Field.Store.YES, Field.Index.ANALYZED);
			Field moduleField = new Field(MODULE, result.Module, Field.Store.YES, Field.Index.NOT_ANALYZED);
			...
			roleList.ForEach(r => document.Add(new Field(ROLE, r.Id, Field.Store.YES, Field.Index.NOT_ANALYZED)));
