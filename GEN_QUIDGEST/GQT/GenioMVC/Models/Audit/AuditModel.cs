...
namespace GenioMVC.Models
{
	public class AuditModel
	{
		...
		public enum LogTables
		{
			...
			logGQTall
			,[Display(Name = "ASSETS12081", ResourceType = typeof(Resources.Resources))]
			ASSET
		}
		...
		public void Where(PersistentSupport sp, int offset, int numberListItems, CriteriaSet filter, List<ColumnSort> sortColumns, bool isExport)
		{
			...
			else
			{
				...
				viewName = AuditModel.LogTables.logGQTall.ToString() + "_" + LogTable.ToString() + "_VIEW";
			}
			...
			else if (LogTable == AuditModel.LogTables.logGQTall)
			{
				query
		...
		public static string GetHumanValue(PersistentSupport sp, AreaInfo area, CSGenio.framework.Field field, string text)
		{
					...
					!string.IsNullOrEmpty(field.ArrayName) && !string.IsNullOrEmpty(text))
			{
				...
				try
				{
					...
					text = CSGenio.framework.Translations.Get(text, Navigation.UserContext.Current.User.Language); //convert to user's language
				}
	...
	public class AuditResult
	{
		...
		[Newtonsoft.Json.JsonIgnore] // Just used for export
