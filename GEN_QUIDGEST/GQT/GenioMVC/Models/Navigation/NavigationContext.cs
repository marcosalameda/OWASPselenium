...
namespace GenioMVC.Models.Navigation
{
    ...
    public class NavigationContextBase
    {
        ...
        /// </summary>
        [JsonProperty("History"), JsonConverter(typeof(Helpers.ConcurrentStackConverter<HistoryLevel>))]
    ...
    public class NavigationContext : NavigationContextBase, ISerializable
    {
        ...
        public NavigationContext(int timeout)
        {
            this.NavigationId = createWinId(11);
        ...
        public NavigationContext(NavigationContextBase source) : base(source) { }
        ...
        {
            string module = UserContext.Current.User.CurrentModule;
            string prefix = module + "_";
            Hashtable ephs = UserContext.Current.User.Ephs;
        ...
        /// </summary>
        /// <param name="location">Navigation Location</param>
        /// <param name="mode">Mode do formulario</param>
        /// <param name="isNested"></param>
        /// <returns></returns>
        public HistoryLevel AddHistoryLevel(NavigationLocation location, FormMode mode, bool isNested = false)
        {
            //Sometimes (in menus) only when the navigation advances do we have the positioning available
            //so we forget the initial empty positioning but invalidating the cached value of the previous level.
            if(PreviousLevel != null)
                PreviousLevel.HumanRoutingDescriptionCache = null;

            HistoryLevel newLevel = new HistoryLevel(location, mode, isNested, History.Count);
            History.Push(newLevel);
            return newLevel;
        }

        /// <summary>
        /// Re-inicializa a lista dos niveis (adiciona level 0)
        /// </summary>
        public void ClearHistoryLevels()
        {
            firstLoad();
        }

        /// <summary>
        /// Remove do ultimo level
        /// </summary>
        public void RemoveHistoryLevel()
        {
            if (History.Count > 0)
                RemoveHistoryLevel(CurrentLevel.Level);
        }

        /// <summary>
        /// Remove de todos niveis a baixo incluido do level indicado
        /// </summary>
        /// <param name="level">QLevel a apagar</param>
        public void RemoveHistoryLevel(int level)
        {
            while (!History.IsEmpty && History.TryPeek(out HistoryLevel hLevel) && hLevel.Level >= level)
                History.TryPop(out HistoryLevel _);
        }
        ...
        internal void RemoveHistoryLevel(NavigationLocation location)
        {
            while (History.Count > 1)
            {
                NavigationLocation current = CurrentLevel.Location;
                if (location.Action == current.Action && location.Controller == current.Controller)
                {
        ...
        {
            if (previousLevel && PreviousLevel != null) return GetValue(key, PreviousLevel.Level);
            else return GetValue(key, CurrentLevel.Level);
        }
        ...
        {
            if (previousLevel && PreviousLevel != null) this.ClearValue(key, PreviousLevel.Level);
            else this.ClearValue(key, CurrentLevel.Level);
        }
        ...
        ///
        /// CÓDIGO ANTIGO - Verificar necesidade desse código, (Usado algures to documentos)
        ///
        /// </remarks>
        ...
        public List<EphMenus> GetAvtarEphMenus()
        {
            ...
            try
            {
                CSGenio.framework.User user = GenioMVC.Models.Navigation.UserContext.Current.User;
        ...
        protected NavigationContext(SerializationInfo info, StreamingContext context)
        {
            ...
            try
            {
                ...
                navigationContext = new NavigationContext(navigationContextBase);
            }
            catch (System.Exception e)
            {
