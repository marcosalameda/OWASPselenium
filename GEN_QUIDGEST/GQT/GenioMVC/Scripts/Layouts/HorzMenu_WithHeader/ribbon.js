(function( $ ){
	$.fn.ribbon = function(id) {
		...
		var thatRet = that;
		var curQLS = QLocalStorage.getLocalStorage('ribbon_selectedTabIndex');
		if($.isEmptyObject(curQLS)) { curQLS = -1; }
		that.selectedTabIndex = parseInt(curQLS || -1);
		...
		that.switchToTabByIndex = function (index) {
			...
			$('#ribbon .ribbon-tab').removeClass('sel');
			$('#ribbon #' + tabNames[index]).show();
			$('#ribbon #' + tabNames[index]).addClass('sel');

			QLocalStorage.setLocalStorage('ribbon_selectedTabIndex', index.toString());
		}
