...
FieldRef[] fields = new FieldRef[] { CSGenioApropr.FldCodpropr, CSGenioApropr.FldZzstate, CSGenioApropr.FldName, CSGenioApropr.FldPrecoest };
			...
			CriteriaSet pais____pseudproprie1Conds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
            tableReload &= hasAllRequiredLimits;

// USE /[MANUAL GQT OVERRQ PAIS_PSEUDPROPRIE1]/

            // This will happen in case there is an error
            if(pais____pseudproprie1Conds == null)
                return;
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioApropr>(false, pais____pseudproprie1Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_PAIS____PSEUDPROPRIE1", true, firstVisibleColumn: firstVisibleColumn);
...
// USE /[MANUAL GQT OVERRQLIST PAIS_PSEUDPROPRIE1]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = UserContext.Current.PersistentSupport.getPagingPos(CSGenioApropr.GetInformation(), QMVC_POS_RECORD, sorts, pais____pseudproprie1Conds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioApropr> listing = Models.ModelBase.Where<CSGenioApropr>(false, pais____pseudproprie1Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_PAIS____PSEUDPROPRIE1", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
