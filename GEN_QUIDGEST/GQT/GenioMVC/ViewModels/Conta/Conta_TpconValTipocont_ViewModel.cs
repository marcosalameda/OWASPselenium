...
{
    public class Conta_TpconValTipocont_ViewModel : ListViewModel
    {
        /// <summary>
        /// Gets or sets the object that represents the table and its elements.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("Table")]
        public TablePartial<GenioMVC.Models.Tpcon> Menu { get; set; }

        /// <inheritdoc/>
        public override string TableAlias { get => "tpcon"; }

        /// <inheritdoc/>
        public override string Uuid { get => "Conta_TpconValTipocont"; }
...
// USE /[MANUAL GQT LIST_LIMITS CONTA_TPCONTIPOCONT]/
        ...
        public Conta_TpconValTipocont_ViewModel(NavigationContext currentNavigation) : base(currentNavigation)
        {
        ...
        public void LoadToExport(out ListingMVC<CSGenioAtpcon> listing, out CriteriaSet conditions, out List<Exports.QColumn> columns, NameValueCollection requestValues, bool ajaxRequest = false)
        {
            ...
            if (ajaxRequest && userColumns!=null)
            {
                ...
                foreach (CSGenioAlstcol column in userColumns)
                {
                    ...
                    string areabase = column.ValTabela.ToLower() != "tpcon" ? CultureInfo.InvariantCulture.TextInfo.ToTitleCase(column.ValTabela) + "." : "";
		...
		public override CriteriaSet BuildCriteriaSet(NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			if(Menu == null)
				Menu = new TablePartial<GenioMVC.Models.Tpcon>();
		...
		public void Load(int numberListItems, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAtpcon> Qlisting, ref CriteriaSet conditions)
		{
			...
			if (ajaxRequest)
				this.Navigation.SetValue("requestValues" + "Conta_TpconValTipocont", requestValues);
			else if (!ajaxRequest && this.Navigation.CheckKey("requestValues" + "Conta_TpconValTipocont"))
				requestValues = this.Navigation.GetValue<NameValueCollection>("requestValues" + "Conta_TpconValTipocont");
			...
			var pageNumber = (ajaxRequest && !String.IsNullOrEmpty(requestValues["pConta_TpconValTipocont"])) ? int.Parse(requestValues["pConta_TpconValTipocont"]) : 1;

			// Added to avoid 0 or -1 pages when setting number of records to -1 to disable pagination
			if (pageNumber < 1)
				pageNumber = 1;

			List<ColumnSort> sorts = GetRequestSorts(this.Menu, "sConta_TpconValTipocont", "dConta_TpconValTipocont", requestValues, "tpcon", allSortOrders);
...
FieldRef[] fields = new FieldRef[] { CSGenioAtpcon.FldCodtpcon, CSGenioAtpcon.FldZzstate, CSGenioAtpcon.FldTipocont };
			...
			//Tooltip for EPHs affecting this viewmodel list
			{
				...
				CSGenioAtpcon model_limit_area = new CSGenioAtpcon(UserContext.Current.User);
			...
			//Limit type: "A"			//Current Area = "TPCON"			//1st Area Limit: "GENRE"			//1st Area Field: "CODGENRE"			//1st Area Value: ""
			{
				Limit limit = new Limit();
			...
			CriteriaSet conta___tpcontipocontConds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
            tableReload &= hasAllRequiredLimits;

// USE /[MANUAL GQT OVERRQ CONTA_TPCONTIPOCONT]/

            // This will happen in case there is an error
            if(conta___tpcontipocontConds == null)
                return;
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAtpcon>(false, conta___tpcontipocontConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_CONTA___TPCONTIPOCONT", true, firstVisibleColumn: firstVisibleColumn);
...
// USE /[MANUAL GQT OVERRQLIST CONTA_TPCONTIPOCONT]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = UserContext.Current.PersistentSupport.getPagingPos(CSGenioAtpcon.GetInformation(), QMVC_POS_RECORD, sorts, conta___tpcontipocontConds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioAtpcon> listing = Models.ModelBase.Where<CSGenioAtpcon>(false, conta___tpcontipocontConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_CONTA___TPCONTIPOCONT", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
        ...
        {
            var Elements = new List<Models.Tpcon>();
            int i = 0;
        ...
        /// to a Models.Tpcon object.
        /// </summary>
        /// <param name="row">The row.</param>
        private Models.Tpcon MapConta_TpconValTipocont(CSGenioAtpcon row)
        {
            var model = new Models.Tpcon(true, _fieldsToSerialize);
            if (row == null)
                return model;
