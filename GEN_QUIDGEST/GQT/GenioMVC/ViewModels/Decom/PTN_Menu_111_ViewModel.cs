...
{
    public class PTN_Menu_111_ViewModel : ListViewModel
    {
        /// <summary>
...
FieldRef[] fields = new FieldRef[] { CSGenioAdecom.FldCoddeco, CSGenioAdecom.FldZzstate, CSGenioAdecom.FldDtdeco, CSGenioAdecom.FldDecomnr, CSGenioAdecom.FldNote, CSGenioAdecom.FldCreatdat, CSGenioAdecom.FldCreatope, CSGenioAdecom.FldChngdate, CSGenioAdecom.FldOperchng };
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="PTN_Menu_111_ViewModel" /> class.
        /// </summary>
        /// <param name="currentNavigation">The current navigation</param>
        public PTN_Menu_111_ViewModel(NavigationContext currentNavigation) : base(currentNavigation)
        {
		...
		public override CriteriaSet BuildCriteriaSet(NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			{
				// EPH
				crs = Models.Decom.AddEPH<CSGenioAdecom>(ref u, crs, "ML111");
			...
			if (tableReload)
			{
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					crs.Equals(Models.Decom.AddEPH<CSGenioAdecom>(ref u, null, "ML111"));
			}
		...
		public void Load(int numberListItems, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAdecom> Qlisting, ref CriteriaSet conditions)
		{
			...
			if (ajaxRequest)
				this.Navigation.SetValue("requestValues" + "PTN_Menu_111", requestValues);
			else if (!ajaxRequest && this.Navigation.CheckKey("requestValues" + "PTN_Menu_111"))
				requestValues = this.Navigation.GetValue<NameValueCollection>("requestValues" + "PTN_Menu_111");
			...
			var pageNumber = (ajaxRequest && !String.IsNullOrEmpty(requestValues["pPTN_Menu_111"])) ? int.Parse(requestValues["pPTN_Menu_111"]) : 1;

			// Added to avoid 0 or -1 pages when setting number of records to -1 to disable pagination
			if (pageNumber < 1)
				pageNumber = 1;

			List<ColumnSort> sorts = GetRequestSorts(this.Menu, "sPTN_Menu_111", "dPTN_Menu_111", requestValues, "decom", allSortOrders);
...
FieldRef[] fields = new FieldRef[] { CSGenioAdecom.FldCoddeco, CSGenioAdecom.FldZzstate, CSGenioAdecom.FldDtdeco, CSGenioAdecom.FldDecomnr, CSGenioAdecom.FldNote, CSGenioAdecom.FldCreatdat, CSGenioAdecom.FldCreatope, CSGenioAdecom.FldChngdate, CSGenioAdecom.FldOperchng };
			...
			CriteriaSet ptn_menu_111Conds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
            tableReload &= hasAllRequiredLimits;

// USE /[MANUAL PTN OVERRQ 111]/

            // This will happen in case there is an error
            if(ptn_menu_111Conds == null)
                return;
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAdecom>(false, ptn_menu_111Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "ML111", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL PTN OVERRQLSTEXP 111]/

                conditions = ptn_menu_111Conds;
                return;
			}
...
// USE /[MANUAL PTN OVERRQLIST 111]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = UserContext.Current.PersistentSupport.getPagingPos(CSGenioAdecom.GetInformation(), QMVC_POS_RECORD, sorts, ptn_menu_111Conds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioAdecom> listing = Models.ModelBase.Where<CSGenioAdecom>(false, ptn_menu_111Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "ML111", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
				...
				Menu.Elements = MapPTN_Menu_111(listing);
            ...
            if (Qlisting.Rows != null)
            {
                foreach (var row in Qlisting.Rows)
				{
					if (Qlisting.NumRegs > 0 && i >= Qlisting.NumRegs) // Copiado da versão antiga do RowsToViewModels
						break;
					Elements.Add(MapPTN_Menu_111(row));
					i++;
				}
            }
        ...
        private Models.Decom MapPTN_Menu_111(CSGenioAdecom row)
        {
        ...
        #region Custom code
// USE /[MANUAL GQT VIEWMODEL_CUSTOM PTN_MENU_111]/
        #endregion
