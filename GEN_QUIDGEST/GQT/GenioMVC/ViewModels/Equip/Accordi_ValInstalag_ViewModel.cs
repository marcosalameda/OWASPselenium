...
{
    public class Accordi_ValInstalag_ViewModel : ListViewModel
    {
        /// <summary>
        ...
        /// <inheritdoc/>
        public override string Uuid { get => "Accordi_ValInstalag"; }
...
// USE /[MANUAL GQT LIST_LIMITS ACCORDI_PSEUDINSTALAG]/
        ...
        public Accordi_ValInstalag_ViewModel(NavigationContext currentNavigation) : base(currentNavigation)
        {
		...
		public void Load(int numberListItems, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAinsta> Qlisting, ref CriteriaSet conditions)
		{
			...
			if (ajaxRequest)
				this.Navigation.SetValue("requestValues" + "Accordi_ValInstalag", requestValues);
			else if (!ajaxRequest && this.Navigation.CheckKey("requestValues" + "Accordi_ValInstalag"))
				requestValues = this.Navigation.GetValue<NameValueCollection>("requestValues" + "Accordi_ValInstalag");
...
FieldRef[] fields = new FieldRef[] { CSGenioAinsta.FldCodinsta, CSGenioAinsta.FldZzstate, CSGenioAinsta.FldSince, CSGenioAinsta.FldUntil, CSGenioAinsta.FldHours, CSGenioAinsta.FldPrecohor, CSGenioAinsta.FldValue };
			...
			CriteriaSet accordi_pseudinstalagConds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
            tableReload &= hasAllRequiredLimits;

// USE /[MANUAL GQT OVERRQ ACCORDI_PSEUDINSTALAG]/

            // This will happen in case there is an error
            if(accordi_pseudinstalagConds == null)
                return;
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAinsta>(false, accordi_pseudinstalagConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_ACCORDI_PSEUDINSTALAG", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL GQT OVERRQLSTEXP ACCORDI_PSEUDINSTALAG]/

                conditions = accordi_pseudinstalagConds;
                return;
			}
...
// USE /[MANUAL GQT OVERRQLIST ACCORDI_PSEUDINSTALAG]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = UserContext.Current.PersistentSupport.getPagingPos(CSGenioAinsta.GetInformation(), QMVC_POS_RECORD, sorts, accordi_pseudinstalagConds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioAinsta> listing = Models.ModelBase.Where<CSGenioAinsta>(false, accordi_pseudinstalagConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_ACCORDI_PSEUDINSTALAG", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
        ...
        private List<Models.Insta> MapAccordi_ValInstalag(ListingMVC<CSGenioAinsta> Qlisting)
        {
        ...
        private Models.Insta MapAccordi_ValInstalag(CSGenioAinsta row)
        {
