...
namespace GenioMVC.ViewModels.Equip
{
    public class Equdocum_ValLisanex_ViewModel : ListViewModel
    {
		...
		{
// USE /[MANUAL GQT LIST_LIMITS EQUDOCUM_PSEUDLISANEX]/
        ...
        public Equdocum_ValLisanex_ViewModel(NavigationContext currentNavigation) : base(currentNavigation)
        {
...
FieldRef[] fields = new FieldRef[] { CSGenioAanexd.FldCodanexd, CSGenioAanexd.FldZzstate, CSGenioAanexd.FldDthranex, CSGenioAanexd.FldTitle, CSGenioAanexd.FldDocument, CSGenioAanexd.FldDocumentfk, CSGenioAanexd.FldTittradu, CSGenioAanexd.FldReferenc, CSGenioAanexd.FldCodequip, CSGenioAequip.FldCodequip };
			...
			CriteriaSet equdocumpseudlisanex_Conds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
            tableReload &= hasAllRequiredLimits;

// USE /[MANUAL GQT OVERRQ EQUDOCUM_PSEUDLISANEX]/

            // This will happen in case there is an error
            if(equdocumpseudlisanex_Conds == null)
                return;
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAanexd>(false, equdocumpseudlisanex_Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_EQUDOCUMPSEUDLISANEX_", true, firstVisibleColumn: firstVisibleColumn);
...
// USE /[MANUAL GQT OVERRQLIST EQUDOCUM_PSEUDLISANEX]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = UserContext.Current.PersistentSupport.getPagingPos(CSGenioAanexd.GetInformation(), QMVC_POS_RECORD, sorts, equdocumpseudlisanex_Conds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioAanexd> listing = Models.ModelBase.Where<CSGenioAanexd>(false, equdocumpseudlisanex_Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_EQUDOCUMPSEUDLISANEX_", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
