...
// USE /[MANUAL GQT LIST_LIMITS EQUIP_PSEUDFOTOEQUI]/
        ...
        public Equip_ValFotoequi_ViewModel(NavigationContext currentNavigation) : base(currentNavigation)
        {
...
FieldRef[] fields = new FieldRef[] { CSGenioAphoto.FldCodphoto, CSGenioAphoto.FldZzstate, CSGenioAphoto.FldTitle, CSGenioAphoto.FldPhotogra };
			...
			//Tooltip for EPHs affecting this viewmodel list
			{
				...
				CSGenioAphoto model_limit_area = new CSGenioAphoto(UserContext.Current.User);
			...
			CriteriaSet equip___pseudfotoequiConds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
            tableReload &= hasAllRequiredLimits;

// USE /[MANUAL GQT OVERRQ EQUIP_PSEUDFOTOEQUI]/

            // This will happen in case there is an error
            if(equip___pseudfotoequiConds == null)
                return;
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAphoto>(false, equip___pseudfotoequiConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_EQUIP___PSEUDFOTOEQUI", true, firstVisibleColumn: firstVisibleColumn);
...
// USE /[MANUAL GQT OVERRQLIST EQUIP_PSEUDFOTOEQUI]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = UserContext.Current.PersistentSupport.getPagingPos(CSGenioAphoto.GetInformation(), QMVC_POS_RECORD, sorts, equip___pseudfotoequiConds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioAphoto> listing = Models.ModelBase.Where<CSGenioAphoto>(false, equip___pseudfotoequiConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_EQUIP___PSEUDFOTOEQUI", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
        ...
        {
            var model = new Models.Photo(true, _fieldsToSerialize);
            if (row == null)
                return model;
