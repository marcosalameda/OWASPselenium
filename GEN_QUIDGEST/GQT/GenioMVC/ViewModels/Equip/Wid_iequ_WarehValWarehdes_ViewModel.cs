...
// USE /[MANUAL GQT LIST_LIMITS WID_IEQU_WAREHWAREHDES]/
        ...
        public Wid_iequ_WarehValWarehdes_ViewModel(NavigationContext currentNavigation) : base(currentNavigation)
        {
		...
		public override CriteriaSet BuildCriteriaSet(NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			{
				// EPH
				crs = Models.Wareh.AddEPH<CSGenioAwareh>(ref u, crs, "IBL_WID_IEQUWAREHWAREHDES");
			...
			if (tableReload)
			{
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					crs.Equals(Models.Wareh.AddEPH<CSGenioAwareh>(ref u, null, "IBL_WID_IEQUWAREHWAREHDES"));
			}
		...
		public void Load(int numberListItems, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAwareh> Qlisting, ref CriteriaSet conditions)
		{
			...
			if (ajaxRequest)
				this.Navigation.SetValue("requestValues" + "Wid_iequ_WarehValWarehdes", requestValues);
			else if (!ajaxRequest && this.Navigation.CheckKey("requestValues" + "Wid_iequ_WarehValWarehdes"))
				requestValues = this.Navigation.GetValue<NameValueCollection>("requestValues" + "Wid_iequ_WarehValWarehdes");
			...
			var pageNumber = (ajaxRequest && !String.IsNullOrEmpty(requestValues["pWid_iequ_WarehValWarehdes"])) ? int.Parse(requestValues["pWid_iequ_WarehValWarehdes"]) : 1;

			// Added to avoid 0 or -1 pages when setting number of records to -1 to disable pagination
			if (pageNumber < 1)
				pageNumber = 1;

			List<ColumnSort> sorts = GetRequestSorts(this.Menu, "sWid_iequ_WarehValWarehdes", "dWid_iequ_WarehValWarehdes", requestValues, "wareh", allSortOrders);
...
FieldRef[] fields = new FieldRef[] { CSGenioAwareh.FldCodwareh, CSGenioAwareh.FldZzstate, CSGenioAwareh.FldWarehdes };
			...
			CriteriaSet wid_iequwarehwarehdesConds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
            tableReload &= hasAllRequiredLimits;

// USE /[MANUAL GQT OVERRQ WID_IEQU_WAREHWAREHDES]/

            // This will happen in case there is an error
            if(wid_iequwarehwarehdesConds == null)
                return;
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAwareh>(false, wid_iequwarehwarehdesConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_WID_IEQUWAREHWAREHDES", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL GQT OVERRQLSTEXP WID_IEQU_WAREHWAREHDES]/

                conditions = wid_iequwarehwarehdesConds;
                return;
			}
...
// USE /[MANUAL GQT OVERRQLIST WID_IEQU_WAREHWAREHDES]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = UserContext.Current.PersistentSupport.getPagingPos(CSGenioAwareh.GetInformation(), QMVC_POS_RECORD, sorts, wid_iequwarehwarehdesConds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioAwareh> listing = Models.ModelBase.Where<CSGenioAwareh>(false, wid_iequwarehwarehdesConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_WID_IEQUWAREHWAREHDES", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
				...
				Menu.Elements = MapWid_iequ_WarehValWarehdes(listing);

				Menu.Identifier = "IBL_WID_IEQUWAREHWAREHDES";

				// Last updated by [CJP] at [2015.02.03]
				// Adds the identifier to each element
				foreach (var element in Menu.Elements)
					element.Identifier = "IBL_WID_IEQUWAREHWAREHDES";
        ...
        private List<Models.Wareh> MapWid_iequ_WarehValWarehdes(ListingMVC<CSGenioAwareh> Qlisting)
        {
            ...
            if (Qlisting.Rows != null)
            {
                foreach (var row in Qlisting.Rows)
				{
					if (Qlisting.NumRegs > 0 && i >= Qlisting.NumRegs) // Copiado da versão antiga do RowsToViewModels
						break;
					Elements.Add(MapWid_iequ_WarehValWarehdes(row));
					i++;
				}
            }
        ...
        private Models.Wareh MapWid_iequ_WarehValWarehdes(CSGenioAwareh row)
        {
