...
namespace GenioMVC.ViewModels.Expen
{
    public class PTN_Menu_LIST_DB_MB_MC_T_ViewModel : ListViewModel
    {
		...
		{
// USE /[MANUAL PTN LIST_LIMITS LIST_DB_MB_MC_T]/
...
FieldRef[] fields = new FieldRef[] { CSGenioAexpen.FldCoddespe, CSGenioAexpen.FldZzstate, CSGenioAexpen.FldCodyear, CSGenioAyear.FldCodyear, CSGenioAyear.FldYear, CSGenioAexpen.FldYearnumb, CSGenioAexpen.FldCodaggre, CSGenioAagreg.FldCodaggre, CSGenioAagreg.FldValue, CSGenioAexpen.FldDescript, CSGenioAexpen.FldValue, CSGenioAexpen.FldPrevval, CSGenioAexpen.FldCodproje, CSGenioAproje.FldCodproje, CSGenioAproje.FldProjecto };
        ...
        public PTN_Menu_LIST_DB_MB_MC_T_ViewModel(NavigationContext currentNavigation) : base(currentNavigation)
        {
		...
		public override CriteriaSet BuildCriteriaSet(NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			Menu.SetFilters(bool.Parse(requestValues["PTN_Menu_LIST_DB_MB_MC_T_tableFilters"] ?? "false"), false);
			...
			crs.SubSets.Add(ProcessSearchFilters(Menu, GetSearchColumns(true), requestValues, "PTN_Menu_LIST_DB_MB_MC_T_"));
			...
			{
				// EPH
				crs = Models.Expen.AddEPH<CSGenioAexpen>(ref u, crs, "MLLIST_DB_MB_MC_T");
			...
			if (tableReload)
			{
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					crs.Equals(Models.Expen.AddEPH<CSGenioAexpen>(ref u, null, "MLLIST_DB_MB_MC_T"));
			}
		...
		public void Load(int numberListItems, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAexpen> Qlisting, ref CriteriaSet conditions)
		{
			...
			if (ajaxRequest)
				this.Navigation.SetValue("requestValues" + "PTN_Menu_LIST_DB_MB_MC_T", requestValues);
			else if (!ajaxRequest && this.Navigation.CheckKey("requestValues" + "PTN_Menu_LIST_DB_MB_MC_T"))
				requestValues = this.Navigation.GetValue<NameValueCollection>("requestValues" + "PTN_Menu_LIST_DB_MB_MC_T");
			...
			var pageNumber = (ajaxRequest && !String.IsNullOrEmpty(requestValues["pPTN_Menu_LIST_DB_MB_MC_T"])) ? int.Parse(requestValues["pPTN_Menu_LIST_DB_MB_MC_T"]) : 1;

			// Added to avoid 0 or -1 pages when setting number of records to -1 to disable pagination
			if (pageNumber < 1)
				pageNumber = 1;

			List<ColumnSort> sorts = GetRequestSorts(this.Menu, "sPTN_Menu_LIST_DB_MB_MC_T", "dPTN_Menu_LIST_DB_MB_MC_T", requestValues, "expen", allSortOrders);
...
FieldRef[] fields = new FieldRef[] { CSGenioAexpen.FldCoddespe, CSGenioAexpen.FldZzstate, CSGenioAexpen.FldCodyear, CSGenioAyear.FldCodyear, CSGenioAyear.FldYear, CSGenioAexpen.FldYearnumb, CSGenioAexpen.FldCodaggre, CSGenioAagreg.FldCodaggre, CSGenioAagreg.FldValue, CSGenioAexpen.FldDescript, CSGenioAexpen.FldValue, CSGenioAexpen.FldPrevval, CSGenioAexpen.FldCodproje, CSGenioAproje.FldCodproje, CSGenioAproje.FldProjecto };
			...
			CriteriaSet ptn_menu_list_db_mb_mc_tConds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
            tableReload &= hasAllRequiredLimits;

// USE /[MANUAL PTN OVERRQ LIST_DB_MB_MC_T]/

            // This will happen in case there is an error
            if(ptn_menu_list_db_mb_mc_tConds == null)
                return;
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAexpen>(false, ptn_menu_list_db_mb_mc_tConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "MLLIST_DB_MB_MC_T", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL PTN OVERRQLSTEXP LIST_DB_MB_MC_T]/

                conditions = ptn_menu_list_db_mb_mc_tConds;
                return;
			}
			...
			if (tableReload)
			{
// USE /[MANUAL PTN OVERRQLIST LIST_DB_MB_MC_T]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = UserContext.Current.PersistentSupport.getPagingPos(CSGenioAexpen.GetInformation(), QMVC_POS_RECORD, sorts, ptn_menu_list_db_mb_mc_tConds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioAexpen> listing = Models.ModelBase.Where<CSGenioAexpen>(false, ptn_menu_list_db_mb_mc_tConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "MLLIST_DB_MB_MC_T", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
				...
				Menu.Elements = MapPTN_Menu_LIST_DB_MB_MC_T(listing);

				Menu.Identifier = "MLLIST_DB_MB_MC_T";
				Menu.Slots = new Dictionary<string, List<object>>();

				// Last updated by [CJP] at [2015.02.03]
				// Adds the identifier to each element
				foreach (var element in Menu.Elements)
					element.Identifier = "MLLIST_DB_MB_MC_T";
        ...
        private List<Models.Expen> MapPTN_Menu_LIST_DB_MB_MC_T(ListingMVC<CSGenioAexpen> Qlisting)
        {
            ...
            if (Qlisting.Rows != null)
            {
                foreach (var row in Qlisting.Rows)
				{
					if (Qlisting.NumRegs > 0 && i >= Qlisting.NumRegs) // Copiado da versão antiga do RowsToViewModels
						break;
					Elements.Add(MapPTN_Menu_LIST_DB_MB_MC_T(row));
					i++;
				}
            }
        ...
        private Models.Expen MapPTN_Menu_LIST_DB_MB_MC_T(CSGenioAexpen row)
        {
