...
namespace GenioMVC.ViewModels.Expen
{
    public class PTN_Menu_LIST_DB_MB_TR_ViewModel : ListViewModel
    {
		...
		{
// USE /[MANUAL PTN LIST_LIMITS LIST_DB_MB_TR]/
...
FieldRef[] fields = new FieldRef[] { CSGenioAexpen.FldCoddespe, CSGenioAexpen.FldZzstate, CSGenioAexpen.FldCodyear, CSGenioAyear.FldCodyear, CSGenioAyear.FldYear, CSGenioAexpen.FldYearnumb, CSGenioAexpen.FldCodaggre, CSGenioAagreg.FldCodaggre, CSGenioAagreg.FldValue, CSGenioAexpen.FldDescript, CSGenioAexpen.FldValue, CSGenioAexpen.FldPrevval, CSGenioAexpen.FldCodproje, CSGenioAproje.FldCodproje, CSGenioAproje.FldProjecto };
        ...
        public PTN_Menu_LIST_DB_MB_TR_ViewModel(NavigationContext currentNavigation) : base(currentNavigation)
        {
		...
		public override CriteriaSet BuildCriteriaSet(NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			Menu.SetFilters(bool.Parse(requestValues["PTN_Menu_LIST_DB_MB_TR_tableFilters"] ?? "false"), false);


			crs.SubSets.Add(ProcessSearchFilters(Menu, GetSearchColumns(true), requestValues, "PTN_Menu_LIST_DB_MB_TR_"));
		...
		public void Load(int numberListItems, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAexpen> Qlisting, ref CriteriaSet conditions)
		{
			...
			if (ajaxRequest)
				this.Navigation.SetValue("requestValues" + "PTN_Menu_LIST_DB_MB_TR", requestValues);
			else if (!ajaxRequest && this.Navigation.CheckKey("requestValues" + "PTN_Menu_LIST_DB_MB_TR"))
				requestValues = this.Navigation.GetValue<NameValueCollection>("requestValues" + "PTN_Menu_LIST_DB_MB_TR");
			...
			var pageNumber = (ajaxRequest && !String.IsNullOrEmpty(requestValues["pPTN_Menu_LIST_DB_MB_TR"])) ? int.Parse(requestValues["pPTN_Menu_LIST_DB_MB_TR"]) : 1;

			// Added to avoid 0 or -1 pages when setting number of records to -1 to disable pagination
			if (pageNumber < 1)
				pageNumber = 1;

			List<ColumnSort> sorts = GetRequestSorts(this.Menu, "sPTN_Menu_LIST_DB_MB_TR", "dPTN_Menu_LIST_DB_MB_TR", requestValues, "expen", allSortOrders);
...
FieldRef[] fields = new FieldRef[] { CSGenioAexpen.FldCoddespe, CSGenioAexpen.FldZzstate, CSGenioAexpen.FldCodyear, CSGenioAyear.FldCodyear, CSGenioAyear.FldYear, CSGenioAexpen.FldYearnumb, CSGenioAexpen.FldCodaggre, CSGenioAagreg.FldCodaggre, CSGenioAagreg.FldValue, CSGenioAexpen.FldDescript, CSGenioAexpen.FldValue, CSGenioAexpen.FldPrevval, CSGenioAexpen.FldCodproje, CSGenioAproje.FldCodproje, CSGenioAproje.FldProjecto };
			...
			CriteriaSet ptn_menu_list_db_mb_trConds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
            tableReload &= hasAllRequiredLimits;

// USE /[MANUAL PTN OVERRQ LIST_DB_MB_TR]/

            // This will happen in case there is an error
            if(ptn_menu_list_db_mb_trConds == null)
                return;
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAexpen>(false, ptn_menu_list_db_mb_trConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "MLLIST_DB_MB_TR", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL PTN OVERRQLSTEXP LIST_DB_MB_TR]/

                conditions = ptn_menu_list_db_mb_trConds;
                return;
			}
...
// USE /[MANUAL PTN OVERRQLIST LIST_DB_MB_TR]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = UserContext.Current.PersistentSupport.getPagingPos(CSGenioAexpen.GetInformation(), QMVC_POS_RECORD, sorts, ptn_menu_list_db_mb_trConds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioAexpen> listing = Models.ModelBase.Where<CSGenioAexpen>(false, ptn_menu_list_db_mb_trConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "MLLIST_DB_MB_TR", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
        ...
        private List<Models.Expen> MapPTN_Menu_LIST_DB_MB_TR(ListingMVC<CSGenioAexpen> Qlisting)
        {
        ...
        private Models.Expen MapPTN_Menu_LIST_DB_MB_TR(CSGenioAexpen row)
        {
