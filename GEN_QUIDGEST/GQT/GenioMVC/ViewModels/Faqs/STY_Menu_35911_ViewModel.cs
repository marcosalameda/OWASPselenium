...
FieldRef[] fields = new FieldRef[] { CSGenioAfaqs.FldCodfaqs, CSGenioAfaqs.FldZzstate, CSGenioAfaqs.FldQuestion, CSGenioAfaqs.FldAnswer };
        ...
        public STY_Menu_35911_ViewModel(NavigationContext currentNavigation) : base(currentNavigation)
        {
...
FieldRef[] fields = new FieldRef[] { CSGenioAfaqs.FldCodfaqs, CSGenioAfaqs.FldZzstate, CSGenioAfaqs.FldQuestion, CSGenioAfaqs.FldAnswer };
			...
			CriteriaSet sty_menu_35911Conds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
            tableReload &= hasAllRequiredLimits;

// USE /[MANUAL STY OVERRQ 35911]/

            // This will happen in case there is an error
            if(sty_menu_35911Conds == null)
                return;
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAfaqs>(false, sty_menu_35911Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "ML35911", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL STY OVERRQLSTEXP 35911]/

                conditions = sty_menu_35911Conds;
                return;
			}
...
// USE /[MANUAL STY OVERRQLIST 35911]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = UserContext.Current.PersistentSupport.getPagingPos(CSGenioAfaqs.GetInformation(), QMVC_POS_RECORD, sorts, sty_menu_35911Conds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioAfaqs> listing = Models.ModelBase.Where<CSGenioAfaqs>(false, sty_menu_35911Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "ML35911", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
        ...
        private readonly SpecialRenderingsCfg _viewModes = new SpecialRenderingsCfg()
        {
            SpecialRenderings = new List<SpecialRendering>()
            {
                new SpecialRendering
                {
                    ...
                    {
                        new SpecialRenderingVariable { Variable = "title", Value = "FAQS.QUESTION", AllowMultiple = false },
                        new SpecialRenderingVariable { Variable = "content", Value = "FAQS.ANSWER", AllowMultiple = false },
                    },
                    StyleVariables = new List<SpecialRenderingVariable>()
                    {
                        new SpecialRenderingVariable { Variable = "html-content", Value = "true" },
                        new SpecialRenderingVariable { Variable = "accordion", Value = "true" },
                    },
                },
            }
        };
