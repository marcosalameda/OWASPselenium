...
// USE /[MANUAL GQT LIST_LIMITS FLDSCOND_PSEUDGRIDTBL]/
        ...
        public Fldscond_ValGridtbl_ViewModel(NavigationContext currentNavigation) : base(currentNavigation)
        {
		...
		public override CriteriaSet BuildCriteriaSet(NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			Menu.SetFilters(bool.Parse(requestValues["ValGridtbl_tableFilters"] ?? "false"), false);


			crs.SubSets.Add(ProcessSearchFilters(Menu, GetSearchColumns(true), requestValues, "ValGridtbl_"));
		...
		public void Load(int numberListItems, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAfeeca> Qlisting, ref CriteriaSet conditions)
		{
			...
			if (ajaxRequest)
				this.Navigation.SetValue("requestValues" + "Fldscond_ValGridtbl", requestValues);
			else if (!ajaxRequest && this.Navigation.CheckKey("requestValues" + "Fldscond_ValGridtbl"))
				requestValues = this.Navigation.GetValue<NameValueCollection>("requestValues" + "Fldscond_ValGridtbl");
			...
			var pageNumber = (ajaxRequest && !String.IsNullOrEmpty(requestValues["pValGridtbl"])) ? int.Parse(requestValues["pValGridtbl"]) : 1;

			// Added to avoid 0 or -1 pages when setting number of records to -1 to disable pagination
			if (pageNumber < 1)
				pageNumber = 1;

			List<ColumnSort> sorts = GetRequestSorts(this.Menu, "sValGridtbl", "dValGridtbl", requestValues, "feeca", allSortOrders);
...
FieldRef[] fields = new FieldRef[] { CSGenioAfeeca.FldCodfeeca, CSGenioAfeeca.FldZzstate, CSGenioAfeeca.FldFeedback, CSGenioAfeeca.FldCodflds };
			...
			CriteriaSet fldscondpseudgridtbl_Conds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
            tableReload &= hasAllRequiredLimits;

// USE /[MANUAL GQT OVERRQ FLDSCOND_PSEUDGRIDTBL]/

            // This will happen in case there is an error
            if(fldscondpseudgridtbl_Conds == null)
                return;
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAfeeca>(false, fldscondpseudgridtbl_Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_FLDSCONDPSEUDGRIDTBL_", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL GQT OVERRQLSTEXP FLDSCOND_PSEUDGRIDTBL]/

                conditions = fldscondpseudgridtbl_Conds;
                return;
			}
...
// USE /[MANUAL GQT OVERRQLIST FLDSCOND_PSEUDGRIDTBL]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = UserContext.Current.PersistentSupport.getPagingPos(CSGenioAfeeca.GetInformation(), QMVC_POS_RECORD, sorts, fldscondpseudgridtbl_Conds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioAfeeca> listing = Models.ModelBase.Where<CSGenioAfeeca>(false, fldscondpseudgridtbl_Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_FLDSCONDPSEUDGRIDTBL_", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
        ...
        private List<Models.Feeca> MapFldscond_ValGridtbl(ListingMVC<CSGenioAfeeca> Qlisting)
        {
        ...
        private Models.Feeca MapFldscond_ValGridtbl(CSGenioAfeeca row)
        {
