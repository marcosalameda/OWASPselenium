...
// USE /[MANUAL GQT LIST_LIMITS FLDSTBL_PSEUDFEECA]/
        ...
        public Fldstbl_ValFeeca_ViewModel(NavigationContext currentNavigation) : base(currentNavigation)
        {
		...
		public override CriteriaSet BuildCriteriaSet(NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			Menu.SetFilters(bool.Parse(requestValues["ValFeeca_tableFilters"] ?? "false"), false);


			crs.SubSets.Add(ProcessSearchFilters(Menu, GetSearchColumns(true), requestValues, "ValFeeca_"));
		...
		public void Load(int numberListItems, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAfeeca> Qlisting, ref CriteriaSet conditions)
		{
			...
			if (ajaxRequest)
				this.Navigation.SetValue("requestValues" + "Fldstbl_ValFeeca", requestValues);
			else if (!ajaxRequest && this.Navigation.CheckKey("requestValues" + "Fldstbl_ValFeeca"))
				requestValues = this.Navigation.GetValue<NameValueCollection>("requestValues" + "Fldstbl_ValFeeca");
			...
			var pageNumber = (ajaxRequest && !String.IsNullOrEmpty(requestValues["pValFeeca"])) ? int.Parse(requestValues["pValFeeca"]) : 1;

			// Added to avoid 0 or -1 pages when setting number of records to -1 to disable pagination
			if (pageNumber < 1)
				pageNumber = 1;

			List<ColumnSort> sorts = GetRequestSorts(this.Menu, "sValFeeca", "dValFeeca", requestValues, "feeca", allSortOrders);
...
FieldRef[] fields = new FieldRef[] { CSGenioAfeeca.FldCodfeeca, CSGenioAfeeca.FldZzstate, CSGenioAfeeca.FldCodflds, CSGenioAflds.FldCodflds, CSGenioAflds.FldDescrip, CSGenioAfeeca.FldFeedback };
			...
			CriteriaSet fldstbl_pseudfeeca___Conds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
            tableReload &= hasAllRequiredLimits;

// USE /[MANUAL GQT OVERRQ FLDSTBL_PSEUDFEECA]/

            // This will happen in case there is an error
            if(fldstbl_pseudfeeca___Conds == null)
                return;
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAfeeca>(false, fldstbl_pseudfeeca___Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_FLDSTBL_PSEUDFEECA___", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL GQT OVERRQLSTEXP FLDSTBL_PSEUDFEECA]/

                conditions = fldstbl_pseudfeeca___Conds;
                return;
			}
...
// USE /[MANUAL GQT OVERRQLIST FLDSTBL_PSEUDFEECA]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = UserContext.Current.PersistentSupport.getPagingPos(CSGenioAfeeca.GetInformation(), QMVC_POS_RECORD, sorts, fldstbl_pseudfeeca___Conds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioAfeeca> listing = Models.ModelBase.Where<CSGenioAfeeca>(false, fldstbl_pseudfeeca___Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_FLDSTBL_PSEUDFEECA___", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
        ...
        private List<Models.Feeca> MapFldstbl_ValFeeca(ListingMVC<CSGenioAfeeca> Qlisting)
        {
        ...
        private Models.Feeca MapFldstbl_ValFeeca(CSGenioAfeeca row)
        {
