...
namespace GenioMVC.ViewModels.Genre
{
    public class TBS_Menu_161_ViewModel : ListViewModel
    {
        ...
        {
            dbeditTitle = Resources.Resources.CONTACT_GENRES29532;
        }
...
FieldRef[] fields = new FieldRef[] { CSGenioAgenre.FldCodgenre, CSGenioAgenre.FldZzstate, CSGenioAgenre.FldGender, CSGenioAgenre.FldAgencont, CSGenioAgenre.FldBackcolo, CSGenioAgenre.FldTextcolo };
        ...
        public TBS_Menu_161_ViewModel(NavigationContext currentNavigation) : base(currentNavigation)
        {
		...
		public CriteriaSet BuildCriteriaSet(NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			{
				// EPH
				crs = Models.Genre.AddEPH<CSGenioAgenre>(ref u, crs, "ML161");
			...
			if (!Navigation.checkFormMode("GENRE", FormMode.New)) // TODO: Check in Duplicate mode
				crs = extendWithZzstateCondition(crs, CSGenioAgenre.FldZzstate, null);

			if (tableReload)
			{
				string QMVC_POS_RECORD = Navigation.GetStrValue("QMVC_POS_RECORD_genre");
				Navigation.DestroyEntry("QMVC_POS_RECORD_genre");
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					crs.Equals(Models.Genre.AddEPH<CSGenioAgenre>(ref u, null, "ML161"));
			}
		...
		public void Load(int numberListItems, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAgenre> Qlisting, ref CriteriaSet conditions)
		{
			...
			if (ajaxRequest)
				this.Navigation.SetValue("requestValues" + "TBS_Menu_161", requestValues);
			else if (!ajaxRequest && this.Navigation.CheckKey("requestValues" + "TBS_Menu_161"))
				requestValues = this.Navigation.GetValue<NameValueCollection>("requestValues" + "TBS_Menu_161");
			...
			var pageNumber = (ajaxRequest && !String.IsNullOrEmpty(requestValues["pTBS_Menu_161"])) ? int.Parse(requestValues["pTBS_Menu_161"]) : 1;

			// Added to avoid 0 or -1 pages when setting number of records to -1 to disable pagination
			if (pageNumber < 1)
				pageNumber = 1;

			List<ColumnSort> sorts = GetRequestSorts(this.Menu, "sTBS_Menu_161", "dTBS_Menu_161", requestValues, "genre", allSortOrders);
...
FieldRef[] fields = new FieldRef[] { CSGenioAgenre.FldCodgenre, CSGenioAgenre.FldZzstate, CSGenioAgenre.FldGender, CSGenioAgenre.FldAgencont, CSGenioAgenre.FldBackcolo, CSGenioAgenre.FldTextcolo };
			...
			CriteriaSet tbs_menu_161Conds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
            tableReload &= hasAllRequiredLimits;
			
// USE /[MANUAL TBS OVERRQ 161]/

            // This will happen in case there is an error
            if(tbs_menu_161Conds == null)
                return;
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAgenre>(false, tbs_menu_161Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "ML161", true, firstVisibleColumn: firstVisibleColumn);
...
// USE /[MANUAL TBS OVERRQLIST 161]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = UserContext.Current.PersistentSupport.getPagingPos(CSGenioAgenre.GetInformation(), QMVC_POS_RECORD, sorts, tbs_menu_161Conds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioAgenre> listing = Models.ModelBase.Where<CSGenioAgenre>(false, tbs_menu_161Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "ML161", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
				...
				Menu.Elements = MapTBS_Menu_161(listing);

				Menu.Identifier = "ML161";
				Menu.Slots = new Dictionary<string, List<object>>();

				// Last updated by [CJP] at [2015.02.03]
				// Adds the identifier to each element
				foreach (var element in Menu.Elements)
					element.Identifier = "ML161";
            ...
            if (Qlisting.Rows != null)
            {
                foreach (var row in Qlisting.Rows)
				{
					if (Qlisting.NumRegs > 0 && i >= Qlisting.NumRegs) // Copiado da versão antiga do RowsToViewModels
						break;
					Elements.Add(MapTBS_Menu_161(row));
					i++;
				}
            }
        ...
        private Models.Genre MapTBS_Menu_161(CSGenioAgenre row)
        {
