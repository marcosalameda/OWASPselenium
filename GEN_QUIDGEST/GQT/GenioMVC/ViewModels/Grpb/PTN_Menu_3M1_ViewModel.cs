...
// USE /[MANUAL PTN LIST_LIMITS 3M1]/
        ...
        {
            dbeditTitle = Resources.Resources.LIST_WITH_COLUMNS_FR36713;
        }
...
FieldRef[] fields = new FieldRef[] { CSGenioAgrpb.FldCodgrpb, CSGenioAgrpb.FldZzstate, CSGenioAgrpb.FldName };
        ...
        public PTN_Menu_3M1_ViewModel(NavigationContext currentNavigation) : base(currentNavigation)
        {
		...
		public override CriteriaSet BuildCriteriaSet(NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			Menu.SetFilters(bool.Parse(requestValues["PTN_Menu_3M1_tableFilters"] ?? "false"), false);


			crs.SubSets.Add(ProcessSearchFilters(Menu, GetSearchColumns(true), requestValues, "PTN_Menu_3M1_"));
		...
		public void Load(int numberListItems, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAgrpb> Qlisting, ref CriteriaSet conditions)
		{
			...
			if (ajaxRequest)
				this.Navigation.SetValue("requestValues" + "PTN_Menu_3M1", requestValues);
			else if (!ajaxRequest && this.Navigation.CheckKey("requestValues" + "PTN_Menu_3M1"))
				requestValues = this.Navigation.GetValue<NameValueCollection>("requestValues" + "PTN_Menu_3M1");
			...
			var pageNumber = (ajaxRequest && !String.IsNullOrEmpty(requestValues["pPTN_Menu_3M1"])) ? int.Parse(requestValues["pPTN_Menu_3M1"]) : 1;

			// Added to avoid 0 or -1 pages when setting number of records to -1 to disable pagination
			if (pageNumber < 1)
				pageNumber = 1;

			List<ColumnSort> sorts = GetRequestSorts(this.Menu, "sPTN_Menu_3M1", "dPTN_Menu_3M1", requestValues, "grpb", allSortOrders);
...
FieldRef[] fields = new FieldRef[] { CSGenioAgrpb.FldCodgrpb, CSGenioAgrpb.FldZzstate, CSGenioAgrpb.FldName };
			...
			CriteriaSet ptn_menu_3m1Conds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
            tableReload &= hasAllRequiredLimits;

// USE /[MANUAL PTN OVERRQ 3M1]/

            // This will happen in case there is an error
            if(ptn_menu_3m1Conds == null)
                return;
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAgrpb>(false, ptn_menu_3m1Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "ML3M1", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL PTN OVERRQLSTEXP 3M1]/

                conditions = ptn_menu_3m1Conds;
                return;
			}
...
// USE /[MANUAL PTN OVERRQLIST 3M1]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = UserContext.Current.PersistentSupport.getPagingPos(CSGenioAgrpb.GetInformation(), QMVC_POS_RECORD, sorts, ptn_menu_3m1Conds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioAgrpb> listing = Models.ModelBase.Where<CSGenioAgrpb>(false, ptn_menu_3m1Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "ML3M1", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
				...
				Menu.Elements = MapPTN_Menu_3M1(listing);
        ...
        private List<Models.Grpb> MapPTN_Menu_3M1(ListingMVC<CSGenioAgrpb> Qlisting)
        {
            ...
            if (Qlisting.Rows != null)
            {
                foreach (var row in Qlisting.Rows)
				{
					if (Qlisting.NumRegs > 0 && i >= Qlisting.NumRegs) // Copiado da versão antiga do RowsToViewModels
						break;
					Elements.Add(MapPTN_Menu_3M1(row));
					i++;
				}
            }
        ...
        private Models.Grpb MapPTN_Menu_3M1(CSGenioAgrpb row)
        {
