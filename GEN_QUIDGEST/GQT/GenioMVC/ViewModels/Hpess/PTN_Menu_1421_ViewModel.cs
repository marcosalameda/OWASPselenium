...
{
    public class PTN_Menu_1421_ViewModel : ListViewModel
    {
        /// <summary>
        ...
        /// <inheritdoc/>
        public override string Uuid { get => "ab8a36df-0588-4ad3-8ca3-f6f500ab1f55"; }
...
// USE /[MANUAL PTN LIST_LIMITS 1421]/
        ...
        {
            dbeditTitle = Resources.Resources.PERSON_HISTORY45111;
        }
...
FieldRef[] fields = new FieldRef[] { CSGenioAhpess.FldCodhpess, CSGenioAhpess.FldZzstate, CSGenioAhpess.FldName, CSGenioAhpess.FldAuthor, CSGenioAhpess.FldDate, CSGenioAhpess.FldCodempre, CSGenioAcmpny.FldCodempre, CSGenioAcmpny.FldDesignat, CSGenioAhpess.FldCodpesso, CSGenioApesso.FldCodpesso, CSGenioApesso.FldName };
            ...
            ListingMVC<CSGenioAhpess> listing = new ListingMVC<CSGenioAhpess>(fields, null, 1, 1, false, user, true, string.Empty, false);
        ...
        public PTN_Menu_1421_ViewModel(NavigationContext currentNavigation) : base(currentNavigation)
        {
		...
		public void Load(int numberListItems, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAhpess> Qlisting, ref CriteriaSet conditions)
		{
			...
			if (ajaxRequest)
				this.Navigation.SetValue("requestValues" + "PTN_Menu_1421", requestValues);
			else if (!ajaxRequest && this.Navigation.CheckKey("requestValues" + "PTN_Menu_1421"))
				requestValues = this.Navigation.GetValue<NameValueCollection>("requestValues" + "PTN_Menu_1421");
			...
			var pageNumber = (ajaxRequest && !String.IsNullOrEmpty(requestValues["pPTN_Menu_1421"])) ? int.Parse(requestValues["pPTN_Menu_1421"]) : 1;

			// Added to avoid 0 or -1 pages when setting number of records to -1 to disable pagination
			if (pageNumber < 1)
				pageNumber = 1;

			List<ColumnSort> sorts = GetRequestSorts(this.Menu, "sPTN_Menu_1421", "dPTN_Menu_1421", requestValues, "hpess", allSortOrders);
...
FieldRef[] fields = new FieldRef[] { CSGenioAhpess.FldCodhpess, CSGenioAhpess.FldZzstate, CSGenioAhpess.FldName, CSGenioAhpess.FldAuthor, CSGenioAhpess.FldDate, CSGenioAhpess.FldCodempre, CSGenioAcmpny.FldCodempre, CSGenioAcmpny.FldDesignat, CSGenioAhpess.FldCodpesso, CSGenioApesso.FldCodpesso, CSGenioApesso.FldName };
			...
			CriteriaSet ptn_menu_1421Conds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
            tableReload &= hasAllRequiredLimits;

// USE /[MANUAL PTN OVERRQ 1421]/

            // This will happen in case there is an error
            if(ptn_menu_1421Conds == null)
                return;
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAhpess>(false, ptn_menu_1421Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "ML1421", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL PTN OVERRQLSTEXP 1421]/

                conditions = ptn_menu_1421Conds;
                return;
			}
...
// USE /[MANUAL PTN OVERRQLIST 1421]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = UserContext.Current.PersistentSupport.getPagingPos(CSGenioAhpess.GetInformation(), QMVC_POS_RECORD, sorts, ptn_menu_1421Conds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioAhpess> listing = Models.ModelBase.Where<CSGenioAhpess>(false, ptn_menu_1421Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "ML1421", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
        ...
        private List<Models.Hpess> MapPTN_Menu_1421(ListingMVC<CSGenioAhpess> Qlisting)
        {
        ...
        private Models.Hpess MapPTN_Menu_1421(CSGenioAhpess row)
        {
