...
namespace GenioMVC.ViewModels.Indoc
{
    public class Dentr_PessoValName_ViewModel : ListViewModel
    {
		...
		{
// USE /[MANUAL GQT LIST_LIMITS DENTR_PESSONAME]/
        ...
        public Dentr_PessoValName_ViewModel(NavigationContext currentNavigation) : base(currentNavigation)
        {
...
FieldRef[] fields = new FieldRef[] { CSGenioApesso.FldCodpesso, CSGenioApesso.FldZzstate, CSGenioApesso.FldName };
			...
			CriteriaSet dentr___pessoname____Conds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
            tableReload &= hasAllRequiredLimits;

// USE /[MANUAL GQT OVERRQ DENTR_PESSONAME]/

            // This will happen in case there is an error
            if(dentr___pessoname____Conds == null)
                return;
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioApesso>(false, dentr___pessoname____Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_DENTR___PESSONAME____", true, firstVisibleColumn: firstVisibleColumn);
...
// USE /[MANUAL GQT OVERRQLIST DENTR_PESSONAME]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = UserContext.Current.PersistentSupport.getPagingPos(CSGenioApesso.GetInformation(), QMVC_POS_RECORD, sorts, dentr___pessoname____Conds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioApesso> listing = Models.ModelBase.Where<CSGenioApesso>(false, dentr___pessoname____Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_DENTR___PESSONAME____", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
