...
namespace GenioMVC.ViewModels.Insta
{
    public class Insta_EquipValRegistnr_ViewModel : ListViewModel
    {
        ...
        public Insta_EquipValRegistnr_ViewModel(NavigationContext currentNavigation) : base(currentNavigation)
        {
...
FieldRef[] fields = new FieldRef[] { CSGenioAequip.FldCodequip, CSGenioAequip.FldZzstate, CSGenioAequip.FldDesignat, CSGenioAequip.FldRegistnr, CSGenioAequip.FldSequennr, CSGenioAequip.FldDtaquisi, CSGenioAequip.FldDtdeco, CSGenioAequip.FldPhotogra, CSGenioAequip.FldValortot };
			...
			CriteriaSet insta___equipregistnrConds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
            tableReload &= hasAllRequiredLimits;

// USE /[MANUAL GQT OVERRQ INSTA_EQUIPREGISTNR]/

            // This will happen in case there is an error
            if(insta___equipregistnrConds == null)
                return;
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAequip>(false, insta___equipregistnrConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_INSTA___EQUIPREGISTNR", true, firstVisibleColumn: firstVisibleColumn);
...
// USE /[MANUAL GQT OVERRQLIST INSTA_EQUIPREGISTNR]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = UserContext.Current.PersistentSupport.getPagingPos(CSGenioAequip.GetInformation(), QMVC_POS_RECORD, sorts, insta___equipregistnrConds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioAequip> listing = Models.ModelBase.Where<CSGenioAequip>(false, insta___equipregistnrConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_INSTA___EQUIPREGISTNR", true, true, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
