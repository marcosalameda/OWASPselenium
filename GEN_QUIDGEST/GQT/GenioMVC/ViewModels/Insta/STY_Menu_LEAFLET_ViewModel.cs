...
{
    public class STY_Menu_LEAFLET_ViewModel : ListViewModel
    {
        /// <summary>
        ...
        /// <inheritdoc/>
        public override string Uuid { get => "88365620-f672-4648-bd5a-98987baf3816"; }
        ...
        {
            dbeditTitle = Resources.Resources.LEAFLET16035;
        }
...
FieldRef[] fields = new FieldRef[] { CSGenioAinsta.FldCodinsta, CSGenioAinsta.FldZzstate, CSGenioAinsta.FldCodtpequ, CSGenioAtpequ.FldCodtpequ, CSGenioAtpequ.FldTipoequi, CSGenioAinsta.FldCodequip, CSGenioAequip.FldCodequip, CSGenioAequip.FldRegistnr, CSGenioAinsta.FldDesignat, CSGenioAinsta.FldDtiniage, CSGenioAinsta.FldDtfimage, CSGenioAinsta.FldDescript, CSGenioAinsta.FldAllday, CSGenioAinsta.FldSince, CSGenioAinsta.FldUntil, CSGenioAinsta.FldHours, CSGenioAinsta.FldPrecohor, CSGenioAinsta.FldValue, CSGenioAinsta.FldCoordgeo };
        ...
        public STY_Menu_LEAFLET_ViewModel(NavigationContext currentNavigation) : base(currentNavigation)
        {
		...
		public void Load(int numberListItems, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAinsta> Qlisting, ref CriteriaSet conditions)
		{
			...
			if (ajaxRequest)
				this.Navigation.SetValue("requestValues" + "STY_Menu_LEAFLET", requestValues);
			else if (!ajaxRequest && this.Navigation.CheckKey("requestValues" + "STY_Menu_LEAFLET"))
				requestValues = this.Navigation.GetValue<NameValueCollection>("requestValues" + "STY_Menu_LEAFLET");
			...
			var pageNumber = (ajaxRequest && !String.IsNullOrEmpty(requestValues["pSTY_Menu_LEAFLET"])) ? int.Parse(requestValues["pSTY_Menu_LEAFLET"]) : 1;

			// Added to avoid 0 or -1 pages when setting number of records to -1 to disable pagination
			if (pageNumber < 1)
				pageNumber = 1;

			List<ColumnSort> sorts = GetRequestSorts(this.Menu, "sSTY_Menu_LEAFLET", "dSTY_Menu_LEAFLET", requestValues, "insta", allSortOrders);
...
FieldRef[] fields = new FieldRef[] { CSGenioAinsta.FldCodinsta, CSGenioAinsta.FldZzstate, CSGenioAinsta.FldCodtpequ, CSGenioAtpequ.FldCodtpequ, CSGenioAtpequ.FldTipoequi, CSGenioAinsta.FldCodequip, CSGenioAequip.FldCodequip, CSGenioAequip.FldRegistnr, CSGenioAinsta.FldDesignat, CSGenioAinsta.FldDtiniage, CSGenioAinsta.FldDtfimage, CSGenioAinsta.FldDescript, CSGenioAinsta.FldAllday, CSGenioAinsta.FldSince, CSGenioAinsta.FldUntil, CSGenioAinsta.FldHours, CSGenioAinsta.FldPrecohor, CSGenioAinsta.FldValue, CSGenioAinsta.FldCoordgeo };
			...
			CriteriaSet sty_menu_leafletConds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
            tableReload &= hasAllRequiredLimits;

// USE /[MANUAL STY OVERRQ LEAFLET]/

            // This will happen in case there is an error
            if(sty_menu_leafletConds == null)
                return;
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAinsta>(false, sty_menu_leafletConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "MLLEAFLET", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL STY OVERRQLSTEXP LEAFLET]/

                conditions = sty_menu_leafletConds;
                return;
			}
...
// USE /[MANUAL STY OVERRQLIST LEAFLET]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = UserContext.Current.PersistentSupport.getPagingPos(CSGenioAinsta.GetInformation(), QMVC_POS_RECORD, sorts, sty_menu_leafletConds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioAinsta> listing = Models.ModelBase.Where<CSGenioAinsta>(false, sty_menu_leafletConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "MLLEAFLET", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
        ...
        private List<Models.Insta> MapSTY_Menu_LEAFLET(ListingMVC<CSGenioAinsta> Qlisting)
        {
        ...
        private Models.Insta MapSTY_Menu_LEAFLET(CSGenioAinsta row)
        {
