...
// USE /[MANUAL GQT LIST_LIMITS ARTIGVAL_GITEMITEMDES]/
		...
		public override CriteriaSet BuildCriteriaSet(NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			Menu.SetFilters(bool.Parse(requestValues["Artigval_GitemValItemdes_tableFilters"] ?? "false"), false);


			crs.SubSets.Add(ProcessSearchFilters(Menu, GetSearchColumns(true), requestValues, "Artigval_GitemValItemdes_"));
		...
		public void Load(int numberListItems, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAgitem> Qlisting, ref CriteriaSet conditions)
		{
			...
			if (ajaxRequest)
				this.Navigation.SetValue("requestValues" + "Artigval_GitemValItemdes", requestValues);
			else if (!ajaxRequest && this.Navigation.CheckKey("requestValues" + "Artigval_GitemValItemdes"))
				requestValues = this.Navigation.GetValue<NameValueCollection>("requestValues" + "Artigval_GitemValItemdes");
			...
			var pageNumber = (ajaxRequest && !String.IsNullOrEmpty(requestValues["pArtigval_GitemValItemdes"])) ? int.Parse(requestValues["pArtigval_GitemValItemdes"]) : 1;

			// Added to avoid 0 or -1 pages when setting number of records to -1 to disable pagination
			if (pageNumber < 1)
				pageNumber = 1;

			List<ColumnSort> sorts = GetRequestSorts(this.Menu, "sArtigval_GitemValItemdes", "dArtigval_GitemValItemdes", requestValues, "gitem", allSortOrders);
...
FieldRef[] fields = new FieldRef[] { CSGenioAgitem.FldCodgitem, CSGenioAgitem.FldZzstate, CSGenioAgitem.FldItemdes };
			...
			CriteriaSet artigvalgitemitemdes_Conds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
            tableReload &= hasAllRequiredLimits;

// USE /[MANUAL GQT OVERRQ ARTIGVAL_GITEMITEMDES]/

            // This will happen in case there is an error
            if(artigvalgitemitemdes_Conds == null)
                return;
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAgitem>(false, artigvalgitemitemdes_Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_ARTIGVALGITEMITEMDES_", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL GQT OVERRQLSTEXP ARTIGVAL_GITEMITEMDES]/

                conditions = artigvalgitemitemdes_Conds;
                return;
			}
...
// USE /[MANUAL GQT OVERRQLIST ARTIGVAL_GITEMITEMDES]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = UserContext.Current.PersistentSupport.getPagingPos(CSGenioAgitem.GetInformation(), QMVC_POS_RECORD, sorts, artigvalgitemitemdes_Conds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioAgitem> listing = Models.ModelBase.Where<CSGenioAgitem>(false, artigvalgitemitemdes_Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_ARTIGVALGITEMITEMDES_", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
        ...
        private List<Models.Gitem> MapArtigval_GitemValItemdes(ListingMVC<CSGenioAgitem> Qlisting)
        {
        ...
        private Models.Gitem MapArtigval_GitemValItemdes(CSGenioAgitem row)
        {
