...
namespace GenioMVC.ViewModels.Item
{
    public class PTN_Menu_LIST_DB_MC_R_ViewModel : ListViewModel
    {
		...
		{
// USE /[MANUAL PTN LIST_LIMITS LIST_DB_MC_R]/
...
FieldRef[] fields = new FieldRef[] { CSGenioAitem.FldCoditem, CSGenioAitem.FldZzstate, CSGenioAitem.FldImage, CSGenioAitem.FldDate, CSGenioAitem.FldDisponib, CSGenioAitem.FldValid };
        ...
        public PTN_Menu_LIST_DB_MC_R_ViewModel(NavigationContext currentNavigation) : base(currentNavigation)
        {
		...
		public override CriteriaSet BuildCriteriaSet(NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			Menu.SetFilters(bool.Parse(requestValues["PTN_Menu_LIST_DB_MC_R_tableFilters"] ?? "false"), false);


			crs.SubSets.Add(ProcessSearchFilters(Menu, GetSearchColumns(true), requestValues, "PTN_Menu_LIST_DB_MC_R_"));
		...
		public void Load(int numberListItems, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAitem> Qlisting, ref CriteriaSet conditions)
		{
			...
			if (ajaxRequest)
				this.Navigation.SetValue("requestValues" + "PTN_Menu_LIST_DB_MC_R", requestValues);
			else if (!ajaxRequest && this.Navigation.CheckKey("requestValues" + "PTN_Menu_LIST_DB_MC_R"))
				requestValues = this.Navigation.GetValue<NameValueCollection>("requestValues" + "PTN_Menu_LIST_DB_MC_R");
			...
			var pageNumber = (ajaxRequest && !String.IsNullOrEmpty(requestValues["pPTN_Menu_LIST_DB_MC_R"])) ? int.Parse(requestValues["pPTN_Menu_LIST_DB_MC_R"]) : 1;

			// Added to avoid 0 or -1 pages when setting number of records to -1 to disable pagination
			if (pageNumber < 1)
				pageNumber = 1;

			List<ColumnSort> sorts = GetRequestSorts(this.Menu, "sPTN_Menu_LIST_DB_MC_R", "dPTN_Menu_LIST_DB_MC_R", requestValues, "item", allSortOrders);
...
FieldRef[] fields = new FieldRef[] { CSGenioAitem.FldCoditem, CSGenioAitem.FldZzstate, CSGenioAitem.FldImage, CSGenioAitem.FldDate, CSGenioAitem.FldDisponib, CSGenioAitem.FldValid };
			...
			CriteriaSet ptn_menu_list_db_mc_rConds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
            tableReload &= hasAllRequiredLimits;

// USE /[MANUAL PTN OVERRQ LIST_DB_MC_R]/

            // This will happen in case there is an error
            if(ptn_menu_list_db_mc_rConds == null)
                return;
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAitem>(false, ptn_menu_list_db_mc_rConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "MLLIST_DB_MC_R", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL PTN OVERRQLSTEXP LIST_DB_MC_R]/

                conditions = ptn_menu_list_db_mc_rConds;
                return;
			}
...
// USE /[MANUAL PTN OVERRQLIST LIST_DB_MC_R]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = UserContext.Current.PersistentSupport.getPagingPos(CSGenioAitem.GetInformation(), QMVC_POS_RECORD, sorts, ptn_menu_list_db_mc_rConds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioAitem> listing = Models.ModelBase.Where<CSGenioAitem>(false, ptn_menu_list_db_mc_rConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "MLLIST_DB_MC_R", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
        ...
        private List<Models.Item> MapPTN_Menu_LIST_DB_MC_R(ListingMVC<CSGenioAitem> Qlisting)
        {
        ...
        private Models.Item MapPTN_Menu_LIST_DB_MC_R(CSGenioAitem row)
        {
