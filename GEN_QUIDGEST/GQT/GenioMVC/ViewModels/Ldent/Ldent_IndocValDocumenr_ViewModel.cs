...
// USE /[MANUAL GQT LIST_LIMITS LDENT_INDOCDOCUMENR]/
        ...
        public Ldent_IndocValDocumenr_ViewModel(NavigationContext currentNavigation) : base(currentNavigation)
        {
...
FieldRef[] fields = new FieldRef[] { CSGenioAindoc.FldCoddentr, CSGenioAindoc.FldZzstate, CSGenioAindoc.FldDocumenr, CSGenioAindoc.FldDhdocume };
			...
			CriteriaSet ldent___indocdocumenrConds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
            tableReload &= hasAllRequiredLimits;

// USE /[MANUAL GQT OVERRQ LDENT_INDOCDOCUMENR]/

            // This will happen in case there is an error
            if(ldent___indocdocumenrConds == null)
                return;
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAindoc>(false, ldent___indocdocumenrConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_LDENT___INDOCDOCUMENR", true, firstVisibleColumn: firstVisibleColumn);
...
// USE /[MANUAL GQT OVERRQLIST LDENT_INDOCDOCUMENR]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = UserContext.Current.PersistentSupport.getPagingPos(CSGenioAindoc.GetInformation(), QMVC_POS_RECORD, sorts, ldent___indocdocumenrConds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioAindoc> listing = Models.ModelBase.Where<CSGenioAindoc>(false, ldent___indocdocumenrConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_LDENT___INDOCDOCUMENR", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
