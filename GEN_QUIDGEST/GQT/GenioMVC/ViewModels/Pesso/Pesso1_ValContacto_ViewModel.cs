...
// USE /[MANUAL GQT LIST_LIMITS PESSO1_PSEUDCONTACTO]/
        ...
        public Pesso1_ValContacto_ViewModel(NavigationContext currentNavigation) : base(currentNavigation)
        {
...
FieldRef[] fields = new FieldRef[] { CSGenioAconta.FldCodconta, CSGenioAconta.FldZzstate, CSGenioAconta.FldCodtpcon, CSGenioAtpcon.FldCodtpcon, CSGenioAtpcon.FldTipocont, CSGenioAconta.FldContacto };
			...
			CriteriaSet pesso1__pseudcontactoConds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
            tableReload &= hasAllRequiredLimits;

// USE /[MANUAL GQT OVERRQ PESSO1_PSEUDCONTACTO]/

            // This will happen in case there is an error
            if(pesso1__pseudcontactoConds == null)
                return;
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAconta>(false, pesso1__pseudcontactoConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_PESSO1__PSEUDCONTACTO", true, firstVisibleColumn: firstVisibleColumn);
...
// USE /[MANUAL GQT OVERRQLIST PESSO1_PSEUDCONTACTO]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = UserContext.Current.PersistentSupport.getPagingPos(CSGenioAconta.GetInformation(), QMVC_POS_RECORD, sorts, pesso1__pseudcontactoConds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioAconta> listing = Models.ModelBase.Where<CSGenioAconta>(false, pesso1__pseudcontactoConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_PESSO1__PSEUDCONTACTO", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
