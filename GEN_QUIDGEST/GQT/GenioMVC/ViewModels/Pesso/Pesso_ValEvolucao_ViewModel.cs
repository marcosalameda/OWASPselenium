...
// USE /[MANUAL GQT LIST_LIMITS PESSO_PSEUDEVOLUCAO]/
        ...
        public Pesso_ValEvolucao_ViewModel(NavigationContext currentNavigation) : base(currentNavigation)
        {
...
FieldRef[] fields = new FieldRef[] { CSGenioAevcat.FldCodprogr, CSGenioAevcat.FldZzstate, CSGenioAevcat.FldSince, CSGenioAevcat.FldCodcateg, CSGenioAcate1.FldCodcateg, CSGenioAcate1.FldCategoria, CSGenioAevcat.FldFimperio, CSGenioAevcat.FldObservat };
			...
			CriteriaSet pesso___pseudevolucaoConds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
            tableReload &= hasAllRequiredLimits;

// USE /[MANUAL GQT OVERRQ PESSO_PSEUDEVOLUCAO]/

            // This will happen in case there is an error
            if(pesso___pseudevolucaoConds == null)
                return;
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAevcat>(false, pesso___pseudevolucaoConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_PESSO___PSEUDEVOLUCAO", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL GQT OVERRQLSTEXP PESSO_PSEUDEVOLUCAO]/

                conditions = pesso___pseudevolucaoConds;
                return;
			}
...
// USE /[MANUAL GQT OVERRQLIST PESSO_PSEUDEVOLUCAO]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = UserContext.Current.PersistentSupport.getPagingPos(CSGenioAevcat.GetInformation(), QMVC_POS_RECORD, sorts, pesso___pseudevolucaoConds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioAevcat> listing = Models.ModelBase.Where<CSGenioAevcat>(false, pesso___pseudevolucaoConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_PESSO___PSEUDEVOLUCAO", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
