...
namespace GenioMVC.ViewModels.Pesso
{
    public class Pessohis_ValField001_ViewModel : ListViewModel
    {
        ...
        public TablePartial<GenioMVC.Models.Hpess> Menu { get; set; }
		...
		{
// USE /[MANUAL GQT LIST_LIMITS PESSOHIS_PSEUDFIELD001]/
        ...
        public Pessohis_ValField001_ViewModel(NavigationContext currentNavigation) : base(currentNavigation)
        {
        ...
        public void LoadToExport(out ListingMVC<CSGenioAhpess> listing, out CriteriaSet conditions, out List<Exports.QColumn> columns, NameValueCollection requestValues, bool ajaxRequest = false)
        {
            ...
            if (ajaxRequest && userColumns!=null)
            {
                ...
                foreach (CSGenioAlstcol column in userColumns)
                {
                    ...
                    string areabase = column.ValTabela.ToLower() != "hpess" ? CultureInfo.InvariantCulture.TextInfo.ToTitleCase(column.ValTabela) + "." : "";
		...
		public override CriteriaSet BuildCriteriaSet(NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			if(Menu == null)
				Menu = new TablePartial<GenioMVC.Models.Hpess>();
		...
		public void Load(int numberListItems, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAhpess> Qlisting, ref CriteriaSet conditions)
		{
			...
			if (ajaxRequest)
				this.Navigation.SetValue("requestValues" + "Pessohis_ValField001", requestValues);
			else if (!ajaxRequest && this.Navigation.CheckKey("requestValues" + "Pessohis_ValField001"))
				requestValues = this.Navigation.GetValue<NameValueCollection>("requestValues" + "Pessohis_ValField001");
			...
			List<ColumnSort> sorts = GetRequestSorts(this.Menu, "sValField001", "dValField001", requestValues, "hpess", allSortOrders);
...
FieldRef[] fields = new FieldRef[] { CSGenioAhpess.FldCodhpess, CSGenioAhpess.FldZzstate, CSGenioAhpess.FldName, CSGenioAhpess.FldDate, CSGenioAhpess.FldAuthor };
			...
			//Tooltip for EPHs affecting this viewmodel list
			{
				...
				CSGenioAhpess model_limit_area = new CSGenioAhpess(UserContext.Current.User);
			...
			CriteriaSet pessohispseudfield001Conds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
            tableReload &= hasAllRequiredLimits;

// USE /[MANUAL GQT OVERRQ PESSOHIS_PSEUDFIELD001]/

            // This will happen in case there is an error
            if(pessohispseudfield001Conds == null)
                return;
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAhpess>(false, pessohispseudfield001Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_PESSOHISPSEUDFIELD001", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL GQT OVERRQLSTEXP PESSOHIS_PSEUDFIELD001]/

                conditions = pessohispseudfield001Conds;
                return;
			}
...
// USE /[MANUAL GQT OVERRQLIST PESSOHIS_PSEUDFIELD001]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = UserContext.Current.PersistentSupport.getPagingPos(CSGenioAhpess.GetInformation(), QMVC_POS_RECORD, sorts, pessohispseudfield001Conds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioAhpess> listing = Models.ModelBase.Where<CSGenioAhpess>(false, pessohispseudfield001Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_PESSOHISPSEUDFIELD001", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
        ...
        private List<Models.Hpess> MapPessohis_ValField001(ListingMVC<CSGenioAhpess> Qlisting)
        {
            var Elements = new List<Models.Hpess>();
            int i = 0;
        ...
        /// to a Models.Hpess object.
        /// </summary>
        /// <param name="row">The row.</param>
        private Models.Hpess MapPessohis_ValField001(CSGenioAhpess row)
        {
            var model = new Models.Hpess(true, _fieldsToSerialize);
            if (row == null)
                return model;
...
// USE /[MANUAL GQT VIEWMODEL_CUSTOM PESSOHIS_VALFIELD001]/
        ...
        {
            "Hpess", "Hpess.ValCodhpess", "Hpess.ValZzstate", "Hpess.ValName", "Hpess.ValDate", "Hpess.ValAuthor", "Hpess.ValCodempre", "Hpess.ValCodpesso"
        };
