...
{
    public class Pessos01_ValContacto_ViewModel : ListViewModel
    {
        /// <summary>
        ...
        /// <inheritdoc/>
        public override string Uuid { get => "Pessos01_ValContacto"; }
...
// USE /[MANUAL GQT LIST_LIMITS PESSOS01_PSEUDCONTACTO]/
        ...
        public Pessos01_ValContacto_ViewModel(NavigationContext currentNavigation) : base(currentNavigation)
        {
		...
		public void Load(int numberListItems, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAconta> Qlisting, ref CriteriaSet conditions)
		{
			...
			if (ajaxRequest)
				this.Navigation.SetValue("requestValues" + "Pessos01_ValContacto", requestValues);
			else if (!ajaxRequest && this.Navigation.CheckKey("requestValues" + "Pessos01_ValContacto"))
				requestValues = this.Navigation.GetValue<NameValueCollection>("requestValues" + "Pessos01_ValContacto");
...
FieldRef[] fields = new FieldRef[] { CSGenioAconta.FldCodconta, CSGenioAconta.FldZzstate, CSGenioAconta.FldCodtpcon, CSGenioAtpcon.FldCodtpcon, CSGenioAtpcon.FldTipocont, CSGenioAconta.FldContacto };
			...
			CriteriaSet pessos01pseudcontactoConds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
            tableReload &= hasAllRequiredLimits;

// USE /[MANUAL GQT OVERRQ PESSOS01_PSEUDCONTACTO]/

            // This will happen in case there is an error
            if(pessos01pseudcontactoConds == null)
                return;
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAconta>(false, pessos01pseudcontactoConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_PESSOS01PSEUDCONTACTO", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL GQT OVERRQLSTEXP PESSOS01_PSEUDCONTACTO]/

                conditions = pessos01pseudcontactoConds;
                return;
			}
...
// USE /[MANUAL GQT OVERRQLIST PESSOS01_PSEUDCONTACTO]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = UserContext.Current.PersistentSupport.getPagingPos(CSGenioAconta.GetInformation(), QMVC_POS_RECORD, sorts, pessos01pseudcontactoConds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioAconta> listing = Models.ModelBase.Where<CSGenioAconta>(false, pessos01pseudcontactoConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_PESSOS01PSEUDCONTACTO", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
        ...
        private List<Models.Conta> MapPessos01_ValContacto(ListingMVC<CSGenioAconta> Qlisting)
        {
        ...
        private Models.Conta MapPessos01_ValContacto(CSGenioAconta row)
        {
