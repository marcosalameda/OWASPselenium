...
// USE /[MANUAL GQT LIST_LIMITS PESSOSEP_CATEGCATEGORY]/
        ...
        public Pessosep_CategValCategoria_ViewModel(NavigationContext currentNavigation) : base(currentNavigation)
        {
...
FieldRef[] fields = new FieldRef[] { CSGenioAcateg.FldCodcateg, CSGenioAcateg.FldZzstate, CSGenioAcateg.FldCategoria, CSGenioAcateg.FldAbbreviation };
			...
			CriteriaSet pessosepcategcategoryConds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
            tableReload &= hasAllRequiredLimits;

// USE /[MANUAL GQT OVERRQ PESSOSEP_CATEGCATEGORY]/

            // This will happen in case there is an error
            if(pessosepcategcategoryConds == null)
                return;
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAcateg>(false, pessosepcategcategoryConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_PESSOSEPCATEGCATEGORY", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL GQT OVERRQLSTEXP PESSOSEP_CATEGCATEGORY]/

                conditions = pessosepcategcategoryConds;
                return;
			}
...
// USE /[MANUAL GQT OVERRQLIST PESSOSEP_CATEGCATEGORY]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = UserContext.Current.PersistentSupport.getPagingPos(CSGenioAcateg.GetInformation(), QMVC_POS_RECORD, sorts, pessosepcategcategoryConds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioAcateg> listing = Models.ModelBase.Where<CSGenioAcateg>(false, pessosepcategcategoryConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_PESSOSEPCATEGCATEGORY", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
