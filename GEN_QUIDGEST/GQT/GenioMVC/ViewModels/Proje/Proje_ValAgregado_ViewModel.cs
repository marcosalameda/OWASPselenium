...
// USE /[MANUAL GQT LIST_LIMITS PROJE_PSEUDAGREGADO]/
        ...
        public Proje_ValAgregado_ViewModel(NavigationContext currentNavigation) : base(currentNavigation)
        {
...
FieldRef[] fields = new FieldRef[] { CSGenioAagreg.FldCodaggre, CSGenioAagreg.FldZzstate, CSGenioAagreg.FldCodyear, CSGenioAyear.FldCodyear, CSGenioAyear.FldYear, CSGenioAagreg.FldValue };
			...
			CriteriaSet proje___pseudagregadoConds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
            tableReload &= hasAllRequiredLimits;

// USE /[MANUAL GQT OVERRQ PROJE_PSEUDAGREGADO]/

            // This will happen in case there is an error
            if(proje___pseudagregadoConds == null)
                return;
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAagreg>(false, proje___pseudagregadoConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_PROJE___PSEUDAGREGADO", true, firstVisibleColumn: firstVisibleColumn);
...
// USE /[MANUAL GQT OVERRQLIST PROJE_PSEUDAGREGADO]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = UserContext.Current.PersistentSupport.getPagingPos(CSGenioAagreg.GetInformation(), QMVC_POS_RECORD, sorts, proje___pseudagregadoConds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioAagreg> listing = Models.ModelBase.Where<CSGenioAagreg>(false, proje___pseudagregadoConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_PROJE___PSEUDAGREGADO", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
