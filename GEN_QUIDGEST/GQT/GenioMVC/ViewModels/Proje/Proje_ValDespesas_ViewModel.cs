...
namespace GenioMVC.ViewModels.Proje
{
    public class Proje_ValDespesas_ViewModel : ListViewModel
    {
		...
		{
// USE /[MANUAL GQT LIST_LIMITS PROJE_PSEUDDESPESAS]/
		...
		public override CriteriaSet BuildCriteriaSet(NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			crs.SubSets.Add(ProcessSearchFilters(Menu, GetSearchColumns(true), requestValues, "ValDespesas_"));
		...
		public void Load(int numberListItems, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAexpen> Qlisting, ref CriteriaSet conditions)
		{
			...
			if (ajaxRequest)
				this.Navigation.SetValue("requestValues" + "Proje_ValDespesas", requestValues);
			else if (!ajaxRequest && this.Navigation.CheckKey("requestValues" + "Proje_ValDespesas"))
				requestValues = this.Navigation.GetValue<NameValueCollection>("requestValues" + "Proje_ValDespesas");
			...
			var pageNumber = (ajaxRequest && !String.IsNullOrEmpty(requestValues["pValDespesas"])) ? int.Parse(requestValues["pValDespesas"]) : 1;

			// Added to avoid 0 or -1 pages when setting number of records to -1 to disable pagination
			if (pageNumber < 1)
				pageNumber = 1;

			List<ColumnSort> sorts = GetRequestSorts(this.Menu, "sValDespesas", "dValDespesas", requestValues, "expen", allSortOrders);
...
FieldRef[] fields = new FieldRef[] { CSGenioAexpen.FldCoddespe, CSGenioAexpen.FldZzstate, CSGenioAexpen.FldDescript, CSGenioAexpen.FldValue, CSGenioAexpen.FldCodyear, CSGenioAyear.FldCodyear, CSGenioAyear.FldYear };
			...
			CriteriaSet proje___pseuddespesasConds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
            tableReload &= hasAllRequiredLimits;

// USE /[MANUAL GQT OVERRQ PROJE_PSEUDDESPESAS]/

            // This will happen in case there is an error
            if(proje___pseuddespesasConds == null)
                return;
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAexpen>(false, proje___pseuddespesasConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_PROJE___PSEUDDESPESAS", true, firstVisibleColumn: firstVisibleColumn);
...
// USE /[MANUAL GQT OVERRQLIST PROJE_PSEUDDESPESAS]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = UserContext.Current.PersistentSupport.getPagingPos(CSGenioAexpen.GetInformation(), QMVC_POS_RECORD, sorts, proje___pseuddespesasConds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioAexpen> listing = Models.ModelBase.Where<CSGenioAexpen>(false, proje___pseuddespesasConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_PROJE___PSEUDDESPESAS", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
        ...
        private Models.Expen MapProje_ValDespesas(CSGenioAexpen row)
        {
...
// USE /[MANUAL GQT VIEWMODEL_CUSTOM PROJE_VALDESPESAS]/
        ...
        {
            "Expen", "Expen.ValCoddespe", "Expen.ValZzstate", "Expen.ValDescript", "Expen.ValValue", "Year", "Year.ValYear", "Expen.ValCodaggre", "Expen.ValCodproje", "Expen.ValCodyear"
        };
