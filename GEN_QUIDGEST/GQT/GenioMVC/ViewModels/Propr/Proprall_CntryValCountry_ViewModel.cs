...
FieldRef[] fields = new FieldRef[] { CSGenioAcntry.FldCodcntry, CSGenioAcntry.FldZzstate, CSGenioAcntry.FldCountry };
			...
			CriteriaSet proprallcntrycountry_Conds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
            tableReload &= hasAllRequiredLimits;

// USE /[MANUAL GQT OVERRQ PROPRALL_CNTRYCOUNTRY]/

            // This will happen in case there is an error
            if(proprallcntrycountry_Conds == null)
                return;
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAcntry>(false, proprallcntrycountry_Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_PROPRALLCNTRYCOUNTRY_", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL GQT OVERRQLSTEXP PROPRALL_CNTRYCOUNTRY]/

                conditions = proprallcntrycountry_Conds;
                return;
			}
...
// USE /[MANUAL GQT OVERRQLIST PROPRALL_CNTRYCOUNTRY]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = UserContext.Current.PersistentSupport.getPagingPos(CSGenioAcntry.GetInformation(), QMVC_POS_RECORD, sorts, proprallcntrycountry_Conds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioAcntry> listing = Models.ModelBase.Where<CSGenioAcntry>(false, proprallcntrycountry_Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_PROPRALLCNTRYCOUNTRY_", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
