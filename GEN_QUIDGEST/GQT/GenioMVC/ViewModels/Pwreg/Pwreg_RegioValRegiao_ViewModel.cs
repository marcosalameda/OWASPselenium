...
// USE /[MANUAL GQT OVERRQLIST PWREG_REGIAO]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = UserContext.Current.PersistentSupport.getPagingPos(CSGenioAregio.GetInformation(), QMVC_POS_RECORD, sorts, pwreg___regioregiao__Conds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioAregio> listing = Models.ModelBase.Where<CSGenioAregio>(false, pwreg___regioregiao__Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_PWREG___REGIOREGIAO__", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
