...
FieldRef[] fields = new FieldRef[] { CSGenioAregio.FldCodregia, CSGenioAregio.FldZzstate, CSGenioAregio.FldCodcntry, CSGenioAcntry.FldCodcntry, CSGenioAcntry.FldCountry, CSGenioAregio.FldRegiao, CSGenioAregio.FldCodpais1, CSGenioApais1.FldCodcntry, CSGenioApais1.FldCountry };
        ...
        public PTN_Menu_411_ViewModel(NavigationContext currentNavigation) : base(currentNavigation)
        {
		...
		public CriteriaSet BuildCriteriaSet(NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			{
				// EPH
				crs = Models.Regio.AddEPH<CSGenioAregio>(ref u, crs, "ML411");
			...
			if (tableReload)
			{
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					crs.Equals(Models.Regio.AddEPH<CSGenioAregio>(ref u, null, "ML411"));
			}
		...
		public void Load(int numberListItems, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAregio> Qlisting, ref CriteriaSet conditions)
		{
			...
			if (ajaxRequest)
				this.Navigation.SetValue("requestValues" + "PTN_Menu_411", requestValues);
			else if (!ajaxRequest && this.Navigation.CheckKey("requestValues" + "PTN_Menu_411"))
				requestValues = this.Navigation.GetValue<NameValueCollection>("requestValues" + "PTN_Menu_411");
			...
			var pageNumber = (ajaxRequest && !String.IsNullOrEmpty(requestValues["pPTN_Menu_411"])) ? int.Parse(requestValues["pPTN_Menu_411"]) : 1;

			// Added to avoid 0 or -1 pages when setting number of records to -1 to disable pagination
			if (pageNumber < 1)
				pageNumber = 1;

			List<ColumnSort> sorts = GetRequestSorts(this.Menu, "sPTN_Menu_411", "dPTN_Menu_411", requestValues, "regio", allSortOrders);
...
FieldRef[] fields = new FieldRef[] { CSGenioAregio.FldCodregia, CSGenioAregio.FldZzstate, CSGenioAregio.FldCodcntry, CSGenioAcntry.FldCodcntry, CSGenioAcntry.FldCountry, CSGenioAregio.FldRegiao, CSGenioAregio.FldCodpais1, CSGenioApais1.FldCodcntry, CSGenioApais1.FldCountry };
			...
			CriteriaSet ptn_menu_411Conds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
            tableReload &= hasAllRequiredLimits;
			
// USE /[MANUAL PTN OVERRQ 411]/

            // This will happen in case there is an error
            if(ptn_menu_411Conds == null)
                return;
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAregio>(false, ptn_menu_411Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "ML411", true, firstVisibleColumn: firstVisibleColumn);
...
// USE /[MANUAL PTN OVERRQLIST 411]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = UserContext.Current.PersistentSupport.getPagingPos(CSGenioAregio.GetInformation(), QMVC_POS_RECORD, sorts, ptn_menu_411Conds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioAregio> listing = Models.ModelBase.Where<CSGenioAregio>(false, ptn_menu_411Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "ML411", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
        ...
        private List<Models.Regio> MapPTN_Menu_411(ListingMVC<CSGenioAregio> Qlisting)
        {
        ...
        private Models.Regio MapPTN_Menu_411(CSGenioAregio row)
        {
