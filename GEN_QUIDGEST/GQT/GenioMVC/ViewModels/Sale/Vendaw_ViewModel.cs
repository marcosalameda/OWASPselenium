...
{
	public class Vendaw_ViewModel : FormViewModel<Models.Sale>
	{
		...
		public Vendaw_ViewModel() : base("FVENDAW") { }

		public Vendaw_ViewModel(NavigationContext currentNavigation, bool nestedForm = false) : base("FVENDAW", currentNavigation, nestedForm) { }

		public Vendaw_ViewModel(Models.Sale row, NavigationContext currentNavigation, bool nestedForm = false) : base("FVENDAW", row, currentNavigation, nestedForm) { }

		public Vendaw_ViewModel(NavigationContext currentNavigation, string id, bool nestedForm = false, string[] fieldsToLoad = null) : this(currentNavigation, nestedForm)
		{
			this.Navigation.SetValue("sale", id);
			Model = Models.Sale.Find(id, "FVENDAW", fieldsToQuery: fieldsToLoad);
			if (Model == null)
		...
		public static StatusMessage InsertConditions(NavigationContext navigation)
		{
			...
			Models.Sale model = new Models.Sale() { Identifier = "FVENDAW" };
		...
		public override void MapFromModel(Models.Sale m)
		{
			if (m == null)
			{
				CSGenio.framework.Log.Error("Map Model (Sale) to ViewModel (Vendaw) - Model is a null reference");
				throw new ModelNotFoundException("Model not found");
			}
			try
			{
			...
			{
				CSGenio.framework.Log.Error("Map Model (Sale) to ViewModel (Vendaw) - Error during mapping");
				throw;
			}
		}
		...
		public override void MapToModel(Models.Sale m)
		{
			if (m == null)
			{
				CSGenio.framework.Log.Error("Map ViewModel (Vendaw) to Model (Sale) - Model is a null reference");
				throw new ModelNotFoundException("Model not found");
			}
			try
			{
			...
			{
				CSGenio.framework.Log.Error("Map ViewModel (Vendaw) to Model (Sale) - Error during mapping");
				throw;
			}
		}
		...
		public override void Load(NameValueCollection qs, bool editable, bool ajaxRequest = false, bool lazyLoad = false)
		{
			...
			try
			{
				Model = Models.Sale.Find(Navigation.GetStrValue("sale"), "FVENDAW");
			}
			finally
			{
				if (Navigation.CurrentLevel.FormMode == FormMode.New || Navigation.CurrentLevel.FormMode == FormMode.Duplicate)
				{
					if (Model == null)
					{
						Model = new Models.Sale() { Identifier = "FVENDAW" };
...
// USE /[MANUAL GQT VIEWMODEL_LOADPARTIAL VENDAW]/
		}

// USE /[MANUAL GQT VIEWMODEL_NEW VENDAW]/
...
// USE /[MANUAL GQT VIEWMODEL_SAVE VENDAW]/
		public override void Save()
		{

			try { Model = Models.Sale.Find(Navigation.GetStrValue("sale"), "FVENDAW"); }
			finally { if (Model == null) Model = new Models.Sale() { Identifier = "FVENDAW" }; }

			base.Save();
		}

// USE /[MANUAL GQT VIEWMODEL_APPLY VENDAW]/
...
// USE /[MANUAL GQT VIEWMODEL_DUPLICATE VENDAW]/

// USE /[MANUAL GQT VIEWMODEL_DESTROY VENDAW]/
		public override void Destroy(string id)
		{
			Model = Models.Sale.Find(id, "FVENDAW");
			if (Model == null)
				throw new ModelNotFoundException("Model not found");
...
// USE /[MANUAL GQT OVERRQ VENDAW01_ORGANORGANIZA]/
		...
		#region Custom code
// USE /[MANUAL GQT VIEWMODEL_CUSTOM VENDAW]/
		#endregion
	}
}
