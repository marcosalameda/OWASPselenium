...
{
	public class Vendawv_ViewModel : FormViewModel<Models.Sale>
	{
		...
		public Vendawv_ViewModel() : base("FVENDAWV") { }

		public Vendawv_ViewModel(NavigationContext currentNavigation, bool nestedForm = false) : base("FVENDAWV", currentNavigation, nestedForm) { }

		public Vendawv_ViewModel(Models.Sale row, NavigationContext currentNavigation, bool nestedForm = false) : base("FVENDAWV", row, currentNavigation, nestedForm) { }

		public Vendawv_ViewModel(NavigationContext currentNavigation, string id, bool nestedForm = false, string[] fieldsToLoad = null) : this(currentNavigation, nestedForm)
		{
			this.Navigation.SetValue("sale", id);
			Model = Models.Sale.Find(id, "FVENDAWV", fieldsToQuery: fieldsToLoad);
			if (Model == null)
		...
		public static StatusMessage InsertConditions(NavigationContext navigation)
		{
			...
			Models.Sale model = new Models.Sale() { Identifier = "FVENDAWV" };
		...
		public override void MapFromModel(Models.Sale m)
		{
			if (m == null)
			{
				CSGenio.framework.Log.Error("Map Model (Sale) to ViewModel (Vendawv) - Model is a null reference");
				throw new ModelNotFoundException("Model not found");
			}
			try
			{
			...
			{
				CSGenio.framework.Log.Error("Map Model (Sale) to ViewModel (Vendawv) - Error during mapping");
				throw;
			}
		}
		...
		public override void MapToModel(Models.Sale m)
		{
			if (m == null)
			{
				CSGenio.framework.Log.Error("Map ViewModel (Vendawv) to Model (Sale) - Model is a null reference");
				throw new ModelNotFoundException("Model not found");
			}
			try
			{
			...
			{
				CSGenio.framework.Log.Error("Map ViewModel (Vendawv) to Model (Sale) - Error during mapping");
				throw;
			}
		}
		...
		public override void Load(NameValueCollection qs, bool editable, bool ajaxRequest = false, bool lazyLoad = false)
		{
			...
			try
			{
				Model = Models.Sale.Find(Navigation.GetStrValue("sale"), "FVENDAWV");
			}
			finally
			{
				if (Navigation.CurrentLevel.FormMode == FormMode.New || Navigation.CurrentLevel.FormMode == FormMode.Duplicate)
				{
					if (Model == null)
					{
						Model = new Models.Sale() { Identifier = "FVENDAWV" };
...
// USE /[MANUAL GQT VIEWMODEL_LOADPARTIAL VENDAWV]/
		}

// USE /[MANUAL GQT VIEWMODEL_NEW VENDAWV]/
...
// USE /[MANUAL GQT VIEWMODEL_SAVE VENDAWV]/
		public override void Save()
		{

			try { Model = Models.Sale.Find(Navigation.GetStrValue("sale"), "FVENDAWV"); }
			finally { if (Model == null) Model = new Models.Sale() { Identifier = "FVENDAWV" }; }

			base.Save();
		}

// USE /[MANUAL GQT VIEWMODEL_APPLY VENDAWV]/
...
// USE /[MANUAL GQT VIEWMODEL_DUPLICATE VENDAWV]/

// USE /[MANUAL GQT VIEWMODEL_DESTROY VENDAWV]/
		public override void Destroy(string id)
		{
			Model = Models.Sale.Find(id, "FVENDAWV");
			if (Model == null)
				throw new ModelNotFoundException("Model not found");
...
// USE /[MANUAL GQT OVERRQ VENDAW01_ORGANORGANIZA]/
		...
		#region Custom code
// USE /[MANUAL GQT VIEWMODEL_CUSTOM VENDAWV]/
		#endregion
	}
}
