...
namespace GenioMVC.ViewModels
{
	public class DocumsVersionsDBEdit_ViewModel : ViewModelBase
	{
		...
		public TablePartial<GenioMVC.Models.Docums> Menu;
		...
		public DocumsVersionsDBEdit_ViewModel(string ticket, string documid, string tableName, string fieldName, bool onlyshow)
		{
			this.Ticket = ticket;
		...
		public void Load(int numberListItems, NameValueCollection requestValues)
		{
			...
			var currentModule = UserContext.Current.User.CurrentModule;
			if (!UserContext.Current.User.IsAdmin(currentModule))
			...
			ListingMVC<CSGenioAdocums> listing = Models.ModelBase.Where<CSGenioAdocums>(false, filters, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts);

			User u = UserContext.Current.User;
			Menu.Elements = listing.RowsForViewModel<GenioMVC.Models.Docums>();
