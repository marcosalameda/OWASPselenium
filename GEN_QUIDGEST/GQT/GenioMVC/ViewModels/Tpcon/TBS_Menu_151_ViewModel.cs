...
// USE /[MANUAL TBS LIST_LIMITS 151]/
        ...
        {
            dbeditTitle = Resources.Resources.CONTACT_TYPES56696;
        }
...
FieldRef[] fields = new FieldRef[] { CSGenioAtpcon.FldCodtpcon, CSGenioAtpcon.FldZzstate, CSGenioAtpcon.FldGenconta, CSGenioAtpcon.FldTipocont };
        ...
        public TBS_Menu_151_ViewModel(NavigationContext currentNavigation) : base(currentNavigation)
        {
		...
		public void Load(int numberListItems, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAtpcon> Qlisting, ref CriteriaSet conditions)
		{
			...
			if (ajaxRequest)
				this.Navigation.SetValue("requestValues" + "TBS_Menu_151", requestValues);
			else if (!ajaxRequest && this.Navigation.CheckKey("requestValues" + "TBS_Menu_151"))
				requestValues = this.Navigation.GetValue<NameValueCollection>("requestValues" + "TBS_Menu_151");
			...
			var pageNumber = (ajaxRequest && !String.IsNullOrEmpty(requestValues["pTBS_Menu_151"])) ? int.Parse(requestValues["pTBS_Menu_151"]) : 1;

			// Added to avoid 0 or -1 pages when setting number of records to -1 to disable pagination
			if (pageNumber < 1)
				pageNumber = 1;

			List<ColumnSort> sorts = GetRequestSorts(this.Menu, "sTBS_Menu_151", "dTBS_Menu_151", requestValues, "tpcon", allSortOrders);
...
FieldRef[] fields = new FieldRef[] { CSGenioAtpcon.FldCodtpcon, CSGenioAtpcon.FldZzstate, CSGenioAtpcon.FldGenconta, CSGenioAtpcon.FldTipocont };
			...
			CriteriaSet tbs_menu_151Conds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
            tableReload &= hasAllRequiredLimits;

// USE /[MANUAL TBS OVERRQ 151]/

            // This will happen in case there is an error
            if(tbs_menu_151Conds == null)
                return;
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAtpcon>(false, tbs_menu_151Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "ML151", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL TBS OVERRQLSTEXP 151]/

                conditions = tbs_menu_151Conds;
                return;
			}
...
// USE /[MANUAL TBS OVERRQLIST 151]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = UserContext.Current.PersistentSupport.getPagingPos(CSGenioAtpcon.GetInformation(), QMVC_POS_RECORD, sorts, tbs_menu_151Conds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioAtpcon> listing = Models.ModelBase.Where<CSGenioAtpcon>(false, tbs_menu_151Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "ML151", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
        ...
        private List<Models.Tpcon> MapTBS_Menu_151(ListingMVC<CSGenioAtpcon> Qlisting)
        {
        ...
        private Models.Tpcon MapTBS_Menu_151(CSGenioAtpcon row)
        {
...
// USE /[MANUAL GQT VIEWMODEL_CUSTOM TBS_MENU_151]/
        ...
        {
            "Tpcon", "Tpcon.ValCodtpcon", "Tpcon.ValZzstate", "Tpcon.ValGenconta", "Tpcon.ValTipocont", "Tpcon.ValCodgenre"
        };
