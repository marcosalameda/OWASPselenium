...
// USE /[MANUAL GQT LIST_LIMITS TPEQU_PSEUDINSTALAC]/
        ...
        public Tpequ_ValInstalac_ViewModel(NavigationContext currentNavigation) : base(currentNavigation)
        {
		...
		public void Load(int numberListItems, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAinsta> Qlisting, ref CriteriaSet conditions)
		{
			...
			if (ajaxRequest)
				this.Navigation.SetValue("requestValues" + "Tpequ_ValInstalac", requestValues);
			else if (!ajaxRequest && this.Navigation.CheckKey("requestValues" + "Tpequ_ValInstalac"))
				requestValues = this.Navigation.GetValue<NameValueCollection>("requestValues" + "Tpequ_ValInstalac");
...
FieldRef[] fields = new FieldRef[] { CSGenioAinsta.FldCodinsta, CSGenioAinsta.FldZzstate, CSGenioAinsta.FldDesignat, CSGenioAinsta.FldDtiniage, CSGenioAinsta.FldDtfimage, CSGenioAinsta.FldDescript, CSGenioAinsta.FldAllday, CSGenioAinsta.FldSince, CSGenioAinsta.FldUntil, CSGenioAinsta.FldHours, CSGenioAinsta.FldPrecohor, CSGenioAinsta.FldValue, CSGenioAinsta.FldCoordgeo };
			...
			CriteriaSet tpequ___pseudinstalacConds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
            tableReload &= hasAllRequiredLimits;

// USE /[MANUAL GQT OVERRQ TPEQU_PSEUDINSTALAC]/

            // This will happen in case there is an error
            if(tpequ___pseudinstalacConds == null)
                return;
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAinsta>(false, tpequ___pseudinstalacConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_TPEQU___PSEUDINSTALAC", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL GQT OVERRQLSTEXP TPEQU_PSEUDINSTALAC]/

                conditions = tpequ___pseudinstalacConds;
                return;
			}
...
// USE /[MANUAL GQT OVERRQLIST TPEQU_PSEUDINSTALAC]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = UserContext.Current.PersistentSupport.getPagingPos(CSGenioAinsta.GetInformation(), QMVC_POS_RECORD, sorts, tpequ___pseudinstalacConds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioAinsta> listing = Models.ModelBase.Where<CSGenioAinsta>(false, tpequ___pseudinstalacConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_TPEQU___PSEUDINSTALAC", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
        ...
        private List<Models.Insta> MapTpequ_ValInstalac(ListingMVC<CSGenioAinsta> Qlisting)
        {
        ...
        private Models.Insta MapTpequ_ValInstalac(CSGenioAinsta row)
        {
