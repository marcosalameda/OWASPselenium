...
FieldRef[] fields = new FieldRef[] { CSGenioAuicom.FldCoduicom, CSGenioAuicom.FldZzstate, CSGenioAuicom.FldName, CSGenioAuicom.FldCategory, CSGenioAuicom.FldMenuid, CSGenioAuicom.FldThumbnai };
        ...
        public STY_Menu_OVERVIEW_ViewModel(NavigationContext currentNavigation) : base(currentNavigation)
        {
...
FieldRef[] fields = new FieldRef[] { CSGenioAuicom.FldCoduicom, CSGenioAuicom.FldZzstate, CSGenioAuicom.FldName, CSGenioAuicom.FldCategory, CSGenioAuicom.FldMenuid, CSGenioAuicom.FldThumbnai };
			...
			//Tooltip for EPHs affecting this viewmodel list
			{
				...
				CSGenioAuicom model_limit_area = new CSGenioAuicom(UserContext.Current.User);
			...
			CriteriaSet sty_menu_overviewConds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
            tableReload &= hasAllRequiredLimits;

// USE /[MANUAL STY OVERRQ OVERVIEW]/

            // This will happen in case there is an error
            if(sty_menu_overviewConds == null)
                return;
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAuicom>(false, sty_menu_overviewConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "MLOVERVIEW", true, firstVisibleColumn: firstVisibleColumn);
...
// USE /[MANUAL STY OVERRQLIST OVERVIEW]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = UserContext.Current.PersistentSupport.getPagingPos(CSGenioAuicom.GetInformation(), QMVC_POS_RECORD, sorts, sty_menu_overviewConds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioAuicom> listing = Models.ModelBase.Where<CSGenioAuicom>(false, sty_menu_overviewConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "MLOVERVIEW", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
        ...
        {
            var model = new Models.Uicom(true, _fieldsToSerialize);
            if (row == null)
                return model;
        ...
        private readonly SpecialRenderingsCfg _viewModes = new SpecialRenderingsCfg()
        {
            SpecialRenderings = new List<SpecialRendering>()
            {
                new SpecialRendering
                {
                    ...
                    {
                        new SpecialRenderingVariable { Variable = "title", Value = "UICOM.NAME", AllowMultiple = false },
                        new SpecialRenderingVariable { Variable = "image", Value = "UICOM.THUMBNAI", AllowMultiple = false },
                    },
                    StyleVariables = new List<SpecialRenderingVariable>()
                    {
                        new SpecialRenderingVariable { Variable = "responsive", Value = "true" },
                        ...
                        new SpecialRenderingVariable { Variable = "actions-style", Value = "grouped" },
                        new SpecialRenderingVariable { Variable = "img-placement", Value = "header" },
                        ...
                        new SpecialRenderingVariable { Variable = "btn-color", Value = "btn-link" },
