...
FieldRef[] fields = new FieldRef[] { CSGenioAitem.FldCoditem, CSGenioAitem.FldZzstate, CSGenioAitem.FldItemdes, CSGenioAitem.FldItemcod, CSGenioAitem.FldEntries, CSGenioAitem.FldExits, CSGenioAitem.FldExistenc, CSGenioAitem.FldImage };
			...
			CriteriaSet armaz03_pseudartigos_Conds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
            tableReload &= hasAllRequiredLimits;

// USE /[MANUAL GQT OVERRQ ARMAZ03_PSEUDARTIGOS]/

            // This will happen in case there is an error
            if(armaz03_pseudartigos_Conds == null)
                return;
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAitem>(false, armaz03_pseudartigos_Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_ARMAZ03_PSEUDARTIGOS_", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL GQT OVERRQLSTEXP ARMAZ03_PSEUDARTIGOS]/

                conditions = armaz03_pseudartigos_Conds;
                return;
			}
...
// USE /[MANUAL GQT OVERRQLIST ARMAZ03_PSEUDARTIGOS]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = UserContext.Current.PersistentSupport.getPagingPos(CSGenioAitem.GetInformation(), QMVC_POS_RECORD, sorts, armaz03_pseudartigos_Conds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioAitem> listing = Models.ModelBase.Where<CSGenioAitem>(false, armaz03_pseudartigos_Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_ARMAZ03_PSEUDARTIGOS_", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
