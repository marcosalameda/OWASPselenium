...
{
    public class Armaz_ValPessarma_ViewModel : ListViewModel
    {
        /// <summary>
        ...
        /// <inheritdoc/>
        public override string Uuid { get => "Armaz_ValPessarma"; }
...
// USE /[MANUAL GQT LIST_LIMITS ARMAZ_PSEUDPESSARMA]/
		...
		public void Load(int numberListItems, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAwpess> Qlisting, ref CriteriaSet conditions)
		{
			...
			if (ajaxRequest)
				this.Navigation.SetValue("requestValues" + "Armaz_ValPessarma", requestValues);
			else if (!ajaxRequest && this.Navigation.CheckKey("requestValues" + "Armaz_ValPessarma"))
				requestValues = this.Navigation.GetValue<NameValueCollection>("requestValues" + "Armaz_ValPessarma");
			...
			var pageNumber = (ajaxRequest && !String.IsNullOrEmpty(requestValues["pValPessarma"])) ? int.Parse(requestValues["pValPessarma"]) : 1;
...
// USE /[MANUAL GQT OVERRQ ARMAZ_PSEUDPESSARMA]/
            ...
            if(armaz___pseudpessarmaConds == null)
                return;
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAwpess>(false, armaz___pseudpessarmaConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_ARMAZ___PSEUDPESSARMA", true, firstVisibleColumn: firstVisibleColumn);
...
// USE /[MANUAL GQT OVERRQLIST ARMAZ_PSEUDPESSARMA]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = UserContext.Current.PersistentSupport.getPagingPos(CSGenioAwpess.GetInformation(), QMVC_POS_RECORD, sorts, armaz___pseudpessarmaConds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioAwpess> listing = Models.ModelBase.Where<CSGenioAwpess>(false, armaz___pseudpessarmaConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_ARMAZ___PSEUDPESSARMA", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
        ...
        private Models.Wpess MapArmaz_ValPessarma(CSGenioAwpess row)
        {
