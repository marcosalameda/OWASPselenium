...
// USE /[MANUAL STY OVERRQ AUTH]/
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAwareh>(false, sty_menu_authConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "MLAUTH", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL STY OVERRQLSTEXP AUTH]/

                conditions = sty_menu_authConds;
                return;
			}
...
// USE /[MANUAL STY OVERRQLIST AUTH]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = UserContext.Current.PersistentSupport.getPagingPos(CSGenioAwareh.GetInformation(), QMVC_POS_RECORD, sorts, sty_menu_authConds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioAwareh> listing = Models.ModelBase.Where<CSGenioAwareh>(false, sty_menu_authConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "MLAUTH", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
