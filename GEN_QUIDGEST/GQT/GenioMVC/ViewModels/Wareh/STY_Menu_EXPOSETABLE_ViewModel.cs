...
FieldRef[] fields = new FieldRef[] { CSGenioAwareh.FldCodwareh, CSGenioAwareh.FldZzstate, CSGenioAwareh.FldWarehdes, CSGenioAwareh.FldWarehcod, CSGenioAwareh.FldActivity };
        ...
        public STY_Menu_EXPOSETABLE_ViewModel(NavigationContext currentNavigation) : base(currentNavigation)
        {
		...
		public override CriteriaSet BuildCriteriaSet(NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			Menu.SetFilters(bool.Parse(requestValues["STY_Menu_EXPOSETABLE_tableFilters"] ?? "false"), false);


			crs.SubSets.Add(ProcessSearchFilters(Menu, GetSearchColumns(true), requestValues, "STY_Menu_EXPOSETABLE_"));
		...
		public void Load(int numberListItems, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAwareh> Qlisting, ref CriteriaSet conditions)
		{
			...
			if (ajaxRequest)
				this.Navigation.SetValue("requestValues" + "STY_Menu_EXPOSETABLE", requestValues);
			else if (!ajaxRequest && this.Navigation.CheckKey("requestValues" + "STY_Menu_EXPOSETABLE"))
				requestValues = this.Navigation.GetValue<NameValueCollection>("requestValues" + "STY_Menu_EXPOSETABLE");
			...
			var pageNumber = (ajaxRequest && !String.IsNullOrEmpty(requestValues["pSTY_Menu_EXPOSETABLE"])) ? int.Parse(requestValues["pSTY_Menu_EXPOSETABLE"]) : 1;

			// Added to avoid 0 or -1 pages when setting number of records to -1 to disable pagination
			if (pageNumber < 1)
				pageNumber = 1;

			List<ColumnSort> sorts = GetRequestSorts(this.Menu, "sSTY_Menu_EXPOSETABLE", "dSTY_Menu_EXPOSETABLE", requestValues, "wareh", allSortOrders);
...
FieldRef[] fields = new FieldRef[] { CSGenioAwareh.FldCodwareh, CSGenioAwareh.FldZzstate, CSGenioAwareh.FldWarehdes, CSGenioAwareh.FldWarehcod, CSGenioAwareh.FldActivity };
			...
			CriteriaSet sty_menu_exposetableConds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
            tableReload &= hasAllRequiredLimits;

// USE /[MANUAL STY OVERRQ EXPOSETABLE]/

            // This will happen in case there is an error
            if(sty_menu_exposetableConds == null)
                return;
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAwareh>(false, sty_menu_exposetableConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "MLEXPOSETABLE", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL STY OVERRQLSTEXP EXPOSETABLE]/

                conditions = sty_menu_exposetableConds;
                return;
			}
...
// USE /[MANUAL STY OVERRQLIST EXPOSETABLE]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = UserContext.Current.PersistentSupport.getPagingPos(CSGenioAwareh.GetInformation(), QMVC_POS_RECORD, sorts, sty_menu_exposetableConds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioAwareh> listing = Models.ModelBase.Where<CSGenioAwareh>(false, sty_menu_exposetableConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "MLEXPOSETABLE", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
        ...
        private List<Models.Wareh> MapSTY_Menu_EXPOSETABLE(ListingMVC<CSGenioAwareh> Qlisting)
        {
        ...
        private Models.Wareh MapSTY_Menu_EXPOSETABLE(CSGenioAwareh row)
        {
