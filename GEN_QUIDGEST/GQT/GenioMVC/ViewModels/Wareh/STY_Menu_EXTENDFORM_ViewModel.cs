...
FieldRef[] fields = new FieldRef[] { CSGenioAwareh.FldCodwareh, CSGenioAwareh.FldZzstate, CSGenioAwareh.FldWarehdes, CSGenioAwareh.FldWarehcod, CSGenioAwareh.FldActivity };
        ...
        public STY_Menu_EXTENDFORM_ViewModel(NavigationContext currentNavigation) : base(currentNavigation)
        {
...
FieldRef[] fields = new FieldRef[] { CSGenioAwareh.FldCodwareh, CSGenioAwareh.FldZzstate, CSGenioAwareh.FldWarehdes, CSGenioAwareh.FldWarehcod, CSGenioAwareh.FldActivity };
			...
			CriteriaSet sty_menu_extendformConds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
            tableReload &= hasAllRequiredLimits;

// USE /[MANUAL STY OVERRQ EXTENDFORM]/

            // This will happen in case there is an error
            if(sty_menu_extendformConds == null)
                return;
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAwareh>(false, sty_menu_extendformConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "MLEXTENDFORM", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL STY OVERRQLSTEXP EXTENDFORM]/

                conditions = sty_menu_extendformConds;
                return;
			}
...
// USE /[MANUAL STY OVERRQLIST EXTENDFORM]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = UserContext.Current.PersistentSupport.getPagingPos(CSGenioAwareh.GetInformation(), QMVC_POS_RECORD, sorts, sty_menu_extendformConds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioAwareh> listing = Models.ModelBase.Where<CSGenioAwareh>(false, sty_menu_extendformConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "MLEXTENDFORM", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
