...
{
    public class STY_Menu_TIMELIN_ViewModel : ListViewModel
    {
        /// <summary>
        ...
        /// <inheritdoc/>
        public override string Uuid { get => "5eb0c848-4e4b-498d-b451-080d4d4c725b"; }
...
FieldRef[] fields = new FieldRef[] { CSGenioAwareh.FldCodwareh, CSGenioAwareh.FldZzstate, CSGenioAwareh.FldWarehdes, CSGenioAwareh.FldWarehcod, CSGenioAwareh.FldActivity };
        ...
        public STY_Menu_TIMELIN_ViewModel(NavigationContext currentNavigation) : base(currentNavigation)
        {
		...
		public CriteriaSet BuildCriteriaSet(NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			Menu.SetFilters(bool.Parse(requestValues["STY_Menu_TIMELIN_tableFilters"] ?? "false"), false);
			...
			crs.SubSets.Add(ProcessSearchFilters(Menu, GetSearchColumns(true), requestValues, "STY_Menu_TIMELIN_"));
		...
		public void Load(int numberListItems, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAwareh> Qlisting, ref CriteriaSet conditions)
		{
			...
			if (ajaxRequest)
				this.Navigation.SetValue("requestValues" + "STY_Menu_TIMELIN", requestValues);
			else if (!ajaxRequest && this.Navigation.CheckKey("requestValues" + "STY_Menu_TIMELIN"))
				requestValues = this.Navigation.GetValue<NameValueCollection>("requestValues" + "STY_Menu_TIMELIN");
			...
			var pageNumber = (ajaxRequest && !String.IsNullOrEmpty(requestValues["pSTY_Menu_TIMELIN"])) ? int.Parse(requestValues["pSTY_Menu_TIMELIN"]) : 1;

			// Added to avoid 0 or -1 pages when setting number of records to -1 to disable pagination
			if (pageNumber < 1)
				pageNumber = 1;

			List<ColumnSort> sorts = GetRequestSorts(this.Menu, "sSTY_Menu_TIMELIN", "dSTY_Menu_TIMELIN", requestValues, "wareh", allSortOrders);
...
FieldRef[] fields = new FieldRef[] { CSGenioAwareh.FldCodwareh, CSGenioAwareh.FldZzstate, CSGenioAwareh.FldWarehdes, CSGenioAwareh.FldWarehcod, CSGenioAwareh.FldActivity };
			...
			CriteriaSet sty_menu_timelinConds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
            tableReload &= hasAllRequiredLimits;
			
// USE /[MANUAL STY OVERRQ TIMELIN]/

            // This will happen in case there is an error
            if(sty_menu_timelinConds == null)
                return;
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAwareh>(false, sty_menu_timelinConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "MLTIMELIN", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL STY OVERRQLSTEXP TIMELIN]/

                conditions = sty_menu_timelinConds;
                return;
			}
...
// USE /[MANUAL STY OVERRQLIST TIMELIN]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = UserContext.Current.PersistentSupport.getPagingPos(CSGenioAwareh.GetInformation(), QMVC_POS_RECORD, sorts, sty_menu_timelinConds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioAwareh> listing = Models.ModelBase.Where<CSGenioAwareh>(false, sty_menu_timelinConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "MLTIMELIN", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
        ...
        private List<Models.Wareh> MapSTY_Menu_TIMELIN(ListingMVC<CSGenioAwareh> Qlisting)
        {
        ...
        private Models.Wareh MapSTY_Menu_TIMELIN(CSGenioAwareh row)
        {
