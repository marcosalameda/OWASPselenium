...
namespace GenioMVC.ViewModels.Wpess
{
    public class STY_Menu_CARDIMGTOP_ViewModel : ListViewModel
    {
		...
		{
// USE /[MANUAL STY LIST_LIMITS CARDIMGTOP]/
...
FieldRef[] fields = new FieldRef[] { CSGenioAwpess.FldCodpess, CSGenioAwpess.FldZzstate, CSGenioAwpess.FldName, CSGenioAwpess.FldDate, CSGenioAwpess.FldSex, CSGenioAwpess.FldNfunc, CSGenioAwpess.FldAdress, CSGenioAwpess.FldZipcode, CSGenioAwpess.FldCountry, CSGenioAwpess.FldEmail, CSGenioAwpess.FldCellphon, CSGenioAwpess.FldNaturali, CSGenioAwpess.FldNacional, CSGenioAwpess.FldCodwareh, CSGenioAwareh.FldCodwareh, CSGenioAwareh.FldWarehdes, CSGenioAwpess.FldFtimgtop };
        ...
        public STY_Menu_CARDIMGTOP_ViewModel(NavigationContext currentNavigation) : base(currentNavigation)
        {
...
FieldRef[] fields = new FieldRef[] { CSGenioAwpess.FldCodpess, CSGenioAwpess.FldZzstate, CSGenioAwpess.FldName, CSGenioAwpess.FldDate, CSGenioAwpess.FldSex, CSGenioAwpess.FldNfunc, CSGenioAwpess.FldAdress, CSGenioAwpess.FldZipcode, CSGenioAwpess.FldCountry, CSGenioAwpess.FldEmail, CSGenioAwpess.FldCellphon, CSGenioAwpess.FldNaturali, CSGenioAwpess.FldNacional, CSGenioAwpess.FldCodwareh, CSGenioAwareh.FldCodwareh, CSGenioAwareh.FldWarehdes, CSGenioAwpess.FldFtimgtop };
			...
			CriteriaSet sty_menu_cardimgtopConds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
            tableReload &= hasAllRequiredLimits;

// USE /[MANUAL STY OVERRQ CARDIMGTOP]/

            // This will happen in case there is an error
            if(sty_menu_cardimgtopConds == null)
                return;
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAwpess>(false, sty_menu_cardimgtopConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "MLCARDIMGTOP", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL STY OVERRQLSTEXP CARDIMGTOP]/

                conditions = sty_menu_cardimgtopConds;
                return;
			}
...
// USE /[MANUAL STY OVERRQLIST CARDIMGTOP]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = UserContext.Current.PersistentSupport.getPagingPos(CSGenioAwpess.GetInformation(), QMVC_POS_RECORD, sorts, sty_menu_cardimgtopConds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioAwpess> listing = Models.ModelBase.Where<CSGenioAwpess>(false, sty_menu_cardimgtopConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "MLCARDIMGTOP", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
