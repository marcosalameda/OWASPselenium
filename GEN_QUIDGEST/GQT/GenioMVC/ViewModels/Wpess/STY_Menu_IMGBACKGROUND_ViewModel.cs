...
FieldRef[] fields = new FieldRef[] { CSGenioAwpess.FldCodpess, CSGenioAwpess.FldZzstate, CSGenioAwpess.FldName, CSGenioAwpess.FldDate, CSGenioAwpess.FldSex, CSGenioAwpess.FldNfunc, CSGenioAwpess.FldAdress, CSGenioAwpess.FldZipcode, CSGenioAwpess.FldCountry, CSGenioAwpess.FldEmail, CSGenioAwpess.FldCellphon, CSGenioAwpess.FldNaturali, CSGenioAwpess.FldNacional, CSGenioAwpess.FldCodwareh, CSGenioAwareh.FldCodwareh, CSGenioAwareh.FldWarehdes, CSGenioAwpess.FldFtbackgr };
			...
			CriteriaSet sty_menu_imgbackgroundConds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
            tableReload &= hasAllRequiredLimits;

// USE /[MANUAL STY OVERRQ IMGBACKGROUND]/

            // This will happen in case there is an error
            if(sty_menu_imgbackgroundConds == null)
                return;
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAwpess>(false, sty_menu_imgbackgroundConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "MLIMGBACKGROUND", true, firstVisibleColumn: firstVisibleColumn);
...
// USE /[MANUAL STY OVERRQLIST IMGBACKGROUND]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = UserContext.Current.PersistentSupport.getPagingPos(CSGenioAwpess.GetInformation(), QMVC_POS_RECORD, sorts, sty_menu_imgbackgroundConds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioAwpess> listing = Models.ModelBase.Where<CSGenioAwpess>(false, sty_menu_imgbackgroundConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "MLIMGBACKGROUND", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
            ...
            SpecialRenderings = new List<SpecialRendering>()
            {
                ...
                new SpecialRendering
                {
                    ...
                    MappingVariables = new List<SpecialRenderingVariable>()
                    {
                        ...
                        new SpecialRenderingVariable { Variable = "image", Value = "WPESS.FTBACKGR", AllowMultiple = false },
                    },
                    StyleVariables = new List<SpecialRenderingVariable>()
                    {
