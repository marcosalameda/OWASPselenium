...
// USE /[MANUAL STY LIST_LIMITS PESSCARD]/
        ...
        {
            dbeditTitle = Resources.Resources.CARD_CENTERED65028;
        }
...
FieldRef[] fields = new FieldRef[] { CSGenioAwpess.FldCodpess, CSGenioAwpess.FldZzstate, CSGenioAwpess.FldName, CSGenioAwpess.FldDate, CSGenioAwpess.FldSex, CSGenioAwpess.FldNfunc, CSGenioAwpess.FldAdress, CSGenioAwpess.FldZipcode, CSGenioAwpess.FldCountry, CSGenioAwpess.FldEmail, CSGenioAwpess.FldCellphon, CSGenioAwpess.FldNaturali, CSGenioAwpess.FldNacional, CSGenioAwpess.FldPfoto, CSGenioAwpess.FldCodwareh, CSGenioAwareh.FldCodwareh, CSGenioAwareh.FldWarehdes };
        ...
        public STY_Menu_PESSCARD_ViewModel(NavigationContext currentNavigation) : base(currentNavigation)
        {
		...
		public override CriteriaSet BuildCriteriaSet(NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			Menu.SetFilters(bool.Parse(requestValues["STY_Menu_PESSCARD_tableFilters"] ?? "false"), false);
			...
			{
				// EPH
				crs = Models.Wpess.AddEPH<CSGenioAwpess>(ref u, crs, "MLPESSCARD");
			...
			if (tableReload)
			{
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					crs.Equals(Models.Wpess.AddEPH<CSGenioAwpess>(ref u, null, "MLPESSCARD"));
			}
		...
		public void Load(int numberListItems, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAwpess> Qlisting, ref CriteriaSet conditions)
		{
			...
			if (ajaxRequest)
				this.Navigation.SetValue("requestValues" + "STY_Menu_PESSCARD", requestValues);
			else if (!ajaxRequest && this.Navigation.CheckKey("requestValues" + "STY_Menu_PESSCARD"))
				requestValues = this.Navigation.GetValue<NameValueCollection>("requestValues" + "STY_Menu_PESSCARD");
			...
			var pageNumber = (ajaxRequest && !String.IsNullOrEmpty(requestValues["pSTY_Menu_PESSCARD"])) ? int.Parse(requestValues["pSTY_Menu_PESSCARD"]) : 1;

			// Added to avoid 0 or -1 pages when setting number of records to -1 to disable pagination
			if (pageNumber < 1)
				pageNumber = 1;

			List<ColumnSort> sorts = GetRequestSorts(this.Menu, "sSTY_Menu_PESSCARD", "dSTY_Menu_PESSCARD", requestValues, "wpess", allSortOrders);
...
FieldRef[] fields = new FieldRef[] { CSGenioAwpess.FldCodpess, CSGenioAwpess.FldZzstate, CSGenioAwpess.FldName, CSGenioAwpess.FldDate, CSGenioAwpess.FldSex, CSGenioAwpess.FldNfunc, CSGenioAwpess.FldAdress, CSGenioAwpess.FldZipcode, CSGenioAwpess.FldCountry, CSGenioAwpess.FldEmail, CSGenioAwpess.FldCellphon, CSGenioAwpess.FldNaturali, CSGenioAwpess.FldNacional, CSGenioAwpess.FldPfoto, CSGenioAwpess.FldCodwareh, CSGenioAwareh.FldCodwareh, CSGenioAwareh.FldWarehdes };
			...
			CriteriaSet sty_menu_pesscardConds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
            tableReload &= hasAllRequiredLimits;

// USE /[MANUAL STY OVERRQ PESSCARD]/

            // This will happen in case there is an error
            if(sty_menu_pesscardConds == null)
                return;
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAwpess>(false, sty_menu_pesscardConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "MLPESSCARD", true, firstVisibleColumn: firstVisibleColumn);
...
// USE /[MANUAL STY OVERRQLSTEXP PESSCARD]/
			...
			if (tableReload)
			{
// USE /[MANUAL STY OVERRQLIST PESSCARD]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = UserContext.Current.PersistentSupport.getPagingPos(CSGenioAwpess.GetInformation(), QMVC_POS_RECORD, sorts, sty_menu_pesscardConds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioAwpess> listing = Models.ModelBase.Where<CSGenioAwpess>(false, sty_menu_pesscardConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "MLPESSCARD", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
				...
				Menu.Elements = MapSTY_Menu_PESSCARD(listing);

				Menu.Identifier = "MLPESSCARD";
				Menu.Slots = new Dictionary<string, List<object>>();

				// Last updated by [CJP] at [2015.02.03]
				// Adds the identifier to each element
				foreach (var element in Menu.Elements)
					element.Identifier = "MLPESSCARD";
        ...
        private List<Models.Wpess> MapSTY_Menu_PESSCARD(ListingMVC<CSGenioAwpess> Qlisting)
        {
            ...
            if (Qlisting.Rows != null)
            {
                foreach (var row in Qlisting.Rows)
				{
					if (Qlisting.NumRegs > 0 && i >= Qlisting.NumRegs) // Copiado da versão antiga do RowsToViewModels
						break;
					Elements.Add(MapSTY_Menu_PESSCARD(row));
					i++;
				}
            }
        ...
        private Models.Wpess MapSTY_Menu_PESSCARD(CSGenioAwpess row)
        {
            ...
            SpecialRenderings = new List<SpecialRendering>()
            {
                ...
                new SpecialRendering
                {
                    ...
                    MappingVariables = new List<SpecialRenderingVariable>()
                    {
                        ...
                        new SpecialRenderingVariable { Variable = "image", Value = "WPESS.PFOTO", AllowMultiple = false },
                    },
                    StyleVariables = new List<SpecialRenderingVariable>()
                    {
                        new SpecialRenderingVariable { Variable = "actions-placement", Value = "header" },
                        ...
                        new SpecialRenderingVariable { Variable = "image-shape", Value = "circular" },
