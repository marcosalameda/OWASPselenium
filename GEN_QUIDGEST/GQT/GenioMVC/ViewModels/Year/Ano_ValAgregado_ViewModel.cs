...
// USE /[MANUAL GQT LIST_LIMITS ANO_PSEUDAGREGADO]/
        ...
        public Ano_ValAgregado_ViewModel(NavigationContext currentNavigation) : base(currentNavigation)
        {
...
FieldRef[] fields = new FieldRef[] { CSGenioAagreg.FldCodaggre, CSGenioAagreg.FldZzstate, CSGenioAagreg.FldValue };
			...
			CriteriaSet ano_____pseudagregadoConds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
            tableReload &= hasAllRequiredLimits;

// USE /[MANUAL GQT OVERRQ ANO_PSEUDAGREGADO]/

            // This will happen in case there is an error
            if(ano_____pseudagregadoConds == null)
                return;
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAagreg>(false, ano_____pseudagregadoConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_ANO_____PSEUDAGREGADO", true, firstVisibleColumn: firstVisibleColumn);
...
// USE /[MANUAL GQT OVERRQLIST ANO_PSEUDAGREGADO]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = UserContext.Current.PersistentSupport.getPagingPos(CSGenioAagreg.GetInformation(), QMVC_POS_RECORD, sorts, ano_____pseudagregadoConds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioAagreg> listing = Models.ModelBase.Where<CSGenioAagreg>(false, ano_____pseudagregadoConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_ANO_____PSEUDAGREGADO", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
