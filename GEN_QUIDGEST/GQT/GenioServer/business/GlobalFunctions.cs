                    ...
                    case 13:
                        {
                            ...
                            string arg1 = obj[1];
                            string arg2 = obj[2];
                            return OpenDocument(arg0,arg1,arg2);
                        }
                    default:
...
// USE /[MANUAL GQT ONASSINA]/
        ...
        /// <summary>
        /// Created by [SF] at [2014.09-23]
        /// Função to converter documento to pdf
        /// </summary>
        /// <param name="codtabela">key primaria da table</param>
        /// <param name="tabela">table to fazer join com a docums</param>
        /// <returns></returns>
        public string[] OpenDocument(string codtable, string table, string fkField)
        {
            string[] docum = new string[1];
            docum[0] = "";

            var info = Area.GetInfoArea(table);
            AreaRef area = new AreaRef(info.QSystem, info.TableName, info.Alias);

            //verifica se exists um registo de documento
            DataMatrix documentos;
            if (codtable.Length > 0)
            {
                documentos =
                    sp.Execute(new SelectQuery()
                    .Select("docums", "nome", "nome").Select("docums", "versao", "versao").Select("docums", "document", "document")
                    .From(area).Join("docums", "docums", TableJoinType.Inner).On(CriteriaSet.And().Equal(area.Alias, fkField + "fk", "docums", "documid"))
                    .Where(CriteriaSet.And()
                    .Equal(area.Alias, info.PrimaryKeyName, codtable))
                    .OrderBy("docums", "datacria", SortOrder.Descending)
                    .OrderBy("docums", "chave", SortOrder.Ascending));

                if (documentos.NumRows == 0)
                {
                    //retira o código interno da última versão do primeiro documento attachment ao registo
                    documentos = sp.Execute(new SelectQuery()
                        .Select("docums", "nome", "nome").Select("docums", "versao", "versao").Select("docums", "document", "document")
                        .From(area).Join("docums", "docums", TableJoinType.Inner).On(CriteriaSet.And().Equal(area.Alias, fkField + "fk", "docums", "documid"))
                        .Where(CriteriaSet.And()
                        .Equal(area.Alias, info.PrimaryKeyName, codtable))
                ...
                if (documentos.NumRows == 0)
                    return docum;
                string fileName = documentos.GetString(0, 0);
                string version = documentos.GetString(0, 1);
                byte[] bytesFicheiro = documentos.GetBinary(0, 2);
                int size = bytesFicheiro.Length;
                if (size != 0) // se não exists imagem
                {
                    string caminho = AppDomain.CurrentDomain.BaseDirectory;

                    //create a unique filename
                    string extension = Path.GetExtension(fileName);
                    string name = Path.GetFileNameWithoutExtension(fileName) + "_V" + version;
                    fileName = name + extension;
                    int add = 1;
                    while (File.Exists(Path.Combine(caminho, "temp", fileName)))
                    {
                        fileName = name + "(" + add + ")" + extension;
                        add++;
                    }

                    MemoryStream inStream = new MemoryStream(bytesFicheiro);

                    string[] linkRec = new string[2];
                    Resource recfich;

                    string caminhoFicheiro = "";

                    linkRec[1] = Configuration.GetProperty("PreviewDocConvert", "0");
                    string convertServiceUrl = Configuration.GetProperty("PreviewDocUrl", "");
                    //no use converting pdf's to pdf
                    if (extension == ".pdf")
                        linkRec[1] = "0";
                    if (linkRec[1] == "1" && !string.IsNullOrEmpty(convertServiceUrl))
                    {
                        caminhoFicheiro = Path.Combine(caminho, "temp", Path.GetFileNameWithoutExtension(fileName) + ".pdf");
                        using (FileStream outStream = new FileStream(caminhoFicheiro, FileMode.Create))
                        {
                            string pathdocconvert = CallConvertPdf(convertServiceUrl, fileName, inStream, outStream).GetAwaiter().GetResult();
                            if (string.IsNullOrEmpty(pathdocconvert))
                                linkRec[1] = "0";
                        }
                    }
                    ...
                    if (linkRec[1] == "0")
                    {
                        caminhoFicheiro = Path.Combine(caminho, "temp", fileName);
                        using (FileStream outStream = new FileStream(caminhoFicheiro, FileMode.Create))
                            outStream.Write(bytesFicheiro, 0, size);
                    }

                    recfich = new ResourceFile(fileName, caminhoFicheiro);

                    string recSer = QResources.CreateTicketEncryptedBase64(user.Name, user.Location, recfich);
                    linkRec[0] = System.Web.HttpUtility.UrlEncode(recSer);
                    return linkRec;
                }
            }
            return docum;
        ...
        /// </summary>
        private static readonly Dictionary<string, string> FLAT_MVC_THEME_VARIABLES = new Dictionary<string, string>()
        {
        ...
        /// <summary>
        /// Genio Quality Tests theme variables
        /// </summary>
        private static readonly Dictionary<string, string> GQT_THEME_VARIABLES = new Dictionary<string, string>()
        {
            { "$footer-bg", "transparent" },
            { "$menu-sidebar-width", "16rem" },
            { "$menu-behaviour", "partial_collapse" },
        ...
        /// <summary>
        /// Warehouse Management System theme variables
        /// </summary>
        private static readonly Dictionary<string, string> WMS_THEME_VARIABLES = new Dictionary<string, string>()
        {
            { "$footer-bg", "transparent" },
            { "$menu-sidebar-width", "16rem" },
            { "$menu-behaviour", "partial_collapse" },
        ...
        /// <summary>
        /// Horizontal Layout theme variables
        /// </summary>
        private static readonly Dictionary<string, string> HORIZONTAL_MVC_THEME_VARIABLES = new Dictionary<string, string>()
        {
            { "$footer-bg", "transparent" },
            { "$menu-sidebar-width", "16rem" },
            { "$menu-behaviour", "partial_collapse" },
        ...
        {
            return GQT_THEME_VARIABLES[variable];
        }
        ...
        {
            if ("FLAT_MVC" == appID)
                return FLAT_MVC_THEME_VARIABLES[variable];
            if ("GQT" == appID)
                return GQT_THEME_VARIABLES[variable];
            if ("WMS" == appID)
                return WMS_THEME_VARIABLES[variable];
            if ("HORIZONTAL_MVC" == appID)
                return HORIZONTAL_MVC_THEME_VARIABLES[variable];
            return "";
        }
        ...
        public DbArea GetGlob()
        {
            try
            {
                return CSGenioAglob.searchGlob(sp, user);
            }
            catch
            {
                return null;
