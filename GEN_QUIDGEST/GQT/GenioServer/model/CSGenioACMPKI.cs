...
#pragma warning restore CS0168, S1481 // Variable is declared but never used
			//- - - - - - - - - - - - - - - - - - -
			Qfield = new Field("codcmpki", FieldType.CHAVE_PRIMARIA_GUID);
			...
			//- - - - - - - - - - - - - - - - - - -
			Qfield = new Field("quantida", FieldType.NUMERO);
			...
			//- - - - - - - - - - - - - - - - - - -
			Qfield = new Field("url", FieldType.TEXTO);
        ...
        /// <remarks>Persistence operations should not be used on a partially positioned register</remarks>
        [Obsolete("Use List<CSGenioAcmpki> searchList(PersistentSupport sp, User user, CriteriaSet where, string []fields) instead")]
        public static List<CSGenioAcmpki> searchList(PersistentSupport sp, User user, string where, string []fields = null)
        {
            return sp.searchListWhere<CSGenioAcmpki>(where, user, fields);
        }
  ...
  /*
        ...
        public void Reorder_Order(PersistentSupport sp, int position, CriteriaSet condition, List<Relation> relations = null)
        {
            ...
            UpdateQuery up = new UpdateQuery()
                        ...
                        .Set(CSGenioAcmpki.FldOrder, middle)
                        .Where(CriteriaSet.And().Equal(CSGenioAcmpki.FldCodcmpki, QPrimaryKey));
            sp.Execute(up);

            sp.ReorderSequence(Area.AreaCMPKI, CSGenioAcmpki.FldOrder, condition, relations);
