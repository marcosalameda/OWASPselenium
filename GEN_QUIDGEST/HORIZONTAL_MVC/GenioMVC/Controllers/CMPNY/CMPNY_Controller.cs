...
// USE /[MANUAL GQT CONTROLLER_NAVIGATION CMPNY]/
        ...
        {
            CSGenio.business.Area area = CSGenio.business.Area.createArea<CSGenioAcmpny>(UserContext.Current.User, UserContext.Current.User.CurrentModule);
...
// USE /[MANUAL GQT MANUAL_CONTROLLER CMPNY]/
        ...
        public JsonResult ReloadDBEdit(string Identifier, IDictionary<string, object> Values)
        {
            ...
            try { row = Models.Cmpny.Find(navigation.GetStrValue("cmpny")); }
            catch (Exception)
            {
					...
					case "EMPRE___CNTRYCOUNTRY_":	// Field (DB)
                        {
						    ...
						    var model = new Empre_ViewModel(navigation) { editable = false };
        ...
        public JsonResult GetDependants(string Identifier, string Selected, IDictionary<string, object> Limits)
        {
            ...
            try
            {
				...
				{
					case "EMPRE___CNTRYCOUNTRY_":	// Field (DB)
						values = Empre_ViewModel.GetDependant_EmpreTableCntryCountry(Selected, navigation);
						break;
					default: break;
				}
		...
		[AuthorizeForUsers]
        [ActionSessionState(System.Web.SessionState.SessionStateBehavior.ReadOnly)]
        public JsonResult RecalculateFormulas_Empre(Empre_ViewModel form_data)
        {
