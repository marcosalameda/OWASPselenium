...
namespace GenioMVC.Controllers
{
	public partial class InstaController : ControllerBase
	{
		...
		[AuthorizeForUsers]
		public ActionResult ChangeFormMode_Leafletd(string mode)
		{
			...
			switch (mode)
			{
				case "show":
					if (m.Contains("v"))
						_mode = ACTION_LEAFLETD_SHOW;
					break;
				case "delete":
					if (m.Contains("a"))
						_mode = ACTION_LEAFLETD_DELETE;
					break;
				case "edit":
					if (m.Contains("e"))
						_mode = ACTION_LEAFLETD_EDIT;
					break;
				case "duplicate":
					if (m.Contains("d"))
						_mode = ACTION_LEAFLETD_DUPLICATE;
					break;
				case "new":
					if (m.Contains("i"))
						_mode = ACTION_LEAFLETD_NEW;
					break;
			}
		...
		[AuthorizeForUsers]
		public ActionResult Leafletd_ModalDBEdit(string partialView)
		{
			Leafletd_ViewModel model = new Leafletd_ViewModel();
...
// USE /[MANUAL GQT CONTROLLER_SHOW LEAFLETD]/
		//
		// GET: /Insta/Leafletd_Show
		[AuthorizeForUsers]
		public ActionResult Leafletd_Show(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";
			var model = new Leafletd_ViewModel(Navigation, nestedForm);
			model.setModes(Request.QueryString["m"]);
			...
			string partialView = qs["partialView"] ?? "Leafletd"; // MF send the patial view name

			var navigationLocationAction = ACTION_LEAFLETD_SHOW.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
...
// USE /[MANUAL GQT BEFORE_LOAD_SHOW LEAFLETD]/
			...
			catch (ModelNotFoundException)
			{
				...
				CSGenio.framework.Log.Error("Leafletd_Show - GET " + id + " " + e.Message);
				throw;
			}
...
// USE /[MANUAL GQT AFTER_LOAD_SHOW LEAFLETD]/
			...
			if (isHomePage)
			{
				Navigation.SetValue("HomePage", "Leafletd");
				return PartialView(partialView, model);
			}
			else if (!Request.IsAjaxRequest())
				return View("Leafletd", model);
			else
				return PartialView(partialView, model);
		}
		...
		[ActionName("Leafletd_New_Insert")]
		[AuthorizeForUsers]
		[HttpGet]
		public ActionResult Leafletd_New_Insert()
		{
			...
			return RedirectToAction("Leafletd_New");
		}

// USE /[MANUAL GQT CONTROLLER_NEW_GET LEAFLETD]/
		//
		// GET: /Insta/Leafletd_New
		[ActionName("Leafletd_New")]
		[AuthorizeForUsers]
		[HttpGet]
		public ActionResult Leafletd_New()
		{
			...
			string partialView = qs["partialView"] ?? "Leafletd";

			var navigationLocationAction = ACTION_LEAFLETD_NEW.SetRoutedValues(new { m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);

			//FOR: OVERRIDE SKIP IF JUST ONE
			//Allow child form to use "Go Back" to menu list without "skip if only one"
			if (Navigation.OverrideSkipIfJustOne.ContainsKey("LEAFLETD"))
				Navigation.OverrideSkipIfJustOne["LEAFLETD"] = true;
...
// USE /[MANUAL GQT AFTER_LOAD_NEW LEAFLETD]/
			...
			catch (Exception e)
			{
				...
				CSGenio.framework.Log.Error("Leafletd_New - GET " + e.Message);
...
// USE /[MANUAL GQT CONTROLLER_NEW_POST LEAFLETD]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Leafletd_New(Leafletd_ViewModel model, bool redirect = true)
		{
...
// USE /[MANUAL GQT AFTER_LOAD_NEW_EX LEAFLETD]/
		...
		[AuthorizeForUsers]
		public ActionResult Leafletd_New_Redirect(bool internalRedirect = false)
		{
			...
			if (Convert.ToBoolean(Request.QueryString.Get("repeatInsertion")))
				return RedirectToLocation(ACTION_LEAFLETD_NEW, new { repeatInsertion = true });
...
// USE /[MANUAL GQT CONTROLLER_EDIT_GET LEAFLETD]/
		//
		// GET: /Insta/Leafletd_Edit
		[AuthorizeForUsers]
		[HttpGet]
		[ActionName("Leafletd_Edit")]
		public ActionResult Leafletd_Edit(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";
			string partialView = qs["partialView"] ?? "Leafletd"; // MF send the partial view name

			//Useful to initialize a record that had to be created by the framework prior to edition (and method New doesnt support this). This way, if the form then is canceled by the user, the record will be automatically deleted
			var isNewEdit = Navigation.GetStrValue("NewEdit") == "true";

			var navigationLocationAction =  isNewEdit ? ACTION_LEAFLETD_NEW.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] }) : ACTION_LEAFLETD_EDIT.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);
...
// USE /[MANUAL GQT BEFORE_LOAD_EDIT LEAFLETD]/
			...
			catch (ModelNotFoundException)
			{
				...
				sp.closeConnection();
				//JFG 05-05-2017 This need to better handled, for now it's handled on webconfig's customErrors section.
				CSGenio.framework.Log.Error("Leafletd_Edit - " + id + " " + e.Message);
				throw;
			}
...
// USE /[MANUAL GQT CONTROLLER_EDIT_POST LEAFLETD]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Leafletd_Edit(Leafletd_ViewModel model, bool redirect = true)
		{
...
// USE /[MANUAL GQT AFTER_LOAD_EDIT_EX LEAFLETD]/
		...
		[AuthorizeForUsers]
		public ActionResult Leafletd_Edit_Redirect(bool internalRedirect = false)
		{
...
// USE /[MANUAL GQT CONTROLLER_DELETE_GET LEAFLETD]/
		//
		// GET: /Insta/Leafletd_Delete
		[AuthorizeForUsers]
		[HttpGet]
		[ActionName("Leafletd_Delete")]
		public ActionResult Leafletd_Delete(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";

			var navigationLocationAction = ACTION_LEAFLETD_DELETE.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);
...
// USE /[MANUAL GQT BEFORE_LOAD_DELETE LEAFLETD]/
			...
			catch (ModelNotFoundException)
			{
				...
				//JFG 05-05-2017 This need to better handled, for now it's handled on webconfig's customErrors section.
				CSGenio.framework.Log.Error("Leafletd_Delete - " + id + " " + e.Message);
				throw;
			}
...
// USE /[MANUAL GQT AFTER_LOAD_DELETE LEAFLETD]/
			...
			if (!Request.IsAjaxRequest())
				...
				if (permission.Status.Equals(CSGenio.framework.Status.E))
					...
					return PartialView("Leafletd", model);
		}
...
// USE /[MANUAL GQT CONTROLLER_DELETE_POST LEAFLETD]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Leafletd_Delete(string id, FormCollection collection)
		{

			var model = new Leafletd_ViewModel (Navigation, id);
			model.MapFromModel();
...
// USE /[MANUAL GQT CONTROLLER_DUPLICATE_GET LEAFLETD]/
		//
		// GET: /Insta/Leafletd_Duplicate
		[AuthorizeForUsers]
		[HttpGet]
		public ActionResult Leafletd_Duplicate(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";
			var model = new Leafletd_ViewModel(Navigation, nestedForm);
			...
			var navigationLocationAction = ACTION_LEAFLETD_DUPLICATE.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);
...
// USE /[MANUAL GQT AFTER_LOAD_DUPLICATE LEAFLETD]/
					...
					sp.closeTransaction();
					Navigation.CurrentLevel.SetLocation(ACTION_LEAFLETD_DUPLICATE.SetRoutedValues(new { Id = model.ValCodinsta }));
...
// USE /[MANUAL GQT CONTROLLER_DUPLICATE_POST LEAFLETD]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Leafletd_Duplicate(Leafletd_ViewModel model, bool redirect = true)
		{
			var eventSink = new EventSink()
			{
...
// USE /[MANUAL GQT AFTER_LOAD_DUPLICATE_EX LEAFLETD]/
		...
		[AuthorizeForUsers]
		public ActionResult Leafletd_Duplicate_Redirect(bool internalRedirect = false)
		{
...
// USE /[MANUAL GQT AFTER_CANCEL LEAFLETD]/
		...
		{
			var model = new Leafletd_ViewModel(Navigation, true);
			model.setModes(Request.QueryString["m"]);
			PersistentSupport sp = UserContext.Current.PersistentSupport;
			var navigationLocationAction = ACTION_LEAFLETD_NEW.SetRoutedValues(new { m = Request.QueryString["m"] });
			CheckLevels(navigationLocationAction);
			...
			return PartialView("MFLeafletd", model);
		}
		...
		public ActionResult MFLeafletd_Edit(string id)
		{
			return this.RedirectToAction("Leafletd_Edit", "Insta", new { id = id, partialView = "MFLeafletd", nestedForm = "true", multiForm = "true" });
		}
		...
		public ActionResult MFLeafletd_Cancel(string id)
		{
			//MH - Validate if current level is it correct. Remove all levels that is higher than the currently level.
			CheckLevels(NavigationLocation.Any);

			if (Navigation.CurrentLevel.IsNestedContext &&
				(Navigation.CurrentLevel.Location.Action == ACTION_LEAFLETD_NEW.Action || Navigation.CurrentLevel.Location.Action == ACTION_LEAFLETD_EDIT.Action))
			{
				if (Navigation.CurrentLevel.FormMode == FormMode.New)
				{
					...
					catch (Exception e)
					{
						...
						Log.Error("MFLeafletd_Cancel - " + exceptionUserMessage);
						ErrorMessage(exceptionUserMessage);
					}
				}
			...
			return this.RedirectToAction("Leafletd_Show", "Insta", new { id = id, partialView = "MFLeafletd", nestedForm = "true", multiForm = "true", nav });
		}
		...
		public JsonResult MFLeafletd_Delete(string id)
		{
			var eventSink = new EventSink()
			{
		...
		public ActionResult Leafletd_EquipValRegistnr(string id, string partialView,  IDictionary<string, string> Limits)
		{
			...
			Leafletd_EquipValRegistnr_ViewModel model = new Leafletd_EquipValRegistnr_ViewModel(navigation);
			model.setModes(Request.QueryString["m"]);
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Leafletd_SaveEdit(Leafletd_ViewModel model)
		{
			var eventSink = new EventSink()
			{
