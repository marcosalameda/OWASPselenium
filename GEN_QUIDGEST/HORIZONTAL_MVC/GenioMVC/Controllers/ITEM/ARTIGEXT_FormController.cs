...
namespace GenioMVC.Controllers
{
	public partial class ItemController : ControllerBase
	{
		...
		[AuthorizeForUsers]
		public ActionResult ChangeFormMode_Artigext(string mode)
		{
			...
			switch (mode)
			{
				case "show":
					if (m.Contains("v"))
						_mode = ACTION_ARTIGEXT_SHOW;
					break;
				case "delete":
					if (m.Contains("a"))
						_mode = ACTION_ARTIGEXT_DELETE;
					break;
				case "edit":
					if (m.Contains("e"))
						_mode = ACTION_ARTIGEXT_EDIT;
					break;
				case "duplicate":
					if (m.Contains("d"))
						_mode = ACTION_ARTIGEXT_DUPLICATE;
					break;
				case "new":
					if (m.Contains("i"))
						_mode = ACTION_ARTIGEXT_NEW;
					break;
			}
		...
		[AuthorizeForUsers]
		public ActionResult Artigext_ModalDBEdit(string partialView)
		{
			Artigext_ViewModel model = new Artigext_ViewModel();
...
// USE /[MANUAL GQT CONTROLLER_SHOW ARTIGEXT]/
		//
		// GET: /Item/Artigext_Show
		[AuthorizeForUsers]
		public ActionResult Artigext_Show(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";
			var model = new Artigext_ViewModel(Navigation, nestedForm);
			model.setModes(Request.QueryString["m"]);
			...
			string partialView = qs["partialView"] ?? "Artigext"; // MF send the patial view name

			var navigationLocationAction = ACTION_ARTIGEXT_SHOW.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
...
// USE /[MANUAL GQT BEFORE_LOAD_SHOW ARTIGEXT]/
			...
			catch (ModelNotFoundException)
			{
				...
				CSGenio.framework.Log.Error("Artigext_Show - GET " + id + " " + e.Message);
				throw;
			}
...
// USE /[MANUAL GQT AFTER_LOAD_SHOW ARTIGEXT]/
			...
			if (isHomePage)
			{
				Navigation.SetValue("HomePage", "Artigext");
				return PartialView(partialView, model);
			}
			else if (!Request.IsAjaxRequest())
				return View("Artigext", model);
			else
				return PartialView(partialView, model);
		}
		...
		[ActionName("Artigext_New_Insert")]
		[AuthorizeForUsers]
		[HttpGet]
		public ActionResult Artigext_New_Insert()
		{
			...
			return RedirectToAction("Artigext_New");
		}

// USE /[MANUAL GQT CONTROLLER_NEW_GET ARTIGEXT]/
		//
		// GET: /Item/Artigext_New
		[ActionName("Artigext_New")]
		[AuthorizeForUsers]
		[HttpGet]
		public ActionResult Artigext_New()
		{
			...
			string partialView = qs["partialView"] ?? "Artigext";

			var navigationLocationAction = ACTION_ARTIGEXT_NEW.SetRoutedValues(new { m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);

			//FOR: OVERRIDE SKIP IF JUST ONE
			//Allow child form to use "Go Back" to menu list without "skip if only one"
			if (Navigation.OverrideSkipIfJustOne.ContainsKey("ARTIGEXT"))
				Navigation.OverrideSkipIfJustOne["ARTIGEXT"] = true;
...
// USE /[MANUAL GQT AFTER_LOAD_NEW ARTIGEXT]/
			...
			catch (Exception e)
			{
				...
				CSGenio.framework.Log.Error("Artigext_New - GET " + e.Message);
...
// USE /[MANUAL GQT CONTROLLER_NEW_POST ARTIGEXT]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Artigext_New(Artigext_ViewModel model, bool redirect = true)
		{
...
// USE /[MANUAL GQT AFTER_LOAD_NEW_EX ARTIGEXT]/
		...
		[AuthorizeForUsers]
		public ActionResult Artigext_New_Redirect(bool internalRedirect = false)
		{
			...
			if (Convert.ToBoolean(Request.QueryString.Get("repeatInsertion")))
				return RedirectToLocation(ACTION_ARTIGEXT_NEW, new { repeatInsertion = true });
...
// USE /[MANUAL GQT CONTROLLER_EDIT_GET ARTIGEXT]/
		//
		// GET: /Item/Artigext_Edit
		[AuthorizeForUsers]
		[HttpGet]
		[ActionName("Artigext_Edit")]
		public ActionResult Artigext_Edit(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";
			string partialView = qs["partialView"] ?? "Artigext"; // MF send the partial view name

			//Useful to initialize a record that had to be created by the framework prior to edition (and method New doesnt support this). This way, if the form then is canceled by the user, the record will be automatically deleted
			var isNewEdit = Navigation.GetStrValue("NewEdit") == "true";

			var navigationLocationAction =  isNewEdit ? ACTION_ARTIGEXT_NEW.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] }) : ACTION_ARTIGEXT_EDIT.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);
...
// USE /[MANUAL GQT BEFORE_LOAD_EDIT ARTIGEXT]/
			...
			catch (ModelNotFoundException)
			{
				...
				sp.closeConnection();
				//JFG 05-05-2017 This need to better handled, for now it's handled on webconfig's customErrors section.
				CSGenio.framework.Log.Error("Artigext_Edit - " + id + " " + e.Message);
				throw;
			}
...
// USE /[MANUAL GQT CONTROLLER_EDIT_POST ARTIGEXT]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Artigext_Edit(Artigext_ViewModel model, bool redirect = true)
		{
...
// USE /[MANUAL GQT AFTER_LOAD_EDIT_EX ARTIGEXT]/
		...
		[AuthorizeForUsers]
		public ActionResult Artigext_Edit_Redirect(bool internalRedirect = false)
		{
...
// USE /[MANUAL GQT CONTROLLER_DELETE_GET ARTIGEXT]/
		//
		// GET: /Item/Artigext_Delete
		[AuthorizeForUsers]
		[HttpGet]
		[ActionName("Artigext_Delete")]
		public ActionResult Artigext_Delete(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";

			var navigationLocationAction = ACTION_ARTIGEXT_DELETE.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);
...
// USE /[MANUAL GQT BEFORE_LOAD_DELETE ARTIGEXT]/
			...
			catch (ModelNotFoundException)
			{
				...
				//JFG 05-05-2017 This need to better handled, for now it's handled on webconfig's customErrors section.
				CSGenio.framework.Log.Error("Artigext_Delete - " + id + " " + e.Message);
				throw;
			}
...
// USE /[MANUAL GQT AFTER_LOAD_DELETE ARTIGEXT]/
			...
			if (!Request.IsAjaxRequest())
				...
				if (permission.Status.Equals(CSGenio.framework.Status.E))
					...
					return PartialView("Artigext", model);
		}
...
// USE /[MANUAL GQT CONTROLLER_DELETE_POST ARTIGEXT]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Artigext_Delete(string id, FormCollection collection)
		{

			var model = new Artigext_ViewModel (Navigation, id);
			model.MapFromModel();
...
// USE /[MANUAL GQT CONTROLLER_DUPLICATE_GET ARTIGEXT]/
		//
		// GET: /Item/Artigext_Duplicate
		[AuthorizeForUsers]
		[HttpGet]
		public ActionResult Artigext_Duplicate(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";
			var model = new Artigext_ViewModel(Navigation, nestedForm);
			...
			var navigationLocationAction = ACTION_ARTIGEXT_DUPLICATE.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);
...
// USE /[MANUAL GQT AFTER_LOAD_DUPLICATE ARTIGEXT]/
					...
					sp.closeTransaction();
					Navigation.CurrentLevel.SetLocation(ACTION_ARTIGEXT_DUPLICATE.SetRoutedValues(new { Id = model.ValCoditem }));
...
// USE /[MANUAL GQT CONTROLLER_DUPLICATE_POST ARTIGEXT]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Artigext_Duplicate(Artigext_ViewModel model, bool redirect = true)
		{
			var eventSink = new EventSink()
			{
...
// USE /[MANUAL GQT AFTER_LOAD_DUPLICATE_EX ARTIGEXT]/
		...
		[AuthorizeForUsers]
		public ActionResult Artigext_Duplicate_Redirect(bool internalRedirect = false)
		{
...
// USE /[MANUAL GQT AFTER_CANCEL ARTIGEXT]/
		...
		{
			var model = new Artigext_ViewModel(Navigation, true);
			model.setModes(Request.QueryString["m"]);
			PersistentSupport sp = UserContext.Current.PersistentSupport;
			var navigationLocationAction = ACTION_ARTIGEXT_NEW.SetRoutedValues(new { m = Request.QueryString["m"] });
			CheckLevels(navigationLocationAction);
			...
			return PartialView("MFArtigext", model);
		}
		...
		public ActionResult MFArtigext_Edit(string id)
		{
			return this.RedirectToAction("Artigext_Edit", "Item", new { id = id, partialView = "MFArtigext", nestedForm = "true", multiForm = "true" });
		}
		...
		public ActionResult MFArtigext_Cancel(string id)
		{
			//MH - Validate if current level is it correct. Remove all levels that is higher than the currently level.
			CheckLevels(NavigationLocation.Any);

			if (Navigation.CurrentLevel.IsNestedContext &&
				(Navigation.CurrentLevel.Location.Action == ACTION_ARTIGEXT_NEW.Action || Navigation.CurrentLevel.Location.Action == ACTION_ARTIGEXT_EDIT.Action))
			{
				if (Navigation.CurrentLevel.FormMode == FormMode.New)
				{
					...
					catch (Exception e)
					{
						...
						Log.Error("MFArtigext_Cancel - " + exceptionUserMessage);
						ErrorMessage(exceptionUserMessage);
					}
				}
			...
			return this.RedirectToAction("Artigext_Show", "Item", new { id = id, partialView = "MFArtigext", nestedForm = "true", multiForm = "true", nav });
		}
		...
		public JsonResult MFArtigext_Delete(string id)
		{
			var eventSink = new EventSink()
			{
		...
		public ActionResult Artigext_WarehValWarehdes(string id, string partialView,  IDictionary<string, string> Limits)
		{
			...
			Artigext_WarehValWarehdes_ViewModel model = new Artigext_WarehValWarehdes_ViewModel(navigation);
			model.setModes(Request.QueryString["m"]);
			model.ValCoditem = id;
		...
		[ActionSessionState(System.Web.SessionState.SessionStateBehavior.ReadOnly)]
		public ActionResult Artigext_GitemValItemdes(string id, string partialView,  IDictionary<string, string> Limits)
		{
			int perPage = CSGenio.framework.Configuration.NrRegDBedit;
			...
			NameValueCollection requestValues = Request.Unvalidated.Form.Count > 0 ? Request.Unvalidated.Form : Request.QueryString; //TSX (01.07.2020) Can not access directly to the FormCollection or made Request.Form otherwise the tags on viewmodel will be ignored
			var navigation = Navigation.Clone();
			Artigext_GitemValItemdes_ViewModel model = new Artigext_GitemValItemdes_ViewModel(navigation);
			model.setModes(Request.QueryString["m"]);
			model.ValCoditem = id;
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Artigext_SaveEdit(Artigext_ViewModel model)
		{
			var eventSink = new EventSink()
			{
