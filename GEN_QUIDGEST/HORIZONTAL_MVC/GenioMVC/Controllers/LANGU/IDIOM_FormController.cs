...
namespace GenioMVC.Controllers
{
	public partial class LanguController : ControllerBase
	{
		...
		[AuthorizeForUsers]
		public ActionResult ChangeFormMode_Idiom(string mode)
		{
			var _mode = Navigation.CurrentLevel.Location;
			var id = Navigation.GetStrValue("langu");
			...
			switch (mode)
			{
				case "show":
					if (m.Contains("v"))
						_mode = ACTION_IDIOM_SHOW;
					break;
				case "delete":
					if (m.Contains("a"))
						_mode = ACTION_IDIOM_DELETE;
					break;
				case "edit":
					if (m.Contains("e"))
						_mode = ACTION_IDIOM_EDIT;
					break;
				case "duplicate":
					if (m.Contains("d"))
						_mode = ACTION_IDIOM_DUPLICATE;
					break;
				case "new":
					if (m.Contains("i"))
						_mode = ACTION_IDIOM_NEW;
					break;
			}
...
// USE /[MANUAL GQT CONTROLLER_SHOW IDIOM]/
		//
		// GET: /Langu/Idiom_Show
		[AuthorizeForUsers]
		public ActionResult Idiom_Show(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";
			var model = new Idiom_ViewModel(Navigation, nestedForm);
			...
			string partialView = qs["partialView"] ?? "Idiom"; // MF send the patial view name

			var navigationLocationAction = ACTION_IDIOM_SHOW.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			...
			if (!isHomePage && (IsNewLocation(navigationLocationAction) || !string.Equals(Navigation.GetStrValue("langu"), id)))
			{
				Navigation.AddHistoryLevel(navigationLocationAction, FormMode.Show, nestedForm);
				CSGenio.framework.Audit.registAction(UserContext.Current.User, Resources.Resources.FORM54242 + " " + Navigation.CurrentLevel.Location.ShortDescription());
			}

			Navigation.SetValue("langu", id);
...
// USE /[MANUAL GQT BEFORE_LOAD_SHOW IDIOM]/
			...
			catch (ModelNotFoundException)
			{
				...
				CSGenio.framework.Log.Error("Idiom_Show - GET " + id + " " + e.Message);
				throw;
			}
...
// USE /[MANUAL GQT AFTER_LOAD_SHOW IDIOM]/
			...
			if (isHomePage)
			{
				Navigation.SetValue("HomePage", "Idiom");
				return PartialView(partialView, model);
			}
			else if (!Request.IsAjaxRequest())
				return View("Idiom", model);
			else
				return PartialView(partialView, model);
		}
		...
		[ActionName("Idiom_New_Insert")]
		[AuthorizeForUsers]
		[HttpGet]
		public ActionResult Idiom_New_Insert()
		{
			...
			return RedirectToAction("Idiom_New");
		}

// USE /[MANUAL GQT CONTROLLER_NEW_GET IDIOM]/
		//
		// GET: /Langu/Idiom_New
		[ActionName("Idiom_New")]
		[AuthorizeForUsers]
		[HttpGet]
		public ActionResult Idiom_New()
		{
			...
			string partialView = qs["partialView"] ?? "Idiom";

			var navigationLocationAction = ACTION_IDIOM_NEW.SetRoutedValues(new { m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);

			//FOR: OVERRIDE SKIP IF JUST ONE
			//Allow child form to use "Go Back" to menu list without "skip if only one"
			if (Navigation.OverrideSkipIfJustOne.ContainsKey("IDIOM"))
				Navigation.OverrideSkipIfJustOne["IDIOM"] = true;
...
// USE /[MANUAL GQT AFTER_LOAD_NEW IDIOM]/
			...
			catch (Exception e)
			{
				...
				CSGenio.framework.Log.Error("Idiom_New - GET " + e.Message);
...
// USE /[MANUAL GQT CONTROLLER_NEW_POST IDIOM]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Idiom_New(Idiom_ViewModel model, bool redirect = true)
		{
...
// USE /[MANUAL GQT AFTER_LOAD_NEW_EX IDIOM]/
		...
		[AuthorizeForUsers]
		public ActionResult Idiom_New_Redirect(bool internalRedirect = false)
		{
			IList<string> warningMsgs = TempData["NEW_WARNINGS_LIST"] as List<string>;
			string saveMsg = TempData["NEW_SAVE_LIST"] as string;

			if (!internalRedirect)
				SuccessMessage(String.IsNullOrEmpty(saveMsg) ? Resources.Resources.REGISTO_CRIADO_COM_S18746 + GetHumanKeyToQMessage("langu", Convert.ToString(Navigation.CurrentLevel.GetEntry("langu"))) : saveMsg);
			...
			if (Convert.ToBoolean(Request.QueryString.Get("repeatInsertion")))
				return RedirectToLocation(ACTION_IDIOM_NEW, new { repeatInsertion = true });
...
// USE /[MANUAL GQT CONTROLLER_EDIT_GET IDIOM]/
		//
		// GET: /Langu/Idiom_Edit
		[AuthorizeForUsers]
		[HttpGet]
		[ActionName("Idiom_Edit")]
		public ActionResult Idiom_Edit(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";
			string partialView = qs["partialView"] ?? "Idiom"; // MF send the partial view name

			//Useful to initialize a record that had to be created by the framework prior to edition (and method New doesnt support this). This way, if the form then is canceled by the user, the record will be automatically deleted
			var isNewEdit = Navigation.GetStrValue("NewEdit") == "true";

			var navigationLocationAction =  isNewEdit ? ACTION_IDIOM_NEW.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] }) : ACTION_IDIOM_EDIT.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);
...
// USE /[MANUAL GQT BEFORE_LOAD_EDIT IDIOM]/
			...
			catch (ModelNotFoundException)
			{
				...
				sp.closeConnection();
				//JFG 05-05-2017 This need to better handled, for now it's handled on webconfig's customErrors section.
				CSGenio.framework.Log.Error("Idiom_Edit - " + id + " " + e.Message);
				throw;
			}
...
// USE /[MANUAL GQT CONTROLLER_EDIT_POST IDIOM]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Idiom_Edit(Idiom_ViewModel model, bool redirect = true)
		{
...
// USE /[MANUAL GQT AFTER_LOAD_EDIT_EX IDIOM]/
		...
		[AuthorizeForUsers]
		public ActionResult Idiom_Edit_Redirect(bool internalRedirect = false)
		{
			IList<string> warningMsgs = TempData["EDIT_WARNINGS_LIST"] as List<string>;
			string saveMsg = TempData["NEW_SAVE_LIST"] as string;

			if (!internalRedirect || !Request.IsAjaxRequest())
				SuccessMessage(String.IsNullOrEmpty(saveMsg) ? Resources.Resources.ALTERACOES_EFECTUADA64514 + GetHumanKeyToQMessage("langu", Convert.ToString(Navigation.CurrentLevel.GetEntry("langu"))) : saveMsg);
...
// USE /[MANUAL GQT CONTROLLER_DELETE_GET IDIOM]/
		//
		// GET: /Langu/Idiom_Delete
		[AuthorizeForUsers]
		[HttpGet]
		[ActionName("Idiom_Delete")]
		public ActionResult Idiom_Delete(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";

			var navigationLocationAction = ACTION_IDIOM_DELETE.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);
...
// USE /[MANUAL GQT BEFORE_LOAD_DELETE IDIOM]/
			...
			catch (ModelNotFoundException)
			{
				...
				//JFG 05-05-2017 This need to better handled, for now it's handled on webconfig's customErrors section.
				CSGenio.framework.Log.Error("Idiom_Delete - " + id + " " + e.Message);
				throw;
			}
...
// USE /[MANUAL GQT AFTER_LOAD_DELETE IDIOM]/
			...
			if (!Request.IsAjaxRequest())
				...
				if (permission.Status.Equals(CSGenio.framework.Status.E))
					...
					return PartialView("Idiom", model);
		}
...
// USE /[MANUAL GQT CONTROLLER_DELETE_POST IDIOM]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Idiom_Delete(string id, FormCollection collection)
		{

			var model = new Idiom_ViewModel (Navigation, id);
			model.MapFromModel();
...
// USE /[MANUAL GQT CONTROLLER_DUPLICATE_GET IDIOM]/
		//
		// GET: /Langu/Idiom_Duplicate
		[AuthorizeForUsers]
		[HttpGet]
		public ActionResult Idiom_Duplicate(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";
			var model = new Idiom_ViewModel(Navigation, nestedForm);
			...
			var navigationLocationAction = ACTION_IDIOM_DUPLICATE.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);
...
// USE /[MANUAL GQT AFTER_LOAD_DUPLICATE IDIOM]/
					...
					sp.closeTransaction();
					Navigation.CurrentLevel.SetLocation(ACTION_IDIOM_DUPLICATE.SetRoutedValues(new { Id = model.ValCodlang }));
...
// USE /[MANUAL GQT CONTROLLER_DUPLICATE_POST IDIOM]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Idiom_Duplicate(Idiom_ViewModel model, bool redirect = true)
		{
			var eventSink = new EventSink()
			{
...
// USE /[MANUAL GQT AFTER_LOAD_DUPLICATE_EX IDIOM]/
		...
		[AuthorizeForUsers]
		public ActionResult Idiom_Duplicate_Redirect(bool internalRedirect = false)
		{
...
// USE /[MANUAL GQT CONTROLLER_CANCEL_GET IDIOM]/
		...
		public ActionResult Idiom_Cancel()
		{
			...
			if (Navigation.CurrentLevel.FormMode == FormMode.New || Navigation.CurrentLevel.FormMode == FormMode.Duplicate)
			{
				...
				try
				{
					var model = new GenioMVC.Models.Langu();
...
// USE /[MANUAL GQT AFTER_CANCEL IDIOM]/
			...
			else if (Navigation.PreviousLevel != null)
			{
				...
				string previousLevelArea = Navigation.PreviousLevel.Location.Controller?.ToLower() ?? "langu";
		...
		{
			var model = new Idiom_ViewModel(Navigation, true);
			model.setModes(Request.QueryString["m"]);
			PersistentSupport sp = UserContext.Current.PersistentSupport;
			var navigationLocationAction = ACTION_IDIOM_NEW.SetRoutedValues(new { m = Request.QueryString["m"] });
			CheckLevels(navigationLocationAction);
			...
			return PartialView("MFIdiom", model);
		}
		...
		public ActionResult MFIdiom_Edit(string id)
		{
			return this.RedirectToAction("Idiom_Edit", "Langu", new { id = id, partialView = "MFIdiom", nestedForm = "true", multiForm = "true" });
		}
		...
		public ActionResult MFIdiom_Cancel(string id)
		{
			//MH - Validate if current level is it correct. Remove all levels that is higher than the currently level.
			CheckLevels(NavigationLocation.Any);

			if (Navigation.CurrentLevel.IsNestedContext &&
				(Navigation.CurrentLevel.Location.Action == ACTION_IDIOM_NEW.Action || Navigation.CurrentLevel.Location.Action == ACTION_IDIOM_EDIT.Action))
			{
				if (Navigation.CurrentLevel.FormMode == FormMode.New)
				{
					...
					catch (Exception e)
					{
						...
						Log.Error("MFIdiom_Cancel - " + exceptionUserMessage);
						ErrorMessage(exceptionUserMessage);
					}
				}
			...
			return this.RedirectToAction("Idiom_Show", "Langu", new { id = id, partialView = "MFIdiom", nestedForm = "true", multiForm = "true", nav });
		}
		...
		public JsonResult MFIdiom_Delete(string id)
		{
			var eventSink = new EventSink()
			{
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Idiom_SaveEdit(Idiom_ViewModel model)
		{
			var eventSink = new EventSink()
			{
