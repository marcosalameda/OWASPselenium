...
namespace GenioMVC.Controllers
{
	public partial class NotifController : ControllerBase
	{
		...
		[AuthorizeForUsers]
		public ActionResult ChangeFormMode_Notif(string mode)
		{
			var _mode = Navigation.CurrentLevel.Location;
			var id = Navigation.GetStrValue("notif");
			...
			switch (mode)
			{
				case "show":
					if (m.Contains("v"))
						_mode = ACTION_NOTIF_SHOW;
					break;
				case "delete":
					if (m.Contains("a"))
						_mode = ACTION_NOTIF_DELETE;
					break;
				case "edit":
					if (m.Contains("e"))
						_mode = ACTION_NOTIF_EDIT;
					break;
				case "duplicate":
					if (m.Contains("d"))
						_mode = ACTION_NOTIF_DUPLICATE;
					break;
				case "new":
					if (m.Contains("i"))
						_mode = ACTION_NOTIF_NEW;
					break;
			}
		...
		[AuthorizeForUsers]
		public ActionResult Notif_ModalDBEdit(string partialView)
		{
			Notif_ViewModel model = new Notif_ViewModel();
...
// USE /[MANUAL GQT CONTROLLER_SHOW NOTIF]/
		//
		// GET: /Notif/Notif_Show
		[AuthorizeForUsers]
		public ActionResult Notif_Show(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";
			var model = new Notif_ViewModel(Navigation, nestedForm);
			model.setModes(Request.QueryString["m"]);
			...
			string partialView = qs["partialView"] ?? "Notif"; // MF send the patial view name

			var navigationLocationAction = ACTION_NOTIF_SHOW.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			...
			if (!isHomePage && (IsNewLocation(navigationLocationAction) || !string.Equals(Navigation.GetStrValue("notif"), id)))
			{
				Navigation.AddHistoryLevel(navigationLocationAction, FormMode.Show, nestedForm);
				CSGenio.framework.Audit.registAction(UserContext.Current.User, Resources.Resources.FORM54242 + " " + Navigation.CurrentLevel.Location.ShortDescription());
			}

			Navigation.SetValue("notif", id);
...
// USE /[MANUAL GQT BEFORE_LOAD_SHOW NOTIF]/
			...
			catch (ModelNotFoundException)
			{
				...
				CSGenio.framework.Log.Error("Notif_Show - GET " + id + " " + e.Message);
				throw;
			}
...
// USE /[MANUAL GQT AFTER_LOAD_SHOW NOTIF]/
			...
			if (isHomePage)
			{
				Navigation.SetValue("HomePage", "Notif");
				return PartialView(partialView, model);
			}
			else if (!Request.IsAjaxRequest())
				return View("Notif", model);
			else
				return PartialView(partialView, model);
		}
		...
		[ActionName("Notif_New_Insert")]
		[AuthorizeForUsers]
		[HttpGet]
		public ActionResult Notif_New_Insert()
		{
			...
			return RedirectToAction("Notif_New");
		}

// USE /[MANUAL GQT CONTROLLER_NEW_GET NOTIF]/
		//
		// GET: /Notif/Notif_New
		[ActionName("Notif_New")]
		[AuthorizeForUsers]
		[HttpGet]
		public ActionResult Notif_New()
		{
			...
			string partialView = qs["partialView"] ?? "Notif";

			var navigationLocationAction = ACTION_NOTIF_NEW.SetRoutedValues(new { m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);

			//FOR: OVERRIDE SKIP IF JUST ONE
			//Allow child form to use "Go Back" to menu list without "skip if only one"
			if (Navigation.OverrideSkipIfJustOne.ContainsKey("NOTIF"))
				Navigation.OverrideSkipIfJustOne["NOTIF"] = true;
...
// USE /[MANUAL GQT AFTER_LOAD_NEW NOTIF]/
			...
			catch (Exception e)
			{
				...
				CSGenio.framework.Log.Error("Notif_New - GET " + e.Message);
...
// USE /[MANUAL GQT CONTROLLER_NEW_POST NOTIF]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Notif_New(Notif_ViewModel model, bool redirect = true)
		{
...
// USE /[MANUAL GQT AFTER_LOAD_NEW_EX NOTIF]/
		...
		[AuthorizeForUsers]
		public ActionResult Notif_New_Redirect(bool internalRedirect = false)
		{
			IList<string> warningMsgs = TempData["NEW_WARNINGS_LIST"] as List<string>;
			string saveMsg = TempData["NEW_SAVE_LIST"] as string;

			if (!internalRedirect)
				SuccessMessage(String.IsNullOrEmpty(saveMsg) ? Resources.Resources.REGISTO_CRIADO_COM_S18746 + GetHumanKeyToQMessage("notif", Convert.ToString(Navigation.CurrentLevel.GetEntry("notif"))) : saveMsg);
			...
			if (Convert.ToBoolean(Request.QueryString.Get("repeatInsertion")))
				return RedirectToLocation(ACTION_NOTIF_NEW, new { repeatInsertion = true });
...
// USE /[MANUAL GQT CONTROLLER_EDIT_GET NOTIF]/
		//
		// GET: /Notif/Notif_Edit
		[AuthorizeForUsers]
		[HttpGet]
		[ActionName("Notif_Edit")]
		public ActionResult Notif_Edit(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";
			string partialView = qs["partialView"] ?? "Notif"; // MF send the partial view name

			//Useful to initialize a record that had to be created by the framework prior to edition (and method New doesnt support this). This way, if the form then is canceled by the user, the record will be automatically deleted
			var isNewEdit = Navigation.GetStrValue("NewEdit") == "true";

			var navigationLocationAction =  isNewEdit ? ACTION_NOTIF_NEW.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] }) : ACTION_NOTIF_EDIT.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);
...
// USE /[MANUAL GQT BEFORE_LOAD_EDIT NOTIF]/
			...
			catch (ModelNotFoundException)
			{
				...
				sp.closeConnection();
				//JFG 05-05-2017 This need to better handled, for now it's handled on webconfig's customErrors section.
				CSGenio.framework.Log.Error("Notif_Edit - " + id + " " + e.Message);
				throw;
			}
...
// USE /[MANUAL GQT CONTROLLER_EDIT_POST NOTIF]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Notif_Edit(Notif_ViewModel model, bool redirect = true)
		{
...
// USE /[MANUAL GQT AFTER_LOAD_EDIT_EX NOTIF]/
		...
		[AuthorizeForUsers]
		public ActionResult Notif_Edit_Redirect(bool internalRedirect = false)
		{
			IList<string> warningMsgs = TempData["EDIT_WARNINGS_LIST"] as List<string>;
			string saveMsg = TempData["NEW_SAVE_LIST"] as string;

			if (!internalRedirect || !Request.IsAjaxRequest())
				SuccessMessage(String.IsNullOrEmpty(saveMsg) ? Resources.Resources.ALTERACOES_EFECTUADA64514 + GetHumanKeyToQMessage("notif", Convert.ToString(Navigation.CurrentLevel.GetEntry("notif"))) : saveMsg);
...
// USE /[MANUAL GQT CONTROLLER_DELETE_GET NOTIF]/
		//
		// GET: /Notif/Notif_Delete
		[AuthorizeForUsers]
		[HttpGet]
		[ActionName("Notif_Delete")]
		public ActionResult Notif_Delete(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";

			var navigationLocationAction = ACTION_NOTIF_DELETE.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);
...
// USE /[MANUAL GQT BEFORE_LOAD_DELETE NOTIF]/
			...
			catch (ModelNotFoundException)
			{
				...
				//JFG 05-05-2017 This need to better handled, for now it's handled on webconfig's customErrors section.
				CSGenio.framework.Log.Error("Notif_Delete - " + id + " " + e.Message);
				throw;
			}
...
// USE /[MANUAL GQT AFTER_LOAD_DELETE NOTIF]/
			...
			if (!Request.IsAjaxRequest())
				...
				if (permission.Status.Equals(CSGenio.framework.Status.E))
					...
					return PartialView("Notif", model);
		}
...
// USE /[MANUAL GQT CONTROLLER_DELETE_POST NOTIF]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Notif_Delete(string id, FormCollection collection)
		{

			var model = new Notif_ViewModel (Navigation, id);
			model.MapFromModel();
...
// USE /[MANUAL GQT CONTROLLER_DUPLICATE_GET NOTIF]/
		//
		// GET: /Notif/Notif_Duplicate
		[AuthorizeForUsers]
		[HttpGet]
		public ActionResult Notif_Duplicate(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";
			var model = new Notif_ViewModel(Navigation, nestedForm);
			...
			var navigationLocationAction = ACTION_NOTIF_DUPLICATE.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);
...
// USE /[MANUAL GQT AFTER_LOAD_DUPLICATE NOTIF]/
					...
					sp.closeTransaction();
					Navigation.CurrentLevel.SetLocation(ACTION_NOTIF_DUPLICATE.SetRoutedValues(new { Id = model.ValCodnotif }));
...
// USE /[MANUAL GQT CONTROLLER_DUPLICATE_POST NOTIF]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Notif_Duplicate(Notif_ViewModel model, bool redirect = true)
		{
			var eventSink = new EventSink()
			{
...
// USE /[MANUAL GQT AFTER_LOAD_DUPLICATE_EX NOTIF]/
		...
		[AuthorizeForUsers]
		public ActionResult Notif_Duplicate_Redirect(bool internalRedirect = false)
		{
...
// USE /[MANUAL GQT CONTROLLER_CANCEL_GET NOTIF]/
		...
		public ActionResult Notif_Cancel()
		{
			...
			if (Navigation.CurrentLevel.FormMode == FormMode.New || Navigation.CurrentLevel.FormMode == FormMode.Duplicate)
			{
				...
				try
				{
					var model = new GenioMVC.Models.Notif();
...
// USE /[MANUAL GQT AFTER_CANCEL NOTIF]/
			...
			else if (Navigation.PreviousLevel != null)
			{
				...
				string previousLevelArea = Navigation.PreviousLevel.Location.Controller?.ToLower() ?? "notif";
		...
		{
			var model = new Notif_ViewModel(Navigation, true);
			model.setModes(Request.QueryString["m"]);
			PersistentSupport sp = UserContext.Current.PersistentSupport;
			var navigationLocationAction = ACTION_NOTIF_NEW.SetRoutedValues(new { m = Request.QueryString["m"] });
			CheckLevels(navigationLocationAction);
			...
			return PartialView("MFNotif", model);
		}
		...
		public ActionResult MFNotif_Edit(string id)
		{
			return this.RedirectToAction("Notif_Edit", "Notif", new { id = id, partialView = "MFNotif", nestedForm = "true", multiForm = "true" });
		}
		...
		public ActionResult MFNotif_Cancel(string id)
		{
			//MH - Validate if current level is it correct. Remove all levels that is higher than the currently level.
			CheckLevels(NavigationLocation.Any);

			if (Navigation.CurrentLevel.IsNestedContext &&
				(Navigation.CurrentLevel.Location.Action == ACTION_NOTIF_NEW.Action || Navigation.CurrentLevel.Location.Action == ACTION_NOTIF_EDIT.Action))
			{
				if (Navigation.CurrentLevel.FormMode == FormMode.New)
				{
					...
					catch (Exception e)
					{
						...
						Log.Error("MFNotif_Cancel - " + exceptionUserMessage);
						ErrorMessage(exceptionUserMessage);
					}
				}
			...
			return this.RedirectToAction("Notif_Show", "Notif", new { id = id, partialView = "MFNotif", nestedForm = "true", multiForm = "true", nav });
		}
		...
		public JsonResult MFNotif_Delete(string id)
		{
			var eventSink = new EventSink()
			{
		...
		public ActionResult Notif_Pess2ValName(string id, string partialView,  IDictionary<string, string> Limits)
		{
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Notif_SaveEdit(Notif_ViewModel model)
		{
			var eventSink = new EventSink()
			{
