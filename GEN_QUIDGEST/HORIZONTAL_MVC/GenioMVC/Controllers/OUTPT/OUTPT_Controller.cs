...
// USE /[MANUAL GQT MANUAL_CONTROLLER OUTPT]/
        ...
        public JsonResult ReloadDBEdit(string Identifier, IDictionary<string, object> Values)
        {
            ...
            try { row = Models.Outpt.Find(navigation.GetStrValue("outpt")); }
            catch (Exception)
            {
					...
					case "DSAID___WARE1WAREHDES":	// Field (DB)
                        {
						    ...
						    var model = new Dsaid_ViewModel(navigation) { editable = false };
        ...
        public JsonResult GetDependants(string Identifier, string Selected, IDictionary<string, object> Limits)
        {
            ...
            try
            {
				...
				{
					case "DSAID___WARE1WAREHDES":	// Field (DB)
						values = Dsaid_ViewModel.GetDependant_DsaidTableWare1Warehdes(Selected, navigation);
						break;
					default: break;
				}
		...
		[AuthorizeForUsers]
        [ActionSessionState(System.Web.SessionState.SessionStateBehavior.ReadOnly)]
        public JsonResult RecalculateFormulas_Dsaid(Dsaid_ViewModel form_data)
        {
            return GenericRecalculateFormulas(form_data, "outpt",
                (primaryKey) => Models.Outpt.Find(primaryKey, "FDSAID"),
