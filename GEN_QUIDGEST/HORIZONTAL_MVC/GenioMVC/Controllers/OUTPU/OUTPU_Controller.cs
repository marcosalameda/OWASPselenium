...
// USE /[MANUAL GQT MANUAL_CONTROLLER OUTPU]/
        ...
        public JsonResult ReloadDBEdit(string Identifier, IDictionary<string, object> Values)
        {
            ...
            try { row = Models.Outpu.Find(navigation.GetStrValue("outpu")); }
            catch (Exception)
            {
					...
					case "LDSAI___OUTPTDOCUMENR":	// Field (DB)
                        {
						    ...
						    var model = new Ldsai_ViewModel(navigation) { editable = false };
        ...
        public JsonResult GetDependants(string Identifier, string Selected, IDictionary<string, object> Limits)
        {
            ...
            try
            {
				...
				{
					case "LDSAI___OUTPTDOCUMENR":	// Field (DB)
						values = Ldsai_ViewModel.GetDependant_LdsaiTableOutptDocumenr(Selected, navigation);
						break;
					case "LDSAI___WAREHWAREHDES":	// Field (DB)
						values = Ldsai_ViewModel.GetDependant_LdsaiTableWarehWarehdes(Selected, navigation);
						break;
					case "LDSAI___ITEM_ITEMDES_":	// Field (DB)
						values = Ldsai_ViewModel.GetDependant_LdsaiTableItemItemdes(Selected, navigation);
						break;
					case "LDSAI___OUDOCNRDOCSDA":	// Field (DB)
						values = Ldsai_ViewModel.GetDependant_LdsaiTableOudocNrdocsda(Selected, navigation);
						break;
					default: break;
				}
		...
		[AuthorizeForUsers]
        [ActionSessionState(System.Web.SessionState.SessionStateBehavior.ReadOnly)]
        public JsonResult RecalculateFormulas_Ldsai(Ldsai_ViewModel form_data)
        {
            return GenericRecalculateFormulas(form_data, "outpu",
                (primaryKey) => Models.Outpu.Find(primaryKey, "FLDSAI"),
