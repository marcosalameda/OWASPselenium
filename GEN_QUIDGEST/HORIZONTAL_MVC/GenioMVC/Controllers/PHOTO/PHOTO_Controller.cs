...
// USE /[MANUAL GQT MANUAL_CONTROLLER PHOTO]/
        ...
        public JsonResult ReloadDBEdit(string Identifier, IDictionary<string, object> Values)
        {
            ...
            try { row = Models.Photo.Find(navigation.GetStrValue("photo")); }
            catch (Exception)
            {
					...
					case "FOTOS___EQUIPREGISTNR":	// Field (DB)
                        {
						    ...
						    var model = new Fotos_ViewModel(navigation) { editable = false };
        ...
        public JsonResult GetDependants(string Identifier, string Selected, IDictionary<string, object> Limits)
        {
            ...
            try
            {
				...
				{
					case "FOTOS___EQUIPREGISTNR":	// Field (DB)
						values = Fotos_ViewModel.GetDependant_FotosTableEquipRegistnr(Selected, navigation);
						break;
					default: break;
				}
		...
		[AuthorizeForUsers]
        [ActionSessionState(System.Web.SessionState.SessionStateBehavior.ReadOnly)]
        public JsonResult RecalculateFormulas_Fotos(Fotos_ViewModel form_data)
        {
            return GenericRecalculateFormulas(form_data, "photo",
                (primaryKey) => Models.Photo.Find(primaryKey, "FFOTOS"),
