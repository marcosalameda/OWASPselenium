...
// USE /[MANUAL GQT MANUAL_CONTROLLER REPAR]/
        ...
        public JsonResult ReloadDBEdit(string Identifier, IDictionary<string, object> Values)
        {
            ...
            try { row = Models.Repar.Find(navigation.GetStrValue("repar")); }
            catch (Exception)
            {
					...
					case "REPAR___EQUIPREGISTNR":	// Field (DB)
                        {
						    ...
						    var model = new Repar_ViewModel(navigation) { editable = false };
        ...
        public JsonResult GetDependants(string Identifier, string Selected, IDictionary<string, object> Limits)
        {
            ...
            try
            {
				...
				{
					case "REPAR___EQUIPREGISTNR":	// Field (DB)
						values = Repar_ViewModel.GetDependant_ReparTableEquipRegistnr(Selected, navigation);
						break;
					case "REPAR___SPECIESPECIAL":	// Field (DB)
						values = Repar_ViewModel.GetDependant_ReparTableSpeciEspecial(Selected, navigation);
						break;
					case "REPAR___PESSONAME____":	// Field (DB)
						values = Repar_ViewModel.GetDependant_ReparTablePessoName(Selected, navigation);
						break;
					default: break;
				}
		...
		[AuthorizeForUsers]
        [ActionSessionState(System.Web.SessionState.SessionStateBehavior.ReadOnly)]
        public JsonResult RecalculateFormulas_Repar(Repar_ViewModel form_data)
        {
            return GenericRecalculateFormulas(form_data, "repar",
                (primaryKey) => Models.Repar.Find(primaryKey, "FREPAR"),
