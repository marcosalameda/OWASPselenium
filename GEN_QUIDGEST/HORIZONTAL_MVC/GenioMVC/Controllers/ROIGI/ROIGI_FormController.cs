...
namespace GenioMVC.Controllers
{
	public partial class RoigiController : ControllerBase
	{
		...
		[AuthorizeForUsers]
		public ActionResult ChangeFormMode_Roigi(string mode)
		{
			var _mode = Navigation.CurrentLevel.Location;
			var id = Navigation.GetStrValue("roigi");
			...
			switch (mode)
			{
				case "show":
					if (m.Contains("v"))
						_mode = ACTION_ROIGI_SHOW;
					break;
				case "delete":
					if (m.Contains("a"))
						_mode = ACTION_ROIGI_DELETE;
					break;
				case "edit":
					if (m.Contains("e"))
						_mode = ACTION_ROIGI_EDIT;
					break;
				case "duplicate":
					if (m.Contains("d"))
						_mode = ACTION_ROIGI_DUPLICATE;
					break;
				case "new":
					if (m.Contains("i"))
						_mode = ACTION_ROIGI_NEW;
					break;
			}
		...
		[AuthorizeForUsers]
		public ActionResult Roigi_ModalDBEdit(string partialView)
		{
			Roigi_ViewModel model = new Roigi_ViewModel();
...
// USE /[MANUAL GQT CONTROLLER_SHOW ROIGI]/
		//
		// GET: /Roigi/Roigi_Show
		[AuthorizeForUsers]
		public ActionResult Roigi_Show(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";
			var model = new Roigi_ViewModel(Navigation, nestedForm);
			model.setModes(Request.QueryString["m"]);
			...
			string partialView = qs["partialView"] ?? "Roigi"; // MF send the patial view name

			var navigationLocationAction = ACTION_ROIGI_SHOW.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			...
			if (!isHomePage && (IsNewLocation(navigationLocationAction) || !string.Equals(Navigation.GetStrValue("roigi"), id)))
			{
				Navigation.AddHistoryLevel(navigationLocationAction, FormMode.Show, nestedForm);
				CSGenio.framework.Audit.registAction(UserContext.Current.User, Resources.Resources.FORM54242 + " " + Navigation.CurrentLevel.Location.ShortDescription());
			}

			Navigation.SetValue("roigi", id);
...
// USE /[MANUAL GQT BEFORE_LOAD_SHOW ROIGI]/
			...
			catch (ModelNotFoundException)
			{
				...
				CSGenio.framework.Log.Error("Roigi_Show - GET " + id + " " + e.Message);
				throw;
			}
...
// USE /[MANUAL GQT AFTER_LOAD_SHOW ROIGI]/
			...
			if (isHomePage)
			{
				Navigation.SetValue("HomePage", "Roigi");
				return PartialView(partialView, model);
			}
			else if (!Request.IsAjaxRequest())
				return View("Roigi", model);
			else
				return PartialView(partialView, model);
		}
		...
		[ActionName("Roigi_New_Insert")]
		[AuthorizeForUsers]
		[HttpGet]
		public ActionResult Roigi_New_Insert()
		{
			...
			return RedirectToAction("Roigi_New");
		}

// USE /[MANUAL GQT CONTROLLER_NEW_GET ROIGI]/
		//
		// GET: /Roigi/Roigi_New
		[ActionName("Roigi_New")]
		[AuthorizeForUsers]
		[HttpGet]
		public ActionResult Roigi_New()
		{
			...
			string partialView = qs["partialView"] ?? "Roigi";

			var navigationLocationAction = ACTION_ROIGI_NEW.SetRoutedValues(new { m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);

			//FOR: OVERRIDE SKIP IF JUST ONE
			//Allow child form to use "Go Back" to menu list without "skip if only one"
			if (Navigation.OverrideSkipIfJustOne.ContainsKey("ROIGI"))
				Navigation.OverrideSkipIfJustOne["ROIGI"] = true;
...
// USE /[MANUAL GQT AFTER_LOAD_NEW ROIGI]/
			...
			catch (Exception e)
			{
				...
				CSGenio.framework.Log.Error("Roigi_New - GET " + e.Message);
...
// USE /[MANUAL GQT CONTROLLER_NEW_POST ROIGI]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Roigi_New(Roigi_ViewModel model, bool redirect = true)
		{
...
// USE /[MANUAL GQT AFTER_LOAD_NEW_EX ROIGI]/
		...
		[AuthorizeForUsers]
		public ActionResult Roigi_New_Redirect(bool internalRedirect = false)
		{
			IList<string> warningMsgs = TempData["NEW_WARNINGS_LIST"] as List<string>;
			string saveMsg = TempData["NEW_SAVE_LIST"] as string;

			if (!internalRedirect)
				SuccessMessage(String.IsNullOrEmpty(saveMsg) ? Resources.Resources.REGISTO_CRIADO_COM_S18746 + GetHumanKeyToQMessage("roigi", Convert.ToString(Navigation.CurrentLevel.GetEntry("roigi"))) : saveMsg);
			...
			if (Convert.ToBoolean(Request.QueryString.Get("repeatInsertion")))
				return RedirectToLocation(ACTION_ROIGI_NEW, new { repeatInsertion = true });
...
// USE /[MANUAL GQT CONTROLLER_EDIT_GET ROIGI]/
		//
		// GET: /Roigi/Roigi_Edit
		[AuthorizeForUsers]
		[HttpGet]
		[ActionName("Roigi_Edit")]
		public ActionResult Roigi_Edit(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";
			string partialView = qs["partialView"] ?? "Roigi"; // MF send the partial view name

			//Useful to initialize a record that had to be created by the framework prior to edition (and method New doesnt support this). This way, if the form then is canceled by the user, the record will be automatically deleted
			var isNewEdit = Navigation.GetStrValue("NewEdit") == "true";

			var navigationLocationAction =  isNewEdit ? ACTION_ROIGI_NEW.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] }) : ACTION_ROIGI_EDIT.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);
...
// USE /[MANUAL GQT BEFORE_LOAD_EDIT ROIGI]/
			...
			catch (ModelNotFoundException)
			{
				...
				sp.closeConnection();
				//JFG 05-05-2017 This need to better handled, for now it's handled on webconfig's customErrors section.
				CSGenio.framework.Log.Error("Roigi_Edit - " + id + " " + e.Message);
				throw;
			}
...
// USE /[MANUAL GQT CONTROLLER_EDIT_POST ROIGI]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Roigi_Edit(Roigi_ViewModel model, bool redirect = true)
		{
...
// USE /[MANUAL GQT AFTER_LOAD_EDIT_EX ROIGI]/
		...
		[AuthorizeForUsers]
		public ActionResult Roigi_Edit_Redirect(bool internalRedirect = false)
		{
			IList<string> warningMsgs = TempData["EDIT_WARNINGS_LIST"] as List<string>;
			string saveMsg = TempData["NEW_SAVE_LIST"] as string;

			if (!internalRedirect || !Request.IsAjaxRequest())
				SuccessMessage(String.IsNullOrEmpty(saveMsg) ? Resources.Resources.ALTERACOES_EFECTUADA64514 + GetHumanKeyToQMessage("roigi", Convert.ToString(Navigation.CurrentLevel.GetEntry("roigi"))) : saveMsg);
...
// USE /[MANUAL GQT CONTROLLER_DELETE_GET ROIGI]/
		//
		// GET: /Roigi/Roigi_Delete
		[AuthorizeForUsers]
		[HttpGet]
		[ActionName("Roigi_Delete")]
		public ActionResult Roigi_Delete(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";

			var navigationLocationAction = ACTION_ROIGI_DELETE.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);
...
// USE /[MANUAL GQT BEFORE_LOAD_DELETE ROIGI]/
			...
			catch (ModelNotFoundException)
			{
				...
				//JFG 05-05-2017 This need to better handled, for now it's handled on webconfig's customErrors section.
				CSGenio.framework.Log.Error("Roigi_Delete - " + id + " " + e.Message);
				throw;
			}
...
// USE /[MANUAL GQT AFTER_LOAD_DELETE ROIGI]/
			...
			if (!Request.IsAjaxRequest())
				...
				if (permission.Status.Equals(CSGenio.framework.Status.E))
					...
					return PartialView("Roigi", model);
		}
...
// USE /[MANUAL GQT CONTROLLER_DELETE_POST ROIGI]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Roigi_Delete(string id, FormCollection collection)
		{

			var model = new Roigi_ViewModel (Navigation, id);
			model.MapFromModel();
...
// USE /[MANUAL GQT CONTROLLER_DUPLICATE_GET ROIGI]/
		//
		// GET: /Roigi/Roigi_Duplicate
		[AuthorizeForUsers]
		[HttpGet]
		public ActionResult Roigi_Duplicate(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";
			var model = new Roigi_ViewModel(Navigation, nestedForm);
			...
			var navigationLocationAction = ACTION_ROIGI_DUPLICATE.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);
...
// USE /[MANUAL GQT AFTER_LOAD_DUPLICATE ROIGI]/
					...
					sp.closeTransaction();
					Navigation.CurrentLevel.SetLocation(ACTION_ROIGI_DUPLICATE.SetRoutedValues(new { Id = model.ValCodroigi }));
...
// USE /[MANUAL GQT CONTROLLER_DUPLICATE_POST ROIGI]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Roigi_Duplicate(Roigi_ViewModel model, bool redirect = true)
		{
			var eventSink = new EventSink()
			{
...
// USE /[MANUAL GQT AFTER_LOAD_DUPLICATE_EX ROIGI]/
		...
		[AuthorizeForUsers]
		public ActionResult Roigi_Duplicate_Redirect(bool internalRedirect = false)
		{
...
// USE /[MANUAL GQT CONTROLLER_CANCEL_GET ROIGI]/
		...
		public ActionResult Roigi_Cancel()
		{
			...
			if (Navigation.CurrentLevel.FormMode == FormMode.New || Navigation.CurrentLevel.FormMode == FormMode.Duplicate)
			{
				...
				try
				{
					var model = new GenioMVC.Models.Roigi();
...
// USE /[MANUAL GQT AFTER_CANCEL ROIGI]/
			...
			else if (Navigation.PreviousLevel != null)
			{
				...
				string previousLevelArea = Navigation.PreviousLevel.Location.Controller?.ToLower() ?? "roigi";
		...
		{
			var model = new Roigi_ViewModel(Navigation, true);
			model.setModes(Request.QueryString["m"]);
			PersistentSupport sp = UserContext.Current.PersistentSupport;
			var navigationLocationAction = ACTION_ROIGI_NEW.SetRoutedValues(new { m = Request.QueryString["m"] });
			CheckLevels(navigationLocationAction);
			...
			return PartialView("MFRoigi", model);
		}
		...
		public ActionResult MFRoigi_Edit(string id)
		{
			return this.RedirectToAction("Roigi_Edit", "Roigi", new { id = id, partialView = "MFRoigi", nestedForm = "true", multiForm = "true" });
		}
		...
		public ActionResult MFRoigi_Cancel(string id)
		{
			//MH - Validate if current level is it correct. Remove all levels that is higher than the currently level.
			CheckLevels(NavigationLocation.Any);

			if (Navigation.CurrentLevel.IsNestedContext &&
				(Navigation.CurrentLevel.Location.Action == ACTION_ROIGI_NEW.Action || Navigation.CurrentLevel.Location.Action == ACTION_ROIGI_EDIT.Action))
			{
				if (Navigation.CurrentLevel.FormMode == FormMode.New)
				{
					...
					catch (Exception e)
					{
						...
						Log.Error("MFRoigi_Cancel - " + exceptionUserMessage);
						ErrorMessage(exceptionUserMessage);
					}
				}
			...
			return this.RedirectToAction("Roigi_Show", "Roigi", new { id = id, partialView = "MFRoigi", nestedForm = "true", multiForm = "true", nav });
		}
		...
		public JsonResult MFRoigi_Delete(string id)
		{
			var eventSink = new EventSink()
			{
		...
		public ActionResult Roigi_Rogl1ValTitle(string id, string partialView,  IDictionary<string, string> Limits)
		{
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Roigi_SaveEdit(Roigi_ViewModel model)
		{
			var eventSink = new EventSink()
			{
