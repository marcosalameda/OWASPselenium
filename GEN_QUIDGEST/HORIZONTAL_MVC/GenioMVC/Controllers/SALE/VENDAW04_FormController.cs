...
namespace GenioMVC.Controllers
{
	public partial class SaleController : ControllerBase
	{
		...
		[AuthorizeForUsers]
		public ActionResult ChangeFormMode_Vendaw04(string mode)
		{
			...
			switch (mode)
			{
				case "show":
					if (m.Contains("v"))
						_mode = ACTION_VENDAW04_SHOW;
					break;
				case "delete":
					if (m.Contains("a"))
						_mode = ACTION_VENDAW04_DELETE;
					break;
				case "edit":
					if (m.Contains("e"))
						_mode = ACTION_VENDAW04_EDIT;
					break;
				case "duplicate":
					if (m.Contains("d"))
						_mode = ACTION_VENDAW04_DUPLICATE;
					break;
				case "new":
					if (m.Contains("i"))
						_mode = ACTION_VENDAW04_NEW;
					break;
			}
		...
		[AuthorizeForUsers]
		public ActionResult Vendaw04_ModalDBEdit(string partialView)
		{
			Vendaw04_ViewModel model = new Vendaw04_ViewModel();
...
// USE /[MANUAL GQT CONTROLLER_SHOW VENDAW04]/
		//
		// GET: /Sale/Vendaw04_Show
		[AuthorizeForUsers]
		public ActionResult Vendaw04_Show(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";
			var model = new Vendaw04_ViewModel(Navigation, nestedForm);
			model.setModes(Request.QueryString["m"]);
			...
			string partialView = qs["partialView"] ?? "Vendaw04"; // MF send the patial view name

			var navigationLocationAction = ACTION_VENDAW04_SHOW.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
...
// USE /[MANUAL GQT BEFORE_LOAD_SHOW VENDAW04]/
			...
			catch (ModelNotFoundException)
			{
				...
				CSGenio.framework.Log.Error("Vendaw04_Show - GET " + id + " " + e.Message);
				throw;
			}
...
// USE /[MANUAL GQT AFTER_LOAD_SHOW VENDAW04]/
			...
			if (isHomePage)
			{
				Navigation.SetValue("HomePage", "Vendaw04");
				return PartialView(partialView, model);
			}
			else if (!Request.IsAjaxRequest())
				return View("Vendaw04", model);
			else
				return PartialView(partialView, model);
		}
		...
		[ActionName("Vendaw04_New_Insert")]
		[AuthorizeForUsers]
		[HttpGet]
		public ActionResult Vendaw04_New_Insert()
		{
			...
			return RedirectToAction("Vendaw04_New");
		}

// USE /[MANUAL GQT CONTROLLER_NEW_GET VENDAW04]/
		//
		// GET: /Sale/Vendaw04_New
		[ActionName("Vendaw04_New")]
		[AuthorizeForUsers]
		[HttpGet]
		public ActionResult Vendaw04_New()
		{
			...
			string partialView = qs["partialView"] ?? "Vendaw04";

			var navigationLocationAction = ACTION_VENDAW04_NEW.SetRoutedValues(new { m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);

			//FOR: OVERRIDE SKIP IF JUST ONE
			//Allow child form to use "Go Back" to menu list without "skip if only one"
			if (Navigation.OverrideSkipIfJustOne.ContainsKey("VENDAW04"))
				Navigation.OverrideSkipIfJustOne["VENDAW04"] = true;
...
// USE /[MANUAL GQT AFTER_LOAD_NEW VENDAW04]/
			...
			catch (Exception e)
			{
				...
				CSGenio.framework.Log.Error("Vendaw04_New - GET " + e.Message);
...
// USE /[MANUAL GQT CONTROLLER_NEW_POST VENDAW04]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Vendaw04_New(Vendaw04_ViewModel model, bool redirect = true)
		{
...
// USE /[MANUAL GQT AFTER_LOAD_NEW_EX VENDAW04]/
		...
		[AuthorizeForUsers]
		public ActionResult Vendaw04_New_Redirect(bool internalRedirect = false)
		{
			...
			if (Convert.ToBoolean(Request.QueryString.Get("repeatInsertion")))
				return RedirectToLocation(ACTION_VENDAW04_NEW, new { repeatInsertion = true });
...
// USE /[MANUAL GQT CONTROLLER_EDIT_GET VENDAW04]/
		//
		// GET: /Sale/Vendaw04_Edit
		[AuthorizeForUsers]
		[HttpGet]
		[ActionName("Vendaw04_Edit")]
		public ActionResult Vendaw04_Edit(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";
			string partialView = qs["partialView"] ?? "Vendaw04"; // MF send the partial view name

			//Useful to initialize a record that had to be created by the framework prior to edition (and method New doesnt support this). This way, if the form then is canceled by the user, the record will be automatically deleted
			var isNewEdit = Navigation.GetStrValue("NewEdit") == "true";

			var navigationLocationAction =  isNewEdit ? ACTION_VENDAW04_NEW.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] }) : ACTION_VENDAW04_EDIT.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);
...
// USE /[MANUAL GQT BEFORE_LOAD_EDIT VENDAW04]/
			...
			catch (ModelNotFoundException)
			{
				...
				sp.closeConnection();
				//JFG 05-05-2017 This need to better handled, for now it's handled on webconfig's customErrors section.
				CSGenio.framework.Log.Error("Vendaw04_Edit - " + id + " " + e.Message);
				throw;
			}
...
// USE /[MANUAL GQT CONTROLLER_EDIT_POST VENDAW04]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Vendaw04_Edit(Vendaw04_ViewModel model, bool redirect = true)
		{
...
// USE /[MANUAL GQT AFTER_LOAD_EDIT_EX VENDAW04]/
		...
		[AuthorizeForUsers]
		public ActionResult Vendaw04_Edit_Redirect(bool internalRedirect = false)
		{
...
// USE /[MANUAL GQT CONTROLLER_DELETE_GET VENDAW04]/
		//
		// GET: /Sale/Vendaw04_Delete
		[AuthorizeForUsers]
		[HttpGet]
		[ActionName("Vendaw04_Delete")]
		public ActionResult Vendaw04_Delete(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";

			var navigationLocationAction = ACTION_VENDAW04_DELETE.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);
...
// USE /[MANUAL GQT BEFORE_LOAD_DELETE VENDAW04]/
			...
			catch (ModelNotFoundException)
			{
				...
				//JFG 05-05-2017 This need to better handled, for now it's handled on webconfig's customErrors section.
				CSGenio.framework.Log.Error("Vendaw04_Delete - " + id + " " + e.Message);
				throw;
			}
...
// USE /[MANUAL GQT AFTER_LOAD_DELETE VENDAW04]/
			...
			if (!Request.IsAjaxRequest())
				...
				if (permission.Status.Equals(CSGenio.framework.Status.E))
					...
					return PartialView("Vendaw04", model);
		}
...
// USE /[MANUAL GQT CONTROLLER_DELETE_POST VENDAW04]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Vendaw04_Delete(string id, FormCollection collection)
		{

			var model = new Vendaw04_ViewModel (Navigation, id);
			model.MapFromModel();
...
// USE /[MANUAL GQT CONTROLLER_DUPLICATE_GET VENDAW04]/
		//
		// GET: /Sale/Vendaw04_Duplicate
		[AuthorizeForUsers]
		[HttpGet]
		public ActionResult Vendaw04_Duplicate(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";
			var model = new Vendaw04_ViewModel(Navigation, nestedForm);
			...
			var navigationLocationAction = ACTION_VENDAW04_DUPLICATE.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);
...
// USE /[MANUAL GQT AFTER_LOAD_DUPLICATE VENDAW04]/
					...
					sp.closeTransaction();
					Navigation.CurrentLevel.SetLocation(ACTION_VENDAW04_DUPLICATE.SetRoutedValues(new { Id = model.ValCodvenda }));
...
// USE /[MANUAL GQT CONTROLLER_DUPLICATE_POST VENDAW04]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Vendaw04_Duplicate(Vendaw04_ViewModel model, bool redirect = true)
		{
			var eventSink = new EventSink()
			{
...
// USE /[MANUAL GQT AFTER_LOAD_DUPLICATE_EX VENDAW04]/
		...
		[AuthorizeForUsers]
		public ActionResult Vendaw04_Duplicate_Redirect(bool internalRedirect = false)
		{
...
// USE /[MANUAL GQT AFTER_CANCEL VENDAW04]/
		...
		{
			var model = new Vendaw04_ViewModel(Navigation, true);
			model.setModes(Request.QueryString["m"]);
			PersistentSupport sp = UserContext.Current.PersistentSupport;
			var navigationLocationAction = ACTION_VENDAW04_NEW.SetRoutedValues(new { m = Request.QueryString["m"] });
			CheckLevels(navigationLocationAction);
			...
			return PartialView("MFVendaw04", model);
		}
		...
		public ActionResult MFVendaw04_Edit(string id)
		{
			return this.RedirectToAction("Vendaw04_Edit", "Sale", new { id = id, partialView = "MFVendaw04", nestedForm = "true", multiForm = "true" });
		}
		...
		public ActionResult MFVendaw04_Cancel(string id)
		{
			//MH - Validate if current level is it correct. Remove all levels that is higher than the currently level.
			CheckLevels(NavigationLocation.Any);

			if (Navigation.CurrentLevel.IsNestedContext &&
				(Navigation.CurrentLevel.Location.Action == ACTION_VENDAW04_NEW.Action || Navigation.CurrentLevel.Location.Action == ACTION_VENDAW04_EDIT.Action))
			{
				if (Navigation.CurrentLevel.FormMode == FormMode.New)
				{
					...
					catch (Exception e)
					{
						...
						Log.Error("MFVendaw04_Cancel - " + exceptionUserMessage);
						ErrorMessage(exceptionUserMessage);
					}
				}
			...
			return this.RedirectToAction("Vendaw04_Show", "Sale", new { id = id, partialView = "MFVendaw04", nestedForm = "true", multiForm = "true", nav });
		}
		...
		public JsonResult MFVendaw04_Delete(string id)
		{
			var eventSink = new EventSink()
			{
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Vendaw04_SaveEdit(Vendaw04_ViewModel model)
		{
			var eventSink = new EventSink()
			{
