...
namespace GenioMVC.Controllers
{
	public partial class SaleController : ControllerBase
	{
		...
		[AuthorizeForUsers]
		public ActionResult ChangeFormMode_Vendaw05(string mode)
		{
			...
			switch (mode)
			{
				case "show":
					if (m.Contains("v"))
						_mode = ACTION_VENDAW05_SHOW;
					break;
				case "delete":
					if (m.Contains("a"))
						_mode = ACTION_VENDAW05_DELETE;
					break;
				case "edit":
					if (m.Contains("e"))
						_mode = ACTION_VENDAW05_EDIT;
					break;
				case "duplicate":
					if (m.Contains("d"))
						_mode = ACTION_VENDAW05_DUPLICATE;
					break;
				case "new":
					if (m.Contains("i"))
						_mode = ACTION_VENDAW05_NEW;
					break;
			}
		...
		[AuthorizeForUsers]
		public ActionResult Vendaw05_ModalDBEdit(string partialView)
		{
			Vendaw05_ViewModel model = new Vendaw05_ViewModel();
...
// USE /[MANUAL GQT CONTROLLER_SHOW VENDAW05]/
		//
		// GET: /Sale/Vendaw05_Show
		[AuthorizeForUsers]
		public ActionResult Vendaw05_Show(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";
			var model = new Vendaw05_ViewModel(Navigation, nestedForm);
			model.setModes(Request.QueryString["m"]);
			...
			string partialView = qs["partialView"] ?? "Vendaw05"; // MF send the patial view name

			var navigationLocationAction = ACTION_VENDAW05_SHOW.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
...
// USE /[MANUAL GQT BEFORE_LOAD_SHOW VENDAW05]/
			...
			catch (ModelNotFoundException)
			{
				...
				CSGenio.framework.Log.Error("Vendaw05_Show - GET " + id + " " + e.Message);
				throw;
			}
...
// USE /[MANUAL GQT AFTER_LOAD_SHOW VENDAW05]/
			...
			if (isHomePage)
			{
				Navigation.SetValue("HomePage", "Vendaw05");
				return PartialView(partialView, model);
			}
			else if (!Request.IsAjaxRequest())
				return View("Vendaw05", model);
			else
				return PartialView(partialView, model);
		}
		...
		[ActionName("Vendaw05_New_Insert")]
		[AuthorizeForUsers]
		[HttpGet]
		public ActionResult Vendaw05_New_Insert()
		{
			...
			return RedirectToAction("Vendaw05_New");
		}

// USE /[MANUAL GQT CONTROLLER_NEW_GET VENDAW05]/
		//
		// GET: /Sale/Vendaw05_New
		[ActionName("Vendaw05_New")]
		[AuthorizeForUsers]
		[HttpGet]
		public ActionResult Vendaw05_New()
		{
			...
			string partialView = qs["partialView"] ?? "Vendaw05";

			var navigationLocationAction = ACTION_VENDAW05_NEW.SetRoutedValues(new { m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);

			//FOR: OVERRIDE SKIP IF JUST ONE
			//Allow child form to use "Go Back" to menu list without "skip if only one"
			if (Navigation.OverrideSkipIfJustOne.ContainsKey("VENDAW05"))
				Navigation.OverrideSkipIfJustOne["VENDAW05"] = true;
...
// USE /[MANUAL GQT AFTER_LOAD_NEW VENDAW05]/
			...
			catch (Exception e)
			{
				...
				CSGenio.framework.Log.Error("Vendaw05_New - GET " + e.Message);
...
// USE /[MANUAL GQT CONTROLLER_NEW_POST VENDAW05]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Vendaw05_New(Vendaw05_ViewModel model, bool redirect = true)
		{
...
// USE /[MANUAL GQT AFTER_LOAD_NEW_EX VENDAW05]/
		...
		[AuthorizeForUsers]
		public ActionResult Vendaw05_New_Redirect(bool internalRedirect = false)
		{
			...
			if (Convert.ToBoolean(Request.QueryString.Get("repeatInsertion")))
				return RedirectToLocation(ACTION_VENDAW05_NEW, new { repeatInsertion = true });
...
// USE /[MANUAL GQT CONTROLLER_EDIT_GET VENDAW05]/
		//
		// GET: /Sale/Vendaw05_Edit
		[AuthorizeForUsers]
		[HttpGet]
		[ActionName("Vendaw05_Edit")]
		public ActionResult Vendaw05_Edit(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";
			string partialView = qs["partialView"] ?? "Vendaw05"; // MF send the partial view name

			//Useful to initialize a record that had to be created by the framework prior to edition (and method New doesnt support this). This way, if the form then is canceled by the user, the record will be automatically deleted
			var isNewEdit = Navigation.GetStrValue("NewEdit") == "true";

			var navigationLocationAction =  isNewEdit ? ACTION_VENDAW05_NEW.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] }) : ACTION_VENDAW05_EDIT.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);
...
// USE /[MANUAL GQT BEFORE_LOAD_EDIT VENDAW05]/
			...
			catch (ModelNotFoundException)
			{
				...
				sp.closeConnection();
				//JFG 05-05-2017 This need to better handled, for now it's handled on webconfig's customErrors section.
				CSGenio.framework.Log.Error("Vendaw05_Edit - " + id + " " + e.Message);
				throw;
			}
...
// USE /[MANUAL GQT CONTROLLER_EDIT_POST VENDAW05]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Vendaw05_Edit(Vendaw05_ViewModel model, bool redirect = true)
		{
...
// USE /[MANUAL GQT AFTER_LOAD_EDIT_EX VENDAW05]/
		...
		[AuthorizeForUsers]
		public ActionResult Vendaw05_Edit_Redirect(bool internalRedirect = false)
		{
...
// USE /[MANUAL GQT CONTROLLER_DELETE_GET VENDAW05]/
		//
		// GET: /Sale/Vendaw05_Delete
		[AuthorizeForUsers]
		[HttpGet]
		[ActionName("Vendaw05_Delete")]
		public ActionResult Vendaw05_Delete(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";

			var navigationLocationAction = ACTION_VENDAW05_DELETE.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);
...
// USE /[MANUAL GQT BEFORE_LOAD_DELETE VENDAW05]/
			...
			catch (ModelNotFoundException)
			{
				...
				//JFG 05-05-2017 This need to better handled, for now it's handled on webconfig's customErrors section.
				CSGenio.framework.Log.Error("Vendaw05_Delete - " + id + " " + e.Message);
				throw;
			}
...
// USE /[MANUAL GQT AFTER_LOAD_DELETE VENDAW05]/
			...
			if (!Request.IsAjaxRequest())
				...
				if (permission.Status.Equals(CSGenio.framework.Status.E))
					...
					return PartialView("Vendaw05", model);
		}
...
// USE /[MANUAL GQT CONTROLLER_DELETE_POST VENDAW05]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Vendaw05_Delete(string id, FormCollection collection)
		{

			var model = new Vendaw05_ViewModel (Navigation, id);
			model.MapFromModel();
...
// USE /[MANUAL GQT CONTROLLER_DUPLICATE_GET VENDAW05]/
		//
		// GET: /Sale/Vendaw05_Duplicate
		[AuthorizeForUsers]
		[HttpGet]
		public ActionResult Vendaw05_Duplicate(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";
			var model = new Vendaw05_ViewModel(Navigation, nestedForm);
			...
			var navigationLocationAction = ACTION_VENDAW05_DUPLICATE.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);
...
// USE /[MANUAL GQT AFTER_LOAD_DUPLICATE VENDAW05]/
					...
					sp.closeTransaction();
					Navigation.CurrentLevel.SetLocation(ACTION_VENDAW05_DUPLICATE.SetRoutedValues(new { Id = model.ValCodvenda }));
...
// USE /[MANUAL GQT CONTROLLER_DUPLICATE_POST VENDAW05]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Vendaw05_Duplicate(Vendaw05_ViewModel model, bool redirect = true)
		{
			var eventSink = new EventSink()
			{
...
// USE /[MANUAL GQT AFTER_LOAD_DUPLICATE_EX VENDAW05]/
		...
		[AuthorizeForUsers]
		public ActionResult Vendaw05_Duplicate_Redirect(bool internalRedirect = false)
		{
...
// USE /[MANUAL GQT AFTER_CANCEL VENDAW05]/
		...
		{
			var model = new Vendaw05_ViewModel(Navigation, true);
			model.setModes(Request.QueryString["m"]);
			PersistentSupport sp = UserContext.Current.PersistentSupport;
			var navigationLocationAction = ACTION_VENDAW05_NEW.SetRoutedValues(new { m = Request.QueryString["m"] });
			CheckLevels(navigationLocationAction);
			...
			return PartialView("MFVendaw05", model);
		}
		...
		public ActionResult MFVendaw05_Edit(string id)
		{
			return this.RedirectToAction("Vendaw05_Edit", "Sale", new { id = id, partialView = "MFVendaw05", nestedForm = "true", multiForm = "true" });
		}
		...
		public ActionResult MFVendaw05_Cancel(string id)
		{
			//MH - Validate if current level is it correct. Remove all levels that is higher than the currently level.
			CheckLevels(NavigationLocation.Any);

			if (Navigation.CurrentLevel.IsNestedContext &&
				(Navigation.CurrentLevel.Location.Action == ACTION_VENDAW05_NEW.Action || Navigation.CurrentLevel.Location.Action == ACTION_VENDAW05_EDIT.Action))
			{
				if (Navigation.CurrentLevel.FormMode == FormMode.New)
				{
					...
					catch (Exception e)
					{
						...
						Log.Error("MFVendaw05_Cancel - " + exceptionUserMessage);
						ErrorMessage(exceptionUserMessage);
					}
				}
			...
			return this.RedirectToAction("Vendaw05_Show", "Sale", new { id = id, partialView = "MFVendaw05", nestedForm = "true", multiForm = "true", nav });
		}
		...
		public JsonResult MFVendaw05_Delete(string id)
		{
			var eventSink = new EventSink()
			{
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Vendaw05_SaveEdit(Vendaw05_ViewModel model)
		{
			var eventSink = new EventSink()
			{
