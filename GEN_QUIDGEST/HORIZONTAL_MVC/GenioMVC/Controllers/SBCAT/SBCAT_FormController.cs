...
namespace GenioMVC.Controllers
{
	public partial class SbcatController : ControllerBase
	{
		...
		[AuthorizeForUsers]
		public ActionResult ChangeFormMode_Sbcat(string mode)
		{
			var _mode = Navigation.CurrentLevel.Location;
			var id = Navigation.GetStrValue("sbcat");
			...
			switch (mode)
			{
				case "show":
					if (m.Contains("v"))
						_mode = ACTION_SBCAT_SHOW;
					break;
				case "delete":
					if (m.Contains("a"))
						_mode = ACTION_SBCAT_DELETE;
					break;
				case "edit":
					if (m.Contains("e"))
						_mode = ACTION_SBCAT_EDIT;
					break;
				case "duplicate":
					if (m.Contains("d"))
						_mode = ACTION_SBCAT_DUPLICATE;
					break;
				case "new":
					if (m.Contains("i"))
						_mode = ACTION_SBCAT_NEW;
					break;
			}
		...
		[AuthorizeForUsers]
		public ActionResult Sbcat_ModalDBEdit(string partialView)
		{
			Sbcat_ViewModel model = new Sbcat_ViewModel();
...
// USE /[MANUAL GQT CONTROLLER_SHOW SBCAT]/
		//
		// GET: /Sbcat/Sbcat_Show
		[AuthorizeForUsers]
		public ActionResult Sbcat_Show(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";
			var model = new Sbcat_ViewModel(Navigation, nestedForm);
			model.setModes(Request.QueryString["m"]);
			...
			string partialView = qs["partialView"] ?? "Sbcat"; // MF send the patial view name

			var navigationLocationAction = ACTION_SBCAT_SHOW.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			...
			if (!isHomePage && (IsNewLocation(navigationLocationAction) || !string.Equals(Navigation.GetStrValue("sbcat"), id)))
			{
				Navigation.AddHistoryLevel(navigationLocationAction, FormMode.Show, nestedForm);
				CSGenio.framework.Audit.registAction(UserContext.Current.User, Resources.Resources.FORM54242 + " " + Navigation.CurrentLevel.Location.ShortDescription());
			}

			Navigation.SetValue("sbcat", id);
...
// USE /[MANUAL GQT BEFORE_LOAD_SHOW SBCAT]/
			...
			catch (ModelNotFoundException)
			{
				...
				CSGenio.framework.Log.Error("Sbcat_Show - GET " + id + " " + e.Message);
				throw;
			}
...
// USE /[MANUAL GQT AFTER_LOAD_SHOW SBCAT]/
			...
			if (isHomePage)
			{
				Navigation.SetValue("HomePage", "Sbcat");
				return PartialView(partialView, model);
			}
			else if (!Request.IsAjaxRequest())
				return View("Sbcat", model);
			else
				return PartialView(partialView, model);
		}
		...
		[ActionName("Sbcat_New_Insert")]
		[AuthorizeForUsers]
		[HttpGet]
		public ActionResult Sbcat_New_Insert()
		{
			...
			return RedirectToAction("Sbcat_New");
		}

// USE /[MANUAL GQT CONTROLLER_NEW_GET SBCAT]/
		//
		// GET: /Sbcat/Sbcat_New
		[ActionName("Sbcat_New")]
		[AuthorizeForUsers]
		[HttpGet]
		public ActionResult Sbcat_New()
		{
			...
			string partialView = qs["partialView"] ?? "Sbcat";

			var navigationLocationAction = ACTION_SBCAT_NEW.SetRoutedValues(new { m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);

			//FOR: OVERRIDE SKIP IF JUST ONE
			//Allow child form to use "Go Back" to menu list without "skip if only one"
			if (Navigation.OverrideSkipIfJustOne.ContainsKey("SBCAT"))
				Navigation.OverrideSkipIfJustOne["SBCAT"] = true;
...
// USE /[MANUAL GQT AFTER_LOAD_NEW SBCAT]/
			...
			catch (Exception e)
			{
				...
				CSGenio.framework.Log.Error("Sbcat_New - GET " + e.Message);
...
// USE /[MANUAL GQT CONTROLLER_NEW_POST SBCAT]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Sbcat_New(Sbcat_ViewModel model, bool redirect = true)
		{
...
// USE /[MANUAL GQT AFTER_LOAD_NEW_EX SBCAT]/
		...
		[AuthorizeForUsers]
		public ActionResult Sbcat_New_Redirect(bool internalRedirect = false)
		{
			IList<string> warningMsgs = TempData["NEW_WARNINGS_LIST"] as List<string>;
			string saveMsg = TempData["NEW_SAVE_LIST"] as string;

			if (!internalRedirect)
				SuccessMessage(String.IsNullOrEmpty(saveMsg) ? Resources.Resources.REGISTO_CRIADO_COM_S18746 + GetHumanKeyToQMessage("sbcat", Convert.ToString(Navigation.CurrentLevel.GetEntry("sbcat"))) : saveMsg);
			...
			if (Convert.ToBoolean(Request.QueryString.Get("repeatInsertion")))
				return RedirectToLocation(ACTION_SBCAT_NEW, new { repeatInsertion = true });
...
// USE /[MANUAL GQT CONTROLLER_EDIT_GET SBCAT]/
		//
		// GET: /Sbcat/Sbcat_Edit
		[AuthorizeForUsers]
		[HttpGet]
		[ActionName("Sbcat_Edit")]
		public ActionResult Sbcat_Edit(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";
			string partialView = qs["partialView"] ?? "Sbcat"; // MF send the partial view name

			//Useful to initialize a record that had to be created by the framework prior to edition (and method New doesnt support this). This way, if the form then is canceled by the user, the record will be automatically deleted
			var isNewEdit = Navigation.GetStrValue("NewEdit") == "true";

			var navigationLocationAction =  isNewEdit ? ACTION_SBCAT_NEW.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] }) : ACTION_SBCAT_EDIT.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);
...
// USE /[MANUAL GQT BEFORE_LOAD_EDIT SBCAT]/
			...
			catch (ModelNotFoundException)
			{
				...
				sp.closeConnection();
				//JFG 05-05-2017 This need to better handled, for now it's handled on webconfig's customErrors section.
				CSGenio.framework.Log.Error("Sbcat_Edit - " + id + " " + e.Message);
				throw;
			}
...
// USE /[MANUAL GQT CONTROLLER_EDIT_POST SBCAT]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Sbcat_Edit(Sbcat_ViewModel model, bool redirect = true)
		{
...
// USE /[MANUAL GQT AFTER_LOAD_EDIT_EX SBCAT]/
		...
		[AuthorizeForUsers]
		public ActionResult Sbcat_Edit_Redirect(bool internalRedirect = false)
		{
			IList<string> warningMsgs = TempData["EDIT_WARNINGS_LIST"] as List<string>;
			string saveMsg = TempData["NEW_SAVE_LIST"] as string;

			if (!internalRedirect || !Request.IsAjaxRequest())
				SuccessMessage(String.IsNullOrEmpty(saveMsg) ? Resources.Resources.ALTERACOES_EFECTUADA64514 + GetHumanKeyToQMessage("sbcat", Convert.ToString(Navigation.CurrentLevel.GetEntry("sbcat"))) : saveMsg);
...
// USE /[MANUAL GQT CONTROLLER_DELETE_GET SBCAT]/
		//
		// GET: /Sbcat/Sbcat_Delete
		[AuthorizeForUsers]
		[HttpGet]
		[ActionName("Sbcat_Delete")]
		public ActionResult Sbcat_Delete(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";

			var navigationLocationAction = ACTION_SBCAT_DELETE.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);
...
// USE /[MANUAL GQT BEFORE_LOAD_DELETE SBCAT]/
			...
			catch (ModelNotFoundException)
			{
				...
				//JFG 05-05-2017 This need to better handled, for now it's handled on webconfig's customErrors section.
				CSGenio.framework.Log.Error("Sbcat_Delete - " + id + " " + e.Message);
				throw;
			}
...
// USE /[MANUAL GQT AFTER_LOAD_DELETE SBCAT]/
			...
			if (!Request.IsAjaxRequest())
				...
				if (permission.Status.Equals(CSGenio.framework.Status.E))
					...
					return PartialView("Sbcat", model);
		}
...
// USE /[MANUAL GQT CONTROLLER_DELETE_POST SBCAT]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Sbcat_Delete(string id, FormCollection collection)
		{

			var model = new Sbcat_ViewModel (Navigation, id);
			model.MapFromModel();
...
// USE /[MANUAL GQT CONTROLLER_DUPLICATE_GET SBCAT]/
		//
		// GET: /Sbcat/Sbcat_Duplicate
		[AuthorizeForUsers]
		[HttpGet]
		public ActionResult Sbcat_Duplicate(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";
			var model = new Sbcat_ViewModel(Navigation, nestedForm);
			...
			var navigationLocationAction = ACTION_SBCAT_DUPLICATE.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);
...
// USE /[MANUAL GQT AFTER_LOAD_DUPLICATE SBCAT]/
					...
					sp.closeTransaction();
					Navigation.CurrentLevel.SetLocation(ACTION_SBCAT_DUPLICATE.SetRoutedValues(new { Id = model.ValCodsbcat }));
...
// USE /[MANUAL GQT CONTROLLER_DUPLICATE_POST SBCAT]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Sbcat_Duplicate(Sbcat_ViewModel model, bool redirect = true)
		{
			var eventSink = new EventSink()
			{
...
// USE /[MANUAL GQT AFTER_LOAD_DUPLICATE_EX SBCAT]/
		...
		[AuthorizeForUsers]
		public ActionResult Sbcat_Duplicate_Redirect(bool internalRedirect = false)
		{
...
// USE /[MANUAL GQT CONTROLLER_CANCEL_GET SBCAT]/
		...
		public ActionResult Sbcat_Cancel()
		{
			...
			if (Navigation.CurrentLevel.FormMode == FormMode.New || Navigation.CurrentLevel.FormMode == FormMode.Duplicate)
			{
				...
				try
				{
					var model = new GenioMVC.Models.Sbcat();
...
// USE /[MANUAL GQT AFTER_CANCEL SBCAT]/
			...
			else if (Navigation.PreviousLevel != null)
			{
				...
				string previousLevelArea = Navigation.PreviousLevel.Location.Controller?.ToLower() ?? "sbcat";
		...
		{
			var model = new Sbcat_ViewModel(Navigation, true);
			model.setModes(Request.QueryString["m"]);
			PersistentSupport sp = UserContext.Current.PersistentSupport;
			var navigationLocationAction = ACTION_SBCAT_NEW.SetRoutedValues(new { m = Request.QueryString["m"] });
			CheckLevels(navigationLocationAction);
			...
			return PartialView("MFSbcat", model);
		}
		...
		public ActionResult MFSbcat_Edit(string id)
		{
			return this.RedirectToAction("Sbcat_Edit", "Sbcat", new { id = id, partialView = "MFSbcat", nestedForm = "true", multiForm = "true" });
		}
		...
		public ActionResult MFSbcat_Cancel(string id)
		{
			//MH - Validate if current level is it correct. Remove all levels that is higher than the currently level.
			CheckLevels(NavigationLocation.Any);

			if (Navigation.CurrentLevel.IsNestedContext &&
				(Navigation.CurrentLevel.Location.Action == ACTION_SBCAT_NEW.Action || Navigation.CurrentLevel.Location.Action == ACTION_SBCAT_EDIT.Action))
			{
				if (Navigation.CurrentLevel.FormMode == FormMode.New)
				{
					...
					catch (Exception e)
					{
						...
						Log.Error("MFSbcat_Cancel - " + exceptionUserMessage);
						ErrorMessage(exceptionUserMessage);
					}
				}
			...
			return this.RedirectToAction("Sbcat_Show", "Sbcat", new { id = id, partialView = "MFSbcat", nestedForm = "true", multiForm = "true", nav });
		}
		...
		public JsonResult MFSbcat_Delete(string id)
		{
			var eventSink = new EventSink()
			{
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Sbcat_SaveEdit(Sbcat_ViewModel model)
		{
			var eventSink = new EventSink()
			{
