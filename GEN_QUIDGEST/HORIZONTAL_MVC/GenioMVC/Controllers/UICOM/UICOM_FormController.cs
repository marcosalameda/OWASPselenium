...
namespace GenioMVC.Controllers
{
	public partial class UicomController : ControllerBase
	{
		...
		[AuthorizeForUsers]
		public ActionResult ChangeFormMode_Uicom(string mode)
		{
			var _mode = Navigation.CurrentLevel.Location;
			var id = Navigation.GetStrValue("uicom");
			...
			switch (mode)
			{
				case "show":
					if (m.Contains("v"))
						_mode = ACTION_UICOM_SHOW;
					break;
				case "delete":
					if (m.Contains("a"))
						_mode = ACTION_UICOM_DELETE;
					break;
				case "edit":
					if (m.Contains("e"))
						_mode = ACTION_UICOM_EDIT;
					break;
				case "duplicate":
					if (m.Contains("d"))
						_mode = ACTION_UICOM_DUPLICATE;
					break;
				case "new":
					if (m.Contains("i"))
						_mode = ACTION_UICOM_NEW;
					break;
			}
		...
		[AuthorizeForUsers]
		public ActionResult Uicom_ModalDBEdit(string partialView)
		{
			Uicom_ViewModel model = new Uicom_ViewModel();
...
// USE /[MANUAL GQT CONTROLLER_SHOW UICOM]/
		//
		// GET: /Uicom/Uicom_Show
		[AuthorizeForUsers]
		public ActionResult Uicom_Show(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";
			var model = new Uicom_ViewModel(Navigation, nestedForm);
			model.setModes(Request.QueryString["m"]);
			...
			string partialView = qs["partialView"] ?? "Uicom"; // MF send the patial view name

			var navigationLocationAction = ACTION_UICOM_SHOW.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			...
			if (!isHomePage && (IsNewLocation(navigationLocationAction) || !string.Equals(Navigation.GetStrValue("uicom"), id)))
			{
				Navigation.AddHistoryLevel(navigationLocationAction, FormMode.Show, nestedForm);
				CSGenio.framework.Audit.registAction(UserContext.Current.User, Resources.Resources.FORM54242 + " " + Navigation.CurrentLevel.Location.ShortDescription());
			}

			Navigation.SetValue("uicom", id);
...
// USE /[MANUAL GQT BEFORE_LOAD_SHOW UICOM]/
			...
			catch (ModelNotFoundException)
			{
				...
				CSGenio.framework.Log.Error("Uicom_Show - GET " + id + " " + e.Message);
				throw;
			}
...
// USE /[MANUAL GQT AFTER_LOAD_SHOW UICOM]/
			...
			if (isHomePage)
			{
				Navigation.SetValue("HomePage", "Uicom");
				return PartialView(partialView, model);
			}
			else if (!Request.IsAjaxRequest())
				return View("Uicom", model);
			else
				return PartialView(partialView, model);
		}
		...
		[ActionName("Uicom_New_Insert")]
		[AuthorizeForUsers]
		[HttpGet]
		public ActionResult Uicom_New_Insert()
		{
			...
			return RedirectToAction("Uicom_New");
		}

// USE /[MANUAL GQT CONTROLLER_NEW_GET UICOM]/
		//
		// GET: /Uicom/Uicom_New
		[ActionName("Uicom_New")]
		[AuthorizeForUsers]
		[HttpGet]
		public ActionResult Uicom_New()
		{
			...
			string partialView = qs["partialView"] ?? "Uicom";

			var navigationLocationAction = ACTION_UICOM_NEW.SetRoutedValues(new { m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);

			//FOR: OVERRIDE SKIP IF JUST ONE
			//Allow child form to use "Go Back" to menu list without "skip if only one"
			if (Navigation.OverrideSkipIfJustOne.ContainsKey("UICOM"))
				Navigation.OverrideSkipIfJustOne["UICOM"] = true;
...
// USE /[MANUAL GQT AFTER_LOAD_NEW UICOM]/
			...
			catch (Exception e)
			{
				...
				CSGenio.framework.Log.Error("Uicom_New - GET " + e.Message);
...
// USE /[MANUAL GQT CONTROLLER_NEW_POST UICOM]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Uicom_New(Uicom_ViewModel model, bool redirect = true)
		{
...
// USE /[MANUAL GQT AFTER_LOAD_NEW_EX UICOM]/
		...
		[AuthorizeForUsers]
		public ActionResult Uicom_New_Redirect(bool internalRedirect = false)
		{
			IList<string> warningMsgs = TempData["NEW_WARNINGS_LIST"] as List<string>;
			string saveMsg = TempData["NEW_SAVE_LIST"] as string;

			if (!internalRedirect)
				SuccessMessage(String.IsNullOrEmpty(saveMsg) ? Resources.Resources.REGISTO_CRIADO_COM_S18746 + GetHumanKeyToQMessage("uicom", Convert.ToString(Navigation.CurrentLevel.GetEntry("uicom"))) : saveMsg);
			...
			if (Convert.ToBoolean(Request.QueryString.Get("repeatInsertion")))
				return RedirectToLocation(ACTION_UICOM_NEW, new { repeatInsertion = true });
...
// USE /[MANUAL GQT CONTROLLER_EDIT_GET UICOM]/
		//
		// GET: /Uicom/Uicom_Edit
		[AuthorizeForUsers]
		[HttpGet]
		[ActionName("Uicom_Edit")]
		public ActionResult Uicom_Edit(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";
			string partialView = qs["partialView"] ?? "Uicom"; // MF send the partial view name

			//Useful to initialize a record that had to be created by the framework prior to edition (and method New doesnt support this). This way, if the form then is canceled by the user, the record will be automatically deleted
			var isNewEdit = Navigation.GetStrValue("NewEdit") == "true";

			var navigationLocationAction =  isNewEdit ? ACTION_UICOM_NEW.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] }) : ACTION_UICOM_EDIT.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);
...
// USE /[MANUAL GQT BEFORE_LOAD_EDIT UICOM]/
			...
			catch (ModelNotFoundException)
			{
				...
				sp.closeConnection();
				//JFG 05-05-2017 This need to better handled, for now it's handled on webconfig's customErrors section.
				CSGenio.framework.Log.Error("Uicom_Edit - " + id + " " + e.Message);
				throw;
			}
...
// USE /[MANUAL GQT CONTROLLER_EDIT_POST UICOM]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Uicom_Edit(Uicom_ViewModel model, bool redirect = true)
		{
...
// USE /[MANUAL GQT AFTER_LOAD_EDIT_EX UICOM]/
		...
		[AuthorizeForUsers]
		public ActionResult Uicom_Edit_Redirect(bool internalRedirect = false)
		{
			IList<string> warningMsgs = TempData["EDIT_WARNINGS_LIST"] as List<string>;
			string saveMsg = TempData["NEW_SAVE_LIST"] as string;

			if (!internalRedirect || !Request.IsAjaxRequest())
				SuccessMessage(String.IsNullOrEmpty(saveMsg) ? Resources.Resources.ALTERACOES_EFECTUADA64514 + GetHumanKeyToQMessage("uicom", Convert.ToString(Navigation.CurrentLevel.GetEntry("uicom"))) : saveMsg);
...
// USE /[MANUAL GQT CONTROLLER_DELETE_GET UICOM]/
		//
		// GET: /Uicom/Uicom_Delete
		[AuthorizeForUsers]
		[HttpGet]
		[ActionName("Uicom_Delete")]
		public ActionResult Uicom_Delete(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";

			var navigationLocationAction = ACTION_UICOM_DELETE.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);
...
// USE /[MANUAL GQT BEFORE_LOAD_DELETE UICOM]/
			...
			catch (ModelNotFoundException)
			{
				...
				//JFG 05-05-2017 This need to better handled, for now it's handled on webconfig's customErrors section.
				CSGenio.framework.Log.Error("Uicom_Delete - " + id + " " + e.Message);
				throw;
			}
...
// USE /[MANUAL GQT AFTER_LOAD_DELETE UICOM]/
			...
			if (!Request.IsAjaxRequest())
				...
				if (permission.Status.Equals(CSGenio.framework.Status.E))
					...
					return PartialView("Uicom", model);
		}
...
// USE /[MANUAL GQT CONTROLLER_DELETE_POST UICOM]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Uicom_Delete(string id, FormCollection collection)
		{

			var model = new Uicom_ViewModel (Navigation, id);
			model.MapFromModel();
...
// USE /[MANUAL GQT CONTROLLER_DUPLICATE_GET UICOM]/
		//
		// GET: /Uicom/Uicom_Duplicate
		[AuthorizeForUsers]
		[HttpGet]
		public ActionResult Uicom_Duplicate(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";
			var model = new Uicom_ViewModel(Navigation, nestedForm);
			...
			var navigationLocationAction = ACTION_UICOM_DUPLICATE.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);
...
// USE /[MANUAL GQT AFTER_LOAD_DUPLICATE UICOM]/
					...
					sp.closeTransaction();
					Navigation.CurrentLevel.SetLocation(ACTION_UICOM_DUPLICATE.SetRoutedValues(new { Id = model.ValCoduicom }));
...
// USE /[MANUAL GQT CONTROLLER_DUPLICATE_POST UICOM]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Uicom_Duplicate(Uicom_ViewModel model, bool redirect = true)
		{
			var eventSink = new EventSink()
			{
...
// USE /[MANUAL GQT AFTER_LOAD_DUPLICATE_EX UICOM]/
		...
		[AuthorizeForUsers]
		public ActionResult Uicom_Duplicate_Redirect(bool internalRedirect = false)
		{
...
// USE /[MANUAL GQT CONTROLLER_CANCEL_GET UICOM]/
		...
		public ActionResult Uicom_Cancel()
		{
			...
			if (Navigation.CurrentLevel.FormMode == FormMode.New || Navigation.CurrentLevel.FormMode == FormMode.Duplicate)
			{
				...
				try
				{
					var model = new GenioMVC.Models.Uicom();
...
// USE /[MANUAL GQT AFTER_CANCEL UICOM]/
			...
			else if (Navigation.PreviousLevel != null)
			{
				...
				string previousLevelArea = Navigation.PreviousLevel.Location.Controller?.ToLower() ?? "uicom";
		...
		{
			var model = new Uicom_ViewModel(Navigation, true);
			model.setModes(Request.QueryString["m"]);
			PersistentSupport sp = UserContext.Current.PersistentSupport;
			var navigationLocationAction = ACTION_UICOM_NEW.SetRoutedValues(new { m = Request.QueryString["m"] });
			CheckLevels(navigationLocationAction);
			...
			return PartialView("MFUicom", model);
		}
		...
		public ActionResult MFUicom_Edit(string id)
		{
			return this.RedirectToAction("Uicom_Edit", "Uicom", new { id = id, partialView = "MFUicom", nestedForm = "true", multiForm = "true" });
		}
		...
		public ActionResult MFUicom_Cancel(string id)
		{
			//MH - Validate if current level is it correct. Remove all levels that is higher than the currently level.
			CheckLevels(NavigationLocation.Any);

			if (Navigation.CurrentLevel.IsNestedContext &&
				(Navigation.CurrentLevel.Location.Action == ACTION_UICOM_NEW.Action || Navigation.CurrentLevel.Location.Action == ACTION_UICOM_EDIT.Action))
			{
				if (Navigation.CurrentLevel.FormMode == FormMode.New)
				{
					...
					catch (Exception e)
					{
						...
						Log.Error("MFUicom_Cancel - " + exceptionUserMessage);
						ErrorMessage(exceptionUserMessage);
					}
				}
			...
			return this.RedirectToAction("Uicom_Show", "Uicom", new { id = id, partialView = "MFUicom", nestedForm = "true", multiForm = "true", nav });
		}
		...
		public JsonResult MFUicom_Delete(string id)
		{
			var eventSink = new EventSink()
			{
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Uicom_SaveEdit(Uicom_ViewModel model)
		{
			var eventSink = new EventSink()
			{
