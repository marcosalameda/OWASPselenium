...
namespace GenioMVC.Controllers
{
	public partial class WarehController : ControllerBase
	{
		...
		[AuthorizeForUsers]
		public ActionResult ChangeFormMode_Armazpop(string mode)
		{
			...
			switch (mode)
			{
				case "show":
					if (m.Contains("v"))
						_mode = ACTION_ARMAZPOP_SHOW;
					break;
				case "delete":
					if (m.Contains("a"))
						_mode = ACTION_ARMAZPOP_DELETE;
					break;
				case "edit":
					if (m.Contains("e"))
						_mode = ACTION_ARMAZPOP_EDIT;
					break;
				case "duplicate":
					if (m.Contains("d"))
						_mode = ACTION_ARMAZPOP_DUPLICATE;
					break;
				case "new":
					if (m.Contains("i"))
						_mode = ACTION_ARMAZPOP_NEW;
					break;
			}
		...
		[AuthorizeForUsers]
		public ActionResult Armazpop_ModalDBEdit(string partialView)
		{
			Armazpop_ViewModel model = new Armazpop_ViewModel();
...
// USE /[MANUAL GQT CONTROLLER_SHOW ARMAZPOP]/
		//
		// GET: /Wareh/Armazpop_Show
		[AuthorizeForUsers]
		public ActionResult Armazpop_Show(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";
			var model = new Armazpop_ViewModel(Navigation, nestedForm);
			model.setModes(Request.QueryString["m"]);
			...
			string partialView = qs["partialView"] ?? "Armazpop"; // MF send the patial view name

			var navigationLocationAction = ACTION_ARMAZPOP_SHOW.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
...
// USE /[MANUAL GQT BEFORE_LOAD_SHOW ARMAZPOP]/
			...
			catch (ModelNotFoundException)
			{
				...
				CSGenio.framework.Log.Error("Armazpop_Show - GET " + id + " " + e.Message);
				throw;
			}

// USE /[MANUAL GQT AFTER_LOAD_SHOW ARMAZPOP]/

			return PartialView("Armazpop", model);
		}
		...
		[ActionName("Armazpop_New_Insert")]
		[AuthorizeForUsers]
		[HttpGet]
		public ActionResult Armazpop_New_Insert()
		{
			...
			return RedirectToAction("Armazpop_New");
		}

// USE /[MANUAL GQT CONTROLLER_NEW_GET ARMAZPOP]/
		//
		// GET: /Wareh/Armazpop_New
		[ActionName("Armazpop_New")]
		[AuthorizeForUsers]
		[HttpGet]
		public ActionResult Armazpop_New()
		{
			...
			var navigationLocationAction = ACTION_ARMAZPOP_NEW.SetRoutedValues(new { m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);

			//FOR: OVERRIDE SKIP IF JUST ONE
			//Allow child form to use "Go Back" to menu list without "skip if only one"
			if (Navigation.OverrideSkipIfJustOne.ContainsKey("ARMAZPOP"))
				Navigation.OverrideSkipIfJustOne["ARMAZPOP"] = true;
...
// USE /[MANUAL GQT AFTER_LOAD_NEW ARMAZPOP]/
			...
			catch (Exception e)
			{
				...
				CSGenio.framework.Log.Error("Armazpop_New - GET " + e.Message);
...
// USE /[MANUAL GQT CONTROLLER_NEW_POST ARMAZPOP]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Armazpop_New(Armazpop_ViewModel model, bool redirect = true)
		{
...
// USE /[MANUAL GQT AFTER_LOAD_NEW_EX ARMAZPOP]/
		...
		[AuthorizeForUsers]
		public ActionResult Armazpop_New_Redirect(bool internalRedirect = false)
		{
...
// USE /[MANUAL GQT CONTROLLER_EDIT_GET ARMAZPOP]/
		//
		// GET: /Wareh/Armazpop_Edit
		[AuthorizeForUsers]
		[HttpGet]
		[ActionName("Armazpop_Edit")]
		public ActionResult Armazpop_Edit(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";
			string partialView = qs["partialView"] ?? "Armazpop"; // MF send the partial view name

			//Useful to initialize a record that had to be created by the framework prior to edition (and method New doesnt support this). This way, if the form then is canceled by the user, the record will be automatically deleted
			var isNewEdit = Navigation.GetStrValue("NewEdit") == "true";

			var navigationLocationAction =  isNewEdit ? ACTION_ARMAZPOP_NEW.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] }) : ACTION_ARMAZPOP_EDIT.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);
...
// USE /[MANUAL GQT BEFORE_LOAD_EDIT ARMAZPOP]/
			...
			catch (ModelNotFoundException)
			{
				...
				sp.closeConnection();
				//JFG 05-05-2017 This need to better handled, for now it's handled on webconfig's customErrors section.
				CSGenio.framework.Log.Error("Armazpop_Edit - " + id + " " + e.Message);
				throw;
			}
...
// USE /[MANUAL GQT CONTROLLER_EDIT_POST ARMAZPOP]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Armazpop_Edit(Armazpop_ViewModel model, bool redirect = true)
		{
...
// USE /[MANUAL GQT AFTER_LOAD_EDIT_EX ARMAZPOP]/
		...
		[AuthorizeForUsers]
		public ActionResult Armazpop_Edit_Redirect(bool internalRedirect = false)
		{
...
// USE /[MANUAL GQT CONTROLLER_DELETE_GET ARMAZPOP]/
		//
		// GET: /Wareh/Armazpop_Delete
		[AuthorizeForUsers]
		[HttpGet]
		[ActionName("Armazpop_Delete")]
		public ActionResult Armazpop_Delete(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";

			var navigationLocationAction = ACTION_ARMAZPOP_DELETE.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);
...
// USE /[MANUAL GQT BEFORE_LOAD_DELETE ARMAZPOP]/
			...
			catch (ModelNotFoundException)
			{
				...
				//JFG 05-05-2017 This need to better handled, for now it's handled on webconfig's customErrors section.
				CSGenio.framework.Log.Error("Armazpop_Delete - " + id + " " + e.Message);
				throw;
			}
...
// USE /[MANUAL GQT CONTROLLER_DELETE_POST ARMAZPOP]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Armazpop_Delete(string id, FormCollection collection)
		{

			var model = new Armazpop_ViewModel (Navigation, id);
			model.MapFromModel();
...
// USE /[MANUAL GQT CONTROLLER_DUPLICATE_GET ARMAZPOP]/
		//
		// GET: /Wareh/Armazpop_Duplicate
		[AuthorizeForUsers]
		[HttpGet]
		public ActionResult Armazpop_Duplicate(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";
			var model = new Armazpop_ViewModel(Navigation, nestedForm);
			...
			var navigationLocationAction = ACTION_ARMAZPOP_DUPLICATE.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);
...
// USE /[MANUAL GQT AFTER_LOAD_DUPLICATE ARMAZPOP]/
					...
					sp.closeTransaction();
					Navigation.CurrentLevel.SetLocation(ACTION_ARMAZPOP_DUPLICATE.SetRoutedValues(new { Id = model.ValCodwareh }));
...
// USE /[MANUAL GQT CONTROLLER_DUPLICATE_POST ARMAZPOP]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Armazpop_Duplicate(Armazpop_ViewModel model, bool redirect = true)
		{
			var eventSink = new EventSink()
			{
...
// USE /[MANUAL GQT AFTER_LOAD_DUPLICATE_EX ARMAZPOP]/
		...
		[AuthorizeForUsers]
		public ActionResult Armazpop_Duplicate_Redirect(bool internalRedirect = false)
		{
...
// USE /[MANUAL GQT AFTER_CANCEL ARMAZPOP]/
		...
		{
			var model = new Armazpop_ViewModel(Navigation, true);
			model.setModes(Request.QueryString["m"]);
			PersistentSupport sp = UserContext.Current.PersistentSupport;
			var navigationLocationAction = ACTION_ARMAZPOP_NEW.SetRoutedValues(new { m = Request.QueryString["m"] });
			CheckLevels(navigationLocationAction);
			...
			return PartialView("MFArmazpop", model);
		}
		...
		public ActionResult MFArmazpop_Edit(string id)
		{
			return this.RedirectToAction("Armazpop_Edit", "Wareh", new { id = id, partialView = "MFArmazpop", nestedForm = "true", multiForm = "true" });
		}
		...
		public ActionResult MFArmazpop_Cancel(string id)
		{
			//MH - Validate if current level is it correct. Remove all levels that is higher than the currently level.
			CheckLevels(NavigationLocation.Any);

			if (Navigation.CurrentLevel.IsNestedContext &&
				(Navigation.CurrentLevel.Location.Action == ACTION_ARMAZPOP_NEW.Action || Navigation.CurrentLevel.Location.Action == ACTION_ARMAZPOP_EDIT.Action))
			{
				if (Navigation.CurrentLevel.FormMode == FormMode.New)
				{
					...
					catch (Exception e)
					{
						...
						Log.Error("MFArmazpop_Cancel - " + exceptionUserMessage);
						ErrorMessage(exceptionUserMessage);
					}
				}
			...
			return this.RedirectToAction("Armazpop_Show", "Wareh", new { id = id, partialView = "MFArmazpop", nestedForm = "true", multiForm = "true", nav });
		}
		...
		public JsonResult MFArmazpop_Delete(string id)
		{
			var eventSink = new EventSink()
			{
		...
		public ActionResult Armaz02_ValArtigos(string id, string partialView)
		{
			...
			Armaz02_ValArtigos_ViewModel model = new Armaz02_ValArtigos_ViewModel(navigation);
			model.setModes(Request.QueryString["m"]);
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Armazpop_SaveEdit(Armazpop_ViewModel model)
		{
			var eventSink = new EventSink()
			{
