...
{
    public struct NavigationLocation/* : ISerializable */
    {
        public static NavigationLocation Empty = new NavigationLocation();
        public static NavigationLocation Any = new NavigationLocation(null, "index", "home") { m_any = true };
        ...
        {
            string id = "";
            if (m_routedValues != null && m_routedValues.ContainsKey("id"))
                id = "id:" + m_routedValues["id"]?.ToString();

            string descr = string.Format("{0} {1} {2}", m_action, m_name, id);
            return descr;
        }
    }
}
