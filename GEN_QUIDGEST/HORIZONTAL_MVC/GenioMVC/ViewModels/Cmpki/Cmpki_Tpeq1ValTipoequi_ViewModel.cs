...
{
    public class Cmpki_Tpeq1ValTipoequi_ViewModel : ListViewModel
    {
        /// <summary>
        ...
        /// <inheritdoc/>
        public override string Uuid { get => "Cmpki_Tpeq1ValTipoequi"; }
        ...
        public Cmpki_Tpeq1ValTipoequi_ViewModel(NavigationContext currentNavigation) : base(currentNavigation)
        {
		...
		public void Load(int numberListItems, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAtpeq1> Qlisting, ref CriteriaSet conditions)
		{
			...
			if (ajaxRequest)
				this.Navigation.SetValue("requestValues" + "Cmpki_Tpeq1ValTipoequi", requestValues);
			else if (!ajaxRequest && this.Navigation.CheckKey("requestValues" + "Cmpki_Tpeq1ValTipoequi"))
				requestValues = this.Navigation.GetValue<NameValueCollection>("requestValues" + "Cmpki_Tpeq1ValTipoequi");
			...
			var pageNumber = (ajaxRequest && !String.IsNullOrEmpty(requestValues["pCmpki_Tpeq1ValTipoequi"])) ? int.Parse(requestValues["pCmpki_Tpeq1ValTipoequi"]) : 1;

			// Added to avoid 0 or -1 pages when setting number of records to -1 to disable pagination
			if (pageNumber < 1)
				pageNumber = 1;

			List<ColumnSort> sorts = GetRequestSorts(this.Menu, "sCmpki_Tpeq1ValTipoequi", "dCmpki_Tpeq1ValTipoequi", requestValues, "tpeq1", allSortOrders);
...
// USE /[MANUAL GQT OVERRQLIST CMPKI_TIPOEQUI]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = UserContext.Current.PersistentSupport.getPagingPos(CSGenioAtpeq1.GetInformation(), QMVC_POS_RECORD, sorts, cmpki___tpeq1tipoequiConds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioAtpeq1> listing = Models.ModelBase.Where<CSGenioAtpeq1>(false, cmpki___tpeq1tipoequiConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_CMPKI___TPEQ1TIPOEQUI", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
        ...
        private List<Models.Tpeq1> MapCmpki_Tpeq1ValTipoequi(ListingMVC<CSGenioAtpeq1> Qlisting)
        {
        ...
        private Models.Tpeq1 MapCmpki_Tpeq1ValTipoequi(CSGenioAtpeq1 row)
        {
