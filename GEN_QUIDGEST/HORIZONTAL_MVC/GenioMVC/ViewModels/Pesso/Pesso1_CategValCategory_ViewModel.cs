...
{
    public class Pesso1_CategValCategoria_ViewModel : ListViewModel
    {
        /// <summary>
        ...
        /// <inheritdoc/>
        public override string Uuid { get => "Pesso1_CategValCategoria"; }
        ...
        public Pesso1_CategValCategoria_ViewModel(NavigationContext currentNavigation) : base(currentNavigation)
        {
		...
		public override CriteriaSet BuildCriteriaSet(NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			Menu.SetFilters(bool.Parse(requestValues["Pesso1_CategValCategoria_tableFilters"] ?? "false"), false);
		...
		public void Load(int numberListItems, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAcateg> Qlisting, ref CriteriaSet conditions)
		{
			...
			if (ajaxRequest)
				this.Navigation.SetValue("requestValues" + "Pesso1_CategValCategoria", requestValues);
			else if (!ajaxRequest && this.Navigation.CheckKey("requestValues" + "Pesso1_CategValCategoria"))
				requestValues = this.Navigation.GetValue<NameValueCollection>("requestValues" + "Pesso1_CategValCategoria");
			...
			var pageNumber = (ajaxRequest && !String.IsNullOrEmpty(requestValues["pPesso1_CategValCategoria"])) ? int.Parse(requestValues["pPesso1_CategValCategoria"]) : 1;

			// Added to avoid 0 or -1 pages when setting number of records to -1 to disable pagination
			if (pageNumber < 1)
				pageNumber = 1;

			List<ColumnSort> sorts = GetRequestSorts(this.Menu, "sPesso1_CategValCategoria", "dPesso1_CategValCategoria", requestValues, "categ", allSortOrders);
...
FieldRef[] fields = new FieldRef[] { CSGenioAcateg.FldCodcateg, CSGenioAcateg.FldZzstate, CSGenioAcateg.FldCategoria, CSGenioAcateg.FldAbbreviation };
			...
			CriteriaSet pesso1__categcategoryConds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
            tableReload &= hasAllRequiredLimits;

// USE /[MANUAL GQT OVERRQ PESSO1_CATEGCATEGORY]/

            // This will happen in case there is an error
            if(pesso1__categcategoryConds == null)
                return;
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAcateg>(false, pesso1__categcategoryConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_PESSO1__CATEGCATEGORY", true, firstVisibleColumn: firstVisibleColumn);
...
// USE /[MANUAL GQT OVERRQLIST PESSO1_CATEGCATEGORY]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = UserContext.Current.PersistentSupport.getPagingPos(CSGenioAcateg.GetInformation(), QMVC_POS_RECORD, sorts, pesso1__categcategoryConds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioAcateg> listing = Models.ModelBase.Where<CSGenioAcateg>(false, pesso1__categcategoryConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_PESSO1__CATEGCATEGORY", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
        ...
        private Models.Categ MapPesso1_CategValCategoria(CSGenioAcateg row)
        {
