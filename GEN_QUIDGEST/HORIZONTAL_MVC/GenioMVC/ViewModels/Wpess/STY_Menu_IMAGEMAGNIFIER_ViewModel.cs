...
namespace GenioMVC.ViewModels.Wpess
{
    public class STY_Menu_IMAGEMAGNIFIER_ViewModel : ListViewModel
    {
        ...
        {
            dbeditTitle = Resources.Resources.IMAGE_MAGNIFIER35311;
        }
...
FieldRef[] fields = new FieldRef[] { CSGenioAwpess.FldCodpess, CSGenioAwpess.FldZzstate, CSGenioAwpess.FldName, CSGenioAwpess.FldDate, CSGenioAwpess.FldSex, CSGenioAwpess.FldNfunc, CSGenioAwpess.FldAdress, CSGenioAwpess.FldZipcode, CSGenioAwpess.FldCountry, CSGenioAwpess.FldEmail, CSGenioAwpess.FldCellphon, CSGenioAwpess.FldNaturali, CSGenioAwpess.FldNacional, CSGenioAwpess.FldPfoto, CSGenioAwpess.FldCodwareh, CSGenioAwareh.FldCodwareh, CSGenioAwareh.FldWarehdes, CSGenioAwpess.FldFtimgtop, CSGenioAwpess.FldFtthumb, CSGenioAwpess.FldFtbackgr };
        ...
        public STY_Menu_IMAGEMAGNIFIER_ViewModel(NavigationContext currentNavigation) : base(currentNavigation)
        {
		...
		public void Load(int numberListItems, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAwpess> Qlisting, ref CriteriaSet conditions)
		{
			...
			if (ajaxRequest)
				this.Navigation.SetValue("requestValues" + "STY_Menu_IMAGEMAGNIFIER", requestValues);
			else if (!ajaxRequest && this.Navigation.CheckKey("requestValues" + "STY_Menu_IMAGEMAGNIFIER"))
				requestValues = this.Navigation.GetValue<NameValueCollection>("requestValues" + "STY_Menu_IMAGEMAGNIFIER");
			...
			var pageNumber = (ajaxRequest && !String.IsNullOrEmpty(requestValues["pSTY_Menu_IMAGEMAGNIFIER"])) ? int.Parse(requestValues["pSTY_Menu_IMAGEMAGNIFIER"]) : 1;

			// Added to avoid 0 or -1 pages when setting number of records to -1 to disable pagination
			if (pageNumber < 1)
				pageNumber = 1;

			List<ColumnSort> sorts = GetRequestSorts(this.Menu, "sSTY_Menu_IMAGEMAGNIFIER", "dSTY_Menu_IMAGEMAGNIFIER", requestValues, "wpess", allSortOrders);
...
FieldRef[] fields = new FieldRef[] { CSGenioAwpess.FldCodpess, CSGenioAwpess.FldZzstate, CSGenioAwpess.FldName, CSGenioAwpess.FldDate, CSGenioAwpess.FldSex, CSGenioAwpess.FldNfunc, CSGenioAwpess.FldAdress, CSGenioAwpess.FldZipcode, CSGenioAwpess.FldCountry, CSGenioAwpess.FldEmail, CSGenioAwpess.FldCellphon, CSGenioAwpess.FldNaturali, CSGenioAwpess.FldNacional, CSGenioAwpess.FldPfoto, CSGenioAwpess.FldCodwareh, CSGenioAwareh.FldCodwareh, CSGenioAwareh.FldWarehdes, CSGenioAwpess.FldFtimgtop, CSGenioAwpess.FldFtthumb, CSGenioAwpess.FldFtbackgr };
			...
			//Limit type: "SC"			//Current Area = "WPESS"			//1st Area Limit: "WPESS"			//1st Area Field: "SHOWRECO"			//1st Area Value: "1"
			{
				Limit limit = new Limit();
			...
			CriteriaSet sty_menu_imagemagnifierConds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
            tableReload &= hasAllRequiredLimits;

// USE /[MANUAL STY OVERRQ IMAGEMAGNIFIER]/

            // This will happen in case there is an error
            if(sty_menu_imagemagnifierConds == null)
                return;
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAwpess>(false, sty_menu_imagemagnifierConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "MLIMAGEMAGNIFIER", true, firstVisibleColumn: firstVisibleColumn);
...
// USE /[MANUAL STY OVERRQLIST IMAGEMAGNIFIER]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = UserContext.Current.PersistentSupport.getPagingPos(CSGenioAwpess.GetInformation(), QMVC_POS_RECORD, sorts, sty_menu_imagemagnifierConds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioAwpess> listing = Models.ModelBase.Where<CSGenioAwpess>(false, sty_menu_imagemagnifierConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "MLIMAGEMAGNIFIER", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
        ...
        private Models.Wpess MapSTY_Menu_IMAGEMAGNIFIER(CSGenioAwpess row)
        {
