...
// USE /[MANUAL GQT MANUAL_CONTROLLER ASSET]/
        ...
        public JsonResult ReloadDBEdit(string Identifier, IDictionary<string, object> Values)
        {
            ...
            try { row = Models.Asset.Find(navigation.GetStrValue("asset")); }
            catch (Exception)
            {
					...
					case "EQUIPM__MANUFNAME____":	// Field (DB)
                        {
						    ...
						    var model = new Equipm_ViewModel(navigation) { editable = false };
        ...
        public JsonResult GetDependants(string Identifier, string Selected, IDictionary<string, object> Limits)
        {
            ...
            try
            {
				...
				{
					case "EQUIPM__MANUFNAME____":	// Field (DB)
						values = Equipm_ViewModel.GetDependant_EquipmTableManufName(Selected, navigation);
						break;
					case "EQUIPM__KINDEDESIGNAT":	// Field (DB)
						values = Equipm_ViewModel.GetDependant_EquipmTableKindeDesignat(Selected, navigation);
						break;
					default: break;
				}
		...
		[AuthorizeForUsers]
        [ActionSessionState(System.Web.SessionState.SessionStateBehavior.ReadOnly)]
        public JsonResult RecalculateFormulas_Equipm(Equipm_ViewModel form_data)
        {
            return GenericRecalculateFormulas(form_data, "asset",
                (primaryKey) => Models.Asset.Find(primaryKey, "FEQUIPM"),
