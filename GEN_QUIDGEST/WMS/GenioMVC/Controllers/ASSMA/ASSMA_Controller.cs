...
// USE /[MANUAL GQT MANUAL_CONTROLLER ASSMA]/
        ...
        public JsonResult ReloadDBEdit(string Identifier, IDictionary<string, object> Values)
        {
            ...
            try { row = Models.Assma.Find(navigation.GetStrValue("assma")); }
            catch (Exception)
            {
					...
					case "ASSMA___ASSETNAME____":	// Field (DB)
                        {
						    ...
						    var model = new Assma_ViewModel(navigation) { editable = false };
        ...
        public JsonResult GetDependants(string Identifier, string Selected, IDictionary<string, object> Limits)
        {
            ...
            try
            {
				...
				{
					case "ASSMA___ASSETNAME____":	// Field (DB)
						values = Assma_ViewModel.GetDependant_AssmaTableAssetName(Selected, navigation);
						break;
					default: break;
				}
		...
		[AuthorizeForUsers]
        [ActionSessionState(System.Web.SessionState.SessionStateBehavior.ReadOnly)]
        public JsonResult RecalculateFormulas_Assma(Assma_ViewModel form_data)
        {
            return GenericRecalculateFormulas(form_data, "assma",
                (primaryKey) => Models.Assma.Find(primaryKey, "FASSMA"),
