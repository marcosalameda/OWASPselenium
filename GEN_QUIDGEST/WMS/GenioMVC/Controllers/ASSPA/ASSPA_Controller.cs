...
// USE /[MANUAL GQT MANUAL_CONTROLLER ASSPA]/
        ...
        public JsonResult ReloadDBEdit(string Identifier, IDictionary<string, object> Values)
        {
            ...
            try { row = Models.Asspa.Find(navigation.GetStrValue("asspa")); }
            catch (Exception)
            {
					...
					case "ASSPA___ASSETNAME____":	// Field (DB)
                        {
						    ...
						    var model = new Asspa_ViewModel(navigation) { editable = false };
        ...
        public JsonResult GetDependants(string Identifier, string Selected, IDictionary<string, object> Limits)
        {
            ...
            try
            {
				...
				{
					case "ASSPA___ASSETNAME____":	// Field (DB)
						values = Asspa_ViewModel.GetDependant_AsspaTableAssetName(Selected, navigation);
						break;
					case "ASSPA___PARAMPARAMETE":	// Field (DB)
						values = Asspa_ViewModel.GetDependant_AsspaTableParamParamete(Selected, navigation);
						break;
					default: break;
				}
		...
		[AuthorizeForUsers]
        [ActionSessionState(System.Web.SessionState.SessionStateBehavior.ReadOnly)]
        public JsonResult RecalculateFormulas_Asspa(Asspa_ViewModel form_data)
        {
            return GenericRecalculateFormulas(form_data, "asspa",
                (primaryKey) => Models.Asspa.Find(primaryKey, "FASSPA"),
