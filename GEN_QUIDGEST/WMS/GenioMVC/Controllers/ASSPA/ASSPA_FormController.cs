...
namespace GenioMVC.Controllers
{
	public partial class AsspaController : ControllerBase
	{
		...
		[AuthorizeForUsers]
		public ActionResult ChangeFormMode_Asspa(string mode)
		{
			var _mode = Navigation.CurrentLevel.Location;
			var id = Navigation.GetStrValue("asspa");
			...
			switch (mode)
			{
				case "show":
					if (m.Contains("v"))
						_mode = ACTION_ASSPA_SHOW;
					break;
				case "delete":
					if (m.Contains("a"))
						_mode = ACTION_ASSPA_DELETE;
					break;
				case "edit":
					if (m.Contains("e"))
						_mode = ACTION_ASSPA_EDIT;
					break;
				case "duplicate":
					if (m.Contains("d"))
						_mode = ACTION_ASSPA_DUPLICATE;
					break;
				case "new":
					if (m.Contains("i"))
						_mode = ACTION_ASSPA_NEW;
					break;
			}
...
// USE /[MANUAL GQT CONTROLLER_SHOW ASSPA]/
		//
		// GET: /Asspa/Asspa_Show
		[AuthorizeForUsers]
		public ActionResult Asspa_Show(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";
			var model = new Asspa_ViewModel(Navigation, nestedForm);
			...
			string partialView = qs["partialView"] ?? "Asspa"; // MF send the patial view name

			var navigationLocationAction = ACTION_ASSPA_SHOW.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			...
			if (!isHomePage && (IsNewLocation(navigationLocationAction) || !string.Equals(Navigation.GetStrValue("asspa"), id)))
			{
				Navigation.AddHistoryLevel(navigationLocationAction, FormMode.Show, nestedForm);
				CSGenio.framework.Audit.registAction(UserContext.Current.User, Resources.Resources.FORM54242 + " " + Navigation.CurrentLevel.Location.ShortDescription());
			}

			Navigation.SetValue("asspa", id);
...
// USE /[MANUAL GQT BEFORE_LOAD_SHOW ASSPA]/
			...
			catch (ModelNotFoundException)
			{
				...
				CSGenio.framework.Log.Error("Asspa_Show - GET " + id + " " + e.Message);
				throw;
			}
...
// USE /[MANUAL GQT AFTER_LOAD_SHOW ASSPA]/
			...
			if (isHomePage)
			{
				Navigation.SetValue("HomePage", "Asspa");
				return PartialView(partialView, model);
			}
			else if (!Request.IsAjaxRequest())
				return View("Asspa", model);
			else
				return PartialView(partialView, model);
		}
		...
		[ActionName("Asspa_New_Insert")]
		[AuthorizeForUsers]
		[HttpGet]
		public ActionResult Asspa_New_Insert()
		{
			...
			return RedirectToAction("Asspa_New");
		}

// USE /[MANUAL GQT CONTROLLER_NEW_GET ASSPA]/
		//
		// GET: /Asspa/Asspa_New
		[ActionName("Asspa_New")]
		[AuthorizeForUsers]
		[HttpGet]
		public ActionResult Asspa_New()
		{
			...
			string partialView = qs["partialView"] ?? "Asspa";

			var navigationLocationAction = ACTION_ASSPA_NEW.SetRoutedValues(new { m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);

			//FOR: OVERRIDE SKIP IF JUST ONE
			//Allow child form to use "Go Back" to menu list without "skip if only one"
			if (Navigation.OverrideSkipIfJustOne.ContainsKey("ASSPA"))
				Navigation.OverrideSkipIfJustOne["ASSPA"] = true;
...
// USE /[MANUAL GQT AFTER_LOAD_NEW ASSPA]/
			...
			catch (Exception e)
			{
				...
				CSGenio.framework.Log.Error("Asspa_New - GET " + e.Message);
...
// USE /[MANUAL GQT CONTROLLER_NEW_POST ASSPA]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Asspa_New(Asspa_ViewModel model, bool redirect = true)
		{
...
// USE /[MANUAL GQT AFTER_LOAD_NEW_EX ASSPA]/
		...
		[AuthorizeForUsers]
		public ActionResult Asspa_New_Redirect(bool internalRedirect = false)
		{
			IList<string> warningMsgs = TempData["NEW_WARNINGS_LIST"] as List<string>;
			string saveMsg = TempData["NEW_SAVE_LIST"] as string;

			if (!internalRedirect)
				SuccessMessage(String.IsNullOrEmpty(saveMsg) ? Resources.Resources.REGISTO_CRIADO_COM_S18746 + GetHumanKeyToQMessage("asspa", Convert.ToString(Navigation.CurrentLevel.GetEntry("asspa"))) : saveMsg);
			...
			if (Convert.ToBoolean(Request.QueryString.Get("repeatInsertion")))
				return RedirectToLocation(ACTION_ASSPA_NEW, new { repeatInsertion = true });
...
// USE /[MANUAL GQT CONTROLLER_EDIT_GET ASSPA]/
		//
		// GET: /Asspa/Asspa_Edit
		[AuthorizeForUsers]
		[HttpGet]
		[ActionName("Asspa_Edit")]
		public ActionResult Asspa_Edit(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";
			string partialView = qs["partialView"] ?? "Asspa"; // MF send the partial view name

			//Useful to initialize a record that had to be created by the framework prior to edition (and method New doesnt support this). This way, if the form then is canceled by the user, the record will be automatically deleted
			var isNewEdit = Navigation.GetStrValue("NewEdit") == "true";

			var navigationLocationAction =  isNewEdit ? ACTION_ASSPA_NEW.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] }) : ACTION_ASSPA_EDIT.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);
...
// USE /[MANUAL GQT BEFORE_LOAD_EDIT ASSPA]/
			...
			catch (ModelNotFoundException)
			{
				...
				sp.closeConnection();
				//JFG 05-05-2017 This need to better handled, for now it's handled on webconfig's customErrors section.
				CSGenio.framework.Log.Error("Asspa_Edit - " + id + " " + e.Message);
				throw;
			}
...
// USE /[MANUAL GQT CONTROLLER_EDIT_POST ASSPA]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Asspa_Edit(Asspa_ViewModel model, bool redirect = true)
		{
...
// USE /[MANUAL GQT AFTER_LOAD_EDIT_EX ASSPA]/
		...
		[AuthorizeForUsers]
		public ActionResult Asspa_Edit_Redirect(bool internalRedirect = false)
		{
			IList<string> warningMsgs = TempData["EDIT_WARNINGS_LIST"] as List<string>;
			string saveMsg = TempData["NEW_SAVE_LIST"] as string;

			if (!internalRedirect || !Request.IsAjaxRequest())
				SuccessMessage(String.IsNullOrEmpty(saveMsg) ? Resources.Resources.ALTERACOES_EFECTUADA64514 + GetHumanKeyToQMessage("asspa", Convert.ToString(Navigation.CurrentLevel.GetEntry("asspa"))) : saveMsg);
...
// USE /[MANUAL GQT CONTROLLER_DELETE_GET ASSPA]/
		//
		// GET: /Asspa/Asspa_Delete
		[AuthorizeForUsers]
		[HttpGet]
		[ActionName("Asspa_Delete")]
		public ActionResult Asspa_Delete(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";

			var navigationLocationAction = ACTION_ASSPA_DELETE.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);
...
// USE /[MANUAL GQT BEFORE_LOAD_DELETE ASSPA]/
			...
			catch (ModelNotFoundException)
			{
				...
				//JFG 05-05-2017 This need to better handled, for now it's handled on webconfig's customErrors section.
				CSGenio.framework.Log.Error("Asspa_Delete - " + id + " " + e.Message);
				throw;
			}
...
// USE /[MANUAL GQT AFTER_LOAD_DELETE ASSPA]/
			...
			if (!Request.IsAjaxRequest())
				...
				if (permission.Status.Equals(CSGenio.framework.Status.E))
					...
					return PartialView("Asspa", model);
		}
...
// USE /[MANUAL GQT CONTROLLER_DELETE_POST ASSPA]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Asspa_Delete(string id, FormCollection collection)
		{

			var model = new Asspa_ViewModel (Navigation, id);
			model.MapFromModel();
...
// USE /[MANUAL GQT CONTROLLER_DUPLICATE_GET ASSPA]/
		//
		// GET: /Asspa/Asspa_Duplicate
		[AuthorizeForUsers]
		[HttpGet]
		public ActionResult Asspa_Duplicate(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";
			var model = new Asspa_ViewModel(Navigation, nestedForm);
			...
			var navigationLocationAction = ACTION_ASSPA_DUPLICATE.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);
...
// USE /[MANUAL GQT AFTER_LOAD_DUPLICATE ASSPA]/
					...
					sp.closeTransaction();
					Navigation.CurrentLevel.SetLocation(ACTION_ASSPA_DUPLICATE.SetRoutedValues(new { Id = model.ValCodasspa }));
...
// USE /[MANUAL GQT CONTROLLER_DUPLICATE_POST ASSPA]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Asspa_Duplicate(Asspa_ViewModel model, bool redirect = true)
		{
			var eventSink = new EventSink()
			{
...
// USE /[MANUAL GQT AFTER_LOAD_DUPLICATE_EX ASSPA]/
		...
		[AuthorizeForUsers]
		public ActionResult Asspa_Duplicate_Redirect(bool internalRedirect = false)
		{
...
// USE /[MANUAL GQT CONTROLLER_CANCEL_GET ASSPA]/
		...
		public ActionResult Asspa_Cancel()
		{
			...
			if (Navigation.CurrentLevel.FormMode == FormMode.New || Navigation.CurrentLevel.FormMode == FormMode.Duplicate)
			{
				...
				try
				{
					var model = new GenioMVC.Models.Asspa();
...
// USE /[MANUAL GQT AFTER_CANCEL ASSPA]/
			...
			else if (Navigation.PreviousLevel != null)
			{
				...
				string previousLevelArea = Navigation.PreviousLevel.Location.Controller?.ToLower() ?? "asspa";
		...
		{
			var model = new Asspa_ViewModel(Navigation, true);
			model.setModes(Request.QueryString["m"]);
			PersistentSupport sp = UserContext.Current.PersistentSupport;
			var navigationLocationAction = ACTION_ASSPA_NEW.SetRoutedValues(new { m = Request.QueryString["m"] });
			CheckLevels(navigationLocationAction);
			...
			return PartialView("MFAsspa", model);
		}
		...
		public ActionResult MFAsspa_Edit(string id)
		{
			return this.RedirectToAction("Asspa_Edit", "Asspa", new { id = id, partialView = "MFAsspa", nestedForm = "true", multiForm = "true" });
		}
		...
		public ActionResult MFAsspa_Cancel(string id)
		{
			//MH - Validate if current level is it correct. Remove all levels that is higher than the currently level.
			CheckLevels(NavigationLocation.Any);

			if (Navigation.CurrentLevel.IsNestedContext &&
				(Navigation.CurrentLevel.Location.Action == ACTION_ASSPA_NEW.Action || Navigation.CurrentLevel.Location.Action == ACTION_ASSPA_EDIT.Action))
			{
				if (Navigation.CurrentLevel.FormMode == FormMode.New)
				{
					...
					catch (Exception e)
					{
						...
						Log.Error("MFAsspa_Cancel - " + exceptionUserMessage);
						ErrorMessage(exceptionUserMessage);
					}
				}
			...
			return this.RedirectToAction("Asspa_Show", "Asspa", new { id = id, partialView = "MFAsspa", nestedForm = "true", multiForm = "true", nav });
		}
		...
		public JsonResult MFAsspa_Delete(string id)
		{
			var eventSink = new EventSink()
			{
		...
		public ActionResult Asspa_AssetValName(string id, string partialView,  IDictionary<string, string> Limits)
		{
			...
			Asspa_AssetValName_ViewModel model = new Asspa_AssetValName_ViewModel(navigation);
			model.setModes(Request.QueryString["m"]);
			model.ValCodasspa = id;
		...
		[ActionSessionState(System.Web.SessionState.SessionStateBehavior.ReadOnly)]
		public ActionResult Asspa_ParamValParameter(string id, string partialView,  IDictionary<string, string> Limits)
		{
			int perPage = CSGenio.framework.Configuration.NrRegDBedit;
			...
			NameValueCollection requestValues = Request.Unvalidated.Form.Count > 0 ? Request.Unvalidated.Form : Request.QueryString; //TSX (01.07.2020) Can not access directly to the FormCollection or made Request.Form otherwise the tags on viewmodel will be ignored
			var navigation = Navigation.Clone();
			Asspa_ParamValParameter_ViewModel model = new Asspa_ParamValParameter_ViewModel(navigation);
			model.setModes(Request.QueryString["m"]);
			model.ValCodasspa = id;
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Asspa_SaveEdit(Asspa_ViewModel model)
		{
			var eventSink = new EventSink()
			{
