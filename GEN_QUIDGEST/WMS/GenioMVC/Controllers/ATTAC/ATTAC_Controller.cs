...
// USE /[MANUAL GQT MANUAL_CONTROLLER ATTAC]/
        ...
        public JsonResult ReloadDBEdit(string Identifier, IDictionary<string, object> Values)
        {
            ...
            try { row = Models.Attac.Find(navigation.GetStrValue("attac")); }
            catch (Exception)
            {
					...
					case "ATTAC___ASSETNAME____":	// Field (DB)
                        {
						    ...
						    var model = new Attac_ViewModel(navigation) { editable = false };
        ...
        public JsonResult GetDependants(string Identifier, string Selected, IDictionary<string, object> Limits)
        {
            ...
            try
            {
				...
				{
					case "ATTAC___ASSETNAME____":	// Field (DB)
						values = Attac_ViewModel.GetDependant_AttacTableAssetName(Selected, navigation);
						break;
					default: break;
				}
		...
		[AuthorizeForUsers]
        [ActionSessionState(System.Web.SessionState.SessionStateBehavior.ReadOnly)]
        public JsonResult RecalculateFormulas_Attac(Attac_ViewModel form_data)
        {
            return GenericRecalculateFormulas(form_data, "attac",
                (primaryKey) => Models.Attac.Find(primaryKey, "FATTAC"),
