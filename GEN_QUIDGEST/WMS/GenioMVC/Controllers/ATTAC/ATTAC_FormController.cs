...
namespace GenioMVC.Controllers
{
	public partial class AttacController : ControllerBase
	{
		...
		[AuthorizeForUsers]
		public ActionResult ChangeFormMode_Attac(string mode)
		{
			var _mode = Navigation.CurrentLevel.Location;
			var id = Navigation.GetStrValue("attac");
			...
			switch (mode)
			{
				case "show":
					if (m.Contains("v"))
						_mode = ACTION_ATTAC_SHOW;
					break;
				case "delete":
					if (m.Contains("a"))
						_mode = ACTION_ATTAC_DELETE;
					break;
				case "edit":
					if (m.Contains("e"))
						_mode = ACTION_ATTAC_EDIT;
					break;
				case "duplicate":
					if (m.Contains("d"))
						_mode = ACTION_ATTAC_DUPLICATE;
					break;
				case "new":
					if (m.Contains("i"))
						_mode = ACTION_ATTAC_NEW;
					break;
			}
...
// USE /[MANUAL GQT CONTROLLER_SHOW ATTAC]/
		//
		// GET: /Attac/Attac_Show
		[AuthorizeForUsers]
		public ActionResult Attac_Show(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";
			var model = new Attac_ViewModel(Navigation, nestedForm);
			...
			string partialView = qs["partialView"] ?? "Attac"; // MF send the patial view name

			var navigationLocationAction = ACTION_ATTAC_SHOW.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			...
			if (!isHomePage && (IsNewLocation(navigationLocationAction) || !string.Equals(Navigation.GetStrValue("attac"), id)))
			{
				Navigation.AddHistoryLevel(navigationLocationAction, FormMode.Show, nestedForm);
				CSGenio.framework.Audit.registAction(UserContext.Current.User, Resources.Resources.FORM54242 + " " + Navigation.CurrentLevel.Location.ShortDescription());
			}

			Navigation.SetValue("attac", id);
...
// USE /[MANUAL GQT BEFORE_LOAD_SHOW ATTAC]/
			...
			catch (ModelNotFoundException)
			{
				...
				CSGenio.framework.Log.Error("Attac_Show - GET " + id + " " + e.Message);
				throw;
			}
...
// USE /[MANUAL GQT AFTER_LOAD_SHOW ATTAC]/
			...
			if (isHomePage)
			{
				Navigation.SetValue("HomePage", "Attac");
				return PartialView(partialView, model);
			}
			else if (!Request.IsAjaxRequest())
				return View("Attac", model);
			else
				return PartialView(partialView, model);
		}
		...
		[ActionName("Attac_New_Insert")]
		[AuthorizeForUsers]
		[HttpGet]
		public ActionResult Attac_New_Insert()
		{
			...
			return RedirectToAction("Attac_New");
		}

// USE /[MANUAL GQT CONTROLLER_NEW_GET ATTAC]/
		//
		// GET: /Attac/Attac_New
		[ActionName("Attac_New")]
		[AuthorizeForUsers]
		[HttpGet]
		public ActionResult Attac_New()
		{
			...
			string partialView = qs["partialView"] ?? "Attac";

			var navigationLocationAction = ACTION_ATTAC_NEW.SetRoutedValues(new { m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);

			//FOR: OVERRIDE SKIP IF JUST ONE
			//Allow child form to use "Go Back" to menu list without "skip if only one"
			if (Navigation.OverrideSkipIfJustOne.ContainsKey("ATTAC"))
				Navigation.OverrideSkipIfJustOne["ATTAC"] = true;
...
// USE /[MANUAL GQT AFTER_LOAD_NEW ATTAC]/
			...
			catch (Exception e)
			{
				...
				CSGenio.framework.Log.Error("Attac_New - GET " + e.Message);
...
// USE /[MANUAL GQT CONTROLLER_NEW_POST ATTAC]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Attac_New(Attac_ViewModel model, bool redirect = true)
		{
...
// USE /[MANUAL GQT AFTER_LOAD_NEW_EX ATTAC]/
		...
		[AuthorizeForUsers]
		public ActionResult Attac_New_Redirect(bool internalRedirect = false)
		{
			IList<string> warningMsgs = TempData["NEW_WARNINGS_LIST"] as List<string>;
			string saveMsg = TempData["NEW_SAVE_LIST"] as string;

			if (!internalRedirect)
				SuccessMessage(String.IsNullOrEmpty(saveMsg) ? Resources.Resources.REGISTO_CRIADO_COM_S18746 + GetHumanKeyToQMessage("attac", Convert.ToString(Navigation.CurrentLevel.GetEntry("attac"))) : saveMsg);
			...
			if (Convert.ToBoolean(Request.QueryString.Get("repeatInsertion")))
				return RedirectToLocation(ACTION_ATTAC_NEW, new { repeatInsertion = true });
...
// USE /[MANUAL GQT CONTROLLER_EDIT_GET ATTAC]/
		//
		// GET: /Attac/Attac_Edit
		[AuthorizeForUsers]
		[HttpGet]
		[ActionName("Attac_Edit")]
		public ActionResult Attac_Edit(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";
			string partialView = qs["partialView"] ?? "Attac"; // MF send the partial view name

			//Useful to initialize a record that had to be created by the framework prior to edition (and method New doesnt support this). This way, if the form then is canceled by the user, the record will be automatically deleted
			var isNewEdit = Navigation.GetStrValue("NewEdit") == "true";

			var navigationLocationAction =  isNewEdit ? ACTION_ATTAC_NEW.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] }) : ACTION_ATTAC_EDIT.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);
...
// USE /[MANUAL GQT BEFORE_LOAD_EDIT ATTAC]/
			...
			catch (ModelNotFoundException)
			{
				...
				sp.closeConnection();
				//JFG 05-05-2017 This need to better handled, for now it's handled on webconfig's customErrors section.
				CSGenio.framework.Log.Error("Attac_Edit - " + id + " " + e.Message);
				throw;
			}
...
// USE /[MANUAL GQT CONTROLLER_EDIT_POST ATTAC]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Attac_Edit(Attac_ViewModel model, bool redirect = true)
		{
...
// USE /[MANUAL GQT AFTER_LOAD_EDIT_EX ATTAC]/
		...
		[AuthorizeForUsers]
		public ActionResult Attac_Edit_Redirect(bool internalRedirect = false)
		{
			IList<string> warningMsgs = TempData["EDIT_WARNINGS_LIST"] as List<string>;
			string saveMsg = TempData["NEW_SAVE_LIST"] as string;

			if (!internalRedirect || !Request.IsAjaxRequest())
				SuccessMessage(String.IsNullOrEmpty(saveMsg) ? Resources.Resources.ALTERACOES_EFECTUADA64514 + GetHumanKeyToQMessage("attac", Convert.ToString(Navigation.CurrentLevel.GetEntry("attac"))) : saveMsg);
...
// USE /[MANUAL GQT CONTROLLER_DELETE_GET ATTAC]/
		//
		// GET: /Attac/Attac_Delete
		[AuthorizeForUsers]
		[HttpGet]
		[ActionName("Attac_Delete")]
		public ActionResult Attac_Delete(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";

			var navigationLocationAction = ACTION_ATTAC_DELETE.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);
...
// USE /[MANUAL GQT BEFORE_LOAD_DELETE ATTAC]/
			...
			catch (ModelNotFoundException)
			{
				...
				//JFG 05-05-2017 This need to better handled, for now it's handled on webconfig's customErrors section.
				CSGenio.framework.Log.Error("Attac_Delete - " + id + " " + e.Message);
				throw;
			}
...
// USE /[MANUAL GQT AFTER_LOAD_DELETE ATTAC]/
			...
			if (!Request.IsAjaxRequest())
				...
				if (permission.Status.Equals(CSGenio.framework.Status.E))
					...
					return PartialView("Attac", model);
		}
...
// USE /[MANUAL GQT CONTROLLER_DELETE_POST ATTAC]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Attac_Delete(string id, FormCollection collection)
		{

			var model = new Attac_ViewModel (Navigation, id);
			model.MapFromModel();
...
// USE /[MANUAL GQT CONTROLLER_DUPLICATE_GET ATTAC]/
		//
		// GET: /Attac/Attac_Duplicate
		[AuthorizeForUsers]
		[HttpGet]
		public ActionResult Attac_Duplicate(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";
			var model = new Attac_ViewModel(Navigation, nestedForm);
			...
			var navigationLocationAction = ACTION_ATTAC_DUPLICATE.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);
...
// USE /[MANUAL GQT AFTER_LOAD_DUPLICATE ATTAC]/
					...
					sp.closeTransaction();
					Navigation.CurrentLevel.SetLocation(ACTION_ATTAC_DUPLICATE.SetRoutedValues(new { Id = model.ValCodattac }));
...
// USE /[MANUAL GQT CONTROLLER_DUPLICATE_POST ATTAC]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Attac_Duplicate(Attac_ViewModel model, bool redirect = true)
		{
			var eventSink = new EventSink()
			{
...
// USE /[MANUAL GQT AFTER_LOAD_DUPLICATE_EX ATTAC]/
		...
		[AuthorizeForUsers]
		public ActionResult Attac_Duplicate_Redirect(bool internalRedirect = false)
		{
...
// USE /[MANUAL GQT CONTROLLER_CANCEL_GET ATTAC]/
		...
		public ActionResult Attac_Cancel()
		{
			...
			if (Navigation.CurrentLevel.FormMode == FormMode.New || Navigation.CurrentLevel.FormMode == FormMode.Duplicate)
			{
				...
				try
				{
					var model = new GenioMVC.Models.Attac();
...
// USE /[MANUAL GQT AFTER_CANCEL ATTAC]/
			...
			else if (Navigation.PreviousLevel != null)
			{
				...
				string previousLevelArea = Navigation.PreviousLevel.Location.Controller?.ToLower() ?? "attac";
		...
		{
			var model = new Attac_ViewModel(Navigation, true);
			model.setModes(Request.QueryString["m"]);
			PersistentSupport sp = UserContext.Current.PersistentSupport;
			var navigationLocationAction = ACTION_ATTAC_NEW.SetRoutedValues(new { m = Request.QueryString["m"] });
			CheckLevels(navigationLocationAction);
			...
			return PartialView("MFAttac", model);
		}
		...
		public ActionResult MFAttac_Edit(string id)
		{
			return this.RedirectToAction("Attac_Edit", "Attac", new { id = id, partialView = "MFAttac", nestedForm = "true", multiForm = "true" });
		}
		...
		public ActionResult MFAttac_Cancel(string id)
		{
			//MH - Validate if current level is it correct. Remove all levels that is higher than the currently level.
			CheckLevels(NavigationLocation.Any);

			if (Navigation.CurrentLevel.IsNestedContext &&
				(Navigation.CurrentLevel.Location.Action == ACTION_ATTAC_NEW.Action || Navigation.CurrentLevel.Location.Action == ACTION_ATTAC_EDIT.Action))
			{
				if (Navigation.CurrentLevel.FormMode == FormMode.New)
				{
					...
					catch (Exception e)
					{
						...
						Log.Error("MFAttac_Cancel - " + exceptionUserMessage);
						ErrorMessage(exceptionUserMessage);
					}
				}
			...
			return this.RedirectToAction("Attac_Show", "Attac", new { id = id, partialView = "MFAttac", nestedForm = "true", multiForm = "true", nav });
		}
		...
		public JsonResult MFAttac_Delete(string id)
		{
			var eventSink = new EventSink()
			{
		...
		public ActionResult Attac_AssetValName(string id, string partialView,  IDictionary<string, string> Limits)
		{
			...
			Attac_AssetValName_ViewModel model = new Attac_AssetValName_ViewModel(navigation);
			model.setModes(Request.QueryString["m"]);
			model.ValCodattac = id;
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Attac_SaveEdit(Attac_ViewModel model)
		{
			var eventSink = new EventSink()
			{
