...
// USE /[MANUAL GQT MANUAL_CONTROLLER CNTRY]/
        ...
        public JsonResult RecalculateFormulas_Pais(Pais_ViewModel form_data)
        {
            return GenericRecalculateFormulas(form_data, "cntry",
                (primaryKey) => Models.Cntry.Find(primaryKey, "FPAIS"),
		...
		[AuthorizeForUsers]
        [ActionSessionState(System.Web.SessionState.SessionStateBehavior.ReadOnly)]
        public JsonResult RecalculateFormulas_Proppais(Proppais_ViewModel form_data)
        {
            return GenericRecalculateFormulas(form_data, "cntry",
                (primaryKey) => Models.Cntry.Find(primaryKey, "FPROPPAIS"),
                (model) => form_data.MapToModel(model as Models.Cntry)
            );
        }
