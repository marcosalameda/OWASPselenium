...
// USE /[MANUAL GQT MANUAL_CONTROLLER DILIN]/
        ...
        public JsonResult ReloadDBEdit(string Identifier, IDictionary<string, object> Values)
        {
            ...
            try { row = Models.Dilin.Find(navigation.GetStrValue("dilin")); }
            catch (Exception)
            {
					...
					case "DILIN___DISPADISPANR_":	// Field (DB)
                        {
						    ...
						    var model = new Dilin_ViewModel(navigation) { editable = false };
        ...
        public JsonResult GetDependants(string Identifier, string Selected, IDictionary<string, object> Limits)
        {
            ...
            try
            {
				...
				{
					case "DILIN___DISPADISPANR_":	// Field (DB)
						values = Dilin_ViewModel.GetDependant_DilinTableDispaDispanr(Selected, navigation);
						break;
					case "DILIN___PRODUPRODUCT_":	// Field (DB)
						values = Dilin_ViewModel.GetDependant_DilinTableProduProduct(Selected, navigation);
						break;
					default: break;
				}
		...
		[AuthorizeForUsers]
        [ActionSessionState(System.Web.SessionState.SessionStateBehavior.ReadOnly)]
        public JsonResult RecalculateFormulas_Dilin(Dilin_ViewModel form_data)
        {
            return GenericRecalculateFormulas(form_data, "dilin",
                (primaryKey) => Models.Dilin.Find(primaryKey, "FDILIN"),
