...
namespace GenioMVC.Controllers
{
	public partial class DilinController : ControllerBase
	{
		...
		[AuthorizeForUsers]
		public ActionResult ChangeFormMode_Dilin(string mode)
		{
			var _mode = Navigation.CurrentLevel.Location;
			var id = Navigation.GetStrValue("dilin");
			...
			switch (mode)
			{
				case "show":
					if (m.Contains("v"))
						_mode = ACTION_DILIN_SHOW;
					break;
				case "delete":
					if (m.Contains("a"))
						_mode = ACTION_DILIN_DELETE;
					break;
				case "edit":
					if (m.Contains("e"))
						_mode = ACTION_DILIN_EDIT;
					break;
				case "duplicate":
					if (m.Contains("d"))
						_mode = ACTION_DILIN_DUPLICATE;
					break;
				case "new":
					if (m.Contains("i"))
						_mode = ACTION_DILIN_NEW;
					break;
			}
...
// USE /[MANUAL GQT CONTROLLER_SHOW DILIN]/
		//
		// GET: /Dilin/Dilin_Show
		[AuthorizeForUsers]
		public ActionResult Dilin_Show(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";
			var model = new Dilin_ViewModel(Navigation, nestedForm);
			...
			string partialView = qs["partialView"] ?? "Dilin"; // MF send the patial view name

			var navigationLocationAction = ACTION_DILIN_SHOW.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			...
			if (!isHomePage && (IsNewLocation(navigationLocationAction) || !string.Equals(Navigation.GetStrValue("dilin"), id)))
			{
				Navigation.AddHistoryLevel(navigationLocationAction, FormMode.Show, nestedForm);
				CSGenio.framework.Audit.registAction(UserContext.Current.User, Resources.Resources.FORM54242 + " " + Navigation.CurrentLevel.Location.ShortDescription());
			}

			Navigation.SetValue("dilin", id);
...
// USE /[MANUAL GQT BEFORE_LOAD_SHOW DILIN]/
			...
			catch (ModelNotFoundException)
			{
				...
				CSGenio.framework.Log.Error("Dilin_Show - GET " + id + " " + e.Message);
				throw;
			}
...
// USE /[MANUAL GQT AFTER_LOAD_SHOW DILIN]/
			...
			if (isHomePage)
			{
				Navigation.SetValue("HomePage", "Dilin");
				return PartialView(partialView, model);
			}
			else if (!Request.IsAjaxRequest())
				return View("Dilin", model);
			else
				return PartialView(partialView, model);
		}
		...
		[ActionName("Dilin_New_Insert")]
		[AuthorizeForUsers]
		[HttpGet]
		public ActionResult Dilin_New_Insert()
		{
			...
			return RedirectToAction("Dilin_New");
		}

// USE /[MANUAL GQT CONTROLLER_NEW_GET DILIN]/
		//
		// GET: /Dilin/Dilin_New
		[ActionName("Dilin_New")]
		[AuthorizeForUsers]
		[HttpGet]
		public ActionResult Dilin_New()
		{
			...
			string partialView = qs["partialView"] ?? "Dilin";

			var navigationLocationAction = ACTION_DILIN_NEW.SetRoutedValues(new { m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);

			//FOR: OVERRIDE SKIP IF JUST ONE
			//Allow child form to use "Go Back" to menu list without "skip if only one"
			if (Navigation.OverrideSkipIfJustOne.ContainsKey("DILIN"))
				Navigation.OverrideSkipIfJustOne["DILIN"] = true;
...
// USE /[MANUAL GQT AFTER_LOAD_NEW DILIN]/
			...
			catch (Exception e)
			{
				...
				CSGenio.framework.Log.Error("Dilin_New - GET " + e.Message);
...
// USE /[MANUAL GQT CONTROLLER_NEW_POST DILIN]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Dilin_New(Dilin_ViewModel model, bool redirect = true)
		{
...
// USE /[MANUAL GQT AFTER_LOAD_NEW_EX DILIN]/
		...
		[AuthorizeForUsers]
		public ActionResult Dilin_New_Redirect(bool internalRedirect = false)
		{
			IList<string> warningMsgs = TempData["NEW_WARNINGS_LIST"] as List<string>;
			string saveMsg = TempData["NEW_SAVE_LIST"] as string;

			if (!internalRedirect)
				SuccessMessage(String.IsNullOrEmpty(saveMsg) ? Resources.Resources.REGISTO_CRIADO_COM_S18746 + GetHumanKeyToQMessage("dilin", Convert.ToString(Navigation.CurrentLevel.GetEntry("dilin"))) : saveMsg);
			...
			if (Convert.ToBoolean(Request.QueryString.Get("repeatInsertion")))
				return RedirectToLocation(ACTION_DILIN_NEW, new { repeatInsertion = true });
...
// USE /[MANUAL GQT CONTROLLER_EDIT_GET DILIN]/
		//
		// GET: /Dilin/Dilin_Edit
		[AuthorizeForUsers]
		[HttpGet]
		[ActionName("Dilin_Edit")]
		public ActionResult Dilin_Edit(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";
			string partialView = qs["partialView"] ?? "Dilin"; // MF send the partial view name

			//Useful to initialize a record that had to be created by the framework prior to edition (and method New doesnt support this). This way, if the form then is canceled by the user, the record will be automatically deleted
			var isNewEdit = Navigation.GetStrValue("NewEdit") == "true";

			var navigationLocationAction =  isNewEdit ? ACTION_DILIN_NEW.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] }) : ACTION_DILIN_EDIT.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);
...
// USE /[MANUAL GQT BEFORE_LOAD_EDIT DILIN]/
			...
			catch (ModelNotFoundException)
			{
				...
				sp.closeConnection();
				//JFG 05-05-2017 This need to better handled, for now it's handled on webconfig's customErrors section.
				CSGenio.framework.Log.Error("Dilin_Edit - " + id + " " + e.Message);
				throw;
			}
...
// USE /[MANUAL GQT CONTROLLER_EDIT_POST DILIN]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Dilin_Edit(Dilin_ViewModel model, bool redirect = true)
		{
...
// USE /[MANUAL GQT AFTER_LOAD_EDIT_EX DILIN]/
		...
		[AuthorizeForUsers]
		public ActionResult Dilin_Edit_Redirect(bool internalRedirect = false)
		{
			IList<string> warningMsgs = TempData["EDIT_WARNINGS_LIST"] as List<string>;
			string saveMsg = TempData["NEW_SAVE_LIST"] as string;

			if (!internalRedirect || !Request.IsAjaxRequest())
				SuccessMessage(String.IsNullOrEmpty(saveMsg) ? Resources.Resources.ALTERACOES_EFECTUADA64514 + GetHumanKeyToQMessage("dilin", Convert.ToString(Navigation.CurrentLevel.GetEntry("dilin"))) : saveMsg);
...
// USE /[MANUAL GQT CONTROLLER_DELETE_GET DILIN]/
		//
		// GET: /Dilin/Dilin_Delete
		[AuthorizeForUsers]
		[HttpGet]
		[ActionName("Dilin_Delete")]
		public ActionResult Dilin_Delete(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";

			var navigationLocationAction = ACTION_DILIN_DELETE.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);
...
// USE /[MANUAL GQT BEFORE_LOAD_DELETE DILIN]/
			...
			catch (ModelNotFoundException)
			{
				...
				//JFG 05-05-2017 This need to better handled, for now it's handled on webconfig's customErrors section.
				CSGenio.framework.Log.Error("Dilin_Delete - " + id + " " + e.Message);
				throw;
			}
...
// USE /[MANUAL GQT AFTER_LOAD_DELETE DILIN]/
			...
			if (!Request.IsAjaxRequest())
				...
				if (permission.Status.Equals(CSGenio.framework.Status.E))
					...
					return PartialView("Dilin", model);
		}
...
// USE /[MANUAL GQT CONTROLLER_DELETE_POST DILIN]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Dilin_Delete(string id, FormCollection collection)
		{

			var model = new Dilin_ViewModel (Navigation, id);
			model.MapFromModel();
...
// USE /[MANUAL GQT CONTROLLER_DUPLICATE_GET DILIN]/
		//
		// GET: /Dilin/Dilin_Duplicate
		[AuthorizeForUsers]
		[HttpGet]
		public ActionResult Dilin_Duplicate(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";
			var model = new Dilin_ViewModel(Navigation, nestedForm);
			...
			var navigationLocationAction = ACTION_DILIN_DUPLICATE.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);
...
// USE /[MANUAL GQT AFTER_LOAD_DUPLICATE DILIN]/
					...
					sp.closeTransaction();
					Navigation.CurrentLevel.SetLocation(ACTION_DILIN_DUPLICATE.SetRoutedValues(new { Id = model.ValCoddilin }));
...
// USE /[MANUAL GQT CONTROLLER_DUPLICATE_POST DILIN]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Dilin_Duplicate(Dilin_ViewModel model, bool redirect = true)
		{
			var eventSink = new EventSink()
			{
...
// USE /[MANUAL GQT AFTER_LOAD_DUPLICATE_EX DILIN]/
		...
		[AuthorizeForUsers]
		public ActionResult Dilin_Duplicate_Redirect(bool internalRedirect = false)
		{
...
// USE /[MANUAL GQT CONTROLLER_CANCEL_GET DILIN]/
		...
		public ActionResult Dilin_Cancel()
		{
			...
			if (Navigation.CurrentLevel.FormMode == FormMode.New || Navigation.CurrentLevel.FormMode == FormMode.Duplicate)
			{
				...
				try
				{
					var model = new GenioMVC.Models.Dilin();
...
// USE /[MANUAL GQT AFTER_CANCEL DILIN]/
			...
			else if (Navigation.PreviousLevel != null)
			{
				...
				string previousLevelArea = Navigation.PreviousLevel.Location.Controller?.ToLower() ?? "dilin";
		...
		{
			var model = new Dilin_ViewModel(Navigation, true);
			model.setModes(Request.QueryString["m"]);
			PersistentSupport sp = UserContext.Current.PersistentSupport;
			var navigationLocationAction = ACTION_DILIN_NEW.SetRoutedValues(new { m = Request.QueryString["m"] });
			CheckLevels(navigationLocationAction);
			...
			return PartialView("MFDilin", model);
		}
		...
		public ActionResult MFDilin_Edit(string id)
		{
			return this.RedirectToAction("Dilin_Edit", "Dilin", new { id = id, partialView = "MFDilin", nestedForm = "true", multiForm = "true" });
		}
		...
		public ActionResult MFDilin_Cancel(string id)
		{
			//MH - Validate if current level is it correct. Remove all levels that is higher than the currently level.
			CheckLevels(NavigationLocation.Any);

			if (Navigation.CurrentLevel.IsNestedContext &&
				(Navigation.CurrentLevel.Location.Action == ACTION_DILIN_NEW.Action || Navigation.CurrentLevel.Location.Action == ACTION_DILIN_EDIT.Action))
			{
				if (Navigation.CurrentLevel.FormMode == FormMode.New)
				{
					...
					catch (Exception e)
					{
						...
						Log.Error("MFDilin_Cancel - " + exceptionUserMessage);
						ErrorMessage(exceptionUserMessage);
					}
				}
			...
			return this.RedirectToAction("Dilin_Show", "Dilin", new { id = id, partialView = "MFDilin", nestedForm = "true", multiForm = "true", nav });
		}
		...
		public JsonResult MFDilin_Delete(string id)
		{
			var eventSink = new EventSink()
			{
		...
		public ActionResult Dilin_DispaValDispanr(string id, string partialView,  IDictionary<string, string> Limits)
		{
			...
			Dilin_DispaValDispanr_ViewModel model = new Dilin_DispaValDispanr_ViewModel(navigation);
			model.setModes(Request.QueryString["m"]);
			model.ValCoddilin = id;
		...
		[ActionSessionState(System.Web.SessionState.SessionStateBehavior.ReadOnly)]
		public ActionResult Dilin_ProduValProduct(string id, string partialView,  IDictionary<string, string> Limits)
		{
			int perPage = CSGenio.framework.Configuration.NrRegDBedit;
			...
			NameValueCollection requestValues = Request.Unvalidated.Form.Count > 0 ? Request.Unvalidated.Form : Request.QueryString; //TSX (01.07.2020) Can not access directly to the FormCollection or made Request.Form otherwise the tags on viewmodel will be ignored
			var navigation = Navigation.Clone();
			Dilin_ProduValProduct_ViewModel model = new Dilin_ProduValProduct_ViewModel(navigation);
			model.setModes(Request.QueryString["m"]);
			model.ValCoddilin = id;
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Dilin_SaveEdit(Dilin_ViewModel model)
		{
			var eventSink = new EventSink()
			{
