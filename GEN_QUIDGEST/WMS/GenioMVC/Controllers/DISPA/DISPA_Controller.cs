...
// USE /[MANUAL GQT CONTROLLER_NAVIGATION DISPA]/
        ...
        {
            CSGenio.business.Area area = CSGenio.business.Area.createArea<CSGenioAdispa>(UserContext.Current.User, UserContext.Current.User.CurrentModule);
...
// USE /[MANUAL GQT MANUAL_CONTROLLER DISPA]/
        ...
        public JsonResult ReloadDBEdit(string Identifier, IDictionary<string, object> Values)
        {
            ...
            try { row = Models.Dispa.Find(navigation.GetStrValue("dispa")); }
            catch (Exception)
            {
                CSGenio.framework.Log.Error("ReloadDBEdit - " + Identifier + " Not found Model dispa");
            }
            if(row == null)
            {
                row = new Models.Dispa();
					...
					case "DISPA___ENTITNAME____":	// Field (DB)
                        {
						    ...
						    var model = new Dispa_ViewModel(navigation) { editable = false };
        ...
        public JsonResult GetDependants(string Identifier, string Selected, IDictionary<string, object> Limits)
        {
            ...
            try
            {
				...
				{
					case "DISPA___ENTITNAME____":	// Field (DB)
						values = Dispa_ViewModel.GetDependant_DispaTableEntitName(Selected, navigation);
						break;
					case "DISPA___PERSONAME____":	// Field (DB)
						values = Dispa_ViewModel.GetDependant_DispaTablePersoName(Selected, navigation);
						break;
					default: break;
				}
        ...
        /// <summary>
        /// Recalculate formulas of the "Dispa" form. (++, CT, SR, CL and U1)
        /// </summary>
		...
		[AuthorizeForUsers]
        [ActionSessionState(System.Web.SessionState.SessionStateBehavior.ReadOnly)]
        public JsonResult RecalculateFormulas_Dispa(Dispa_ViewModel form_data)
        {
            return GenericRecalculateFormulas(form_data, "dispa",
                (primaryKey) => Models.Dispa.Find(primaryKey, "FDISPA"),
                (model) => form_data.MapToModel(model as Models.Dispa)
            );
        }
