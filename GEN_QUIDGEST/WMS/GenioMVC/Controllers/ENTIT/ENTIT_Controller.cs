...
// USE /[MANUAL GQT CONTROLLER_NAVIGATION ENTIT]/
        ...
        {
            CSGenio.business.Area area = CSGenio.business.Area.createArea<CSGenioAentit>(UserContext.Current.User, UserContext.Current.User.CurrentModule);
...
// USE /[MANUAL GQT MANUAL_CONTROLLER ENTIT]/
        ...
        public JsonResult ReloadDBEdit(string Identifier, IDictionary<string, object> Values)
        {
            ...
            try { row = Models.Entit.Find(navigation.GetStrValue("entit")); }
            catch (Exception)
            {
			...
			try
			{
				switch ((string.IsNullOrEmpty(Identifier) || Identifier.Length < 5) ? "" : Identifier.Substring(4)) // Substring(4) => to retirar o IFF_ e LED_
				{
					case "ENTIT___FACI1NAME____":	// Field (DB)
                        {
						    ...
						    var model = new Entit_ViewModel(navigation) { editable = false };
					...
					case "ENTIX___FACI1NAME____":	// Field (F1)
                        {
                            row.LoadKeysFormHistory(navigation, navigation.CurrentLevel.Level, false, true, true, true);
						    var model = new Entix_ViewModel(navigation) { editable = false };
						    model.MapFromModel(row);
                            TryUpdateModel(model); // Map recived values to fields - The 'field' type limits
        ...
        public JsonResult GetDependants(string Identifier, string Selected, IDictionary<string, object> Limits)
        {
            ...
            try
            {
				...
				{
					case "ENTIT___FACI1NAME____":	// Field (DB)
						values = Entit_ViewModel.GetDependant_EntitTableFaci1Name(Selected, navigation);
						break;
					case "ENTIT___FACI2NAME____":	// Field (DB)
						values = Entit_ViewModel.GetDependant_EntitTableFaci2Name(Selected, navigation);
						break;
					case "ENTIX___FACI1NAME____":	// Field (F1)
						values = Entix_ViewModel.GetDependant_EntixTableFaci1Name(Selected, navigation);
						break;
					case "ENTIX___FACI2NAME____":	// Field (F1)
						values = Entix_ViewModel.GetDependant_EntixTableFaci2Name(Selected, navigation);
						break;
					default: break;
				}
		...
		[AuthorizeForUsers]
        [ActionSessionState(System.Web.SessionState.SessionStateBehavior.ReadOnly)]
        public JsonResult RecalculateFormulas_Entit(Entit_ViewModel form_data)
        {
            return GenericRecalculateFormulas(form_data, "entit",
                (primaryKey) => Models.Entit.Find(primaryKey, "FENTIT"),
		...
		[AuthorizeForUsers]
        [ActionSessionState(System.Web.SessionState.SessionStateBehavior.ReadOnly)]
        public JsonResult RecalculateFormulas_Entix(Entix_ViewModel form_data)
        {
            return GenericRecalculateFormulas(form_data, "entit",
                (primaryKey) => Models.Entit.Find(primaryKey, "FENTIX"),
                (model) => form_data.MapToModel(model as Models.Entit)
            );
        }
