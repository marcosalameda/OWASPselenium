...
namespace GenioMVC.Controllers
{
	public partial class EntitController : ControllerBase
	{
		...
		[AuthorizeForUsers]
		public ActionResult ChangeFormMode_Entit(string mode)
		{
			...
			switch (mode)
			{
				case "show":
					if (m.Contains("v"))
						_mode = ACTION_ENTIT_SHOW;
					break;
				case "delete":
					if (m.Contains("a"))
						_mode = ACTION_ENTIT_DELETE;
					break;
				case "edit":
					if (m.Contains("e"))
						_mode = ACTION_ENTIT_EDIT;
					break;
				case "duplicate":
					if (m.Contains("d"))
						_mode = ACTION_ENTIT_DUPLICATE;
					break;
				case "new":
					if (m.Contains("i"))
						_mode = ACTION_ENTIT_NEW;
					break;
			}
...
// USE /[MANUAL GQT CONTROLLER_SHOW ENTIT]/
		//
		// GET: /Entit/Entit_Show
		[AuthorizeForUsers]
		public ActionResult Entit_Show(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";
			var model = new Entit_ViewModel(Navigation, nestedForm);
			...
			string partialView = qs["partialView"] ?? "Entit"; // MF send the patial view name

			var navigationLocationAction = ACTION_ENTIT_SHOW.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
...
// USE /[MANUAL GQT BEFORE_LOAD_SHOW ENTIT]/
			...
			catch (ModelNotFoundException)
			{
				...
				CSGenio.framework.Log.Error("Entit_Show - GET " + id + " " + e.Message);
				throw;
			}
...
// USE /[MANUAL GQT AFTER_LOAD_SHOW ENTIT]/
			...
			if (isHomePage)
			{
				Navigation.SetValue("HomePage", "Entit");
				return PartialView(partialView, model);
			}
			else if (!Request.IsAjaxRequest())
				return View("Entit", model);
			else
				return PartialView(partialView, model);
		}
		...
		[ActionName("Entit_New_Insert")]
		[AuthorizeForUsers]
		[HttpGet]
		public ActionResult Entit_New_Insert()
		{
			...
			return RedirectToAction("Entit_New");
		}

// USE /[MANUAL GQT CONTROLLER_NEW_GET ENTIT]/
		//
		// GET: /Entit/Entit_New
		[ActionName("Entit_New")]
		[AuthorizeForUsers]
		[HttpGet]
		public ActionResult Entit_New()
		{
			...
			string partialView = qs["partialView"] ?? "Entit";

			var navigationLocationAction = ACTION_ENTIT_NEW.SetRoutedValues(new { m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);

			//FOR: OVERRIDE SKIP IF JUST ONE
			//Allow child form to use "Go Back" to menu list without "skip if only one"
			if (Navigation.OverrideSkipIfJustOne.ContainsKey("ENTIT"))
				Navigation.OverrideSkipIfJustOne["ENTIT"] = true;
...
// USE /[MANUAL GQT AFTER_LOAD_NEW ENTIT]/
			...
			catch (Exception e)
			{
				...
				CSGenio.framework.Log.Error("Entit_New - GET " + e.Message);
...
// USE /[MANUAL GQT CONTROLLER_NEW_POST ENTIT]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Entit_New(Entit_ViewModel model, bool redirect = true)
		{
...
// USE /[MANUAL GQT AFTER_LOAD_NEW_EX ENTIT]/
		...
		[AuthorizeForUsers]
		public ActionResult Entit_New_Redirect(bool internalRedirect = false)
		{
			...
			if (Convert.ToBoolean(Request.QueryString.Get("repeatInsertion")))
				return RedirectToLocation(ACTION_ENTIT_NEW, new { repeatInsertion = true });
...
// USE /[MANUAL GQT CONTROLLER_EDIT_GET ENTIT]/
		//
		// GET: /Entit/Entit_Edit
		[AuthorizeForUsers]
		[HttpGet]
		[ActionName("Entit_Edit")]
		public ActionResult Entit_Edit(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";
			string partialView = qs["partialView"] ?? "Entit"; // MF send the partial view name

			//Useful to initialize a record that had to be created by the framework prior to edition (and method New doesnt support this). This way, if the form then is canceled by the user, the record will be automatically deleted
			var isNewEdit = Navigation.GetStrValue("NewEdit") == "true";

			var navigationLocationAction =  isNewEdit ? ACTION_ENTIT_NEW.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] }) : ACTION_ENTIT_EDIT.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);
...
// USE /[MANUAL GQT BEFORE_LOAD_EDIT ENTIT]/
			...
			catch (ModelNotFoundException)
			{
				...
				sp.closeConnection();
				//JFG 05-05-2017 This need to better handled, for now it's handled on webconfig's customErrors section.
				CSGenio.framework.Log.Error("Entit_Edit - " + id + " " + e.Message);
				throw;
			}
...
// USE /[MANUAL GQT CONTROLLER_EDIT_POST ENTIT]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Entit_Edit(Entit_ViewModel model, bool redirect = true)
		{
...
// USE /[MANUAL GQT AFTER_LOAD_EDIT_EX ENTIT]/
		...
		[AuthorizeForUsers]
		public ActionResult Entit_Edit_Redirect(bool internalRedirect = false)
		{
			IList<string> warningMsgs = TempData["EDIT_WARNINGS_LIST"] as List<string>;
			string saveMsg = TempData["NEW_SAVE_LIST"] as string;

			if (!internalRedirect || !Request.IsAjaxRequest())
				SuccessMessage(String.IsNullOrEmpty(saveMsg) ? Resources.Resources.ALTERACOES_EFETUADAS10166 + GetHumanKeyToQMessage("entit", Convert.ToString(Navigation.CurrentLevel.GetEntry("entit"))) : saveMsg);
...
// USE /[MANUAL GQT CONTROLLER_DELETE_GET ENTIT]/
		//
		// GET: /Entit/Entit_Delete
		[AuthorizeForUsers]
		[HttpGet]
		[ActionName("Entit_Delete")]
		public ActionResult Entit_Delete(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";

			var navigationLocationAction = ACTION_ENTIT_DELETE.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);
...
// USE /[MANUAL GQT BEFORE_LOAD_DELETE ENTIT]/
			...
			catch (ModelNotFoundException)
			{
				...
				//JFG 05-05-2017 This need to better handled, for now it's handled on webconfig's customErrors section.
				CSGenio.framework.Log.Error("Entit_Delete - " + id + " " + e.Message);
				throw;
			}
...
// USE /[MANUAL GQT AFTER_LOAD_DELETE ENTIT]/
			...
			if (!Request.IsAjaxRequest())
				...
				if (permission.Status.Equals(CSGenio.framework.Status.E))
					...
					return PartialView("Entit", model);
		}
...
// USE /[MANUAL GQT CONTROLLER_DELETE_POST ENTIT]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Entit_Delete(string id, FormCollection collection)
		{

			var model = new Entit_ViewModel (Navigation, id);
			model.MapFromModel();
...
// USE /[MANUAL GQT CONTROLLER_DUPLICATE_GET ENTIT]/
		//
		// GET: /Entit/Entit_Duplicate
		[AuthorizeForUsers]
		[HttpGet]
		public ActionResult Entit_Duplicate(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";
			var model = new Entit_ViewModel(Navigation, nestedForm);
			...
			var navigationLocationAction = ACTION_ENTIT_DUPLICATE.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);
...
// USE /[MANUAL GQT AFTER_LOAD_DUPLICATE ENTIT]/
					...
					sp.closeTransaction();
					Navigation.CurrentLevel.SetLocation(ACTION_ENTIT_DUPLICATE.SetRoutedValues(new { Id = model.ValCodentit }));
...
// USE /[MANUAL GQT CONTROLLER_DUPLICATE_POST ENTIT]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Entit_Duplicate(Entit_ViewModel model, bool redirect = true)
		{
			var eventSink = new EventSink()
			{
...
// USE /[MANUAL GQT AFTER_LOAD_DUPLICATE_EX ENTIT]/
		...
		[AuthorizeForUsers]
		public ActionResult Entit_Duplicate_Redirect(bool internalRedirect = false)
		{
...
// USE /[MANUAL GQT AFTER_CANCEL ENTIT]/
		...
		{
			var model = new Entit_ViewModel(Navigation, true);
			model.setModes(Request.QueryString["m"]);
			PersistentSupport sp = UserContext.Current.PersistentSupport;
			var navigationLocationAction = ACTION_ENTIT_NEW.SetRoutedValues(new { m = Request.QueryString["m"] });
			CheckLevels(navigationLocationAction);
			...
			return PartialView("MFEntit", model);
		}
		...
		public ActionResult MFEntit_Edit(string id)
		{
			return this.RedirectToAction("Entit_Edit", "Entit", new { id = id, partialView = "MFEntit", nestedForm = "true", multiForm = "true" });
		}
		...
		public ActionResult MFEntit_Cancel(string id)
		{
			//MH - Validate if current level is it correct. Remove all levels that is higher than the currently level.
			CheckLevels(NavigationLocation.Any);

			if (Navigation.CurrentLevel.IsNestedContext &&
				(Navigation.CurrentLevel.Location.Action == ACTION_ENTIT_NEW.Action || Navigation.CurrentLevel.Location.Action == ACTION_ENTIT_EDIT.Action))
			{
				if (Navigation.CurrentLevel.FormMode == FormMode.New)
				{
					...
					catch (Exception e)
					{
						...
						Log.Error("MFEntit_Cancel - " + exceptionUserMessage);
						ErrorMessage(exceptionUserMessage);
					}
				}
			...
			return this.RedirectToAction("Entit_Show", "Entit", new { id = id, partialView = "MFEntit", nestedForm = "true", multiForm = "true", nav });
		}
		...
		public JsonResult MFEntit_Delete(string id)
		{
			var eventSink = new EventSink()
			{
		...
		public ActionResult Entit_Faci1ValName(string id, string partialView,  IDictionary<string, string> Limits)
		{
			...
			Entit_Faci1ValName_ViewModel model = new Entit_Faci1ValName_ViewModel(navigation);
			model.setModes(Request.QueryString["m"]);
		...
		[ActionSessionState(System.Web.SessionState.SessionStateBehavior.ReadOnly)]
		public ActionResult Entit_Faci2ValName(string id, string partialView,  IDictionary<string, string> Limits)
		{
			int perPage = CSGenio.framework.Configuration.NrRegDBedit;
			...
			NameValueCollection requestValues = Request.Unvalidated.Form.Count > 0 ? Request.Unvalidated.Form : Request.QueryString; //TSX (01.07.2020) Can not access directly to the FormCollection or made Request.Form otherwise the tags on viewmodel will be ignored
			var navigation = Navigation.Clone();
			Entit_Faci2ValName_ViewModel model = new Entit_Faci2ValName_ViewModel(navigation);
			model.setModes(Request.QueryString["m"]);
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Entit_SaveEdit(Entit_ViewModel model)
		{
			var eventSink = new EventSink()
			{
