...
namespace GenioMVC.Controllers
{
	public partial class FacilController : ControllerBase
	{
		...
		[AuthorizeForUsers]
		public ActionResult ChangeFormMode_Facilfex(string mode)
		{
			...
			switch (mode)
			{
				case "show":
					if (m.Contains("v"))
						_mode = ACTION_FACILFEX_SHOW;
					break;
				case "delete":
					if (m.Contains("a"))
						_mode = ACTION_FACILFEX_DELETE;
					break;
				case "edit":
					if (m.Contains("e"))
						_mode = ACTION_FACILFEX_EDIT;
					break;
				case "duplicate":
					if (m.Contains("d"))
						_mode = ACTION_FACILFEX_DUPLICATE;
					break;
				case "new":
					if (m.Contains("i"))
						_mode = ACTION_FACILFEX_NEW;
					break;
			}
...
// USE /[MANUAL GQT CONTROLLER_SHOW FACILFEX]/
		//
		// GET: /Facil/Facilfex_Show
		[AuthorizeForUsers]
		public ActionResult Facilfex_Show(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";
			var model = new Facilfex_ViewModel(Navigation, nestedForm);
			...
			string partialView = qs["partialView"] ?? "Facilfex"; // MF send the patial view name

			var navigationLocationAction = ACTION_FACILFEX_SHOW.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
...
// USE /[MANUAL GQT BEFORE_LOAD_SHOW FACILFEX]/
			...
			catch (ModelNotFoundException)
			{
				...
				CSGenio.framework.Log.Error("Facilfex_Show - GET " + id + " " + e.Message);
				throw;
			}
...
// USE /[MANUAL GQT AFTER_LOAD_SHOW FACILFEX]/
			...
			if (isHomePage)
			{
				Navigation.SetValue("HomePage", "Facilfex");
				return PartialView(partialView, model);
			}
			else if (!Request.IsAjaxRequest())
				return View("Facilfex", model);
			else
				return PartialView(partialView, model);
		}
		...
		[ActionName("Facilfex_New_Insert")]
		[AuthorizeForUsers]
		[HttpGet]
		public ActionResult Facilfex_New_Insert()
		{
			...
			return RedirectToAction("Facilfex_New");
		}

// USE /[MANUAL GQT CONTROLLER_NEW_GET FACILFEX]/
		//
		// GET: /Facil/Facilfex_New
		[ActionName("Facilfex_New")]
		[AuthorizeForUsers]
		[HttpGet]
		public ActionResult Facilfex_New()
		{
			...
			string partialView = qs["partialView"] ?? "Facilfex";

			var navigationLocationAction = ACTION_FACILFEX_NEW.SetRoutedValues(new { m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);

			//FOR: OVERRIDE SKIP IF JUST ONE
			//Allow child form to use "Go Back" to menu list without "skip if only one"
			if (Navigation.OverrideSkipIfJustOne.ContainsKey("FACILFEX"))
				Navigation.OverrideSkipIfJustOne["FACILFEX"] = true;
...
// USE /[MANUAL GQT AFTER_LOAD_NEW FACILFEX]/
			...
			catch (Exception e)
			{
				...
				CSGenio.framework.Log.Error("Facilfex_New - GET " + e.Message);
...
// USE /[MANUAL GQT CONTROLLER_NEW_POST FACILFEX]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Facilfex_New(Facilfex_ViewModel model, bool redirect = true)
		{
...
// USE /[MANUAL GQT AFTER_LOAD_NEW_EX FACILFEX]/
		...
		[AuthorizeForUsers]
		public ActionResult Facilfex_New_Redirect(bool internalRedirect = false)
		{
			...
			if (Convert.ToBoolean(Request.QueryString.Get("repeatInsertion")))
				return RedirectToLocation(ACTION_FACILFEX_NEW, new { repeatInsertion = true });
...
// USE /[MANUAL GQT CONTROLLER_EDIT_GET FACILFEX]/
		//
		// GET: /Facil/Facilfex_Edit
		[AuthorizeForUsers]
		[HttpGet]
		[ActionName("Facilfex_Edit")]
		public ActionResult Facilfex_Edit(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";
			string partialView = qs["partialView"] ?? "Facilfex"; // MF send the partial view name

			//Useful to initialize a record that had to be created by the framework prior to edition (and method New doesnt support this). This way, if the form then is canceled by the user, the record will be automatically deleted
			var isNewEdit = Navigation.GetStrValue("NewEdit") == "true";

			var navigationLocationAction =  isNewEdit ? ACTION_FACILFEX_NEW.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] }) : ACTION_FACILFEX_EDIT.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);
...
// USE /[MANUAL GQT BEFORE_LOAD_EDIT FACILFEX]/
			...
			catch (ModelNotFoundException)
			{
				...
				sp.closeConnection();
				//JFG 05-05-2017 This need to better handled, for now it's handled on webconfig's customErrors section.
				CSGenio.framework.Log.Error("Facilfex_Edit - " + id + " " + e.Message);
				throw;
			}
...
// USE /[MANUAL GQT CONTROLLER_EDIT_POST FACILFEX]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Facilfex_Edit(Facilfex_ViewModel model, bool redirect = true)
		{
...
// USE /[MANUAL GQT AFTER_LOAD_EDIT_EX FACILFEX]/
		...
		[AuthorizeForUsers]
		public ActionResult Facilfex_Edit_Redirect(bool internalRedirect = false)
		{
...
// USE /[MANUAL GQT CONTROLLER_DELETE_GET FACILFEX]/
		//
		// GET: /Facil/Facilfex_Delete
		[AuthorizeForUsers]
		[HttpGet]
		[ActionName("Facilfex_Delete")]
		public ActionResult Facilfex_Delete(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";

			var navigationLocationAction = ACTION_FACILFEX_DELETE.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);
...
// USE /[MANUAL GQT BEFORE_LOAD_DELETE FACILFEX]/
			...
			catch (ModelNotFoundException)
			{
				...
				//JFG 05-05-2017 This need to better handled, for now it's handled on webconfig's customErrors section.
				CSGenio.framework.Log.Error("Facilfex_Delete - " + id + " " + e.Message);
				throw;
			}
...
// USE /[MANUAL GQT AFTER_LOAD_DELETE FACILFEX]/
			...
			if (!Request.IsAjaxRequest())
				...
				if (permission.Status.Equals(CSGenio.framework.Status.E))
					...
					return PartialView("Facilfex", model);
		}
...
// USE /[MANUAL GQT CONTROLLER_DELETE_POST FACILFEX]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Facilfex_Delete(string id, FormCollection collection)
		{

			var model = new Facilfex_ViewModel (Navigation, id);
			model.MapFromModel();
...
// USE /[MANUAL GQT CONTROLLER_DUPLICATE_GET FACILFEX]/
		//
		// GET: /Facil/Facilfex_Duplicate
		[AuthorizeForUsers]
		[HttpGet]
		public ActionResult Facilfex_Duplicate(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";
			var model = new Facilfex_ViewModel(Navigation, nestedForm);
			...
			var navigationLocationAction = ACTION_FACILFEX_DUPLICATE.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);
...
// USE /[MANUAL GQT AFTER_LOAD_DUPLICATE FACILFEX]/
					...
					sp.closeTransaction();
					Navigation.CurrentLevel.SetLocation(ACTION_FACILFEX_DUPLICATE.SetRoutedValues(new { Id = model.ValCodfacil }));
...
// USE /[MANUAL GQT CONTROLLER_DUPLICATE_POST FACILFEX]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Facilfex_Duplicate(Facilfex_ViewModel model, bool redirect = true)
		{
			var eventSink = new EventSink()
			{
...
// USE /[MANUAL GQT AFTER_LOAD_DUPLICATE_EX FACILFEX]/
		...
		[AuthorizeForUsers]
		public ActionResult Facilfex_Duplicate_Redirect(bool internalRedirect = false)
		{
...
// USE /[MANUAL GQT AFTER_CANCEL FACILFEX]/
		...
		{
			var model = new Facilfex_ViewModel(Navigation, true);
			model.setModes(Request.QueryString["m"]);
			PersistentSupport sp = UserContext.Current.PersistentSupport;
			var navigationLocationAction = ACTION_FACILFEX_NEW.SetRoutedValues(new { m = Request.QueryString["m"] });
			CheckLevels(navigationLocationAction);
			...
			return PartialView("MFFacilfex", model);
		}
		...
		public ActionResult MFFacilfex_Edit(string id)
		{
			return this.RedirectToAction("Facilfex_Edit", "Facil", new { id = id, partialView = "MFFacilfex", nestedForm = "true", multiForm = "true" });
		}
		...
		public ActionResult MFFacilfex_Cancel(string id)
		{
			//MH - Validate if current level is it correct. Remove all levels that is higher than the currently level.
			CheckLevels(NavigationLocation.Any);

			if (Navigation.CurrentLevel.IsNestedContext &&
				(Navigation.CurrentLevel.Location.Action == ACTION_FACILFEX_NEW.Action || Navigation.CurrentLevel.Location.Action == ACTION_FACILFEX_EDIT.Action))
			{
				if (Navigation.CurrentLevel.FormMode == FormMode.New)
				{
					...
					catch (Exception e)
					{
						...
						Log.Error("MFFacilfex_Cancel - " + exceptionUserMessage);
						ErrorMessage(exceptionUserMessage);
					}
				}
			...
			return this.RedirectToAction("Facilfex_Show", "Facil", new { id = id, partialView = "MFFacilfex", nestedForm = "true", multiForm = "true", nav });
		}
		...
		public JsonResult MFFacilfex_Delete(string id)
		{
			var eventSink = new EventSink()
			{
		...
		public ActionResult Facilfex_EntitValName(string id, string partialView,  IDictionary<string, string> Limits)
		{
			...
			Facilfex_EntitValName_ViewModel model = new Facilfex_EntitValName_ViewModel(navigation);
			model.setModes(Request.QueryString["m"]);
		...
		[ActionSessionState(System.Web.SessionState.SessionStateBehavior.ReadOnly)]
		public ActionResult Facilfex_FactyValType(string id, string partialView,  IDictionary<string, string> Limits)
		{
			int perPage = CSGenio.framework.Configuration.NrRegDBedit;
			...
			NameValueCollection requestValues = Request.Unvalidated.Form.Count > 0 ? Request.Unvalidated.Form : Request.QueryString; //TSX (01.07.2020) Can not access directly to the FormCollection or made Request.Form otherwise the tags on viewmodel will be ignored
			var navigation = Navigation.Clone();
			Facilfex_FactyValType_ViewModel model = new Facilfex_FactyValType_ViewModel(navigation);
			model.setModes(Request.QueryString["m"]);
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Facilfex_SaveEdit(Facilfex_ViewModel model)
		{
			var eventSink = new EventSink()
			{
