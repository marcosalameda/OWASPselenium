...
// USE /[MANUAL GQT MANUAL_CONTROLLER FACIL]/
        ...
        public JsonResult ReloadDBEdit(string Identifier, IDictionary<string, object> Values)
        {
            ...
            try { row = Models.Facil.Find(navigation.GetStrValue("facil")); }
            catch (Exception)
            {
			...
			try
			{
				switch ((string.IsNullOrEmpty(Identifier) || Identifier.Length < 5) ? "" : Identifier.Substring(4)) // Substring(4) => to retirar o IFF_ e LED_
				{
					case "FACIL___ENTITNAME____":	// Field (DB)
                        {
						    ...
						    var model = new Facil_ViewModel(navigation) { editable = false };
					...
					case "FACILFEXENTITNAME____":	// Field (DB)
                        {
                            row.LoadKeysFormHistory(navigation, navigation.CurrentLevel.Level, false, true, true, true);
						    var model = new Facilfex_ViewModel(navigation) { editable = false };
						    model.MapFromModel(row);
                            TryUpdateModel(model); // Map recived values to fields - The 'field' type limits
        ...
        public JsonResult GetDependants(string Identifier, string Selected, IDictionary<string, object> Limits)
        {
            ...
            try
            {
				...
				{
					case "FACIL___ENTITNAME____":	// Field (DB)
						values = Facil_ViewModel.GetDependant_FacilTableEntitName(Selected, navigation);
						break;
					case "FACIL___FACTYTYPE____":	// Field (DB)
						values = Facil_ViewModel.GetDependant_FacilTableFactyType(Selected, navigation);
						break;
					case "FACILFEXENTITNAME____":	// Field (DB)
						values = Facilfex_ViewModel.GetDependant_FacilfexTableEntitName(Selected, navigation);
						break;
					case "FACILFEXFACTYTYPE____":	// Field (DB)
						values = Facilfex_ViewModel.GetDependant_FacilfexTableFactyType(Selected, navigation);
						break;
					default: break;
				}
		...
		[AuthorizeForUsers]
        [ActionSessionState(System.Web.SessionState.SessionStateBehavior.ReadOnly)]
        public JsonResult RecalculateFormulas_Facil(Facil_ViewModel form_data)
        {
            return GenericRecalculateFormulas(form_data, "facil",
                (primaryKey) => Models.Facil.Find(primaryKey, "FFACIL"),
		...
		[AuthorizeForUsers]
        [ActionSessionState(System.Web.SessionState.SessionStateBehavior.ReadOnly)]
        public JsonResult RecalculateFormulas_Facilfex(Facilfex_ViewModel form_data)
        {
            return GenericRecalculateFormulas(form_data, "facil",
                (primaryKey) => Models.Facil.Find(primaryKey, "FFACILFEX"),
                (model) => form_data.MapToModel(model as Models.Facil)
            );
        }
