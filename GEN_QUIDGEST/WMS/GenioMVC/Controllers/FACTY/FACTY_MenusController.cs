...
namespace GenioMVC.Controllers
{
    public partial class FactyController : ControllerBase
    {
        ...
        public ActionResult WMS_Menu_4231(bool allSelected = false)
        {
			int perPage = CSGenio.framework.Configuration.NrRegDBedit;

            WMS_Menu_4231_ViewModel model = new WMS_Menu_4231_ViewModel(Navigation);
            ...
            {
                if (Navigation.CurrentLevel == null || !ACTION_WMS_MENU_4231.IsSameAction(Navigation.CurrentLevel.Location))
                {
                    // reset the selections for this new navigation flow
                    // TODO: This change still requires more testing
                    Navigation.RemoveHistoryLevel(ACTION_WMS_MENU_4231);
                    if (Navigation.CurrentLevel.Location.Action != ACTION_WMS_MENU_4231.Action)
                    {
                        Navigation.AddHistoryLevel(ACTION_WMS_MENU_4231, FormMode.List);
...
// USE /[MANUAL WMS MENU_GET 4231]/
            ...
            if(isHomePage)
                return PartialView("WMS_Menu_4231", model);
            else if (!Request.IsAjaxRequest())
                return View(model);
            else
                return PartialView("WMS_Menu_4231_Partial", model);
        }
