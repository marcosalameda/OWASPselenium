...
// USE /[MANUAL GQT MANUAL_CONTROLLER GLOB]/
        ...
        public JsonResult ReloadDBEdit(string Identifier, IDictionary<string, object> Values)
        {
            ...
            try { row = Models.Glob.Find(navigation.GetStrValue("glob")); }
            catch (Exception)
            {
					...
					case "GLOBFACTFACTYTYPE____":	// Field (DB)
                        {
						    ...
						    var model = new Globfact_ViewModel(navigation) { editable = false };
        ...
        public JsonResult GetDependants(string Identifier, string Selected, IDictionary<string, object> Limits)
        {
            ...
            try
            {
				...
				{
					case "GLOBFACTFACTYTYPE____":	// Field (DB)
						values = Globfact_ViewModel.GetDependant_GlobfactTableFactyType(Selected, navigation);
						break;
					default: break;
				}
		...
		[AuthorizeForUsers]
        [ActionSessionState(System.Web.SessionState.SessionStateBehavior.ReadOnly)]
        public JsonResult RecalculateFormulas_Globfact(Globfact_ViewModel form_data)
        {
            return GenericRecalculateFormulas(form_data, "glob",
                (primaryKey) => Models.Glob.Find(primaryKey, "FGLOBFACT"),
