...
namespace GenioMVC.Controllers
{
    public partial class GlobController : ControllerBase
    {
        ...
        public ActionResult WMS_Menu_4241(bool allSelected = false)
        {
			int perPage = CSGenio.framework.Configuration.NrRegDBedit;

            WMS_Menu_4241_ViewModel model = new WMS_Menu_4241_ViewModel(Navigation);
            ...
            {
                if (Navigation.CurrentLevel == null || !ACTION_WMS_MENU_4241.IsSameAction(Navigation.CurrentLevel.Location))
                {
                    // reset the selections for this new navigation flow
                    // TODO: This change still requires more testing
                    Navigation.RemoveHistoryLevel(ACTION_WMS_MENU_4241);
                    if (Navigation.CurrentLevel.Location.Action != ACTION_WMS_MENU_4241.Action)
                    {
                        Navigation.AddHistoryLevel(ACTION_WMS_MENU_4241, FormMode.List);
...
// USE /[MANUAL WMS MENU_GET 4241]/
			...
			if (!Request.IsAjaxRequest() && curRowsCount == 1 && model.Menu.Filters.FiltersValues.Count == 0 && model.Menu.Elements.First().ValZzstate == 0 && AllowSkipIfOnlyOne)
			{
                ...
                return RedirectToAction("Globfact_Edit", "Glob", new { id = primaryKey, nav = Navigation.NavigationId, isHomePage });
            }
  
            if(isHomePage)
                return PartialView("WMS_Menu_4241", model);
            else if (!Request.IsAjaxRequest())
                return View(model);
            else
                return PartialView("WMS_Menu_4241_Partial", model);
        }
