...
// USE /[MANUAL GQT MANUAL_CONTROLLER LCEXT]/
        ...
        public JsonResult ReloadDBEdit(string Identifier, IDictionary<string, object> Values)
        {
            ...
            try { row = Models.Lcext.Find(navigation.GetStrValue("lcext")); }
            catch (Exception)
            {
					...
					case "LCEXT___LOCATGLN_____":	// Field (DB)
                        {
						    ...
						    var model = new Lcext_ViewModel(navigation) { editable = false };
        ...
        public JsonResult GetDependants(string Identifier, string Selected, IDictionary<string, object> Limits)
        {
            ...
            try
            {
				...
				{
					case "LCEXT___LOCATGLN_____":	// Field (DB)
						values = Lcext_ViewModel.GetDependant_LcextTableLocatGln(Selected, navigation);
						break;
					default: break;
				}
		...
		[AuthorizeForUsers]
        [ActionSessionState(System.Web.SessionState.SessionStateBehavior.ReadOnly)]
        public JsonResult RecalculateFormulas_Lcext(Lcext_ViewModel form_data)
        {
            return GenericRecalculateFormulas(form_data, "lcext",
                (primaryKey) => Models.Lcext.Find(primaryKey, "FLCEXT"),
