...
// USE /[MANUAL GQT MANUAL_CONTROLLER LOCAT]/
        ...
        public JsonResult ReloadDBEdit(string Identifier, IDictionary<string, object> Values)
        {
            ...
            try { row = Models.Locat.Find(navigation.GetStrValue("locat")); }
            catch (Exception)
            {
					...
					case "LOCAT___ENTITNAME____":	// Field (DB)
                        {
						    ...
						    var model = new Locat_ViewModel(navigation) { editable = false };
        ...
        public JsonResult GetDependants(string Identifier, string Selected, IDictionary<string, object> Limits)
        {
            ...
            try
            {
				...
				{
					case "LOCAT___ENTITNAME____":	// Field (DB)
						values = Locat_ViewModel.GetDependant_LocatTableEntitName(Selected, navigation);
						break;
					case "LOCAT___FACILNAME____":	// Field (DB)
						values = Locat_ViewModel.GetDependant_LocatTableFacilName(Selected, navigation);
						break;
					default: break;
				}
		...
		[AuthorizeForUsers]
        [ActionSessionState(System.Web.SessionState.SessionStateBehavior.ReadOnly)]
        public JsonResult RecalculateFormulas_Locat(Locat_ViewModel form_data)
        {
            return GenericRecalculateFormulas(form_data, "locat",
                (primaryKey) => Models.Locat.Find(primaryKey, "FLOCAT"),
