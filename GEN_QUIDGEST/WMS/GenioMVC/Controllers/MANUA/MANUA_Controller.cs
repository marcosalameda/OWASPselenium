...
// USE /[MANUAL GQT MANUAL_CONTROLLER MANUA]/
        ...
        public JsonResult ReloadDBEdit(string Identifier, IDictionary<string, object> Values)
        {
            ...
            try { row = Models.Manua.Find(navigation.GetStrValue("manua")); }
            catch (Exception)
            {
					...
					case "MANUA___KINDEDESIGNAT":	// Field (DB)
                        {
						    ...
						    var model = new Manua_ViewModel(navigation) { editable = false };
        ...
        public JsonResult GetDependants(string Identifier, string Selected, IDictionary<string, object> Limits)
        {
            ...
            try
            {
				...
				{
					case "MANUA___KINDEDESIGNAT":	// Field (DB)
						values = Manua_ViewModel.GetDependant_ManuaTableKindeDesignat(Selected, navigation);
						break;
					default: break;
				}
		...
		[AuthorizeForUsers]
        [ActionSessionState(System.Web.SessionState.SessionStateBehavior.ReadOnly)]
        public JsonResult RecalculateFormulas_Manua(Manua_ViewModel form_data)
        {
            return GenericRecalculateFormulas(form_data, "manua",
                (primaryKey) => Models.Manua.Find(primaryKey, "FMANUA"),
