...
// USE /[MANUAL GQT MANUAL_CONTROLLER MESSA]/
        ...
        public JsonResult ReloadDBEdit(string Identifier, IDictionary<string, object> Values)
        {
            ...
            try { row = Models.Messa.Find(navigation.GetStrValue("messa")); }
            catch (Exception)
            {
					...
					case "MESSA___ENTITNAME____":	// Field (DB)
                        {
						    ...
						    var model = new Messa_ViewModel(navigation) { editable = false };
        ...
        public JsonResult GetDependants(string Identifier, string Selected, IDictionary<string, object> Limits)
        {
            ...
            try
            {
				...
				{
					case "MESSA___ENTITNAME____":	// Field (DB)
						values = Messa_ViewModel.GetDependant_MessaTableEntitName(Selected, navigation);
						break;
					case "MESSA___PERSONAME____":	// Field (DB)
						values = Messa_ViewModel.GetDependant_MessaTablePersoName(Selected, navigation);
						break;
					default: break;
				}
		...
		[AuthorizeForUsers]
        [ActionSessionState(System.Web.SessionState.SessionStateBehavior.ReadOnly)]
        public JsonResult RecalculateFormulas_Messa(Messa_ViewModel form_data)
        {
            return GenericRecalculateFormulas(form_data, "messa",
                (primaryKey) => Models.Messa.Find(primaryKey, "FMESSA"),
