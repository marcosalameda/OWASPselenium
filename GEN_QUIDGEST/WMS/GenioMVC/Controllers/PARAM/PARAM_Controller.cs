...
// USE /[MANUAL GQT MANUAL_CONTROLLER PARAM]/
        ...
        public JsonResult ReloadDBEdit(string Identifier, IDictionary<string, object> Values)
        {
            ...
            try { row = Models.Param.Find(navigation.GetStrValue("param")); }
            catch (Exception)
            {
					...
					case "PARAM___KINDEDESIGNAT":	// Field (DB)
                        {
						    ...
						    var model = new Param_ViewModel(navigation) { editable = false };
        ...
        public JsonResult GetDependants(string Identifier, string Selected, IDictionary<string, object> Limits)
        {
            ...
            try
            {
				...
				{
					case "PARAM___KINDEDESIGNAT":	// Field (DB)
						values = Param_ViewModel.GetDependant_ParamTableKindeDesignat(Selected, navigation);
						break;
					default: break;
				}
		...
		[AuthorizeForUsers]
        [ActionSessionState(System.Web.SessionState.SessionStateBehavior.ReadOnly)]
        public JsonResult RecalculateFormulas_Param(Param_ViewModel form_data)
        {
            return GenericRecalculateFormulas(form_data, "param",
                (primaryKey) => Models.Param.Find(primaryKey, "FPARAM"),
