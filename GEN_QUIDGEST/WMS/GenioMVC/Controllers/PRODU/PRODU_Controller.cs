...
// USE /[MANUAL GQT MANUAL_CONTROLLER PRODU]/
        ...
        public JsonResult ReloadDBEdit(string Identifier, IDictionary<string, object> Values)
        {
            ...
            dynamic result = null;
            Models.Produ row = null;
            try { row = Models.Produ.Find(navigation.GetStrValue("produ")); }
            catch (Exception)
            {
                CSGenio.framework.Log.Error("ReloadDBEdit - " + Identifier + " Not found Model produ");
            }
            if(row == null)
            {
                row = new Models.Produ();
			...
			try
			{
				switch ((string.IsNullOrEmpty(Identifier) || Identifier.Length < 5) ? "" : Identifier.Substring(4)) // Substring(4) => to retirar o IFF_ e LED_
				{
					case "PRODU___LOCATGLN_____":	// Field (DB)
                        {
						    ...
						    var model = new Produ_ViewModel(navigation) { editable = false };
					...
					case "PRODUSIMLOCATGLN_____":	// Field (DB)
                        {
                            row.LoadKeysFormHistory(navigation, navigation.CurrentLevel.Level, false, true, true, true);
						    var model = new Produsim_ViewModel(navigation) { editable = false };
						    model.MapFromModel(row);
                            TryUpdateModel(model); // Map recived values to fields - The 'field' type limits
        ...
        public JsonResult GetDependants(string Identifier, string Selected, IDictionary<string, object> Limits)
        {
            ...
            try
            {
				...
				{
					case "PRODU___LOCATGLN_____":	// Field (DB)
						values = Produ_ViewModel.GetDependant_ProduTableLocatGln(Selected, navigation);
						break;
					case "PRODU___LCEXTGLNEXT__":	// Field (DB)
						values = Produ_ViewModel.GetDependant_ProduTableLcextGlnext(Selected, navigation);
						break;
					case "PRODUSIMLOCATGLN_____":	// Field (DB)
						values = Produsim_ViewModel.GetDependant_ProdusimTableLocatGln(Selected, navigation);
						break;
					case "PRODUSIMLCEXTGLNEXT__":	// Field (DB)
						values = Produsim_ViewModel.GetDependant_ProdusimTableLcextGlnext(Selected, navigation);
						break;
					default: break;
				}
		...
		[AuthorizeForUsers]
        [ActionSessionState(System.Web.SessionState.SessionStateBehavior.ReadOnly)]
        public JsonResult RecalculateFormulas_Produ(Produ_ViewModel form_data)
        {
            return GenericRecalculateFormulas(form_data, "produ",
                (primaryKey) => Models.Produ.Find(primaryKey, "FPRODU"),
                (model) => form_data.MapToModel(model as Models.Produ)
            );
        }
		...
		[AuthorizeForUsers]
        [ActionSessionState(System.Web.SessionState.SessionStateBehavior.ReadOnly)]
        public JsonResult RecalculateFormulas_Produsim(Produsim_ViewModel form_data)
        {
            return GenericRecalculateFormulas(form_data, "produ",
                (primaryKey) => Models.Produ.Find(primaryKey, "FPRODUSIM"),
                (model) => form_data.MapToModel(model as Models.Produ)
            );
        }
