...
// USE /[MANUAL GQT MANUAL_CONTROLLER RECEI]/
        ...
        public JsonResult ReloadDBEdit(string Identifier, IDictionary<string, object> Values)
        {
            ...
            try { row = Models.Recei.Find(navigation.GetStrValue("recei")); }
            catch (Exception)
            {
					...
					case "RECEI___ENTITNAME____":	// Field (DB)
                        {
						    ...
						    var model = new Recei_ViewModel(navigation) { editable = false };
        ...
        public JsonResult GetDependants(string Identifier, string Selected, IDictionary<string, object> Limits)
        {
            ...
            try
            {
				...
				{
					case "RECEI___ENTITNAME____":	// Field (DB)
						values = Recei_ViewModel.GetDependant_ReceiTableEntitName(Selected, navigation);
						break;
					default: break;
				}
		...
		[AuthorizeForUsers]
        [ActionSessionState(System.Web.SessionState.SessionStateBehavior.ReadOnly)]
        public JsonResult RecalculateFormulas_Recei(Recei_ViewModel form_data)
        {
            return GenericRecalculateFormulas(form_data, "recei",
                (primaryKey) => Models.Recei.Find(primaryKey, "FRECEI"),
