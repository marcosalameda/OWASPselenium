...
// USE /[MANUAL GQT MANUAL_CONTROLLER REGIO]/
			...
			try
			{
				switch ((string.IsNullOrEmpty(Identifier) || Identifier.Length < 5) ? "" : Identifier.Substring(4)) // Substring(4) => to retirar o IFF_ e LED_
				{
					...
					case "REGIAPROCNTRYCOUNTRY_":	// Field (DB)
                        {
                            row.LoadKeysFormHistory(navigation, navigation.CurrentLevel.Level, false, true, true, true);
						    var model = new Regiapro_ViewModel(navigation) { editable = false };
						    model.MapFromModel(row);
                            TryUpdateModel(model); // Map recived values to fields - The 'field' type limits
            ...
            try
            {
				...
				switch ((string.IsNullOrEmpty(Identifier) || Identifier.Length < 5) ? "" : Identifier.Substring(4)) // Substring(4) => to retirar o IFF_ e LED_
				{
					case "REGIA___CNTRYCOUNTRY_":	// Field (DB)
						...
						break;
					case "REGIAPROCNTRYCOUNTRY_":	// Field (DB)
						values = Regiapro_ViewModel.GetDependant_RegiaproTableCntryCountry(Selected, navigation);
						break;
					case "REGIAPROPAIS1COUNTRY_":	// Field (DB)
						values = Regiapro_ViewModel.GetDependant_RegiaproTablePais1Country(Selected, navigation);
						break;
					default: break;
				}
		...
		[AuthorizeForUsers]
        [ActionSessionState(System.Web.SessionState.SessionStateBehavior.ReadOnly)]
        public JsonResult RecalculateFormulas_Regiapro(Regiapro_ViewModel form_data)
        {
            return GenericRecalculateFormulas(form_data, "regio",
                (primaryKey) => Models.Regio.Find(primaryKey, "FREGIAPRO"),
                (model) => form_data.MapToModel(model as Models.Regio)
            );
        }
