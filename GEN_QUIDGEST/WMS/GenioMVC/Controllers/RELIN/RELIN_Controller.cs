...
// USE /[MANUAL GQT MANUAL_CONTROLLER RELIN]/
        ...
        public JsonResult ReloadDBEdit(string Identifier, IDictionary<string, object> Values)
        {
            ...
            try { row = Models.Relin.Find(navigation.GetStrValue("relin")); }
            catch (Exception)
            {
					...
					case "RELIN___RECEINUMBER__":	// Field (DB)
                        {
						    ...
						    var model = new Relin_ViewModel(navigation) { editable = false };
        ...
        public JsonResult GetDependants(string Identifier, string Selected, IDictionary<string, object> Limits)
        {
            ...
            try
            {
				...
				{
					case "RELIN___RECEINUMBER__":	// Field (DB)
						values = Relin_ViewModel.GetDependant_RelinTableReceiNumber(Selected, navigation);
						break;
					case "RELIN___PRODUPRODUCT_":	// Field (DB)
						values = Relin_ViewModel.GetDependant_RelinTableProduProduct(Selected, navigation);
						break;
					default: break;
				}
		...
		[AuthorizeForUsers]
        [ActionSessionState(System.Web.SessionState.SessionStateBehavior.ReadOnly)]
        public JsonResult RecalculateFormulas_Relin(Relin_ViewModel form_data)
        {
            return GenericRecalculateFormulas(form_data, "relin",
                (primaryKey) => Models.Relin.Find(primaryKey, "FRELIN"),
