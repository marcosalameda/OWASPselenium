...
namespace GenioMVC.Controllers
{
	public partial class RelinController : ControllerBase
	{
		...
		[AuthorizeForUsers]
		public ActionResult ChangeFormMode_Relin(string mode)
		{
			var _mode = Navigation.CurrentLevel.Location;
			var id = Navigation.GetStrValue("relin");
			...
			switch (mode)
			{
				case "show":
					if (m.Contains("v"))
						_mode = ACTION_RELIN_SHOW;
					break;
				case "delete":
					if (m.Contains("a"))
						_mode = ACTION_RELIN_DELETE;
					break;
				case "edit":
					if (m.Contains("e"))
						_mode = ACTION_RELIN_EDIT;
					break;
				case "duplicate":
					if (m.Contains("d"))
						_mode = ACTION_RELIN_DUPLICATE;
					break;
				case "new":
					if (m.Contains("i"))
						_mode = ACTION_RELIN_NEW;
					break;
			}
...
// USE /[MANUAL GQT CONTROLLER_SHOW RELIN]/
		//
		// GET: /Relin/Relin_Show
		[AuthorizeForUsers]
		public ActionResult Relin_Show(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";
			var model = new Relin_ViewModel(Navigation, nestedForm);
			...
			string partialView = qs["partialView"] ?? "Relin"; // MF send the patial view name

			var navigationLocationAction = ACTION_RELIN_SHOW.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			...
			if (!isHomePage && (IsNewLocation(navigationLocationAction) || !string.Equals(Navigation.GetStrValue("relin"), id)))
			{
				Navigation.AddHistoryLevel(navigationLocationAction, FormMode.Show, nestedForm);
				CSGenio.framework.Audit.registAction(UserContext.Current.User, Resources.Resources.FORM54242 + " " + Navigation.CurrentLevel.Location.ShortDescription());
			}

			Navigation.SetValue("relin", id);
...
// USE /[MANUAL GQT BEFORE_LOAD_SHOW RELIN]/
			...
			catch (ModelNotFoundException)
			{
				...
				CSGenio.framework.Log.Error("Relin_Show - GET " + id + " " + e.Message);
				throw;
			}
...
// USE /[MANUAL GQT AFTER_LOAD_SHOW RELIN]/
			...
			if (isHomePage)
			{
				Navigation.SetValue("HomePage", "Relin");
				return PartialView(partialView, model);
			}
			else if (!Request.IsAjaxRequest())
				return View("Relin", model);
			else
				return PartialView(partialView, model);
		}
		...
		[ActionName("Relin_New_Insert")]
		[AuthorizeForUsers]
		[HttpGet]
		public ActionResult Relin_New_Insert()
		{
			...
			return RedirectToAction("Relin_New");
		}

// USE /[MANUAL GQT CONTROLLER_NEW_GET RELIN]/
		//
		// GET: /Relin/Relin_New
		[ActionName("Relin_New")]
		[AuthorizeForUsers]
		[HttpGet]
		public ActionResult Relin_New()
		{
			...
			string partialView = qs["partialView"] ?? "Relin";

			var navigationLocationAction = ACTION_RELIN_NEW.SetRoutedValues(new { m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);

			//FOR: OVERRIDE SKIP IF JUST ONE
			//Allow child form to use "Go Back" to menu list without "skip if only one"
			if (Navigation.OverrideSkipIfJustOne.ContainsKey("RELIN"))
				Navigation.OverrideSkipIfJustOne["RELIN"] = true;
...
// USE /[MANUAL GQT AFTER_LOAD_NEW RELIN]/
			...
			catch (Exception e)
			{
				...
				CSGenio.framework.Log.Error("Relin_New - GET " + e.Message);
...
// USE /[MANUAL GQT CONTROLLER_NEW_POST RELIN]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Relin_New(Relin_ViewModel model, bool redirect = true)
		{
...
// USE /[MANUAL GQT AFTER_LOAD_NEW_EX RELIN]/
		...
		[AuthorizeForUsers]
		public ActionResult Relin_New_Redirect(bool internalRedirect = false)
		{
			IList<string> warningMsgs = TempData["NEW_WARNINGS_LIST"] as List<string>;
			string saveMsg = TempData["NEW_SAVE_LIST"] as string;

			if (!internalRedirect)
				SuccessMessage(String.IsNullOrEmpty(saveMsg) ? Resources.Resources.REGISTO_CRIADO_COM_S18746 + GetHumanKeyToQMessage("relin", Convert.ToString(Navigation.CurrentLevel.GetEntry("relin"))) : saveMsg);
			...
			if (Convert.ToBoolean(Request.QueryString.Get("repeatInsertion")))
				return RedirectToLocation(ACTION_RELIN_NEW, new { repeatInsertion = true });
...
// USE /[MANUAL GQT CONTROLLER_EDIT_GET RELIN]/
		//
		// GET: /Relin/Relin_Edit
		[AuthorizeForUsers]
		[HttpGet]
		[ActionName("Relin_Edit")]
		public ActionResult Relin_Edit(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";
			string partialView = qs["partialView"] ?? "Relin"; // MF send the partial view name

			//Useful to initialize a record that had to be created by the framework prior to edition (and method New doesnt support this). This way, if the form then is canceled by the user, the record will be automatically deleted
			var isNewEdit = Navigation.GetStrValue("NewEdit") == "true";

			var navigationLocationAction =  isNewEdit ? ACTION_RELIN_NEW.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] }) : ACTION_RELIN_EDIT.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);
...
// USE /[MANUAL GQT BEFORE_LOAD_EDIT RELIN]/
			...
			catch (ModelNotFoundException)
			{
				...
				sp.closeConnection();
				//JFG 05-05-2017 This need to better handled, for now it's handled on webconfig's customErrors section.
				CSGenio.framework.Log.Error("Relin_Edit - " + id + " " + e.Message);
				throw;
			}
...
// USE /[MANUAL GQT CONTROLLER_EDIT_POST RELIN]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Relin_Edit(Relin_ViewModel model, bool redirect = true)
		{
...
// USE /[MANUAL GQT AFTER_LOAD_EDIT_EX RELIN]/
		...
		[AuthorizeForUsers]
		public ActionResult Relin_Edit_Redirect(bool internalRedirect = false)
		{
			IList<string> warningMsgs = TempData["EDIT_WARNINGS_LIST"] as List<string>;
			string saveMsg = TempData["NEW_SAVE_LIST"] as string;

			if (!internalRedirect || !Request.IsAjaxRequest())
				SuccessMessage(String.IsNullOrEmpty(saveMsg) ? Resources.Resources.ALTERACOES_EFECTUADA64514 + GetHumanKeyToQMessage("relin", Convert.ToString(Navigation.CurrentLevel.GetEntry("relin"))) : saveMsg);
...
// USE /[MANUAL GQT CONTROLLER_DELETE_GET RELIN]/
		//
		// GET: /Relin/Relin_Delete
		[AuthorizeForUsers]
		[HttpGet]
		[ActionName("Relin_Delete")]
		public ActionResult Relin_Delete(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";

			var navigationLocationAction = ACTION_RELIN_DELETE.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);
...
// USE /[MANUAL GQT BEFORE_LOAD_DELETE RELIN]/
			...
			catch (ModelNotFoundException)
			{
				...
				//JFG 05-05-2017 This need to better handled, for now it's handled on webconfig's customErrors section.
				CSGenio.framework.Log.Error("Relin_Delete - " + id + " " + e.Message);
				throw;
			}
...
// USE /[MANUAL GQT AFTER_LOAD_DELETE RELIN]/
			...
			if (!Request.IsAjaxRequest())
				...
				if (permission.Status.Equals(CSGenio.framework.Status.E))
					...
					return PartialView("Relin", model);
		}
...
// USE /[MANUAL GQT CONTROLLER_DELETE_POST RELIN]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Relin_Delete(string id, FormCollection collection)
		{

			var model = new Relin_ViewModel (Navigation, id);
			model.MapFromModel();
...
// USE /[MANUAL GQT CONTROLLER_DUPLICATE_GET RELIN]/
		//
		// GET: /Relin/Relin_Duplicate
		[AuthorizeForUsers]
		[HttpGet]
		public ActionResult Relin_Duplicate(string id)
		{
			var qs = Request.QueryString;
			var nestedForm = qs["nestedForm"] == "true";
			var model = new Relin_ViewModel(Navigation, nestedForm);
			...
			var navigationLocationAction = ACTION_RELIN_DUPLICATE.SetRoutedValues(new { Id = id, m = Request.QueryString["m"] });
			//MH - Validate if current level is it correct. Remove all levels that is higher than the previous level.
			CheckLevels(navigationLocationAction);
...
// USE /[MANUAL GQT AFTER_LOAD_DUPLICATE RELIN]/
					...
					sp.closeTransaction();
					Navigation.CurrentLevel.SetLocation(ACTION_RELIN_DUPLICATE.SetRoutedValues(new { Id = model.ValCoddilin }));
...
// USE /[MANUAL GQT CONTROLLER_DUPLICATE_POST RELIN]/
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Relin_Duplicate(Relin_ViewModel model, bool redirect = true)
		{
			var eventSink = new EventSink()
			{
...
// USE /[MANUAL GQT AFTER_LOAD_DUPLICATE_EX RELIN]/
		...
		[AuthorizeForUsers]
		public ActionResult Relin_Duplicate_Redirect(bool internalRedirect = false)
		{
...
// USE /[MANUAL GQT CONTROLLER_CANCEL_GET RELIN]/
		...
		public ActionResult Relin_Cancel()
		{
			...
			if (Navigation.CurrentLevel.FormMode == FormMode.New || Navigation.CurrentLevel.FormMode == FormMode.Duplicate)
			{
				...
				try
				{
					var model = new GenioMVC.Models.Relin();
...
// USE /[MANUAL GQT AFTER_CANCEL RELIN]/
			...
			else if (Navigation.PreviousLevel != null)
			{
				...
				string previousLevelArea = Navigation.PreviousLevel.Location.Controller?.ToLower() ?? "relin";
		...
		{
			var model = new Relin_ViewModel(Navigation, true);
			model.setModes(Request.QueryString["m"]);
			PersistentSupport sp = UserContext.Current.PersistentSupport;
			var navigationLocationAction = ACTION_RELIN_NEW.SetRoutedValues(new { m = Request.QueryString["m"] });
			CheckLevels(navigationLocationAction);
			...
			return PartialView("MFRelin", model);
		}
		...
		public ActionResult MFRelin_Edit(string id)
		{
			return this.RedirectToAction("Relin_Edit", "Relin", new { id = id, partialView = "MFRelin", nestedForm = "true", multiForm = "true" });
		}
		...
		public ActionResult MFRelin_Cancel(string id)
		{
			//MH - Validate if current level is it correct. Remove all levels that is higher than the currently level.
			CheckLevels(NavigationLocation.Any);

			if (Navigation.CurrentLevel.IsNestedContext &&
				(Navigation.CurrentLevel.Location.Action == ACTION_RELIN_NEW.Action || Navigation.CurrentLevel.Location.Action == ACTION_RELIN_EDIT.Action))
			{
				if (Navigation.CurrentLevel.FormMode == FormMode.New)
				{
					...
					catch (Exception e)
					{
						...
						Log.Error("MFRelin_Cancel - " + exceptionUserMessage);
						ErrorMessage(exceptionUserMessage);
					}
				}
			...
			return this.RedirectToAction("Relin_Show", "Relin", new { id = id, partialView = "MFRelin", nestedForm = "true", multiForm = "true", nav });
		}
		...
		public JsonResult MFRelin_Delete(string id)
		{
			var eventSink = new EventSink()
			{
		...
		public ActionResult Relin_ReceiValNumber(string id, string partialView,  IDictionary<string, string> Limits)
		{
			...
			Relin_ReceiValNumber_ViewModel model = new Relin_ReceiValNumber_ViewModel(navigation);
			model.setModes(Request.QueryString["m"]);
		...
		[ActionSessionState(System.Web.SessionState.SessionStateBehavior.ReadOnly)]
		public ActionResult Relin_ProduValProduct(string id, string partialView,  IDictionary<string, string> Limits)
		{
			int perPage = CSGenio.framework.Configuration.NrRegDBedit;
			...
			NameValueCollection requestValues = Request.Unvalidated.Form.Count > 0 ? Request.Unvalidated.Form : Request.QueryString; //TSX (01.07.2020) Can not access directly to the FormCollection or made Request.Form otherwise the tags on viewmodel will be ignored
			var navigation = Navigation.Clone();
			Relin_ProduValProduct_ViewModel model = new Relin_ProduValProduct_ViewModel(navigation);
			model.setModes(Request.QueryString["m"]);
			model.ValCoddilin = id;
		...
		[HttpPost]
		[HttpParamAction]
		public ActionResult Relin_SaveEdit(Relin_ViewModel model)
		{
			var eventSink = new EventSink()
			{
