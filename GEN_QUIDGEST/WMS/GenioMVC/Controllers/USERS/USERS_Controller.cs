...
// USE /[MANUAL GQT MANUAL_CONTROLLER USERS]/
        ...
        public JsonResult ReloadDBEdit(string Identifier, IDictionary<string, object> Values)
        {
            ...
            try { row = Models.Users.Find(navigation.GetStrValue("users")); }
            catch (Exception)
            {
					...
					case "USERS___PSW__NOME____":	// Field (DB)
                        {
						    ...
						    var model = new Users_ViewModel(navigation) { editable = false };
        ...
        public JsonResult GetDependants(string Identifier, string Selected, IDictionary<string, object> Limits)
        {
            ...
            try
            {
				...
				{
					case "USERS___PSW__NOME____":	// Field (DB)
						values = Users_ViewModel.GetDependant_UsersTablePswNome(Selected, navigation);
						break;
					case "USERS___PERSONAME____":	// Field (DB)
						values = Users_ViewModel.GetDependant_UsersTablePersoName(Selected, navigation);
						break;
					default: break;
				}
		...
		[AuthorizeForUsers]
        [ActionSessionState(System.Web.SessionState.SessionStateBehavior.ReadOnly)]
        public JsonResult RecalculateFormulas_Users(Users_ViewModel form_data)
        {
            return GenericRecalculateFormulas(form_data, "users",
                (primaryKey) => Models.Users.Find(primaryKey, "FUSERS"),
