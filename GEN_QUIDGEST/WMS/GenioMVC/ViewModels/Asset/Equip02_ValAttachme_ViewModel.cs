...
namespace GenioMVC.ViewModels.Asset
{
    public class Equip02_ValAttachme_ViewModel : ListViewModel
    {
        ...
        public TablePartial<GenioMVC.Models.Attac> Menu { get; set; }
        ...
        public Equip02_ValAttachme_ViewModel(NavigationContext currentNavigation) : base(currentNavigation)
        {
        ...
        public void LoadToExport(out ListingMVC<CSGenioAattac> listing, out CriteriaSet conditions, out List<Exports.QColumn> columns, NameValueCollection requestValues, bool ajaxRequest = false)
        {
            ...
            if (ajaxRequest && userColumns!=null)
            {
                ...
                foreach (CSGenioAlstcol column in userColumns)
                {
                    ...
                    string areabase = column.ValTabela.ToLower() != "attac" ? CultureInfo.InvariantCulture.TextInfo.ToTitleCase(column.ValTabela) + "." : "";
		...
		public override CriteriaSet BuildCriteriaSet(NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			if(Menu == null)
				Menu = new TablePartial<GenioMVC.Models.Attac>();
			Menu.SetFilters(bool.Parse(requestValues["ValAttachme_tableFilters"] ?? "false"), false);
			...
			crs.SubSets.Add(ProcessSearchFilters(Menu, GetSearchColumns(true), requestValues, "ValAttachme_"));
		...
		public void Load(int numberListItems, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAattac> Qlisting, ref CriteriaSet conditions)
		{
			...
			if (ajaxRequest)
				this.Navigation.SetValue("requestValues" + "Equip02_ValAttachme", requestValues);
			else if (!ajaxRequest && this.Navigation.CheckKey("requestValues" + "Equip02_ValAttachme"))
				requestValues = this.Navigation.GetValue<NameValueCollection>("requestValues" + "Equip02_ValAttachme");
			...
			var pageNumber = (ajaxRequest && !String.IsNullOrEmpty(requestValues["pValAttachme"])) ? int.Parse(requestValues["pValAttachme"]) : 1;

			// Added to avoid 0 or -1 pages when setting number of records to -1 to disable pagination
			if (pageNumber < 1)
				pageNumber = 1;

			List<ColumnSort> sorts = GetRequestSorts(this.Menu, "sValAttachme", "dValAttachme", requestValues, "attac", allSortOrders);
...
FieldRef[] fields = new FieldRef[] { CSGenioAattac.FldCodattac, CSGenioAattac.FldZzstate, CSGenioAattac.FldAttached, CSGenioAattac.FldNote, CSGenioAattac.FldDocument, CSGenioAattac.FldDocumentfk };
			...
			//Tooltip for EPHs affecting this viewmodel list
			{
				...
				CSGenioAattac model_limit_area = new CSGenioAattac(UserContext.Current.User);
			...
			CriteriaSet equip02_pseudattachmeConds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
            tableReload &= hasAllRequiredLimits;

// USE /[MANUAL GQT OVERRQ EQUIP02_PSEUDATTACHME]/

            // This will happen in case there is an error
            if(equip02_pseudattachmeConds == null)
                return;
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAattac>(false, equip02_pseudattachmeConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_EQUIP02_PSEUDATTACHME", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL GQT OVERRQLSTEXP EQUIP02_PSEUDATTACHME]/

                conditions = equip02_pseudattachmeConds;
                return;
			}
...
// USE /[MANUAL GQT OVERRQLIST EQUIP02_PSEUDATTACHME]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = UserContext.Current.PersistentSupport.getPagingPos(CSGenioAattac.GetInformation(), QMVC_POS_RECORD, sorts, equip02_pseudattachmeConds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioAattac> listing = Models.ModelBase.Where<CSGenioAattac>(false, equip02_pseudattachmeConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_EQUIP02_PSEUDATTACHME", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
        ...
        {
            var Elements = new List<Models.Attac>();
            int i = 0;
        ...
        /// to a Models.Attac object.
        /// </summary>
        /// <param name="row">The row.</param>
        private Models.Attac MapEquip02_ValAttachme(CSGenioAattac row)
        {
            var model = new Models.Attac(true, _fieldsToSerialize);
            if (row == null)
                return model;
