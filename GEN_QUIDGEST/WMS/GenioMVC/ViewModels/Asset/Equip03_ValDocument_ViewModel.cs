...
{
    public class Equip03_ValDocument_ViewModel : ListViewModel
    {
        /// <summary>
        /// Gets or sets the object that represents the table and its elements.
        /// </summary>
        [Newtonsoft.Json.JsonProperty("Table")]
        public TablePartial<GenioMVC.Models.Assma> Menu { get; set; }

        /// <inheritdoc/>
        public override string TableAlias { get => "assma"; }

        /// <inheritdoc/>
        public override string Uuid { get => "Equip03_ValDocument"; }
        ...
        public Equip03_ValDocument_ViewModel(NavigationContext currentNavigation) : base(currentNavigation)
        {
        ...
        public void LoadToExport(out ListingMVC<CSGenioAassma> listing, out CriteriaSet conditions, out List<Exports.QColumn> columns, NameValueCollection requestValues, bool ajaxRequest = false)
        {
            ...
            if (ajaxRequest && userColumns!=null)
            {
                ...
                foreach (CSGenioAlstcol column in userColumns)
                {
                    ...
                    string areabase = column.ValTabela.ToLower() != "assma" ? CultureInfo.InvariantCulture.TextInfo.ToTitleCase(column.ValTabela) + "." : "";
		...
		public override CriteriaSet BuildCriteriaSet(NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			if(Menu == null)
				Menu = new TablePartial<GenioMVC.Models.Assma>();
			Menu.SetFilters(bool.Parse(requestValues["ValDocument_tableFilters"] ?? "false"), false);
			...
			crs.SubSets.Add(ProcessSearchFilters(Menu, GetSearchColumns(true), requestValues, "ValDocument_"));
		...
		public void Load(int numberListItems, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAassma> Qlisting, ref CriteriaSet conditions)
		{
			...
			if (ajaxRequest)
				this.Navigation.SetValue("requestValues" + "Equip03_ValDocument", requestValues);
			else if (!ajaxRequest && this.Navigation.CheckKey("requestValues" + "Equip03_ValDocument"))
				requestValues = this.Navigation.GetValue<NameValueCollection>("requestValues" + "Equip03_ValDocument");
			...
			var pageNumber = (ajaxRequest && !String.IsNullOrEmpty(requestValues["pValDocument"])) ? int.Parse(requestValues["pValDocument"]) : 1;

			// Added to avoid 0 or -1 pages when setting number of records to -1 to disable pagination
			if (pageNumber < 1)
				pageNumber = 1;

			List<ColumnSort> sorts = GetRequestSorts(this.Menu, "sValDocument", "dValDocument", requestValues, "assma", allSortOrders);
...
FieldRef[] fields = new FieldRef[] { CSGenioAassma.FldCodassma, CSGenioAassma.FldZzstate, CSGenioAassma.FldName, CSGenioAassma.FldDigdocum, CSGenioAassma.FldDigdocumfk, CSGenioAassma.FldNotes };
			...
			//Tooltip for EPHs affecting this viewmodel list
			{
				...
				CSGenioAassma model_limit_area = new CSGenioAassma(UserContext.Current.User);
			...
			CriteriaSet equip03_pseuddocumentConds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
            tableReload &= hasAllRequiredLimits;

// USE /[MANUAL GQT OVERRQ EQUIP03_PSEUDDOCUMENT]/

            // This will happen in case there is an error
            if(equip03_pseuddocumentConds == null)
                return;
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAassma>(false, equip03_pseuddocumentConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_EQUIP03_PSEUDDOCUMENT", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL GQT OVERRQLSTEXP EQUIP03_PSEUDDOCUMENT]/

                conditions = equip03_pseuddocumentConds;
                return;
			}
...
// USE /[MANUAL GQT OVERRQLIST EQUIP03_PSEUDDOCUMENT]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = UserContext.Current.PersistentSupport.getPagingPos(CSGenioAassma.GetInformation(), QMVC_POS_RECORD, sorts, equip03_pseuddocumentConds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioAassma> listing = Models.ModelBase.Where<CSGenioAassma>(false, equip03_pseuddocumentConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_EQUIP03_PSEUDDOCUMENT", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
        ...
        {
            var Elements = new List<Models.Assma>();
            int i = 0;
        ...
        /// to a Models.Assma object.
        /// </summary>
        /// <param name="row">The row.</param>
        private Models.Assma MapEquip03_ValDocument(CSGenioAassma row)
        {
            var model = new Models.Assma(true, _fieldsToSerialize);
            if (row == null)
                return model;
