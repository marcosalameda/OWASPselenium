...
namespace GenioMVC.ViewModels.Asset
{
    public class Equipm_KindeValDesignat_ViewModel : ListViewModel
    {
        ...
        public Equipm_KindeValDesignat_ViewModel(NavigationContext currentNavigation) : base(currentNavigation)
        {
...
FieldRef[] fields = new FieldRef[] { CSGenioAkinde.FldCodkinde, CSGenioAkinde.FldZzstate, CSGenioAkinde.FldDesignat };
			...
			CriteriaSet equipm__kindedesignatConds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
            tableReload &= hasAllRequiredLimits;

// USE /[MANUAL GQT OVERRQ EQUIPM_DESIGNAT]/

            // This will happen in case there is an error
            if(equipm__kindedesignatConds == null)
                return;
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAkinde>(false, equipm__kindedesignatConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_EQUIPM__KINDEDESIGNAT", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL GQT OVERRQLSTEXP EQUIPM_DESIGNAT]/

                conditions = equipm__kindedesignatConds;
                return;
			}
...
// USE /[MANUAL GQT OVERRQLIST EQUIPM_DESIGNAT]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = UserContext.Current.PersistentSupport.getPagingPos(CSGenioAkinde.GetInformation(), QMVC_POS_RECORD, sorts, equipm__kindedesignatConds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioAkinde> listing = Models.ModelBase.Where<CSGenioAkinde>(false, equipm__kindedesignatConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_EQUIPM__KINDEDESIGNAT", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
