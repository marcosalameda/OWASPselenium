...
namespace GenioMVC.ViewModels.Facil
{
    public class Facil_FactyValType_ViewModel : ListViewModel
    {
        ...
        public Facil_FactyValType_ViewModel(NavigationContext currentNavigation) : base(currentNavigation)
        {
...
FieldRef[] fields = new FieldRef[] { CSGenioAfacty.FldCodfacty, CSGenioAfacty.FldZzstate, CSGenioAfacty.FldType, CSGenioAfacty.FldLayrname, CSGenioAfacty.FldIconurl, CSGenioAfacty.FldShadowur, CSGenioAfacty.FldIconancx, CSGenioAfacty.FldIconancy, CSGenioAfacty.FldIconheig, CSGenioAfacty.FldIconwid, CSGenioAfacty.FldPopupanx, CSGenioAfacty.FldPopupany, CSGenioAfacty.FldShadowax, CSGenioAfacty.FldShadoway, CSGenioAfacty.FldShadowhe, CSGenioAfacty.FldShadowwi };
			...
			CriteriaSet facil___factytype____Conds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
            tableReload &= hasAllRequiredLimits;

// USE /[MANUAL GQT OVERRQ FACIL_FACTYTYPE]/

            // This will happen in case there is an error
            if(facil___factytype____Conds == null)
                return;
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAfacty>(false, facil___factytype____Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_FACIL___FACTYTYPE____", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL GQT OVERRQLSTEXP FACIL_FACTYTYPE]/

                conditions = facil___factytype____Conds;
                return;
			}
...
// USE /[MANUAL GQT OVERRQLIST FACIL_FACTYTYPE]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = UserContext.Current.PersistentSupport.getPagingPos(CSGenioAfacty.GetInformation(), QMVC_POS_RECORD, sorts, facil___factytype____Conds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioAfacty> listing = Models.ModelBase.Where<CSGenioAfacty>(false, facil___factytype____Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_FACIL___FACTYTYPE____", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
