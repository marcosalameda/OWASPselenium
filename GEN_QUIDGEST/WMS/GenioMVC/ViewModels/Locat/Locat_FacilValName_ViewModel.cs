...
namespace GenioMVC.ViewModels.Locat
{
    public class Locat_FacilValName_ViewModel : ListViewModel
    {
        ...
        public Locat_FacilValName_ViewModel(NavigationContext currentNavigation) : base(currentNavigation)
        {
...
FieldRef[] fields = new FieldRef[] { CSGenioAfacil.FldCodfacil, CSGenioAfacil.FldZzstate, CSGenioAfacil.FldName };
			...
			CriteriaSet locat___facilname____Conds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
            tableReload &= hasAllRequiredLimits;

// USE /[MANUAL GQT OVERRQ LOCAT_NAME]/

            // This will happen in case there is an error
            if(locat___facilname____Conds == null)
                return;
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAfacil>(false, locat___facilname____Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_LOCAT___FACILNAME____", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL GQT OVERRQLSTEXP LOCAT_NAME]/

                conditions = locat___facilname____Conds;
                return;
			}
...
// USE /[MANUAL GQT OVERRQLIST LOCAT_NAME]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = UserContext.Current.PersistentSupport.getPagingPos(CSGenioAfacil.GetInformation(), QMVC_POS_RECORD, sorts, locat___facilname____Conds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioAfacil> listing = Models.ModelBase.Where<CSGenioAfacil>(false, locat___facilname____Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_LOCAT___FACILNAME____", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
