...
namespace GenioMVC.ViewModels.Locat
{
    public class Locat_ValLocalext_ViewModel : ListViewModel
    {
        ...
        public Locat_ValLocalext_ViewModel(NavigationContext currentNavigation) : base(currentNavigation)
        {
		...
		public override CriteriaSet BuildCriteriaSet(NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			Menu.SetFilters(bool.Parse(requestValues["ValLocalext_tableFilters"] ?? "false"), false);
			...
			crs.SubSets.Add(ProcessSearchFilters(Menu, GetSearchColumns(true), requestValues, "ValLocalext_"));
			...
			{
				// EPH
				crs = Models.Lcext.AddEPH<CSGenioAlcext>(ref u, crs, "IBL_LOCAT___PSEUDLOCALEXT");
			...
			if (tableReload)
			{
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					crs.Equals(Models.Lcext.AddEPH<CSGenioAlcext>(ref u, null, "IBL_LOCAT___PSEUDLOCALEXT"));
			}
		...
		public void Load(int numberListItems, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAlcext> Qlisting, ref CriteriaSet conditions)
		{
			...
			if (ajaxRequest)
				this.Navigation.SetValue("requestValues" + "Locat_ValLocalext", requestValues);
			else if (!ajaxRequest && this.Navigation.CheckKey("requestValues" + "Locat_ValLocalext"))
				requestValues = this.Navigation.GetValue<NameValueCollection>("requestValues" + "Locat_ValLocalext");
			...
			var pageNumber = (ajaxRequest && !String.IsNullOrEmpty(requestValues["pValLocalext"])) ? int.Parse(requestValues["pValLocalext"]) : 1;

			// Added to avoid 0 or -1 pages when setting number of records to -1 to disable pagination
			if (pageNumber < 1)
				pageNumber = 1;

			List<ColumnSort> sorts = GetRequestSorts(this.Menu, "sValLocalext", "dValLocalext", requestValues, "lcext", allSortOrders);
...
FieldRef[] fields = new FieldRef[] { CSGenioAlcext.FldCodlcext, CSGenioAlcext.FldZzstate, CSGenioAlcext.FldGlnext, CSGenioAlcext.FldSpacetyp, CSGenioAlcext.FldSpaceobs };
			...
			CriteriaSet locat___pseudlocalextConds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
            tableReload &= hasAllRequiredLimits;

// USE /[MANUAL GQT OVERRQ LOCAT_PSEUDLOCALEXT]/

            // This will happen in case there is an error
            if(locat___pseudlocalextConds == null)
                return;
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAlcext>(false, locat___pseudlocalextConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_LOCAT___PSEUDLOCALEXT", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL GQT OVERRQLSTEXP LOCAT_PSEUDLOCALEXT]/

                conditions = locat___pseudlocalextConds;
                return;
			}
			...
			if (tableReload)
			{
// USE /[MANUAL GQT OVERRQLIST LOCAT_PSEUDLOCALEXT]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = UserContext.Current.PersistentSupport.getPagingPos(CSGenioAlcext.GetInformation(), QMVC_POS_RECORD, sorts, locat___pseudlocalextConds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioAlcext> listing = Models.ModelBase.Where<CSGenioAlcext>(false, locat___pseudlocalextConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_LOCAT___PSEUDLOCALEXT", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
				...
				Menu.Elements = MapLocat_ValLocalext(listing);

				Menu.Identifier = "IBL_LOCAT___PSEUDLOCALEXT";

				// Last updated by [CJP] at [2015.02.03]
				// Adds the identifier to each element
				foreach (var element in Menu.Elements)
					element.Identifier = "IBL_LOCAT___PSEUDLOCALEXT";
            ...
            if (Qlisting.Rows != null)
            {
                foreach (var row in Qlisting.Rows)
				{
					if (Qlisting.NumRegs > 0 && i >= Qlisting.NumRegs) // Copiado da versão antiga do RowsToViewModels
						break;
					Elements.Add(MapLocat_ValLocalext(row));
					i++;
				}
            }
        ...
        private Models.Lcext MapLocat_ValLocalext(CSGenioAlcext row)
        {
