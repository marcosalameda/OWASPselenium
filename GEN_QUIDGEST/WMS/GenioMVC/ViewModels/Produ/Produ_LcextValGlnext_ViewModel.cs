...
{
    public class Produ_LcextValGlnext_ViewModel : ListViewModel
    {
        /// <summary>
        ...
        /// <inheritdoc/>
        public override string Uuid { get => "Produ_LcextValGlnext"; }
        ...
        public Produ_LcextValGlnext_ViewModel(NavigationContext currentNavigation) : base(currentNavigation)
        {
		...
		public override CriteriaSet BuildCriteriaSet(NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			Menu.SetFilters(bool.Parse(requestValues["Produ_LcextValGlnext_tableFilters"] ?? "false"), false);
			...
			crs.SubSets.Add(ProcessSearchFilters(Menu, GetSearchColumns(true), requestValues, "Produ_LcextValGlnext_"));
		...
		public void Load(int numberListItems, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAlcext> Qlisting, ref CriteriaSet conditions)
		{
			...
			if (ajaxRequest)
				this.Navigation.SetValue("requestValues" + "Produ_LcextValGlnext", requestValues);
			else if (!ajaxRequest && this.Navigation.CheckKey("requestValues" + "Produ_LcextValGlnext"))
				requestValues = this.Navigation.GetValue<NameValueCollection>("requestValues" + "Produ_LcextValGlnext");
			...
			var pageNumber = (ajaxRequest && !String.IsNullOrEmpty(requestValues["pProdu_LcextValGlnext"])) ? int.Parse(requestValues["pProdu_LcextValGlnext"]) : 1;

			// Added to avoid 0 or -1 pages when setting number of records to -1 to disable pagination
			if (pageNumber < 1)
				pageNumber = 1;

			List<ColumnSort> sorts = GetRequestSorts(this.Menu, "sProdu_LcextValGlnext", "dProdu_LcextValGlnext", requestValues, "lcext", allSortOrders);
...
FieldRef[] fields = new FieldRef[] { CSGenioAlcext.FldCodlcext, CSGenioAlcext.FldZzstate, CSGenioAlcext.FldGlnext };
			...
			CriteriaSet produ___lcextglnext__Conds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
            tableReload &= hasAllRequiredLimits;

// USE /[MANUAL GQT OVERRQ PRODU_GLNEXT]/

            // This will happen in case there is an error
            if(produ___lcextglnext__Conds == null)
                return;
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAlcext>(false, produ___lcextglnext__Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_PRODU___LCEXTGLNEXT__", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL GQT OVERRQLSTEXP PRODU_GLNEXT]/

                conditions = produ___lcextglnext__Conds;
                return;
			}
...
// USE /[MANUAL GQT OVERRQLIST PRODU_GLNEXT]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = UserContext.Current.PersistentSupport.getPagingPos(CSGenioAlcext.GetInformation(), QMVC_POS_RECORD, sorts, produ___lcextglnext__Conds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioAlcext> listing = Models.ModelBase.Where<CSGenioAlcext>(false, produ___lcextglnext__Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_PRODU___LCEXTGLNEXT__", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
        ...
        private List<Models.Lcext> MapProdu_LcextValGlnext(ListingMVC<CSGenioAlcext> Qlisting)
        {
        ...
        private Models.Lcext MapProdu_LcextValGlnext(CSGenioAlcext row)
        {
