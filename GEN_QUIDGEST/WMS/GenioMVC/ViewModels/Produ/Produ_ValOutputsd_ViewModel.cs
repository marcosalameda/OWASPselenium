...
{
    public class Produ_ValOutputsd_ViewModel : ListViewModel
    {
        /// <summary>
        ...
        /// <inheritdoc/>
        public override string Uuid { get => "Produ_ValOutputsd"; }
        ...
        public Produ_ValOutputsd_ViewModel(NavigationContext currentNavigation) : base(currentNavigation)
        {
        ...
        public override List<Exports.QColumn> GetColumnsToExport(bool ajaxRequest = false)
        {
            ...
            {
                new Exports.QColumn(CSGenioAdilin.FldInstant, FieldType.DATAHORA, Resources.Resources.INSTANT35907, 16, 0, true),
		...
		public override CriteriaSet BuildCriteriaSet(NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			Menu.SetFilters(bool.Parse(requestValues["ValOutputsd_tableFilters"] ?? "false"), false);
			...
			crs.SubSets.Add(ProcessSearchFilters(Menu, GetSearchColumns(true), requestValues, "ValOutputsd_"));
			...
			if(this.ValCodprodu != null)
				crs.Equal(CSGenioAdilin.FldCodprodu, this.ValCodprodu);
		...
		public void Load(int numberListItems, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAdilin> Qlisting, ref CriteriaSet conditions)
		{
			...
			if (ajaxRequest)
				this.Navigation.SetValue("requestValues" + "Produ_ValOutputsd", requestValues);
			else if (!ajaxRequest && this.Navigation.CheckKey("requestValues" + "Produ_ValOutputsd"))
				requestValues = this.Navigation.GetValue<NameValueCollection>("requestValues" + "Produ_ValOutputsd");
			...
			var pageNumber = (ajaxRequest && !String.IsNullOrEmpty(requestValues["pValOutputsd"])) ? int.Parse(requestValues["pValOutputsd"]) : 1;

			// Added to avoid 0 or -1 pages when setting number of records to -1 to disable pagination
			if (pageNumber < 1)
				pageNumber = 1;

			List<ColumnSort> sorts = GetRequestSorts(this.Menu, "sValOutputsd", "dValOutputsd", requestValues, "dilin", allSortOrders);
...
FieldRef[] fields = new FieldRef[] { CSGenioAdilin.FldCoddilin, CSGenioAdilin.FldZzstate, CSGenioAdilin.FldInstant, CSGenioAdilin.FldCoddispa, CSGenioAdispa.FldCoddispa, CSGenioAdispa.FldDispanr, CSGenioAentit.FldCodentit, CSGenioAentit.FldName, CSGenioAdilin.FldLinenumb, CSGenioAdilin.FldOrdered, CSGenioAdilin.FldDelivere, CSGenioAdilin.FldOutstand };
			...
			CriteriaSet produ___pseudoutputsdConds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
            tableReload &= hasAllRequiredLimits;

// USE /[MANUAL GQT OVERRQ PRODU_PSEUDOUTPUTSD]/

            // This will happen in case there is an error
            if(produ___pseudoutputsdConds == null)
                return;
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAdilin>(false, produ___pseudoutputsdConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_PRODU___PSEUDOUTPUTSD", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL GQT OVERRQLSTEXP PRODU_PSEUDOUTPUTSD]/

                conditions = produ___pseudoutputsdConds;
                return;
			}
...
// USE /[MANUAL GQT OVERRQLIST PRODU_PSEUDOUTPUTSD]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = UserContext.Current.PersistentSupport.getPagingPos(CSGenioAdilin.GetInformation(), QMVC_POS_RECORD, sorts, produ___pseudoutputsdConds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioAdilin> listing = Models.ModelBase.Where<CSGenioAdilin>(false, produ___pseudoutputsdConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_PRODU___PSEUDOUTPUTSD", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
        ...
        private Models.Dilin MapProdu_ValOutputsd(CSGenioAdilin row)
        {
