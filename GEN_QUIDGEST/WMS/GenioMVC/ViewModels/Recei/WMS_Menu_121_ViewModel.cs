...
{
    public class WMS_Menu_121_ViewModel : ListViewModel
    {
        /// <summary>
        ...
        /// <inheritdoc/>
        public override string Uuid { get => "d5a95839-cfc6-40f7-89d3-b36149d26ab4"; }
...
FieldRef[] fields = new FieldRef[] { CSGenioArecei.FldCodrecei, CSGenioArecei.FldZzstate, CSGenioArecei.FldNumber, CSGenioArecei.FldDtreceip, CSGenioArecei.FldCodentit, CSGenioAentit.FldCodentit, CSGenioAentit.FldName, CSGenioArecei.FldDtcheck, CSGenioArecei.FldChecked, CSGenioArecei.FldTocheck, CSGenioArecei.FldDtstorag };
        ...
        public WMS_Menu_121_ViewModel(NavigationContext currentNavigation) : base(currentNavigation)
        {
		...
		public override CriteriaSet BuildCriteriaSet(NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			Menu.SetFilters(bool.Parse(requestValues["WMS_Menu_121_tableFilters"] ?? "false"), true);
			...
			crs.SubSets.Add(ProcessSearchFilters(Menu, GetSearchColumns(true), requestValues, "WMS_Menu_121_"));
			...
			CriteriaSet subfilters = CriteriaSet.And();
			{
				...
				if (requestValues["filter_WMS_Menu_121_VERIFICATI"] != null)
					filter_WMS_Menu_121_VERIFICATI_1 = requestValues["filter_WMS_Menu_121_VERIFICATI"].Contains("1");
				else if (requestValues["filter_WMS_Menu_121_VERIFICATI"] == null)
					filter_WMS_Menu_121_VERIFICATI_1 = true;
				Navigation.SetValue("filter_WMS_Menu_121_VERIFICATI_1", filter_WMS_Menu_121_VERIFICATI_1);
				...
				if (requestValues["filter_WMS_Menu_121_VERIFICATI"] != null)
					filter_WMS_Menu_121_VERIFICATI_2 = requestValues["filter_WMS_Menu_121_VERIFICATI"].Contains("2");
				Navigation.SetValue("filter_WMS_Menu_121_VERIFICATI_2", filter_WMS_Menu_121_VERIFICATI_2);
		...
		public void Load(int numberListItems, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioArecei> Qlisting, ref CriteriaSet conditions)
		{
			...
			if (ajaxRequest)
				this.Navigation.SetValue("requestValues" + "WMS_Menu_121", requestValues);
			else if (!ajaxRequest && this.Navigation.CheckKey("requestValues" + "WMS_Menu_121"))
				requestValues = this.Navigation.GetValue<NameValueCollection>("requestValues" + "WMS_Menu_121");
			...
			var pageNumber = (ajaxRequest && !String.IsNullOrEmpty(requestValues["pWMS_Menu_121"])) ? int.Parse(requestValues["pWMS_Menu_121"]) : 1;

			// Added to avoid 0 or -1 pages when setting number of records to -1 to disable pagination
			if (pageNumber < 1)
				pageNumber = 1;

			List<ColumnSort> sorts = GetRequestSorts(this.Menu, "sWMS_Menu_121", "dWMS_Menu_121", requestValues, "recei", allSortOrders);
...
FieldRef[] fields = new FieldRef[] { CSGenioArecei.FldCodrecei, CSGenioArecei.FldZzstate, CSGenioArecei.FldNumber, CSGenioArecei.FldDtreceip, CSGenioArecei.FldCodentit, CSGenioAentit.FldCodentit, CSGenioAentit.FldName, CSGenioArecei.FldDtcheck, CSGenioArecei.FldChecked, CSGenioArecei.FldTocheck, CSGenioArecei.FldDtstorag };
			...
			//Tooltip for EPHs affecting this viewmodel list
			{
				...
				CSGenioArecei model_limit_area = new CSGenioArecei(UserContext.Current.User);
			...
			CriteriaSet wms_menu_121Conds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
            tableReload &= hasAllRequiredLimits;

// USE /[MANUAL WMS OVERRQ 121]/

            // This will happen in case there is an error
            if(wms_menu_121Conds == null)
                return;
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioArecei>(false, wms_menu_121Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "ML121", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL WMS OVERRQLSTEXP 121]/

                conditions = wms_menu_121Conds;
                return;
			}
...
// USE /[MANUAL WMS OVERRQLIST 121]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = UserContext.Current.PersistentSupport.getPagingPos(CSGenioArecei.GetInformation(), QMVC_POS_RECORD, sorts, wms_menu_121Conds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioArecei> listing = Models.ModelBase.Where<CSGenioArecei>(false, wms_menu_121Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "ML121", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
        ...
        private List<Models.Recei> MapWMS_Menu_121(ListingMVC<CSGenioArecei> Qlisting)
        {
        ...
        private Models.Recei MapWMS_Menu_121(CSGenioArecei row)
        {
            var model = new Models.Recei(true, _fieldsToSerialize);
            if (row == null)
                return model;
