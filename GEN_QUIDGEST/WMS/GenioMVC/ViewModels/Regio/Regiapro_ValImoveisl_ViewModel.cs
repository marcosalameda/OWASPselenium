...
namespace GenioMVC.ViewModels.Regio
{
    public class Regiapro_ValImoveisl_ViewModel : ListViewModel
    {
        ...
        public Regiapro_ValImoveisl_ViewModel(NavigationContext currentNavigation) : base(currentNavigation)
        {
...
FieldRef[] fields = new FieldRef[] { CSGenioApropr.FldCodpropr, CSGenioApropr.FldZzstate, CSGenioApropr.FldName, CSGenioApropr.FldPrecoest, CSGenioApropr.FldPhotogra, CSGenioApropr.FldDescript, CSGenioApropr.FldCoordgeo, CSGenioApropr.FldCodpais1, CSGenioApais1.FldCodcntry, CSGenioApais1.FldCountry, CSGenioApropr.FldCodcntry, CSGenioAcntry.FldCodcntry, CSGenioAcntry.FldCountry };
			...
			CriteriaSet regiapropseudimoveislConds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
            tableReload &= hasAllRequiredLimits;

// USE /[MANUAL GQT OVERRQ REGIAPRO_PSEUDIMOVEISL]/

            // This will happen in case there is an error
            if(regiapropseudimoveislConds == null)
                return;
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioApropr>(false, regiapropseudimoveislConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_REGIAPROPSEUDIMOVEISL", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL GQT OVERRQLSTEXP REGIAPRO_PSEUDIMOVEISL]/

                conditions = regiapropseudimoveislConds;
                return;
			}
...
// USE /[MANUAL GQT OVERRQLIST REGIAPRO_PSEUDIMOVEISL]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = UserContext.Current.PersistentSupport.getPagingPos(CSGenioApropr.GetInformation(), QMVC_POS_RECORD, sorts, regiapropseudimoveislConds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioApropr> listing = Models.ModelBase.Where<CSGenioApropr>(false, regiapropseudimoveislConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_REGIAPROPSEUDIMOVEISL", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
