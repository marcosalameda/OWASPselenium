...
{
    public class Relin_ReceiValNumber_ViewModel : ListViewModel
    {
        /// <summary>
        ...
        /// <inheritdoc/>
        public override string Uuid { get => "Relin_ReceiValNumber"; }
        ...
        public Relin_ReceiValNumber_ViewModel(NavigationContext currentNavigation) : base(currentNavigation)
        {
		...
		public override CriteriaSet BuildCriteriaSet(NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			Menu.SetFilters(bool.Parse(requestValues["Relin_ReceiValNumber_tableFilters"] ?? "false"), false);
			...
			crs.SubSets.Add(ProcessSearchFilters(Menu, GetSearchColumns(true), requestValues, "Relin_ReceiValNumber_"));
		...
		public void Load(int numberListItems, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioArecei> Qlisting, ref CriteriaSet conditions)
		{
			...
			if (ajaxRequest)
				this.Navigation.SetValue("requestValues" + "Relin_ReceiValNumber", requestValues);
			else if (!ajaxRequest && this.Navigation.CheckKey("requestValues" + "Relin_ReceiValNumber"))
				requestValues = this.Navigation.GetValue<NameValueCollection>("requestValues" + "Relin_ReceiValNumber");
			...
			var pageNumber = (ajaxRequest && !String.IsNullOrEmpty(requestValues["pRelin_ReceiValNumber"])) ? int.Parse(requestValues["pRelin_ReceiValNumber"]) : 1;

			// Added to avoid 0 or -1 pages when setting number of records to -1 to disable pagination
			if (pageNumber < 1)
				pageNumber = 1;

			List<ColumnSort> sorts = GetRequestSorts(this.Menu, "sRelin_ReceiValNumber", "dRelin_ReceiValNumber", requestValues, "recei", allSortOrders);
...
FieldRef[] fields = new FieldRef[] { CSGenioArecei.FldCodrecei, CSGenioArecei.FldZzstate, CSGenioArecei.FldNumber };
			...
			CriteriaSet relin___receinumber__Conds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
            tableReload &= hasAllRequiredLimits;

// USE /[MANUAL GQT OVERRQ RELIN_NUMBER]/

            // This will happen in case there is an error
            if(relin___receinumber__Conds == null)
                return;
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioArecei>(false, relin___receinumber__Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_RELIN___RECEINUMBER__", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL GQT OVERRQLSTEXP RELIN_NUMBER]/

                conditions = relin___receinumber__Conds;
                return;
			}
...
// USE /[MANUAL GQT OVERRQLIST RELIN_NUMBER]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = UserContext.Current.PersistentSupport.getPagingPos(CSGenioArecei.GetInformation(), QMVC_POS_RECORD, sorts, relin___receinumber__Conds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioArecei> listing = Models.ModelBase.Where<CSGenioArecei>(false, relin___receinumber__Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_RELIN___RECEINUMBER__", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
        ...
        private Models.Recei MapRelin_ReceiValNumber(CSGenioArecei row)
        {
