...
namespace CSGenio.business
{
	...
	/// Facility
	/// </summary>
	public class CSGenioAfaci2 : DbArea	{
		/// <summary>
		/// Meta-information on this area
		/// </summary>
		protected readonly static AreaInfo informacao = InicializaAreaInfo();

		public CSGenioAfaci2(User user, string module)
		{
		...
		public CSGenioAfaci2(User user) : this(user, user.CurrentModule)
		{
		}
...
#pragma warning restore CS0168, S1481 // Variable is declared but never used
			//- - - - - - - - - - - - - - - - - - -
			Qfield = new Field("codfacil", FieldType.CHAVE_PRIMARIA_GUID);
			...
			//- - - - - - - - - - - - - - - - - - -
			Qfield = new Field("incorpor", FieldType.DATA);
			Qfield.FieldDescription = "Incorporation";
			Qfield.FieldSize =  8;
			Qfield.Alias = info.Alias;
			Qfield.MQueue = false;
			Qfield.CavDesignation = "INCORPORATION10135";
			...
			Qfield.Alias = info.Alias;
			Qfield.MQueue = false;
			Qfield.CavDesignation = "FACILITY_NAME19514";

			Qfield.Dupmsg = "";
			info.RegisterFieldDB(Qfield);

			//- - - - - - - - - - - - - - - - - - -
			Qfield = new Field("faciltyp", FieldType.ARRAY_COD_TEXTO);
			...
			Qfield.Dupmsg = "";
            Qfield.ArrayName = "dbo.GetValArrayCfaciltyp";
            Qfield.ArrayClassName = "Faciltyp";
			info.RegisterFieldDB(Qfield);

			//- - - - - - - - - - - - - - - - - - -
			Qfield = new Field("address", FieldType.MEMO);
			...
			//- - - - - - - - - - - - - - - - - - -
			Qfield = new Field("codfacty", FieldType.CHAVE_FALSA_GUID);
			Qfield.FieldDescription = ">>Facility type";
			Qfield.FieldSize =  36;
			Qfield.Alias = info.Alias;
			...
			//- - - - - - - - - - - - - - - - - - -
			Qfield = new Field("gpsinput", FieldType.ARRAY_COD_TEXTO);
			...
			Qfield.Dupmsg = "";
            Qfield.ArrayName = "dbo.GetValArrayCgpsinput";
            Qfield.ArrayClassName = "Gpsinput";
			info.RegisterFieldDB(Qfield);

			//- - - - - - - - - - - - - - - - - - -
			Qfield = new Field("latitude", FieldType.NUMERO);
			...
			//- - - - - - - - - - - - - - - - - - -
			Qfield = new Field("longitud", FieldType.NUMERO);
			...
			//- - - - - - - - - - - - - - - - - - -
			Qfield = new Field("geocoori", FieldType.GEOGRAPHY);
			...
			//- - - - - - - - - - - - - - - - - - -
			Qfield = new Field("geocoord", FieldType.GEOGRAPHY);
			Qfield.FieldDescription = "Geographical coordinate";
			Qfield.FieldSize =  50;
			Qfield.Alias = info.Alias;
			Qfield.MQueue = false;
			...
			argumentsListByArea.Add(new ByAreaArguments(new string[] {"gpsinput","latitude","longitud","geocoori"}, new int[] {0,1,2,3}, "faci2", "codfacil"));
		...
		private static void InicializaRelacoes(AreaInfo info)
		{
			...
			info.ChildTable[0]= new ChildRelation("locat", new String[] {"codfacil"}, DeleteProc.NA);
			info.ChildTable[1]= new ChildRelation("entit", new String[] {"firstfacilitie","lastfacilitie"}, DeleteProc.NA);
		...
		private static void InicializaFormulas(AreaInfo info)
		{
			...
			// [FACIL->LONGITUD]>=-180 && [FACIL->LONGITUD]<=180
			{
			List<ByAreaArguments> argumentsListByArea = new List<ByAreaArguments>();
			argumentsListByArea= new List<ByAreaArguments>();
			argumentsListByArea.Add(new ByAreaArguments(new string[] {"longitud","longitud"},new int[] {0,1},"faci2","codfacil"));
			...
			writeCondition.Field = info.DBFields["longitud"];
			conditions.Add(writeCondition);
			}

			// [FACIL->LATITUDE]>=-90 && [FACIL->LATITUDE]<=90
			{
			List<ByAreaArguments> argumentsListByArea = new List<ByAreaArguments>();
			argumentsListByArea= new List<ByAreaArguments>();
			argumentsListByArea.Add(new ByAreaArguments(new string[] {"latitude","latitude"},new int[] {0,1},"faci2","codfacil"));
			ConditionFormula writeCondition = new ConditionFormula(argumentsListByArea, 2, delegate(object []args,User user,string module,PersistentSupport sp) {
            ...
            writeCondition.Type =  ConditionType.ERROR;
            writeCondition.Validate = false;
			writeCondition.Field = info.DBFields["latitude"];
			conditions.Add(writeCondition);
			}
		...
		private static AreaInfo InicializaAreaInfo()
		{
			...
			info.QSystem="GQT";
			info.TableName="gqtfacility";
			info.ShadowTabName="";
			info.ShadowTabKeyName="";

			info.PrimaryKeyName="codfacil";
		...
		private static FieldRef m_fldCodfacil = new FieldRef("faci2", "codfacil");

		/// <summary>Field : "" Tipo: "+" Formula:  ""</summary>
		public string ValCodfacil
		{
			get { return (string)returnValueField(FldCodfacil); }
		...
		private static FieldRef m_fldCodentit = new FieldRef("faci2", "codentit");
		...
		private static FieldRef m_fldIncorpor = new FieldRef("faci2", "incorpor");

		/// <summary>Field : "Incorporation" Tipo: "D" Formula:  ""</summary>
		public DateTime ValIncorpor
		{
			get { return (DateTime)returnValueField(FldIncorpor); }
		...
		private static FieldRef m_fldName = new FieldRef("faci2", "name");
		...
		private static FieldRef m_fldFaciltyp = new FieldRef("faci2", "faciltyp");

		/// <summary>Field : "Facility type" Tipo: "AC" Formula:  ""</summary>
		public string ValFaciltyp
		{
			get { return (string)returnValueField(FldFaciltyp); }
		...
		private static FieldRef m_fldAddress = new FieldRef("faci2", "address");
		...
		private static FieldRef m_fldCodfacty = new FieldRef("faci2", "codfacty");
		...
		private static FieldRef m_fldImage = new FieldRef("faci2", "image");
		...
		private static FieldRef m_fldGpsinput = new FieldRef("faci2", "gpsinput");

		/// <summary>Field : "GPS input" Tipo: "AC" Formula:  ""</summary>
		public string ValGpsinput
		{
			get { return (string)returnValueField(FldGpsinput); }
		...
		private static FieldRef m_fldLatitude = new FieldRef("faci2", "latitude");
		...
		private static FieldRef m_fldLongitud = new FieldRef("faci2", "longitud");
		...
		private static FieldRef m_fldGeocoori = new FieldRef("faci2", "geocoori");

		/// <summary>Field : "Geographical coordinate" Tipo: "GG" Formula:  ""</summary>
		public string ValGeocoori
		{
		...
		private static FieldRef m_fldGeocoord = new FieldRef("faci2", "geocoord");

		/// <summary>Field : "Geographical coordinate" Tipo: "GG" Formula: + "iif([FACI2->GPSINPUT]=="L",GetGeoFromLatLng([FACI2->LATITUDE],[FACI2->LONGITUD]),[FACI2->GEOCOORI])"</summary>
		public string ValGeocoord
		{
			get { return (string)returnValueField(FldGeocoord); }
        ...
        /// <remarks>Persistence operations should not be used on a partially positioned register</remarks>
        [Obsolete("Use List<CSGenioAfaci2> searchList(PersistentSupport sp, User user, CriteriaSet where, string []fields) instead")]
        public static List<CSGenioAfaci2> searchList(PersistentSupport sp, User user, string where, string []fields = null)
        {
            return sp.searchListWhere<CSGenioAfaci2>(where, user, fields);
        }
        ...
        /// <returns>A list of area records with all fields populated</returns>
        /// <remarks>Persistence operations should not be used on a partially positioned register</remarks>
        public static List<CSGenioAfaci2> searchList(PersistentSupport sp, User user, CriteriaSet where, string[] fields = null, bool distinct = false, bool noLock = false)
        {
				return sp.searchListWhere<CSGenioAfaci2>(where, user, fields, distinct, noLock);
        }
        ...
        /// <returns>A list of area records with all fields populated</returns>
        /// <remarks>Persistence operations should not be used on a partially positioned register</remarks>
        public static void searchListAdvancedWhere(PersistentSupport sp, User user, CriteriaSet where, ListingMVC<CSGenioAfaci2> listing)
        {
			sp.searchListAdvancedWhere<CSGenioAfaci2>(where, listing);
        }
