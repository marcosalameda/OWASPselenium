...
namespace CSGenio.business
{
	...
	public class CSGenioAorgan : DbArea	{
		...
		public CSGenioAorgan(User user, string module)
		{
		...
		public CSGenioAorgan(User user) : this(user, user.CurrentModule)
		{
		}
...
#pragma warning restore CS0168, S1481 // Variable is declared but never used
			//- - - - - - - - - - - - - - - - - - -
			Qfield = new Field("codorgan", FieldType.CHAVE_PRIMARIA_GUID);
			...
			//- - - - - - - - - - - - - - - - - - -
			Qfield = new Field("organiza", FieldType.TEXTO);
			Qfield.FieldDescription = "Organization";
			Qfield.FieldSize =  85;
			Qfield.Alias = info.Alias;
			Qfield.MQueue = false;
			Qfield.CavDesignation = "ORGANIZATION64123";
		...
		private static void InicializaRelacoes(AreaInfo info)
		{
			...
			info.ChildTable[0]= new ChildRelation("sale", new String[] {"codorgan"}, DeleteProc.NA);
			info.ChildTable[1]= new ChildRelation("pworg", new String[] {"codorgan"}, DeleteProc.NA);
			info.ChildTable[2]= new ChildRelation("sales", new String[] {"codorgan"}, DeleteProc.NA);
		...
		private static AreaInfo InicializaAreaInfo()
		{
			...
			info.QSystem="GQT";
			info.TableName="gqtorgan";
			info.ShadowTabName="";
			info.ShadowTabKeyName="";

			info.PrimaryKeyName="codorgan";
			info.HumanKeyName="organiza,".TrimEnd(',');
			info.Alias="organ";
			info.IsDomain = true;
		...
		private static FieldRef m_fldCodorgan = new FieldRef("organ", "codorgan");
		...
		private static FieldRef m_fldSigla = new FieldRef("organ", "sigla");
		...
		private static FieldRef m_fldZzstate = new FieldRef("organ", "zzstate");
        ...
        public static CSGenioAorgan search(PersistentSupport sp, string key, User user, string[] fields = null)
        {
			if (string.IsNullOrEmpty(key))
				return null;

		    CSGenioAorgan area = new CSGenioAorgan(user, user.CurrentModule);
        ...
        /// <returns>A list of area records with all fields populated</returns>
        /// <remarks>Persistence operations should not be used on a partially positioned register</remarks>
        public static List<CSGenioAorgan> searchList(PersistentSupport sp, User user, CriteriaSet where, string[] fields = null, bool distinct = false, bool noLock = false)
        {
				return sp.searchListWhere<CSGenioAorgan>(where, user, fields, distinct, noLock);
        }
        ...
        /// <returns>A list of area records with all fields populated</returns>
        /// <remarks>Persistence operations should not be used on a partially positioned register</remarks>
        public static void searchListAdvancedWhere(PersistentSupport sp, User user, CriteriaSet where, ListingMVC<CSGenioAorgan> listing)
        {
			sp.searchListAdvancedWhere<CSGenioAorgan>(where, listing);
        }
