...
namespace CSGenio.business
{
    ...
    /// </summary>
     public class Q_NOTIF_1_EQUIPORDEVOLV : Notification
	{
		/// <summary>
		/// Notification implementation
        /// </summary>
		public Q_NOTIF_1_EQUIPORDEVOLV()
        {
			codtpnot = "a43e624a-f68b-4352-b4f6-9fbc70fce6c7";
			IDNotif = "NOTIF_1_EQUIPORDEVOLV";
			Help = "";
			Notif = "Equipamento a devolver";

			//Manual query
            //está 13 porque existem rotinas no BO usam este valor por default, assim mantêm-se tudo coerente, i.e., exigência de ter pelo menos 13 chars
            String queryname = "EQUIPORDEVOLV";
            if (queryname.ToUpper().Length < 13)
                queryname = FormatTagId(queryname);
            Hashtable manualQueries = PersistentSupport.getManualQueries();
            GenioQuery = (ManualQuery)manualQueries[queryname];

			//Tags mapping
			TagsFieldMapping = new List<Tag>
			{
				Tag_build("[DEVOLVID]", "[DEVOLVID]", ""),
				Tag_build("[DTDEVOLU]", "[DTDEVOLU]", ""),
				Tag_build("[INICIO]", "[INICIO]", ""),
				Tag_build("[FIM]", "[FIM]", ""),
				Tag_build("[NRCOMODA]", "[NRCOMODA]", ""),
 			};

			//Allowed destinations mapping
            AllowedDestinations = new List<AllowedDestination>
			{
				AllowedDestination_build("1cdff3ea-8133-4521-92bb-f38d7f061f27","Comodatário", "PESS2", "CODPESSO", "[EMAIL]", "[CODPESS2]"),
  			};
            ...
            {
                MessagesTable = "NOTI1",
				MessagesTablePKName = "CODNOTIF",

                TableFieldMap = new List<FieldMap>()
                {
                    new FieldMap(){FieldnameApp = "[MESSAGE]", FieldnameQuery = "[MESSAGE]", Reserved = true},
                    new FieldMap(){FieldnameApp = "[NRCOMODA]", FieldnameQuery = "[NRCOMODA]", Reserved = false},
                    new FieldMap(){FieldnameApp = "[IDMSG]", FieldnameQuery = "[IDMSG]", Reserved = true},
                    new FieldMap(){FieldnameApp = "[DTDEVOLU]", FieldnameQuery = "[DTDEVOLU]", Reserved = false},
                    new FieldMap(){FieldnameApp = "[RETURNED]", FieldnameQuery = "[DEVOLVID]", Reserved = false},
                    new FieldMap(){FieldnameApp = "[IDNOTIF]", FieldnameQuery = "[IDNOTIF]", Reserved = true},
                    new FieldMap(){FieldnameApp = "[DESIGNAT]", FieldnameQuery = "[DESIGNAC]", Reserved = true},
                    new FieldMap(){FieldnameApp = "[CREATDAT]", FieldnameQuery = "[DATACRIA]", Reserved = true},
                    new FieldMap(){FieldnameApp = "[BEGIN]", FieldnameQuery = "[INICIO]", Reserved = false},
                    new FieldMap(){FieldnameApp = "[CODPESSO]", FieldnameQuery = "[CODPESS2]", Reserved = false},
                    new FieldMap(){FieldnameApp = "[CREATOPE]", FieldnameQuery = "[OPERCRIA]", Reserved = true},
                    new FieldMap(){FieldnameApp = "[EMAIL]", FieldnameQuery = "[EMAIL]", Reserved = true},
                    new FieldMap(){FieldnameApp = "[MAILERR]", FieldnameQuery = "[MAILERR]", Reserved = true},
                    new FieldMap(){FieldnameApp = "[END]", FieldnameQuery = "[FIM]", Reserved = false},
 				}
  
            };

			//notifications written on BD
			NotifsOnBD = new List<DbArea>();

			AlertID = "NOTIF_1_EQUIPORDEVOLV";

            //Messages
            MessagesConfig = new List<CSGenioAnotificationmessage>();

            SendsAlert = false;
            SendsEmail = true;
            SendsToDatabase = true;

		}
        /// <summary>
        /// Method that will process all notification configuration and send it to the configured communication option
        /// </summary>
        public override DataMatrix RunOpen(PersistentSupport sp, User user)
        {
            String MesgsArea = "notificationmessage";
            MessagesConfig = CSGenioAnotificationmessage.searchList(sp, user, CriteriaSet.And().Equal(new Quidgest.Persistence.FieldRef(MesgsArea, "codtpnot"), this.codtpnot).Equal(new Quidgest.Persistence.FieldRef(MesgsArea, "zzstate"), 0));


            if (MessagesConfig.Count > 0)
            {
                QueryData = this.GenioQuery.Run(sp);
                ConfigMessagesMultiplexer(sp, user);
                ...
                //TODO: WriteAlerts();
                if (SendsToDatabase)
                    WriteMessagesToBD(sp, user);
            }

            return QueryData;
        }
	}
     public class Q_NOTIF_2_DISPATCHALERT : Notification
	{
		/// <summary>
		/// Notification implementation
        /// </summary>
		public Q_NOTIF_2_DISPATCHALERT()
        {
			codtpnot = "7f8784ce-5086-435e-afce-dbe0bde5a072";
			IDNotif = "NOTIF_2_DISPATCHALERT";
			Help = "";
			Notif = "Dispatch notice to be carried out";

			//Manual query
            //está 13 porque existem rotinas no BO usam este valor por default, assim mantêm-se tudo coerente, i.e., exigência de ter pelo menos 13 chars
            String queryname = "DISPATCHALERT";
            if (queryname.ToUpper().Length < 13)
                queryname = FormatTagId(queryname);
            Hashtable manualQueries = PersistentSupport.getManualQueries();
            GenioQuery = (ManualQuery)manualQueries[queryname];
			...
			TagsFieldMapping = new List<Tag>
			{
				Tag_build("[DISPADT]", "[DISPADT]", ""),
				Tag_build("[NAME]", "[NAME]", ""),
				Tag_build("[DISPANR]", "[DISPANR]", ""),
				Tag_build("[PREPARED]", "[PREPARED]", ""),
 			};
            ...
            DatabaseFieldMapping = new MessagesTableMap()
            {
                ...
                TableFieldMap = new List<FieldMap>()
                {
                    ...
                    new FieldMap(){FieldnameApp = "[EMAIL]", FieldnameQuery = "[EMAIL]", Reserved = true},
                    new FieldMap(){FieldnameApp = "[MAILERR]", FieldnameQuery = "[MAILERR]", Reserved = true},
                    new FieldMap(){FieldnameApp = "[MAILSENT]", FieldnameQuery = "[MAILSENT]", Reserved = true},
                    new FieldMap(){FieldnameApp = "[IDNOTIF]", FieldnameQuery = "[IDNOTIF]", Reserved = true},
                    new FieldMap(){FieldnameApp = "[IDMSG]", FieldnameQuery = "[IDMSG]", Reserved = true},
                    new FieldMap(){FieldnameApp = "[CREATOPE]", FieldnameQuery = "[OPERCRIA]", Reserved = true},
                    new FieldMap(){FieldnameApp = "[DESIGNAT]", FieldnameQuery = "[DESIGNAC]", Reserved = true},
                    new FieldMap(){FieldnameApp = "[CREATDAT]", FieldnameQuery = "[DATACRIA]", Reserved = true},
                    new FieldMap(){FieldnameApp = "[CODENTIT]", FieldnameQuery = "[CODENTID]", Reserved = false},
                    new FieldMap(){FieldnameApp = "[CODPERSO]", FieldnameQuery = "[CODPERSO]", Reserved = false},
                    new FieldMap(){FieldnameApp = "[DOCUM_NR]", FieldnameQuery = "[DISPANR]", Reserved = false},
 				}
