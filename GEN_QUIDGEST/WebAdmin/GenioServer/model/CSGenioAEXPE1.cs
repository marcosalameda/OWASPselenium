...
namespace CSGenio.business
{
	...
	public class CSGenioAexpe1 : DbArea
	{
		...
		public CSGenioAexpe1(User user, string module)
		{
            this.user = user;
            this.module = module;
			this.KeyType = CodeType.GUID_KEY;
			// USE /[MANUAL GQT CONSTRUTOR EXPE1]/
		}

		public CSGenioAexpe1(User user) : this(user, user.CurrentModule)
		{
		}
...
#pragma warning restore CS0168, S1481 // Variable is declared but never used
			...
			//- - - - - - - - - - - - - - - - - - -
			Qfield = new Field(info.Alias, "codproje", FieldType.CHAVE_FALSA_GUID);
			...
			//- - - - - - - - - - - - - - - - - - -
			Qfield = new Field(info.Alias, "codyear", FieldType.CHAVE_FALSA_GUID);
			Qfield.FieldDescription = ">YEAR";
			Qfield.FieldSize =  36;
			Qfield.MQueue = false;
			Qfield.CavDesignation = "_YEAR05447";
			...
			//- - - - - - - - - - - - - - - - - - -
			Qfield = new Field(info.Alias, "codaggre", FieldType.CHAVE_FALSA_GUID);
			Qfield.FieldDescription = ">AGGREGATOR";
			Qfield.FieldSize =  36;
			Qfield.MQueue = false;
			Qfield.CavDesignation = "_AGGREGATOR38173";
		...
		private static FieldRef m_fldCoddespe = new FieldRef("expe1", "coddespe");
		...
		private static FieldRef m_fldCodyear = new FieldRef("expe1", "codyear");
		...
		private static FieldRef m_fldCodaggre = new FieldRef("expe1", "codaggre");
		...
		private static FieldRef m_fldZzstate = new FieldRef("expe1", "zzstate");
        ...
        public static CSGenioAexpe1 search(PersistentSupport sp, string key, User user, string[] fields = null)
        {
			if (string.IsNullOrEmpty(key))
				return null;

		    CSGenioAexpe1 area = new CSGenioAexpe1(user, user.CurrentModule);
        ...
        /// <remarks>Persistence operations should not be used on a partially positioned register</remarks>
        public static List<CSGenioAexpe1> searchList(PersistentSupport sp, User user, CriteriaSet where, string[] fields = null, bool distinct = false, bool noLock = false)
        {
				return sp.searchListWhere<CSGenioAexpe1>(where, user, fields, distinct, noLock);
        }
        ...
        /// <returns>A list of area records with all fields populated</returns>
        /// <remarks>Persistence operations should not be used on a partially positioned register</remarks>
        public static void searchListAdvancedWhere(PersistentSupport sp, User user, CriteriaSet where, ListingMVC<CSGenioAexpe1> listing)
        {
			sp.searchListAdvancedWhere<CSGenioAexpe1>(where, listing);
        }
