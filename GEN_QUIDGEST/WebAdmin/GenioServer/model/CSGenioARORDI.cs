...
namespace CSGenio.business
{
	...
	public class CSGenioArordi : DbArea
	{
		...
		public CSGenioArordi(User user, string module)
		{
            this.user = user;
            this.module = module;
			this.KeyType = CodeType.GUID_KEY;
			// USE /[MANUAL GQT CONSTRUTOR RORDI]/
		}

		public CSGenioArordi(User user) : this(user, user.CurrentModule)
		{
		}
...
#pragma warning restore CS0168, S1481 // Variable is declared but never used
			//- - - - - - - - - - - - - - - - - - -
			Qfield = new Field(info.Alias, "codrordi", FieldType.CHAVE_PRIMARIA_GUID);
		...
		private static AreaInfo InicializaAreaInfo()
		{
			...
			info.QSystem="GQT";
			info.TableName="gqtrordi";
			info.ShadowTabName="";
			info.ShadowTabKeyName="";

			info.PrimaryKeyName="codrordi";
			info.HumanKeyName="order,".TrimEnd(',');
			info.Alias="rordi";
			info.IsDomain = true;
			info.PersistenceType = PersistenceType.Database;
			info.AreaDesignation="Order (Integer field)";
			info.AreaPluralDesignation="Orders (Integer field)";
			info.DescriptionCav="ORDER__INTEGER_FIELD38959";
		...
		public static FieldRef FldCodrordi { get { return m_fldCodrordi; } }
		private static FieldRef m_fldCodrordi = new FieldRef("rordi", "codrordi");

		/// <summary>Field : "" Tipo: "+" Formula:  ""</summary>
		public string ValCodrordi
		{
			get { return (string)returnValueField(FldCodrordi); }
		...
		private static FieldRef m_fldTitle = new FieldRef("rordi", "title");
		...
		private static FieldRef m_fldZzstate = new FieldRef("rordi", "zzstate");
        ...
        public static CSGenioArordi search(PersistentSupport sp, string key, User user, string[] fields = null)
        {
			if (string.IsNullOrEmpty(key))
				return null;

		    CSGenioArordi area = new CSGenioArordi(user, user.CurrentModule);
        ...
        /// <remarks>Persistence operations should not be used on a partially positioned register</remarks>
        public static List<CSGenioArordi> searchList(PersistentSupport sp, User user, CriteriaSet where, string[] fields = null, bool distinct = false, bool noLock = false)
        {
				return sp.searchListWhere<CSGenioArordi>(where, user, fields, distinct, noLock);
        }
        ...
        /// <returns>A list of area records with all fields populated</returns>
        /// <remarks>Persistence operations should not be used on a partially positioned register</remarks>
        public static void searchListAdvancedWhere(PersistentSupport sp, User user, CriteriaSet where, ListingMVC<CSGenioArordi> listing)
        {
			sp.searchListAdvancedWhere<CSGenioArordi>(where, listing);
        }
        ...
        public void Reorder_Order(PersistentSupport sp, int position, CriteriaSet condition, List<Relation> relations = null, bool moveRow = true)
        {
            ...
            try
			{
				maxOrder = sp.GetMaxFieldValue(Area.AreaRORDI, CSGenioArordi.FldOrder, condition, relations);
			}
			catch(Exception ex)
			{
            ...
            range_condition.GreaterOrEqual(CSGenioArordi.FldOrder, posLow);
            range_condition.LesserOrEqual(CSGenioArordi.FldOrder, posHigh);

			sp.ReorderSequence(Area.AreaRORDI, CSGenioArordi.FldOrder, range_condition, relations, posLow + difference);

			if (moveRow) {
				//Set moved record position to new position
				UpdateQuery up = new UpdateQuery()
							.Update(Area.AreaRORDI)
							.Set(CSGenioArordi.FldOrder, posnova)
							.Where(CriteriaSet.And().Equal(CSGenioArordi.FldCodrordi, QPrimaryKey));
				sp.Execute(up);
			}
