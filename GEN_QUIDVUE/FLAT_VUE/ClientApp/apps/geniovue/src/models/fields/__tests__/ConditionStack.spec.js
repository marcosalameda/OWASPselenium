...
import { BlockConditionStack, ClearConditionStack, HideConditionStack, RequiredConditionStack } from '@/models/fields/conditionStack.js'
...
describe('ConditionStack', () => {
	...
	it.each([
		...
		['required', RequiredConditionStack.MET_EVENT, new RequiredConditionStack()]
	])('Even when adding various sources to a %s stack, the "%s" event is emitted only once', async (_, __, stack) => {
		let emits = 0
		stack.addOnMetListener(() => emits++)
	...
	it.each([
		...
		['hide', HideConditionStack.MET_EVENT, new HideConditionStack()],
		['required', RequiredConditionStack.MET_EVENT, new RequiredConditionStack()]
	])('Calling "emitMetEvent", in a %s stack, will only trigger the "%s" event if at least one condition is met', async (_, __, stack) => {
		let emits = 0
		stack.addOnMetListener(() => emits++)
	...
	])('Calling "emitUnmetEvent", in a %s stack, will only trigger the "%s" event if no conditions are met', async (_, __, stack) => {
		let emits = 0
		stack.addOnUnmetListener(() => emits++)
	...
	it.each([
		...
		['hide', 'hidden', false, new HideConditionStack()],
		['required', 'required', true, new RequiredConditionStack()]
	])('Adding an asynchronous conditional source to a %s stack will make the field %s only when it evaluates to %s', async (_, __, isMet, stack) => {
		const events = new QEventEmitter()
		stack.setEventEmitter(events)
	...
	it.each([
		...
		['hide', 'hidden', () => new HideConditionStack()],
		['required', 'required', () => new RequiredConditionStack()]
	])('Having a stack associated to a %s stack will make the field %s even if that stack is empty', async (_, __, stackFactory) => {
