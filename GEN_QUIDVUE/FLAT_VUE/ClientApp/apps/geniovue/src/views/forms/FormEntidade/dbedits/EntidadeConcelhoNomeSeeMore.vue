<template>
	<teleport
		v-if="isReady"
		to="#q-modal-see-more-entidade-concelho-nome-body">
		<q-row>
			<q-table
				v-bind="listCtrl"
...
<script>
	...
	const requiredTextResources = ['ENTIDADE__CONCELHO__NOME_SeeMore', 'hardcoded', 'messages']

	export default {
		name: 'EntidadeConcelhoNomeSeeMore',
		...
		data()
		{
			return {
				...
				componentOnLoadProc: asyncProcM.getProcListMonitor('ENTIDADE__CONCELHO__NOME_SeeMore', false),

				interfaceMetadata: {
					id: 'ENTIDADE__CONCELHO__NOME_SeeMore', // Used for resources
					requiredTextResources
				},

				menuInfo: {
					acronym: 'ENTIDADE__CONCELHO__NOME_SeeMore',
					name: 'ENTIDADE__CONCELHO__NOME_SeeMore',
					controller: 'ENTIDADE',
					action: 'ENTIDADE_ConcelhoValNome'
				},
		...
		mounted()
		{
			...
			const modalProps = {
				id: 'see-more-entidade-concelho-nome',
				dismissAction: this.close,
				returnElement: 'ENTIDADE__CONCELHO__NOME_see-more_button'
			}
			const props = {
				class: 'q-dialog-see-more',
				title: computed(() => this.Resources.CONSELHO10765),
				buttons: [
					{
		...
		beforeUnmount()
		{
			...
			removeModal('see-more-entidade-concelho-nome')
		},
			...
			getListConfig()
			{
				...
				const listProps = {
					configuration: {
						...
						uuid: 'Entidade_Entidade_ConcelhoValNome',
