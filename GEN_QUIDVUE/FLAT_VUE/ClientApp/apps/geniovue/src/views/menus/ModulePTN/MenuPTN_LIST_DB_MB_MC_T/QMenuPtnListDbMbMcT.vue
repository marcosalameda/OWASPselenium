		...
		methods: {
			...
			async PTN_MenuTR_3C1111_EMPTYDESCRIPTION_1(id)
			{
				try
				{
					...
					if (!this.isEmpty(result))
					{
						if (result !== 'error')
						{
							...
							return this.PTN_MenuTR_3C1111_EMPTYDESCRIPTION_2(id)
						}
						else
					...
					else
					{
						this.$eventTracker.addError({
							origin: 'Trigger EMPTYDESCRIPTION',
							message: 'Routine "PTN_MenuTR_3C1111" finished execution with an unknown result type: ' + data.success
						})
					}
				}
				catch (e)
				{
					...
					this.$eventTracker.addError({
						origin: 'Trigger EMPTYDESCRIPTION (catch)',
						message: e.toString()
					})
				}
			},
			...
			async PTN_MenuTR_3C1121_FILLDESCRIPTION_1(id)
			{
				try
				{
					...
					if (!this.isEmpty(result))
					{
						...
						{
							// Return the promise of followup method.
							return this.PTN_MenuTR_3C1121_FILLDESCRIPTION_2(id)
						}
						else
							genericFunctions.displayMessage(data.message, 'error')
					}
					...
					{
						this.$eventTracker.addError({
							origin: 'Trigger FILLDESCRIPTION',
							message: 'Routine "PTN_MenuTR_3C1121" finished execution with an unknown result type: ' + data.success
						})
					}
				}
				catch (e)
				{
					genericFunctions.displayMessage(this.Resources.NAO_FOI_POSSIVEL_CON65121, 'error')
					this.$eventTracker.addError({
						origin: 'Trigger FILLDESCRIPTION (catch)',
						message: e.toString()
					})
				}
			},
