<template>
	<teleport
		v-if="isReady"
		to="#q-modal-see-more-anexd-equipregistnr-body">
		<q-row-container>
			<q-table
				v-bind="listCtrl"
...
<script>
	...
	const requiredTextResources = ['ANEXD___EQUIPREGISTNR_SeeMore', 'hardcoded', 'messages']

	export default {
		name: 'AnexdEquipregistnrSeeMore',
		...
		data()
		{
			return {
				...
				componentOnLoadProc: asyncProcM.getProcListMonitor('ANEXD___EQUIPREGISTNR_SeeMore', false),

				interfaceMetadata: {
					id: 'ANEXD___EQUIPREGISTNR_SeeMore', // Used for resources
					requiredTextResources
				},

				menuInfo: {
					acronym: 'ANEXD___EQUIPREGISTNR_SeeMore',
					name: 'ANEXD___EQUIPREGISTNR_SeeMore',
					controller: 'ANEXD',
					action: 'ANEXD_EquipValRegistnr'
				},
		...
		mounted()
		{
			...
			const modalProps = {
				id: 'see-more-anexd-equipregistnr',
				...
				isActive: true,
				returnElement: 'ANEXD___EQUIPREGISTNR_lookup_see-more_button'
			}
		...
		beforeUnmount()
		{
			...
			genericFunctions.removeModal('see-more-anexd-equipregistnr')
		},
			...
			getListConfig()
			{
				...
				const listProps = {
					configuration: {
						...
						uuid: 'Anexd_Anexd_EquipValRegistnr',
