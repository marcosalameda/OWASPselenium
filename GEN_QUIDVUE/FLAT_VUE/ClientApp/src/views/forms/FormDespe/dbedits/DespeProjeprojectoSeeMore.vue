<template>
	<teleport
		v-if="isReady"
		to="#q-modal-see-more-despe-projeprojecto-body">
		<q-row-container>
			<q-table
				v-bind="listCtrl"
...
<script>
	...
	const requiredTextResources = ['DESPE___PROJEPROJECTO_SeeMore', 'hardcoded', 'messages']

	export default {
		name: 'DespeProjeprojectoSeeMore',
		...
		data()
		{
			return {
				...
				componentOnLoadProc: asyncProcM.getProcListMonitor('DESPE___PROJEPROJECTO_SeeMore', false),

				interfaceMetadata: {
					id: 'DESPE___PROJEPROJECTO_SeeMore', // Used for resources
					requiredTextResources
				},

				menuInfo: {
					acronym: 'DESPE___PROJEPROJECTO_SeeMore',
					name: 'DESPE___PROJEPROJECTO_SeeMore',
					controller: 'EXPEN',
					action: 'DESPE_ProjeValProjecto'
				},
		...
		mounted()
		{
			...
			const modalProps = {
				id: 'see-more-despe-projeprojecto',
				headerTitle: computed(() => this.Resources.PROJECTS49194),
				closeButtonEnable: true,
				hideFooter: true,
				dismissWithEsc: true,
				dismissAction: this.close,
				isActive: true,
				returnElement: 'DESPE___PROJEPROJECTO_lookup_see-more_button'
			}
		...
		beforeUnmount()
		{
			...
			genericFunctions.removeModal('see-more-despe-projeprojecto')
		},
			...
			getListConfig()
			{
				...
				const listProps = {
					configuration: {
						...
						uuid: 'Despe_Despe_ProjeValProjecto',
