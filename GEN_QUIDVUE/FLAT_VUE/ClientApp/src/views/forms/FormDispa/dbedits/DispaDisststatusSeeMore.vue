<template>
	<teleport
		v-if="isReady"
		to="#q-modal-see-more-dispa-disststatus-body">
		<q-row-container>
			<q-table
				v-bind="listCtrl"
...
<script>
	...
	const requiredTextResources = ['DISPA___DISSTSTATUS___SeeMore', 'hardcoded', 'messages']

	export default {
		name: 'DispaDisststatusSeeMore',
		...
		data()
		{
			return {
				...
				componentOnLoadProc: asyncProcM.getProcListMonitor('DISPA___DISSTSTATUS___SeeMore', false),

				interfaceMetadata: {
					id: 'DISPA___DISSTSTATUS___SeeMore', // Used for resources
					requiredTextResources
				},

				menuInfo: {
					acronym: 'DISPA___DISSTSTATUS___SeeMore',
					name: 'DISPA___DISSTSTATUS___SeeMore',
					controller: 'DISPA',
					action: 'DISPA_DisstValStatus'
				},
		...
		mounted()
		{
			...
			const modalProps = {
				id: 'see-more-dispa-disststatus',
				headerTitle: computed(() => this.Resources.DISPATCHMENT_STATUS18877),
				closeButtonEnable: true,
				hideFooter: true,
				dismissWithEsc: true,
				dismissAction: this.close,
				isActive: true,
				returnElement: 'DISPA___DISSTSTATUS___lookup_see-more_button'
			}
		...
		beforeUnmount()
		{
			...
			genericFunctions.removeModal('see-more-dispa-disststatus')
		},
			...
			getListConfig()
			{
				...
				const listProps = {
					configuration: {
						...
						config: {
							name: 'Dispa_DisstValStatus',
							serverMode: true,
							pkColumn: 'ValCoddisst',
							tableAlias: 'DISST',
							tableNamePlural: computed(() => this.Resources.DISPATCHMENT_STATUS18877),
							viewManagement: '',
							showLimitsInfo: true,
							tableTitle: '',
							...
							formsDefinition: {
								'DISST': {
									fnKeySelector: (row) => row.Fields.ValCoddisst,
									isPopup: false
								},
							},
						...
						uuid: 'Dispa_Dispa_DisstValStatus',
