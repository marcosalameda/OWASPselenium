<template>
	<teleport
		v-if="isReady"
		to="#q-modal-see-more-entit-faci1name-body">
		<q-row-container>
			<q-table
				v-bind="listCtrl"
...
<script>
	...
	const requiredTextResources = ['ENTIT___FACI1NAME_____SeeMore', 'hardcoded', 'messages']

	export default {
		name: 'EntitFaci1nameSeeMore',
		...
		data()
		{
			return {
				...
				componentOnLoadProc: asyncProcM.getProcListMonitor('ENTIT___FACI1NAME_____SeeMore', false),

				interfaceMetadata: {
					id: 'ENTIT___FACI1NAME_____SeeMore', // Used for resources
					requiredTextResources
				},

				menuInfo: {
					acronym: 'ENTIT___FACI1NAME_____SeeMore',
					name: 'ENTIT___FACI1NAME_____SeeMore',
					controller: 'ENTIT',
					action: 'ENTIT_Faci1ValName'
				},
		...
		mounted()
		{
			...
			const modalProps = {
				...
				isActive: true,
				returnElement: 'ENTIT___FACI1NAME_____lookup_see-more_button'
			}
		...
		beforeUnmount()
		{
			...
			genericFunctions.removeModal('see-more-entit-faci1name')
		},
			...
			getListConfig()
			{
				...
				const listProps = {
					configuration: {
						...
						globalEvents: ['changed-FACI1'],
						uuid: 'Entit_Entit_Faci1ValName',
