<template>
	<teleport
		v-if="isReady"
		to="#q-modal-see-more-fieldhlpaero-name-body">
		<q-row-container>
			<q-table
				v-bind="listCtrl"
...
<script>
	...
	const requiredTextResources = ['FIELDHLPAERO_NAME_____SeeMore', 'hardcoded', 'messages']

	export default {
		name: 'FieldhlpaeroNameSeeMore',
		...
		data()
		{
			return {
				...
				componentOnLoadProc: asyncProcM.getProcListMonitor('FIELDHLPAERO_NAME_____SeeMore', false),

				interfaceMetadata: {
					id: 'FIELDHLPAERO_NAME_____SeeMore', // Used for resources
					requiredTextResources
				},

				menuInfo: {
					acronym: 'FIELDHLPAERO_NAME_____SeeMore',
					name: 'FIELDHLPAERO_NAME_____SeeMore',
					controller: 'FLDS',
					action: 'FIELDHLP_AeroValName'
				},
		...
		mounted()
		{
			...
			const modalProps = {
				id: 'see-more-fieldhlpaero-name',
				headerTitle: computed(() => this.Resources.AIRLINES42267),
				closeButtonEnable: true,
				hideFooter: true,
				dismissWithEsc: true,
				dismissAction: this.close,
				isActive: true,
				returnElement: 'FIELDHLPAERO_NAME_____lookup_see-more_button'
			}
		...
		beforeUnmount()
		{
			...
			genericFunctions.removeModal('see-more-fieldhlpaero-name')
		},
			...
			getListConfig()
			{
				...
				const listProps = {
					configuration: {
						...
						globalEvents: ['changed-AERO'],
						uuid: 'Fieldhlp_Fieldhlp_AeroValName',
