<template>
	<teleport
		v-if="isReady"
		to="#q-modal-see-more-fieldhlpequipregistnr-body">
		<q-row-container>
			<q-table
				v-bind="listCtrl"
...
<script>
	...
	const requiredTextResources = ['FIELDHLPEQUIPREGISTNR_SeeMore', 'hardcoded', 'messages']

	export default {
		name: 'FieldhlpequipregistnrSeeMore',
		...
		data()
		{
			return {
				...
				componentOnLoadProc: asyncProcM.getProcListMonitor('FIELDHLPEQUIPREGISTNR_SeeMore', false),

				interfaceMetadata: {
					id: 'FIELDHLPEQUIPREGISTNR_SeeMore', // Used for resources
					requiredTextResources
				},

				menuInfo: {
					acronym: 'FIELDHLPEQUIPREGISTNR_SeeMore',
					name: 'FIELDHLPEQUIPREGISTNR_SeeMore',
					controller: 'FLDS',
					action: 'FIELDHLP_EquipValRegistnr'
				},
		...
		mounted()
		{
			...
			const modalProps = {
				id: 'see-more-fieldhlpequipregistnr',
				...
				isActive: true,
				returnElement: 'FIELDHLPEQUIPREGISTNR'
			}
		...
		beforeUnmount()
		{
			...
			genericFunctions.removeModal('see-more-fieldhlpequipregistnr')
		},
			...
			getListConfig()
			{
				...
				const listProps = {
					configuration: {
						...
						config: {
							name: 'Fieldhlp_EquipValRegistnr',
							serverMode: true,
						...
						uuid: 'Fieldhlp_Fieldhlp_EquipValRegistnr',
