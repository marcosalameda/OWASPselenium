<template>
	<teleport
		v-if="isReady"
		to="#q-modal-see-more-groupbx-tpequtipoequi-body">
		<q-row-container>
			<q-table
				v-if="!isTreeMode"
				v-bind="listCtrl"
...
<script>
	...
	const requiredTextResources = ['GROUPBX_TPEQUTIPOEQUI_SeeMore', 'hardcoded', 'messages']

	export default {
		name: 'GroupbxTpequtipoequiSeeMore',
		...
		data()
		{
			return {
				...
				componentOnLoadProc: asyncProcM.getProcListMonitor('GROUPBX_TPEQUTIPOEQUI_SeeMore', false),

				interfaceMetadata: {
					id: 'GROUPBX_TPEQUTIPOEQUI_SeeMore', // Used for resources
					requiredTextResources
				},

				menuInfo: {
					acronym: 'GROUPBX_TPEQUTIPOEQUI_SeeMore',
					name: 'GROUPBX_TPEQUTIPOEQUI_SeeMore',
					controller: 'EQUIP',
					action: 'GROUPBX_TpequValTipoequi'
				},
		...
		mounted()
		{
			...
			const modalProps = {
				id: 'see-more-groupbx-tpequtipoequi',
				...
				isActive: true,
				returnElement: 'GROUPBX_TPEQUTIPOEQUI_lookup_see-more_button'
			}
		...
		beforeUnmount()
		{
			...
			genericFunctions.removeModal('see-more-groupbx-tpequtipoequi')
		},
		...
		methods: {
			...
			onTableDBDataChanged()
			{
				const params = {
					id: this.id || null,
					identifier: 'GROUPBX_TPEQUTIPOEQUI',
					limits: this.limits,
			...
			getListConfig()
			{
				...
				const listProps = {
					configuration: {
						...
						uuid: 'Groupbx_Groupbx_TpequValTipoequi',
