<template>
	<teleport
		v-if="isReady"
		to="#q-modal-see-more-item-gitemitemdes-body">
		<q-row-container>
			<q-table
				v-bind="listCtrl"
...
<script>
	...
	const requiredTextResources = ['ITEM____GITEMITEMDES__SeeMore', 'hardcoded', 'messages']

	export default {
		name: 'ItemGitemitemdesSeeMore',
		...
		data()
		{
			return {
				...
				componentOnLoadProc: asyncProcM.getProcListMonitor('ITEM____GITEMITEMDES__SeeMore', false),

				interfaceMetadata: {
					id: 'ITEM____GITEMITEMDES__SeeMore', // Used for resources
					requiredTextResources
				},

				menuInfo: {
					acronym: 'ITEM____GITEMITEMDES__SeeMore',
					name: 'ITEM____GITEMITEMDES__SeeMore',
					controller: 'ITEM',
					action: 'ITEM_GitemValItemdes'
				},
		...
		mounted()
		{
			...
			const modalProps = {
				...
				isActive: true,
				returnElement: 'ITEM____GITEMITEMDES__lookup_see-more_button'
			}
		...
		beforeUnmount()
		{
			...
			genericFunctions.removeModal('see-more-item-gitemitemdes')
		},
			...
			getListConfig()
			{
				...
				const listProps = {
					configuration: {
						...
						uuid: 'Item_Item_GitemValItemdes',
