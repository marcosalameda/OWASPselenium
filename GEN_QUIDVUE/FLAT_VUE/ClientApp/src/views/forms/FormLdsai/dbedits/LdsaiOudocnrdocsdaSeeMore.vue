<template>
	<teleport
		v-if="isReady"
		to="#q-modal-see-more-ldsai-oudocnrdocsda-body">
		<q-row-container>
			<q-table
				v-bind="listCtrl"
...
<script>
	...
	const requiredTextResources = ['LDSAI___OUDOCNRDOCSDA_SeeMore', 'hardcoded', 'messages']

	export default {
		name: 'LdsaiOudocnrdocsdaSeeMore',
		...
		data()
		{
			return {
				...
				componentOnLoadProc: asyncProcM.getProcListMonitor('LDSAI___OUDOCNRDOCSDA_SeeMore', false),

				interfaceMetadata: {
					id: 'LDSAI___OUDOCNRDOCSDA_SeeMore', // Used for resources
					requiredTextResources
				},

				menuInfo: {
					acronym: 'LDSAI___OUDOCNRDOCSDA_SeeMore',
					name: 'LDSAI___OUDOCNRDOCSDA_SeeMore',
					controller: 'OUTPU',
					action: 'LDSAI_OudocValNrdocsda'
				},
		...
		mounted()
		{
			...
			const modalProps = {
				id: 'see-more-ldsai-oudocnrdocsda',
				headerTitle: computed(() => this.Resources.OUTPUT_DOCUMENTS44871),
				closeButtonEnable: true,
				hideFooter: true,
				dismissWithEsc: true,
				dismissAction: this.close,
				isActive: true,
				returnElement: 'LDSAI___OUDOCNRDOCSDA_lookup_see-more_button'
			}
		...
		beforeUnmount()
		{
			...
			genericFunctions.removeModal('see-more-ldsai-oudocnrdocsda')
		},
			...
			getListConfig()
			{
				...
				const listProps = {
					configuration: {
						...
						config: {
							name: 'Ldsai_OudocValNrdocsda',
							serverMode: true,
							pkColumn: 'ValCoddocsd',
							tableAlias: 'OUDOC',
							tableNamePlural: computed(() => this.Resources.OUTPUT_DOCUMENTS44871),
							viewManagement: '',
							showLimitsInfo: true,
							tableTitle: '',
							...
							formsDefinition: {
								'DOCSD': {
									fnKeySelector: (row) => row.Fields.ValCoddocsd,
									isPopup: false
								},
							},
							...
							defaultColumnSorting: {
								columnName: 'ValNrdocsda',
								sortOrder: 'asc'
							}
						},
						changeEvents: ['changed-OUDOC'],
						uuid: 'Ldsai_Ldsai_OudocValNrdocsda',
