<template>
	<teleport
		v-if="isReady"
		to="#q-modal-see-more-movim-equipregistnr-body">
		<q-row-container>
			<q-table
				v-bind="listCtrl"
...
<script>
	...
	const requiredTextResources = ['MOVIM___EQUIPREGISTNR_SeeMore', 'hardcoded', 'messages']

	export default {
		name: 'MovimEquipregistnrSeeMore',
		...
		data()
		{
			return {
				...
				componentOnLoadProc: asyncProcM.getProcListMonitor('MOVIM___EQUIPREGISTNR_SeeMore', false),

				interfaceMetadata: {
					id: 'MOVIM___EQUIPREGISTNR_SeeMore', // Used for resources
					requiredTextResources
				},

				menuInfo: {
					acronym: 'MOVIM___EQUIPREGISTNR_SeeMore',
					name: 'MOVIM___EQUIPREGISTNR_SeeMore',
					controller: 'MOVIM',
					action: 'MOVIM_EquipValRegistnr'
				},
		...
		mounted()
		{
			...
			const modalProps = {
				id: 'see-more-movim-equipregistnr',
				...
				isActive: true,
				returnElement: 'MOVIM___EQUIPREGISTNR_lookup_see-more_button'
			}
		...
		beforeUnmount()
		{
			...
			genericFunctions.removeModal('see-more-movim-equipregistnr')
		},
			...
			getListConfig()
			{
				...
				const listProps = {
					configuration: {
						...
						uuid: 'Movim_Movim_EquipValRegistnr',
