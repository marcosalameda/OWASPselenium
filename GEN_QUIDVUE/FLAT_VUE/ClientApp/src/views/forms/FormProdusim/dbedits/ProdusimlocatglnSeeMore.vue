<template>
	<teleport
		v-if="isReady"
		to="#q-modal-see-more-produsimlocatgln-body">
		<q-row-container>
			<q-table
				v-bind="listCtrl"
...
<script>
	...
	const requiredTextResources = ['PRODUSIMLOCATGLN______SeeMore', 'hardcoded', 'messages']

	export default {
		name: 'ProdusimlocatglnSeeMore',
		...
		data()
		{
			return {
				...
				componentOnLoadProc: asyncProcM.getProcListMonitor('PRODUSIMLOCATGLN______SeeMore', false),

				interfaceMetadata: {
					id: 'PRODUSIMLOCATGLN______SeeMore', // Used for resources
					requiredTextResources
				},

				menuInfo: {
					acronym: 'PRODUSIMLOCATGLN______SeeMore',
					name: 'PRODUSIMLOCATGLN______SeeMore',
					controller: 'PRODU',
					action: 'PRODUSIM_LocatValGln'
				},
		...
		mounted()
		{
			...
			const modalProps = {
				id: 'see-more-produsimlocatgln',
				headerTitle: computed(() => this.Resources.LOCATIONS14064),
				closeButtonEnable: true,
				hideFooter: true,
				dismissWithEsc: true,
				dismissAction: this.close,
				isActive: true,
				returnElement: 'PRODUSIMLOCATGLN______lookup_see-more_button'
			}
		...
		beforeUnmount()
		{
			...
			genericFunctions.removeModal('see-more-produsimlocatgln')
		},
			...
			getListConfig()
			{
				...
				const listProps = {
					configuration: {
						...
						uuid: 'Produsim_Produsim_LocatValGln',
