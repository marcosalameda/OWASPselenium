<template>
	<teleport
		v-if="isReady"
		to="#q-modal-see-more-propralltpprotppropri-body">
		<q-row-container>
			<q-table
				v-bind="listCtrl"
...
<script>
	...
	const requiredTextResources = ['PROPRALLTPPROTPPROPRI_SeeMore', 'hardcoded', 'messages']

	export default {
		name: 'PropralltpprotppropriSeeMore',
		...
		data()
		{
			return {
				...
				componentOnLoadProc: asyncProcM.getProcListMonitor('PROPRALLTPPROTPPROPRI_SeeMore', false),

				interfaceMetadata: {
					id: 'PROPRALLTPPROTPPROPRI_SeeMore', // Used for resources
					requiredTextResources
				},

				menuInfo: {
					acronym: 'PROPRALLTPPROTPPROPRI_SeeMore',
					name: 'PROPRALLTPPROTPPROPRI_SeeMore',
					controller: 'PROPR',
					action: 'PROPRALL_TpproValTppropri'
				},
		...
		mounted()
		{
			...
			const modalProps = {
				id: 'see-more-propralltpprotppropri',
				headerTitle: computed(() => this.Resources.PROPERTY_TYPES24776),
				closeButtonEnable: true,
				hideFooter: true,
				dismissWithEsc: true,
				dismissAction: this.close,
				isActive: true,
				returnElement: 'PROPRALLTPPROTPPROPRI_lookup_see-more_button'
			}
		...
		beforeUnmount()
		{
			...
			genericFunctions.removeModal('see-more-propralltpprotppropri')
		},
			...
			getListConfig()
			{
				...
				const listProps = {
					configuration: {
						...
						config: {
							name: 'Proprall_TpproValTppropri',
							serverMode: true,
							pkColumn: 'ValCodtppro',
							tableAlias: 'TPPRO',
							tableNamePlural: computed(() => this.Resources.PROPERTY_TYPES24776),
							viewManagement: 'N',
							showLimitsInfo: true,
							tableTitle: '',
							...
							formsDefinition: {
								'TPPRO': {
									fnKeySelector: (row) => row.Fields.ValCodtppro,
									isPopup: false
								},
							},
							...
							defaultColumnSorting: {
								columnName: 'ValTppropri',
								sortOrder: 'asc'
							}
						},
						changeEvents: ['changed-TPPRO'],
						uuid: 'Proprall_Proprall_TpproValTppropri',
