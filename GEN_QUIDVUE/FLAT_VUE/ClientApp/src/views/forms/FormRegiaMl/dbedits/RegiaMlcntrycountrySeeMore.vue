<template>
	<teleport
		v-if="isReady"
		to="#q-modal-see-more-regia-mlcntrycountry-body">
		<q-row-container>
			<q-table
				v-bind="listCtrl"
...
<script>
	...
	const requiredTextResources = ['REGIA_MLCNTRYCOUNTRY__SeeMore', 'hardcoded', 'messages']

	export default {
		name: 'RegiaMlcntrycountrySeeMore',
		...
		data()
		{
			return {
				...
				componentOnLoadProc: asyncProcM.getProcListMonitor('REGIA_MLCNTRYCOUNTRY__SeeMore', false),

				interfaceMetadata: {
					id: 'REGIA_MLCNTRYCOUNTRY__SeeMore', // Used for resources
					requiredTextResources
				},

				menuInfo: {
					acronym: 'REGIA_MLCNTRYCOUNTRY__SeeMore',
					name: 'REGIA_MLCNTRYCOUNTRY__SeeMore',
					controller: 'REGIO',
					action: 'REGIA_ML_CntryValCountry'
				},
		...
		mounted()
		{
			...
			const modalProps = {
				id: 'see-more-regia-mlcntrycountry',
				...
				isActive: true,
				returnElement: 'REGIA_MLCNTRYCOUNTRY__lookup_see-more_button'
			}
		...
		beforeUnmount()
		{
			...
			genericFunctions.removeModal('see-more-regia-mlcntrycountry')
		},
			...
			getListConfig()
			{
				...
				const listProps = {
					configuration: {
						...
						uuid: 'Regia_ml_Regia_ml_CntryValCountry',
