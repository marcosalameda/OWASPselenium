<template>
	<teleport
		v-if="isReady"
		to="#q-modal-see-more-repar-speciespecial-body">
		<q-row-container>
			<q-table
				v-bind="listCtrl"
...
<script>
	...
	const requiredTextResources = ['REPAR___SPECIESPECIAL_SeeMore', 'hardcoded', 'messages']

	export default {
		name: 'ReparSpeciespecialSeeMore',
		...
		data()
		{
			return {
				...
				componentOnLoadProc: asyncProcM.getProcListMonitor('REPAR___SPECIESPECIAL_SeeMore', false),

				interfaceMetadata: {
					id: 'REPAR___SPECIESPECIAL_SeeMore', // Used for resources
					requiredTextResources
				},

				menuInfo: {
					acronym: 'REPAR___SPECIESPECIAL_SeeMore',
					name: 'REPAR___SPECIESPECIAL_SeeMore',
					controller: 'REPAR',
					action: 'REPAR_SpeciValEspecial'
				},
		...
		mounted()
		{
			...
			const modalProps = {
				id: 'see-more-repar-speciespecial',
				headerTitle: computed(() => this.Resources.SPECIALTIES08113),
				closeButtonEnable: true,
				hideFooter: true,
				dismissWithEsc: true,
				dismissAction: this.close,
				isActive: true,
				returnElement: 'REPAR___SPECIESPECIAL_lookup_see-more_button'
			}
		...
		beforeUnmount()
		{
			...
			genericFunctions.removeModal('see-more-repar-speciespecial')
		},
			...
			getListConfig()
			{
				...
				const listProps = {
					configuration: {
						...
						uuid: 'Repar_Repar_SpeciValEspecial',
