<template>
	<teleport
		v-if="isReady"
		to="#q-modal-see-more-tpcat-sbcatsubcateg-body">
		<q-row-container>
			<q-table
				v-bind="listCtrl"
...
<script>
	...
	const requiredTextResources = ['TPCAT___SBCATSUBCATEG_SeeMore', 'hardcoded', 'messages']

	export default {
		name: 'TpcatSbcatsubcategSeeMore',
		...
		data()
		{
			return {
				...
				componentOnLoadProc: asyncProcM.getProcListMonitor('TPCAT___SBCATSUBCATEG_SeeMore', false),

				interfaceMetadata: {
					id: 'TPCAT___SBCATSUBCATEG_SeeMore', // Used for resources
					requiredTextResources
				},

				menuInfo: {
					acronym: 'TPCAT___SBCATSUBCATEG_SeeMore',
					name: 'TPCAT___SBCATSUBCATEG_SeeMore',
					controller: 'CATTP',
					action: 'TPCAT_SbcatValSubcateg'
				},
		...
		mounted()
		{
			...
			const modalProps = {
				id: 'see-more-tpcat-sbcatsubcateg',
				headerTitle: computed(() => this.Resources.SUB_CATEGORY06342),
				closeButtonEnable: true,
				hideFooter: true,
				dismissWithEsc: true,
				dismissAction: this.close,
				isActive: true,
				returnElement: 'TPCAT___SBCATSUBCATEG_lookup_see-more_button'
			}
		...
		beforeUnmount()
		{
			...
			genericFunctions.removeModal('see-more-tpcat-sbcatsubcateg')
		},
			...
			getListConfig()
			{
				...
				const listProps = {
					configuration: {
						...
						config: {
							name: 'Tpcat_SbcatValSubcateg',
							serverMode: true,
							pkColumn: 'ValCodsbcat',
							tableAlias: 'SBCAT',
							tableNamePlural: computed(() => this.Resources.SUB_CATEGORY06342),
							viewManagement: '',
							showLimitsInfo: true,
							tableTitle: '',
							...
							formsDefinition: {
								'SBCAT': {
									fnKeySelector: (row) => row.Fields.ValCodsbcat,
									isPopup: false
								},
							},
							...
							defaultColumnSorting: {
								columnName: 'ValSubcateg',
								sortOrder: 'asc'
							}
						},
						globalEvents: ['changed-SBCAT'],
						uuid: 'Tpcat_Tpcat_SbcatValSubcateg',
