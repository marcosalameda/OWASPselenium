<template>
	<teleport
		v-if="isReady"
		to="#q-modal-see-more-visit-equipregistnr-body">
		<q-row-container>
			<q-table
				v-bind="listCtrl"
...
<script>
	...
	const requiredTextResources = ['VISIT___EQUIPREGISTNR_SeeMore', 'hardcoded', 'messages']

	export default {
		name: 'VisitEquipregistnrSeeMore',
		...
		data()
		{
			return {
				...
				componentOnLoadProc: asyncProcM.getProcListMonitor('VISIT___EQUIPREGISTNR_SeeMore', false),

				interfaceMetadata: {
					id: 'VISIT___EQUIPREGISTNR_SeeMore', // Used for resources
					requiredTextResources
				},

				menuInfo: {
					acronym: 'VISIT___EQUIPREGISTNR_SeeMore',
					name: 'VISIT___EQUIPREGISTNR_SeeMore',
					controller: 'VISIT',
					action: 'VISIT_EquipValRegistnr'
				},
		...
		mounted()
		{
			...
			const modalProps = {
				id: 'see-more-visit-equipregistnr',
				...
				isActive: true,
				returnElement: 'VISIT___EQUIPREGISTNR_lookup_see-more_button'
			}
		...
		beforeUnmount()
		{
			...
			genericFunctions.removeModal('see-more-visit-equipregistnr')
		},
			...
			getListConfig()
			{
				...
				const listProps = {
					configuration: {
						...
						uuid: 'Visit_Visit_EquipValRegistnr',
