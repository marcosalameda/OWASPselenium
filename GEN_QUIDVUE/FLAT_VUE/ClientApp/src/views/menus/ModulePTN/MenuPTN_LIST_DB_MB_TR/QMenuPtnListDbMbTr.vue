				...
				netAPI.postData(
					...
					async (data) => {
						try
						{
							...
							{
								await this.PTN_MenuTR_TRIGGER_MENU1_MENUTRIGER_2(id)
							}
							else
								this.$eventTracker.addError({ origin: 'Trigger MENUTRIGER', message: 'Routine "PTN_MenuTR_TRIGGER_MENU1" finished execution with an unknown result type: ' + data.success })
						}
						catch (e)
						{
							genericFunctions.displayMessage(this.Resources.NAO_FOI_POSSIVEL_CON65121, 'error')
							this.$eventTracker.addError({ origin: 'Trigger MENUTRIGER (catch)', message: e.toString() })
						}
					}, () => {
						genericFunctions.displayMessage(this.Resources.NAO_FOI_POSSIVEL_CON65121, 'error')
					},
