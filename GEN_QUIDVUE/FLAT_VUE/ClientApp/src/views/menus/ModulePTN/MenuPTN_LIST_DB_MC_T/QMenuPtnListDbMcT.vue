...
/* eslint-enable indent, vue/html-indent, vue/script-indent */
		...
		methods: {
			...
			{
				return netAPI.postData(this.controls.menu.controller, 'PTN_MenuMC_LIST_DB_MC_T', params, (data) => {
					if (data.actionName)
				...
				netAPI.postData(
					...
					async (data) => {
						try
						{
							...
							{
								await this.PTN_MenuTR_3B111_EMPTYDESCRIPTIO2_2(id)
							}
							else
								this.$eventTracker.addError({ origin: 'Trigger EMPTYDESCRIPTIO2', message: 'Routine "PTN_MenuTR_3B111" finished execution with an unknown result type: ' + data.success })
						}
						catch (e)
						{
							genericFunctions.displayMessage(this.Resources.NAO_FOI_POSSIVEL_CON65121, 'error')
							this.$eventTracker.addError({ origin: 'Trigger EMPTYDESCRIPTIO2 (catch)', message: e.toString() })
						}
					}, () => {
						genericFunctions.displayMessage(this.Resources.NAO_FOI_POSSIVEL_CON65121, 'error')
					},
				...
				netAPI.postData(
					...
					async (data) => {
						try
						{
							...
							if (!this.isEmpty(result))
							{
								await this.PTN_MenuTR_3B121_FILLDESCRIPTION2_2(id)
							}
							else
								this.$eventTracker.addError({ origin: 'Trigger FILLDESCRIPTION2', message: 'Routine "PTN_MenuTR_3B121" finished execution with an unknown result type: ' + data.success })
						}
						catch (e)
						{
							genericFunctions.displayMessage(this.Resources.NAO_FOI_POSSIVEL_CON65121, 'error')
							this.$eventTracker.addError({ origin: 'Trigger FILLDESCRIPTION2 (catch)', message: e.toString() })
						}
					}, () => {
						genericFunctions.displayMessage(this.Resources.NAO_FOI_POSSIVEL_CON65121, 'error')
					},
