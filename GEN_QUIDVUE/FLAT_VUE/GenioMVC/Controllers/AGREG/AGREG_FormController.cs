...
namespace GenioMVC.Controllers
{
	public partial class AgregController : ControllerBase
	{
		...
		public ActionResult Agreg_ModalDBEdit()
		{
			Agreg_ViewModel model = new Agreg_ViewModel(UserContext.Current);
...
// USE /[MANUAL GQT AFTER_CANCEL AGREG]/
		...
		{
			var model = new Agreg_ViewModel(UserContext.Current, true);
			model.setModes(Request.Query["m"].ToString());
			PersistentSupport sp = UserContext.Current.PersistentSupport;
			var navigationLocationAction = ACTION_AGREG_NEW.SetRoutedValues(new { m = Request.Query["m"].ToString() });
		...
		[HttpPost]
		public ActionResult MFAgreg_New_GET()
		{
			return MFAgreg_New();
		}
		...
		public ActionResult MFAgreg_Edit([FromBody]RequestIdModel requestModel)
		{
			var id = requestModel.Id;
			return RedirectToFormAction("AGREG", "EDIT", new { id = id, partialView = "MFAgreg", nestedForm = "true", multiForm = "true" });
		}

		[HttpPost]
		public ActionResult MFAgreg_Edit_GET([FromBody]RequestIdModel requestModel)
		{
			return MFAgreg_Edit(requestModel);
		}

		//
		// GET /Agreg/MFAgreg_Cancel
		[ActionName("MFAgreg_Cancel")]
		public ActionResult MFAgreg_Cancel([FromBody]RequestIdModel requestModel)
		{
			var id = requestModel.Id;
		...
		public JsonResult MFAgreg_Delete([FromBody]RequestIdModel requestModel)
		{
			var id = requestModel.Id;
			var eventSink = new EventSink()
			{
		...
		public ActionResult Agreg_ProjeValProjecto([FromBody]RequestLookupModel requestModel)
		{
			...
			model.setModes(Request.Query["m"].ToString());
			model.ValCodaggre = requestModel.Id;
