...
// USE /[MANUAL GQT MANUAL_CONTROLLER ANEXD]/
		...
		public JsonResult ReloadDBEdit([FromBody]RequestReloadDBEditModel requestModel)
		{
			...
			try
			{
				row = Models.Anexd.Find(Navigation.GetStrValue("anexd"), UserContext.Current);
			}
			catch (Exception)
			{
			...
			if (row == null)
			{
				row = new Models.Anexd(UserContext.Current);
		...
		public JsonResult GetDependants([FromBody]RequestDependantsModel requestModel)
		{
			...
			try
			{
				...
				{
					case "ANEXD___EQUIPREGISTNR":	// Field (DB)
						values = new Anexd_ViewModel(UserContext.Current).GetDependant_AnexdTableEquipRegistnr(Selected);
						break;
					case "ANEXD___LANGULANGUA__":	// Field (DB)
						values = new Anexd_ViewModel(UserContext.Current).GetDependant_AnexdTableLanguLangua(Selected);
						break;
					default: break;
				}
		...
		/// <returns></returns>
		[HttpPost]
		public JsonResult RecalculateFormulas_Anexd([FromBody]Anexd_ViewModel form_data)
		{
			return GenericRecalculateFormulas(form_data, "anexd",
				(primaryKey) => Models.Anexd.Find(primaryKey, UserContext.Current, "FANEXD"),
