...
// USE /[MANUAL GQT MANUAL_CONTROLLER ANEXD]/
		...
		/// <returns></returns>
		[HttpPost]
		public JsonResult RecalculateFormulas_Anexd([FromBody]Anexd_ViewModel formData)
		{
			return GenericRecalculateFormulas(formData, "anexd",
				(primaryKey) => Models.Anexd.Find(primaryKey, UserContext.Current, "FANEXD"),
				(model) => formData.MapToModel(model as Models.Anexd)
			);
		}
		...
		{
			return base.GetDocumsTickets("ANEXD", requestModel.FieldName, requestModel.KeyValue);
		}
