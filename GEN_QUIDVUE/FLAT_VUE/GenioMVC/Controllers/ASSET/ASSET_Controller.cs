...
namespace GenioMVC.Controllers
{
	...
	{
		public AssetController(UserContextService userContext): base(userContext) { }
// USE /[MANUAL GQT CONTROLLER_NAVIGATION ASSET]/



		private List<string> GetActionIds(CriteriaSet crs, CSGenio.persistence.PersistentSupport sp = null)
		{
			CSGenio.business.Area area = CSGenio.business.Area.createArea<CSGenioAasset>(UserContext.Current.User, UserContext.Current.User.CurrentModule);
			return base.GetActionIds(crs, sp, area);
		}

// USE /[MANUAL GQT MANUAL_CONTROLLER ASSET]/
		...
		public JsonResult ReloadDBEdit([FromBody]RequestReloadDBEditModel requestModel)
		{
			...
			dynamic result = null;
			Models.Asset row = null;

			try
			{
				row = Models.Asset.Find(Navigation.GetStrValue("asset"), UserContext.Current);
			}
			catch (Exception)
			{
				CSGenio.framework.Log.Error("ReloadDBEdit - " + Identifier + " Not found Model asset");
			}

			if (row == null)
			{
				row = new Models.Asset(UserContext.Current);
				row.klass.QPrimaryKey = Navigation.GetStrValue("asset");
			}
					...
					case "EQUIPM__MANUFNAME____":	// Field (DB)
						{
							...
							model.Load_Equipm__manufname____(qs);
							result = model.TableManufName;
						}
						break;
					case "EQUIPM__KINDEDESIGNAT":	// Field (DB)
						{
							row.LoadKeysFormHistory(Navigation, Navigation.CurrentLevel.Level, false, true, true, true);
							var model = new Equipm_ViewModel(UserContext.Current) { editable = false };
							model.MapFromModel(row);
							model.Load_Equipm__kindedesignat(qs);
			...
			try
			{
				...
				switch (string.IsNullOrEmpty(Identifier) ? "" : Identifier)
				{
					case "EQUIPM__MANUFNAME____":	// Field (DB)
						...
						break;
					case "EQUIPM__KINDEDESIGNAT":	// Field (DB)
						values = new Equipm_ViewModel(UserContext.Current).GetDependant_EquipmTableKindeDesignat(Selected);
						break;
					default: break;
				}
		...
		/// <summary>
		/// Recalculate formulas of the "Equipm" form. (++, CT, SR, CL and U1)
		/// </summary>
		/// <param name="form_data">Current form data</param>
		/// <returns></returns>
		[HttpPost]
		public JsonResult RecalculateFormulas_Equipm([FromBody]Equipm_ViewModel form_data)
		{
			return GenericRecalculateFormulas(form_data, "asset",
				(primaryKey) => Models.Asset.Find(primaryKey, UserContext.Current, "FEQUIPM"),
				(model) => form_data.MapToModel(model as Models.Asset)
			);
		}
