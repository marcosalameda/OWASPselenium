...
namespace GenioMVC.Controllers
{
	public partial class AssmaController : ControllerBase
	{
		...
		public ActionResult Assma_ModalDBEdit()
		{
			Assma_ViewModel model = new Assma_ViewModel(UserContext.Current);
...
// USE /[MANUAL GQT AFTER_CANCEL ASSMA]/
		...
		{
			var model = new Assma_ViewModel(UserContext.Current, true);
			model.setModes(Request.Query["m"].ToString());
			PersistentSupport sp = UserContext.Current.PersistentSupport;
			var navigationLocationAction = ACTION_ASSMA_NEW.SetRoutedValues(new { m = Request.Query["m"].ToString() });
		...
		[HttpPost]
		public ActionResult MFAssma_New_GET()
		{
			return MFAssma_New();
		}
		...
		public ActionResult MFAssma_Edit([FromBody]RequestIdModel requestModel)
		{
			var id = requestModel.Id;
			return RedirectToFormAction("ASSMA", "EDIT", new { id = id, partialView = "MFAssma", nestedForm = "true", multiForm = "true" });
		}

		[HttpPost]
		public ActionResult MFAssma_Edit_GET([FromBody]RequestIdModel requestModel)
		{
			return MFAssma_Edit(requestModel);
		}

		//
		// GET /Assma/MFAssma_Cancel
		[ActionName("MFAssma_Cancel")]
		public ActionResult MFAssma_Cancel([FromBody]RequestIdModel requestModel)
		{
			var id = requestModel.Id;
		...
		public JsonResult MFAssma_Delete([FromBody]RequestIdModel requestModel)
		{
			var id = requestModel.Id;
			var eventSink = new EventSink()
			{
		...
		public ActionResult Assma_AssetValName([FromBody]RequestLookupModel requestModel)
		{
			...
			model.setModes(Request.Query["m"].ToString());
			model.ValCodassma = requestModel.Id;
