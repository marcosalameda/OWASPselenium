...
// USE /[MANUAL GQT CONTROLLER_SHOW ASSMA]/
		...
		public ActionResult Assma_Show_GET([FromBody] RequestIdModel requestModel)
		{
			...
			Assma_ViewModel model = new(UserContext.Current);
...
// USE /[MANUAL GQT CONTROLLER_DELETE_POST ASSMA]/
		...
		{
			string id = requestModel.Id;
			Assma_ViewModel model = new(UserContext.Current, id);
			model.MapFromModel();
...
// USE /[MANUAL GQT CONTROLLER_CANCEL_GET ASSMA]/
		public ActionResult Assma_Cancel()
		{
			if (Navigation.CurrentLevel.FormMode == FormMode.New || Navigation.CurrentLevel.FormMode == FormMode.Duplicate)
			{
				...
				try
				{
					var recordKey = Navigation.GetStrValue("assma");
					var model = GenioMVC.Models.Assma.Find(recordKey, UserContext.Current);
...
// USE /[MANUAL GQT AFTER_CANCEL ASSMA]/
		...
		public ActionResult Assma_AssetValName([FromBody] Assma_AssetValNameModel requestModel)
		{
			...
			Models.Assma parentCtx = requestModel.Model == null ? null : new(m_userContext);
			requestModel.Model?.Init(m_userContext);
			requestModel.Model?.MapToModel(parentCtx);
			Assma_AssetValName_ViewModel model = new(m_userContext, parentCtx);
...
// USE /[MANUAL GQT AFTER_APPLY_EDIT ASSMA]/
		...
		public class AssmaDocumValidateTickets : RequestDocumValidateTickets
		{
			public Assma_ViewModel Model { get; set; }
		}
		...
		public ActionResult UpdateFilesTicketsAssma([FromBody] AssmaDocumValidateTickets requestModel)
		{
		...
		/// <summary>
		/// Stores a new document, in the Docums table, associated to field DIGDOCUM
		/// </summary>
		/// <param name="requestModel">The request model with the document and ticket</param>
		/// <returns>A JSON response with the result of the operation</returns>
		public ActionResult SetFileAssmaDigdocum([FromForm] RequestDocumsCreateModel requestModel)
		{
