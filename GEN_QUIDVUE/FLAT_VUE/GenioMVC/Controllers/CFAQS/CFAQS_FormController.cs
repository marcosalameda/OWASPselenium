...
namespace GenioMVC.Controllers
{
	public partial class CfaqsController : ControllerBase
	{
		...
		public ActionResult Cfaqs_ModalDBEdit()
		{
			Cfaqs_ViewModel model = new Cfaqs_ViewModel(UserContext.Current);
...
// USE /[MANUAL GQT AFTER_CANCEL CFAQS]/
		...
		{
			var model = new Cfaqs_ViewModel(UserContext.Current, true);
			model.setModes(Request.Query["m"].ToString());
			PersistentSupport sp = UserContext.Current.PersistentSupport;
			var navigationLocationAction = ACTION_CFAQS_NEW.SetRoutedValues(new { m = Request.Query["m"].ToString() });
		...
		[HttpPost]
		public ActionResult MFCfaqs_New_GET()
		{
			return MFCfaqs_New();
		}
		...
		public ActionResult MFCfaqs_Edit([FromBody]RequestIdModel requestModel)
		{
			var id = requestModel.Id;
			return RedirectToFormAction("CFAQS", "EDIT", new { id = id, partialView = "MFCfaqs", nestedForm = "true", multiForm = "true" });
		}

		[HttpPost]
		public ActionResult MFCfaqs_Edit_GET([FromBody]RequestIdModel requestModel)
		{
			return MFCfaqs_Edit(requestModel);
		}

		//
		// GET /Cfaqs/MFCfaqs_Cancel
		[ActionName("MFCfaqs_Cancel")]
		public ActionResult MFCfaqs_Cancel([FromBody]RequestIdModel requestModel)
		{
			var id = requestModel.Id;
		...
		public JsonResult MFCfaqs_Delete([FromBody]RequestIdModel requestModel)
		{
			var id = requestModel.Id;
			var eventSink = new EventSink()
			{
		...
		public ActionResult Cfaqs_ValExpfaqs([FromBody]RequestLookupModel requestModel)
		{
			...
			model.setModes(Request.Query["m"].ToString());
			model.ValCodcfaqs = requestModel.Id;
