...
// USE /[MANUAL GQT CONTROLLER_NAVIGATION C_ADD]/
		...
		{
			CSGenio.business.Area area = CSGenio.business.Area.createArea<CSGenioAc_add>(UserContext.Current.User, UserContext.Current.User.CurrentModule);
...
// USE /[MANUAL GQT MANUAL_CONTROLLER C_ADD]/
		...
		/// <summary>
		/// Recalculate formulas of the "C_add" form. (++, CT, SR, CL and U1)
		/// </summary>
		/// <param name="formData">Current form data</param>
		/// <returns></returns>
		[HttpPost]
		public JsonResult RecalculateFormulas_C_add([FromBody]C_add_ViewModel formData)
		{
			return GenericRecalculateFormulas(formData, "c_add",
				(primaryKey) => Models.C_add.Find(primaryKey, UserContext.Current, "FC_ADD"),
				(model) => formData.MapToModel(model as Models.C_add)
			);
		}
		...
		{
			return base.GetDocumsTickets("C_ADD", requestModel.FieldName, requestModel.KeyValue);
		}
