...
namespace GenioMVC.Controllers
{
	public partial class DecomController : ControllerBase
	{
		...
		public ActionResult Abatereq_ModalDBEdit()
		{
			Abatereq_ViewModel model = new Abatereq_ViewModel(UserContext.Current);
...
// USE /[MANUAL GQT AFTER_CANCEL ABATEREQ]/
		...
		{
			var model = new Abatereq_ViewModel(UserContext.Current, true);
			model.setModes(Request.Query["m"].ToString());
			PersistentSupport sp = UserContext.Current.PersistentSupport;
			var navigationLocationAction = ACTION_ABATEREQ_NEW.SetRoutedValues(new { m = Request.Query["m"].ToString() });
		...
		[HttpPost]
		public ActionResult MFAbatereq_New_GET()
		{
			return MFAbatereq_New();
		}
		...
		public ActionResult MFAbatereq_Edit([FromBody]RequestIdModel requestModel)
		{
			var id = requestModel.Id;
			return RedirectToFormAction("ABATEREQ", "EDIT", new { id = id, partialView = "MFAbatereq", nestedForm = "true", multiForm = "true" });
		}

		[HttpPost]
		public ActionResult MFAbatereq_Edit_GET([FromBody]RequestIdModel requestModel)
		{
			return MFAbatereq_Edit(requestModel);
		}

		//
		// GET /Decom/MFAbatereq_Cancel
		[ActionName("MFAbatereq_Cancel")]
		public ActionResult MFAbatereq_Cancel([FromBody]RequestIdModel requestModel)
		{
			var id = requestModel.Id;
		...
		public JsonResult MFAbatereq_Delete([FromBody]RequestIdModel requestModel)
		{
			var id = requestModel.Id;
			var eventSink = new EventSink()
			{
