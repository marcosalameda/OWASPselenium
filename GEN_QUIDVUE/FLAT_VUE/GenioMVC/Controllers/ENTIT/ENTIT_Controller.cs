...
// USE /[MANUAL GQT MANUAL_CONTROLLER ENTIT]/
		...
		public JsonResult GetDependants([FromBody]RequestDependantsModel requestModel)
		{
			...
			try
			{
				...
				{
					case "ENTIT___FACI1NAME____":	// Field (DB)
						values = new Entit_ViewModel(UserContext.Current).GetDependant_EntitTableFaci1Name(Selected);
						break;
					case "ENTIT___FACI2NAME____":	// Field (DB)
						values = new Entit_ViewModel(UserContext.Current).GetDependant_EntitTableFaci2Name(Selected);
						break;
					case "ENTIX___FACI1NAME____":	// Field (F1)
						values = new Entix_ViewModel(UserContext.Current).GetDependant_EntixTableFaci1Name(Selected);
						break;
					case "ENTIX___FACI2NAME____":	// Field (F1)
						values = new Entix_ViewModel(UserContext.Current).GetDependant_EntixTableFaci2Name(Selected);
						break;
					default: break;
				}
		...
		{
			return base.GetDocumsTickets("ENTIT", requestModel.FieldName, requestModel.KeyValue);
		}
