...
namespace GenioMVC.Controllers
{
	...
	{
		public EsppeController(UserContextService userContext): base(userContext) { }
// USE /[MANUAL GQT CONTROLLER_NAVIGATION ESPPE]/



		private List<string> GetActionIds(CriteriaSet crs, CSGenio.persistence.PersistentSupport sp = null)
		{
			CSGenio.business.Area area = CSGenio.business.Area.createArea<CSGenioAesppe>(UserContext.Current.User, UserContext.Current.User.CurrentModule);
...
// USE /[MANUAL GQT MANUAL_CONTROLLER ESPPE]/
		...
		public JsonResult ReloadDBEdit([FromBody]RequestReloadDBEditModel requestModel)
		{
			...
			try
			{
				row = Models.Esppe.Find(Navigation.GetStrValue("esppe"), UserContext.Current);
			}
			catch (Exception)
			{
				CSGenio.framework.Log.Error("ReloadDBEdit - " + Identifier + " Not found Model esppe");
			}

			if (row == null)
			{
				row = new Models.Esppe(UserContext.Current);
		...
		public JsonResult GetDependants([FromBody]RequestDependantsModel requestModel)
		{
			...
			try
			{
				...
				{
					case "ESPPE___PESSONAME____":	// Field (DB)
						values = new Esppe_ViewModel(UserContext.Current).GetDependant_EsppeTablePessoName(Selected);
						break;
					case "ESPPE___SPECIESPECIAL":	// Field (DB)
						values = new Esppe_ViewModel(UserContext.Current).GetDependant_EsppeTableSpeciEspecial(Selected);
						break;
					default: break;
				}
		...
		/// <summary>
		/// Recalculate formulas of the "Esppe" form. (++, CT, SR, CL and U1)
		/// </summary>
		/// <param name="form_data">Current form data</param>
		/// <returns></returns>
		[HttpPost]
		public JsonResult RecalculateFormulas_Esppe([FromBody]Esppe_ViewModel form_data)
		{
			return GenericRecalculateFormulas(form_data, "esppe",
				(primaryKey) => Models.Esppe.Find(primaryKey, UserContext.Current, "FESPPE"),
				(model) => form_data.MapToModel(model as Models.Esppe)
			);
		}
