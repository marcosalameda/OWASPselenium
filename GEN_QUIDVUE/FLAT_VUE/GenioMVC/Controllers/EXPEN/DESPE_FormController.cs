...
namespace GenioMVC.Controllers
{
	public partial class ExpenController : ControllerBase
	{
		...
		public ActionResult Despe_ModalDBEdit()
		{
			Despe_ViewModel model = new Despe_ViewModel(UserContext.Current);
...
// USE /[MANUAL GQT AFTER_CANCEL DESPE]/
		...
		{
			var model = new Despe_ViewModel(UserContext.Current, true);
			model.setModes(Request.Query["m"].ToString());
			PersistentSupport sp = UserContext.Current.PersistentSupport;
			var navigationLocationAction = ACTION_DESPE_NEW.SetRoutedValues(new { m = Request.Query["m"].ToString() });
		...
		[HttpPost]
		public ActionResult MFDespe_New_GET()
		{
			return MFDespe_New();
		}
		...
		public ActionResult MFDespe_Edit([FromBody]RequestIdModel requestModel)
		{
			var id = requestModel.Id;
			return RedirectToFormAction("DESPE", "EDIT", new { id = id, partialView = "MFDespe", nestedForm = "true", multiForm = "true" });
		}

		[HttpPost]
		public ActionResult MFDespe_Edit_GET([FromBody]RequestIdModel requestModel)
		{
			return MFDespe_Edit(requestModel);
		}

		//
		// GET /Expen/MFDespe_Cancel
		[ActionName("MFDespe_Cancel")]
		public ActionResult MFDespe_Cancel([FromBody]RequestIdModel requestModel)
		{
			var id = requestModel.Id;
		...
		public JsonResult MFDespe_Delete([FromBody]RequestIdModel requestModel)
		{
			var id = requestModel.Id;
			var eventSink = new EventSink()
			{
		...
		public ActionResult Despe_ProjeValProjecto([FromBody]RequestLookupModel requestModel)
		{
			...
			model.setModes(Request.Query["m"].ToString());
			model.ValCoddespe = requestModel.Id;
