...
namespace GenioMVC.Controllers
{
	...
	{
		public FeecaController(UserContextService userContext): base(userContext) { }
...
// USE /[MANUAL GQT MANUAL_CONTROLLER FEECA]/
		...
		public JsonResult ReloadDBEdit([FromBody]RequestReloadDBEditModel requestModel)
		{
			...
			try
			{
				row = Models.Feeca.Find(Navigation.GetStrValue("feeca"), UserContext.Current);
			}
			catch (Exception)
			{
				CSGenio.framework.Log.Error("ReloadDBEdit - " + Identifier + " Not found Model feeca");
			}

			if (row == null)
			{
				row = new Models.Feeca(UserContext.Current);
		...
		public JsonResult GetDependants([FromBody]RequestDependantsModel requestModel)
		{
			...
			try
			{
				...
				{
					case "FEECA___FLDS_DESCRIP_":	// Field (DB)
						values = new Feeca_ViewModel(UserContext.Current).GetDependant_FeecaTableFldsDescrip(Selected);
						break;
					default: break;
				}
		...
		/// <returns></returns>
		[HttpPost]
		public JsonResult RecalculateFormulas_Feeca([FromBody]Feeca_ViewModel form_data)
		{
