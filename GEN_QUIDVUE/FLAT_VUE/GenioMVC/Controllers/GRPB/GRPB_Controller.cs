...
namespace GenioMVC.Controllers
{
	public partial class GrpbController : ControllerBase
	{
		...
		public GrpbController(UserContextService userContext, IChatbotService aiService) : base(userContext)
		{
			_aiService = aiService;
		}

// USE /[MANUAL GQT CONTROLLER_NAVIGATION GRPB]/



		private List<string> GetActionIds(CriteriaSet crs, CSGenio.persistence.PersistentSupport sp = null)
		{
			CSGenio.business.Area area = CSGenio.business.Area.createArea<CSGenioAgrpb>(UserContext.Current.User, UserContext.Current.User.CurrentModule);
...
// USE /[MANUAL GQT MANUAL_CONTROLLER GRPB]/
		...
		/// <returns></returns>
		[HttpPost]
		public JsonResult RecalculateFormulas_Grpb([FromBody]Grpb_ViewModel formData)
		{
			return GenericRecalculateFormulas(formData, "grpb",
				(primaryKey) => Models.Grpb.Find(primaryKey, UserContext.Current, "FGRPB"),
				(model) => formData.MapToModel(model as Models.Grpb)
			);
		}
		...
		{
			return base.GetDocumsTickets("GRPB", requestModel.FieldName, requestModel.KeyValue);
		}
