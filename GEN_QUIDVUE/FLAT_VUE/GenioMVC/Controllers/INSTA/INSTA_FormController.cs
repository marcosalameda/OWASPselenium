...
namespace GenioMVC.Controllers
{
	public partial class InstaController : ControllerBase
	{
		...
		public ActionResult Insta_ModalDBEdit()
		{
			Insta_ViewModel model = new Insta_ViewModel(UserContext.Current);
...
// USE /[MANUAL GQT CONTROLLER_SHOW INSTA]/
		...
		public ActionResult Insta_Show_GET([FromBody]RequestIdModel requestModel)
		{
			...
			var model = new Insta_ViewModel(UserContext.Current);
...
// USE /[MANUAL GQT CONTROLLER_DELETE_POST INSTA]/
		...
		{
			var id = requestModel.Id;
			var model = new Insta_ViewModel (UserContext.Current, id);
			model.MapFromModel();
...
// USE /[MANUAL GQT AFTER_CANCEL INSTA]/
		...
		{
			var model = new Insta_ViewModel(UserContext.Current, true);
			model.setModes(Request.Query["m"].ToString());
			PersistentSupport sp = UserContext.Current.PersistentSupport;
			var navigationLocationAction = ACTION_INSTA_NEW.SetRoutedValues(new { m = Request.Query["m"].ToString() });
		...
		[HttpPost]
		public ActionResult MFInsta_New_GET()
		{
			return MFInsta_New();
		}
		...
		public ActionResult MFInsta_Edit([FromBody]RequestIdModel requestModel)
		{
			var id = requestModel.Id;
			return RedirectToFormAction("INSTA", "EDIT", new { id = id, partialView = "MFInsta", nestedForm = "true", multiForm = "true" });
		}

		[HttpPost]
		public ActionResult MFInsta_Edit_GET([FromBody]RequestIdModel requestModel)
		{
			return MFInsta_Edit(requestModel);
		}

		//
		// GET /Insta/MFInsta_Cancel
		[ActionName("MFInsta_Cancel")]
		public ActionResult MFInsta_Cancel([FromBody]RequestIdModel requestModel)
		{
			var id = requestModel.Id;
		...
		public JsonResult MFInsta_Delete([FromBody]RequestIdModel requestModel)
		{
			var id = requestModel.Id;
			var eventSink = new EventSink()
			{
		...
		public ActionResult Insta_TpequValTipoequi([FromBody]RequestLookupModel requestModel)
		{
			...
			Insta_TpequValTipoequi_ViewModel model = new Insta_TpequValTipoequi_ViewModel(UserContext.Current);
			model.setModes(Request.Query["m"].ToString());
		...
		public ActionResult Insta_EquipValRegistnr([FromBody]RequestLookupModel requestModel)
		{
			var queryParams = requestModel.QueryParams;
			...
			IsStateReadonly = true;
			Insta_EquipValRegistnr_ViewModel model = new Insta_EquipValRegistnr_ViewModel(UserContext.Current);
			model.setModes(Request.Query["m"].ToString());
			model.ValCodinsta = requestModel.Id;
			model.Load(perPage, requestValues, Request.IsAjaxRequest());
