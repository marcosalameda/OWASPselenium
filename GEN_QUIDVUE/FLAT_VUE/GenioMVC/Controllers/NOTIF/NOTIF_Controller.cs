...
// USE /[MANUAL GQT CONTROLLER_NAVIGATION NOTIF]/
		...
		{
			CSGenio.business.Area area = CSGenio.business.Area.createArea<CSGenioAnotif>(UserContext.Current.User, UserContext.Current.User.CurrentModule);
			return base.GetActionIds(crs, sp, area);
		}

// USE /[MANUAL GQT MANUAL_CONTROLLER NOTIF]/
		...
		public JsonResult ReloadDBEdit([FromBody]RequestReloadDBEditModel requestModel)
		{
			...
			dynamic result = null;
			Models.Notif row = null;

			try
			{
				row = Models.Notif.Find(Navigation.GetStrValue("notif"), UserContext.Current);
			}
			catch (Exception)
			{
				CSGenio.framework.Log.Error("ReloadDBEdit - " + Identifier + " Not found Model notif");
			}

			if (row == null)
			{
				row = new Models.Notif(UserContext.Current);
				row.klass.QPrimaryKey = Navigation.GetStrValue("notif");
			}
			...
			try
			{
				...
				{
					case "NOTIF___PESS2NAME____":	// Field (DB)
						{
							row.LoadKeysFormHistory(Navigation, Navigation.CurrentLevel.Level, false, true, true, true);
							var model = new Notif_ViewModel(UserContext.Current) { editable = false };
							model.MapFromModel(row);
							model.Load_Notif___pess2name____(qs);
		...
		public JsonResult GetDependants([FromBody]RequestDependantsModel requestModel)
		{
			...
			try
			{
				...
				{
					case "NOTIF___PESS2NAME____":	// Field (DB)
						values = new Notif_ViewModel(UserContext.Current).GetDependant_NotifTablePess2Name(Selected);
						break;
					default: break;
				}
		...
		/// <summary>
		/// Recalculate formulas of the "Notif" form. (++, CT, SR, CL and U1)
		/// </summary>
		/// <param name="form_data">Current form data</param>
		/// <returns></returns>
		[HttpPost]
		public JsonResult RecalculateFormulas_Notif([FromBody]Notif_ViewModel form_data)
		{
			return GenericRecalculateFormulas(form_data, "notif",
				(primaryKey) => Models.Notif.Find(primaryKey, UserContext.Current, "FNOTIF"),
				(model) => form_data.MapToModel(model as Models.Notif)
			);
		}
