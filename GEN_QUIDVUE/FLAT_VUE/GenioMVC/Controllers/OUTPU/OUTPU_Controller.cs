...
namespace GenioMVC.Controllers
{
	public partial class OutpuController : ControllerBase
	{
		...
		public OutpuController(UserContextService userContext, IChatbotService aiService) : base(userContext)
		{
			_aiService = aiService;
		}

// USE /[MANUAL GQT CONTROLLER_NAVIGATION OUTPU]/



		private List<string> GetActionIds(CriteriaSet crs, CSGenio.persistence.PersistentSupport sp = null)
		{
			CSGenio.business.Area area = CSGenio.business.Area.createArea<CSGenioAoutpu>(UserContext.Current.User, UserContext.Current.User.CurrentModule);
			return base.GetActionIds(crs, sp, area);
		}

// USE /[MANUAL GQT MANUAL_CONTROLLER OUTPU]/
		...
		public JsonResult ReloadDBEdit([FromBody]RequestReloadDBEditModel requestModel)
		{
			...
			*/
			Models.Outpu row = new Models.Outpu(UserContext.Current, isEmpty: true);
		...
		public JsonResult GetDependants([FromBody]RequestDependantsModel requestModel)
		{
			...
			try
			{
				...
				{
					case "LDSAI___OUTPTDOCUMENR":	// Field (DB)
						values = new Ldsai_ViewModel(UserContext.Current).GetDependant_LdsaiTableOutptDocumenr(Selected);
						break;
					case "LDSAI___WAREHWAREHDES":	// Field (DB)
						values = new Ldsai_ViewModel(UserContext.Current).GetDependant_LdsaiTableWarehWarehdes(Selected);
						break;
					case "LDSAI___ITEM_ITEMDES_":	// Field (DB)
						values = new Ldsai_ViewModel(UserContext.Current).GetDependant_LdsaiTableItemItemdes(Selected);
						break;
					case "LDSAI___OUDOCNRDOCSDA":	// Field (DB)
						values = new Ldsai_ViewModel(UserContext.Current).GetDependant_LdsaiTableOudocNrdocsda(Selected);
						break;
					default: break;
				}
		...
		{
			return base.GetDocumsTickets("OUTPU", requestModel.FieldName, requestModel.KeyValue);
		}
