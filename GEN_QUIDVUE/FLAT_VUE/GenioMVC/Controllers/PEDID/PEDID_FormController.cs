...
namespace GenioMVC.Controllers
{
	public partial class PedidController : ControllerBase
	{
		...
		public ActionResult Pedid_ModalDBEdit()
		{
			Pedid_ViewModel model = new Pedid_ViewModel(UserContext.Current);
...
// USE /[MANUAL GQT AFTER_CANCEL PEDID]/
		...
		{
			var model = new Pedid_ViewModel(UserContext.Current, true);
			model.setModes(Request.Query["m"].ToString());
			PersistentSupport sp = UserContext.Current.PersistentSupport;
			var navigationLocationAction = ACTION_PEDID_NEW.SetRoutedValues(new { m = Request.Query["m"].ToString() });
		...
		[HttpPost]
		public ActionResult MFPedid_New_GET()
		{
			return MFPedid_New();
		}
		...
		public ActionResult MFPedid_Edit([FromBody]RequestIdModel requestModel)
		{
			var id = requestModel.Id;
			return RedirectToFormAction("PEDID", "EDIT", new { id = id, partialView = "MFPedid", nestedForm = "true", multiForm = "true" });
		}

		[HttpPost]
		public ActionResult MFPedid_Edit_GET([FromBody]RequestIdModel requestModel)
		{
			return MFPedid_Edit(requestModel);
		}

		//
		// GET /Pedid/MFPedid_Cancel
		[ActionName("MFPedid_Cancel")]
		public ActionResult MFPedid_Cancel([FromBody]RequestIdModel requestModel)
		{
			var id = requestModel.Id;
		...
		public JsonResult MFPedid_Delete([FromBody]RequestIdModel requestModel)
		{
			var id = requestModel.Id;
			var eventSink = new EventSink()
			{
		...
		public ActionResult Pedid_ValLinhas([FromBody]RequestLookupModel requestModel)
		{
			...
			model.setModes(Request.Query["m"].ToString());
			model.ValCodpedid = requestModel.Id;
