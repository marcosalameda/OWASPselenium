...
namespace GenioMVC.Controllers
{
	public partial class PessoController : ControllerBase
	{
		...
		public ActionResult Externo_ModalDBEdit()
		{
			Externo_ViewModel model = new Externo_ViewModel(UserContext.Current);
...
// USE /[MANUAL GQT AFTER_CANCEL EXTERNO]/
		...
		{
			var model = new Externo_ViewModel(UserContext.Current, true);
			model.setModes(Request.Query["m"].ToString());
			PersistentSupport sp = UserContext.Current.PersistentSupport;
			var navigationLocationAction = ACTION_EXTERNO_NEW.SetRoutedValues(new { m = Request.Query["m"].ToString() });
		...
		[HttpPost]
		public ActionResult MFExterno_New_GET()
		{
			return MFExterno_New();
		}
		...
		public ActionResult MFExterno_Edit([FromBody]RequestIdModel requestModel)
		{
			var id = requestModel.Id;
			return RedirectToFormAction("EXTERNO", "EDIT", new { id = id, partialView = "MFExterno", nestedForm = "true", multiForm = "true" });
		}

		[HttpPost]
		public ActionResult MFExterno_Edit_GET([FromBody]RequestIdModel requestModel)
		{
			return MFExterno_Edit(requestModel);
		}

		//
		// GET /Pesso/MFExterno_Cancel
		[ActionName("MFExterno_Cancel")]
		public ActionResult MFExterno_Cancel([FromBody]RequestIdModel requestModel)
		{
			var id = requestModel.Id;
		...
		public JsonResult MFExterno_Delete([FromBody]RequestIdModel requestModel)
		{
			var id = requestModel.Id;
			var eventSink = new EventSink()
			{
