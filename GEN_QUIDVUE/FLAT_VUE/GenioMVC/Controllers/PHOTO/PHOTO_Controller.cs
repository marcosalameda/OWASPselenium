...
namespace GenioMVC.Controllers
{
	...
	{
		public PhotoController(UserContextService userContext): base(userContext) { }
// USE /[MANUAL GQT CONTROLLER_NAVIGATION PHOTO]/
...
// USE /[MANUAL GQT MANUAL_CONTROLLER PHOTO]/
		...
		public JsonResult ReloadDBEdit([FromBody]RequestReloadDBEditModel requestModel)
		{
			...
			dynamic result = null;
			Models.Photo row = null;

			try
			{
				row = Models.Photo.Find(Navigation.GetStrValue("photo"), UserContext.Current);
			}
			catch (Exception)
			{
				CSGenio.framework.Log.Error("ReloadDBEdit - " + Identifier + " Not found Model photo");
			}

			if (row == null)
			{
				row = new Models.Photo(UserContext.Current);
			...
			try
			{
				...
				{
					case "FOTOS___EQUIPREGISTNR":	// Field (DB)
						{
							...
							model.Load_Fotos___equipregistnr(qs);
		...
		public JsonResult GetDependants([FromBody]RequestDependantsModel requestModel)
		{
			...
			try
			{
				...
				{
					case "FOTOS___EQUIPREGISTNR":	// Field (DB)
						values = new Fotos_ViewModel(UserContext.Current).GetDependant_FotosTableEquipRegistnr(Selected);
						break;
					default: break;
				}
		...
		/// <returns></returns>
		[HttpPost]
		public JsonResult RecalculateFormulas_Fotos([FromBody]Fotos_ViewModel form_data)
		{
			return GenericRecalculateFormulas(form_data, "photo",
				(primaryKey) => Models.Photo.Find(primaryKey, UserContext.Current, "FFOTOS"),
				(model) => form_data.MapToModel(model as Models.Photo)
			);
		}
