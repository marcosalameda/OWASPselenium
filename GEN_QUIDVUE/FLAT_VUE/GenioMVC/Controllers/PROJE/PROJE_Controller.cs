...
namespace GenioMVC.Controllers
{
	...
	{
		public ProjeController(UserContextService userContext): base(userContext) { }
// USE /[MANUAL GQT CONTROLLER_NAVIGATION PROJE]/
...
// USE /[MANUAL GQT MANUAL_CONTROLLER PROJE]/
		...
		public JsonResult ReloadDBEdit([FromBody]RequestReloadDBEditModel requestModel)
		{
			...
			dynamic result = null;
			Models.Proje row = null;

			try
			{
				row = Models.Proje.Find(Navigation.GetStrValue("proje"), UserContext.Current);
			}
			catch (Exception)
			{
				CSGenio.framework.Log.Error("ReloadDBEdit - " + Identifier + " Not found Model proje");
			}

			if (row == null)
			{
				row = new Models.Proje(UserContext.Current);
				row.klass.QPrimaryKey = Navigation.GetStrValue("proje");
			}
			...
			try
			{
				...
				{
					case "PROJE___YEAR1YEAR____":	// Field (DB)
						{
							row.LoadKeysFormHistory(Navigation, Navigation.CurrentLevel.Level, false, true, true, true);
							var model = new Proje_ViewModel(UserContext.Current) { editable = false };
							model.MapFromModel(row);
							model.Load_Proje___year1year____(qs);
							result = model.TableYear1Year;
						}
						break;
					default: break;
				}
		...
		public JsonResult GetDependants([FromBody]RequestDependantsModel requestModel)
		{
			...
			try
			{
				...
				{
					case "PROJE___YEAR1YEAR____":	// Field (DB)
						values = new Proje_ViewModel(UserContext.Current).GetDependant_ProjeTableYear1Year(Selected);
						break;
					default: break;
				}
		...
		/// <summary>
		/// Recalculate formulas of the "Proje" form. (++, CT, SR, CL and U1)
		/// </summary>
		/// <param name="form_data">Current form data</param>
		/// <returns></returns>
		[HttpPost]
		public JsonResult RecalculateFormulas_Proje([FromBody]Proje_ViewModel form_data)
		{
			return GenericRecalculateFormulas(form_data, "proje",
				(primaryKey) => Models.Proje.Find(primaryKey, UserContext.Current, "FPROJE"),
				(model) => form_data.MapToModel(model as Models.Proje)
			);
		}
