...
// USE /[MANUAL GQT CONTROLLER_SHOW RECEI]/
		...
		public ActionResult Recei_Show_GET([FromBody] RequestIdModel requestModel)
		{
			...
			Recei_ViewModel model = new(UserContext.Current);
...
// USE /[MANUAL GQT CONTROLLER_DELETE_POST RECEI]/
		...
		{
			string id = requestModel.Id;
			Recei_ViewModel model = new(UserContext.Current, id);
			model.MapFromModel();
...
// USE /[MANUAL GQT CONTROLLER_CANCEL_GET RECEI]/
		public ActionResult Recei_Cancel()
		{
			if (Navigation.CurrentLevel.FormMode == FormMode.New || Navigation.CurrentLevel.FormMode == FormMode.Duplicate)
			{
				...
				try
				{
					var recordKey = Navigation.GetStrValue("recei");
					var model = GenioMVC.Models.Recei.Find(recordKey, UserContext.Current);
...
// USE /[MANUAL GQT AFTER_CANCEL RECEI]/
		...
		public ActionResult Recei_EntitValName([FromBody] Recei_EntitValNameModel requestModel)
		{
			...
			Models.Recei parentCtx = requestModel.Model == null ? null : new(m_userContext);
			requestModel.Model?.Init(m_userContext);
			requestModel.Model?.MapToModel(parentCtx);
			Recei_EntitValName_ViewModel model = new(m_userContext, parentCtx);
		...
		public ActionResult Recei_ValReceiptl([FromBody] Recei_ValReceiptlModel requestModel)
		{
			...
			requestModel.Model?.Init(m_userContext);
			requestModel.Model?.MapToModel(parentCtx);
			Recei_ValReceiptl_ViewModel model = new(m_userContext, parentCtx);
