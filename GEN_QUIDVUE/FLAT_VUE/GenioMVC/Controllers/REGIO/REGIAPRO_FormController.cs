...
namespace GenioMVC.Controllers
{
	public partial class RegioController : ControllerBase
	{
		...
		public ActionResult Regiapro_ModalDBEdit()
		{
			Regiapro_ViewModel model = new Regiapro_ViewModel(UserContext.Current);
...
// USE /[MANUAL GQT AFTER_CANCEL REGIAPRO]/
		...
		{
			var model = new Regiapro_ViewModel(UserContext.Current, true);
			model.setModes(Request.Query["m"].ToString());
			PersistentSupport sp = UserContext.Current.PersistentSupport;
			var navigationLocationAction = ACTION_REGIAPRO_NEW.SetRoutedValues(new { m = Request.Query["m"].ToString() });
		...
		[HttpPost]
		public ActionResult MFRegiapro_New_GET()
		{
			return MFRegiapro_New();
		}
		...
		public ActionResult MFRegiapro_Edit([FromBody]RequestIdModel requestModel)
		{
			var id = requestModel.Id;
			return RedirectToFormAction("REGIAPRO", "EDIT", new { id = id, partialView = "MFRegiapro", nestedForm = "true", multiForm = "true" });
		}

		[HttpPost]
		public ActionResult MFRegiapro_Edit_GET([FromBody]RequestIdModel requestModel)
		{
			return MFRegiapro_Edit(requestModel);
		}

		//
		// GET /Regio/MFRegiapro_Cancel
		[ActionName("MFRegiapro_Cancel")]
		public ActionResult MFRegiapro_Cancel([FromBody]RequestIdModel requestModel)
		{
			var id = requestModel.Id;
		...
		public JsonResult MFRegiapro_Delete([FromBody]RequestIdModel requestModel)
		{
			var id = requestModel.Id;
			var eventSink = new EventSink()
			{
