...
namespace GenioMVC.Controllers
{
	public partial class ReparController : ControllerBase
	{
		...
		public ActionResult Repar_ModalDBEdit()
		{
			Repar_ViewModel model = new Repar_ViewModel(UserContext.Current);
...
// USE /[MANUAL GQT AFTER_CANCEL REPAR]/
		...
		{
			var model = new Repar_ViewModel(UserContext.Current, true);
			model.setModes(Request.Query["m"].ToString());
			PersistentSupport sp = UserContext.Current.PersistentSupport;
			var navigationLocationAction = ACTION_REPAR_NEW.SetRoutedValues(new { m = Request.Query["m"].ToString() });
		...
		[HttpPost]
		public ActionResult MFRepar_New_GET()
		{
			return MFRepar_New();
		}
		...
		public ActionResult MFRepar_Edit([FromBody]RequestIdModel requestModel)
		{
			var id = requestModel.Id;
			return RedirectToFormAction("REPAR", "EDIT", new { id = id, partialView = "MFRepar", nestedForm = "true", multiForm = "true" });
		}

		[HttpPost]
		public ActionResult MFRepar_Edit_GET([FromBody]RequestIdModel requestModel)
		{
			return MFRepar_Edit(requestModel);
		}

		//
		// GET /Repar/MFRepar_Cancel
		[ActionName("MFRepar_Cancel")]
		public ActionResult MFRepar_Cancel([FromBody]RequestIdModel requestModel)
		{
			var id = requestModel.Id;
		...
		public JsonResult MFRepar_Delete([FromBody]RequestIdModel requestModel)
		{
			var id = requestModel.Id;
			var eventSink = new EventSink()
			{
		...
		public ActionResult Repar_EquipValRegistnr([FromBody]RequestLookupModel requestModel)
		{
			...
			model.setModes(Request.Query["m"].ToString());
			model.ValCodrepar = requestModel.Id;
