...
namespace GenioMVC.Controllers
{
	public partial class RoigfController : ControllerBase
	{
		...
		public RoigfController(UserContextService userContext, IChatbotService aiService) : base(userContext)
		{
			_aiService = aiService;
		}

// USE /[MANUAL GQT CONTROLLER_NAVIGATION ROIGF]/



		private List<string> GetActionIds(CriteriaSet crs, CSGenio.persistence.PersistentSupport sp = null)
		{
			CSGenio.business.Area area = CSGenio.business.Area.createArea<CSGenioAroigf>(UserContext.Current.User, UserContext.Current.User.CurrentModule);
			return base.GetActionIds(crs, sp, area);
		}

// USE /[MANUAL GQT MANUAL_CONTROLLER ROIGF]/
		...
		public JsonResult ReloadDBEdit([FromBody]RequestReloadDBEditModel requestModel)
		{
			...
			*/
			Models.Roigf row = new Models.Roigf(UserContext.Current, isEmpty: true);
			...
			try
			{
				switch (string.IsNullOrEmpty(Identifier) ? "" : Identifier)
				{
					case "ROIGF___ROGL1TITLE___":	// Field (DB)
						{
							var model = new Roigf_ViewModel(UserContext.Current) { editable = false };
							model.MapFromModel(row);
							model.Load_Roigf___rogl1title___(qs);
		...
		public JsonResult GetDependants([FromBody]RequestDependantsModel requestModel)
		{
			...
			try
			{
				...
				{
					case "ROIGF___ROGL1TITLE___":	// Field (DB)
						values = new Roigf_ViewModel(UserContext.Current).GetDependant_RoigfTableRogl1Title(Selected);
						break;
					default: break;
				}
		...
		/// <summary>
		/// Recalculate formulas of the "Roigf" form. (++, CT, SR, CL and U1)
		/// </summary>
		/// <param name="formData">Current form data</param>
		/// <returns></returns>
		[HttpPost]
		public JsonResult RecalculateFormulas_Roigf([FromBody]Roigf_ViewModel formData)
		{
			return GenericRecalculateFormulas(formData, "roigf",
				(primaryKey) => Models.Roigf.Find(primaryKey, UserContext.Current, "FROIGF"),
				(model) => formData.MapToModel(model as Models.Roigf)
			);
		}
		...
		{
			return base.GetDocumsTickets("ROIGF", requestModel.FieldName, requestModel.KeyValue);
		}
