...
namespace GenioMVC.Controllers
{
	public partial class SearchController : ControllerBase
	{
		...
		public SearchController(UserContextService userContext, IChatbotService aiService) : base(userContext)
		{
			_aiService = aiService;
		}

// USE /[MANUAL GQT CONTROLLER_NAVIGATION SEARCH]/



		private List<string> GetActionIds(CriteriaSet crs, CSGenio.persistence.PersistentSupport sp = null)
		{
			CSGenio.business.Area area = CSGenio.business.Area.createArea<CSGenioAsearch>(UserContext.Current.User, UserContext.Current.User.CurrentModule);
			return base.GetActionIds(crs, sp, area);
		}

// USE /[MANUAL GQT MANUAL_CONTROLLER SEARCH]/
		...
		public JsonResult ReloadDBEdit([FromBody]RequestReloadDBEditModel requestModel)
		{
			...
			*/
			Models.Search row = new Models.Search(UserContext.Current, isEmpty: true);
			row.klass.QPrimaryKey = Navigation.GetStrValue("search");
			...
			try
			{
				...
				{
					case "SEARCH__CNTRYCOUNTRY_":	// Field (DB)
						{
							var model = new Search_ViewModel(UserContext.Current) { editable = false };
							model.MapFromModel(row);
							model.Load_Search__cntrycountry_(qs);
							result = model.TableCntryCountry;
						}
						break;
					case "SEARCH__REGIOREGIAO__":	// Field (DB)
						{
							var model = new Search_ViewModel(UserContext.Current) { editable = false };
							model.MapFromModel(row);
							model.Load_Search__regioregiao__(qs);
		...
		public JsonResult GetDependants([FromBody]RequestDependantsModel requestModel)
		{
			...
			try
			{
				...
				{
					case "SEARCH__CNTRYCOUNTRY_":	// Field (DB)
						values = new Search_ViewModel(UserContext.Current).GetDependant_SearchTableCntryCountry(Selected);
						break;
					case "SEARCH__REGIOREGIAO__":	// Field (DB)
						values = new Search_ViewModel(UserContext.Current).GetDependant_SearchTableRegioRegiao(Selected);
						break;
					default: break;
				}
		...
		/// <summary>
		/// Recalculate formulas of the "Search" form. (++, CT, SR, CL and U1)
		/// </summary>
		/// <param name="formData">Current form data</param>
		/// <returns></returns>
		[HttpPost]
		public JsonResult RecalculateFormulas_Search([FromBody]Search_ViewModel formData)
		{
			return GenericRecalculateFormulas(formData, "search",
				(primaryKey) => Models.Search.Find(primaryKey, UserContext.Current, "FSEARCH"),
				(model) => formData.MapToModel(model as Models.Search)
			);
		}
		...
		{
			return base.GetDocumsTickets("SEARCH", requestModel.FieldName, requestModel.KeyValue);
		}
