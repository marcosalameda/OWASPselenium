...
namespace GenioMVC.Controllers
{
	public partial class TblkController : ControllerBase
	{
		...
		public ActionResult Tblk_ModalDBEdit()
		{
			Tblk_ViewModel model = new Tblk_ViewModel(UserContext.Current);
...
// USE /[MANUAL GQT AFTER_CANCEL TBLK]/
		...
		{
			var model = new Tblk_ViewModel(UserContext.Current, true);
			model.setModes(Request.Query["m"].ToString());
			PersistentSupport sp = UserContext.Current.PersistentSupport;
			var navigationLocationAction = ACTION_TBLK_NEW.SetRoutedValues(new { m = Request.Query["m"].ToString() });
		...
		[HttpPost]
		public ActionResult MFTblk_New_GET()
		{
			return MFTblk_New();
		}
		...
		public ActionResult MFTblk_Edit([FromBody]RequestIdModel requestModel)
		{
			var id = requestModel.Id;
			return RedirectToFormAction("TBLK", "EDIT", new { id = id, partialView = "MFTblk", nestedForm = "true", multiForm = "true" });
		}

		[HttpPost]
		public ActionResult MFTblk_Edit_GET([FromBody]RequestIdModel requestModel)
		{
			return MFTblk_Edit(requestModel);
		}

		//
		// GET /Tblk/MFTblk_Cancel
		[ActionName("MFTblk_Cancel")]
		public ActionResult MFTblk_Cancel([FromBody]RequestIdModel requestModel)
		{
			var id = requestModel.Id;
		...
		public JsonResult MFTblk_Delete([FromBody]RequestIdModel requestModel)
		{
			var id = requestModel.Id;
			var eventSink = new EventSink()
			{
		...
		public ActionResult Tblk_GrpbValName([FromBody]RequestLookupModel requestModel)
		{
			...
			model.setModes(Request.Query["m"].ToString());
			model.ValCodtblk = requestModel.Id;
