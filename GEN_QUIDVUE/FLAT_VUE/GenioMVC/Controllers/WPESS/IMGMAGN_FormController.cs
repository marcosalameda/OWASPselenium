...
namespace GenioMVC.Controllers
{
	public partial class WpessController : ControllerBase
	{
		...
		public ActionResult Imgmagn_ModalDBEdit()
		{
			Imgmagn_ViewModel model = new Imgmagn_ViewModel(UserContext.Current);
...
// USE /[MANUAL GQT AFTER_CANCEL IMGMAGN]/
		...
		{
			var model = new Imgmagn_ViewModel(UserContext.Current, true);
			model.setModes(Request.Query["m"].ToString());
			PersistentSupport sp = UserContext.Current.PersistentSupport;
			var navigationLocationAction = ACTION_IMGMAGN_NEW.SetRoutedValues(new { m = Request.Query["m"].ToString() });
		...
		[HttpPost]
		public ActionResult MFImgmagn_New_GET()
		{
			return MFImgmagn_New();
		}
		...
		public ActionResult MFImgmagn_Edit([FromBody]RequestIdModel requestModel)
		{
			var id = requestModel.Id;
			return RedirectToFormAction("IMGMAGN", "EDIT", new { id = id, partialView = "MFImgmagn", nestedForm = "true", multiForm = "true" });
		}

		[HttpPost]
		public ActionResult MFImgmagn_Edit_GET([FromBody]RequestIdModel requestModel)
		{
			return MFImgmagn_Edit(requestModel);
		}

		//
		// GET /Wpess/MFImgmagn_Cancel
		[ActionName("MFImgmagn_Cancel")]
		public ActionResult MFImgmagn_Cancel([FromBody]RequestIdModel requestModel)
		{
			var id = requestModel.Id;
		...
		public JsonResult MFImgmagn_Delete([FromBody]RequestIdModel requestModel)
		{
			var id = requestModel.Id;
			var eventSink = new EventSink()
			{
