...
namespace GenioMVC.Controllers
{
	public partial class WpessController : ControllerBase
	{
		...
		public ActionResult Pesspop_ModalDBEdit()
		{
			Pesspop_ViewModel model = new Pesspop_ViewModel(UserContext.Current);
...
// USE /[MANUAL GQT AFTER_CANCEL PESSPOP]/
		...
		{
			var model = new Pesspop_ViewModel(UserContext.Current, true);
			model.setModes(Request.Query["m"].ToString());
			PersistentSupport sp = UserContext.Current.PersistentSupport;
			var navigationLocationAction = ACTION_PESSPOP_NEW.SetRoutedValues(new { m = Request.Query["m"].ToString() });
		...
		[HttpPost]
		public ActionResult MFPesspop_New_GET()
		{
			return MFPesspop_New();
		}
		...
		public ActionResult MFPesspop_Edit([FromBody]RequestIdModel requestModel)
		{
			var id = requestModel.Id;
			return RedirectToFormAction("PESSPOP", "EDIT", new { id = id, partialView = "MFPesspop", nestedForm = "true", multiForm = "true" });
		}

		[HttpPost]
		public ActionResult MFPesspop_Edit_GET([FromBody]RequestIdModel requestModel)
		{
			return MFPesspop_Edit(requestModel);
		}

		//
		// GET /Wpess/MFPesspop_Cancel
		[ActionName("MFPesspop_Cancel")]
		public ActionResult MFPesspop_Cancel([FromBody]RequestIdModel requestModel)
		{
			var id = requestModel.Id;
		...
		public JsonResult MFPesspop_Delete([FromBody]RequestIdModel requestModel)
		{
			var id = requestModel.Id;
			var eventSink = new EventSink()
			{
		...
		public ActionResult Pesspop_WarehValWarehdes([FromBody]RequestLookupModel requestModel)
		{
			...
			model.setModes(Request.Query["m"].ToString());
			model.ValCodpess = requestModel.Id;
