...
// USE /[MANUAL GQT CONTROLLER_NAVIGATION WPESS]/
		...
		{
			CSGenio.business.Area area = CSGenio.business.Area.createArea<CSGenioAwpess>(UserContext.Current.User, UserContext.Current.User.CurrentModule);
...
// USE /[MANUAL GQT MANUAL_CONTROLLER WPESS]/
		...
		public JsonResult ReloadDBEdit([FromBody]RequestReloadDBEditModel requestModel)
		{
			...
			try
			{
				row = Models.Wpess.Find(Navigation.GetStrValue("wpess"), UserContext.Current);
			}
			catch (Exception)
			{
			...
			if (row == null)
			{
				row = new Models.Wpess(UserContext.Current);
		...
		public JsonResult GetDependants([FromBody]RequestDependantsModel requestModel)
		{
			...
			try
			{
				...
				{
					case "ARMAPESSWAREHWAREHDES":	// Field (DB)
						values = new Armapess_ViewModel(UserContext.Current).GetDependant_ArmapessTableWarehWarehdes(Selected);
						break;
					case "PESSPOP_WAREHWAREHDES":	// Field (DB)
						values = new Pesspop_ViewModel(UserContext.Current).GetDependant_PesspopTableWarehWarehdes(Selected);
						break;
					default: break;
				}
		...
		/// <returns></returns>
		[HttpPost]
		public JsonResult RecalculateFormulas_Armapess([FromBody]Armapess_ViewModel form_data)
		{
			return GenericRecalculateFormulas(form_data, "wpess",
				(primaryKey) => Models.Wpess.Find(primaryKey, UserContext.Current, "FARMAPESS"),
				(model) => form_data.MapToModel(model as Models.Wpess)
			);
		}
		...
		/// <returns></returns>
		[HttpPost]
		public JsonResult RecalculateFormulas_Imgmagn([FromBody]Imgmagn_ViewModel form_data)
		{
			return GenericRecalculateFormulas(form_data, "wpess",
				(primaryKey) => Models.Wpess.Find(primaryKey, UserContext.Current, "FIMGMAGN"),
				(model) => form_data.MapToModel(model as Models.Wpess)
			);
		}
		...
		/// <returns></returns>
		[HttpPost]
		public JsonResult RecalculateFormulas_Pesspop([FromBody]Pesspop_ViewModel form_data)
		{
			return GenericRecalculateFormulas(form_data, "wpess",
				(primaryKey) => Models.Wpess.Find(primaryKey, UserContext.Current, "FPESSPOP"),
				(model) => form_data.MapToModel(model as Models.Wpess)
			);
		}
