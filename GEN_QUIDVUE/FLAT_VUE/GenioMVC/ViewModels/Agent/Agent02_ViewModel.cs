...
{
	public class Agent02_ViewModel : FormViewModel<Models.Agent>, IPreparableForSerialization
	{
		[JsonIgnore]
		...
		public Agent02_ViewModel() : base(null!) { }

		public Agent02_ViewModel(UserContext userContext, bool nestedForm = false) : base(userContext, "FAGENT02", nestedForm) { }

		public Agent02_ViewModel(UserContext userContext, Models.Agent row, bool nestedForm = false) : base(userContext, "FAGENT02", row, nestedForm) { }

		public Agent02_ViewModel(UserContext userContext, string id, bool nestedForm = false, string[]? fieldsToLoad = null) : this(userContext, nestedForm)
		{
			this.Navigation.SetValue("agent", id);
			Model = Models.Agent.Find(id, userContext, "FAGENT02", fieldsToQuery: fieldsToLoad);
			if (Model == null)
		...
		public static StatusMessage InsertConditions(UserContext userContext)
		{
			...
			Models.Agent model = new Models.Agent(userContext) { Identifier = "FAGENT02" };

			var navigation = m_userContext.CurrentNavigation;
			// The "LoadKeysFromHistory" must be after the "LoadEPH" because the PHE's in the tree mark Foreign Keys to null
			// (since they cannot assign multiple values to a single field) and thus the value that comes from Navigation is lost.
			// And this makes it more like the order of loading the model when opening the form.
			model.LoadEPH("FAGENT02");
			if (navigation != null)
		...
		public override void MapFromModel(Models.Agent m)
		{
			if (m == null)
			{
				CSGenio.framework.Log.Error("Map Model (Agent) to ViewModel (Agent02) - Model is a null reference");
				throw new ModelNotFoundException("Model not found");
			}
			...
			{
				CSGenio.framework.Log.Error("Map Model (Agent) to ViewModel (Agent02) - Error during mapping");
				throw;
			}
		}
		...
		public override void MapToModel(Models.Agent m)
		{
			if (m == null)
			{
				CSGenio.framework.Log.Error("Map ViewModel (Agent02) to Model (Agent) - Model is a null reference");
				throw new ModelNotFoundException("Model not found");
			}
			...
			{
				CSGenio.framework.Log.Error($"Map ViewModel (Agent02) to Model (Agent) - Error during mapping. All user values: {HasDisabledUserValuesSecurity}");
				throw;
			}
		}
			...
			try
			{
				...
				switch (fullFieldName)
				{
					...
					case "agent.codagent":
						...
						break;
					default:
						Log.Error($"SetViewModelValue (Agent02) - Unexpected field identifier {fullFieldName}");
						break;
				}
			}
			catch (Exception ex)
			{
				throw new FrameworkException(Resources.Resources.PEDIMOS_DESCULPA__OC63848, "SetViewModelValue (Agent02)", "Unexpected error", ex);
			}
		}
		...
		{
			try { Model = Models.Agent.Find(id ?? Navigation.GetStrValue("agent"), m_userContext, "FAGENT02"); }
			finally { Model ??= new Models.Agent(m_userContext) { Identifier = "FAGENT02" }; }
		...
		public override void Load(NameValueCollection qs, bool editable, bool ajaxRequest = false, bool lazyLoad = false)
		{
			...
			try
			{
				Model = Models.Agent.Find(Navigation.GetStrValue("agent"), m_userContext, "FAGENT02");
			}
			finally
			{
				if (Model == null)
		...
		public override void LoadPartial(NameValueCollection qs, bool lazyLoad = false)
		{
			...
			if (Model == null)
			{
				...
				if (Model == null)
				{
					Model = new Models.Agent(m_userContext) { Identifier = "FAGENT02" };
...
// USE /[MANUAL GQT VIEWMODEL_LOADPARTIAL AGENT02]/
		}

// USE /[MANUAL GQT VIEWMODEL_NEW AGENT02]/
...
// USE /[MANUAL GQT VIEWMODEL_SAVE AGENT02]/
...
// USE /[MANUAL GQT VIEWMODEL_APPLY AGENT02]/

// USE /[MANUAL GQT VIEWMODEL_DUPLICATE AGENT02]/

// USE /[MANUAL GQT VIEWMODEL_DESTROY AGENT02]/
		public override void Destroy(string id)
		{
			Model = Models.Agent.Find(id, m_userContext, "FAGENT02");
			if (Model == null)
				throw new ModelNotFoundException("Model not found");
...
// USE /[MANUAL GQT VIEWMODEL_CUSTOM AGENT02]/
