...
{
	public class TRN_Menu_T02AGENT_ViewModel : ListViewModel
	{
		/// <summary>
		/// Gets or sets the object that represents the table and its elements. List type: "${exposeField.Fajuda}"
		/// </summary>
		[JsonPropertyName("Table")]
		public TablePartial<TRN_Menu_T02AGENT_RowViewModel> Menu { get; set; }

		/// <inheritdoc/>
		public override string TableAlias { get => "agent"; }

		/// <inheritdoc/>
		public override string Uuid { get => "472be16b-6b14-4c30-a982-53f54ef89d9d"; }
		...
		public override int GetCount(User user)
		{
			...
			CriteriaSet trn_menu_t02agentConds = CSGenio.business.Listing.CalculateConditionsEphGeneric(areaBase, "MLT02AGENT");
			trn_menu_t02agentConds.Equal(CSGenioAagent.FldZzstate, 0); //valid zzstate only

			//Menu fixed limits and relations:

			

// USE /[MANUAL TRN OVERRQ T02AGENT]/

			// Checks for foreign tables in fields and conditions
			FieldRef[] fields = new FieldRef[] { CSGenioAagent.FldCodagent, CSGenioAagent.FldZzstate, CSGenioAagent.FldBirthdat, CSGenioAagent.FldName, CSGenioAagent.FldPhoto, CSGenioAagent.FldEmail, CSGenioAagent.FldTelephon };

			ListingMVC<CSGenioAagent> listing = new ListingMVC<CSGenioAagent>(fields, null, 1, 1, false, user, true, string.Empty, false);
			SelectQuery qs = sp.getSelectQueryFromListingMVC(trn_menu_t02agentConds, listing);
		...
		/// <summary>
		/// Initializes a new instance of the <see cref="TRN_Menu_T02AGENT_ViewModel" /> class.
		/// </summary>
		/// <param name="userContext">The current user request context</param>
		public TRN_Menu_T02AGENT_ViewModel(UserContext userContext) : base(userContext)
		{
		...
		public override CriteriaSet BuildCriteriaSet(CSGenio.framework.TableConfiguration.TableConfiguration tableConfig, NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			if (Menu == null)
				Menu = new TablePartial<TRN_Menu_T02AGENT_RowViewModel>();
			...
			{
				// EPH
				crs = Models.Agent.AddEPH<CSGenioAagent>(ref u, crs, "MLT02AGENT");
			...
			if (tableReload)
			{
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					crs.Equals(Models.Agent.AddEPH<CSGenioAagent>(ref u, null, "MLT02AGENT"));
			}
		...
		public void Load(CSGenio.framework.TableConfiguration.TableConfiguration tableConfig, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAagent> Qlisting, ref CriteriaSet conditions)
		{
			using (GenioDI.MetricsOtlp.RecordTime("menu_load_time", new List<KeyValuePair<string, object>>() {
				...
				CriteriaSet trn_menu_t02agentConds = CriteriaSet.And();
			...
			//Tooltip for EPHs affecting this viewmodel list
			{
				...
				List<Limit> area_EPH_limits = EPH_Limit_Filler(ref limit, model_limit_area, "MLT02AGENT");
				...
				conditions.SubSets.Add(trn_menu_t02agentConds);
				trn_menu_t02agentConds = BuildCriteriaSet(tableConfig, requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
...
// USE /[MANUAL TRN OVERRQ T02AGENT]/
				...
				if (isToExport)
				{
					...
					Qlisting = Models.ModelBase.Where<CSGenioAagent>(m_userContext, false, trn_menu_t02agentConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "MLT02AGENT", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL TRN OVERRQLSTEXP T02AGENT]/
				...
				if (tableReload)
				{
// USE /[MANUAL TRN OVERRQLIST T02AGENT]/
					...
					if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					{
						var m_iCurPag = m_userContext.PersistentSupport.getPagingPos(CSGenioAagent.GetInformation(), QMVC_POS_RECORD, sorts, trn_menu_t02agentConds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
						if (m_iCurPag != -1)
							pageNumber = ((m_iCurPag - 1) / numberListItems) + 1;
					}

					ListingMVC<CSGenioAagent> listing = Models.ModelBase.Where<CSGenioAagent>(m_userContext, false, trn_menu_t02agentConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "MLT02AGENT", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn, fieldsWithTotalizers, tableConfig.SelectedRows);
					...
					Menu.Elements = MapTRN_Menu_T02AGENT(listing);

					Menu.Identifier = "MLT02AGENT";
					Menu.Slots = new Dictionary<string, List<object>>();

					// Last updated by [CJP] at [2015.02.03]
					// Adds the identifier to each element
					foreach (var element in Menu.Elements)
						element.Identifier = "MLT02AGENT";
		...
		private List<TRN_Menu_T02AGENT_RowViewModel> MapTRN_Menu_T02AGENT(ListingMVC<CSGenioAagent> Qlisting)
		{
			var Elements = new List<TRN_Menu_T02AGENT_RowViewModel>();
			int i = 0;
			...
			if (Qlisting.Rows != null)
			{
				foreach (var row in Qlisting.Rows)
				{
					if (Qlisting.NumRegs > 0 && i >= Qlisting.NumRegs) // Copiado da versão antiga do RowsToViewModels
						break;
					Elements.Add(MapTRN_Menu_T02AGENT(row));
					i++;
				}
			}
		...
		/// to a TRN_Menu_T02AGENT_RowViewModel object.
		/// </summary>
		/// <param name="row">The row.</param>
		private TRN_Menu_T02AGENT_RowViewModel MapTRN_Menu_T02AGENT(CSGenioAagent row)
		{
			var model = new TRN_Menu_T02AGENT_RowViewModel(m_userContext, true, _fieldsToSerialize);
		...
		/// </summary>
		public void CalculateButtonPermissions(TRN_Menu_T02AGENT_RowViewModel model)
		{
			bool canView = true;
			bool canEdit = true;
			bool canDelete = true;
		...
		#region Custom code
// USE /[MANUAL GQT VIEWMODEL_CUSTOM TRN_MENU_T02AGENT]/
		#endregion

		private static readonly string[] _fieldsToSerialize =
		[
			"Agent", "Agent.ValCodagent", "Agent.ValZzstate", "Agent.ValBirthdat", "Agent.ValName", "Agent.ValPhoto", "Agent.ValEmail", "Agent.ValTelephon", "BtnPermission"
		];
