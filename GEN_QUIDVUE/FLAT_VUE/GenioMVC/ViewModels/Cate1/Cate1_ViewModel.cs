...
{
	public class Cate1_ViewModel : FormViewModel<Models.Cate1>, IPreparableForSerialization
	{
		[JsonIgnore]
		...
		public override void MapToModel(Models.Cate1 m)
		{
			...
			{
				CSGenio.framework.Log.Error($"Map ViewModel (Cate1) to Model (Cate1) - Error during mapping. All user values: {HasDisabledUserValuesSecurity}");
				throw;
			}
		}
			...
			try
			{
				...
				switch (fullFieldName)
				{
					...
					case "cate1.codcateg":
						...
						break;
					default:
						Log.Error($"SetViewModelValue (Cate1) - Unexpected field identifier {fullFieldName}");
						break;
				}
			}
			catch (Exception ex)
			{
				throw new FrameworkException(Resources.Resources.PEDIMOS_DESCULPA__OC63848, "SetViewModelValue (Cate1)", "Unexpected error", ex);
			}
		}
		...
		{
			try { Model = Models.Cate1.Find(id ?? Navigation.GetStrValue("cate1"), m_userContext, "FCATE1"); }
			finally { Model ??= new Models.Cate1(m_userContext) { Identifier = "FCATE1" }; }
