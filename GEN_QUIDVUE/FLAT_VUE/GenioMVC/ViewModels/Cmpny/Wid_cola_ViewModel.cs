...
{
	public class Wid_cola_ViewModel : FormViewModel<Models.Cmpny>, IPreparableForSerialization
	{
		[JsonIgnore]
		...
		public Wid_cola_ViewModel() : base(null!) { }

		public Wid_cola_ViewModel(UserContext userContext, bool nestedForm = false) : base(userContext, "FWID_COLA", nestedForm) { }

		public Wid_cola_ViewModel(UserContext userContext, Models.Cmpny row, bool nestedForm = false) : base(userContext, "FWID_COLA", row, nestedForm) { }

		public Wid_cola_ViewModel(UserContext userContext, string id, bool nestedForm = false, string[]? fieldsToLoad = null) : this(userContext, nestedForm)
		{
			this.Navigation.SetValue("cmpny", id);
			Model = Models.Cmpny.Find(id, userContext, "FWID_COLA", fieldsToQuery: fieldsToLoad);
			if (Model == null)
		...
		public static StatusMessage InsertConditions(UserContext userContext)
		{
			...
			Models.Cmpny model = new Models.Cmpny(userContext) { Identifier = "FWID_COLA" };

			var navigation = m_userContext.CurrentNavigation;
			// The "LoadKeysFromHistory" must be after the "LoadEPH" because the PHE's in the tree mark Foreign Keys to null
			// (since they cannot assign multiple values to a single field) and thus the value that comes from Navigation is lost.
			// And this makes it more like the order of loading the model when opening the form.
			model.LoadEPH("FWID_COLA");
			if (navigation != null)
		...
		public override void MapToModel(Models.Cmpny m)
		{
			...
			{
				CSGenio.framework.Log.Error($"Map ViewModel (Wid_cola) to Model (Cmpny) - Error during mapping. All user values: {HasDisabledUserValuesSecurity}");
				throw;
			}
		}
			...
			try
			{
				...
				switch (fullFieldName)
				{
					...
					case "cmpny.codempre":
						...
						break;
					default:
						Log.Error($"SetViewModelValue (Wid_cola) - Unexpected field identifier {fullFieldName}");
						break;
				}
			}
			catch (Exception ex)
			{
				throw new FrameworkException(Resources.Resources.PEDIMOS_DESCULPA__OC63848, "SetViewModelValue (Wid_cola)", "Unexpected error", ex);
			}
		}
		...
		{
			try { Model = Models.Cmpny.Find(id ?? Navigation.GetStrValue("cmpny"), m_userContext, "FWID_COLA"); }
			finally { Model ??= new Models.Cmpny(m_userContext) { Identifier = "FWID_COLA" }; }
		...
		public override void Load(NameValueCollection qs, bool editable, bool ajaxRequest = false, bool lazyLoad = false)
		{
			...
			try
			{
				Model = Models.Cmpny.Find(Navigation.GetStrValue("cmpny"), m_userContext, "FWID_COLA");
			}
			finally
			{
				if (Model == null)
		...
		public override void LoadPartial(NameValueCollection qs, bool lazyLoad = false)
		{
			...
			if (Model == null)
			{
				...
				if (Model == null)
				{
					Model = new Models.Cmpny(m_userContext) { Identifier = "FWID_COLA" };
...
// USE /[MANUAL GQT VIEWMODEL_DESTROY WID_COLA]/
		...
		{
			Model = Models.Cmpny.Find(id, m_userContext, "FWID_COLA");
			if (Model == null)
				throw new ModelNotFoundException("Model not found");
