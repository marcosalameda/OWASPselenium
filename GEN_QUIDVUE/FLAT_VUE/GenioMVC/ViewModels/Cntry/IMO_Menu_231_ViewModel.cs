...
{
	public class IMO_Menu_231_ViewModel : ListViewModel
	{
		/// <summary>
		...
		/// <inheritdoc/>
		public override string Uuid { get => "92e9ac39-cb12-4a4f-9153-c5e720ca8e78"; }
...
FieldRef[] fields = new FieldRef[] { CSGenioAcntry.FldCodcntry, CSGenioAcntry.FldZzstate, CSGenioAcntry.FldCountry, CSGenioAcntry.FldActive, CSGenioAcntry.FldCodigonr, CSGenioAcntry.FldAlfa2, CSGenioAcntry.FldAlfa3 };
		...
		/// <summary>
		/// Initializes a new instance of the <see cref="IMO_Menu_231_ViewModel" /> class.
		/// </summary>
		/// <param name="userContext">The current user request context</param>
		public IMO_Menu_231_ViewModel(UserContext userContext) : base(userContext)
		{
		...
		public CriteriaSet BuildCriteriaSet(NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			Menu.SetFilters(bool.Parse(requestValues["IMO_Menu_231_tableFilters"] ?? "false"), false);
			...
			LoadUserTableConfig(requestValues, allSortOrders, "IMO_Menu_231", ref numberListItems);

			crs.SubSets.Add(ProcessSearchFilters(Menu, GetSearchColumns(true), requestValues, "IMO_Menu_231_"));
			...
			{
				// EPH
				crs = Models.Cntry.AddEPH<CSGenioAcntry>(ref u, crs, "ML231");
			...
			if (tableReload)
			{
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					crs.Equals(Models.Cntry.AddEPH<CSGenioAcntry>(ref u, null, "ML231"));
			}
		...
		public void Load(int numberListItems, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAcntry> Qlisting, ref CriteriaSet conditions)
		{
			...
			if (ajaxRequest)
				this.Navigation.SetValue("requestValues" + "IMO_Menu_231", requestValues);
			else if (!ajaxRequest && this.Navigation.CheckKey("requestValues" + "IMO_Menu_231"))
				requestValues = this.Navigation.GetValue<NameValueCollection>("requestValues" + "IMO_Menu_231");

			User u = m_userContext.User;
			Menu = new TablePartial<GenioMVC.Models.Cntry>();

			CriteriaSet imo_menu_231Conds = CriteriaSet.And();
			...
			var pageNumber = (ajaxRequest && !String.IsNullOrEmpty(requestValues["pIMO_Menu_231"])) ? int.Parse(requestValues["pIMO_Menu_231"]) : 1;

			// Added to avoid 0 or -1 pages when setting number of records to -1 to disable pagination
			if (pageNumber < 1)
				pageNumber = 1;

			List<ColumnSort> sorts = GetRequestSorts(this.Menu, "sIMO_Menu_231", "dIMO_Menu_231", requestValues, "cntry", allSortOrders);
...
FieldRef[] fields = new FieldRef[] { CSGenioAcntry.FldCodcntry, CSGenioAcntry.FldZzstate, CSGenioAcntry.FldCountry, CSGenioAcntry.FldActive, CSGenioAcntry.FldCodigonr, CSGenioAcntry.FldAlfa2, CSGenioAcntry.FldAlfa3 };
			...
			conditions.SubSets.Add(imo_menu_231Conds);
			imo_menu_231Conds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
...
// USE /[MANUAL IMO OVERRQ 231]/
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAcntry>(m_userContext, false, imo_menu_231Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "ML231", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL IMO OVERRQLSTEXP 231]/
...
// USE /[MANUAL IMO OVERRQLIST 231]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = m_userContext.PersistentSupport.getPagingPos(CSGenioAcntry.GetInformation(), QMVC_POS_RECORD, sorts, imo_menu_231Conds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioAcntry> listing = Models.ModelBase.Where<CSGenioAcntry>(m_userContext, false, imo_menu_231Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "ML231", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
				...
				Menu.Elements = MapIMO_Menu_231(listing);
		...
		private List<Models.Cntry> MapIMO_Menu_231(ListingMVC<CSGenioAcntry> Qlisting)
		{
			...
			if (Qlisting.Rows != null)
			{
				foreach (var row in Qlisting.Rows)
				{
					if (Qlisting.NumRegs > 0 && i >= Qlisting.NumRegs) // Copiado da versão antiga do RowsToViewModels
						break;
					Elements.Add(MapIMO_Menu_231(row));
					i++;
				}
			}
		...
		private Models.Cntry MapIMO_Menu_231(CSGenioAcntry row)
		{
		...
		#region Custom code
// USE /[MANUAL GQT VIEWMODEL_CUSTOM IMO_MENU_231]/
		#endregion

		private static readonly string[] _fieldsToSerialize =
		{
			"Cntry", "Cntry.ValCodcntry", "Cntry.ValZzstate", "Cntry.ValCountry", "Cntry.ValActive", "Cntry.ValCodigonr", "Cntry.ValAlfa2", "Cntry.ValAlfa3"
		};
