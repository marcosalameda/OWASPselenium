...
{
	public class Proppais_ViewModel : FormViewModel<Models.Cntry>, IPreparableForSerialization
	{
		[JsonIgnore]
		...
		public override void MapToModel(Models.Cntry m)
		{
			...
			{
				CSGenio.framework.Log.Error($"Map ViewModel (Proppais) to Model (Cntry) - Error during mapping. All user values: {HasDisabledUserValuesSecurity}");
				throw;
			}
		}
			...
			try
			{
				...
				switch (fullFieldName)
				{
					...
					case "cntry.codcntry":
						...
						break;
					default:
						Log.Error($"SetViewModelValue (Proppais) - Unexpected field identifier {fullFieldName}");
						break;
				}
			}
			catch (Exception ex)
			{
				throw new FrameworkException(Resources.Resources.PEDIMOS_DESCULPA__OC63848, "SetViewModelValue (Proppais)", "Unexpected error", ex);
			}
		}
		...
		{
			try { Model = Models.Cntry.Find(id ?? Navigation.GetStrValue("cntry"), m_userContext, "FPROPPAIS"); }
			finally { Model ??= new Models.Cntry(m_userContext) { Identifier = "FPROPPAIS" }; }
