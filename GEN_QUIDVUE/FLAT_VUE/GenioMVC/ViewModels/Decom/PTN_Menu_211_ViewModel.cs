...
{
	public class PTN_Menu_211_ViewModel : ListViewModel
	{
		/// <summary>
		...
		/// <inheritdoc/>
		public override string Uuid { get => "08896786-8e39-461e-b166-e259f92211f7"; }
		...
		{
			dbeditTitle = Resources.Resources.EQUIPMENT_DECOMISSIO62648;
		}
...
FieldRef[] fields = new FieldRef[] { CSGenioAdecom.FldCoddeco, CSGenioAdecom.FldZzstate, CSGenioAdecom.FldDtdeco, CSGenioAdecom.FldDecomnr, CSGenioAdecom.FldNote, CSGenioAdecom.FldCreatdat, CSGenioAdecom.FldCreatope, CSGenioAdecom.FldChngdate, CSGenioAdecom.FldOperchng };
		...
		/// <summary>
		/// Initializes a new instance of the <see cref="PTN_Menu_211_ViewModel" /> class.
		/// </summary>
		/// <param name="userContext">The current user request context</param>
		public PTN_Menu_211_ViewModel(UserContext userContext) : base(userContext)
		{
		...
		public CriteriaSet BuildCriteriaSet(NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			Menu.SetFilters(bool.Parse(requestValues["PTN_Menu_211_tableFilters"] ?? "false"), false);
			...
			LoadUserTableConfig(requestValues, allSortOrders, "PTN_Menu_211", ref numberListItems);

			crs.SubSets.Add(ProcessSearchFilters(Menu, GetSearchColumns(true), requestValues, "PTN_Menu_211_"));
			...
			{
				// EPH
				crs = Models.Decom.AddEPH<CSGenioAdecom>(ref u, crs, "ML211");
			...
			if (tableReload)
			{
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					crs.Equals(Models.Decom.AddEPH<CSGenioAdecom>(ref u, null, "ML211"));
			}
		...
		public void Load(int numberListItems, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAdecom> Qlisting, ref CriteriaSet conditions)
		{
			...
			if (ajaxRequest)
				this.Navigation.SetValue("requestValues" + "PTN_Menu_211", requestValues);
			else if (!ajaxRequest && this.Navigation.CheckKey("requestValues" + "PTN_Menu_211"))
				requestValues = this.Navigation.GetValue<NameValueCollection>("requestValues" + "PTN_Menu_211");

			User u = m_userContext.User;
			Menu = new TablePartial<GenioMVC.Models.Decom>();

			CriteriaSet ptn_menu_211Conds = CriteriaSet.And();
			...
			var pageNumber = (ajaxRequest && !String.IsNullOrEmpty(requestValues["pPTN_Menu_211"])) ? int.Parse(requestValues["pPTN_Menu_211"]) : 1;

			// Added to avoid 0 or -1 pages when setting number of records to -1 to disable pagination
			if (pageNumber < 1)
				pageNumber = 1;

			List<ColumnSort> sorts = GetRequestSorts(this.Menu, "sPTN_Menu_211", "dPTN_Menu_211", requestValues, "decom", allSortOrders);
...
FieldRef[] fields = new FieldRef[] { CSGenioAdecom.FldCoddeco, CSGenioAdecom.FldZzstate, CSGenioAdecom.FldDtdeco, CSGenioAdecom.FldDecomnr, CSGenioAdecom.FldNote, CSGenioAdecom.FldCreatdat, CSGenioAdecom.FldCreatope, CSGenioAdecom.FldChngdate, CSGenioAdecom.FldOperchng };
			...
			conditions.SubSets.Add(ptn_menu_211Conds);
			ptn_menu_211Conds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
			tableReload &= hasAllRequiredLimits;

// USE /[MANUAL PTN OVERRQ 211]/
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAdecom>(m_userContext, false, ptn_menu_211Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "ML211", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL PTN OVERRQLSTEXP 211]/
...
// USE /[MANUAL PTN OVERRQLIST 211]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = m_userContext.PersistentSupport.getPagingPos(CSGenioAdecom.GetInformation(), QMVC_POS_RECORD, sorts, ptn_menu_211Conds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioAdecom> listing = Models.ModelBase.Where<CSGenioAdecom>(m_userContext, false, ptn_menu_211Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "ML211", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
				...
				Menu.Elements = MapPTN_Menu_211(listing);
		...
		private List<Models.Decom> MapPTN_Menu_211(ListingMVC<CSGenioAdecom> Qlisting)
		{
			...
			if (Qlisting.Rows != null)
			{
				foreach (var row in Qlisting.Rows)
				{
					if (Qlisting.NumRegs > 0 && i >= Qlisting.NumRegs) // Copiado da versão antiga do RowsToViewModels
						break;
					Elements.Add(MapPTN_Menu_211(row));
					i++;
				}
			}
		...
		private Models.Decom MapPTN_Menu_211(CSGenioAdecom row)
		{
		...
		#region Custom code
// USE /[MANUAL GQT VIEWMODEL_CUSTOM PTN_MENU_211]/
		#endregion

		private static readonly string[] _fieldsToSerialize =
		{
			"Decom", "Decom.ValCoddeco", "Decom.ValZzstate", "Decom.ValDtdeco", "Decom.ValDecomnr", "Decom.ValNote", "Decom.ValCreatdat", "Decom.ValCreatope", "Decom.ValChngdate", "Decom.ValOperchng"
		};
