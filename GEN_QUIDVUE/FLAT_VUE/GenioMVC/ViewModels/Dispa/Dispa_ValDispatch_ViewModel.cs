...
namespace GenioMVC.ViewModels.Dispa
{
	public class Dispa_ValDispatch_ViewModel : ListViewModel
	{
		...
		public TablePartial<Dispa_ValDispatch_RowViewModel> Menu { get; set; }
		...
		{
// USE /[MANUAL GQT LIST_LIMITS DISPA_PSEUDDISPATCH]/
		...
		public void LoadToExport(out ListingMVC<CSGenioAdilin> listing, out CriteriaSet conditions, out List<Exports.QColumn> columns, CSGenio.framework.TableConfiguration.TableConfiguration tableConfig, NameValueCollection requestValues, bool ajaxRequest = false)
		{
			listing = null;
			conditions = null;
		...
		public override CriteriaSet BuildCriteriaSet(CSGenio.framework.TableConfiguration.TableConfiguration tableConfig, NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			if (Menu == null)
				Menu = new TablePartial<Dispa_ValDispatch_RowViewModel>();
		...
		/// <param name="Qlisting">The rows.</param>
		/// <param name="conditions">The conditions.</param>
		public void Load(CSGenio.framework.TableConfiguration.TableConfiguration tableConfig, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAdilin> Qlisting, ref CriteriaSet conditions)
		{
			using (GenioDI.MetricsOtlp.RecordTime("form_load_time", new List<KeyValuePair<string, object>>() {
				...
				CriteriaSet dispa___pseuddispatchConds = CriteriaSet.And();
				...
				List<ColumnSort> sorts = GetRequestSorts(this.Menu, tableConfig.ColumnOrderBy, "dilin", allSortOrders);
			...
			//Tooltip for EPHs affecting this viewmodel list
			{
				...
				conditions.SubSets.Add(dispa___pseuddispatchConds);
				dispa___pseuddispatchConds = BuildCriteriaSet(tableConfig, requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
...
// USE /[MANUAL GQT OVERRQ DISPA_PSEUDDISPATCH]/
				...
				if (isToExport)
				{
					...
					Qlisting = Models.ModelBase.Where<CSGenioAdilin>(m_userContext, false, dispa___pseuddispatchConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_DISPA___PSEUDDISPATCH", true, firstVisibleColumn: firstVisibleColumn);
...
// USE /[MANUAL GQT OVERRQLIST DISPA_PSEUDDISPATCH]/
					...
					if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					{
						var m_iCurPag = m_userContext.PersistentSupport.getPagingPos(CSGenioAdilin.GetInformation(), QMVC_POS_RECORD, sorts, dispa___pseuddispatchConds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
						if (m_iCurPag != -1)
		...
		private List<Dispa_ValDispatch_RowViewModel> MapDispa_ValDispatch(ListingMVC<CSGenioAdilin> Qlisting)
		{
			var Elements = new List<Dispa_ValDispatch_RowViewModel>();
			int i = 0;
		...
		/// to a Dispa_ValDispatch_RowViewModel object.
		/// </summary>
		/// <param name="row">The row.</param>
		private Dispa_ValDispatch_RowViewModel MapDispa_ValDispatch(CSGenioAdilin row)
		{
			var model = new Dispa_ValDispatch_RowViewModel(m_userContext, true, _fieldsToSerialize);
		...
		/// </summary>
		public void CalculateButtonPermissions(Dispa_ValDispatch_RowViewModel model)
		{
			bool canView = true;
			bool canEdit = true;
			bool canDelete = true;
		...
		private void SetDocumentFields(ListingMVC<CSGenioAdilin> listing)
		{
			...
			foreach (CSGenioAdilin row in listing.Rows)
			{
			}
		}
...
// USE /[MANUAL GQT VIEWMODEL_CUSTOM DISPA_VALDISPATCH]/
		...
		[
			"Dilin", "Dilin.ValCoddilin", "Dilin.ValZzstate", "Dilin.ValLinenumb", "Produ", "Produ.ValSku", "Produ.ValGtin", "Produ.ValProduct", "Dilin.ValOrdered", "Dilin.ValDelivere", "Dilin.ValOutstand", "Dilin.ValCoddispa", "Dilin.ValCodprodu", "BtnPermission"
		];
