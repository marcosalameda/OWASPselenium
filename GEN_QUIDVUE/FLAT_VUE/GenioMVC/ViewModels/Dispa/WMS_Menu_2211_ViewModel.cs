...
// USE /[MANUAL WMS LIST_LIMITS 2211]/
		...
		public void Load(CSGenio.core.framework.table.TableConfiguration tableConfig, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAdispa> Qlisting, ref CriteriaSet conditions)
		{
			...
			conditions.SubSets.Add(wms_menu_2211Conds);
			wms_menu_2211Conds = BuildCriteriaSet(tableConfig, requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
...
// USE /[MANUAL WMS OVERRQ 2211]/
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.BuildListingForExport<CSGenioAdispa>(m_userContext, false, ref wms_menu_2211Conds, exportFieldRefs, (pageNumber - 1) * numberListItems, numberListItems, sorts, "ML2211", true, firstVisibleColumn: firstVisibleColumn);
...
// USE /[MANUAL WMS OVERRQLIST 2211]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = m_userContext.PersistentSupport.getPagingPos(CSGenioAdispa.GetInformation(), QMVC_POS_RECORD, sorts, wms_menu_2211Conds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
