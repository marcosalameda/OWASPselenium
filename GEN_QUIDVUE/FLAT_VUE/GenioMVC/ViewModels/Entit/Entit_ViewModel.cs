...
{
	public class Entit_ViewModel : FormViewModel<Models.Entit>, IPreparableForSerialization
	{
		[JsonIgnore]
		...
		public Entit_ViewModel() : base(null!) { }

		public Entit_ViewModel(UserContext userContext, bool nestedForm = false) : base(userContext, "FENTIT", nestedForm) { }

		public Entit_ViewModel(UserContext userContext, Models.Entit row, bool nestedForm = false) : base(userContext, "FENTIT", row, nestedForm) { }

		public Entit_ViewModel(UserContext userContext, string id, bool nestedForm = false, string[]? fieldsToLoad = null) : this(userContext, nestedForm)
		{
			this.Navigation.SetValue("entit", id);
			Model = Models.Entit.Find(id, userContext, "FENTIT", fieldsToQuery: fieldsToLoad);
			if (Model == null)
		...
		public static StatusMessage InsertConditions(UserContext userContext)
		{
			...
			Models.Entit model = new Models.Entit(userContext) { Identifier = "FENTIT" };

			var navigation = m_userContext.CurrentNavigation;
			// The "LoadKeysFromHistory" must be after the "LoadEPH" because the PHE's in the tree mark Foreign Keys to null
			// (since they cannot assign multiple values to a single field) and thus the value that comes from Navigation is lost.
			// And this makes it more like the order of loading the model when opening the form.
			model.LoadEPH("FENTIT");
			if (navigation != null)
		...
		public override void MapToModel(Models.Entit m)
		{
			...
			{
				CSGenio.framework.Log.Error($"Map ViewModel (Entit) to Model (Entit) - Error during mapping. All user values: {HasDisabledUserValuesSecurity}");
				throw;
			}
		}
			...
			try
			{
				...
				switch (fullFieldName)
				{
					...
					case "entit.codentit":
						...
						break;
					default:
						Log.Error($"SetViewModelValue (Entit) - Unexpected field identifier {fullFieldName}");
						break;
				}
			}
			catch (Exception ex)
			{
				throw new FrameworkException(Resources.Resources.PEDIMOS_DESCULPA__OC63848, "SetViewModelValue (Entit)", "Unexpected error", ex);
			}
		}
		...
		{
			try { Model = Models.Entit.Find(id ?? Navigation.GetStrValue("entit"), m_userContext, "FENTIT"); }
			finally { Model ??= new Models.Entit(m_userContext) { Identifier = "FENTIT" }; }
		...
		public override void Load(NameValueCollection qs, bool editable, bool ajaxRequest = false, bool lazyLoad = false)
		{
			...
			try
			{
				Model = Models.Entit.Find(Navigation.GetStrValue("entit"), m_userContext, "FENTIT");
			}
			finally
			{
				if (Model == null)
		...
		public override void LoadPartial(NameValueCollection qs, bool lazyLoad = false)
		{
			...
			if (Model == null)
			{
				...
				if (Model == null)
				{
					Model = new Models.Entit(m_userContext) { Identifier = "FENTIT" };
...
// USE /[MANUAL GQT VIEWMODEL_DESTROY ENTIT]/
		...
		{
			Model = Models.Entit.Find(id, m_userContext, "FENTIT");
			if (Model == null)
				throw new ModelNotFoundException("Model not found");
		...
		public void Load_Entit___faci1name____(NameValueCollection qs, bool lazyLoad = false)
		{
			...
			CriteriaSet entit___faci1name____Conds = CriteriaSet.And();
			{
				...
				{
					entit___faci1name____Conds.Equal(CSGenioAfaci1.FldCodfacil, hValue);
...
// USE /[MANUAL GQT OVERRQ ENTIT_FACI1NAME]/
				...
				*/
				...
				ListingMVC<CSGenioAfaci1> listing = Models.ModelBase.Where<CSGenioAfaci1>(m_userContext, false, entit___faci1name____Conds, fields, offset, numberItems, sorts, "LED_ENTIT___FACI1NAME____", true, false, firstVisibleColumn: firstVisibleColumn);
		...
		public ConcurrentDictionary<string, object> GetDependant_EntitTableFaci1Name(string PKey)
		{
		...
		{
			var row = GetDependant_EntitTableFaci1Name(this.ValFirstfacilitie);
			try
			{
		...
		public void Load_Entit___faci2name____(NameValueCollection qs, bool lazyLoad = false)
		{
			...
			CriteriaSet entit___faci2name____Conds = CriteriaSet.And();
			{
				...
				if (hValue != null && !(hValue is Array) && !string.IsNullOrEmpty(Convert.ToString(hValue)))
				{
					entit___faci2name____Conds.Equal(CSGenioAfaci2.FldCodfacil, hValue);
...
// USE /[MANUAL GQT OVERRQ ENTIT_FACI2NAME]/
				...
				*/
				...
				ListingMVC<CSGenioAfaci2> listing = Models.ModelBase.Where<CSGenioAfaci2>(m_userContext, false, entit___faci2name____Conds, fields, offset, numberItems, sorts, "LED_ENTIT___FACI2NAME____", true, false, firstVisibleColumn: firstVisibleColumn);
		...
		public ConcurrentDictionary<string, object> GetDependant_EntitTableFaci2Name(string PKey)
		{
		...
		{
			var row = GetDependant_EntitTableFaci2Name(this.ValLastfacilitie);
			try
			{
