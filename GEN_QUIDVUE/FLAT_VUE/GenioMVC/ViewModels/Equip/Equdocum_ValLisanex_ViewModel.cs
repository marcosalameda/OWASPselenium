...
{
	public class Equdocum_ValLisanex_ViewModel : ListViewModel
	{
		/// <summary>
		...
		/// <inheritdoc/>
		public override string Uuid { get => "Equdocum_ValLisanex"; }
		...
		/// <summary>
		/// Initializes a new instance of the <see cref="Equdocum_ValLisanex_ViewModel" /> class.
		/// </summary>
		...
		public CriteriaSet BuildCriteriaSet(NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			Menu.SetFilters(bool.Parse(requestValues["ValLisanex_tableFilters"] ?? "false"), false);
			...
			LoadUserTableConfig(requestValues, allSortOrders, "ValLisanex", ref numberListItems);

			crs.SubSets.Add(ProcessSearchFilters(Menu, GetSearchColumns(true), requestValues, "ValLisanex_"));
			...
			{
				// EPH
				crs = Models.Anexd.AddEPH<CSGenioAanexd>(ref u, crs, "IBL_EQUDOCUMPSEUDLISANEX_");
			...
			if (tableReload)
			{
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					crs.Equals(Models.Anexd.AddEPH<CSGenioAanexd>(ref u, null, "IBL_EQUDOCUMPSEUDLISANEX_"));
			}
		...
		public void Load(int numberListItems, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAanexd> Qlisting, ref CriteriaSet conditions)
		{
			...
			if (ajaxRequest)
				this.Navigation.SetValue("requestValues" + "Equdocum_ValLisanex", requestValues);
			else if (!ajaxRequest && this.Navigation.CheckKey("requestValues" + "Equdocum_ValLisanex"))
				requestValues = this.Navigation.GetValue<NameValueCollection>("requestValues" + "Equdocum_ValLisanex");

			User u = m_userContext.User;
			Menu = new TablePartial<GenioMVC.Models.Anexd>();

			CriteriaSet equdocumpseudlisanex_Conds = CriteriaSet.And();
			...
			var pageNumber = (ajaxRequest && !String.IsNullOrEmpty(requestValues["pValLisanex"])) ? int.Parse(requestValues["pValLisanex"]) : 1;

			// Added to avoid 0 or -1 pages when setting number of records to -1 to disable pagination
			if (pageNumber < 1)
				pageNumber = 1;

			List<ColumnSort> sorts = GetRequestSorts(this.Menu, "sValLisanex", "dValLisanex", requestValues, "anexd", allSortOrders);
...
FieldRef[] fields = new FieldRef[] { CSGenioAanexd.FldCodanexd, CSGenioAanexd.FldZzstate, CSGenioAanexd.FldDthranex, CSGenioAanexd.FldTitle, CSGenioAanexd.FldDocument, CSGenioAanexd.FldDocumentfk, CSGenioAanexd.FldTittradu, CSGenioAanexd.FldReferenc };
			...
			//Tooltip for EPHs affecting this viewmodel list
			{
				...
				List<Limit> area_EPH_limits = EPH_Limit_Filler(ref limit, model_limit_area, "IBL_EQUDOCUMPSEUDLISANEX_");
			...
			conditions.SubSets.Add(equdocumpseudlisanex_Conds);
			equdocumpseudlisanex_Conds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
...
// USE /[MANUAL GQT OVERRQ EQUDOCUM_PSEUDLISANEX]/
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAanexd>(m_userContext, false, equdocumpseudlisanex_Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_EQUDOCUMPSEUDLISANEX_", true, firstVisibleColumn: firstVisibleColumn);
...
// USE /[MANUAL GQT OVERRQLSTEXP EQUDOCUM_PSEUDLISANEX]/
			...
			if (tableReload)
			{
// USE /[MANUAL GQT OVERRQLIST EQUDOCUM_PSEUDLISANEX]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = m_userContext.PersistentSupport.getPagingPos(CSGenioAanexd.GetInformation(), QMVC_POS_RECORD, sorts, equdocumpseudlisanex_Conds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioAanexd> listing = Models.ModelBase.Where<CSGenioAanexd>(m_userContext, false, equdocumpseudlisanex_Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_EQUDOCUMPSEUDLISANEX_", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
				...
				Menu.Elements = MapEqudocum_ValLisanex(listing);

				Menu.Identifier = "IBL_EQUDOCUMPSEUDLISANEX_";

				// Last updated by [CJP] at [2015.02.03]
				// Adds the identifier to each element
				foreach (var element in Menu.Elements)
					element.Identifier = "IBL_EQUDOCUMPSEUDLISANEX_";
			...
			if (Qlisting.Rows != null)
			{
				foreach (var row in Qlisting.Rows)
				{
					if (Qlisting.NumRegs > 0 && i >= Qlisting.NumRegs) // Copiado da versão antiga do RowsToViewModels
						break;
					Elements.Add(MapEqudocum_ValLisanex(row));
					i++;
				}
			}
		...
		private Models.Anexd MapEqudocum_ValLisanex(CSGenioAanexd row)
		{
			var model = new Models.Anexd(m_userContext, true, _fieldsToSerialize);
		...
		#region Custom code
// USE /[MANUAL GQT VIEWMODEL_CUSTOM EQUDOCUM_VALLISANEX]/
		#endregion
