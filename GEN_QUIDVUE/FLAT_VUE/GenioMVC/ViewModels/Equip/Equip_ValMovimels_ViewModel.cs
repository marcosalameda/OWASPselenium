...
{
	public class Equip_ValMovimels_ViewModel : MenuListViewModel<Models.Movim>
	{
		/// <summary>
		/// Gets or sets the object that represents the table and its elements.
		/// </summary>
		[JsonPropertyName("table")]
		public TablePartial<Equip_ValMovimels_RowViewModel> Menu { get; set; }

		/// <inheritdoc/>
		[JsonIgnore]
		public override string TableAlias => "movim";

		/// <inheritdoc/>
		[JsonPropertyName("uuid")]
		public override string Uuid => "Equip_ValMovimels";
...
// USE /[MANUAL GQT LIST_LIMITS EQUIP_PSEUDMOVIMELS]/
		...
		public Equip_ValMovimels_ViewModel() : base(null!) { }
		...
		public Equip_ValMovimels_ViewModel(UserContext userContext, Models.ModelBase parentCtx) : this(userContext)
		{
			ParentCtx = parentCtx;
		}
		...
		public void LoadToExport(out ListingMVC<CSGenioAmovim> listing, out CriteriaSet conditions, out List<Exports.QColumn> columns, CSGenio.core.framework.table.TableConfiguration tableConfig, NameValueCollection requestValues, bool ajaxRequest = false)
		{
			listing = null;
			conditions = null;
		...
		/// <param name="Qlisting">The rows.</param>
		/// <param name="conditions">The conditions.</param>
		public void Load(CSGenio.core.framework.table.TableConfiguration tableConfig, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAmovim> Qlisting, ref CriteriaSet conditions)
		{
			User u = m_userContext.User;
			Menu = new TablePartial<Equip_ValMovimels_RowViewModel>();

			CriteriaSet equip___pseudmovimelsConds = CriteriaSet.And();
			...
			List<ColumnSort> sorts = GetRequestSorts(this.Menu, tableConfig, "movim", allSortOrders);
			...
			//Tooltip for EPHs affecting this viewmodel list
			{
				...
				CSGenioAmovim model_limit_area = new CSGenioAmovim(m_userContext.User);
			...
			conditions.SubSets.Add(equip___pseudmovimelsConds);
			equip___pseudmovimelsConds = BuildCriteriaSet(tableConfig, requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
...
// USE /[MANUAL GQT OVERRQ EQUIP_PSEUDMOVIMELS]/
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.BuildListingForExport<CSGenioAmovim>(m_userContext, false, ref equip___pseudmovimelsConds, exportFieldRefs, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_EQUIP___PSEUDMOVIMELS", true, firstVisibleColumn: firstVisibleColumn);
...
// USE /[MANUAL GQT OVERRQLIST EQUIP_PSEUDMOVIMELS]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = m_userContext.PersistentSupport.getPagingPos(CSGenioAmovim.GetInformation(), QMVC_POS_RECORD, sorts, equip___pseudmovimelsConds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
		...
		{
			List<Equip_ValMovimels_RowViewModel> Elements = [];
			int i = 0;
		...
		/// to a Equip_ValMovimels_RowViewModel object.
		/// </summary>
		/// <param name="row">The row.</param>
		private Equip_ValMovimels_RowViewModel MapEquip_ValMovimels(CSGenioAmovim row)
		{
			var model = new Equip_ValMovimels_RowViewModel(m_userContext, true, _fieldsToSerialize);
			if (row == null)
				return model;
		...
		private void SetDocumentFields(ListingMVC<CSGenioAmovim> listing)
		{
		}
