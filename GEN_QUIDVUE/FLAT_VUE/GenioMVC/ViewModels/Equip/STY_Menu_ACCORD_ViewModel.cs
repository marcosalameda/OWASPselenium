...
{
	public class STY_Menu_ACCORD_ViewModel : MenuListViewModel<Models.Equip>
	{
		/// <summary>
		/// Gets or sets the object that represents the table and its elements.
		/// </summary>
		[JsonPropertyName("table")]
		public TablePartial<STY_Menu_ACCORD_RowViewModel> Menu { get; set; }
		...
		public override string Uuid => "63f47415-010f-4c3a-890d-4d78a7386e11";
		...
		{
// USE /[MANUAL STY LIST_LIMITS ACCORD]/
		...
		public override int GetCount(User user)
		{
			...
			CriteriaSet conditions = CSGenio.business.Listing.CalculateConditionsEphGeneric(areaBase, "MLACCORD");
		...
		public STY_Menu_ACCORD_ViewModel() : base(null!) { }

		/// <summary>
		/// Initializes a new instance of the <see cref="STY_Menu_ACCORD_ViewModel" /> class.
		/// </summary>
		/// <param name="userContext">The current user request context</param>
		public STY_Menu_ACCORD_ViewModel(UserContext userContext) : base(userContext)
		{
		...
		public STY_Menu_ACCORD_ViewModel(UserContext userContext, Models.ModelBase parentCtx) : this(userContext)
		{
			ParentCtx = parentCtx;
		}
		...
		public override List<Exports.QColumn> GetColumnsToExport()
		{
			...
			[
				...
				new Exports.QColumn(CSGenioAequip.FldDtdeco, FieldType.DATE, Resources.Resources.DECOMISSION14486, 8, 0, true),
		...
		public override CriteriaSet BuildCriteriaSet(CSGenio.core.framework.table.TableConfiguration tableConfig, NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			Menu ??= new TablePartial<STY_Menu_ACCORD_RowViewModel>();
			...
			{
				// EPH
				crs = Models.Equip.AddEPH<CSGenioAequip>(ref u, crs, "MLACCORD");
			...
			if (tableReload)
			{
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					crs.Equals(Models.Equip.AddEPH<CSGenioAequip>(ref u, null, "MLACCORD"));
			}
		...
		public void Load(CSGenio.core.framework.table.TableConfiguration tableConfig, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAequip> Qlisting, ref CriteriaSet conditions)
		{
			...
			CriteriaSet sty_menu_accordConds = CriteriaSet.And();
			...
			//Tooltip for EPHs affecting this viewmodel list
			{
				...
				List<Limit> area_EPH_limits = EPH_Limit_Filler(ref limit, model_limit_area, "MLACCORD");
			...
			conditions.SubSets.Add(sty_menu_accordConds);
			sty_menu_accordConds = BuildCriteriaSet(tableConfig, requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
			tableReload &= hasAllRequiredLimits;

// USE /[MANUAL STY OVERRQ ACCORD]/
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.BuildListingForExport<CSGenioAequip>(m_userContext, false, ref sty_menu_accordConds, exportFieldRefs, (pageNumber - 1) * numberListItems, numberListItems, sorts, "MLACCORD", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL STY OVERRQLSTEXP ACCORD]/
			...
			if (tableReload)
			{
// USE /[MANUAL STY OVERRQLIST ACCORD]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = m_userContext.PersistentSupport.getPagingPos(CSGenioAequip.GetInformation(), QMVC_POS_RECORD, sorts, sty_menu_accordConds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
						pageNumber = ((m_iCurPag - 1) / numberListItems) + 1;
				}

				ListingMVC<CSGenioAequip> listing = Models.ModelBase.Where<CSGenioAequip>(m_userContext, distinct, sty_menu_accordConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "MLACCORD", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn, fieldsWithTotalizers, tableConfig.SelectedRows);
				...
				Menu.Elements = MapSTY_Menu_ACCORD(listing);

				Menu.Identifier = "MLACCORD";
				Menu.Slots = new Dictionary<string, List<object>>();

				// Last updated by [CJP] at [2015.02.03]
				// Adds the identifier to each element
				foreach (var element in Menu.Elements)
					element.Identifier = "MLACCORD";
		...
		private List<STY_Menu_ACCORD_RowViewModel> MapSTY_Menu_ACCORD(ListingMVC<CSGenioAequip> Qlisting)
		{
			List<STY_Menu_ACCORD_RowViewModel> Elements = [];
			int i = 0;
			...
			if (Qlisting.Rows != null)
			{
				foreach (var row in Qlisting.Rows)
				{
					if (Qlisting.NumRegs > 0 && i >= Qlisting.NumRegs) // Copiado da versão antiga do RowsToViewModels
						break;
					Elements.Add(MapSTY_Menu_ACCORD(row));
					i++;
				}
			}
		...
		/// to a STY_Menu_ACCORD_RowViewModel object.
		/// </summary>
		/// <param name="row">The row.</param>
		private STY_Menu_ACCORD_RowViewModel MapSTY_Menu_ACCORD(CSGenioAequip row)
		{
			var model = new STY_Menu_ACCORD_RowViewModel(m_userContext, true, _fieldsToSerialize);
			if (row == null)
				return model;
...
// USE /[MANUAL GQT VIEWMODEL_CUSTOM STY_MENU_ACCORD]/
