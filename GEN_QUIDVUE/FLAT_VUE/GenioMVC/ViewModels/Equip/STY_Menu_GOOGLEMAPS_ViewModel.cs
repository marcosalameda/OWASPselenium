...
namespace GenioMVC.ViewModels.Equip
{
	public class STY_Menu_GOOGLEMAPS_ViewModel : ListViewModel
	{
		...
		public override int GetCount(User user)
		{
			...
			CriteriaSet sty_menu_googlemapsConds = CSGenio.business.Listing.CalculateConditionsEphGeneric(areaBase, "MLGOOGLEMAPS");
			sty_menu_googlemapsConds.Equal(CSGenioAequip.FldZzstate, 0); //valid zzstate only

			//Menu fixed limits and relations:

						sty_menu_googlemapsConds.Equal(CSGenioAequip.FldShowrc, 1);
...
// USE /[MANUAL STY OVERRQ GOOGLEMAPS]/
			...
			SelectQuery qs = sp.getSelectQueryFromListingMVC(sty_menu_googlemapsConds, listing);
...
// USE /[MANUAL STY OVERRQLIST GOOGLEMAPS]/
					...
					ListingMVC<CSGenioAequip> listing = Models.ModelBase.Where<CSGenioAequip>(m_userContext, false, sty_menu_googlemapsConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "MLGOOGLEMAPS", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn, fieldsWithTotalizers, tableConfig.SelectedRows);
		...
		{
			var Elements = new List<STY_Menu_GOOGLEMAPS_RowViewModel>();
			int i = 0;
		...
		/// </summary>
		public void CalculateButtonPermissions(STY_Menu_GOOGLEMAPS_RowViewModel model)
		{
			bool canView = true;
			bool canEdit = true;
			bool canDelete = true;
