...
{
	public class Timequip_ValSecundar_ViewModel : ViewModelBase
	{
		...
		public string Uuid => "Timequip_ValSecundar";

		/// <summary>
		/// Initializes a new instance of the <see cref="Timequip_ValSecundar_ViewModel" /> class.
		/// </summary>
		...
		public Timequip_ValSecundar_ViewModel(UserContext userContext, Models.ModelBase parentCtx) : this(userContext)
		{
			ParentCtx = parentCtx;
		}
		...
		public void Load(CSGenio.core.framework.table.TableConfiguration tableConfig, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref List<Models.TimelineItem> Qlisting, ref CriteriaSet conditions)
		{
			...
			else if (!ajaxRequest && this.Navigation.CheckKey("requestValues" + "Timequip_ValSecundar"))
			...
			// PREPAIRS
			CriteriaSet filterPREPAIRS = conditions ?? CriteriaSet.And();
			List<ColumnSort> sortsPREPAIRS = new List<ColumnSort>();
			filterPREPAIRS.Equal(CSGenioArepar.FldCodequip, this.ValCodequip);
			TablePartial<GenioMVC.Models.Repar> Menu_PREPAIRS = new TablePartial<GenioMVC.Models.Repar>();
			FieldRef[] fieldsPREPAIRS = new FieldRef[] { CSGenioArepar.FldCodrepar, CSGenioArepar.FldZzstate, CSGenioArepar.FldNrrepara, CSGenioArepar.FldDtrepara, CSGenioArepar.FldHours, CSGenioArepar.FldCodpesso, CSGenioApesso.FldCodpesso, CSGenioApesso.FldName, CSGenioArepar.FldDescript, CSGenioArepar.FldCodespec, CSGenioAspeci.FldCodespec, CSGenioAspeci.FldEspecial };


			ListingMVC<CSGenioArepar> listingPREPAIRS = Models.ModelBase.Where<CSGenioArepar>(m_userContext, false, filterPREPAIRS, fieldsPREPAIRS, 0, totalrecords, sortsPREPAIRS, "IBL_TIMEQUIPPSEUDSECUNDAR", true, false);
			datalist.AddRange(MapPREPAIRS(listingPREPAIRS));
		...
		private List<Models.TimelineItem> MapPREPAIRS(ListingMVC<CSGenioArepar> Qlisting)
		{
			int i = 0;
		...
		private Models.TimelineItem MapPREPAIRS(CSGenioArepar row)
		{
			...
			model.Background = backgroundColorPREPAIRS(row);
...
// USE /[MANUAL GQT VIEWMODEL_CUSTOM TIMEQUIP_VALSECUNDAR]/
