...
{
	public class Esppe_PessoValName_ViewModel : MenuListViewModel<Models.Pesso>
	{
		/// <summary>
		/// Gets or sets the object that represents the table and its elements.
		/// </summary>
		[JsonPropertyName("Table")]
		public TablePartial<Esppe_PessoValName_RowViewModel> Menu { get; set; }
		...
		/// <inheritdoc/>
		public override string Uuid => "Esppe_PessoValName";
...
// USE /[MANUAL GQT LIST_LIMITS ESPPE_PESSONAME]/
		...
		public Esppe_PessoValName_ViewModel() : base(null!) { }

		/// <summary>
		/// Initializes a new instance of the <see cref="Esppe_PessoValName_ViewModel" /> class.
		/// </summary>
		/// <param name="userContext">The current user request context</param>
		public Esppe_PessoValName_ViewModel(UserContext userContext) : base(userContext)
		{
		...
		public Esppe_PessoValName_ViewModel(UserContext userContext, Models.ModelBase parentCtx) : this(userContext)
		{
			ParentCtx = parentCtx;
		}
		...
		public override CriteriaSet BuildCriteriaSet(CSGenio.framework.TableConfiguration.TableConfiguration tableConfig, NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			if (Menu == null)
				Menu = new TablePartial<Esppe_PessoValName_RowViewModel>();
		...
		public void Load(CSGenio.framework.TableConfiguration.TableConfiguration tableConfig, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioApesso> Qlisting, ref CriteriaSet conditions)
		{
			...
			}, "ms", "Time to load the form."))
			{
				...
				conditions.SubSets.Add(esppe___pessoname____Conds);
				esppe___pessoname____Conds = BuildCriteriaSet(tableConfig, requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
...
// USE /[MANUAL GQT OVERRQ ESPPE_PESSONAME]/
				...
				if (isToExport)
				{
					...
					Qlisting = Models.ModelBase.Where<CSGenioApesso>(m_userContext, false, esppe___pessoname____Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_ESPPE___PESSONAME____", true, firstVisibleColumn: firstVisibleColumn);
...
// USE /[MANUAL GQT OVERRQLIST ESPPE_PESSONAME]/
					...
					if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					{
						var m_iCurPag = m_userContext.PersistentSupport.getPagingPos(CSGenioApesso.GetInformation(), QMVC_POS_RECORD, sorts, esppe___pessoname____Conds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
						if (m_iCurPag != -1)
							pageNumber = ((m_iCurPag - 1) / numberListItems) + 1;
					}

					ListingMVC<CSGenioApesso> listing = Models.ModelBase.Where<CSGenioApesso>(m_userContext, false, esppe___pessoname____Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_ESPPE___PESSONAME____", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn, fieldsWithTotalizers, tableConfig.SelectedRows);
		...
		private List<Esppe_PessoValName_RowViewModel> MapEsppe_PessoValName(ListingMVC<CSGenioApesso> Qlisting)
		{
			List<Esppe_PessoValName_RowViewModel> Elements = [];
			int i = 0;
		...
		/// to a Esppe_PessoValName_RowViewModel object.
		/// </summary>
		/// <param name="row">The row.</param>
		private Esppe_PessoValName_RowViewModel MapEsppe_PessoValName(CSGenioApesso row)
		{
			var model = new Esppe_PessoValName_RowViewModel(m_userContext, true, _fieldsToSerialize);
			if (row == null)
				return model;
