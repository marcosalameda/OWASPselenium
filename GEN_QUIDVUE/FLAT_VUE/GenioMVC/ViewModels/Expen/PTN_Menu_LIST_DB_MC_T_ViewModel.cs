...
namespace GenioMVC.ViewModels.Expen
{
	public class PTN_Menu_LIST_DB_MC_T_ViewModel : ListViewModel
	{
		...
		public override int GetCount(User user)
		{
			...
			CriteriaSet ptn_menu_list_db_mc_tConds = CSGenio.business.Listing.CalculateConditionsEphGeneric(areaBase, "MLLIST_DB_MC_T");
			ptn_menu_list_db_mc_tConds.Equal(CSGenioAexpen.FldZzstate, 0); //valid zzstate only
...
// USE /[MANUAL PTN OVERRQ LIST_DB_MC_T]/
			...
			SelectQuery qs = sp.getSelectQueryFromListingMVC(ptn_menu_list_db_mc_tConds, listing);
...
// USE /[MANUAL PTN OVERRQLIST LIST_DB_MC_T]/
					...
					ListingMVC<CSGenioAexpen> listing = Models.ModelBase.Where<CSGenioAexpen>(m_userContext, false, ptn_menu_list_db_mc_tConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "MLLIST_DB_MC_T", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn, fieldsWithTotalizers, tableConfig.SelectedRows);
		...
		{
			var Elements = new List<PTN_Menu_LIST_DB_MC_T_RowViewModel>();
			int i = 0;
		...
		/// </summary>
		public void CalculateButtonPermissions(PTN_Menu_LIST_DB_MC_T_RowViewModel model)
		{
			bool canView = true;
			bool canEdit = true;
			bool canDelete = true;
