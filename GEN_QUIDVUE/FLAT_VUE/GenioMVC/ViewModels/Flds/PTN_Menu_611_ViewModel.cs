...
namespace GenioMVC.ViewModels.Flds
{
	public class PTN_Menu_611_ViewModel : ListViewModel
	{
		...
		public override int GetCount(User user)
		{
			...
			CriteriaSet ptn_menu_611Conds = CSGenio.business.Listing.CalculateConditionsEphGeneric(areaBase, "ML611");
			ptn_menu_611Conds.Equal(CSGenioAflds.FldZzstate, 0); //valid zzstate only
...
// USE /[MANUAL PTN OVERRQ 611]/
			...
			SelectQuery qs = sp.getSelectQueryFromListingMVC(ptn_menu_611Conds, listing);
...
// USE /[MANUAL PTN OVERRQLIST 611]/
					...
					ListingMVC<CSGenioAflds> listing = Models.ModelBase.Where<CSGenioAflds>(m_userContext, false, ptn_menu_611Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "ML611", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn, fieldsWithTotalizers, tableConfig.SelectedRows);
		...
		{
			var Elements = new List<PTN_Menu_611_RowViewModel>();
			int i = 0;
		...
		/// </summary>
		public void CalculateButtonPermissions(PTN_Menu_611_RowViewModel model)
		{
			bool canView = true;
			bool canEdit = true;
			bool canDelete = true;
