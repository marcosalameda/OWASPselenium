...
// USE /[MANUAL STY OVERRQLIST TABS]/
					...
					ListingMVC<CSGenioAflds> listing = Models.ModelBase.Where<CSGenioAflds>(m_userContext, false, sty_menu_tabsConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "MLTABS", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn, fieldsWithTotalizers, tableConfig.SelectedRows);
		...
		{
			var Elements = new List<STY_Menu_TABS_RowViewModel>();
			int i = 0;
		...
		/// </summary>
		public void CalculateButtonPermissions(STY_Menu_TABS_RowViewModel model)
		{
			bool canView = true;
			bool canEdit = true;
			bool canDelete = true;
...
// USE /[MANUAL GQT VIEWMODEL_CUSTOM STY_MENU_TABS]/
		...
		[
			"Flds", "Flds.ValCodflds", "Flds.ValZzstate", "Flds.ValTxtfield", "Flds.ValCodaero", "Flds.ValCodequip", "BtnPermission"
		];
