...
// USE /[MANUAL PTN LIST_LIMITS EDITABLETABLELIST]/
		...
		public PTN_Menu_EDITABLETABLELIST_ViewModel(UserContext userContext) : base(userContext)
		{
		...
		public void Load(CSGenio.core.framework.table.TableConfiguration tableConfig, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAgrpb> Qlisting, ref CriteriaSet conditions)
		{
			...
			conditions.SubSets.Add(ptn_menu_editabletablelistConds);
			ptn_menu_editabletablelistConds = BuildCriteriaSet(tableConfig, requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
...
// USE /[MANUAL PTN OVERRQ EDITABLETABLELIST]/
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.BuildListingForExport<CSGenioAgrpb>(m_userContext, false, ref ptn_menu_editabletablelistConds, exportFieldRefs, (pageNumber - 1) * numberListItems, numberListItems, sorts, "MLEDITABLETABLELIST", true, firstVisibleColumn: firstVisibleColumn);
...
// USE /[MANUAL PTN OVERRQLIST EDITABLETABLELIST]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = m_userContext.PersistentSupport.getPagingPos(CSGenioAgrpb.GetInformation(), QMVC_POS_RECORD, sorts, ptn_menu_editabletablelistConds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
