...
{
	public class Dentr_CmpnyValDesignat_ViewModel : ListViewModel
	{
		/// <summary>
		...
		/// <inheritdoc/>
		public override string Uuid { get => "Dentr_CmpnyValDesignat"; }
		...
		/// <summary>
		/// Initializes a new instance of the <see cref="Dentr_CmpnyValDesignat_ViewModel" /> class.
		/// </summary>
		/// <param name="userContext">The current user request context</param>
		public Dentr_CmpnyValDesignat_ViewModel(UserContext userContext) : base(userContext)
		{
		}
		...
		public CriteriaSet BuildCriteriaSet(NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			LoadUserTableConfig(requestValues, allSortOrders, "Dentr_CmpnyValDesignat", ref numberListItems);
			...
			{
				// EPH
				crs = Models.Cmpny.AddEPH<CSGenioAcmpny>(ref u, crs, "IBL_DENTR___CMPNYDESIGNAT");
			...
			if (tableReload)
			{
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					crs.Equals(Models.Cmpny.AddEPH<CSGenioAcmpny>(ref u, null, "IBL_DENTR___CMPNYDESIGNAT"));
			}
		...
		public void Load(int numberListItems, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAcmpny> Qlisting, ref CriteriaSet conditions)
		{
			...
			if (ajaxRequest)
				this.Navigation.SetValue("requestValues" + "Dentr_CmpnyValDesignat", requestValues);
			else if (!ajaxRequest && this.Navigation.CheckKey("requestValues" + "Dentr_CmpnyValDesignat"))
				requestValues = this.Navigation.GetValue<NameValueCollection>("requestValues" + "Dentr_CmpnyValDesignat");
			...
			var pageNumber = (ajaxRequest && !String.IsNullOrEmpty(requestValues["pDentr_CmpnyValDesignat"])) ? int.Parse(requestValues["pDentr_CmpnyValDesignat"]) : 1;

			// Added to avoid 0 or -1 pages when setting number of records to -1 to disable pagination
			if (pageNumber < 1)
				pageNumber = 1;

			List<ColumnSort> sorts = GetRequestSorts(this.Menu, "sDentr_CmpnyValDesignat", "dDentr_CmpnyValDesignat", requestValues, "cmpny", allSortOrders);
...
FieldRef[] fields = new FieldRef[] { CSGenioAcmpny.FldCodempre, CSGenioAcmpny.FldZzstate, CSGenioAcmpny.FldDesignat };
			...
			//Tooltip for EPHs affecting this viewmodel list
			{
				...
				List<Limit> area_EPH_limits = EPH_Limit_Filler(ref limit, model_limit_area, "IBL_DENTR___CMPNYDESIGNAT");
			...
			//Limit type: "A"			//Current Area = "CMPNY"			//1st Area Limit: "CNTRY"			//1st Area Field: "CODCNTRY"			//1st Area Value: ""
			{
				Limit limit = new Limit();
			...
			conditions.SubSets.Add(dentr___cmpnydesignatConds);
			dentr___cmpnydesignatConds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
...
// USE /[MANUAL GQT OVERRQ DENTR_CMPNYDESIGNAT]/
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAcmpny>(m_userContext, false, dentr___cmpnydesignatConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_DENTR___CMPNYDESIGNAT", true, firstVisibleColumn: firstVisibleColumn);
...
// USE /[MANUAL GQT OVERRQLSTEXP DENTR_CMPNYDESIGNAT]/
			...
			if (tableReload)
			{
// USE /[MANUAL GQT OVERRQLIST DENTR_CMPNYDESIGNAT]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = m_userContext.PersistentSupport.getPagingPos(CSGenioAcmpny.GetInformation(), QMVC_POS_RECORD, sorts, dentr___cmpnydesignatConds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioAcmpny> listing = Models.ModelBase.Where<CSGenioAcmpny>(m_userContext, false, dentr___cmpnydesignatConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_DENTR___CMPNYDESIGNAT", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
				...
				Menu.Elements = MapDentr_CmpnyValDesignat(listing);

				Menu.Identifier = "IBL_DENTR___CMPNYDESIGNAT";

				// Last updated by [CJP] at [2015.02.03]
				// Adds the identifier to each element
				foreach (var element in Menu.Elements)
					element.Identifier = "IBL_DENTR___CMPNYDESIGNAT";
			...
			if (Qlisting.Rows != null)
			{
				foreach (var row in Qlisting.Rows)
				{
					if (Qlisting.NumRegs > 0 && i >= Qlisting.NumRegs) // Copiado da versão antiga do RowsToViewModels
						break;
					Elements.Add(MapDentr_CmpnyValDesignat(row));
					i++;
				}
			}
		...
		private Models.Cmpny MapDentr_CmpnyValDesignat(CSGenioAcmpny row)
		{
		...
		#region Custom code
// USE /[MANUAL GQT VIEWMODEL_CUSTOM DENTR_CMPNYVALDESIGNAT]/
		#endregion
