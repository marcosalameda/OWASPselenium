...
{
	public class Dentr_CntryValCountry_ViewModel : MenuListViewModel<Models.Cntry>
	{
		/// <summary>
		/// Gets or sets the object that represents the table and its elements.
		/// </summary>
		[JsonPropertyName("Table")]
		public TablePartial<Dentr_CntryValCountry_RowViewModel> Menu { get; set; }
		...
		/// <inheritdoc/>
		public override string Uuid => "Dentr_CntryValCountry";
		...
		{
// USE /[MANUAL GQT LIST_LIMITS DENTR_CNTRYCOUNTRY]/
		...
		public Dentr_CntryValCountry_ViewModel() : base(null!) { }

		/// <summary>
		/// Initializes a new instance of the <see cref="Dentr_CntryValCountry_ViewModel" /> class.
		/// </summary>
		/// <param name="userContext">The current user request context</param>
		public Dentr_CntryValCountry_ViewModel(UserContext userContext) : base(userContext)
		{
		...
		public Dentr_CntryValCountry_ViewModel(UserContext userContext, Models.ModelBase parentCtx) : this(userContext)
		{
			ParentCtx = parentCtx;
		}
		...
		public override CriteriaSet BuildCriteriaSet(CSGenio.framework.TableConfiguration.TableConfiguration tableConfig, NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			if (Menu == null)
				Menu = new TablePartial<Dentr_CntryValCountry_RowViewModel>();
		...
		public void Load(CSGenio.framework.TableConfiguration.TableConfiguration tableConfig, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAcntry> Qlisting, ref CriteriaSet conditions)
		{
			...
			}, "ms", "Time to load the form."))
			{
				...
				conditions.SubSets.Add(dentr___cntrycountry_Conds);
				dentr___cntrycountry_Conds = BuildCriteriaSet(tableConfig, requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
...
// USE /[MANUAL GQT OVERRQ DENTR_CNTRYCOUNTRY]/
				...
				if (isToExport)
				{
					...
					Qlisting = Models.ModelBase.Where<CSGenioAcntry>(m_userContext, false, dentr___cntrycountry_Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_DENTR___CNTRYCOUNTRY_", true, firstVisibleColumn: firstVisibleColumn);
...
// USE /[MANUAL GQT OVERRQLIST DENTR_CNTRYCOUNTRY]/
					...
					if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					{
						var m_iCurPag = m_userContext.PersistentSupport.getPagingPos(CSGenioAcntry.GetInformation(), QMVC_POS_RECORD, sorts, dentr___cntrycountry_Conds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
						if (m_iCurPag != -1)
							pageNumber = ((m_iCurPag - 1) / numberListItems) + 1;
					}

					ListingMVC<CSGenioAcntry> listing = Models.ModelBase.Where<CSGenioAcntry>(m_userContext, false, dentr___cntrycountry_Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_DENTR___CNTRYCOUNTRY_", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn, fieldsWithTotalizers, tableConfig.SelectedRows);
		...
		private List<Dentr_CntryValCountry_RowViewModel> MapDentr_CntryValCountry(ListingMVC<CSGenioAcntry> Qlisting)
		{
			List<Dentr_CntryValCountry_RowViewModel> Elements = [];
			int i = 0;
		...
		/// to a Dentr_CntryValCountry_RowViewModel object.
		/// </summary>
		/// <param name="row">The row.</param>
		private Dentr_CntryValCountry_RowViewModel MapDentr_CntryValCountry(CSGenioAcntry row)
		{
			var model = new Dentr_CntryValCountry_RowViewModel(m_userContext, true, _fieldsToSerialize);
			if (row == null)
				return model;
