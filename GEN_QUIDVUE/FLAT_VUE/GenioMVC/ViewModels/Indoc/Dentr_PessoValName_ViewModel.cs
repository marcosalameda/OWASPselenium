...
{
	public class Dentr_PessoValName_ViewModel : ListViewModel
	{
		/// <summary>
		...
		/// <inheritdoc/>
		public override string Uuid { get => "Dentr_PessoValName"; }
		...
		/// <summary>
		/// Initializes a new instance of the <see cref="Dentr_PessoValName_ViewModel" /> class.
		/// </summary>
		/// <param name="userContext">The current user request context</param>
		public Dentr_PessoValName_ViewModel(UserContext userContext) : base(userContext)
		{
		}
		...
		public CriteriaSet BuildCriteriaSet(NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			// Area limit
			tableReload &= AddCriteriaAreaLimit(crs, CSGenio.business.CSGenioAcntry.FldCodcntry, "cntry", this.ValCodpaise, true);
			...
			LoadUserTableConfig(requestValues, allSortOrders, "Dentr_PessoValName", ref numberListItems);
			...
			{
				// EPH
				crs = Models.Pesso.AddEPH<CSGenioApesso>(ref u, crs, "IBL_DENTR___PESSONAME____");
			...
			if (tableReload)
			{
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					crs.Equals(Models.Pesso.AddEPH<CSGenioApesso>(ref u, null, "IBL_DENTR___PESSONAME____"));
			}
		...
		public void Load(int numberListItems, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioApesso> Qlisting, ref CriteriaSet conditions)
		{
			...
			if (ajaxRequest)
				this.Navigation.SetValue("requestValues" + "Dentr_PessoValName", requestValues);
			else if (!ajaxRequest && this.Navigation.CheckKey("requestValues" + "Dentr_PessoValName"))
				requestValues = this.Navigation.GetValue<NameValueCollection>("requestValues" + "Dentr_PessoValName");
			...
			var pageNumber = (ajaxRequest && !String.IsNullOrEmpty(requestValues["pDentr_PessoValName"])) ? int.Parse(requestValues["pDentr_PessoValName"]) : 1;

			// Added to avoid 0 or -1 pages when setting number of records to -1 to disable pagination
			if (pageNumber < 1)
				pageNumber = 1;

			List<ColumnSort> sorts = GetRequestSorts(this.Menu, "sDentr_PessoValName", "dDentr_PessoValName", requestValues, "pesso", allSortOrders);
...
FieldRef[] fields = new FieldRef[] { CSGenioApesso.FldCodpesso, CSGenioApesso.FldZzstate, CSGenioApesso.FldName };
			...
			//Tooltip for EPHs affecting this viewmodel list
			{
				...
				List<Limit> area_EPH_limits = EPH_Limit_Filler(ref limit, model_limit_area, "IBL_DENTR___PESSONAME____");
			...
			//Limit type: "A"			//Current Area = "PESSO"			//1st Area Limit: "CNTRY"			//1st Area Field: "CODCNTRY"			//1st Area Value: ""
			{
				Limit limit = new Limit();
				...
				object this_limit_field = Navigation.GetValue("cntry") == null ? this.ValCodpaise : Navigation.GetValue("cntry");
			...
			//Limit type: "A"			//Current Area = "PESSO"			//1st Area Limit: "CMPNY"			//1st Area Field: "CODEMPRE"			//1st Area Value: ""
			{
				Limit limit = new Limit();
				limit.TipoLimite = LimitType.A;
				limit.NaoAplicaSeNulo = false;
			...
			conditions.SubSets.Add(dentr___pessoname____Conds);
			dentr___pessoname____Conds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
...
// USE /[MANUAL GQT OVERRQ DENTR_PESSONAME]/
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioApesso>(m_userContext, false, dentr___pessoname____Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_DENTR___PESSONAME____", true, firstVisibleColumn: firstVisibleColumn);
...
// USE /[MANUAL GQT OVERRQLSTEXP DENTR_PESSONAME]/
			...
			if (tableReload)
			{
// USE /[MANUAL GQT OVERRQLIST DENTR_PESSONAME]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = m_userContext.PersistentSupport.getPagingPos(CSGenioApesso.GetInformation(), QMVC_POS_RECORD, sorts, dentr___pessoname____Conds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioApesso> listing = Models.ModelBase.Where<CSGenioApesso>(m_userContext, false, dentr___pessoname____Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_DENTR___PESSONAME____", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
				...
				Menu.Elements = MapDentr_PessoValName(listing);

				Menu.Identifier = "IBL_DENTR___PESSONAME____";

				// Last updated by [CJP] at [2015.02.03]
				// Adds the identifier to each element
				foreach (var element in Menu.Elements)
					element.Identifier = "IBL_DENTR___PESSONAME____";
		...
		private List<Models.Pesso> MapDentr_PessoValName(ListingMVC<CSGenioApesso> Qlisting)
		{
			...
			if (Qlisting.Rows != null)
			{
				foreach (var row in Qlisting.Rows)
				{
					if (Qlisting.NumRegs > 0 && i >= Qlisting.NumRegs) // Copiado da versão antiga do RowsToViewModels
						break;
					Elements.Add(MapDentr_PessoValName(row));
					i++;
				}
			}
		...
		private Models.Pesso MapDentr_PessoValName(CSGenioApesso row)
		{
		...
		#region Custom code
// USE /[MANUAL GQT VIEWMODEL_CUSTOM DENTR_PESSOVALNAME]/
		#endregion
