...
{
	public class Dentr_ViewModel : FormViewModel<Models.Indoc>, IPreparableForSerialization
	{
		[JsonIgnore]
		...
		public override void MapToModel(Models.Indoc m)
		{
			...
			{
				CSGenio.framework.Log.Error($"Map ViewModel (Dentr) to Model (Indoc) - Error during mapping. All user values: {HasDisabledUserValuesSecurity}");
				throw;
			}
		}
			...
			try
			{
				...
				switch (fullFieldName)
				{
					...
					case "indoc.coddentr":
						...
						break;
					default:
						Log.Error($"SetViewModelValue (Dentr) - Unexpected field identifier {fullFieldName}");
						break;
				}
			}
			catch (Exception ex)
			{
				throw new FrameworkException(Resources.Resources.PEDIMOS_DESCULPA__OC63848, "SetViewModelValue (Dentr)", "Unexpected error", ex);
			}
		}
		...
		{
			try { Model = Models.Indoc.Find(id ?? Navigation.GetStrValue("indoc"), m_userContext, "FDENTR"); }
			finally { Model ??= new Models.Indoc(m_userContext) { Identifier = "FDENTR" }; }
...
// USE /[MANUAL GQT VIEWMODEL_DESTROY DENTR]/
		...
		public void Load_Dentr___cntrycountry_(NameValueCollection qs, bool lazyLoad = false)
		{
			...
			CriteriaSet dentr___cntrycountry_Conds = CriteriaSet.And();
			{
				...
				{
					dentr___cntrycountry_Conds.Equal(CSGenioAcntry.FldCodcntry, hValue);
...
// USE /[MANUAL GQT OVERRQ DENTR_CNTRYCOUNTRY]/
				...
				*/
				...
				TableCntryCountry.Elements = listing.RowsForViewModel((r) => new GenioMVC.Models.Cntry(m_userContext, r, true, _fieldsToSerialize_DENTR___CNTRYCOUNTRY_));
		...
		public void Load_Dentr___cmpnydesignat(NameValueCollection qs, bool lazyLoad = false)
		{
			...
			CriteriaSet dentr___cmpnydesignatConds = CriteriaSet.And();
			{
				...
				if (hValue != null && !(hValue is Array) && !string.IsNullOrEmpty(Convert.ToString(hValue)))
				{
					dentr___cmpnydesignatConds.Equal(CSGenioAcmpny.FldCodempre, hValue);
...
// USE /[MANUAL GQT OVERRQ DENTR_CMPNYDESIGNAT]/
				...
				*/
				...
				TableCmpnyDesignat.Elements = listing.RowsForViewModel((r) => new GenioMVC.Models.Cmpny(m_userContext, r, true, _fieldsToSerialize_DENTR___CMPNYDESIGNAT));
		...
		public void Load_Dentr___pessoname____(NameValueCollection qs, bool lazyLoad = false)
		{
			...
			CriteriaSet dentr___pessoname____Conds = CriteriaSet.And();
			{
				...
				if (hValue != null && !(hValue is Array) && !string.IsNullOrEmpty(Convert.ToString(hValue)))
				{
					dentr___pessoname____Conds.Equal(CSGenioApesso.FldCodpesso, hValue);
...
// USE /[MANUAL GQT OVERRQ DENTR_PESSONAME]/
				...
				*/
				...
				TablePessoName.Elements = listing.RowsForViewModel((r) => new GenioMVC.Models.Pesso(m_userContext, r, true, _fieldsToSerialize_DENTR___PESSONAME____));
		...
		public void Load_Dentr___ware1warehdes(NameValueCollection qs, bool lazyLoad = false)
		{
			...
			CriteriaSet dentr___ware1warehdesConds = CriteriaSet.And();
			{
				...
				if (hValue != null && !(hValue is Array) && !string.IsNullOrEmpty(Convert.ToString(hValue)))
				{
					dentr___ware1warehdesConds.Equal(CSGenioAware1.FldCodwareh, hValue);
			...
			if (dentr___ware1warehdesDoLoad)
			{
				...
				FieldRef[] fields = [CSGenioAware1.FldCodwareh, CSGenioAware1.FldWarehdes, CSGenioAware1.FldZzstate];
...
// USE /[MANUAL GQT OVERRQ DENTR_WARE1WAREHDES]/
				...
				*/
				...
				TableWare1Warehdes.Elements = listing.RowsForViewModel((r) => new GenioMVC.Models.Ware1(m_userContext, r, true, _fieldsToSerialize_DENTR___WARE1WAREHDES));
