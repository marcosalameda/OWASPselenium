...
namespace GenioMVC.ViewModels.Insta
{
	public class Insta_EquipValRegistnr_ViewModel : ListViewModel
	{
		...
		public CriteriaSet BuildCriteriaSet(NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			Menu.SetFilters(bool.Parse(requestValues["Insta_EquipValRegistnr_tableFilters"] ?? "false"), false);
			...
			LoadUserTableConfig(requestValues, allSortOrders, "Insta_EquipValRegistnr", ref numberListItems);
			...
			{
				// EPH
				crs = Models.Equip.AddEPH<CSGenioAequip>(ref u, crs, "IBL_INSTA___EQUIPREGISTNR");
			...
			if (tableReload)
			{
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					crs.Equals(Models.Equip.AddEPH<CSGenioAequip>(ref u, null, "IBL_INSTA___EQUIPREGISTNR"));
			}
		...
		public void Load(int numberListItems, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAequip> Qlisting, ref CriteriaSet conditions)
		{
			...
			if (ajaxRequest)
				this.Navigation.SetValue("requestValues" + "Insta_EquipValRegistnr", requestValues);
			else if (!ajaxRequest && this.Navigation.CheckKey("requestValues" + "Insta_EquipValRegistnr"))
				requestValues = this.Navigation.GetValue<NameValueCollection>("requestValues" + "Insta_EquipValRegistnr");
			...
			var pageNumber = (ajaxRequest && !String.IsNullOrEmpty(requestValues["pInsta_EquipValRegistnr"])) ? int.Parse(requestValues["pInsta_EquipValRegistnr"]) : 1;

			// Added to avoid 0 or -1 pages when setting number of records to -1 to disable pagination
			if (pageNumber < 1)
				pageNumber = 1;

			List<ColumnSort> sorts = GetRequestSorts(this.Menu, "sInsta_EquipValRegistnr", "dInsta_EquipValRegistnr", requestValues, "equip", allSortOrders);
...
FieldRef[] fields = new FieldRef[] { CSGenioAequip.FldCodequip, CSGenioAequip.FldZzstate, CSGenioAequip.FldDesignat, CSGenioAequip.FldRegistnr, CSGenioAequip.FldSequennr, CSGenioAequip.FldDtaquisi, CSGenioAequip.FldDtdeco, CSGenioAequip.FldPhotogra, CSGenioAequip.FldValortot };
			...
			//Limit type: "A"			//Current Area = "EQUIP"			//1st Area Limit: "TPEQU"			//1st Area Field: "CODTPEQU"			//1st Area Value: ""
			{
				Limit limit = new Limit();
			...
			conditions.SubSets.Add(insta___equipregistnrConds);
			insta___equipregistnrConds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
...
// USE /[MANUAL GQT OVERRQ INSTA_EQUIPREGISTNR]/
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAequip>(m_userContext, false, insta___equipregistnrConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_INSTA___EQUIPREGISTNR", true, firstVisibleColumn: firstVisibleColumn);
...
// USE /[MANUAL GQT OVERRQLSTEXP INSTA_EQUIPREGISTNR]/
			...
			if (tableReload)
			{
// USE /[MANUAL GQT OVERRQLIST INSTA_EQUIPREGISTNR]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = m_userContext.PersistentSupport.getPagingPos(CSGenioAequip.GetInformation(), QMVC_POS_RECORD, sorts, insta___equipregistnrConds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioAequip> listing = Models.ModelBase.Where<CSGenioAequip>(m_userContext, false, insta___equipregistnrConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_INSTA___EQUIPREGISTNR", true, true, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
				...
				Menu.Elements = MapInsta_EquipValRegistnr(listing);

				Menu.Identifier = "IBL_INSTA___EQUIPREGISTNR";

				// Last updated by [CJP] at [2015.02.03]
				// Adds the identifier to each element
				foreach (var element in Menu.Elements)
					element.Identifier = "IBL_INSTA___EQUIPREGISTNR";
		...
		private List<Models.Equip> MapInsta_EquipValRegistnr(ListingMVC<CSGenioAequip> Qlisting)
		{
			...
			if (Qlisting.Rows != null)
			{
				foreach (var row in Qlisting.Rows)
				{
					if (Qlisting.NumRegs > 0 && i >= Qlisting.NumRegs) // Copiado da versão antiga do RowsToViewModels
						break;
					Elements.Add(MapInsta_EquipValRegistnr(row));
					i++;
				}
			}
		...
		private Models.Equip MapInsta_EquipValRegistnr(CSGenioAequip row)
		{
...
// USE /[MANUAL GQT VIEWMODEL_CUSTOM INSTA_EQUIPVALREGISTNR]/
		...
		{
			"Equip", "Equip.ValCodequip", "Equip.ValZzstate", "Equip.ValDesignat", "Equip.ValRegistnr", "Equip.ValSequennr", "Equip.ValDtaquisi", "Equip.ValDtdeco", "Equip.ValPhotogra", "Equip.ValValortot", "Equip.ValCodempre", "Equip.ValCoddeco", "Equip.ValCoditem", "Equip.ValCodpess1", "Equip.ValCodtpequ", "Equip.ValCodwareh", "Equip.ValCodrooms"
		};
