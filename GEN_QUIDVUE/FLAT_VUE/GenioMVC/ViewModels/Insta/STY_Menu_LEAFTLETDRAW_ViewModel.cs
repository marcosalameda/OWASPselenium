...
{
	public class STY_Menu_LEAFTLETDRAW_ViewModel : ListViewModel
	{
		/// <summary>
		...
		/// <inheritdoc/>
		public override string Uuid { get => "5b5648c2-338b-4f0a-8b4e-33bb84218448"; }
		...
		{
			dbeditTitle = Resources.Resources.LEAFLETDRAW29465;
		}
...
FieldRef[] fields = new FieldRef[] { CSGenioAinsta.FldCodinsta, CSGenioAinsta.FldZzstate, CSGenioAinsta.FldCodtpequ, CSGenioAtpequ.FldCodtpequ, CSGenioAtpequ.FldTipoequi, CSGenioAinsta.FldCodequip, CSGenioAequip.FldCodequip, CSGenioAequip.FldRegistnr, CSGenioAinsta.FldDesignat, CSGenioAinsta.FldDtiniage, CSGenioAinsta.FldDtfimage, CSGenioAinsta.FldDescript, CSGenioAinsta.FldAllday, CSGenioAinsta.FldSince, CSGenioAinsta.FldUntil, CSGenioAinsta.FldHours, CSGenioAinsta.FldPrecohor, CSGenioAinsta.FldValue, CSGenioAinsta.FldCoordgeo };
		...
		/// <summary>
		/// Initializes a new instance of the <see cref="STY_Menu_LEAFTLETDRAW_ViewModel" /> class.
		/// </summary>
		/// <param name="userContext">The current user request context</param>
		public STY_Menu_LEAFTLETDRAW_ViewModel(UserContext userContext) : base(userContext)
		{
		...
		public CriteriaSet BuildCriteriaSet(NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			LoadUserTableConfig(requestValues, allSortOrders, "STY_Menu_LEAFTLETDRAW", ref numberListItems);

			crs.SubSets.Add(ProcessSearchFilters(Menu, GetSearchColumns(true), requestValues, "STY_Menu_LEAFTLETDRAW_"));
			...
			{
				// EPH
				crs = Models.Insta.AddEPH<CSGenioAinsta>(ref u, crs, "MLLEAFTLETDRAW");
			...
			if (tableReload)
			{
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					crs.Equals(Models.Insta.AddEPH<CSGenioAinsta>(ref u, null, "MLLEAFTLETDRAW"));
			}
		...
		public void Load(int numberListItems, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAinsta> Qlisting, ref CriteriaSet conditions)
		{
			...
			if (ajaxRequest)
				this.Navigation.SetValue("requestValues" + "STY_Menu_LEAFTLETDRAW", requestValues);
			else if (!ajaxRequest && this.Navigation.CheckKey("requestValues" + "STY_Menu_LEAFTLETDRAW"))
				requestValues = this.Navigation.GetValue<NameValueCollection>("requestValues" + "STY_Menu_LEAFTLETDRAW");

			User u = m_userContext.User;
			Menu = new TablePartial<GenioMVC.Models.Insta>();

			CriteriaSet sty_menu_leaftletdrawConds = CriteriaSet.And();
			...
			var pageNumber = (ajaxRequest && !String.IsNullOrEmpty(requestValues["pSTY_Menu_LEAFTLETDRAW"])) ? int.Parse(requestValues["pSTY_Menu_LEAFTLETDRAW"]) : 1;

			// Added to avoid 0 or -1 pages when setting number of records to -1 to disable pagination
			if (pageNumber < 1)
				pageNumber = 1;

			List<ColumnSort> sorts = GetRequestSorts(this.Menu, "sSTY_Menu_LEAFTLETDRAW", "dSTY_Menu_LEAFTLETDRAW", requestValues, "insta", allSortOrders);
...
FieldRef[] fields = new FieldRef[] { CSGenioAinsta.FldCodinsta, CSGenioAinsta.FldZzstate, CSGenioAinsta.FldCodtpequ, CSGenioAtpequ.FldCodtpequ, CSGenioAtpequ.FldTipoequi, CSGenioAinsta.FldCodequip, CSGenioAequip.FldCodequip, CSGenioAequip.FldRegistnr, CSGenioAinsta.FldDesignat, CSGenioAinsta.FldDtiniage, CSGenioAinsta.FldDtfimage, CSGenioAinsta.FldDescript, CSGenioAinsta.FldAllday, CSGenioAinsta.FldSince, CSGenioAinsta.FldUntil, CSGenioAinsta.FldHours, CSGenioAinsta.FldPrecohor, CSGenioAinsta.FldValue, CSGenioAinsta.FldCoordgeo };
			...
			//Tooltip for EPHs affecting this viewmodel list
			{
				...
				List<Limit> area_EPH_limits = EPH_Limit_Filler(ref limit, model_limit_area, "MLLEAFTLETDRAW");
			...
			conditions.SubSets.Add(sty_menu_leaftletdrawConds);
			sty_menu_leaftletdrawConds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
...
// USE /[MANUAL STY OVERRQ LEAFTLETDRAW]/
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAinsta>(m_userContext, false, sty_menu_leaftletdrawConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "MLLEAFTLETDRAW", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL STY OVERRQLSTEXP LEAFTLETDRAW]/
			...
			if (tableReload)
			{
// USE /[MANUAL STY OVERRQLIST LEAFTLETDRAW]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = m_userContext.PersistentSupport.getPagingPos(CSGenioAinsta.GetInformation(), QMVC_POS_RECORD, sorts, sty_menu_leaftletdrawConds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioAinsta> listing = Models.ModelBase.Where<CSGenioAinsta>(m_userContext, false, sty_menu_leaftletdrawConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "MLLEAFTLETDRAW", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
				...
				Menu.Elements = MapSTY_Menu_LEAFTLETDRAW(listing);

				Menu.Identifier = "MLLEAFTLETDRAW";
				Menu.Slots = new Dictionary<string, List<object>>();

				// Last updated by [CJP] at [2015.02.03]
				// Adds the identifier to each element
				foreach (var element in Menu.Elements)
					element.Identifier = "MLLEAFTLETDRAW";
		...
		private List<Models.Insta> MapSTY_Menu_LEAFTLETDRAW(ListingMVC<CSGenioAinsta> Qlisting)
		{
			...
			if (Qlisting.Rows != null)
			{
				foreach (var row in Qlisting.Rows)
				{
					if (Qlisting.NumRegs > 0 && i >= Qlisting.NumRegs) // Copiado da versão antiga do RowsToViewModels
						break;
					Elements.Add(MapSTY_Menu_LEAFTLETDRAW(row));
					i++;
				}
			}
		...
		private Models.Insta MapSTY_Menu_LEAFTLETDRAW(CSGenioAinsta row)
		{
		...
		#region Custom code
// USE /[MANUAL GQT VIEWMODEL_CUSTOM STY_MENU_LEAFTLETDRAW]/
		#endregion
