...
{
	public class Artigval_ViewModel : FormViewModel<Models.Item>, IPreparableForSerialization
	{
		[JsonIgnore]
		...
		public override void MapToModel(Models.Item m)
		{
			...
			{
				CSGenio.framework.Log.Error($"Map ViewModel (Artigval) to Model (Item) - Error during mapping. All user values: {HasDisabledUserValuesSecurity}");
				throw;
			}
		}
			...
			try
			{
				...
				switch (fullFieldName)
				{
					...
					case "item.coditem":
						...
						break;
					default:
						Log.Error($"SetViewModelValue (Artigval) - Unexpected field identifier {fullFieldName}");
						break;
				}
			}
			catch (Exception ex)
			{
				throw new FrameworkException(Resources.Resources.PEDIMOS_DESCULPA__OC63848, "SetViewModelValue (Artigval)", "Unexpected error", ex);
			}
		}
		...
		{
			try { Model = Models.Item.Find(id ?? Navigation.GetStrValue("item"), m_userContext, "FARTIGVAL"); }
			finally { Model ??= new Models.Item(m_userContext) { Identifier = "FARTIGVAL" }; }
...
// USE /[MANUAL GQT VIEWMODEL_DESTROY ARTIGVAL]/
		...
		public void Load_Artigvalgitemitemdes_(NameValueCollection qs, bool lazyLoad = false)
		{
			...
			CriteriaSet artigvalgitemitemdes_Conds = CriteriaSet.And();
			{
				...
				{
					artigvalgitemitemdes_Conds.Equal(CSGenioAgitem.FldCodgitem, hValue);
...
// USE /[MANUAL GQT OVERRQ ARTIGVAL_GITEMITEMDES]/
				...
				*/
				...
				TableGitemItemdes.Elements = listing.RowsForViewModel((r) => new GenioMVC.Models.Gitem(m_userContext, r, true, _fieldsToSerialize_ARTIGVALGITEMITEMDES_));
		...
		public void Load_Artigvalwarehwarehdes(NameValueCollection qs, bool lazyLoad = false)
		{
			...
			CriteriaSet artigvalwarehwarehdesConds = CriteriaSet.And();
			{
				...
				if (hValue != null && !(hValue is Array) && !string.IsNullOrEmpty(Convert.ToString(hValue)))
				{
					artigvalwarehwarehdesConds.Equal(CSGenioAwareh.FldCodwareh, hValue);
...
// USE /[MANUAL GQT OVERRQ ARTIGVAL_WAREHWAREHDES]/
				...
				*/
				...
				TableWarehWarehdes.Elements = listing.RowsForViewModel((r) => new GenioMVC.Models.Wareh(m_userContext, r, true, _fieldsToSerialize_ARTIGVALWAREHWAREHDES));
