...
{
	public class Ldent_ItemValItemdes_ViewModel : ListViewModel
	{
		/// <summary>
		...
		/// <inheritdoc/>
		public override string Uuid { get => "Ldent_ItemValItemdes"; }
		...
		/// <summary>
		/// Initializes a new instance of the <see cref="Ldent_ItemValItemdes_ViewModel" /> class.
		/// </summary>
		/// <param name="userContext">The current user request context</param>
		public Ldent_ItemValItemdes_ViewModel(UserContext userContext) : base(userContext)
		{
		}
		...
		public CriteriaSet BuildCriteriaSet(NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			Menu.SetFilters(bool.Parse(requestValues["Ldent_ItemValItemdes_tableFilters"] ?? "false"), false);
			...
			LoadUserTableConfig(requestValues, allSortOrders, "Ldent_ItemValItemdes", ref numberListItems);

			crs.SubSets.Add(ProcessSearchFilters(Menu, GetSearchColumns(true), requestValues, "Ldent_ItemValItemdes_"));
			...
			{
				// EPH
				crs = Models.Item.AddEPH<CSGenioAitem>(ref u, crs, "IBL_LDENT___ITEM_ITEMDES_");
			...
			if (tableReload)
			{
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					crs.Equals(Models.Item.AddEPH<CSGenioAitem>(ref u, null, "IBL_LDENT___ITEM_ITEMDES_"));
			}
		...
		public void Load(int numberListItems, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAitem> Qlisting, ref CriteriaSet conditions)
		{
			...
			if (ajaxRequest)
				this.Navigation.SetValue("requestValues" + "Ldent_ItemValItemdes", requestValues);
			else if (!ajaxRequest && this.Navigation.CheckKey("requestValues" + "Ldent_ItemValItemdes"))
				requestValues = this.Navigation.GetValue<NameValueCollection>("requestValues" + "Ldent_ItemValItemdes");
			...
			var pageNumber = (ajaxRequest && !String.IsNullOrEmpty(requestValues["pLdent_ItemValItemdes"])) ? int.Parse(requestValues["pLdent_ItemValItemdes"]) : 1;

			// Added to avoid 0 or -1 pages when setting number of records to -1 to disable pagination
			if (pageNumber < 1)
				pageNumber = 1;

			List<ColumnSort> sorts = GetRequestSorts(this.Menu, "sLdent_ItemValItemdes", "dLdent_ItemValItemdes", requestValues, "item", allSortOrders);
...
FieldRef[] fields = new FieldRef[] { CSGenioAitem.FldCoditem, CSGenioAitem.FldZzstate, CSGenioAitem.FldItemdes, CSGenioAitem.FldItemcod, CSGenioAitem.FldValid, CSGenioAitem.FldItemtype };
			...
			//Tooltip for EPHs affecting this viewmodel list
			{
				...
				List<Limit> area_EPH_limits = EPH_Limit_Filler(ref limit, model_limit_area, "IBL_LDENT___ITEM_ITEMDES_");
			...
			conditions.SubSets.Add(ldent___item_itemdes_Conds);
			ldent___item_itemdes_Conds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
...
// USE /[MANUAL GQT OVERRQ LDENT_ITEMITEMDES]/
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAitem>(m_userContext, false, ldent___item_itemdes_Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_LDENT___ITEM_ITEMDES_", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL GQT OVERRQLSTEXP LDENT_ITEMITEMDES]/
			...
			if (tableReload)
			{
// USE /[MANUAL GQT OVERRQLIST LDENT_ITEMITEMDES]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = m_userContext.PersistentSupport.getPagingPos(CSGenioAitem.GetInformation(), QMVC_POS_RECORD, sorts, ldent___item_itemdes_Conds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioAitem> listing = Models.ModelBase.Where<CSGenioAitem>(m_userContext, false, ldent___item_itemdes_Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_LDENT___ITEM_ITEMDES_", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
				...
				Menu.Elements = MapLdent_ItemValItemdes(listing);

				Menu.Identifier = "IBL_LDENT___ITEM_ITEMDES_";

				// Last updated by [CJP] at [2015.02.03]
				// Adds the identifier to each element
				foreach (var element in Menu.Elements)
					element.Identifier = "IBL_LDENT___ITEM_ITEMDES_";
		...
		private List<Models.Item> MapLdent_ItemValItemdes(ListingMVC<CSGenioAitem> Qlisting)
		{
			...
			if (Qlisting.Rows != null)
			{
				foreach (var row in Qlisting.Rows)
				{
					if (Qlisting.NumRegs > 0 && i >= Qlisting.NumRegs) // Copiado da versão antiga do RowsToViewModels
						break;
					Elements.Add(MapLdent_ItemValItemdes(row));
					i++;
				}
			}
		...
		private Models.Item MapLdent_ItemValItemdes(CSGenioAitem row)
		{
		...
		#region Custom code
// USE /[MANUAL GQT VIEWMODEL_CUSTOM LDENT_ITEMVALITEMDES]/
		#endregion
