...
namespace GenioMVC.ViewModels.Lendi
{
	public class GQT_Menu_DEVOL_ViewModel : ListViewModel
	{
		...
		{
			dbeditTitle = Resources.Resources.OUT_OF_DATE_LENDINGS05506;
		}
...
FieldRef[] fields = new FieldRef[] { CSGenioAlendi.FldCodlendi, CSGenioAlendi.FldZzstate, CSGenioAlendi.FldCodpess1, CSGenioApess1.FldCodpesso, CSGenioApess1.FldName, CSGenioAlendi.FldCodequip, CSGenioAequip.FldCodequip, CSGenioAequip.FldDesignat, CSGenioAequip.FldRegistnr, CSGenioAequip.FldFrequenc, CSGenioAlendi.FldCodpess2, CSGenioApess2.FldCodpesso, CSGenioApess2.FldName, CSGenioAlendi.FldLendinnr, CSGenioAlendi.FldStart, CSGenioAlendi.FldWarndt, CSGenioAlendi.FldEnd, CSGenioAlendi.FldObservat, CSGenioAlendi.FldReturndt, CSGenioAlendi.FldDayslimi, CSGenioAlendi.FldReturned, CSGenioAlendi.FldIfoutdt };
		...
		public GQT_Menu_DEVOL_ViewModel(UserContext userContext) : base(userContext)
		{
		...
		public CriteriaSet BuildCriteriaSet(NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			Menu.SetFilters(bool.Parse(requestValues["GQT_Menu_DEVOL_tableFilters"] ?? "false"), false);
			...
			LoadUserTableConfig(requestValues, allSortOrders, "GQT_Menu_DEVOL", ref numberListItems);

			crs.SubSets.Add(ProcessSearchFilters(Menu, GetSearchColumns(true), requestValues, "GQT_Menu_DEVOL_"));
			...
			{
				// EPH
				crs = Models.Lendi.AddEPH<CSGenioAlendi>(ref u, crs, "MLDEVOL");
			...
			if (tableReload)
			{
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					crs.Equals(Models.Lendi.AddEPH<CSGenioAlendi>(ref u, null, "MLDEVOL"));
			}
		...
		public void Load(int numberListItems, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAlendi> Qlisting, ref CriteriaSet conditions)
		{
			...
			if (ajaxRequest)
				this.Navigation.SetValue("requestValues" + "GQT_Menu_DEVOL", requestValues);
			else if (!ajaxRequest && this.Navigation.CheckKey("requestValues" + "GQT_Menu_DEVOL"))
				requestValues = this.Navigation.GetValue<NameValueCollection>("requestValues" + "GQT_Menu_DEVOL");
			...
			var pageNumber = (ajaxRequest && !String.IsNullOrEmpty(requestValues["pGQT_Menu_DEVOL"])) ? int.Parse(requestValues["pGQT_Menu_DEVOL"]) : 1;

			// Added to avoid 0 or -1 pages when setting number of records to -1 to disable pagination
			if (pageNumber < 1)
				pageNumber = 1;

			List<ColumnSort> sorts = GetRequestSorts(this.Menu, "sGQT_Menu_DEVOL", "dGQT_Menu_DEVOL", requestValues, "lendi", allSortOrders);
...
FieldRef[] fields = new FieldRef[] { CSGenioAlendi.FldCodlendi, CSGenioAlendi.FldZzstate, CSGenioAlendi.FldCodpess1, CSGenioApess1.FldCodpesso, CSGenioApess1.FldName, CSGenioAlendi.FldCodequip, CSGenioAequip.FldCodequip, CSGenioAequip.FldDesignat, CSGenioAequip.FldRegistnr, CSGenioAequip.FldFrequenc, CSGenioAlendi.FldCodpess2, CSGenioApess2.FldCodpesso, CSGenioApess2.FldName, CSGenioAlendi.FldLendinnr, CSGenioAlendi.FldStart, CSGenioAlendi.FldWarndt, CSGenioAlendi.FldEnd, CSGenioAlendi.FldObservat, CSGenioAlendi.FldReturndt, CSGenioAlendi.FldDayslimi, CSGenioAlendi.FldReturned, CSGenioAlendi.FldIfoutdt };
			...
			//Limit type: "SC"			//Current Area = "LENDI"			//1st Area Limit: "LENDI"			//1st Area Field: "RETURNED"			//1st Area Value: "0"
			{
				Limit limit = new Limit();
			...
			//Limit type: "SC"			//Current Area = "LENDI"			//1st Area Limit: "LENDI"			//1st Area Field: "IFOUTDT"			//1st Area Value: "1"
			{
				Limit limit = new Limit();
				limit.TipoLimite = LimitType.SC;
				limit.NaoAplicaSeNulo = false;
			...
			conditions.SubSets.Add(gqt_menu_devolConds);
			gqt_menu_devolConds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
...
// USE /[MANUAL GQT OVERRQ DEVOL]/
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAlendi>(m_userContext, false, gqt_menu_devolConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "MLDEVOL", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL GQT OVERRQLSTEXP DEVOL]/
			...
			if (tableReload)
			{
// USE /[MANUAL GQT OVERRQLIST DEVOL]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = m_userContext.PersistentSupport.getPagingPos(CSGenioAlendi.GetInformation(), QMVC_POS_RECORD, sorts, gqt_menu_devolConds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioAlendi> listing = Models.ModelBase.Where<CSGenioAlendi>(m_userContext, false, gqt_menu_devolConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "MLDEVOL", true, true, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
				...
				Menu.Elements = MapGQT_Menu_DEVOL(listing);

				Menu.Identifier = "MLDEVOL";
				Menu.Slots = new Dictionary<string, List<object>>();

				// Last updated by [CJP] at [2015.02.03]
				// Adds the identifier to each element
				foreach (var element in Menu.Elements)
					element.Identifier = "MLDEVOL";
		...
		private List<Models.Lendi> MapGQT_Menu_DEVOL(ListingMVC<CSGenioAlendi> Qlisting)
		{
			...
			if (Qlisting.Rows != null)
			{
				foreach (var row in Qlisting.Rows)
				{
					if (Qlisting.NumRegs > 0 && i >= Qlisting.NumRegs) // Copiado da versão antiga do RowsToViewModels
						break;
					Elements.Add(MapGQT_Menu_DEVOL(row));
					i++;
				}
			}
		...
		private Models.Lendi MapGQT_Menu_DEVOL(CSGenioAlendi row)
		{
