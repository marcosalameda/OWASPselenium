...
namespace GenioMVC.ViewModels.Lnhag
{
	public class Lnhag_Tpeq1ValTipoequi_ViewModel : ListViewModel
	{
		...
		public CriteriaSet BuildCriteriaSet(NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			Menu.SetFilters(bool.Parse(requestValues["Lnhag_Tpeq1ValTipoequi_tableFilters"] ?? "false"), false);
			...
			LoadUserTableConfig(requestValues, allSortOrders, "Lnhag_Tpeq1ValTipoequi", ref numberListItems);

			crs.SubSets.Add(ProcessSearchFilters(Menu, GetSearchColumns(true), requestValues, "Lnhag_Tpeq1ValTipoequi_"));
			...
			{
				// EPH
				crs = Models.Tpeq1.AddEPH<CSGenioAtpeq1>(ref u, crs, "IBL_LNHAG___TPEQ1TIPOEQUI");
			...
			if (tableReload)
			{
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					crs.Equals(Models.Tpeq1.AddEPH<CSGenioAtpeq1>(ref u, null, "IBL_LNHAG___TPEQ1TIPOEQUI"));
			}
		...
		public void Load(int numberListItems, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAtpeq1> Qlisting, ref CriteriaSet conditions)
		{
			...
			if (ajaxRequest)
				this.Navigation.SetValue("requestValues" + "Lnhag_Tpeq1ValTipoequi", requestValues);
			else if (!ajaxRequest && this.Navigation.CheckKey("requestValues" + "Lnhag_Tpeq1ValTipoequi"))
				requestValues = this.Navigation.GetValue<NameValueCollection>("requestValues" + "Lnhag_Tpeq1ValTipoequi");
			...
			var pageNumber = (ajaxRequest && !String.IsNullOrEmpty(requestValues["pLnhag_Tpeq1ValTipoequi"])) ? int.Parse(requestValues["pLnhag_Tpeq1ValTipoequi"]) : 1;

			// Added to avoid 0 or -1 pages when setting number of records to -1 to disable pagination
			if (pageNumber < 1)
				pageNumber = 1;

			List<ColumnSort> sorts = GetRequestSorts(this.Menu, "sLnhag_Tpeq1ValTipoequi", "dLnhag_Tpeq1ValTipoequi", requestValues, "tpeq1", allSortOrders);
...
FieldRef[] fields = new FieldRef[] { CSGenioAtpeq1.FldCodtpequ, CSGenioAtpeq1.FldZzstate, CSGenioAtpeq1.FldTipoequi };
			...
			conditions.SubSets.Add(lnhag___tpeq1tipoequiConds);
			lnhag___tpeq1tipoequiConds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
			tableReload &= hasAllRequiredLimits;

// USE /[MANUAL GQT OVERRQ LNHAG_TIPOEQUI]/
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAtpeq1>(m_userContext, false, lnhag___tpeq1tipoequiConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_LNHAG___TPEQ1TIPOEQUI", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL GQT OVERRQLSTEXP LNHAG_TIPOEQUI]/
			...
			if (tableReload)
			{
// USE /[MANUAL GQT OVERRQLIST LNHAG_TIPOEQUI]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = m_userContext.PersistentSupport.getPagingPos(CSGenioAtpeq1.GetInformation(), QMVC_POS_RECORD, sorts, lnhag___tpeq1tipoequiConds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioAtpeq1> listing = Models.ModelBase.Where<CSGenioAtpeq1>(m_userContext, false, lnhag___tpeq1tipoequiConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_LNHAG___TPEQ1TIPOEQUI", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
				...
				Menu.Elements = MapLnhag_Tpeq1ValTipoequi(listing);

				Menu.Identifier = "IBL_LNHAG___TPEQ1TIPOEQUI";

				// Last updated by [CJP] at [2015.02.03]
				// Adds the identifier to each element
				foreach (var element in Menu.Elements)
					element.Identifier = "IBL_LNHAG___TPEQ1TIPOEQUI";
		...
		private List<Models.Tpeq1> MapLnhag_Tpeq1ValTipoequi(ListingMVC<CSGenioAtpeq1> Qlisting)
		{
			...
			if (Qlisting.Rows != null)
			{
				foreach (var row in Qlisting.Rows)
				{
					if (Qlisting.NumRegs > 0 && i >= Qlisting.NumRegs) // Copiado da versão antiga do RowsToViewModels
						break;
					Elements.Add(MapLnhag_Tpeq1ValTipoequi(row));
					i++;
				}
			}
		...
		private Models.Tpeq1 MapLnhag_Tpeq1ValTipoequi(CSGenioAtpeq1 row)
		{
