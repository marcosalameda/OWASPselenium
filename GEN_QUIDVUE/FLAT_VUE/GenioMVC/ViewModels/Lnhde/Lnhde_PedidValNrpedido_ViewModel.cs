...
{
	public class Lnhde_PedidValNrpedido_ViewModel : ListViewModel
	{
		/// <summary>
		...
		/// <inheritdoc/>
		public override string Uuid { get => "Lnhde_PedidValNrpedido"; }
		...
		public CriteriaSet BuildCriteriaSet(NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			Menu.SetFilters(bool.Parse(requestValues["Lnhde_PedidValNrpedido_tableFilters"] ?? "false"), false);
			...
			LoadUserTableConfig(requestValues, allSortOrders, "Lnhde_PedidValNrpedido", ref numberListItems);

			crs.SubSets.Add(ProcessSearchFilters(Menu, GetSearchColumns(true), requestValues, "Lnhde_PedidValNrpedido_"));
		...
		public void Load(int numberListItems, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioApedid> Qlisting, ref CriteriaSet conditions)
		{
			...
			if (ajaxRequest)
				this.Navigation.SetValue("requestValues" + "Lnhde_PedidValNrpedido", requestValues);
			else if (!ajaxRequest && this.Navigation.CheckKey("requestValues" + "Lnhde_PedidValNrpedido"))
				requestValues = this.Navigation.GetValue<NameValueCollection>("requestValues" + "Lnhde_PedidValNrpedido");
			...
			var pageNumber = (ajaxRequest && !String.IsNullOrEmpty(requestValues["pLnhde_PedidValNrpedido"])) ? int.Parse(requestValues["pLnhde_PedidValNrpedido"]) : 1;

			// Added to avoid 0 or -1 pages when setting number of records to -1 to disable pagination
			if (pageNumber < 1)
				pageNumber = 1;

			List<ColumnSort> sorts = GetRequestSorts(this.Menu, "sLnhde_PedidValNrpedido", "dLnhde_PedidValNrpedido", requestValues, "pedid", allSortOrders);
...
FieldRef[] fields = new FieldRef[] { CSGenioApedid.FldCodpedid, CSGenioApedid.FldZzstate, CSGenioApedid.FldNrpedido };
			...
			lnhde___pedidnrpedidoConds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
...
// USE /[MANUAL GQT OVERRQ LNHDE_NRPEDIDO]/
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioApedid>(m_userContext, false, lnhde___pedidnrpedidoConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_LNHDE___PEDIDNRPEDIDO", true, firstVisibleColumn: firstVisibleColumn);
...
// USE /[MANUAL GQT OVERRQLIST LNHDE_NRPEDIDO]/
				...
				ListingMVC<CSGenioApedid> listing = Models.ModelBase.Where<CSGenioApedid>(m_userContext, false, lnhde___pedidnrpedidoConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_LNHDE___PEDIDNRPEDIDO", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
		...
		private Models.Pedid MapLnhde_PedidValNrpedido(CSGenioApedid row)
		{
