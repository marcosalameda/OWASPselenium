...
// USE /[MANUAL GQT OVERRQLIST LNHDE_PSEUDLNPROPS]/
		...
		{
			var Elements = new List<Lnhde_ValLnprops_RowViewModel>();
			int i = 0;
		...
		/// </summary>
		public void CalculateButtonPermissions(Lnhde_ValLnprops_RowViewModel model)
		{
			bool canView = true;
			bool canEdit = true;
			bool canDelete = true;
		...
		private void SetDocumentFields(ListingMVC<CSGenioAlnhdf> listing)
		{
			...
			foreach (CSGenioAlnhdf row in listing.Rows)
			{
			}
		}
...
// USE /[MANUAL GQT VIEWMODEL_CUSTOM LNHDE_VALLNPROPS]/
		...
		[
			"Lnhdf", "Lnhdf.ValCodlnhdf", "Lnhdf.ValZzstate", "Lnhdf.ValName", "Lnhdf.ValCodlnhde", "BtnPermission"
		];
