...
// USE /[MANUAL GQT LIST_LIMITS 81]/
		...
		public void LoadToExport(out ListingMVC<CSGenioAnotif> listing, out CriteriaSet conditions, out List<Exports.QColumn> columns, CSGenio.core.framework.table.TableConfiguration tableConfig, NameValueCollection requestValues, bool ajaxRequest = false)
		{
			listing = null;
			conditions = null;
		...
		/// <param name="Qlisting">The rows.</param>
		/// <param name="conditions">The conditions.</param>
		public void Load(CSGenio.core.framework.table.TableConfiguration tableConfig, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAnotif> Qlisting, ref CriteriaSet conditions)
		{
			User u = m_userContext.User;
			Menu = new TablePartial<GQT_Menu_81_RowViewModel>();
			...
			List<ColumnSort> sorts = GetRequestSorts(this.Menu, tableConfig, "notif", allSortOrders);
...
// USE /[MANUAL GQT OVERRQ 81]/
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.BuildListingForExport<CSGenioAnotif>(m_userContext, false, ref gqt_menu_81Conds, exportFieldRefs, (pageNumber - 1) * numberListItems, numberListItems, sorts, "ML81", true, firstVisibleColumn: firstVisibleColumn);
