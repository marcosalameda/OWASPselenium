...
{
	public class Pessosep_CategValCategoria_ViewModel : ListViewModel
	{
		/// <summary>
		...
		/// <inheritdoc/>
		public override string Uuid { get => "Pessosep_CategValCategoria"; }
		...
		public CriteriaSet BuildCriteriaSet(NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			LoadUserTableConfig(requestValues, allSortOrders, "Pessosep_CategValCategoria", ref numberListItems);
		...
		public void Load(int numberListItems, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAcateg> Qlisting, ref CriteriaSet conditions)
		{
			...
			if (ajaxRequest)
				this.Navigation.SetValue("requestValues" + "Pessosep_CategValCategoria", requestValues);
			else if (!ajaxRequest && this.Navigation.CheckKey("requestValues" + "Pessosep_CategValCategoria"))
				requestValues = this.Navigation.GetValue<NameValueCollection>("requestValues" + "Pessosep_CategValCategoria");
			...
			var pageNumber = (ajaxRequest && !String.IsNullOrEmpty(requestValues["pPessosep_CategValCategoria"])) ? int.Parse(requestValues["pPessosep_CategValCategoria"]) : 1;

			// Added to avoid 0 or -1 pages when setting number of records to -1 to disable pagination
			if (pageNumber < 1)
				pageNumber = 1;

			List<ColumnSort> sorts = GetRequestSorts(this.Menu, "sPessosep_CategValCategoria", "dPessosep_CategValCategoria", requestValues, "categ", allSortOrders);
...
FieldRef[] fields = new FieldRef[] { CSGenioAcateg.FldCodcateg, CSGenioAcateg.FldZzstate, CSGenioAcateg.FldCategoria, CSGenioAcateg.FldAbbreviation };
			...
			pessosepcategcategoryConds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
...
// USE /[MANUAL GQT OVERRQ PESSOSEP_CATEGCATEGORY]/
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAcateg>(m_userContext, false, pessosepcategcategoryConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_PESSOSEPCATEGCATEGORY", true, firstVisibleColumn: firstVisibleColumn);
...
// USE /[MANUAL GQT OVERRQLIST PESSOSEP_CATEGCATEGORY]/
				...
				ListingMVC<CSGenioAcateg> listing = Models.ModelBase.Where<CSGenioAcateg>(m_userContext, false, pessosepcategcategoryConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_PESSOSEPCATEGCATEGORY", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
		...
		private List<Models.Categ> MapPessosep_CategValCategoria(ListingMVC<CSGenioAcateg> Qlisting)
		{
		...
		private Models.Categ MapPessosep_CategValCategoria(CSGenioAcateg row)
		{
