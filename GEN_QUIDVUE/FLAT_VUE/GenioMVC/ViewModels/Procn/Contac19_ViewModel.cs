...
{
	public class Contac19_ViewModel : FormViewModel<Models.Procn>, IPreparableForSerialization
	{
		[JsonIgnore]
		...
		public Contac19_ViewModel() : base(null!) { }

		public Contac19_ViewModel(UserContext userContext, bool nestedForm = false) : base(userContext, "FCONTAC19", nestedForm) { }

		public Contac19_ViewModel(UserContext userContext, Models.Procn row, bool nestedForm = false) : base(userContext, "FCONTAC19", row, nestedForm) { }

		public Contac19_ViewModel(UserContext userContext, string id, bool nestedForm = false, string[]? fieldsToLoad = null) : this(userContext, nestedForm)
		{
			this.Navigation.SetValue("procn", id);
			Model = Models.Procn.Find(id, userContext, "FCONTAC19", fieldsToQuery: fieldsToLoad);
			if (Model == null)
		...
		public static StatusMessage InsertConditions(UserContext userContext)
		{
			...
			Models.Procn model = new Models.Procn(userContext) { Identifier = "FCONTAC19" };

			var navigation = m_userContext.CurrentNavigation;
			// The "LoadKeysFromHistory" must be after the "LoadEPH" because the PHE's in the tree mark Foreign Keys to null
			// (since they cannot assign multiple values to a single field) and thus the value that comes from Navigation is lost.
			// And this makes it more like the order of loading the model when opening the form.
			model.LoadEPH("FCONTAC19");
			if (navigation != null)
		...
		public override void MapFromModel(Models.Procn m)
		{
			if (m == null)
			{
				CSGenio.framework.Log.Error("Map Model (Procn) to ViewModel (Contac19) - Model is a null reference");
				throw new ModelNotFoundException("Model not found");
			}
			...
			{
				CSGenio.framework.Log.Error("Map Model (Procn) to ViewModel (Contac19) - Error during mapping");
				throw;
			}
		}
		...
		public override void MapToModel(Models.Procn m)
		{
			if (m == null)
			{
				CSGenio.framework.Log.Error("Map ViewModel (Contac19) to Model (Procn) - Model is a null reference");
				throw new ModelNotFoundException("Model not found");
			}
			...
			try
			{
				...
				if (HasDisabledUserValuesSecurity || !(Logical)((((string)m.ValTelephon) == "")))
				{
			...
			{
				CSGenio.framework.Log.Error($"Map ViewModel (Contac19) to Model (Procn) - Error during mapping. All user values: {HasDisabledUserValuesSecurity}");
				throw;
			}
		}
			...
			try
			{
				...
				switch (fullFieldName)
				{
					...
					case "procn.codprocn":
						...
						break;
					default:
						Log.Error($"SetViewModelValue (Contac19) - Unexpected field identifier {fullFieldName}");
						break;
				}
			}
			catch (Exception ex)
			{
				throw new FrameworkException(Resources.Resources.PEDIMOS_DESCULPA__OC63848, "SetViewModelValue (Contac19)", "Unexpected error", ex);
			}
		}
		...
		{
			try { Model = Models.Procn.Find(id ?? Navigation.GetStrValue("procn"), m_userContext, "FCONTAC19"); }
			finally { Model ??= new Models.Procn(m_userContext) { Identifier = "FCONTAC19" }; }
		...
		public override void Load(NameValueCollection qs, bool editable, bool ajaxRequest = false, bool lazyLoad = false)
		{
			...
			try
			{
				Model = Models.Procn.Find(Navigation.GetStrValue("procn"), m_userContext, "FCONTAC19");
			}
			finally
			{
				if (Model == null)
		...
		public override void LoadPartial(NameValueCollection qs, bool lazyLoad = false)
		{
			...
			if (Model == null)
			{
				...
				if (Model == null)
				{
					Model = new Models.Procn(m_userContext) { Identifier = "FCONTAC19" };
...
// USE /[MANUAL GQT VIEWMODEL_NEW CONTAC19]/
...
// USE /[MANUAL GQT VIEWMODEL_SAVE CONTAC19]/
...
// USE /[MANUAL GQT VIEWMODEL_APPLY CONTAC19]/

// USE /[MANUAL GQT VIEWMODEL_DUPLICATE CONTAC19]/

// USE /[MANUAL GQT VIEWMODEL_DESTROY CONTAC19]/
		public override void Destroy(string id)
		{
			Model = Models.Procn.Find(id, m_userContext, "FCONTAC19");
			if (Model == null)
				throw new ModelNotFoundException("Model not found");
		...
		/// <param name="qs"></param>
		/// <param name="lazyLoad">Lazy loading of dropdown items</param>
		public void Load_Contac19propetitle___(NameValueCollection qs, bool lazyLoad = false)
		{
			bool contac19propetitle___DoLoad = true;
			CriteriaSet contac19propetitle___Conds = CriteriaSet.And();
			{
				object hValue = Navigation.GetValue("prope", true);
				if (hValue != null && !(hValue is Array) && !string.IsNullOrEmpty(Convert.ToString(hValue)))
				{
					contac19propetitle___Conds.Equal(CSGenioAprope.FldCodprope, hValue);
			...
			if (lazyLoad)
			{
				...
				FillDependant_Contac19TablePropeTitle(lazyLoad);
				return;
			}

			if (contac19propetitle___DoLoad)
			{
				...
				contac19propetitle___Conds.SubSet(search_filters);
...
// USE /[MANUAL GQT OVERRQ CONTAC19_PROPETITLE]/
				...
				*/
				if (Navigation.checkFormMode("prope", FormMode.New) || Navigation.checkFormMode("prope", FormMode.Duplicate))
					contac19propetitle___Conds.SubSet(CriteriaSet.Or()
						.Equal(CSGenioAprope.FldZzstate, 0)
						.Equal(CSGenioAprope.FldCodprope, Navigation.GetStrValue("prope")));
				else
					contac19propetitle___Conds.Criterias.Add(new Criteria(new ColumnReference(CSGenioAprope.FldZzstate), CriteriaOperator.Equal, 0));

				FieldRef firstVisibleColumn = new FieldRef("prope", "title");
				ListingMVC<CSGenioAprope> listing = Models.ModelBase.Where<CSGenioAprope>(m_userContext, false, contac19propetitle___Conds, fields, offset, numberItems, sorts, "LED_CONTAC19PROPETITLE___", true, false, firstVisibleColumn: firstVisibleColumn);
				...
				TablePropeTitle.Elements = listing.RowsForViewModel<GenioMVC.Models.Prope>((r) => new GenioMVC.Models.Prope(m_userContext, r, true, _fieldsToSerialize_CONTAC19PROPETITLE___));
				...
				FillDependant_Contac19TablePropeTitle();
			}
		}
		...
		public ConcurrentDictionary<string, object> GetDependant_Contac19TablePropeTitle(string PKey)
		{
		...
		/// </summary>
		/// <param name="lazyLoad">Lazy loading of dropdown items</param>
		public void FillDependant_Contac19TablePropeTitle(bool lazyLoad = false)
		{
			var row = GetDependant_Contac19TablePropeTitle(this.ValCodprope);
			try
			{
		...
		private readonly string[] _fieldsToSerialize_CONTAC19PROPETITLE___ = ["Prope", "Prope.ValCodprope", "Prope.ValZzstate", "Prope.ValTitle"];
...
// USE /[MANUAL GQT VIEWMODEL_CUSTOM CONTAC19]/
