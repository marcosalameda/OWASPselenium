...
// USE /[MANUAL GQT LIST_LIMITS PRODU_PSEUDSTOCKEVO]/
		...
		public void LoadToExport(out ListingMVC<CSGenioAstock> listing, out CriteriaSet conditions, out List<Exports.QColumn> columns, CSGenio.core.framework.table.TableConfiguration tableConfig, NameValueCollection requestValues, bool ajaxRequest = false)
		{
			listing = null;
			conditions = null;
		...
		/// <param name="Qlisting">The rows.</param>
		/// <param name="conditions">The conditions.</param>
		public void Load(CSGenio.core.framework.table.TableConfiguration tableConfig, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAstock> Qlisting, ref CriteriaSet conditions)
		{
			User u = m_userContext.User;
			Menu = new TablePartial<Produ_ValStockevo_RowViewModel>();
			...
			List<ColumnSort> sorts = GetRequestSorts(this.Menu, tableConfig, "stock", allSortOrders);
			...
			conditions.SubSets.Add(produ___pseudstockevoConds);
			produ___pseudstockevoConds = BuildCriteriaSet(tableConfig, requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
...
// USE /[MANUAL GQT OVERRQ PRODU_PSEUDSTOCKEVO]/
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.BuildListingForExport<CSGenioAstock>(m_userContext, false, ref produ___pseudstockevoConds, exportFieldRefs, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_PRODU___PSEUDSTOCKEVO", true, firstVisibleColumn: firstVisibleColumn);
...
// USE /[MANUAL GQT OVERRQLIST PRODU_PSEUDSTOCKEVO]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = m_userContext.PersistentSupport.getPagingPos(CSGenioAstock.GetInformation(), QMVC_POS_RECORD, sorts, produ___pseudstockevoConds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
