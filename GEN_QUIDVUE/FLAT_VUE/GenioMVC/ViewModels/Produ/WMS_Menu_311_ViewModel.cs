...
{
	public class WMS_Menu_311_ViewModel : ListViewModel
	{
		/// <summary>
		/// Gets or sets the object that represents the table and its elements.
		/// </summary>
		[JsonPropertyName("Table")]
		public TablePartial<GenioMVC.Models.Produ> Menu { get; set; }

		protected override TableViewsManagementMode ViewsManagementMode { get => TableViewsManagementMode.PersistOne; }

		/// <inheritdoc/>
		public override string TableAlias { get => "produ"; }

		/// <inheritdoc/>
		public override string Uuid { get => "92697297-35db-4e66-a7dc-b3b9b29febb5"; }
		...
		{
			dbeditTitle = Resources.Resources.PRODUCTS34689;
		}
...
FieldRef[] fields = new FieldRef[] { CSGenioAprodu.FldCodprodu, CSGenioAprodu.FldZzstate, CSGenioAprodu.FldProduct, CSGenioAprodu.FldSku, CSGenioAprodu.FldDescript, CSGenioAprodu.FldGtin, CSGenioAprodu.FldSize, CSGenioAprodu.FldWeight, CSGenioAprodu.FldCodlocat, CSGenioAlocat.FldCodlocat, CSGenioAlocat.FldGln, CSGenioAprodu.FldCodlcext, CSGenioAlcext.FldCodlcext, CSGenioAlcext.FldGlnext, CSGenioAprodu.FldInputs, CSGenioAprodu.FldOutputs, CSGenioAprodu.FldStock, CSGenioAprodu.FldPrice, CSGenioAprodu.FldIn_use, CSGenioAprodu.FldImage };

			ListingMVC<CSGenioAprodu> listing = new ListingMVC<CSGenioAprodu>(fields, null, 1, 1, false, user, true, string.Empty, false);
		...
		/// <summary>
		/// Initializes a new instance of the <see cref="WMS_Menu_311_ViewModel" /> class.
		/// </summary>
		/// <param name="userContext">The current user request context</param>
		public WMS_Menu_311_ViewModel(UserContext userContext) : base(userContext)
		{
		...
		public void LoadToExport(out ListingMVC<CSGenioAprodu> listing, out CriteriaSet conditions, out List<Exports.QColumn> columns, NameValueCollection requestValues, bool ajaxRequest = false)
		{
			...
			if (ajaxRequest && userColumns!=null)
			{
				...
				foreach (CSGenioAlstcol column in userColumns)
				{
					...
					string areabase = column.ValTabela.ToLower() != "produ" ? CultureInfo.InvariantCulture.TextInfo.ToTitleCase(column.ValTabela) + "." : "";
		...
		public CriteriaSet BuildCriteriaSet(NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			if (Menu == null)
				Menu = new TablePartial<GenioMVC.Models.Produ>();
			Menu.SetFilters(bool.Parse(requestValues["WMS_Menu_311_tableFilters"] ?? "false"), false);
			...
			LoadUserTableConfig(requestValues, allSortOrders, "WMS_Menu_311", ref numberListItems);

			crs.SubSets.Add(ProcessSearchFilters(Menu, GetSearchColumns(true), requestValues, "WMS_Menu_311_"));
			...
			{
				// EPH
				crs = Models.Produ.AddEPH<CSGenioAprodu>(ref u, crs, "ML311");
			...
			if (tableReload)
			{
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					crs.Equals(Models.Produ.AddEPH<CSGenioAprodu>(ref u, null, "ML311"));
			}
		...
		public void Load(int numberListItems, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAprodu> Qlisting, ref CriteriaSet conditions)
		{
			...
			if (ajaxRequest)
				this.Navigation.SetValue("requestValues" + "WMS_Menu_311", requestValues);
			else if (!ajaxRequest && this.Navigation.CheckKey("requestValues" + "WMS_Menu_311"))
				requestValues = this.Navigation.GetValue<NameValueCollection>("requestValues" + "WMS_Menu_311");

			User u = m_userContext.User;
			Menu = new TablePartial<GenioMVC.Models.Produ>();

			CriteriaSet wms_menu_311Conds = CriteriaSet.And();
			...
			var pageNumber = (ajaxRequest && !String.IsNullOrEmpty(requestValues["pWMS_Menu_311"])) ? int.Parse(requestValues["pWMS_Menu_311"]) : 1;

			// Added to avoid 0 or -1 pages when setting number of records to -1 to disable pagination
			if (pageNumber < 1)
				pageNumber = 1;

			List<ColumnSort> sorts = GetRequestSorts(this.Menu, "sWMS_Menu_311", "dWMS_Menu_311", requestValues, "produ", allSortOrders);
...
FieldRef[] fields = new FieldRef[] { CSGenioAprodu.FldCodprodu, CSGenioAprodu.FldZzstate, CSGenioAprodu.FldProduct, CSGenioAprodu.FldSku, CSGenioAprodu.FldDescript, CSGenioAprodu.FldGtin, CSGenioAprodu.FldSize, CSGenioAprodu.FldWeight, CSGenioAprodu.FldCodlocat, CSGenioAlocat.FldCodlocat, CSGenioAlocat.FldGln, CSGenioAprodu.FldCodlcext, CSGenioAlcext.FldCodlcext, CSGenioAlcext.FldGlnext, CSGenioAprodu.FldInputs, CSGenioAprodu.FldOutputs, CSGenioAprodu.FldStock, CSGenioAprodu.FldPrice, CSGenioAprodu.FldIn_use, CSGenioAprodu.FldImage };
			...
			conditions.SubSets.Add(wms_menu_311Conds);
			wms_menu_311Conds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
...
// USE /[MANUAL WMS OVERRQ 311]/
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAprodu>(m_userContext, false, wms_menu_311Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "ML311", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL WMS OVERRQLSTEXP 311]/
...
// USE /[MANUAL WMS OVERRQLIST 311]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = m_userContext.PersistentSupport.getPagingPos(CSGenioAprodu.GetInformation(), QMVC_POS_RECORD, sorts, wms_menu_311Conds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioAprodu> listing = Models.ModelBase.Where<CSGenioAprodu>(m_userContext, false, wms_menu_311Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "ML311", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
				...
				Menu.Elements = MapWMS_Menu_311(listing);
		...
		{
			var Elements = new List<Models.Produ>();
			int i = 0;
			...
			if (Qlisting.Rows != null)
			{
				foreach (var row in Qlisting.Rows)
				{
					if (Qlisting.NumRegs > 0 && i >= Qlisting.NumRegs) // Copiado da versão antiga do RowsToViewModels
						break;
					Elements.Add(MapWMS_Menu_311(row));
					i++;
				}
			}
		...
		/// to a Models.Produ object.
		/// </summary>
		/// <param name="row">The row.</param>
		private Models.Produ MapWMS_Menu_311(CSGenioAprodu row)
		{
			var model = new Models.Produ(m_userContext, true, _fieldsToSerialize);
		...
		private void SetDocumentFields(ListingMVC<CSGenioAprodu> listing)
		{
			...
			foreach (CSGenioAprodu row in listing.Rows)
			{
			}
		}

		#region Custom code
// USE /[MANUAL GQT VIEWMODEL_CUSTOM WMS_MENU_311]/
		#endregion

		private static readonly string[] _fieldsToSerialize =
		{
			"Produ", "Produ.ValCodprodu", "Produ.ValZzstate", "Produ.ValProduct", "Produ.ValSku", "Produ.ValDescript", "Produ.ValGtin", "Produ.ValSize", "Produ.ValWeight", "Locat", "Locat.ValGln", "Lcext", "Lcext.ValGlnext", "Produ.ValInputs", "Produ.ValOutputs", "Produ.ValStock", "Produ.ValPrice", "Produ.ValIn_use", "Produ.ValImage", "Produ.ValCodlcext", "Produ.ValCodlocat"
		};
		...
		private static readonly List<TableSearchColumn> _searchableColumns = new List<TableSearchColumn>
		{
			...
			new TableSearchColumn("ValDescript", CSGenioAprodu.FldDescript, typeof(string), visible : false),
			new TableSearchColumn("ValGtin", CSGenioAprodu.FldGtin, typeof(string), visible : false),
			new TableSearchColumn("ValSize", CSGenioAprodu.FldSize, typeof(string), visible : false),
			new TableSearchColumn("ValWeight", CSGenioAprodu.FldWeight, typeof(decimal?), visible : false),
			...
			new TableSearchColumn("ValInputs", CSGenioAprodu.FldInputs, typeof(decimal?)),
