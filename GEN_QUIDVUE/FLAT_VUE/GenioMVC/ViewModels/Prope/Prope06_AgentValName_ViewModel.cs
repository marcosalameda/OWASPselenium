...
namespace GenioMVC.ViewModels.Prope
{
	public class Prope06_AgentValName_ViewModel : MenuListViewModel<Models.Agent>
	{
		...
		{
// USE /[MANUAL GQT LIST_LIMITS PROPE06_AGENTNAME]/
		...
		public override CriteriaSet BuildCriteriaSet(CSGenio.framework.TableConfiguration.TableConfiguration tableConfig, NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			{
				// EPH
				crs = Models.Agent.AddEPH<CSGenioAagent>(ref u, crs, "IBL_PROPE06_AGENTNAME____");
			...
			if (tableReload)
			{
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					crs.Equals(Models.Agent.AddEPH<CSGenioAagent>(ref u, null, "IBL_PROPE06_AGENTNAME____"));
			}
		...
		public void Load(CSGenio.framework.TableConfiguration.TableConfiguration tableConfig, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAagent> Qlisting, ref CriteriaSet conditions)
		{
			...
			}, "ms", "Time to load the form."))
			{
				...
				conditions.SubSets.Add(prope06_agentname____Conds);
				prope06_agentname____Conds = BuildCriteriaSet(tableConfig, requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
...
// USE /[MANUAL GQT OVERRQ PROPE06_AGENTNAME]/
				...
				if (isToExport)
				{
					...
					Qlisting = Models.ModelBase.Where<CSGenioAagent>(m_userContext, false, prope06_agentname____Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_PROPE06_AGENTNAME____", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL GQT OVERRQLSTEXP PROPE06_AGENTNAME]/
				...
				if (tableReload)
				{
// USE /[MANUAL GQT OVERRQLIST PROPE06_AGENTNAME]/
					...
					if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					{
						var m_iCurPag = m_userContext.PersistentSupport.getPagingPos(CSGenioAagent.GetInformation(), QMVC_POS_RECORD, sorts, prope06_agentname____Conds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
						if (m_iCurPag != -1)
							pageNumber = ((m_iCurPag - 1) / numberListItems) + 1;
					}

					ListingMVC<CSGenioAagent> listing = Models.ModelBase.Where<CSGenioAagent>(m_userContext, false, prope06_agentname____Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_PROPE06_AGENTNAME____", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn, fieldsWithTotalizers, tableConfig.SelectedRows);
					...
					Menu.Elements = MapPrope06_AgentValName(listing);

					Menu.Identifier = "IBL_PROPE06_AGENTNAME____";

					// Last updated by [CJP] at [2015.02.03]
					// Adds the identifier to each element
					foreach (var element in Menu.Elements)
						element.Identifier = "IBL_PROPE06_AGENTNAME____";
			...
			if (Qlisting.Rows != null)
			{
				foreach (var row in Qlisting.Rows)
				{
					if (Qlisting.NumRegs > 0 && i >= Qlisting.NumRegs) // Copiado da versão antiga do RowsToViewModels
						break;
					Elements.Add(MapPrope06_AgentValName(row));
					i++;
				}
			}
