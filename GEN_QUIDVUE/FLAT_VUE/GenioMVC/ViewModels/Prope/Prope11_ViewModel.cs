...
{
	public class Prope11_ViewModel : FormViewModel<Models.Prope>, IPreparableForSerialization
	{
		[JsonIgnore]
		...
		public Prope11_ViewModel() : base(null!) { }

		public Prope11_ViewModel(UserContext userContext, bool nestedForm = false) : base(userContext, "FPROPE11", nestedForm) { }

		public Prope11_ViewModel(UserContext userContext, Models.Prope row, bool nestedForm = false) : base(userContext, "FPROPE11", row, nestedForm) { }

		public Prope11_ViewModel(UserContext userContext, string id, bool nestedForm = false, string[]? fieldsToLoad = null) : this(userContext, nestedForm)
		{
			this.Navigation.SetValue("prope", id);
			Model = Models.Prope.Find(id, userContext, "FPROPE11", fieldsToQuery: fieldsToLoad);
			if (Model == null)
		...
		public static StatusMessage InsertConditions(UserContext userContext)
		{
			...
			Models.Prope model = new Models.Prope(userContext) { Identifier = "FPROPE11" };

			var navigation = m_userContext.CurrentNavigation;
			// The "LoadKeysFromHistory" must be after the "LoadEPH" because the PHE's in the tree mark Foreign Keys to null
			// (since they cannot assign multiple values to a single field) and thus the value that comes from Navigation is lost.
			// And this makes it more like the order of loading the model when opening the form.
			model.LoadEPH("FPROPE11");
			if (navigation != null)
		...
		public override void MapFromModel(Models.Prope m)
		{
			if (m == null)
			{
				CSGenio.framework.Log.Error("Map Model (Prope) to ViewModel (Prope11) - Model is a null reference");
				throw new ModelNotFoundException("Model not found");
			}
			...
			{
				CSGenio.framework.Log.Error("Map Model (Prope) to ViewModel (Prope11) - Error during mapping");
				throw;
			}
		}
		...
		public override void MapToModel(Models.Prope m)
		{
			if (m == null)
			{
				CSGenio.framework.Log.Error("Map ViewModel (Prope11) to Model (Prope) - Model is a null reference");
				throw new ModelNotFoundException("Model not found");
			}
			...
			{
				CSGenio.framework.Log.Error($"Map ViewModel (Prope11) to Model (Prope) - Error during mapping. All user values: {HasDisabledUserValuesSecurity}");
				throw;
			}
		}
			...
			try
			{
				...
				switch (fullFieldName)
				{
					...
					case "prope.codprope":
						...
						break;
					default:
						Log.Error($"SetViewModelValue (Prope11) - Unexpected field identifier {fullFieldName}");
						break;
				}
			}
			catch (Exception ex)
			{
				throw new FrameworkException(Resources.Resources.PEDIMOS_DESCULPA__OC63848, "SetViewModelValue (Prope11)", "Unexpected error", ex);
			}
		}
		...
		{
			try { Model = Models.Prope.Find(id ?? Navigation.GetStrValue("prope"), m_userContext, "FPROPE11"); }
			finally { Model ??= new Models.Prope(m_userContext) { Identifier = "FPROPE11" }; }
		...
		public override void Load(NameValueCollection qs, bool editable, bool ajaxRequest = false, bool lazyLoad = false)
		{
			...
			try
			{
				Model = Models.Prope.Find(Navigation.GetStrValue("prope"), m_userContext, "FPROPE11");
			}
			finally
			{
				if (Model == null)
		...
		public override void LoadPartial(NameValueCollection qs, bool lazyLoad = false)
		{
			...
			if (Model == null)
			{
				...
				if (Model == null)
				{
					Model = new Models.Prope(m_userContext) { Identifier = "FPROPE11" };
			...
			Load_Prope11_city_city____(qs, lazyLoad);
			Load_Prope11_agentname____(qs, lazyLoad);
// USE /[MANUAL GQT VIEWMODEL_LOADPARTIAL PROPE11]/
		}

// USE /[MANUAL GQT VIEWMODEL_NEW PROPE11]/
...
// USE /[MANUAL GQT VIEWMODEL_SAVE PROPE11]/
...
// USE /[MANUAL GQT VIEWMODEL_APPLY PROPE11]/

// USE /[MANUAL GQT VIEWMODEL_DUPLICATE PROPE11]/

// USE /[MANUAL GQT VIEWMODEL_DESTROY PROPE11]/
		public override void Destroy(string id)
		{
			Model = Models.Prope.Find(id, m_userContext, "FPROPE11");
			if (Model == null)
				throw new ModelNotFoundException("Model not found");
		...
		/// <param name="qs"></param>
		/// <param name="lazyLoad">Lazy loading of dropdown items</param>
		public void Load_Prope11_city_city____(NameValueCollection qs, bool lazyLoad = false)
		{
			bool prope11_city_city____DoLoad = true;
			CriteriaSet prope11_city_city____Conds = CriteriaSet.And();
			{
				object hValue = Navigation.GetValue("city", true);
				if (hValue != null && !(hValue is Array) && !string.IsNullOrEmpty(Convert.ToString(hValue)))
				{
					prope11_city_city____Conds.Equal(CSGenioAcity.FldCodcity, hValue);
			...
			if (lazyLoad)
			{
				...
				FillDependant_Prope11TableCityCity(lazyLoad);
				return;
			}

			if (prope11_city_city____DoLoad)
			{
				...
				prope11_city_city____Conds.SubSet(search_filters);
...
// USE /[MANUAL GQT OVERRQ PROPE11_CITYCITY]/
				...
				*/
				if (Navigation.checkFormMode("city", FormMode.New) || Navigation.checkFormMode("city", FormMode.Duplicate))
					prope11_city_city____Conds.SubSet(CriteriaSet.Or()
						.Equal(CSGenioAcity.FldZzstate, 0)
						.Equal(CSGenioAcity.FldCodcity, Navigation.GetStrValue("city")));
				else
					prope11_city_city____Conds.Criterias.Add(new Criteria(new ColumnReference(CSGenioAcity.FldZzstate), CriteriaOperator.Equal, 0));

				FieldRef firstVisibleColumn = new FieldRef("city", "city");
				ListingMVC<CSGenioAcity> listing = Models.ModelBase.Where<CSGenioAcity>(m_userContext, false, prope11_city_city____Conds, fields, offset, numberItems, sorts, "LED_PROPE11_CITY_CITY____", true, false, firstVisibleColumn: firstVisibleColumn);
				...
				TableCityCity.Elements = listing.RowsForViewModel<GenioMVC.Models.City>((r) => new GenioMVC.Models.City(m_userContext, r, true, _fieldsToSerialize_PROPE11_CITY_CITY____));
				...
				FillDependant_Prope11TableCityCity();
			}
		}
		...
		public ConcurrentDictionary<string, object> GetDependant_Prope11TableCityCity(string PKey)
		{
		...
		/// </summary>
		/// <param name="lazyLoad">Lazy loading of dropdown items</param>
		public void FillDependant_Prope11TableCityCity(bool lazyLoad = false)
		{
			var row = GetDependant_Prope11TableCityCity(this.ValCodcity);
			try
			{
		...
		private readonly string[] _fieldsToSerialize_PROPE11_CITY_CITY____ = ["City", "City.ValCodcity", "City.ValZzstate", "City.ValCity"];
		...
		/// <param name="qs"></param>
		/// <param name="lazyLoad">Lazy loading of dropdown items</param>
		public void Load_Prope11_agentname____(NameValueCollection qs, bool lazyLoad = false)
		{
			bool prope11_agentname____DoLoad = true;
			CriteriaSet prope11_agentname____Conds = CriteriaSet.And();
			{
				object hValue = Navigation.GetValue("agent", true);
				if (hValue != null && !(hValue is Array) && !string.IsNullOrEmpty(Convert.ToString(hValue)))
				{
					prope11_agentname____Conds.Equal(CSGenioAagent.FldCodagent, hValue);
			...
			if (lazyLoad)
			{
				...
				FillDependant_Prope11TableAgentName(lazyLoad);
				return;
			}

			if (prope11_agentname____DoLoad)
			{
				List<ColumnSort> sorts = new List<ColumnSort>();
				...
				prope11_agentname____Conds.SubSet(search_filters);
...
// USE /[MANUAL GQT OVERRQ PROPE11_AGENTNAME]/
				...
				*/
				if (Navigation.checkFormMode("agent", FormMode.New) || Navigation.checkFormMode("agent", FormMode.Duplicate))
					prope11_agentname____Conds.SubSet(CriteriaSet.Or()
						.Equal(CSGenioAagent.FldZzstate, 0)
						.Equal(CSGenioAagent.FldCodagent, Navigation.GetStrValue("agent")));
				else
					prope11_agentname____Conds.Criterias.Add(new Criteria(new ColumnReference(CSGenioAagent.FldZzstate), CriteriaOperator.Equal, 0));

				FieldRef firstVisibleColumn = new FieldRef("agent", "name");
				ListingMVC<CSGenioAagent> listing = Models.ModelBase.Where<CSGenioAagent>(m_userContext, false, prope11_agentname____Conds, fields, offset, numberItems, sorts, "LED_PROPE11_AGENTNAME____", true, false, firstVisibleColumn: firstVisibleColumn);
				...
				TableAgentName.Elements = listing.RowsForViewModel<GenioMVC.Models.Agent>((r) => new GenioMVC.Models.Agent(m_userContext, r, true, _fieldsToSerialize_PROPE11_AGENTNAME____));
				...
				FillDependant_Prope11TableAgentName();
			}
		}
		...
		public ConcurrentDictionary<string, object> GetDependant_Prope11TableAgentName(string PKey)
		{
		...
		/// </summary>
		/// <param name="lazyLoad">Lazy loading of dropdown items</param>
		public void FillDependant_Prope11TableAgentName(bool lazyLoad = false)
		{
			var row = GetDependant_Prope11TableAgentName(this.ValCodagent);
			try
			{
		...
		private readonly string[] _fieldsToSerialize_PROPE11_AGENTNAME____ = ["Agent", "Agent.ValCodagent", "Agent.ValZzstate", "Agent.ValName"];
...
// USE /[MANUAL GQT VIEWMODEL_CUSTOM PROPE11]/
