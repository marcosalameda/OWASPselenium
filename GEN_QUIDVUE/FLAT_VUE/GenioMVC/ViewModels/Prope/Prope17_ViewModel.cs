...
{
	public class Prope17_ViewModel : FormViewModel<Models.Prope>, IPreparableForSerialization
	{
		[JsonIgnore]
		...
		public Prope17_ViewModel() : base(null!) { }

		public Prope17_ViewModel(UserContext userContext, bool nestedForm = false) : base(userContext, "FPROPE17", nestedForm) { }

		public Prope17_ViewModel(UserContext userContext, Models.Prope row, bool nestedForm = false) : base(userContext, "FPROPE17", row, nestedForm) { }

		public Prope17_ViewModel(UserContext userContext, string id, bool nestedForm = false, string[]? fieldsToLoad = null) : this(userContext, nestedForm)
		{
			this.Navigation.SetValue("prope", id);
			Model = Models.Prope.Find(id, userContext, "FPROPE17", fieldsToQuery: fieldsToLoad);
			if (Model == null)
		...
		public static StatusMessage InsertConditions(UserContext userContext)
		{
			...
			Models.Prope model = new Models.Prope(userContext) { Identifier = "FPROPE17" };

			var navigation = m_userContext.CurrentNavigation;
			// The "LoadKeysFromHistory" must be after the "LoadEPH" because the PHE's in the tree mark Foreign Keys to null
			// (since they cannot assign multiple values to a single field) and thus the value that comes from Navigation is lost.
			// And this makes it more like the order of loading the model when opening the form.
			model.LoadEPH("FPROPE17");
			if (navigation != null)
		...
		public override void MapFromModel(Models.Prope m)
		{
			if (m == null)
			{
				CSGenio.framework.Log.Error("Map Model (Prope) to ViewModel (Prope17) - Model is a null reference");
				throw new ModelNotFoundException("Model not found");
			}
			...
			{
				CSGenio.framework.Log.Error("Map Model (Prope) to ViewModel (Prope17) - Error during mapping");
				throw;
			}
		}
		...
		public override void MapToModel(Models.Prope m)
		{
			if (m == null)
			{
				CSGenio.framework.Log.Error("Map ViewModel (Prope17) to Model (Prope) - Model is a null reference");
				throw new ModelNotFoundException("Model not found");
			}
			...
			{
				CSGenio.framework.Log.Error($"Map ViewModel (Prope17) to Model (Prope) - Error during mapping. All user values: {HasDisabledUserValuesSecurity}");
				throw;
			}
		}
			...
			try
			{
				...
				switch (fullFieldName)
				{
					...
					case "prope.codprope":
						...
						break;
					default:
						Log.Error($"SetViewModelValue (Prope17) - Unexpected field identifier {fullFieldName}");
						break;
				}
			}
			catch (Exception ex)
			{
				throw new FrameworkException(Resources.Resources.PEDIMOS_DESCULPA__OC63848, "SetViewModelValue (Prope17)", "Unexpected error", ex);
			}
		}
		...
		{
			try { Model = Models.Prope.Find(id ?? Navigation.GetStrValue("prope"), m_userContext, "FPROPE17"); }
			finally { Model ??= new Models.Prope(m_userContext) { Identifier = "FPROPE17" }; }
		...
		public override void Load(NameValueCollection qs, bool editable, bool ajaxRequest = false, bool lazyLoad = false)
		{
			...
			try
			{
				Model = Models.Prope.Find(Navigation.GetStrValue("prope"), m_userContext, "FPROPE17");
			}
			finally
			{
				if (Model == null)
		...
		public override void LoadPartial(NameValueCollection qs, bool lazyLoad = false)
		{
			...
			if (Model == null)
			{
				...
				if (Model == null)
				{
					Model = new Models.Prope(m_userContext) { Identifier = "FPROPE17" };
...
// USE /[MANUAL GQT VIEWMODEL_NEW PROPE17]/
...
// USE /[MANUAL GQT VIEWMODEL_SAVE PROPE17]/
...
// USE /[MANUAL GQT VIEWMODEL_APPLY PROPE17]/

// USE /[MANUAL GQT VIEWMODEL_DUPLICATE PROPE17]/

// USE /[MANUAL GQT VIEWMODEL_DESTROY PROPE17]/
		public override void Destroy(string id)
		{
			Model = Models.Prope.Find(id, m_userContext, "FPROPE17");
			if (Model == null)
				throw new ModelNotFoundException("Model not found");
		...
		/// <param name="qs"></param>
		/// <param name="lazyLoad">Lazy loading of dropdown items</param>
		public void Load_Prope17_city_city____(NameValueCollection qs, bool lazyLoad = false)
		{
			bool prope17_city_city____DoLoad = true;
			CriteriaSet prope17_city_city____Conds = CriteriaSet.And();
			{
				object hValue = Navigation.GetValue("city", true);
				if (hValue != null && !(hValue is Array) && !string.IsNullOrEmpty(Convert.ToString(hValue)))
				{
					prope17_city_city____Conds.Equal(CSGenioAcity.FldCodcity, hValue);
			...
			if (lazyLoad)
			{
				...
				FillDependant_Prope17TableCityCity(lazyLoad);
				return;
			}

			if (prope17_city_city____DoLoad)
			{
				...
				prope17_city_city____Conds.SubSet(search_filters);
...
// USE /[MANUAL GQT OVERRQ PROPE17_CITYCITY]/
				...
				*/
				if (Navigation.checkFormMode("city", FormMode.New) || Navigation.checkFormMode("city", FormMode.Duplicate))
					prope17_city_city____Conds.SubSet(CriteriaSet.Or()
						.Equal(CSGenioAcity.FldZzstate, 0)
						.Equal(CSGenioAcity.FldCodcity, Navigation.GetStrValue("city")));
				else
					prope17_city_city____Conds.Criterias.Add(new Criteria(new ColumnReference(CSGenioAcity.FldZzstate), CriteriaOperator.Equal, 0));

				FieldRef firstVisibleColumn = new FieldRef("city", "city");
				ListingMVC<CSGenioAcity> listing = Models.ModelBase.Where<CSGenioAcity>(m_userContext, false, prope17_city_city____Conds, fields, offset, numberItems, sorts, "LED_PROPE17_CITY_CITY____", true, false, firstVisibleColumn: firstVisibleColumn);
				...
				TableCityCity.Elements = listing.RowsForViewModel<GenioMVC.Models.City>((r) => new GenioMVC.Models.City(m_userContext, r, true, _fieldsToSerialize_PROPE17_CITY_CITY____));
				...
				FillDependant_Prope17TableCityCity();
			}
		}
		...
		public ConcurrentDictionary<string, object> GetDependant_Prope17TableCityCity(string PKey)
		{
		...
		/// </summary>
		/// <param name="lazyLoad">Lazy loading of dropdown items</param>
		public void FillDependant_Prope17TableCityCity(bool lazyLoad = false)
		{
			var row = GetDependant_Prope17TableCityCity(this.ValCodcity);
			try
			{
		...
		private readonly string[] _fieldsToSerialize_PROPE17_CITY_CITY____ = ["City", "City.ValCodcity", "City.ValZzstate", "City.ValCity"];
		...
		/// <param name="qs"></param>
		/// <param name="lazyLoad">Lazy loading of dropdown items</param>
		public void Load_Prope17_agentname____(NameValueCollection qs, bool lazyLoad = false)
		{
			bool prope17_agentname____DoLoad = true;
			CriteriaSet prope17_agentname____Conds = CriteriaSet.And();
			{
				object hValue = Navigation.GetValue("agent", true);
				if (hValue != null && !(hValue is Array) && !string.IsNullOrEmpty(Convert.ToString(hValue)))
				{
					prope17_agentname____Conds.Equal(CSGenioAagent.FldCodagent, hValue);
			...
			if (lazyLoad)
			{
				...
				FillDependant_Prope17TableAgentName(lazyLoad);
				return;
			}

			if (prope17_agentname____DoLoad)
			{
				List<ColumnSort> sorts = new List<ColumnSort>();
				...
				prope17_agentname____Conds.SubSet(search_filters);
...
// USE /[MANUAL GQT OVERRQ PROPE17_AGENTNAME]/
				...
				*/
				if (Navigation.checkFormMode("agent", FormMode.New) || Navigation.checkFormMode("agent", FormMode.Duplicate))
					prope17_agentname____Conds.SubSet(CriteriaSet.Or()
						.Equal(CSGenioAagent.FldZzstate, 0)
						.Equal(CSGenioAagent.FldCodagent, Navigation.GetStrValue("agent")));
				else
					prope17_agentname____Conds.Criterias.Add(new Criteria(new ColumnReference(CSGenioAagent.FldZzstate), CriteriaOperator.Equal, 0));

				FieldRef firstVisibleColumn = new FieldRef("agent", "name");
				ListingMVC<CSGenioAagent> listing = Models.ModelBase.Where<CSGenioAagent>(m_userContext, false, prope17_agentname____Conds, fields, offset, numberItems, sorts, "LED_PROPE17_AGENTNAME____", true, false, firstVisibleColumn: firstVisibleColumn);
				...
				TableAgentName.Elements = listing.RowsForViewModel<GenioMVC.Models.Agent>((r) => new GenioMVC.Models.Agent(m_userContext, r, true, _fieldsToSerialize_PROPE17_AGENTNAME____));
				...
				FillDependant_Prope17TableAgentName();
			}
		}
		...
		public ConcurrentDictionary<string, object> GetDependant_Prope17TableAgentName(string PKey)
		{
		...
		/// </summary>
		/// <param name="lazyLoad">Lazy loading of dropdown items</param>
		public void FillDependant_Prope17TableAgentName(bool lazyLoad = false)
		{
			var row = GetDependant_Prope17TableAgentName(this.ValCodagent);
			try
			{
		...
		private readonly string[] _fieldsToSerialize_PROPE17_AGENTNAME____ = ["Agent", "Agent.ValCodagent", "Agent.ValZzstate", "Agent.ValName"];
...
// USE /[MANUAL GQT VIEWMODEL_CUSTOM PROPE17]/
