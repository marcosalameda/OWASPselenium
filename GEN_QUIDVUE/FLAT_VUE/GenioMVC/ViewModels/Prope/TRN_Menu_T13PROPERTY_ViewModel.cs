...
// USE /[MANUAL TRN LIST_LIMITS T13PROPERTY]/
		...
		public TRN_Menu_T13PROPERTY_ViewModel(UserContext userContext) : base(userContext)
		{
		...
		public override CriteriaSet BuildCriteriaSet(CSGenio.core.framework.table.TableConfiguration tableConfig, NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			{
				// EPH
				crs = Models.Prope.AddEPH<CSGenioAprope>(ref u, crs, "MLT13PROPERTY");
			...
			if (tableReload)
			{
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					crs.Equals(Models.Prope.AddEPH<CSGenioAprope>(ref u, null, "MLT13PROPERTY"));
			}
		...
		public void Load(CSGenio.core.framework.table.TableConfiguration tableConfig, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAprope> Qlisting, ref CriteriaSet conditions)
		{
			...
			conditions.SubSets.Add(trn_menu_t13propertyConds);
			trn_menu_t13propertyConds = BuildCriteriaSet(tableConfig, requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
...
// USE /[MANUAL TRN OVERRQ T13PROPERTY]/
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.BuildListingForExport<CSGenioAprope>(m_userContext, false, ref trn_menu_t13propertyConds, exportFieldRefs, (pageNumber - 1) * numberListItems, numberListItems, sorts, "MLT13PROPERTY", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL TRN OVERRQLSTEXP T13PROPERTY]/
			...
			if (tableReload)
			{
// USE /[MANUAL TRN OVERRQLIST T13PROPERTY]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = m_userContext.PersistentSupport.getPagingPos(CSGenioAprope.GetInformation(), QMVC_POS_RECORD, sorts, trn_menu_t13propertyConds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
				...
				Menu.Elements = MapTRN_Menu_T13PROPERTY(listing);

				Menu.Identifier = "MLT13PROPERTY";
				Menu.Slots = new Dictionary<string, List<object>>();

				// Last updated by [CJP] at [2015.02.03]
				// Adds the identifier to each element
				foreach (var element in Menu.Elements)
					element.Identifier = "MLT13PROPERTY";
			...
			if (Qlisting.Rows != null)
			{
				foreach (var row in Qlisting.Rows)
				{
					if (Qlisting.NumRegs > 0 && i >= Qlisting.NumRegs) // Copiado da versão antiga do RowsToViewModels
						break;
					Elements.Add(MapTRN_Menu_T13PROPERTY(row));
					i++;
				}
			}
