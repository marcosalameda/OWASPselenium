...
{
	public class Proprall_ViewModel : FormViewModel<Models.Propr>, IPreparableForSerialization
	{
		[JsonIgnore]
		...
		public override void MapToModel(Models.Propr m)
		{
			...
			{
				CSGenio.framework.Log.Error($"Map ViewModel (Proprall) to Model (Propr) - Error during mapping. All user values: {HasDisabledUserValuesSecurity}");
				throw;
			}
		}
			...
			try
			{
				...
				switch (fullFieldName)
				{
					...
					case "propr.codpropr":
						...
						break;
					default:
						Log.Error($"SetViewModelValue (Proprall) - Unexpected field identifier {fullFieldName}");
						break;
				}
			}
			catch (Exception ex)
			{
				throw new FrameworkException(Resources.Resources.PEDIMOS_DESCULPA__OC63848, "SetViewModelValue (Proprall)", "Unexpected error", ex);
			}
		}
		...
		{
			try { Model = Models.Propr.Find(id ?? Navigation.GetStrValue("propr"), m_userContext, "FPROPRALL"); }
			finally { Model ??= new Models.Propr(m_userContext) { Identifier = "FPROPRALL" }; }
...
// USE /[MANUAL GQT VIEWMODEL_DESTROY PROPRALL]/
		...
		public void Load_Propralltpprotppropri(NameValueCollection qs, bool lazyLoad = false)
		{
			...
			CriteriaSet propralltpprotppropriConds = CriteriaSet.And();
			{
				...
				{
					propralltpprotppropriConds.Equal(CSGenioAtppro.FldCodtppro, hValue);
...
// USE /[MANUAL GQT OVERRQ PROPRALL_TPPROTPPROPRI]/
				...
				*/
				...
				TableTpproTppropri.Elements = listing.RowsForViewModel((r) => new GenioMVC.Models.Tppro(m_userContext, r, true, _fieldsToSerialize_PROPRALLTPPROTPPROPRI));
		...
		public void Load_Proprallcntrycountry_(NameValueCollection qs, bool lazyLoad = false)
		{
			...
			CriteriaSet proprallcntrycountry_Conds = CriteriaSet.And();
			{
				...
				if (hValue != null && !(hValue is Array) && !string.IsNullOrEmpty(Convert.ToString(hValue)))
				{
					proprallcntrycountry_Conds.Equal(CSGenioAcntry.FldCodcntry, hValue);
...
// USE /[MANUAL GQT OVERRQ PROPRALL_CNTRYCOUNTRY]/
				...
				*/
				...
				TableCntryCountry.Elements = listing.RowsForViewModel((r) => new GenioMVC.Models.Cntry(m_userContext, r, true, _fieldsToSerialize_PROPRALLCNTRYCOUNTRY_));
		...
		public void Load_Proprallregioregiao__(NameValueCollection qs, bool lazyLoad = false)
		{
			...
			CriteriaSet proprallregioregiao__Conds = CriteriaSet.And();
			{
				...
				if (hValue != null && !(hValue is Array) && !string.IsNullOrEmpty(Convert.ToString(hValue)))
				{
					proprallregioregiao__Conds.Equal(CSGenioAregio.FldCodregia, hValue);
...
// USE /[MANUAL GQT OVERRQ PROPRALL_REGIOREGIAO]/
				...
				*/
				...
				TableRegioRegiao.Elements = listing.RowsForViewModel((r) => new GenioMVC.Models.Regio(m_userContext, r, true, _fieldsToSerialize_PROPRALLREGIOREGIAO__));
		...
		public void Load_Proprallpessoname____(NameValueCollection qs, bool lazyLoad = false)
		{
			...
			CriteriaSet proprallpessoname____Conds = CriteriaSet.And();
			{
				...
				if (hValue != null && !(hValue is Array) && !string.IsNullOrEmpty(Convert.ToString(hValue)))
				{
					proprallpessoname____Conds.Equal(CSGenioApesso.FldCodpesso, hValue);
...
// USE /[MANUAL GQT OVERRQ PROPRALL_PESSONAME]/
				...
				*/
				...
				TablePessoName.Elements = listing.RowsForViewModel((r) => new GenioMVC.Models.Pesso(m_userContext, r, true, _fieldsToSerialize_PROPRALLPESSONAME____));
