...
{
	public class Pworg_ViewModel : FormViewModel<Models.Pworg>, IPreparableForSerialization
	{
		[JsonIgnore]
		...
		public override void MapToModel(Models.Pworg m)
		{
			...
			{
				CSGenio.framework.Log.Error($"Map ViewModel (Pworg) to Model (Pworg) - Error during mapping. All user values: {HasDisabledUserValuesSecurity}");
				throw;
			}
		}
			...
			try
			{
				...
				switch (fullFieldName)
				{
					...
					case "pworg.codpsw":
						...
						break;
					case "pworg.codpworg":
						this.ValCodpworg = ViewModelConversion.ToString(_value);
						break;
					default:
						Log.Error($"SetViewModelValue (Pworg) - Unexpected field identifier {fullFieldName}");
						break;
				}
			}
			catch (Exception ex)
			{
				throw new FrameworkException(Resources.Resources.PEDIMOS_DESCULPA__OC63848, "SetViewModelValue (Pworg)", "Unexpected error", ex);
			}
		}
		...
		{
			try { Model = Models.Pworg.Find(id ?? Navigation.GetStrValue("pworg"), m_userContext, "FPWORG"); }
			finally { Model ??= new Models.Pworg(m_userContext) { Identifier = "FPWORG" }; }
...
// USE /[MANUAL GQT VIEWMODEL_DESTROY PWORG]/
		...
		public void Load_Pworg___psw__nome____(NameValueCollection qs, bool lazyLoad = false)
		{
			...
			CriteriaSet pworg___psw__nome____Conds = CriteriaSet.And();
			{
				...
				{
					pworg___psw__nome____Conds.Equal(CSGenioApsw.FldCodpsw, hValue);
...
// USE /[MANUAL GQT OVERRQ PWORG_PSWNOME]/
				...
				*/
				...
				TablePswNome.Elements = listing.RowsForViewModel((r) => new GenioMVC.Models.Psw(m_userContext, r, true, _fieldsToSerialize_PWORG___PSW__NOME____));
		...
		public void Load_Pworg___organorganiza(NameValueCollection qs, bool lazyLoad = false)
		{
			...
			CriteriaSet pworg___organorganizaConds = CriteriaSet.And();
			{
				...
				if (hValue != null && !(hValue is Array) && !string.IsNullOrEmpty(Convert.ToString(hValue)))
				{
					pworg___organorganizaConds.Equal(CSGenioAorgan.FldCodorgan, hValue);
...
// USE /[MANUAL GQT OVERRQ PWORG_ORGANORGANIZA]/
				...
				*/
				...
				TableOrganOrganiza.Elements = listing.RowsForViewModel((r) => new GenioMVC.Models.Organ(m_userContext, r, true, _fieldsToSerialize_PWORG___ORGANORGANIZA));
