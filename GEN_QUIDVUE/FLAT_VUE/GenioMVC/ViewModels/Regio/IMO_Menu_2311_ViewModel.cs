...
{
	public class IMO_Menu_2311_ViewModel : ListViewModel
	{
		/// <summary>
		...
		/// <inheritdoc/>
		public override string Uuid { get => "bf308678-026e-4364-bff4-f47500c90029"; }
		...
		{
			dbeditTitle = Resources.Resources.REGIONS31874;
		}
...
FieldRef[] fields = new FieldRef[] { CSGenioAregio.FldCodregia, CSGenioAregio.FldZzstate, CSGenioAregio.FldCodcntry, CSGenioAcntry.FldCodcntry, CSGenioAcntry.FldCountry, CSGenioAregio.FldRegiao };
			...
			if (!qs.Joins.Select(x => x.Table).Select(y=>y.TableAlias).Contains(CSGenio.business.Area.AreaCNTRY.Alias))
				qs.Join(CSGenio.business.Area.AreaCNTRY, TableJoinType.Inner).On(CriteriaSet.And().Equal(CSGenioAcntry.FldCodcntry, CSGenioAregio.FldCodcntry));
		...
		/// <summary>
		/// Initializes a new instance of the <see cref="IMO_Menu_2311_ViewModel" /> class.
		/// </summary>
		/// <param name="userContext">The current user request context</param>
		public IMO_Menu_2311_ViewModel(UserContext userContext) : base(userContext)
		{
		...
		public CriteriaSet BuildCriteriaSet(NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			Menu.SetFilters(bool.Parse(requestValues["IMO_Menu_2311_tableFilters"] ?? "false"), false);
			...
			LoadUserTableConfig(requestValues, allSortOrders, "IMO_Menu_2311", ref numberListItems);

			crs.SubSets.Add(ProcessSearchFilters(Menu, GetSearchColumns(true), requestValues, "IMO_Menu_2311_"));
			...
			// Limitations
			// Limit "DB"
			crs.Equal(CSGenioAregio.FldCodcntry, Navigation.GetValue("cntry"));

			if (isToExport)
			{
				// EPH
				crs = Models.Regio.AddEPH<CSGenioAregio>(ref u, crs, "ML2311");
			...
			if (tableReload)
			{
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					crs.Equals(Models.Regio.AddEPH<CSGenioAregio>(ref u, null, "ML2311"));
			}
		...
		public void Load(int numberListItems, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAregio> Qlisting, ref CriteriaSet conditions)
		{
			...
			if (ajaxRequest)
				this.Navigation.SetValue("requestValues" + "IMO_Menu_2311", requestValues);
			else if (!ajaxRequest && this.Navigation.CheckKey("requestValues" + "IMO_Menu_2311"))
				requestValues = this.Navigation.GetValue<NameValueCollection>("requestValues" + "IMO_Menu_2311");

			User u = m_userContext.User;
			Menu = new TablePartial<GenioMVC.Models.Regio>();

			CriteriaSet imo_menu_2311Conds = CriteriaSet.And();
			...
			var pageNumber = (ajaxRequest && !String.IsNullOrEmpty(requestValues["pIMO_Menu_2311"])) ? int.Parse(requestValues["pIMO_Menu_2311"]) : 1;

			// Added to avoid 0 or -1 pages when setting number of records to -1 to disable pagination
			if (pageNumber < 1)
				pageNumber = 1;

			List<ColumnSort> sorts = GetRequestSorts(this.Menu, "sIMO_Menu_2311", "dIMO_Menu_2311", requestValues, "regio", allSortOrders);
...
FieldRef[] fields = new FieldRef[] { CSGenioAregio.FldCodregia, CSGenioAregio.FldZzstate, CSGenioAregio.FldCodcntry, CSGenioAcntry.FldCodcntry, CSGenioAcntry.FldCountry, CSGenioAregio.FldRegiao };
			...
			//Limit type: "DB"			//Current Area = "REGIO"			//1st Area Limit: "CNTRY"			//1st Area Field: "CODCNTRY"			//1st Area Value: ""
			{
				Limit limit = new Limit();
			...
			conditions.SubSets.Add(imo_menu_2311Conds);
			imo_menu_2311Conds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
...
// USE /[MANUAL IMO OVERRQ 2311]/
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAregio>(m_userContext, false, imo_menu_2311Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "ML2311", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL IMO OVERRQLSTEXP 2311]/
			...
			if (tableReload)
			{
// USE /[MANUAL IMO OVERRQLIST 2311]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = m_userContext.PersistentSupport.getPagingPos(CSGenioAregio.GetInformation(), QMVC_POS_RECORD, sorts, imo_menu_2311Conds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioAregio> listing = Models.ModelBase.Where<CSGenioAregio>(m_userContext, false, imo_menu_2311Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "ML2311", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
				...
				Menu.Elements = MapIMO_Menu_2311(listing);
			...
			if (Qlisting.Rows != null)
			{
				foreach (var row in Qlisting.Rows)
				{
					if (Qlisting.NumRegs > 0 && i >= Qlisting.NumRegs) // Copiado da versão antiga do RowsToViewModels
						break;
					Elements.Add(MapIMO_Menu_2311(row));
					i++;
				}
			}
		...
		private Models.Regio MapIMO_Menu_2311(CSGenioAregio row)
		{
			var model = new Models.Regio(m_userContext, true, _fieldsToSerialize);
		...
		#region Custom code
// USE /[MANUAL GQT VIEWMODEL_CUSTOM IMO_MENU_2311]/
		#endregion

		private static readonly string[] _fieldsToSerialize =
		{
			"Regio", "Regio.ValCodregia", "Regio.ValZzstate", "Cntry", "Cntry.ValCountry", "Regio.ValRegiao", "Regio.ValCodcntry", "Regio.ValCodpais1"
		};
