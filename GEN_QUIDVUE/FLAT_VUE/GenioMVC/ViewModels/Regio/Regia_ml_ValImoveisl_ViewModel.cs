...
{
	public class Regia_ml_ValImoveisl_ViewModel : MenuListViewModel<Models.Propr>
	{
		/// <summary>
		/// Gets or sets the object that represents the table and its elements.
		/// </summary>
		[JsonPropertyName("table")]
		public TablePartial<Regia_ml_ValImoveisl_RowViewModel> Menu { get; set; }
		...
		public override string Uuid => "Regia_ml_ValImoveisl";
		...
		{
// USE /[MANUAL GQT LIST_LIMITS REGIA_ML_PSEUDIMOVEISL]/
		...
		public Regia_ml_ValImoveisl_ViewModel() : base(null!) { }

		/// <summary>
		/// Initializes a new instance of the <see cref="Regia_ml_ValImoveisl_ViewModel" /> class.
		/// </summary>
		/// <param name="userContext">The current user request context</param>
		public Regia_ml_ValImoveisl_ViewModel(UserContext userContext) : base(userContext)
		{
		...
		public Regia_ml_ValImoveisl_ViewModel(UserContext userContext, Models.ModelBase parentCtx) : this(userContext)
		{
			ParentCtx = parentCtx;
		}
		...
		public override CriteriaSet BuildCriteriaSet(CSGenio.core.framework.table.TableConfiguration tableConfig, NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			Menu ??= new TablePartial<Regia_ml_ValImoveisl_RowViewModel>();
			...
			{
				// EPH
				crs = Models.Propr.AddEPH<CSGenioApropr>(ref u, crs, "IBL_REGIA_MLPSEUDIMOVEISL");
			...
			if (tableReload)
			{
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					crs.Equals(Models.Propr.AddEPH<CSGenioApropr>(ref u, null, "IBL_REGIA_MLPSEUDIMOVEISL"));
			}
		...
		public void Load(CSGenio.core.framework.table.TableConfiguration tableConfig, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioApropr> Qlisting, ref CriteriaSet conditions)
		{
			...
			CriteriaSet regia_mlpseudimoveislConds = CriteriaSet.And();
			...
			//Tooltip for EPHs affecting this viewmodel list
			{
				...
				List<Limit> area_EPH_limits = EPH_Limit_Filler(ref limit, model_limit_area, "IBL_REGIA_MLPSEUDIMOVEISL");
			...
			conditions.SubSets.Add(regia_mlpseudimoveislConds);
			regia_mlpseudimoveislConds = BuildCriteriaSet(tableConfig, requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
			tableReload &= hasAllRequiredLimits;

// USE /[MANUAL GQT OVERRQ REGIA_ML_PSEUDIMOVEISL]/
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.BuildListingForExport<CSGenioApropr>(m_userContext, false, ref regia_mlpseudimoveislConds, exportFieldRefs, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_REGIA_MLPSEUDIMOVEISL", true, firstVisibleColumn: firstVisibleColumn);
...
// USE /[MANUAL GQT OVERRQLSTEXP REGIA_ML_PSEUDIMOVEISL]/
			...
			if (tableReload)
			{
// USE /[MANUAL GQT OVERRQLIST REGIA_ML_PSEUDIMOVEISL]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = m_userContext.PersistentSupport.getPagingPos(CSGenioApropr.GetInformation(), QMVC_POS_RECORD, sorts, regia_mlpseudimoveislConds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
						pageNumber = ((m_iCurPag - 1) / numberListItems) + 1;
				}

				ListingMVC<CSGenioApropr> listing = Models.ModelBase.Where<CSGenioApropr>(m_userContext, distinct, regia_mlpseudimoveislConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_REGIA_MLPSEUDIMOVEISL", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn, fieldsWithTotalizers, tableConfig.SelectedRows);
				...
				Menu.Elements = MapRegia_ml_ValImoveisl(listing);

				Menu.Identifier = "IBL_REGIA_MLPSEUDIMOVEISL";

				// Last updated by [CJP] at [2015.02.03]
				// Adds the identifier to each element
				foreach (var element in Menu.Elements)
					element.Identifier = "IBL_REGIA_MLPSEUDIMOVEISL";
		...
		private List<Regia_ml_ValImoveisl_RowViewModel> MapRegia_ml_ValImoveisl(ListingMVC<CSGenioApropr> Qlisting)
		{
			List<Regia_ml_ValImoveisl_RowViewModel> Elements = [];
			int i = 0;
			...
			if (Qlisting.Rows != null)
			{
				foreach (var row in Qlisting.Rows)
				{
					if (Qlisting.NumRegs > 0 && i >= Qlisting.NumRegs) // Copiado da versão antiga do RowsToViewModels
						break;
					Elements.Add(MapRegia_ml_ValImoveisl(row));
					i++;
				}
			}
		...
		/// to a Regia_ml_ValImoveisl_RowViewModel object.
		/// </summary>
		/// <param name="row">The row.</param>
		private Regia_ml_ValImoveisl_RowViewModel MapRegia_ml_ValImoveisl(CSGenioApropr row)
		{
			var model = new Regia_ml_ValImoveisl_RowViewModel(m_userContext, true, _fieldsToSerialize);
			if (row == null)
				return model;
...
// USE /[MANUAL GQT VIEWMODEL_CUSTOM REGIA_ML_VALIMOVEISL]/
