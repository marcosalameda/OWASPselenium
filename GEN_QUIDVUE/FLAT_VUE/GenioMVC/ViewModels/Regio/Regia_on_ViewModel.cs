...
{
	public class Regia_on_ViewModel : FormViewModel<Models.Regio>, IPreparableForSerialization
	{
		[JsonIgnore]
		...
		public Regia_on_ViewModel() : base(null!) { }

		public Regia_on_ViewModel(UserContext userContext, bool nestedForm = false) : base(userContext, "FREGIA_ON", nestedForm) { }

		public Regia_on_ViewModel(UserContext userContext, Models.Regio row, bool nestedForm = false) : base(userContext, "FREGIA_ON", row, nestedForm) { }

		public Regia_on_ViewModel(UserContext userContext, string id, bool nestedForm = false, string[]? fieldsToLoad = null) : this(userContext, nestedForm)
		{
			this.Navigation.SetValue("regio", id);
			Model = Models.Regio.Find(id, userContext, "FREGIA_ON", fieldsToQuery: fieldsToLoad);
			if (Model == null)
		...
		public static StatusMessage InsertConditions(UserContext userContext)
		{
			...
			Models.Regio model = new Models.Regio(userContext) { Identifier = "FREGIA_ON" };

			var navigation = m_userContext.CurrentNavigation;
			// The "LoadKeysFromHistory" must be after the "LoadEPH" because the PHE's in the tree mark Foreign Keys to null
			// (since they cannot assign multiple values to a single field) and thus the value that comes from Navigation is lost.
			// And this makes it more like the order of loading the model when opening the form.
			model.LoadEPH("FREGIA_ON");
			if (navigation != null)
		...
		public override void MapToModel(Models.Regio m)
		{
			...
			{
				CSGenio.framework.Log.Error($"Map ViewModel (Regia_on) to Model (Regio) - Error during mapping. All user values: {HasDisabledUserValuesSecurity}");
				throw;
			}
		}
			...
			try
			{
				...
				switch (fullFieldName)
				{
					...
					case "regio.codregia":
						...
						break;
					default:
						Log.Error($"SetViewModelValue (Regia_on) - Unexpected field identifier {fullFieldName}");
						break;
				}
			}
			catch (Exception ex)
			{
				throw new FrameworkException(Resources.Resources.PEDIMOS_DESCULPA__OC63848, "SetViewModelValue (Regia_on)", "Unexpected error", ex);
			}
		}
		...
		{
			try { Model = Models.Regio.Find(id ?? Navigation.GetStrValue("regio"), m_userContext, "FREGIA_ON"); }
			finally { Model ??= new Models.Regio(m_userContext) { Identifier = "FREGIA_ON" }; }
		...
		public override void Load(NameValueCollection qs, bool editable, bool ajaxRequest = false, bool lazyLoad = false)
		{
			...
			try
			{
				Model = Models.Regio.Find(Navigation.GetStrValue("regio"), m_userContext, "FREGIA_ON");
			}
			finally
			{
				if (Model == null)
		...
		public override void LoadPartial(NameValueCollection qs, bool lazyLoad = false)
		{
			...
			if (Model == null)
			{
				...
				if (Model == null)
				{
					Model = new Models.Regio(m_userContext) { Identifier = "FREGIA_ON" };
...
// USE /[MANUAL GQT VIEWMODEL_DESTROY REGIA_ON]/
		...
		{
			Model = Models.Regio.Find(id, m_userContext, "FREGIA_ON");
			if (Model == null)
				throw new ModelNotFoundException("Model not found");
		...
		public void Load_Regia_oncntrycountry_(NameValueCollection qs, bool lazyLoad = false)
		{
			...
			CriteriaSet regia_oncntrycountry_Conds = CriteriaSet.And();
			{
				...
				{
					regia_oncntrycountry_Conds.Equal(CSGenioAcntry.FldCodcntry, hValue);
...
// USE /[MANUAL GQT OVERRQ REGIA_ON_CNTRYCOUNTRY]/
				...
				*/
				...
				ListingMVC<CSGenioAcntry> listing = Models.ModelBase.Where<CSGenioAcntry>(m_userContext, false, regia_oncntrycountry_Conds, fields, offset, numberItems, sorts, "LED_REGIA_ONCNTRYCOUNTRY_", true, false, firstVisibleColumn: firstVisibleColumn);
				...
				TableCntryCountry.Elements = listing.RowsForViewModel<GenioMVC.Models.Cntry>((r) => new GenioMVC.Models.Cntry(m_userContext, r, true, _fieldsToSerialize_REGIA_ONCNTRYCOUNTRY_));
		...
		public ConcurrentDictionary<string, object> GetDependant_Regia_onTableCntryCountry(string PKey)
		{
		...
		{
			var row = GetDependant_Regia_onTableCntryCountry(this.ValCodcntry);
			try
			{
		...
		public void Load_Regia_onpais1country_(NameValueCollection qs, bool lazyLoad = false)
		{
			...
			CriteriaSet regia_onpais1country_Conds = CriteriaSet.And();
			{
				...
				if (hValue != null && !(hValue is Array) && !string.IsNullOrEmpty(Convert.ToString(hValue)))
				{
					regia_onpais1country_Conds.Equal(CSGenioApais1.FldCodcntry, hValue);
...
// USE /[MANUAL GQT OVERRQ REGIA_ON_PAIS1COUNTRY]/
				...
				*/
				...
				ListingMVC<CSGenioApais1> listing = Models.ModelBase.Where<CSGenioApais1>(m_userContext, false, regia_onpais1country_Conds, fields, offset, numberItems, sorts, "LED_REGIA_ONPAIS1COUNTRY_", true, false, firstVisibleColumn: firstVisibleColumn);
				...
				TablePais1Country.Elements = listing.RowsForViewModel<GenioMVC.Models.Pais1>((r) => new GenioMVC.Models.Pais1(m_userContext, r, true, _fieldsToSerialize_REGIA_ONPAIS1COUNTRY_));
		...
		public ConcurrentDictionary<string, object> GetDependant_Regia_onTablePais1Country(string PKey)
		{
		...
		{
			var row = GetDependant_Regia_onTablePais1Country(this.ValCodpais1);
			try
			{
