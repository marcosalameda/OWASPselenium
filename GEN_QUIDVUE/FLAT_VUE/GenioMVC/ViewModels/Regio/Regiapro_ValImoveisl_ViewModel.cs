...
{
	public class Regiapro_ValImoveisl_ViewModel : MenuListViewModel<Models.Propr>
	{
		/// <summary>
		/// Gets or sets the object that represents the table and its elements.
		/// </summary>
		[JsonPropertyName("table")]
		public TablePartial<Regiapro_ValImoveisl_RowViewModel> Menu { get; set; }
		...
		public override string Uuid => "Regiapro_ValImoveisl";
		...
		{
// USE /[MANUAL GQT LIST_LIMITS REGIAPRO_PSEUDIMOVEISL]/
		...
		public Regiapro_ValImoveisl_ViewModel() : base(null!) { }

		/// <summary>
		/// Initializes a new instance of the <see cref="Regiapro_ValImoveisl_ViewModel" /> class.
		/// </summary>
		/// <param name="userContext">The current user request context</param>
		public Regiapro_ValImoveisl_ViewModel(UserContext userContext) : base(userContext)
		{
		...
		public Regiapro_ValImoveisl_ViewModel(UserContext userContext, Models.ModelBase parentCtx) : this(userContext)
		{
			ParentCtx = parentCtx;
		}
		...
		public void LoadToExport(out ListingMVC<CSGenioApropr> listing, out CriteriaSet conditions, out List<Exports.QColumn> columns, CSGenio.core.framework.table.TableConfiguration tableConfig, NameValueCollection requestValues, bool ajaxRequest = false)
		{
			listing = null;
			conditions = null;
		...
		public override CriteriaSet BuildCriteriaSet(CSGenio.core.framework.table.TableConfiguration tableConfig, NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			Menu ??= new TablePartial<Regiapro_ValImoveisl_RowViewModel>();
			...
			{
				// EPH
				crs = Models.Propr.AddEPH<CSGenioApropr>(ref u, crs, "IBL_REGIAPROPSEUDIMOVEISL");
			...
			if (tableReload)
			{
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					crs.Equals(Models.Propr.AddEPH<CSGenioApropr>(ref u, null, "IBL_REGIAPROPSEUDIMOVEISL"));
			}
		...
		/// <param name="Qlisting">The rows.</param>
		/// <param name="conditions">The conditions.</param>
		public void Load(CSGenio.core.framework.table.TableConfiguration tableConfig, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioApropr> Qlisting, ref CriteriaSet conditions)
		{
			User u = m_userContext.User;
			Menu = new TablePartial<Regiapro_ValImoveisl_RowViewModel>();

			CriteriaSet regiapropseudimoveislConds = CriteriaSet.And();
			...
			List<ColumnSort> sorts = GetRequestSorts(this.Menu, tableConfig, "propr", allSortOrders);
			...
			//Tooltip for EPHs affecting this viewmodel list
			{
				...
				List<Limit> area_EPH_limits = EPH_Limit_Filler(ref limit, model_limit_area, "IBL_REGIAPROPSEUDIMOVEISL");
			...
			conditions.SubSets.Add(regiapropseudimoveislConds);
			regiapropseudimoveislConds = BuildCriteriaSet(tableConfig, requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
			tableReload &= hasAllRequiredLimits;

// USE /[MANUAL GQT OVERRQ REGIAPRO_PSEUDIMOVEISL]/
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.BuildListingForExport<CSGenioApropr>(m_userContext, false, ref regiapropseudimoveislConds, exportFieldRefs, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_REGIAPROPSEUDIMOVEISL", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL GQT OVERRQLSTEXP REGIAPRO_PSEUDIMOVEISL]/
			...
			if (tableReload)
			{
// USE /[MANUAL GQT OVERRQLIST REGIAPRO_PSEUDIMOVEISL]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = m_userContext.PersistentSupport.getPagingPos(CSGenioApropr.GetInformation(), QMVC_POS_RECORD, sorts, regiapropseudimoveislConds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
						pageNumber = ((m_iCurPag - 1) / numberListItems) + 1;
				}

				ListingMVC<CSGenioApropr> listing = Models.ModelBase.Where<CSGenioApropr>(m_userContext, distinct, regiapropseudimoveislConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_REGIAPROPSEUDIMOVEISL", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn, fieldsWithTotalizers, tableConfig.SelectedRows);
				...
				Menu.Elements = MapRegiapro_ValImoveisl(listing);

				Menu.Identifier = "IBL_REGIAPROPSEUDIMOVEISL";

				// Last updated by [CJP] at [2015.02.03]
				// Adds the identifier to each element
				foreach (var element in Menu.Elements)
					element.Identifier = "IBL_REGIAPROPSEUDIMOVEISL";
		...
		private List<Regiapro_ValImoveisl_RowViewModel> MapRegiapro_ValImoveisl(ListingMVC<CSGenioApropr> Qlisting)
		{
			List<Regiapro_ValImoveisl_RowViewModel> Elements = [];
			int i = 0;
			...
			if (Qlisting.Rows != null)
			{
				foreach (var row in Qlisting.Rows)
				{
					if (Qlisting.NumRegs > 0 && i >= Qlisting.NumRegs) // Copiado da versão antiga do RowsToViewModels
						break;
					Elements.Add(MapRegiapro_ValImoveisl(row));
					i++;
				}
			}
		...
		/// to a Regiapro_ValImoveisl_RowViewModel object.
		/// </summary>
		/// <param name="row">The row.</param>
		private Regiapro_ValImoveisl_RowViewModel MapRegiapro_ValImoveisl(CSGenioApropr row)
		{
			var model = new Regiapro_ValImoveisl_RowViewModel(m_userContext, true, _fieldsToSerialize);
			if (row == null)
				return model;
...
// USE /[MANUAL GQT VIEWMODEL_CUSTOM REGIAPRO_VALIMOVEISL]/
