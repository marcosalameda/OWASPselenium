...
namespace GenioMVC.ViewModels.Regio
{
	public class Regiapro_ValImoveisl_ViewModel : ListViewModel
	{
		...
		public CriteriaSet BuildCriteriaSet(NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			LoadUserTableConfig(requestValues, allSortOrders, "ValImoveisl", ref numberListItems);
		...
		public void Load(int numberListItems, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioApropr> Qlisting, ref CriteriaSet conditions)
		{
			...
			if (ajaxRequest)
				this.Navigation.SetValue("requestValues" + "Regiapro_ValImoveisl", requestValues);
			else if (!ajaxRequest && this.Navigation.CheckKey("requestValues" + "Regiapro_ValImoveisl"))
				requestValues = this.Navigation.GetValue<NameValueCollection>("requestValues" + "Regiapro_ValImoveisl");
			...
			var pageNumber = (ajaxRequest && !String.IsNullOrEmpty(requestValues["pValImoveisl"])) ? int.Parse(requestValues["pValImoveisl"]) : 1;

			// Added to avoid 0 or -1 pages when setting number of records to -1 to disable pagination
			if (pageNumber < 1)
				pageNumber = 1;

			List<ColumnSort> sorts = GetRequestSorts(this.Menu, "sValImoveisl", "dValImoveisl", requestValues, "propr", allSortOrders);
...
FieldRef[] fields = new FieldRef[] { CSGenioApropr.FldCodpropr, CSGenioApropr.FldZzstate, CSGenioApropr.FldName, CSGenioApropr.FldPrecoest, CSGenioApropr.FldPhotogra, CSGenioApropr.FldDescript, CSGenioApropr.FldCoordgeo, CSGenioApropr.FldCodpais1, CSGenioApais1.FldCodcntry, CSGenioApais1.FldCountry, CSGenioApropr.FldCodcntry, CSGenioAcntry.FldCodcntry, CSGenioAcntry.FldCountry };
			...
			regiapropseudimoveislConds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
...
// USE /[MANUAL GQT OVERRQLIST REGIAPRO_PSEUDIMOVEISL]/
				...
				ListingMVC<CSGenioApropr> listing = Models.ModelBase.Where<CSGenioApropr>(m_userContext, false, regiapropseudimoveislConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_REGIAPROPSEUDIMOVEISL", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
		...
		private List<Models.Propr> MapRegiapro_ValImoveisl(ListingMVC<CSGenioApropr> Qlisting)
		{
		...
		private Models.Propr MapRegiapro_ValImoveisl(CSGenioApropr row)
		{
