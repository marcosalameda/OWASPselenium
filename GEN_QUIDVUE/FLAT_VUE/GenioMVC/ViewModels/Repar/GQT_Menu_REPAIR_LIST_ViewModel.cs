...
{
	public class GQT_Menu_REPAIR_LIST_ViewModel : MenuListViewModel<Models.Repar>
	{
		/// <summary>
		/// Gets or sets the object that represents the table and its elements.
		/// </summary>
		[JsonPropertyName("table")]
		public TablePartial<GQT_Menu_REPAIR_LIST_RowViewModel> Menu { get; set; }
		...
		{
// USE /[MANUAL GQT LIST_LIMITS REPAIR_LIST]/
		...
		public override int GetCount(User user)
		{
			...
			CriteriaSet conditions = CSGenio.business.Listing.CalculateConditionsEphGeneric(areaBase, "MLREPAIR_LIST");
		...
		public GQT_Menu_REPAIR_LIST_ViewModel() : base(null!) { }

		/// <summary>
		/// Initializes a new instance of the <see cref="GQT_Menu_REPAIR_LIST_ViewModel" /> class.
		/// </summary>
		/// <param name="userContext">The current user request context</param>
		public GQT_Menu_REPAIR_LIST_ViewModel(UserContext userContext) : base(userContext)
		{
		...
		public GQT_Menu_REPAIR_LIST_ViewModel(UserContext userContext, Models.ModelBase parentCtx) : this(userContext)
		{
			ParentCtx = parentCtx;
		}
		...
		public override CriteriaSet BuildCriteriaSet(CSGenio.core.framework.table.TableConfiguration tableConfig, NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			Menu ??= new TablePartial<GQT_Menu_REPAIR_LIST_RowViewModel>();
			...
			{
				// EPH
				crs = Models.Repar.AddEPH<CSGenioArepar>(ref u, crs, "MLREPAIR_LIST");
			...
			if (tableReload)
			{
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					crs.Equals(Models.Repar.AddEPH<CSGenioArepar>(ref u, null, "MLREPAIR_LIST"));
			}
		...
		public void Load(CSGenio.core.framework.table.TableConfiguration tableConfig, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioArepar> Qlisting, ref CriteriaSet conditions)
		{
			...
			CriteriaSet gqt_menu_repair_listConds = CriteriaSet.And();
			...
			//Tooltip for EPHs affecting this viewmodel list
			{
				...
				List<Limit> area_EPH_limits = EPH_Limit_Filler(ref limit, model_limit_area, "MLREPAIR_LIST");
			...
			conditions.SubSets.Add(gqt_menu_repair_listConds);
			gqt_menu_repair_listConds = BuildCriteriaSet(tableConfig, requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
			tableReload &= hasAllRequiredLimits;

// USE /[MANUAL GQT OVERRQ REPAIR_LIST]/
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.BuildListingForExport<CSGenioArepar>(m_userContext, false, ref gqt_menu_repair_listConds, exportFieldRefs, (pageNumber - 1) * numberListItems, numberListItems, sorts, "MLREPAIR_LIST", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL GQT OVERRQLSTEXP REPAIR_LIST]/
			...
			if (tableReload)
			{
// USE /[MANUAL GQT OVERRQLIST REPAIR_LIST]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = m_userContext.PersistentSupport.getPagingPos(CSGenioArepar.GetInformation(), QMVC_POS_RECORD, sorts, gqt_menu_repair_listConds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
						pageNumber = ((m_iCurPag - 1) / numberListItems) + 1;
				}

				ListingMVC<CSGenioArepar> listing = Models.ModelBase.Where<CSGenioArepar>(m_userContext, distinct, gqt_menu_repair_listConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "MLREPAIR_LIST", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn, fieldsWithTotalizers, tableConfig.SelectedRows);
				...
				Menu.Elements = MapGQT_Menu_REPAIR_LIST(listing);

				Menu.Identifier = "MLREPAIR_LIST";
				Menu.Slots = new Dictionary<string, List<object>>();

				// Last updated by [CJP] at [2015.02.03]
				// Adds the identifier to each element
				foreach (var element in Menu.Elements)
					element.Identifier = "MLREPAIR_LIST";
		...
		private List<GQT_Menu_REPAIR_LIST_RowViewModel> MapGQT_Menu_REPAIR_LIST(ListingMVC<CSGenioArepar> Qlisting)
		{
			List<GQT_Menu_REPAIR_LIST_RowViewModel> Elements = [];
			int i = 0;
			...
			if (Qlisting.Rows != null)
			{
				foreach (var row in Qlisting.Rows)
				{
					if (Qlisting.NumRegs > 0 && i >= Qlisting.NumRegs) // Copiado da versão antiga do RowsToViewModels
						break;
					Elements.Add(MapGQT_Menu_REPAIR_LIST(row));
					i++;
				}
			}
		...
		/// to a GQT_Menu_REPAIR_LIST_RowViewModel object.
		/// </summary>
		/// <param name="row">The row.</param>
		private GQT_Menu_REPAIR_LIST_RowViewModel MapGQT_Menu_REPAIR_LIST(CSGenioArepar row)
		{
			var model = new GQT_Menu_REPAIR_LIST_RowViewModel(m_userContext, true, _fieldsToSerialize);
			if (row == null)
				return model;
...
// USE /[MANUAL GQT VIEWMODEL_CUSTOM GQT_MENU_REPAIR_LIST]/
