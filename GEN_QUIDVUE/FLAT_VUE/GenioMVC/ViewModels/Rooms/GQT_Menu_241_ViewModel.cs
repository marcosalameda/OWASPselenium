...
FieldRef[] fields = new FieldRef[] { CSGenioArooms.FldCodrooms, CSGenioArooms.FldZzstate, CSGenioArooms.FldRoomnr, CSGenioArooms.FldDesignat };
		...
		public CriteriaSet BuildCriteriaSet(NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			Menu.SetFilters(bool.Parse(requestValues["GQT_Menu_241_tableFilters"] ?? "false"), false);
			...
			LoadUserTableConfig(requestValues, allSortOrders, "GQT_Menu_241", ref numberListItems);

			crs.SubSets.Add(ProcessSearchFilters(Menu, GetSearchColumns(true), requestValues, "GQT_Menu_241_"));
		...
		public void Load(int numberListItems, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioArooms> Qlisting, ref CriteriaSet conditions)
		{
			...
			if (ajaxRequest)
				this.Navigation.SetValue("requestValues" + "GQT_Menu_241", requestValues);
			else if (!ajaxRequest && this.Navigation.CheckKey("requestValues" + "GQT_Menu_241"))
				requestValues = this.Navigation.GetValue<NameValueCollection>("requestValues" + "GQT_Menu_241");
			...
			var pageNumber = (ajaxRequest && !String.IsNullOrEmpty(requestValues["pGQT_Menu_241"])) ? int.Parse(requestValues["pGQT_Menu_241"]) : 1;

			// Added to avoid 0 or -1 pages when setting number of records to -1 to disable pagination
			if (pageNumber < 1)
				pageNumber = 1;

			List<ColumnSort> sorts = GetRequestSorts(this.Menu, "sGQT_Menu_241", "dGQT_Menu_241", requestValues, "rooms", allSortOrders);
...
FieldRef[] fields = new FieldRef[] { CSGenioArooms.FldCodrooms, CSGenioArooms.FldZzstate, CSGenioArooms.FldRoomnr, CSGenioArooms.FldDesignat };
			...
			gqt_menu_241Conds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
...
// USE /[MANUAL GQT OVERRQLIST 241]/
				...
				ListingMVC<CSGenioArooms> listing = Models.ModelBase.Where<CSGenioArooms>(m_userContext, false, gqt_menu_241Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "ML241", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
		...
		private Models.Rooms MapGQT_Menu_241(CSGenioArooms row)
		{
