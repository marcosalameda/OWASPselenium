...
{
	public class PTN_Menu_1511_ViewModel : ListViewModel
	{
		/// <summary>
		/// Gets or sets the object that represents the table and its elements.
		/// </summary>
		[JsonPropertyName("Table")]
		public TablePartial<GenioMVC.Models.Rordf> Menu { get; set; }
		...
		{
			dbeditTitle = Resources.Resources.ORDERS___FLOAT_FIELD11291;
		}
...
FieldRef[] fields = new FieldRef[] { CSGenioArordf.FldCodrordf, CSGenioArordf.FldZzstate, CSGenioArordf.FldOrder, CSGenioArordf.FldTitle };
		...
		/// <summary>
		/// Initializes a new instance of the <see cref="PTN_Menu_1511_ViewModel" /> class.
		/// </summary>
		/// <param name="userContext">The current user request context</param>
		public PTN_Menu_1511_ViewModel(UserContext userContext) : base(userContext)
		{
		...
		public void LoadToExport(out ListingMVC<CSGenioArordf> listing, out CriteriaSet conditions, out List<Exports.QColumn> columns, NameValueCollection requestValues, bool ajaxRequest = false)
		{
			...
			if (ajaxRequest && userColumns!=null)
			{
				...
				foreach (CSGenioAlstcol column in userColumns)
				{
					...
					string areabase = column.ValTabela.ToLower() != "rordf" ? CultureInfo.InvariantCulture.TextInfo.ToTitleCase(column.ValTabela) + "." : "";
		...
		public CriteriaSet BuildCriteriaSet(NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			if (Menu == null)
				Menu = new TablePartial<GenioMVC.Models.Rordf>();
			Menu.SetFilters(bool.Parse(requestValues["PTN_Menu_1511_tableFilters"] ?? "false"), false);
			...
			LoadUserTableConfig(requestValues, allSortOrders, "PTN_Menu_1511", ref numberListItems);
			...
			{
				// EPH
				crs = Models.Rordf.AddEPH<CSGenioArordf>(ref u, crs, "ML1511");
			...
			if (tableReload)
			{
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					crs.Equals(Models.Rordf.AddEPH<CSGenioArordf>(ref u, null, "ML1511"));
			}
		...
		public void Load(int numberListItems, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioArordf> Qlisting, ref CriteriaSet conditions)
		{
			...
			if (ajaxRequest)
				this.Navigation.SetValue("requestValues" + "PTN_Menu_1511", requestValues);
			else if (!ajaxRequest && this.Navigation.CheckKey("requestValues" + "PTN_Menu_1511"))
				requestValues = this.Navigation.GetValue<NameValueCollection>("requestValues" + "PTN_Menu_1511");

			User u = m_userContext.User;
			Menu = new TablePartial<GenioMVC.Models.Rordf>();

			CriteriaSet ptn_menu_1511Conds = CriteriaSet.And();
			...
			var pageNumber = (ajaxRequest && !String.IsNullOrEmpty(requestValues["pPTN_Menu_1511"])) ? int.Parse(requestValues["pPTN_Menu_1511"]) : 1;

			// Added to avoid 0 or -1 pages when setting number of records to -1 to disable pagination
			if (pageNumber < 1)
				pageNumber = 1;

			List<ColumnSort> sorts = GetRequestSorts(this.Menu, "sPTN_Menu_1511", "dPTN_Menu_1511", requestValues, "rordf", allSortOrders);
...
FieldRef[] fields = new FieldRef[] { CSGenioArordf.FldCodrordf, CSGenioArordf.FldZzstate, CSGenioArordf.FldOrder, CSGenioArordf.FldTitle };
			...
			//Tooltip for EPHs affecting this viewmodel list
			{
				...
				List<Limit> area_EPH_limits = EPH_Limit_Filler(ref limit, model_limit_area, "ML1511");
			...
			conditions.SubSets.Add(ptn_menu_1511Conds);
			ptn_menu_1511Conds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
			tableReload &= hasAllRequiredLimits;

// USE /[MANUAL PTN OVERRQ 1511]/
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioArordf>(m_userContext, false, ptn_menu_1511Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "ML1511", true, firstVisibleColumn: firstVisibleColumn);
...
// USE /[MANUAL PTN OVERRQLSTEXP 1511]/
			...
			if (tableReload)
			{
// USE /[MANUAL PTN OVERRQLIST 1511]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = m_userContext.PersistentSupport.getPagingPos(CSGenioArordf.GetInformation(), QMVC_POS_RECORD, sorts, ptn_menu_1511Conds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioArordf> listing = Models.ModelBase.Where<CSGenioArordf>(m_userContext, false, ptn_menu_1511Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "ML1511", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
				...
				Menu.Elements = MapPTN_Menu_1511(listing);

				Menu.Identifier = "ML1511";
				Menu.Slots = new Dictionary<string, List<object>>();

				// Last updated by [CJP] at [2015.02.03]
				// Adds the identifier to each element
				foreach (var element in Menu.Elements)
					element.Identifier = "ML1511";
		...
		private List<Models.Rordf> MapPTN_Menu_1511(ListingMVC<CSGenioArordf> Qlisting)
		{
			var Elements = new List<Models.Rordf>();
			int i = 0;
			...
			if (Qlisting.Rows != null)
			{
				foreach (var row in Qlisting.Rows)
				{
					if (Qlisting.NumRegs > 0 && i >= Qlisting.NumRegs) // Copiado da versão antiga do RowsToViewModels
						break;
					Elements.Add(MapPTN_Menu_1511(row));
					i++;
				}
			}
		...
		/// to a Models.Rordf object.
		/// </summary>
		/// <param name="row">The row.</param>
		private Models.Rordf MapPTN_Menu_1511(CSGenioArordf row)
		{
			var model = new Models.Rordf(m_userContext, true, _fieldsToSerialize);
		...
		#region ReorderCode
		public void ReorderPTN_Menu_1511(string id, string position)
		{
		...
		#region Custom code
// USE /[MANUAL GQT VIEWMODEL_CUSTOM PTN_MENU_1511]/
		#endregion
