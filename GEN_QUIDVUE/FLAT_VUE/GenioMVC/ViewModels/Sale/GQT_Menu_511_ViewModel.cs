...
namespace GenioMVC.ViewModels.Sale
{
	public class GQT_Menu_511_ViewModel : ListViewModel
	{
		...
		public override int GetCount(User user)
		{
			...
			CriteriaSet gqt_menu_511Conds = CSGenio.business.Listing.CalculateConditionsEphGeneric(areaBase, "ML511");
			gqt_menu_511Conds.Equal(CSGenioAsale.FldZzstate, 0); //valid zzstate only
...
// USE /[MANUAL GQT OVERRQ 511]/
			...
			SelectQuery qs = sp.getSelectQueryFromListingMVC(gqt_menu_511Conds, listing);
...
// USE /[MANUAL GQT OVERRQLIST 511]/
					...
					ListingMVC<CSGenioAsale> listing = Models.ModelBase.Where<CSGenioAsale>(m_userContext, false, gqt_menu_511Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "ML511", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn, fieldsWithTotalizers, tableConfig.SelectedRows);
		...
		{
			var Elements = new List<GQT_Menu_511_RowViewModel>();
			int i = 0;
		...
		/// </summary>
		public void CalculateButtonPermissions(GQT_Menu_511_RowViewModel model)
		{
			bool canView = true;
			bool canEdit = true;
			bool canDelete = true;
