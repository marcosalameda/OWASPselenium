...
{
	public class Vendaw04_ViewModel : FormViewModel<Models.Sale>, IPreparableForSerialization
	{
		[JsonIgnore]
		...
		// Field for formula
		/// <summary>Field: "Homework done" Tipo: "L"</summary>
		[ValidateSetAccess]
		...
		public Vendaw04_ViewModel() : base(null!) { }

		public Vendaw04_ViewModel(UserContext userContext, bool nestedForm = false) : base(userContext, "FVENDAW04", nestedForm) { }

		public Vendaw04_ViewModel(UserContext userContext, Models.Sale row, bool nestedForm = false) : base(userContext, "FVENDAW04", row, nestedForm) { }

		public Vendaw04_ViewModel(UserContext userContext, string id, bool nestedForm = false, string[]? fieldsToLoad = null) : this(userContext, nestedForm)
		{
			this.Navigation.SetValue("sale", id);
			Model = Models.Sale.Find(id, userContext, "FVENDAW04", fieldsToQuery: fieldsToLoad);
			if (Model == null)
		...
		public static StatusMessage InsertConditions(UserContext userContext)
		{
			...
			Models.Sale model = new Models.Sale(userContext) { Identifier = "FVENDAW04" };

			var navigation = m_userContext.CurrentNavigation;
			// The "LoadKeysFromHistory" must be after the "LoadEPH" because the PHE's in the tree mark Foreign Keys to null
			// (since they cannot assign multiple values to a single field) and thus the value that comes from Navigation is lost.
			// And this makes it more like the order of loading the model when opening the form.
			model.LoadEPH("FVENDAW04");
			if (navigation != null)
		...
		public override void MapToModel(Models.Sale m)
		{
			...
			{
				CSGenio.framework.Log.Error($"Map ViewModel (Vendaw04) to Model (Sale) - Error during mapping. All user values: {HasDisabledUserValuesSecurity}");
				throw;
			}
		}
			...
			try
			{
				...
				switch (fullFieldName)
				{
					...
					case "sale.codvenda":
						...
						break;
					default:
						Log.Error($"SetViewModelValue (Vendaw04) - Unexpected field identifier {fullFieldName}");
						break;
				}
			}
			catch (Exception ex)
			{
				throw new FrameworkException(Resources.Resources.PEDIMOS_DESCULPA__OC63848, "SetViewModelValue (Vendaw04)", "Unexpected error", ex);
			}
		}
		...
		{
			try { Model = Models.Sale.Find(id ?? Navigation.GetStrValue("sale"), m_userContext, "FVENDAW04"); }
			finally { Model ??= new Models.Sale(m_userContext) { Identifier = "FVENDAW04" }; }
		...
		public override void Load(NameValueCollection qs, bool editable, bool ajaxRequest = false, bool lazyLoad = false)
		{
			...
			try
			{
				Model = Models.Sale.Find(Navigation.GetStrValue("sale"), m_userContext, "FVENDAW04");
			}
			finally
			{
				if (Model == null)
		...
		public override void LoadPartial(NameValueCollection qs, bool lazyLoad = false)
		{
			...
			if (Model == null)
			{
				...
				if (Model == null)
				{
					Model = new Models.Sale(m_userContext) { Identifier = "FVENDAW04" };
...
// USE /[MANUAL GQT VIEWMODEL_DESTROY VENDAW04]/
		...
		{
			Model = Models.Sale.Find(id, m_userContext, "FVENDAW04");
			if (Model == null)
				throw new ModelNotFoundException("Model not found");
