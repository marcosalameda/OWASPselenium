...
namespace GenioMVC.ViewModels.Tpeq1
{
	public class Tpeq1_Fami1ValFamily_ViewModel : ListViewModel
	{
		...
		public Tpeq1_Fami1ValFamily_ViewModel(UserContext userContext) : base(userContext)
		{
		}
		...
		public CriteriaSet BuildCriteriaSet(NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			LoadUserTableConfig(requestValues, allSortOrders, "Tpeq1_Fami1ValFamily", ref numberListItems);
			...
			{
				// EPH
				crs = Models.Fami1.AddEPH<CSGenioAfami1>(ref u, crs, "IBL_TPEQ1___FAMI1FAMILY__");
			...
			crs.Criterias.Add(new Criteria(new ColumnReference(CSGenioAfami1.FldZzstate), CriteriaOperator.Equal, 0));


			if (tableReload)
			{
				string QMVC_POS_RECORD = requestValues["Q_POS_RECORD_fami1"];
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					crs.Equals(Models.Fami1.AddEPH<CSGenioAfami1>(ref u, null, "IBL_TPEQ1___FAMI1FAMILY__"));
			}
		...
		public void Load(int numberListItems, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAfami1> Qlisting, ref CriteriaSet conditions)
		{
			...
			if (ajaxRequest)
				this.Navigation.SetValue("requestValues" + "Tpeq1_Fami1ValFamily", requestValues);
			else if (!ajaxRequest && this.Navigation.CheckKey("requestValues" + "Tpeq1_Fami1ValFamily"))
				requestValues = this.Navigation.GetValue<NameValueCollection>("requestValues" + "Tpeq1_Fami1ValFamily");
			...
			var pageNumber = (ajaxRequest && !String.IsNullOrEmpty(requestValues["pTpeq1_Fami1ValFamily"])) ? int.Parse(requestValues["pTpeq1_Fami1ValFamily"]) : 1;

			// Added to avoid 0 or -1 pages when setting number of records to -1 to disable pagination
			if (pageNumber < 1)
				pageNumber = 1;

			List<ColumnSort> sorts = GetRequestSorts(this.Menu, "sTpeq1_Fami1ValFamily", "dTpeq1_Fami1ValFamily", requestValues, "fami1", allSortOrders);
...
FieldRef[] fields = new FieldRef[] { CSGenioAfami1.FldCodfamil, CSGenioAfami1.FldZzstate, CSGenioAfami1.FldFamily };
			...
			conditions.SubSets.Add(tpeq1___fami1family__Conds);
			tpeq1___fami1family__Conds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
...
// USE /[MANUAL GQT OVERRQ TPEQ1_FAMI1FAMILY]/
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAfami1>(m_userContext, false, tpeq1___fami1family__Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_TPEQ1___FAMI1FAMILY__", true, firstVisibleColumn: firstVisibleColumn);
...
// USE /[MANUAL GQT OVERRQLSTEXP TPEQ1_FAMI1FAMILY]/
			...
			if (tableReload)
			{
// USE /[MANUAL GQT OVERRQLIST TPEQ1_FAMI1FAMILY]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = m_userContext.PersistentSupport.getPagingPos(CSGenioAfami1.GetInformation(), QMVC_POS_RECORD, sorts, tpeq1___fami1family__Conds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioAfami1> listing = Models.ModelBase.Where<CSGenioAfami1>(m_userContext, false, tpeq1___fami1family__Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_TPEQ1___FAMI1FAMILY__", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
				...
				Menu.Elements = MapTpeq1_Fami1ValFamily(listing);

				Menu.Identifier = "IBL_TPEQ1___FAMI1FAMILY__";

				// Last updated by [CJP] at [2015.02.03]
				// Adds the identifier to each element
				foreach (var element in Menu.Elements)
					element.Identifier = "IBL_TPEQ1___FAMI1FAMILY__";
			...
			if (Qlisting.Rows != null)
			{
				foreach (var row in Qlisting.Rows)
				{
					if (Qlisting.NumRegs > 0 && i >= Qlisting.NumRegs) // Copiado da versão antiga do RowsToViewModels
						break;
					Elements.Add(MapTpeq1_Fami1ValFamily(row));
					i++;
				}
			}
		...
		private Models.Fami1 MapTpeq1_Fami1ValFamily(CSGenioAfami1 row)
		{
			var model = new Models.Fami1(m_userContext, true, _fieldsToSerialize);
