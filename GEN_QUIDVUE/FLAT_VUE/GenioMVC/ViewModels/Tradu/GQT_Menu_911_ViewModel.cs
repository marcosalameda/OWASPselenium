...
namespace GenioMVC.ViewModels.Tradu
{
	public class GQT_Menu_911_ViewModel : ListViewModel
	{
		...
		public TablePartial<GenioMVC.Models.Tradu> Menu { get; set; }
		...
		{
			dbeditTitle = Resources.Resources.TRADUCOES14996;
		}
...
FieldRef[] fields = new FieldRef[] { CSGenioAtradu.FldCodtradu, CSGenioAtradu.FldZzstate, CSGenioAtradu.FldReferenc, CSGenioAtradu.FldCodidio1, CSGenioAlang1.FldCodlang, CSGenioAlang1.FldLangua, CSGenioAtradu.FldAtraduzi, CSGenioAtradu.FldCodidio2, CSGenioAlang2.FldCodlang, CSGenioAlang2.FldLangua, CSGenioAtradu.FldTraduzid };
		...
		public GQT_Menu_911_ViewModel(UserContext userContext) : base(userContext)
		{
		...
		public void LoadToExport(out ListingMVC<CSGenioAtradu> listing, out CriteriaSet conditions, out List<Exports.QColumn> columns, NameValueCollection requestValues, bool ajaxRequest = false)
		{
			listing = null;
			conditions = null;
			...
			if (ajaxRequest && userColumns!=null)
			{
				...
				foreach (CSGenioAlstcol column in userColumns)
				{
					...
					string areabase = column.ValTabela.ToLower() != "tradu" ? CultureInfo.InvariantCulture.TextInfo.ToTitleCase(column.ValTabela) + "." : "";
		...
		public CriteriaSet BuildCriteriaSet(NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			if (Menu == null)
				Menu = new TablePartial<GenioMVC.Models.Tradu>();
			...
			LoadUserTableConfig(requestValues, allSortOrders, "GQT_Menu_911", ref numberListItems);
			...
			{
				// EPH
				crs = Models.Tradu.AddEPH<CSGenioAtradu>(ref u, crs, "ML911");

				// Export only records with ZZState == 0
				crs.Equal(CSGenioAtradu.FldZzstate, 0);

				return crs;
			}

			// Limitation by Zzstate
			if (!Navigation.checkFormMode("TRADU", FormMode.New)) // TODO: Check in Duplicate mode
				crs = extendWithZzstateCondition(crs, CSGenioAtradu.FldZzstate, null);


			if (tableReload)
			{
				string QMVC_POS_RECORD = Navigation.GetStrValue("QMVC_POS_RECORD_tradu");
				Navigation.DestroyEntry("QMVC_POS_RECORD_tradu");
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					crs.Equals(Models.Tradu.AddEPH<CSGenioAtradu>(ref u, null, "ML911"));
			}
		...
		{
			ListingMVC<CSGenioAtradu> listing = null;
		...
		/// <param name="conditions">The conditions.</param>
		public void Load(int numberListItems, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAtradu> Qlisting, ref CriteriaSet conditions)
		{
			//TODO: Tem um problema quando saímos de um form e voltamos ao dbedit e mudamos de página.
			//como não é devolvido to a view o text pesquisado, ao mudar de página assume que o Qfield está a vazio
			if (ajaxRequest)
				this.Navigation.SetValue("requestValues" + "GQT_Menu_911", requestValues);
			else if (!ajaxRequest && this.Navigation.CheckKey("requestValues" + "GQT_Menu_911"))
				requestValues = this.Navigation.GetValue<NameValueCollection>("requestValues" + "GQT_Menu_911");
			...
			var pageNumber = (ajaxRequest && !String.IsNullOrEmpty(requestValues["pGQT_Menu_911"])) ? int.Parse(requestValues["pGQT_Menu_911"]) : 1;

			// Added to avoid 0 or -1 pages when setting number of records to -1 to disable pagination
			if (pageNumber < 1)
				pageNumber = 1;

			List<ColumnSort> sorts = GetRequestSorts(this.Menu, "sGQT_Menu_911", "dGQT_Menu_911", requestValues, "tradu", allSortOrders);

			if (sorts == null || sorts.Count == 0)
			{
				sorts = new List<ColumnSort>();
				sorts.Add(new ColumnSort(new ColumnReference(CSGenioAtradu.FldReferenc), SortOrder.Ascending));
...
FieldRef[] fields = new FieldRef[] { CSGenioAtradu.FldCodtradu, CSGenioAtradu.FldZzstate, CSGenioAtradu.FldReferenc, CSGenioAtradu.FldCodidio1, CSGenioAlang1.FldCodlang, CSGenioAlang1.FldLangua, CSGenioAtradu.FldAtraduzi, CSGenioAtradu.FldCodidio2, CSGenioAlang2.FldCodlang, CSGenioAlang2.FldLangua, CSGenioAtradu.FldTraduzid };
			...
			conditions.SubSets.Add(gqt_menu_911Conds);
			gqt_menu_911Conds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
...
// USE /[MANUAL GQT OVERRQ 911]/
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAtradu>(m_userContext, false, gqt_menu_911Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "ML911", true, firstVisibleColumn: firstVisibleColumn);
...
// USE /[MANUAL GQT OVERRQLIST 911]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = m_userContext.PersistentSupport.getPagingPos(CSGenioAtradu.GetInformation(), QMVC_POS_RECORD, sorts, gqt_menu_911Conds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioAtradu> listing = Models.ModelBase.Where<CSGenioAtradu>(m_userContext, false, gqt_menu_911Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "ML911", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
				...
				Menu.Elements = MapGQT_Menu_911(listing);

				Menu.Identifier = "ML911";
				Menu.Slots = new Dictionary<string, List<object>>();

				// Last updated by [CJP] at [2015.02.03]
				// Adds the identifier to each element
				foreach (var element in Menu.Elements)
					element.Identifier = "ML911";
		...
		{
			var Elements = new List<Models.Tradu>();
			int i = 0;
			...
			if (Qlisting.Rows != null)
			{
				foreach (var row in Qlisting.Rows)
				{
					if (Qlisting.NumRegs > 0 && i >= Qlisting.NumRegs) // Copiado da versão antiga do RowsToViewModels
						break;
					Elements.Add(MapGQT_Menu_911(row));
					i++;
				}
			}
		...
		/// to a Models.Tradu object.
		/// </summary>
		/// <param name="row">The row.</param>
		private Models.Tradu MapGQT_Menu_911(CSGenioAtradu row)
		{
			var model = new Models.Tradu(m_userContext, true, _fieldsToSerialize);
			...
			foreach (RequestedField Qfield in row.Fields.Values)
			{
				switch (Qfield.Area)
				{
					...
					case "lang1":
						model.Lang1.klass.insertNameValueField(Qfield.FullName, Qfield.Value); break;
					case "lang2":
						model.Lang2.klass.insertNameValueField(Qfield.FullName, Qfield.Value); break;
					default:
						break;
				}
			}
