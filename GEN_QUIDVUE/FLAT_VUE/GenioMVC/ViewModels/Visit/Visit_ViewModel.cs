...
{
	public class Visit_ViewModel : FormViewModel<Models.Visit>, IPreparableForSerialization
	{
		[JsonIgnore]
		...
		public Visit_ViewModel() : base(null!) { }

		public Visit_ViewModel(UserContext userContext, bool nestedForm = false) : base(userContext, "FVISIT", nestedForm) { }

		public Visit_ViewModel(UserContext userContext, Models.Visit row, bool nestedForm = false) : base(userContext, "FVISIT", row, nestedForm) { }

		public Visit_ViewModel(UserContext userContext, string id, bool nestedForm = false, string[]? fieldsToLoad = null) : this(userContext, nestedForm)
		{
			this.Navigation.SetValue("visit", id);
			Model = Models.Visit.Find(id, userContext, "FVISIT", fieldsToQuery: fieldsToLoad);
			if (Model == null)
		...
		public static StatusMessage InsertConditions(UserContext userContext)
		{
			...
			Models.Visit model = new Models.Visit(userContext) { Identifier = "FVISIT" };

			var navigation = m_userContext.CurrentNavigation;
			// The "LoadKeysFromHistory" must be after the "LoadEPH" because the PHE's in the tree mark Foreign Keys to null
			// (since they cannot assign multiple values to a single field) and thus the value that comes from Navigation is lost.
			// And this makes it more like the order of loading the model when opening the form.
			model.LoadEPH("FVISIT");
			if (navigation != null)
		...
		public override void MapToModel(Models.Visit m)
		{
			...
			{
				CSGenio.framework.Log.Error($"Map ViewModel (Visit) to Model (Visit) - Error during mapping. All user values: {HasDisabledUserValuesSecurity}");
				throw;
			}
		}
			...
			try
			{
				...
				switch (fullFieldName)
				{
					...
					case "visit.codvisit":
						...
						break;
					default:
						Log.Error($"SetViewModelValue (Visit) - Unexpected field identifier {fullFieldName}");
						break;
				}
			}
			catch (Exception ex)
			{
				throw new FrameworkException(Resources.Resources.PEDIMOS_DESCULPA__OC63848, "SetViewModelValue (Visit)", "Unexpected error", ex);
			}
		}
		...
		{
			try { Model = Models.Visit.Find(id ?? Navigation.GetStrValue("visit"), m_userContext, "FVISIT"); }
			finally { Model ??= new Models.Visit(m_userContext) { Identifier = "FVISIT" }; }
		...
		public override void Load(NameValueCollection qs, bool editable, bool ajaxRequest = false, bool lazyLoad = false)
		{
			...
			try
			{
				Model = Models.Visit.Find(Navigation.GetStrValue("visit"), m_userContext, "FVISIT");
			}
			finally
			{
				if (Model == null)
		...
		public override void LoadPartial(NameValueCollection qs, bool lazyLoad = false)
		{
			...
			if (Model == null)
			{
				...
				if (Model == null)
				{
					Model = new Models.Visit(m_userContext) { Identifier = "FVISIT" };
...
// USE /[MANUAL GQT VIEWMODEL_NEW VISIT]/
		...
		public override CrudViewModelValidationResult Validate()
		{
			...
			validator.StringLength("ValObservat", Resources.Resources.OBSERVATIONS03729, ValObservat, 50);
...
// USE /[MANUAL GQT VIEWMODEL_DESTROY VISIT]/
		...
		{
			Model = Models.Visit.Find(id, m_userContext, "FVISIT");
			if (Model == null)
				throw new ModelNotFoundException("Model not found");
		...
		public void Load_Visit___equipregistnr(NameValueCollection qs, bool lazyLoad = false)
		{
			...
			CriteriaSet visit___equipregistnrConds = CriteriaSet.And();
			{
				...
				{
					visit___equipregistnrConds.Equal(CSGenioAequip.FldCodequip, hValue);
...
// USE /[MANUAL GQT OVERRQ VISIT_EQUIPREGISTNR]/
				...
				*/
				...
				ListingMVC<CSGenioAequip> listing = Models.ModelBase.Where<CSGenioAequip>(m_userContext, false, visit___equipregistnrConds, fields, offset, numberItems, sorts, "LED_VISIT___EQUIPREGISTNR", true, false, firstVisibleColumn: firstVisibleColumn);
				...
				TableEquipRegistnr.Elements = listing.RowsForViewModel<GenioMVC.Models.Equip>((r) => new GenioMVC.Models.Equip(m_userContext, r, true, _fieldsToSerialize_VISIT___EQUIPREGISTNR));
		...
		public ConcurrentDictionary<string, object> GetDependant_VisitTableEquipRegistnr(string PKey)
		{
		...
		{
			var row = GetDependant_VisitTableEquipRegistnr(this.ValCodequip);
			try
			{
		...
		protected override object GetViewModelValue(string identifier, object modelValue)
		{
			return identifier switch
			{
				...
				"visit.observat" => ViewModelConversion.ToString(modelValue),
