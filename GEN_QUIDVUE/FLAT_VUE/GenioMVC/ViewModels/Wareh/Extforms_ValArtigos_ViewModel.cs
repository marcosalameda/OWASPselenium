...
FieldRef[] fields = new FieldRef[] { CSGenioAitem.FldCoditem, CSGenioAitem.FldZzstate, CSGenioAitem.FldItemdes, CSGenioAitem.FldItemcod, CSGenioAitem.FldEntries, CSGenioAitem.FldExits, CSGenioAitem.FldExistenc, CSGenioAitem.FldImage };
			...
			conditions.SubSets.Add(extformspseudartigos_Conds);
			extformspseudartigos_Conds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
...
// USE /[MANUAL GQT OVERRQ EXTFORMS_PSEUDARTIGOS]/
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAitem>(m_userContext, false, extformspseudartigos_Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_EXTFORMSPSEUDARTIGOS_", true, firstVisibleColumn: firstVisibleColumn);
...
// USE /[MANUAL GQT OVERRQLIST EXTFORMS_PSEUDARTIGOS]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = m_userContext.PersistentSupport.getPagingPos(CSGenioAitem.GetInformation(), QMVC_POS_RECORD, sorts, extformspseudartigos_Conds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioAitem> listing = Models.ModelBase.Where<CSGenioAitem>(m_userContext, false, extformspseudartigos_Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_EXTFORMSPSEUDARTIGOS_", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
