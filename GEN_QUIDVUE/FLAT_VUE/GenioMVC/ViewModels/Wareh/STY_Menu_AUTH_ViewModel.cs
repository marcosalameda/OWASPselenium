...
{
	public class STY_Menu_AUTH_ViewModel : ListViewModel
	{
		/// <summary>
		...
		/// <inheritdoc/>
		public override string Uuid { get => "ebf10cb5-a558-4d7e-897a-082053a63545"; }
		...
		{
			dbeditTitle = Resources.Resources.AUTHENTICATION40905;
		}
...
FieldRef[] fields = new FieldRef[] { CSGenioAwareh.FldCodwareh, CSGenioAwareh.FldZzstate, CSGenioAwareh.FldWarehdes };
		...
		public CriteriaSet BuildCriteriaSet(NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			Menu.SetFilters(bool.Parse(requestValues["STY_Menu_AUTH_tableFilters"] ?? "false"), false);
			...
			LoadUserTableConfig(requestValues, allSortOrders, "STY_Menu_AUTH", ref numberListItems);

			crs.SubSets.Add(ProcessSearchFilters(Menu, GetSearchColumns(true), requestValues, "STY_Menu_AUTH_"));
		...
		public void Load(int numberListItems, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAwareh> Qlisting, ref CriteriaSet conditions)
		{
			...
			if (ajaxRequest)
				this.Navigation.SetValue("requestValues" + "STY_Menu_AUTH", requestValues);
			else if (!ajaxRequest && this.Navigation.CheckKey("requestValues" + "STY_Menu_AUTH"))
				requestValues = this.Navigation.GetValue<NameValueCollection>("requestValues" + "STY_Menu_AUTH");
			...
			var pageNumber = (ajaxRequest && !String.IsNullOrEmpty(requestValues["pSTY_Menu_AUTH"])) ? int.Parse(requestValues["pSTY_Menu_AUTH"]) : 1;

			// Added to avoid 0 or -1 pages when setting number of records to -1 to disable pagination
			if (pageNumber < 1)
				pageNumber = 1;

			List<ColumnSort> sorts = GetRequestSorts(this.Menu, "sSTY_Menu_AUTH", "dSTY_Menu_AUTH", requestValues, "wareh", allSortOrders);
...
FieldRef[] fields = new FieldRef[] { CSGenioAwareh.FldCodwareh, CSGenioAwareh.FldZzstate, CSGenioAwareh.FldWarehdes };
			...
			sty_menu_authConds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
...
// USE /[MANUAL STY OVERRQ AUTH]/
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAwareh>(m_userContext, false, sty_menu_authConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "MLAUTH", true, firstVisibleColumn: firstVisibleColumn);
...
// USE /[MANUAL STY OVERRQLIST AUTH]/
				...
				ListingMVC<CSGenioAwareh> listing = Models.ModelBase.Where<CSGenioAwareh>(m_userContext, false, sty_menu_authConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "MLAUTH", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
		...
		private List<Models.Wareh> MapSTY_Menu_AUTH(ListingMVC<CSGenioAwareh> Qlisting)
		{
		...
		private Models.Wareh MapSTY_Menu_AUTH(CSGenioAwareh row)
		{
