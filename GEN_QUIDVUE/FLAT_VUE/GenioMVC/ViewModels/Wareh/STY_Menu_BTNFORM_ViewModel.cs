...
namespace GenioMVC.ViewModels.Wareh
{
	public class STY_Menu_BTNFORM_ViewModel : ListViewModel
	{
		...
		public override int GetCount(User user)
		{
			...
			CriteriaSet sty_menu_btnformConds = CSGenio.business.Listing.CalculateConditionsEphGeneric(areaBase, "MLBTNFORM");
			sty_menu_btnformConds.Equal(CSGenioAwareh.FldZzstate, 0); //valid zzstate only

			//Menu fixed limits and relations:

						sty_menu_btnformConds.Equal(CSGenioAwareh.FldShowreco, 1);
...
// USE /[MANUAL STY OVERRQ BTNFORM]/
			...
			SelectQuery qs = sp.getSelectQueryFromListingMVC(sty_menu_btnformConds, listing);
...
// USE /[MANUAL STY OVERRQLIST BTNFORM]/
					...
					ListingMVC<CSGenioAwareh> listing = Models.ModelBase.Where<CSGenioAwareh>(m_userContext, false, sty_menu_btnformConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "MLBTNFORM", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn, fieldsWithTotalizers, tableConfig.SelectedRows);
		...
		{
			var Elements = new List<STY_Menu_BTNFORM_RowViewModel>();
			int i = 0;
		...
		/// </summary>
		public void CalculateButtonPermissions(STY_Menu_BTNFORM_RowViewModel model)
		{
			bool canView = true;
			bool canEdit = true;
			bool canDelete = true;
