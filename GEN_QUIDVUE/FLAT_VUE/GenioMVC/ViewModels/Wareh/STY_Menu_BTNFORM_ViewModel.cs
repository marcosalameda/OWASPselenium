...
{
	public class STY_Menu_BTNFORM_ViewModel : ListViewModel
	{
		/// <summary>
		...
		/// <inheritdoc/>
		public override string Uuid { get => "0f2bcd97-d616-4f55-889e-3e89da97dfa1"; }
		...
		{
			dbeditTitle = Resources.Resources.BUTTONS20612;
		}
...
FieldRef[] fields = new FieldRef[] { CSGenioAwareh.FldCodwareh, CSGenioAwareh.FldZzstate, CSGenioAwareh.FldWarehdes };
		...
		public CriteriaSet BuildCriteriaSet(NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			Menu.SetFilters(bool.Parse(requestValues["STY_Menu_BTNFORM_tableFilters"] ?? "false"), false);
			...
			LoadUserTableConfig(requestValues, allSortOrders, "STY_Menu_BTNFORM", ref numberListItems);

			crs.SubSets.Add(ProcessSearchFilters(Menu, GetSearchColumns(true), requestValues, "STY_Menu_BTNFORM_"));
		...
		public void Load(int numberListItems, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAwareh> Qlisting, ref CriteriaSet conditions)
		{
			...
			if (ajaxRequest)
				this.Navigation.SetValue("requestValues" + "STY_Menu_BTNFORM", requestValues);
			else if (!ajaxRequest && this.Navigation.CheckKey("requestValues" + "STY_Menu_BTNFORM"))
				requestValues = this.Navigation.GetValue<NameValueCollection>("requestValues" + "STY_Menu_BTNFORM");
			...
			var pageNumber = (ajaxRequest && !String.IsNullOrEmpty(requestValues["pSTY_Menu_BTNFORM"])) ? int.Parse(requestValues["pSTY_Menu_BTNFORM"]) : 1;

			// Added to avoid 0 or -1 pages when setting number of records to -1 to disable pagination
			if (pageNumber < 1)
				pageNumber = 1;

			List<ColumnSort> sorts = GetRequestSorts(this.Menu, "sSTY_Menu_BTNFORM", "dSTY_Menu_BTNFORM", requestValues, "wareh", allSortOrders);
...
FieldRef[] fields = new FieldRef[] { CSGenioAwareh.FldCodwareh, CSGenioAwareh.FldZzstate, CSGenioAwareh.FldWarehdes };
			...
			sty_menu_btnformConds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
...
// USE /[MANUAL STY OVERRQ BTNFORM]/
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAwareh>(m_userContext, false, sty_menu_btnformConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "MLBTNFORM", true, firstVisibleColumn: firstVisibleColumn);
...
// USE /[MANUAL STY OVERRQLIST BTNFORM]/
				...
				ListingMVC<CSGenioAwareh> listing = Models.ModelBase.Where<CSGenioAwareh>(m_userContext, false, sty_menu_btnformConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "MLBTNFORM", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
		...
		private List<Models.Wareh> MapSTY_Menu_BTNFORM(ListingMVC<CSGenioAwareh> Qlisting)
		{
		...
		private Models.Wareh MapSTY_Menu_BTNFORM(CSGenioAwareh row)
		{
