...
{
	public class STY_Menu_EXTENDFORM_ViewModel : MenuListViewModel<Models.Wareh>
	{
		/// <summary>
		/// Gets or sets the object that represents the table and its elements.
		/// </summary>
		[JsonPropertyName("table")]
		public TablePartial<STY_Menu_EXTENDFORM_RowViewModel> Menu { get; set; }
		...
		public override string Uuid => "48d615f9-915f-4df1-8b3a-b03dde4a3d70";
		...
		{
// USE /[MANUAL STY LIST_LIMITS EXTENDFORM]/
		...
		public override int GetCount(User user)
		{
			...
			CriteriaSet conditions = CSGenio.business.Listing.CalculateConditionsEphGeneric(areaBase, "MLEXTENDFORM");
		...
		public STY_Menu_EXTENDFORM_ViewModel() : base(null!) { }

		/// <summary>
		/// Initializes a new instance of the <see cref="STY_Menu_EXTENDFORM_ViewModel" /> class.
		/// </summary>
		/// <param name="userContext">The current user request context</param>
		public STY_Menu_EXTENDFORM_ViewModel(UserContext userContext) : base(userContext)
		{
		...
		public STY_Menu_EXTENDFORM_ViewModel(UserContext userContext, Models.ModelBase parentCtx) : this(userContext)
		{
			ParentCtx = parentCtx;
		}
		...
		public override CriteriaSet BuildCriteriaSet(CSGenio.core.framework.table.TableConfiguration tableConfig, NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			Menu ??= new TablePartial<STY_Menu_EXTENDFORM_RowViewModel>();
			...
			{
				// EPH
				crs = Models.Wareh.AddEPH<CSGenioAwareh>(ref u, crs, "MLEXTENDFORM");
			...
			if (tableReload)
			{
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					crs.Equals(Models.Wareh.AddEPH<CSGenioAwareh>(ref u, null, "MLEXTENDFORM"));
			}
		...
		public void Load(CSGenio.core.framework.table.TableConfiguration tableConfig, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAwareh> Qlisting, ref CriteriaSet conditions)
		{
			...
			CriteriaSet sty_menu_extendformConds = CriteriaSet.And();
			...
			//Tooltip for EPHs affecting this viewmodel list
			{
				...
				List<Limit> area_EPH_limits = EPH_Limit_Filler(ref limit, model_limit_area, "MLEXTENDFORM");
			...
			conditions.SubSets.Add(sty_menu_extendformConds);
			sty_menu_extendformConds = BuildCriteriaSet(tableConfig, requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
...
// USE /[MANUAL STY OVERRQ EXTENDFORM]/
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.BuildListingForExport<CSGenioAwareh>(m_userContext, false, ref sty_menu_extendformConds, exportFieldRefs, (pageNumber - 1) * numberListItems, numberListItems, sorts, "MLEXTENDFORM", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL STY OVERRQLSTEXP EXTENDFORM]/
			...
			if (tableReload)
			{
// USE /[MANUAL STY OVERRQLIST EXTENDFORM]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = m_userContext.PersistentSupport.getPagingPos(CSGenioAwareh.GetInformation(), QMVC_POS_RECORD, sorts, sty_menu_extendformConds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
				...
				Menu.Elements = MapSTY_Menu_EXTENDFORM(listing);

				Menu.Identifier = "MLEXTENDFORM";
				Menu.Slots = new Dictionary<string, List<object>>();

				// Last updated by [CJP] at [2015.02.03]
				// Adds the identifier to each element
				foreach (var element in Menu.Elements)
					element.Identifier = "MLEXTENDFORM";
		...
		private List<STY_Menu_EXTENDFORM_RowViewModel> MapSTY_Menu_EXTENDFORM(ListingMVC<CSGenioAwareh> Qlisting)
		{
			List<STY_Menu_EXTENDFORM_RowViewModel> Elements = [];
			int i = 0;
			...
			if (Qlisting.Rows != null)
			{
				foreach (var row in Qlisting.Rows)
				{
					if (Qlisting.NumRegs > 0 && i >= Qlisting.NumRegs) // Copiado da versão antiga do RowsToViewModels
						break;
					Elements.Add(MapSTY_Menu_EXTENDFORM(row));
					i++;
				}
			}
		...
		/// to a STY_Menu_EXTENDFORM_RowViewModel object.
		/// </summary>
		/// <param name="row">The row.</param>
		private STY_Menu_EXTENDFORM_RowViewModel MapSTY_Menu_EXTENDFORM(CSGenioAwareh row)
		{
			var model = new STY_Menu_EXTENDFORM_RowViewModel(m_userContext, true, _fieldsToSerialize);
			if (row == null)
				return model;
...
// USE /[MANUAL GQT VIEWMODEL_CUSTOM STY_MENU_EXTENDFORM]/
