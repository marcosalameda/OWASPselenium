...
{
	public class STY_Menu_MULTIFORM_ViewModel : ListViewModel
	{
		/// <summary>
		...
		/// <inheritdoc/>
		public override string Uuid { get => "601f1eb0-feab-4d54-8e22-3d4479c3eaab"; }
		...
		{
			dbeditTitle = Resources.Resources.MULTIFORM41286;
		}
...
FieldRef[] fields = new FieldRef[] { CSGenioAwareh.FldCodwareh, CSGenioAwareh.FldZzstate, CSGenioAwareh.FldWarehdes };
		...
		public CriteriaSet BuildCriteriaSet(NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			Menu.SetFilters(bool.Parse(requestValues["STY_Menu_MULTIFORM_tableFilters"] ?? "false"), false);
			...
			LoadUserTableConfig(requestValues, allSortOrders, "STY_Menu_MULTIFORM", ref numberListItems);

			crs.SubSets.Add(ProcessSearchFilters(Menu, GetSearchColumns(true), requestValues, "STY_Menu_MULTIFORM_"));
		...
		public void Load(int numberListItems, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAwareh> Qlisting, ref CriteriaSet conditions)
		{
			...
			if (ajaxRequest)
				this.Navigation.SetValue("requestValues" + "STY_Menu_MULTIFORM", requestValues);
			else if (!ajaxRequest && this.Navigation.CheckKey("requestValues" + "STY_Menu_MULTIFORM"))
				requestValues = this.Navigation.GetValue<NameValueCollection>("requestValues" + "STY_Menu_MULTIFORM");
			...
			var pageNumber = (ajaxRequest && !String.IsNullOrEmpty(requestValues["pSTY_Menu_MULTIFORM"])) ? int.Parse(requestValues["pSTY_Menu_MULTIFORM"]) : 1;

			// Added to avoid 0 or -1 pages when setting number of records to -1 to disable pagination
			if (pageNumber < 1)
				pageNumber = 1;

			List<ColumnSort> sorts = GetRequestSorts(this.Menu, "sSTY_Menu_MULTIFORM", "dSTY_Menu_MULTIFORM", requestValues, "wareh", allSortOrders);
...
FieldRef[] fields = new FieldRef[] { CSGenioAwareh.FldCodwareh, CSGenioAwareh.FldZzstate, CSGenioAwareh.FldWarehdes };
			...
			sty_menu_multiformConds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
...
// USE /[MANUAL STY OVERRQ MULTIFORM]/
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAwareh>(m_userContext, false, sty_menu_multiformConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "MLMULTIFORM", true, firstVisibleColumn: firstVisibleColumn);
...
// USE /[MANUAL STY OVERRQLIST MULTIFORM]/
				...
				ListingMVC<CSGenioAwareh> listing = Models.ModelBase.Where<CSGenioAwareh>(m_userContext, false, sty_menu_multiformConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "MLMULTIFORM", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
		...
		private List<Models.Wareh> MapSTY_Menu_MULTIFORM(ListingMVC<CSGenioAwareh> Qlisting)
		{
		...
		private Models.Wareh MapSTY_Menu_MULTIFORM(CSGenioAwareh row)
		{
