...
{
	public class Armapess_ViewModel : FormViewModel<Models.Wpess>, IPreparableForSerialization
	{
		[JsonIgnore]
		...
		public Armapess_ViewModel() : base(null!) { }

		public Armapess_ViewModel(UserContext userContext, bool nestedForm = false) : base(userContext, "FARMAPESS", nestedForm) { }

		public Armapess_ViewModel(UserContext userContext, Models.Wpess row, bool nestedForm = false) : base(userContext, "FARMAPESS", row, nestedForm) { }

		public Armapess_ViewModel(UserContext userContext, string id, bool nestedForm = false, string[]? fieldsToLoad = null) : this(userContext, nestedForm)
		{
			this.Navigation.SetValue("wpess", id);
			Model = Models.Wpess.Find(id, userContext, "FARMAPESS", fieldsToQuery: fieldsToLoad);
			if (Model == null)
		...
		public static StatusMessage InsertConditions(UserContext userContext)
		{
			...
			Models.Wpess model = new Models.Wpess(userContext) { Identifier = "FARMAPESS" };

			var navigation = m_userContext.CurrentNavigation;
			// The "LoadKeysFromHistory" must be after the "LoadEPH" because the PHE's in the tree mark Foreign Keys to null
			// (since they cannot assign multiple values to a single field) and thus the value that comes from Navigation is lost.
			// And this makes it more like the order of loading the model when opening the form.
			model.LoadEPH("FARMAPESS");
			if (navigation != null)
		...
		public override void MapToModel(Models.Wpess m)
		{
			...
			{
				CSGenio.framework.Log.Error($"Map ViewModel (Armapess) to Model (Wpess) - Error during mapping. All user values: {HasDisabledUserValuesSecurity}");
				throw;
			}
		}
			...
			try
			{
				...
				switch (fullFieldName)
				{
					...
					case "wpess.codpess":
						...
						break;
					default:
						Log.Error($"SetViewModelValue (Armapess) - Unexpected field identifier {fullFieldName}");
						break;
				}
			}
			catch (Exception ex)
			{
				throw new FrameworkException(Resources.Resources.PEDIMOS_DESCULPA__OC63848, "SetViewModelValue (Armapess)", "Unexpected error", ex);
			}
		}
		...
		{
			try { Model = Models.Wpess.Find(id ?? Navigation.GetStrValue("wpess"), m_userContext, "FARMAPESS"); }
			finally { Model ??= new Models.Wpess(m_userContext) { Identifier = "FARMAPESS" }; }
		...
		public override void Load(NameValueCollection qs, bool editable, bool ajaxRequest = false, bool lazyLoad = false)
		{
			...
			try
			{
				Model = Models.Wpess.Find(Navigation.GetStrValue("wpess"), m_userContext, "FARMAPESS");
			}
			finally
			{
				if (Model == null)
		...
		public override void LoadPartial(NameValueCollection qs, bool lazyLoad = false)
		{
			...
			if (Model == null)
			{
				...
				if (Model == null)
				{
					Model = new Models.Wpess(m_userContext) { Identifier = "FARMAPESS" };
...
// USE /[MANUAL GQT VIEWMODEL_DESTROY ARMAPESS]/
		...
		{
			Model = Models.Wpess.Find(id, m_userContext, "FARMAPESS");
			if (Model == null)
				throw new ModelNotFoundException("Model not found");
		...
		public void Load_Armapesswarehwarehdes(NameValueCollection qs, bool lazyLoad = false)
		{
			...
			CriteriaSet armapesswarehwarehdesConds = CriteriaSet.And();
			{
				...
				{
					armapesswarehwarehdesConds.Equal(CSGenioAwareh.FldCodwareh, hValue);
...
// USE /[MANUAL GQT OVERRQ ARMAPESS_WAREHWAREHDES]/
				...
				*/
				...
				ListingMVC<CSGenioAwareh> listing = Models.ModelBase.Where<CSGenioAwareh>(m_userContext, false, armapesswarehwarehdesConds, fields, offset, numberItems, sorts, "LED_ARMAPESSWAREHWAREHDES", true, false, firstVisibleColumn: firstVisibleColumn);
				...
				TableWarehWarehdes.Elements = listing.RowsForViewModel<GenioMVC.Models.Wareh>((r) => new GenioMVC.Models.Wareh(m_userContext, r, true, _fieldsToSerialize_ARMAPESSWAREHWAREHDES));
		...
		public ConcurrentDictionary<string, object> GetDependant_ArmapessTableWarehWarehdes(string PKey)
		{
		...
		{
			var row = GetDependant_ArmapessTableWarehWarehdes(this.ValCodwareh);
			try
			{
