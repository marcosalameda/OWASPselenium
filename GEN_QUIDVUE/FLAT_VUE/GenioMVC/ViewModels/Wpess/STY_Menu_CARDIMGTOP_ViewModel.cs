...
{
	public class STY_Menu_CARDIMGTOP_ViewModel : ListViewModel
	{
		/// <summary>
		...
		/// <inheritdoc/>
		public override string Uuid { get => "0e99ffff-7adf-43a1-9968-81840cdfb35c"; }
		...
		{
			dbeditTitle = Resources.Resources.CARD_IMAGE_TOP01944;
		}
...
FieldRef[] fields = new FieldRef[] { CSGenioAwpess.FldCodpess, CSGenioAwpess.FldZzstate, CSGenioAwpess.FldName, CSGenioAwpess.FldDate, CSGenioAwpess.FldSex, CSGenioAwpess.FldNfunc, CSGenioAwpess.FldAdress, CSGenioAwpess.FldZipcode, CSGenioAwpess.FldCountry, CSGenioAwpess.FldEmail, CSGenioAwpess.FldCellphon, CSGenioAwpess.FldNaturali, CSGenioAwpess.FldNacional, CSGenioAwpess.FldCodwareh, CSGenioAwareh.FldCodwareh, CSGenioAwareh.FldWarehdes, CSGenioAwpess.FldFtimgtop };
		...
		/// <summary>
		/// Initializes a new instance of the <see cref="STY_Menu_CARDIMGTOP_ViewModel" /> class.
		/// </summary>
		/// <param name="userContext">The current user request context</param>
		public STY_Menu_CARDIMGTOP_ViewModel(UserContext userContext) : base(userContext)
		{
		...
		public CriteriaSet BuildCriteriaSet(NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			Menu.SetFilters(bool.Parse(requestValues["STY_Menu_CARDIMGTOP_tableFilters"] ?? "false"), false);
			...
			LoadUserTableConfig(requestValues, allSortOrders, "STY_Menu_CARDIMGTOP", ref numberListItems);

			crs.SubSets.Add(ProcessSearchFilters(Menu, GetSearchColumns(true), requestValues, "STY_Menu_CARDIMGTOP_"));
			...
			{
				// EPH
				crs = Models.Wpess.AddEPH<CSGenioAwpess>(ref u, crs, "MLCARDIMGTOP");
			...
			if (tableReload)
			{
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					crs.Equals(Models.Wpess.AddEPH<CSGenioAwpess>(ref u, null, "MLCARDIMGTOP"));
			}
		...
		public void Load(int numberListItems, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAwpess> Qlisting, ref CriteriaSet conditions)
		{
			...
			if (ajaxRequest)
				this.Navigation.SetValue("requestValues" + "STY_Menu_CARDIMGTOP", requestValues);
			else if (!ajaxRequest && this.Navigation.CheckKey("requestValues" + "STY_Menu_CARDIMGTOP"))
				requestValues = this.Navigation.GetValue<NameValueCollection>("requestValues" + "STY_Menu_CARDIMGTOP");

			User u = m_userContext.User;
			Menu = new TablePartial<GenioMVC.Models.Wpess>();

			CriteriaSet sty_menu_cardimgtopConds = CriteriaSet.And();
			...
			var pageNumber = (ajaxRequest && !String.IsNullOrEmpty(requestValues["pSTY_Menu_CARDIMGTOP"])) ? int.Parse(requestValues["pSTY_Menu_CARDIMGTOP"]) : 1;

			// Added to avoid 0 or -1 pages when setting number of records to -1 to disable pagination
			if (pageNumber < 1)
				pageNumber = 1;

			List<ColumnSort> sorts = GetRequestSorts(this.Menu, "sSTY_Menu_CARDIMGTOP", "dSTY_Menu_CARDIMGTOP", requestValues, "wpess", allSortOrders);
...
FieldRef[] fields = new FieldRef[] { CSGenioAwpess.FldCodpess, CSGenioAwpess.FldZzstate, CSGenioAwpess.FldName, CSGenioAwpess.FldDate, CSGenioAwpess.FldSex, CSGenioAwpess.FldNfunc, CSGenioAwpess.FldAdress, CSGenioAwpess.FldZipcode, CSGenioAwpess.FldCountry, CSGenioAwpess.FldEmail, CSGenioAwpess.FldCellphon, CSGenioAwpess.FldNaturali, CSGenioAwpess.FldNacional, CSGenioAwpess.FldCodwareh, CSGenioAwareh.FldCodwareh, CSGenioAwareh.FldWarehdes, CSGenioAwpess.FldFtimgtop };
			...
			//Tooltip for EPHs affecting this viewmodel list
			{
				...
				List<Limit> area_EPH_limits = EPH_Limit_Filler(ref limit, model_limit_area, "MLCARDIMGTOP");
			...
			conditions.SubSets.Add(sty_menu_cardimgtopConds);
			sty_menu_cardimgtopConds = BuildCriteriaSet(requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
			tableReload &= hasAllRequiredLimits;

// USE /[MANUAL STY OVERRQ CARDIMGTOP]/
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioAwpess>(m_userContext, false, sty_menu_cardimgtopConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "MLCARDIMGTOP", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL STY OVERRQLSTEXP CARDIMGTOP]/
			...
			if (tableReload)
			{
// USE /[MANUAL STY OVERRQLIST CARDIMGTOP]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = m_userContext.PersistentSupport.getPagingPos(CSGenioAwpess.GetInformation(), QMVC_POS_RECORD, sorts, sty_menu_cardimgtopConds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
					{
				...
				ListingMVC<CSGenioAwpess> listing = Models.ModelBase.Where<CSGenioAwpess>(m_userContext, false, sty_menu_cardimgtopConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "MLCARDIMGTOP", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
				...
				Menu.Elements = MapSTY_Menu_CARDIMGTOP(listing);

				Menu.Identifier = "MLCARDIMGTOP";
				Menu.Slots = new Dictionary<string, List<object>>();

				// Last updated by [CJP] at [2015.02.03]
				// Adds the identifier to each element
				foreach (var element in Menu.Elements)
					element.Identifier = "MLCARDIMGTOP";
		...
		private List<Models.Wpess> MapSTY_Menu_CARDIMGTOP(ListingMVC<CSGenioAwpess> Qlisting)
		{
			...
			if (Qlisting.Rows != null)
			{
				foreach (var row in Qlisting.Rows)
				{
					if (Qlisting.NumRegs > 0 && i >= Qlisting.NumRegs) // Copiado da versão antiga do RowsToViewModels
						break;
					Elements.Add(MapSTY_Menu_CARDIMGTOP(row));
					i++;
				}
			}
		...
		private Models.Wpess MapSTY_Menu_CARDIMGTOP(CSGenioAwpess row)
		{
		...
		#region Custom code
// USE /[MANUAL GQT VIEWMODEL_CUSTOM STY_MENU_CARDIMGTOP]/
		#endregion

		private static readonly string[] _fieldsToSerialize =
		{
			"Wpess", "Wpess.ValCodpess", "Wpess.ValZzstate", "Wpess.ValName", "Wpess.ValDate", "Wpess.ValSex", "Wpess.ValNfunc", "Wpess.ValAdress", "Wpess.ValZipcode", "Wpess.ValCountry", "Wpess.ValEmail", "Wpess.ValCellphon", "Wpess.ValNaturali", "Wpess.ValNacional", "Wareh", "Wareh.ValWarehdes", "Wpess.ValFtimgtop", "Wpess.ValCodwareh"
		};
