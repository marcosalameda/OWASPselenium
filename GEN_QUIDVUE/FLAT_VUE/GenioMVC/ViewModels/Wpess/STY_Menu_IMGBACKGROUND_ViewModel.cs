...
{
	public class STY_Menu_IMGBACKGROUND_ViewModel : ListViewModel
	{
		/// <summary>
		/// Gets or sets the object that represents the table and its elements. List type: "${exposeField.Fajuda}"
		/// </summary>
		[JsonPropertyName("Table")]
		public TablePartial<STY_Menu_IMGBACKGROUND_RowViewModel> Menu { get; set; }
		...
		/// <inheritdoc/>
		public override string Uuid { get => "1e458cba-04d7-4e05-b1f9-079d14d87bcf"; }
		...
		public override int GetCount(User user)
		{
			...
			CriteriaSet sty_menu_imgbackgroundConds = CSGenio.business.Listing.CalculateConditionsEphGeneric(areaBase, "MLIMGBACKGROUND");
			sty_menu_imgbackgroundConds.Equal(CSGenioAwpess.FldZzstate, 0); //valid zzstate only

			//Menu fixed limits and relations:

			

// USE /[MANUAL STY OVERRQ IMGBACKGROUND]/
			...
			SelectQuery qs = sp.getSelectQueryFromListingMVC(sty_menu_imgbackgroundConds, listing);
		...
		public STY_Menu_IMGBACKGROUND_ViewModel(UserContext userContext) : base(userContext)
		{
		...
		public override List<Exports.QColumn> GetColumnsToExport(bool ajaxRequest = false)
		{
			var columns = new List<Exports.QColumn>()
			{
				...
				!ajaxRequest ? new Exports.QColumn(CSGenioAwpess.FldFtbackgr, FieldType.IMAGEM_JPEG, Resources.Resources.PROFILE_PICTURE42911, 3, 1, true):null,
			};
		...
		public override CriteriaSet BuildCriteriaSet(CSGenio.framework.TableConfiguration.TableConfiguration tableConfig, NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			if (Menu == null)
				Menu = new TablePartial<STY_Menu_IMGBACKGROUND_RowViewModel>();
			...
			{
				// EPH
				crs = Models.Wpess.AddEPH<CSGenioAwpess>(ref u, crs, "MLIMGBACKGROUND");
			...
			if (tableReload)
			{
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					crs.Equals(Models.Wpess.AddEPH<CSGenioAwpess>(ref u, null, "MLIMGBACKGROUND"));
			}
		...
		public void Load(CSGenio.framework.TableConfiguration.TableConfiguration tableConfig, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAwpess> Qlisting, ref CriteriaSet conditions)
		{
			using (GenioDI.MetricsOtlp.RecordTime("menu_load_time", new List<KeyValuePair<string, object>>() {
				...
				CriteriaSet sty_menu_imgbackgroundConds = CriteriaSet.And();
				...
				sorts.Add(new ColumnSort(new ColumnReference(CSGenioAwpess.FldDate), SortOrder.Ascending));
			...
			//Tooltip for EPHs affecting this viewmodel list
			{
				...
				List<Limit> area_EPH_limits = EPH_Limit_Filler(ref limit, model_limit_area, "MLIMGBACKGROUND");
				...
				conditions.SubSets.Add(sty_menu_imgbackgroundConds);
				sty_menu_imgbackgroundConds = BuildCriteriaSet(tableConfig, requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
...
// USE /[MANUAL STY OVERRQ IMGBACKGROUND]/
				...
				if (isToExport)
				{
					...
					Qlisting = Models.ModelBase.Where<CSGenioAwpess>(m_userContext, false, sty_menu_imgbackgroundConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "MLIMGBACKGROUND", true, firstVisibleColumn: firstVisibleColumn);
...
// USE /[MANUAL STY OVERRQLSTEXP IMGBACKGROUND]/
				...
				if (tableReload)
				{
// USE /[MANUAL STY OVERRQLIST IMGBACKGROUND]/
					...
					if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					{
						var m_iCurPag = m_userContext.PersistentSupport.getPagingPos(CSGenioAwpess.GetInformation(), QMVC_POS_RECORD, sorts, sty_menu_imgbackgroundConds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
						if (m_iCurPag != -1)
							pageNumber = ((m_iCurPag - 1) / numberListItems) + 1;
					}

					ListingMVC<CSGenioAwpess> listing = Models.ModelBase.Where<CSGenioAwpess>(m_userContext, false, sty_menu_imgbackgroundConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "MLIMGBACKGROUND", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn, fieldsWithTotalizers, tableConfig.SelectedRows);
					...
					Menu.Elements = MapSTY_Menu_IMGBACKGROUND(listing);

					Menu.Identifier = "MLIMGBACKGROUND";
					Menu.Slots = new Dictionary<string, List<object>>();

					// Last updated by [CJP] at [2015.02.03]
					// Adds the identifier to each element
					foreach (var element in Menu.Elements)
						element.Identifier = "MLIMGBACKGROUND";
		...
		private List<STY_Menu_IMGBACKGROUND_RowViewModel> MapSTY_Menu_IMGBACKGROUND(ListingMVC<CSGenioAwpess> Qlisting)
		{
			var Elements = new List<STY_Menu_IMGBACKGROUND_RowViewModel>();
			int i = 0;
			...
			if (Qlisting.Rows != null)
			{
				foreach (var row in Qlisting.Rows)
				{
					if (Qlisting.NumRegs > 0 && i >= Qlisting.NumRegs) // Copiado da versão antiga do RowsToViewModels
						break;
					Elements.Add(MapSTY_Menu_IMGBACKGROUND(row));
					i++;
				}
			}
		...
		/// to a STY_Menu_IMGBACKGROUND_RowViewModel object.
		/// </summary>
		/// <param name="row">The row.</param>
		private STY_Menu_IMGBACKGROUND_RowViewModel MapSTY_Menu_IMGBACKGROUND(CSGenioAwpess row)
		{
			var model = new STY_Menu_IMGBACKGROUND_RowViewModel(m_userContext, true, _fieldsToSerialize);
		...
		/// </summary>
		public void CalculateButtonPermissions(STY_Menu_IMGBACKGROUND_RowViewModel model)
		{
			bool canView = true;
			bool canEdit = true;
			bool canDelete = true;
		...
		#region Custom code
// USE /[MANUAL GQT VIEWMODEL_CUSTOM STY_MENU_IMGBACKGROUND]/
		#endregion

		private static readonly string[] _fieldsToSerialize =
		[
			"Wpess", "Wpess.ValCodpess", "Wpess.ValZzstate", "Wpess.ValName", "Wpess.ValDate", "Wpess.ValSex", "Wpess.ValNfunc", "Wpess.ValAdress", "Wpess.ValZipcode", "Wpess.ValCountry", "Wpess.ValEmail", "Wpess.ValCellphon", "Wpess.ValNaturali", "Wpess.ValNacional", "Wareh", "Wareh.ValWarehdes", "Wpess.ValFtbackgr", "Wpess.ValCodwareh", "BtnPermission"
		];
