...
public class ChangeCountryNameTool : McpTool
{
    ...
    public override McpSchemaBase InputSchema => new McpSchemaBase()
    {
        Properties = new Dictionary<string, McpProperty>()
        {
            ...
            { "primaryKey", new McpProperty() {
                Type = "string",
                Format = "uuid",
                Description = "The primary key of the record to be read"
                }
            },
        },
    ...
    public override object Execute(PersistentSupport sp, User user, JsonElement input)
    {
        ...
        if (records.Count != 1)
        {
            string msg = $"Record with primary key {key} not found.";
        ...
        if (input.TryGetProperty("primaryKey", out var propprimaryKey))
            record.ValCodcntry = propprimaryKey.GetString();
        record.update(sp);
        return new
        {
            content = new[]
