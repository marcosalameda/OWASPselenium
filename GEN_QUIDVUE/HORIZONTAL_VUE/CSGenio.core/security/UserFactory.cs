...

{
    public class UserFactory
    {
		...
		/// <summary>
        /// Creates the access level to each model specified by the user registration that uses the REGIS form.
        /// </summary>
        /// <param name="user">A userlogin (psw) record</param>
        public void CreateUser_REGIS(CSGenioApsw user)
        {
			...
			ua.ValSistema = "GQT";
			ua.ValModulo = "GQT";
            ua.ValRole = Role.ROLE_1.Id;
            // We have to mark this to keep compatibility with Backoffice shared applications.
            if (Role.ROLE_1.Type == RoleType.LEVEL)
                ua.ValNivel = Role.ROLE_1.GetLevelInt();
			ua.insert(sp);
		}
