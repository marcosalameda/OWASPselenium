				...
				<q-col
					...
					<template
						...
						<component
							...
							size="block"
							:classes="[{ 'q-field--invalid': displayErrors && getFilterValueErrorMessages(filterIdx, valueIdx).length > 0 }]"
							:table-name="`${tableName}_filters`"
							:column-name="`${filterIdx}_${valueIdx}`"
							:row-index="filterIdx"
							:options="getValueOptions(filter)"
							:value="filter.values[valueIdx]"
							:placeholder="getFilterPlaceholder(filter) || texts.fieldRequiredInfo"
							:texts="texts"
							:locale="locale"
					...
					<q-col
						...
						<template
							...
							<component
								...
								size="block"
								:classes="[{ 'q-field--invalid': displayErrors && getFilterValueErrorMessages(filterIdx, valueIdx, subFilterIdx).length > 0 }]"
								:table-name="`${tableName}_filters`"
								...
								:placeholder="getFilterPlaceholder(subFilter) || texts.fieldRequiredInfo"
								:texts="texts"
								:locale="locale"
...
<script>
	...
	export default {
		...
		props: {
			...
			/**
			 * Whether to display the invalid state errors.
			 */
			displayErrors: {
				type: Boolean,
				default: false
						...
						for (const valueIdx in conditionState.valueStates)
						{
							...
							if (valueState !== 'VALID')
							{
								this.validationErrors.push({
									...
									message: this.texts.fieldIsRequired.replace('{0}', '')
								})
							}
						}
					}
