<template>
	<teleport
		v-if="isReady"
		to="#q-modal-see-more-equip-item-gitem-itemdes-body">
		<q-row>
			<q-table
				v-bind="listCtrl"
...
<script>
	...
	const requiredTextResources = ['EQUIP_ITEM__GITEM__ITEMDES_SeeMore', 'hardcoded', 'messages']

	export default {
		name: 'EquipItemGitemItemdesSeeMore',
		...
		data()
		{
			return {
				...
				componentOnLoadProc: asyncProcM.getProcListMonitor('EQUIP_ITEM__GITEM__ITEMDES_SeeMore', false),

				interfaceMetadata: {
					id: 'EQUIP_ITEM__GITEM__ITEMDES_SeeMore', // Used for resources
					requiredTextResources
				},

				menuInfo: {
					acronym: 'EQUIP_ITEM__GITEM__ITEMDES_SeeMore',
					name: 'EQUIP_ITEM__GITEM__ITEMDES_SeeMore',
					controller: 'ITEM',
					action: 'EQUIP_ITEM_GitemValItemdes'
				},
		...
		mounted()
		{
			...
			const modalProps = {
				id: 'see-more-equip-item-gitem-itemdes',
				dismissAction: this.close,
				returnElement: 'EQUIP_ITEM__GITEM__ITEMDES_see-more_button'
			}
			const props = {
				class: 'q-dialog-see-more',
				title: computed(() => this.Resources.GLOBAL_ARTICLES62131),
				buttons: [
					{
		...
		beforeUnmount()
		{
			...
			removeModal('see-more-equip-item-gitem-itemdes')
		},
			...
			getListConfig()
			{
				...
				const listProps = {
					configuration: {
						...
						config: {
							name: 'Equip_item_GitemValItemdes',
							serverMode: true,
						...
						uuid: 'Equip_item_Equip_item_GitemValItemdes',
