<template>
	<teleport
		v-if="isReady"
		to="#q-modal-see-more-equip-item-pess1-name-fg-body">
		<q-row>
			<q-table
				v-bind="listCtrl"
...
<script>
	...
	const requiredTextResources = ['EQUIP_ITEM__PESS1__NAME_FG_SeeMore', 'hardcoded', 'messages']

	export default {
		name: 'EquipItemPess1NameFgSeeMore',
		...
		data()
		{
			return {
				...
				componentOnLoadProc: asyncProcM.getProcListMonitor('EQUIP_ITEM__PESS1__NAME_FG_SeeMore', false),

				interfaceMetadata: {
					id: 'EQUIP_ITEM__PESS1__NAME_FG_SeeMore', // Used for resources
					requiredTextResources
				},

				menuInfo: {
					acronym: 'EQUIP_ITEM__PESS1__NAME_FG_SeeMore',
					name: 'EQUIP_ITEM__PESS1__NAME_FG_SeeMore',
					controller: 'ITEM',
					action: 'EQUIP_ITEM_ValName'
				},
		...
		mounted()
		{
			...
			const modalProps = {
				id: 'see-more-equip-item-pess1-name-fg',
				dismissAction: this.close,
				returnElement: 'EQUIP_ITEM__PESS1__NAME_FG_see-more_button'
			}
			const props = {
		...
		beforeUnmount()
		{
			...
			removeModal('see-more-equip-item-pess1-name-fg')
		},
			...
			getListConfig()
			{
				...
				const listProps = {
					configuration: {
						...
						uuid: 'Equip_item_Equip_item_ValName',
