<template>
	<teleport
		v-if="isReady"
		to="#q-modal-see-more-externo-cmpnydesignat-body">
		<q-row-container>
			<q-table
				v-bind="listCtrl"
...
<script>
	...
	const requiredTextResources = ['EXTERNO_CMPNYDESIGNAT_SeeMore', 'hardcoded', 'messages']

	export default {
		name: 'ExternoCmpnydesignatSeeMore',
		...
		data()
		{
			return {
				...
				componentOnLoadProc: asyncProcM.getProcListMonitor('EXTERNO_CMPNYDESIGNAT_SeeMore', false),

				interfaceMetadata: {
					id: 'EXTERNO_CMPNYDESIGNAT_SeeMore', // Used for resources
					requiredTextResources
				},

				menuInfo: {
					acronym: 'EXTERNO_CMPNYDESIGNAT_SeeMore',
					name: 'EXTERNO_CMPNYDESIGNAT_SeeMore',
					controller: 'PESSO',
					action: 'EXTERNO_CmpnyValDesignat'
				},
		...
		mounted()
		{
			...
			const modalProps = {
				id: 'see-more-externo-cmpnydesignat',
				...
				isActive: true,
				returnElement: 'EXTERNO_CMPNYDESIGNAT_see-more_button'
			}
		...
		beforeUnmount()
		{
			...
			removeModal('see-more-externo-cmpnydesignat')
		},
			...
			getListConfig()
			{
				...
				const listProps = {
					configuration: {
						...
						globalEvents: ['changed-CNTRY', 'changed-CMPNY'],
						uuid: 'Externo_Externo_CmpnyValDesignat',
