<template>
	<teleport
		v-if="isReady"
		to="#q-modal-see-more-pesso-categcategory-body">
		<q-row>
			<q-table
				v-bind="listCtrl"
...
<script>
	...
	const requiredTextResources = ['PESSO___CATEGCATEGORY_SeeMore', 'hardcoded', 'messages']

	export default {
		name: 'PessoCategcategorySeeMore',
		...
		data()
		{
			return {
				...
				componentOnLoadProc: asyncProcM.getProcListMonitor('PESSO___CATEGCATEGORY_SeeMore', false),

				interfaceMetadata: {
					id: 'PESSO___CATEGCATEGORY_SeeMore', // Used for resources
					requiredTextResources
				},

				menuInfo: {
					acronym: 'PESSO___CATEGCATEGORY_SeeMore',
					name: 'PESSO___CATEGCATEGORY_SeeMore',
					controller: 'PESSO',
					action: 'PESSO_CategValCategoria'
				},
		...
		mounted()
		{
			...
			const modalProps = {
				id: 'see-more-pesso-categcategory',
				dismissAction: this.close,
				returnElement: 'PESSO___CATEGCATEGORY_see-more_button'
			}
			const props = {
		...
		beforeUnmount()
		{
			...
			removeModal('see-more-pesso-categcategory')
		},
			...
			getListConfig()
			{
				...
				const listProps = {
					configuration: {
						...
						uuid: 'Pesso_Pesso_CategValCategoria',
