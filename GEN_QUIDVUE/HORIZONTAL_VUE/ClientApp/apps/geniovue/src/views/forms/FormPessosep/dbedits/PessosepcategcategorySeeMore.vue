<template>
	<teleport
		v-if="isReady"
		to="#q-modal-see-more-pessosepcategcategory-body">
		<q-row>
			<q-table
				v-bind="listCtrl"
...
<script>
	...
	const requiredTextResources = ['PESSOSEPCATEGCATEGORY_SeeMore', 'hardcoded', 'messages']

	export default {
		name: 'PessosepcategcategorySeeMore',
		...
		data()
		{
			return {
				...
				componentOnLoadProc: asyncProcM.getProcListMonitor('PESSOSEPCATEGCATEGORY_SeeMore', false),

				interfaceMetadata: {
					id: 'PESSOSEPCATEGCATEGORY_SeeMore', // Used for resources
					requiredTextResources
				},

				menuInfo: {
					acronym: 'PESSOSEPCATEGCATEGORY_SeeMore',
					name: 'PESSOSEPCATEGCATEGORY_SeeMore',
					controller: 'PESSO',
					action: 'PESSOSEP_CategValCategoria'
				},
		...
		mounted()
		{
			...
			const modalProps = {
				id: 'see-more-pessosepcategcategory',
				dismissAction: this.close,
				returnElement: 'PESSOSEPCATEGCATEGORY_see-more_button'
			}
			const props = {
				class: 'q-dialog-see-more',
				title: computed(() => this.Resources.PROFESSIONAL_CATEGOR49666),
				buttons: [
					{
		...
		beforeUnmount()
		{
			...
			removeModal('see-more-pessosepcategcategory')
		},
			...
			getListConfig()
			{
				...
				const listProps = {
					configuration: {
						...
						config: {
							name: 'Pessosep_CategValCategoria',
							serverMode: true,
							...
							formsDefinition: {
								'CATEG': {
									fnKeySelector: (row) => row.Fields.ValCodcateg,
									isPopup: false
								},
							},
						...
						uuid: 'Pessosep_Pessosep_CategValCategoria',
