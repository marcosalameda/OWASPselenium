<template>
	<teleport
		v-if="isReady"
		to="#q-modal-see-more-regra-up-rules-descript-body">
		<q-row>
			<q-table
				v-bind="listCtrl"
...
<script>
	...
	const requiredTextResources = ['REGRA__UP_RULES__DESCRIPT_SeeMore', 'hardcoded', 'messages']

	export default {
		name: 'RegraUpRulesDescriptSeeMore',
		...
		data()
		{
			return {
				...
				componentOnLoadProc: asyncProcM.getProcListMonitor('REGRA__UP_RULES__DESCRIPT_SeeMore', false),

				interfaceMetadata: {
					id: 'REGRA__UP_RULES__DESCRIPT_SeeMore', // Used for resources
					requiredTextResources
				},

				menuInfo: {
					acronym: 'REGRA__UP_RULES__DESCRIPT_SeeMore',
					name: 'REGRA__UP_RULES__DESCRIPT_SeeMore',
					controller: 'RULES',
					action: 'REGRA_Up_rulesValDescript'
				},
		...
		mounted()
		{
			...
			const modalProps = {
				id: 'see-more-regra-up-rules-descript',
				dismissAction: this.close,
				returnElement: 'REGRA__UP_RULES__DESCRIPT_see-more_button'
			}
			const props = {
		...
		beforeUnmount()
		{
			...
			removeModal('see-more-regra-up-rules-descript')
		},
			...
			getListConfig()
			{
				...
				const listProps = {
					configuration: {
						...
						config: {
							name: 'Regra_Up_rulesValDescript',
							serverMode: true,
						...
						uuid: 'Regra_Regra_Up_rulesValDescript',
