<template>
	<teleport
		v-if="isReady"
		to="#q-modal-see-more-regra2-up-rules-descript-body">
		<q-row>
			<q-table
				v-bind="listCtrl"
...
<script>
	...
	const requiredTextResources = ['REGRA2__UP_RULES__DESCRIPT_SeeMore', 'hardcoded', 'messages']

	export default {
		name: 'Regra2UpRulesDescriptSeeMore',
		...
		data()
		{
			return {
				...
				componentOnLoadProc: asyncProcM.getProcListMonitor('REGRA2__UP_RULES__DESCRIPT_SeeMore', false),

				interfaceMetadata: {
					id: 'REGRA2__UP_RULES__DESCRIPT_SeeMore', // Used for resources
					requiredTextResources
				},

				menuInfo: {
					acronym: 'REGRA2__UP_RULES__DESCRIPT_SeeMore',
					name: 'REGRA2__UP_RULES__DESCRIPT_SeeMore',
					controller: 'RULES',
					action: 'REGRA2_Up_rulesValDescript'
				},
		...
		mounted()
		{
			...
			const modalProps = {
				id: 'see-more-regra2-up-rules-descript',
				dismissAction: this.close,
				returnElement: 'REGRA2__UP_RULES__DESCRIPT_see-more_button'
			}
			const props = {
				class: 'q-dialog-see-more',
				title: computed(() => this.Resources.RULES28427),
				buttons: [
					{
		...
		beforeUnmount()
		{
			...
			removeModal('see-more-regra2-up-rules-descript')
		},
			...
			getListConfig()
			{
				...
				const listProps = {
					configuration: {
						...
						config: {
							name: 'Regra2_Up_rulesValDescript',
							serverMode: true,
							pkColumn: 'ValCodup_rules',
							tableAlias: 'UP_RULES',
							...
							formsDefinition: {
								'UP_RULES': {
									fnKeySelector: (row) => row.Fields.ValCodup_rules,
									isPopup: false
								},
							},
						...
						globalEvents: ['changed-UP_RULES'],
						internalEvents: ['fieldChange:up_rules.descript'],
						uuid: 'Regra2_Regra2_Up_rulesValDescript',
