<template>
	<teleport
		v-if="isReady"
		to="#q-modal-see-more-relin-produproduct-body">
		<q-row>
			<q-table
				v-bind="listCtrl"
...
<script>
	...
	const requiredTextResources = ['RELIN___PRODUPRODUCT__SeeMore', 'hardcoded', 'messages']

	export default {
		name: 'RelinProduproductSeeMore',
		...
		data()
		{
			return {
				...
				componentOnLoadProc: asyncProcM.getProcListMonitor('RELIN___PRODUPRODUCT__SeeMore', false),

				interfaceMetadata: {
					id: 'RELIN___PRODUPRODUCT__SeeMore', // Used for resources
					requiredTextResources
				},

				menuInfo: {
					acronym: 'RELIN___PRODUPRODUCT__SeeMore',
					name: 'RELIN___PRODUPRODUCT__SeeMore',
					controller: 'RELIN',
					action: 'RELIN_ProduValProduct'
				},
		...
		mounted()
		{
			...
			const modalProps = {
				id: 'see-more-relin-produproduct',
				dismissAction: this.close,
				returnElement: 'RELIN___PRODUPRODUCT__see-more_button'
			}
			const props = {
				class: 'q-dialog-see-more',
				title: computed(() => this.Resources.PRODUCTS34689),
				buttons: [
					{
		...
		beforeUnmount()
		{
			...
			removeModal('see-more-relin-produproduct')
		},
			...
			getListConfig()
			{
				...
				const listProps = {
					configuration: {
						...
						globalEvents: ['changed-PRODU', 'changed-LCEXT', 'changed-LOCAT'],
						uuid: 'Relin_Relin_ProduValProduct',
