...
/* eslint-enable indent, vue/html-indent, vue/script-indent */
		...
		{
/* eslint-disable indent, vue/html-indent, vue/script-indent */
// USE /[MANUAL GQT COMPONENT_BEFORE_UNMOUNT PTN_MENU_LIST_DB_TR_F]/
// eslint-disable-next-line
/* eslint-enable indent, vue/html-indent, vue/script-indent */
		},
		...
		methods: {
			...
			async PTN_MenuTR_TRIGGER_MENU2_TRIGMENU2_1(id)
			{
				try
				{
					...
					if (!this.isEmpty(result))
					{
						if (result !== 'error')
						{
							...
							return this.PTN_MenuTR_TRIGGER_MENU2_TRIGMENU2_2(id)
						}
						else
					...
					else
					{
						this.$eventTracker.addError({
							origin: 'Trigger TRIGMENU2',
							message: 'Routine "PTN_MenuTR_TRIGGER_MENU2" finished execution with an unknown result type: ' + data.success
						})
					}
				}
				catch (e)
				{
					...
					this.$eventTracker.addError({
						origin: 'Trigger TRIGMENU2 (catch)',
						message: e.toString()
					})
				}
			},
