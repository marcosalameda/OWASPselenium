<template>
	<teleport
		v-if="isReady"
		to="#q-modal-see-more-leafletdequipregistnr-body">
		<q-row-container>
			<q-table
				v-bind="listCtrl"
...
<script>
	...
	const requiredTextResources = ['LEAFLETDEQUIPREGISTNR_SeeMore', 'hardcoded', 'messages']

	export default {
		name: 'LeafletdequipregistnrSeeMore',
		...
		data()
		{
			return {
				...
				componentOnLoadProc: asyncProcM.getProcListMonitor('LEAFLETDEQUIPREGISTNR_SeeMore', false),

				interfaceMetadata: {
					id: 'LEAFLETDEQUIPREGISTNR_SeeMore', // Used for resources
					requiredTextResources
				},

				menuInfo: {
					acronym: 'LEAFLETDEQUIPREGISTNR_SeeMore',
					name: 'LEAFLETDEQUIPREGISTNR_SeeMore',
					controller: 'INSTA',
					action: 'LEAFLETD_EquipValRegistnr'
				},
		...
		mounted()
		{
			...
			const modalProps = {
				id: 'see-more-leafletdequipregistnr',
				...
				isActive: true,
				returnElement: 'LEAFLETDEQUIPREGISTNR_lookup_see-more_button'
			}
		...
		beforeUnmount()
		{
			...
			genericFunctions.removeModal('see-more-leafletdequipregistnr')
		},
			...
			getListConfig()
			{
				...
				const listProps = {
					configuration: {
						...
						config: {
							name: 'Leafletd_EquipValRegistnr',
							serverMode: true,
						...
						uuid: 'Leafletd_Leafletd_EquipValRegistnr',
