<template>
	<teleport
		v-if="isReady"
		to="#q-modal-see-more-leaflettequipregistnr-body">
		<q-row-container>
			<q-table
				v-bind="listCtrl"
...
<script>
	...
	const requiredTextResources = ['LEAFLETTEQUIPREGISTNR_SeeMore', 'hardcoded', 'messages']

	export default {
		name: 'LeaflettequipregistnrSeeMore',
		...
		data()
		{
			return {
				...
				componentOnLoadProc: asyncProcM.getProcListMonitor('LEAFLETTEQUIPREGISTNR_SeeMore', false),

				interfaceMetadata: {
					id: 'LEAFLETTEQUIPREGISTNR_SeeMore', // Used for resources
					requiredTextResources
				},

				menuInfo: {
					acronym: 'LEAFLETTEQUIPREGISTNR_SeeMore',
					name: 'LEAFLETTEQUIPREGISTNR_SeeMore',
					controller: 'INSTA',
					action: 'LEAFLETT_EquipValRegistnr'
				},
		...
		mounted()
		{
			...
			const modalProps = {
				id: 'see-more-leaflettequipregistnr',
				...
				isActive: true,
				returnElement: 'LEAFLETTEQUIPREGISTNR_lookup_see-more_button'
			}
		...
		beforeUnmount()
		{
			...
			genericFunctions.removeModal('see-more-leaflettequipregistnr')
		},
			...
			getListConfig()
			{
				...
				const listProps = {
					configuration: {
						...
						config: {
							name: 'Leaflett_EquipValRegistnr',
							serverMode: true,
						...
						uuid: 'Leaflett_Leaflett_EquipValRegistnr',
