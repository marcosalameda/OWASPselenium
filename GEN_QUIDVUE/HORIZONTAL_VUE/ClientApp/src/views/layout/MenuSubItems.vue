...
<script>
	...
	export default {
		...
		expose: ['menu', 'openMenu', 'closeMenu', 'toggleMenu', 'changeMenu'],
		...
		mounted()
		{
			...
			watchEffect(() => {
				if (!this.isEmpty(this.menuPath) && this.menuIsOpen(this.menu) && this.level === 0 && this.hasDoubleNavbar)
		...
		methods: {
			...
			/*
			 * Called when opening a sub-menu
			 */
			changeMenu(menu)
			{
				//Open the selected sub-menu and close all other sub-menus
				for(let key in this.$refs.menuSubItem)
				{
					let curMenuComponent = this.$refs?.menuSubItem[key]
					let curMenu = curMenuComponent?.menu
					
					if(curMenu?.Id === menu?.Id)
						curMenuComponent.openMenu()
					else
						curMenuComponent.closeMenu()
				}
			},
		...
		watch: {
			showSubMenu(val)
			{
				if (val)
				{
					//Signal that this menu is being shown
					this.$emit('change-menu', this.menu)
				}
			}
		}
