...
public class FormOperationsTest : BaseSeleniumTest
{
    ...
    /*
    * Deactivated temporarily because it keeps failing despite no issues with the procedure. Always works when done locally.
    * The test is also not ensuring its initial state (if it fails, on the next execution Sunset.jpg will already be in the field, so the first part of the test always passes)
    */
    /*
    [Test]
    ...
    */
    ...
    public void Grid()
    {
        ...
        var menu = new MenuListPage(Driver, "PTN", "EDITABLETABLELIST");
        menu.List.Search.Search("a");
