...
// USE /[MANUAL GQT MANUAL_CONTROLLER AGENT]/
		...
		/// <returns></returns>
		[HttpPost]
		public JsonResult RecalculateFormulas_Agent01([FromBody]Agent01_ViewModel formData)
		{
			return GenericRecalculateFormulas(formData, "agent",
				(primaryKey) => Models.Agent.Find(primaryKey, UserContext.Current, "FAGENT01"),
		...
		/// <returns></returns>
		[HttpPost]
		public JsonResult RecalculateFormulas_Agent02([FromBody]Agent02_ViewModel formData)
		{
			return GenericRecalculateFormulas(formData, "agent",
				(primaryKey) => Models.Agent.Find(primaryKey, UserContext.Current, "FAGENT02"),
				(model) => formData.MapToModel(model as Models.Agent)
			);
		}
		...
		/// <returns></returns>
		[HttpPost]
		public JsonResult RecalculateFormulas_Agent05([FromBody]Agent05_ViewModel formData)
		{
			return GenericRecalculateFormulas(formData, "agent",
				(primaryKey) => Models.Agent.Find(primaryKey, UserContext.Current, "FAGENT05"),
				(model) => formData.MapToModel(model as Models.Agent)
			);
		}
