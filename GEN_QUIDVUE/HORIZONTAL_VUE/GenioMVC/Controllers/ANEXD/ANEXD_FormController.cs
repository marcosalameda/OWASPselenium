...
// USE /[MANUAL GQT CONTROLLER_SHOW ANEXD]/
		...
		public ActionResult Anexd_Show_GET([FromBody] RequestIdModel requestModel)
		{
			...
			Anexd_ViewModel model = new(UserContext.Current);
...
// USE /[MANUAL GQT CONTROLLER_DELETE_POST ANEXD]/
		...
		{
			string id = requestModel.Id;
			Anexd_ViewModel model = new(UserContext.Current, id);
			model.MapFromModel();
...
// USE /[MANUAL GQT CONTROLLER_CANCEL_GET ANEXD]/
		public ActionResult Anexd_Cancel()
		{
			if (Navigation.CurrentLevel.FormMode == FormMode.New || Navigation.CurrentLevel.FormMode == FormMode.Duplicate)
			{
				...
				try
				{
					var recordKey = Navigation.GetStrValue("anexd");
					var model = GenioMVC.Models.Anexd.Find(recordKey, UserContext.Current);
...
// USE /[MANUAL GQT AFTER_CANCEL ANEXD]/
		...
		public ActionResult Anexd_EquipValRegistnr([FromBody] Anexd_EquipValRegistnrModel requestModel)
		{
			...
			Models.Anexd parentCtx = requestModel.Model == null ? null : new(m_userContext);
			requestModel.Model?.Init(m_userContext);
			requestModel.Model?.MapToModel(parentCtx);
			Anexd_EquipValRegistnr_ViewModel model = new(m_userContext, parentCtx);
		...
		public ActionResult Anexd_LanguValLangua([FromBody] Anexd_LanguValLanguaModel requestModel)
		{
			...
			requestModel.Model?.Init(m_userContext);
			requestModel.Model?.MapToModel(parentCtx);
			Anexd_LanguValLangua_ViewModel model = new(m_userContext, parentCtx);
...
// USE /[MANUAL GQT AFTER_APPLY_EDIT ANEXD]/
		...
		/// <returns>A JSON response with the result of the operation</returns>
		public ActionResult SetFileAnexdDocument([FromForm] RequestDocumsCreateModel requestModel)
		{
