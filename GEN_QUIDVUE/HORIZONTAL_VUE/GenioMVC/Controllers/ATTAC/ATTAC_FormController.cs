...
// USE /[MANUAL GQT CONTROLLER_SHOW ATTAC]/
		...
		public ActionResult Attac_Show_GET([FromBody] RequestIdModel requestModel)
		{
			...
			Attac_ViewModel model = new(UserContext.Current);
...
// USE /[MANUAL GQT CONTROLLER_DELETE_POST ATTAC]/
		...
		{
			string id = requestModel.Id;
			Attac_ViewModel model = new(UserContext.Current, id);
			model.MapFromModel();
...
// USE /[MANUAL GQT CONTROLLER_CANCEL_GET ATTAC]/
		public ActionResult Attac_Cancel()
		{
			if (Navigation.CurrentLevel.FormMode == FormMode.New || Navigation.CurrentLevel.FormMode == FormMode.Duplicate)
			{
				...
				try
				{
					var recordKey = Navigation.GetStrValue("attac");
					var model = GenioMVC.Models.Attac.Find(recordKey, UserContext.Current);
...
// USE /[MANUAL GQT AFTER_CANCEL ATTAC]/
		...
		public ActionResult Attac_AssetValName([FromBody] Attac_AssetValNameModel requestModel)
		{
			...
			Models.Attac parentCtx = requestModel.Model == null ? null : new(m_userContext);
			requestModel.Model?.Init(m_userContext);
			requestModel.Model?.MapToModel(parentCtx);
			Attac_AssetValName_ViewModel model = new(m_userContext, parentCtx);
...
// USE /[MANUAL GQT AFTER_APPLY_EDIT ATTAC]/
		...
		/// <returns>A JSON response with the result of the operation</returns>
		public ActionResult SetFileAttacDocument([FromForm] RequestDocumsCreateModel requestModel)
		{
