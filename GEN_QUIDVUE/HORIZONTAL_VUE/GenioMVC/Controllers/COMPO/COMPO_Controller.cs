...
namespace GenioMVC.Controllers
{
	public partial class CompoController : ControllerBase
	{
		...
		public CompoController(UserContextService userContext, IChatbotService aiService) : base(userContext)
		{
			_aiService = aiService;
		}

// USE /[MANUAL GQT CONTROLLER_NAVIGATION COMPO]/



		private List<string> GetActionIds(CriteriaSet crs, CSGenio.persistence.PersistentSupport sp = null)
		{
			CSGenio.business.Area area = CSGenio.business.Area.createArea<CSGenioAcompo>(UserContext.Current.User, UserContext.Current.User.CurrentModule);
			return base.GetActionIds(crs, sp, area);
		}

// USE /[MANUAL GQT MANUAL_CONTROLLER COMPO]/
		...
		public JsonResult ReloadDBEdit([FromBody]RequestReloadDBEditModel requestModel)
		{
			...
			*/
			Models.Compo row = new Models.Compo(UserContext.Current, isEmpty: true);
			row.klass.QPrimaryKey = Navigation.GetStrValue("compo");
			...
			try
			{
				...
				{
					case "COMPTAB_COMPCCOMPCLAS":	// Field (DB)
						{
							var model = new Comptype_ViewModel(UserContext.Current) { editable = false };
							model.MapFromModel(row);
							model.Load_Comptab_compccompclas(qs);
							result = model.TableCompcCompclas;
						}
						break;
					default:
						break;
		...
		public JsonResult GetDependants([FromBody]RequestDependantsModel requestModel)
		{
			...
			try
			{
				...
				{
					case "COMPTAB_COMPCCOMPCLAS":	// Field (DB)
						values = new Comptype_ViewModel(UserContext.Current).GetDependant_ComptabTableCompcCompclas(Selected);
						break;
					default: break;
				}
		...
		// POST: /Compo/COMPTYPE_COMPTYPE__PSEUD__CONFIGCOMP_ShowWhen
		[HttpPost]
		public JsonResult COMPTYPE_COMPTYPE__PSEUD__CONFIGCOMP_ShowWhen([FromBody] ViewModels.Compo.Comptype_ViewModel formData)
		{
			try
			{
				// Create a model from form data to avoid extra database queries.
				var p = new Models.Compo(UserContext.Current);
				...
				// Formula: HasRole("ADMINISTRATOR")
				var result = CSGenio.business.GlobalFunctions.HasRole(m_userContext.User,"ADMINISTRATOR");
		...
		/// <summary>
		/// Recalculate formulas of the "Comptype" form. (++, CT, SR, CL and U1)
		/// </summary>
		/// <param name="formData">Current form data</param>
		/// <returns></returns>
		[HttpPost]
		public JsonResult RecalculateFormulas_Comptype([FromBody]Comptype_ViewModel formData)
		{
			return GenericRecalculateFormulas(formData, "compo",
				(primaryKey) => Models.Compo.Find(primaryKey, UserContext.Current, "FCOMPTYPE"),
				(model) => formData.MapToModel(model as Models.Compo)
			);
		}
		...
		{
			return base.GetDocumsTickets("COMPO", requestModel.FieldName, requestModel.KeyValue);
		}
