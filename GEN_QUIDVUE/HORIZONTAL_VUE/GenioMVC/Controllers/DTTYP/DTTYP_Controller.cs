...
namespace GenioMVC.Controllers
{
	public partial class DttypController : ControllerBase
	{
		...
		public DttypController(UserContextService userContext, IChatbotService aiService) : base(userContext)
		{
			_aiService = aiService;
		}

// USE /[MANUAL GQT CONTROLLER_NAVIGATION DTTYP]/



		private List<string> GetActionIds(CriteriaSet crs, CSGenio.persistence.PersistentSupport sp = null)
		{
			CSGenio.business.Area area = CSGenio.business.Area.createArea<CSGenioAdttyp>(UserContext.Current.User, UserContext.Current.User.CurrentModule);
...
// USE /[MANUAL GQT MANUAL_CONTROLLER DTTYP]/
		...
		/// <returns></returns>
		[HttpPost]
		public JsonResult RecalculateFormulas_Dttyp([FromBody]Dttyp_ViewModel formData)
		{
			return GenericRecalculateFormulas(formData, "dttyp",
				(primaryKey) => Models.Dttyp.Find(primaryKey, UserContext.Current, "FDTTYP"),
				(model) => formData.MapToModel(model as Models.Dttyp)
			);
		}
		...
		{
			return base.GetDocumsTickets("DTTYP", requestModel.FieldName, requestModel.KeyValue);
		}
