...
namespace GenioMVC.Controllers
{
	public partial class EntidadeController : ControllerBase
	{
		...
		public EntidadeController(UserContextService userContext, IChatbotService aiService) : base(userContext)
		{
			_aiService = aiService;
		}

// USE /[MANUAL GQT CONTROLLER_NAVIGATION ENTIDADE]/



		private List<string> GetActionIds(CriteriaSet crs, CSGenio.persistence.PersistentSupport sp = null)
		{
			CSGenio.business.Area area = CSGenio.business.Area.createArea<CSGenioAentidade>(UserContext.Current.User, UserContext.Current.User.CurrentModule);
			return base.GetActionIds(crs, sp, area);
		}

// USE /[MANUAL GQT MANUAL_CONTROLLER ENTIDADE]/
		...
		public JsonResult ReloadDBEdit([FromBody]RequestReloadDBEditModel requestModel)
		{
			...
			*/
			Models.Entidade row = new Models.Entidade(UserContext.Current, isEmpty: true);
			row.klass.QPrimaryKey = Navigation.GetStrValue("entidade");
			...
			try
			{
				...
				{
					case "ENTIDADE__CONCELHO__NOME":	// Field (DB)
						{
							var model = new Entidade_ViewModel(UserContext.Current) { editable = false };
							model.MapFromModel(row);
							model.Load_Entidade__concelho__nome(qs);
							result = model.TableConcelhoNome;
						}
						break;
					default:
						break;
		...
		public JsonResult GetDependants([FromBody]RequestDependantsModel requestModel)
		{
			...
			try
			{
				...
				{
					case "ENTIDADE__CONCELHO__NOME":	// Field (DB)
						values = new Entidade_ViewModel(UserContext.Current).GetDependant_EntidadeTableConcelhoNome(Selected);
						break;
					default: break;
				}
		...
		/// <summary>
		/// Recalculate formulas of the "Entidade" form. (++, CT, SR, CL and U1)
		/// </summary>
		/// <param name="formData">Current form data</param>
		/// <returns></returns>
		[HttpPost]
		public JsonResult RecalculateFormulas_Entidade([FromBody]Entidade_ViewModel formData)
		{
			return GenericRecalculateFormulas(formData, "entidade",
				(primaryKey) => Models.Entidade.Find(primaryKey, UserContext.Current, "FENTIDADE"),
				(model) => formData.MapToModel(model as Models.Entidade)
			);
		}
		...
		{
			return base.GetDocumsTickets("ENTIDADE", requestModel.FieldName, requestModel.KeyValue);
		}
