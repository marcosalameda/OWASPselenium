...
// USE /[MANUAL GQT CONTROLLER_SHOW ACCORDI]/
		...
		public ActionResult Accordi_Show_GET([FromBody] RequestIdModel requestModel)
		{
			...
			Accordi_ViewModel model = new(UserContext.Current);
...
// USE /[MANUAL GQT CONTROLLER_DELETE_POST ACCORDI]/
		...
		{
			string id = requestModel.Id;
			Accordi_ViewModel model = new(UserContext.Current, id);
			model.MapFromModel();
...
// USE /[MANUAL GQT AFTER_CANCEL ACCORDI]/
		...
		public ActionResult Accordi_CmpnyValDesignat([FromBody] Accordi_CmpnyValDesignatModel requestModel)
		{
			...
			Accordi_CmpnyValDesignat_ViewModel model = new(m_userContext, parentCtx);
		...
		public ActionResult Accordi_Pess1ValName([FromBody] Accordi_Pess1ValNameModel requestModel)
		{
			...
			requestModel.Model?.Init(m_userContext);
			requestModel.Model?.MapToModel(parentCtx);
			Accordi_Pess1ValName_ViewModel model = new(m_userContext, parentCtx);

			CSGenio.core.framework.table.legacy.v1.TableConfigurationUpdate.SetFilterShiftValue(model.Uuid, "filter_Pess1ValName_", -1);
			CSGenio.core.framework.table.legacy.v1.TableConfigurationUpdate.SetFilterShiftValue(model.Uuid, "filter_Pess1ValName_", -2);
		...
		public ActionResult Accordi_ValInstalag([FromBody] Accordi_ValInstalagModel requestModel)
		{
			...
			requestModel.Model?.Init(m_userContext);
			requestModel.Model?.MapToModel(parentCtx);
			Accordi_ValInstalag_ViewModel model = new(m_userContext, parentCtx);
		...
		public ActionResult Accordi_ValInstalac([FromBody] Accordi_ValInstalacModel requestModel)
		{
			...
			requestModel.Model?.Init(m_userContext);
			requestModel.Model?.MapToModel(parentCtx);
			Accordi_ValInstalac_ViewModel model = new(m_userContext, parentCtx);
		...
		public ActionResult Accordi_ValReparaco([FromBody] Accordi_ValReparacoModel requestModel)
		{
			...
			requestModel.Model?.Init(m_userContext);
			requestModel.Model?.MapToModel(parentCtx);
			Accordi_ValReparaco_ViewModel model = new(m_userContext, parentCtx);
