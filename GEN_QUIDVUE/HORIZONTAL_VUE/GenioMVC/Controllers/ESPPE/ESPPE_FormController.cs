...
// USE /[MANUAL GQT CONTROLLER_SHOW ESPPE]/
		...
		public ActionResult Esppe_Show_GET([FromBody] RequestIdModel requestModel)
		{
			...
			Esppe_ViewModel model = new(UserContext.Current);
...
// USE /[MANUAL GQT CONTROLLER_DELETE_POST ESPPE]/
		...
		{
			string id = requestModel.Id;
			Esppe_ViewModel model = new(UserContext.Current, id);
			model.MapFromModel();
...
// USE /[MANUAL GQT CONTROLLER_CANCEL_GET ESPPE]/
		public ActionResult Esppe_Cancel()
		{
			if (Navigation.CurrentLevel.FormMode == FormMode.New || Navigation.CurrentLevel.FormMode == FormMode.Duplicate)
			{
				...
				try
				{
					var recordKey = Navigation.GetStrValue("esppe");
					var model = GenioMVC.Models.Esppe.Find(recordKey, UserContext.Current);
...
// USE /[MANUAL GQT AFTER_CANCEL ESPPE]/
		...
		public ActionResult Esppe_PessoValName([FromBody] Esppe_PessoValNameModel requestModel)
		{
			...
			Models.Esppe parentCtx = requestModel.Model == null ? null : new(m_userContext);
			requestModel.Model?.Init(m_userContext);
			requestModel.Model?.MapToModel(parentCtx);
			Esppe_PessoValName_ViewModel model = new(m_userContext, parentCtx);
		...
		public ActionResult Esppe_SpeciValEspecial([FromBody] Esppe_SpeciValEspecialModel requestModel)
		{
			...
			requestModel.Model?.Init(m_userContext);
			requestModel.Model?.MapToModel(parentCtx);
			Esppe_SpeciValEspecial_ViewModel model = new(m_userContext, parentCtx);
...
// USE /[MANUAL GQT AFTER_APPLY_EDIT ESPPE]/
		...
		public class EsppeDocumValidateTickets : RequestDocumValidateTickets
		{
			public Esppe_ViewModel Model { get; set; }
		}
		...
		public ActionResult UpdateFilesTicketsEsppe([FromBody] EsppeDocumValidateTickets requestModel)
		{
