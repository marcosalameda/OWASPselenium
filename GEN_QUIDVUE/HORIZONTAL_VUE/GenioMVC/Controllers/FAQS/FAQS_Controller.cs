...
namespace GenioMVC.Controllers
{
	public partial class FaqsController : ControllerBase
	{
		...
		public FaqsController(UserContextService userContext, IChatbotService aiService) : base(userContext)
		{
			_aiService = aiService;
		}

// USE /[MANUAL GQT CONTROLLER_NAVIGATION FAQS]/



		private List<string> GetActionIds(CriteriaSet crs, CSGenio.persistence.PersistentSupport sp = null)
		{
			CSGenio.business.Area area = CSGenio.business.Area.createArea<CSGenioAfaqs>(UserContext.Current.User, UserContext.Current.User.CurrentModule);
...
// USE /[MANUAL GQT MANUAL_CONTROLLER FAQS]/
		...
		/// <returns></returns>
		[HttpPost]
		public JsonResult RecalculateFormulas_Faqs([FromBody]Faqs_ViewModel formData)
		{
			return GenericRecalculateFormulas(formData, "faqs",
				(primaryKey) => Models.Faqs.Find(primaryKey, UserContext.Current, "FFAQS"),
				(model) => formData.MapToModel(model as Models.Faqs)
			);
		}
		...
		{
			return base.GetDocumsTickets("FAQS", requestModel.FieldName, requestModel.KeyValue);
		}
