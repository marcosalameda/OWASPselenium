...
// USE /[MANUAL GQT MANUAL_CONTROLLER INSTA]/
		...
		public JsonResult GetDependants([FromBody]RequestDependantsModel requestModel)
		{
			...
			try
			{
				...
				{
					case "INSTA___TPEQUTIPOEQUI":	// Field (DB)
						values = new Insta_ViewModel(UserContext.Current).GetDependant_InstaTableTpequTipoequi(Selected);
						break;
					case "INSTA___EQUIPREGISTNR":	// Field (DB)
						values = new Insta_ViewModel(UserContext.Current).GetDependant_InstaTableEquipRegistnr(Selected);
						break;
					case "LEAFLETDEQUIPREGISTNR":	// Field (DB)
						values = new Leafletd_ViewModel(UserContext.Current).GetDependant_LeafletdTableEquipRegistnr(Selected);
						break;
					case "LEAFLETTEQUIPREGISTNR":	// Field (DB)
						values = new Leaflett_ViewModel(UserContext.Current).GetDependant_LeaflettTableEquipRegistnr(Selected);
						break;
					default: break;
				}
		...
		{
			return base.GetDocumsTickets("INSTA", requestModel.FieldName, requestModel.KeyValue);
		}
