...
namespace GenioMVC.Controllers
{
	public partial class KindeController : ControllerBase
	{
		...
		public KindeController(UserContextService userContext, IChatbotService aiService) : base(userContext)
		{
			_aiService = aiService;
		}

// USE /[MANUAL GQT CONTROLLER_NAVIGATION KINDE]/



		private List<string> GetActionIds(CriteriaSet crs, CSGenio.persistence.PersistentSupport sp = null)
		{
			CSGenio.business.Area area = CSGenio.business.Area.createArea<CSGenioAkinde>(UserContext.Current.User, UserContext.Current.User.CurrentModule);
...
// USE /[MANUAL GQT MANUAL_CONTROLLER KINDE]/
		...
		/// <returns></returns>
		[HttpPost]
		public JsonResult RecalculateFormulas_Kinde([FromBody]Kinde_ViewModel formData)
		{
			return GenericRecalculateFormulas(formData, "kinde",
				(primaryKey) => Models.Kinde.Find(primaryKey, UserContext.Current, "FKINDE"),
				(model) => formData.MapToModel(model as Models.Kinde)
			);
		}
		...
		{
			return base.GetDocumsTickets("KINDE", requestModel.FieldName, requestModel.KeyValue);
		}
