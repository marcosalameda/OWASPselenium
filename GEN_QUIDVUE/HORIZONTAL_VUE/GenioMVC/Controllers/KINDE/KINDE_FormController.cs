...
// USE /[MANUAL GQT CONTROLLER_SHOW KINDE]/
		...
		public ActionResult Kinde_Show_GET([FromBody] RequestIdModel requestModel)
		{
			...
			Kinde_ViewModel model = new(UserContext.Current);
...
// USE /[MANUAL GQT CONTROLLER_DELETE_POST KINDE]/
		...
		{
			string id = requestModel.Id;
			Kinde_ViewModel model = new(UserContext.Current, id);
			model.MapFromModel();
...
// USE /[MANUAL GQT CONTROLLER_CANCEL_GET KINDE]/
		public ActionResult Kinde_Cancel()
		{
			if (Navigation.CurrentLevel.FormMode == FormMode.New || Navigation.CurrentLevel.FormMode == FormMode.Duplicate)
			{
				...
				try
				{
					var recordKey = Navigation.GetStrValue("kinde");
					var model = GenioMVC.Models.Kinde.Find(recordKey, UserContext.Current);
...
// USE /[MANUAL GQT AFTER_CANCEL KINDE]/
		...
		public ActionResult Kinde_ValParamete([FromBody] Kinde_ValParameteModel requestModel)
		{
			...
			Models.Kinde parentCtx = requestModel.Model == null ? null : new(m_userContext);
			requestModel.Model?.Init(m_userContext);
			requestModel.Model?.MapToModel(parentCtx);
			Kinde_ValParamete_ViewModel model = new(m_userContext, parentCtx);
		...
		public ActionResult Kinde_ValManuals([FromBody] Kinde_ValManualsModel requestModel)
		{
			...
			requestModel.Model?.Init(m_userContext);
			requestModel.Model?.MapToModel(parentCtx);
			Kinde_ValManuals_ViewModel model = new(m_userContext, parentCtx);
