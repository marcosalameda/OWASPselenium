...
// USE /[MANUAL GQT MANUAL_CONTROLLER LDENT]/
		...
		public JsonResult GetDependants([FromBody]RequestDependantsModel requestModel)
		{
			...
			try
			{
				...
				{
					case "LDENT___INDOCDOCUMENR":	// Field (DB)
						values = new Ldent_ViewModel(UserContext.Current).GetDependant_LdentTableIndocDocumenr(Selected);
						break;
					case "LDENT___WAREHWAREHDES":	// Field (DB)
						values = new Ldent_ViewModel(UserContext.Current).GetDependant_LdentTableWarehWarehdes(Selected);
						break;
					case "LDENT___ITEM_ITEMDES_":	// Field (DB)
						values = new Ldent_ViewModel(UserContext.Current).GetDependant_LdentTableItemItemdes(Selected);
						break;
					case "LDENTNORINDOCDOCUMENR":	// Field (DB)
						values = new Ldentnor_ViewModel(UserContext.Current).GetDependant_LdentnorTableIndocDocumenr(Selected);
						break;
					case "LDENTNORWAREHWAREHDES":	// Field (DB)
						values = new Ldentnor_ViewModel(UserContext.Current).GetDependant_LdentnorTableWarehWarehdes(Selected);
						break;
					case "LDENTNORITEM_ITEMDES_":	// Field (DB)
						values = new Ldentnor_ViewModel(UserContext.Current).GetDependant_LdentnorTableItemItemdes(Selected);
						break;
					default: break;
				}
		...
		{
			return base.GetDocumsTickets("LDENT", requestModel.FieldName, requestModel.KeyValue);
		}
