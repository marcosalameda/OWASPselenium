...
namespace GenioMVC.Controllers
{
	public partial class LnhdeController : ControllerBase
	{
		...
		public LnhdeController(UserContextService userContext, IChatbotService aiService) : base(userContext)
		{
...
// USE /[MANUAL GQT MANUAL_CONTROLLER LNHDE]/
		...
		public JsonResult ReloadDBEdit([FromBody]RequestReloadDBEditModel requestModel)
		{
			...
			*/
			Models.Lnhde row = new Models.Lnhde(UserContext.Current, isEmpty: true);
			...
			try
			{
				switch (string.IsNullOrEmpty(Identifier) ? "" : Identifier)
				{
					case "LNHDE___PEDIDNRPEDIDO":	// Field (DB)
						{
							var model = new Lnhde_ViewModel(UserContext.Current) { editable = false };
		...
		public JsonResult GetDependants([FromBody]RequestDependantsModel requestModel)
		{
			...
			try
			{
				...
				{
					case "LNHDE___PEDIDNRPEDIDO":	// Field (DB)
						values = new Lnhde_ViewModel(UserContext.Current).GetDependant_LnhdeTablePedidNrpedido(Selected);
						break;
					case "LNHDE___LNHPDLINE____":	// Field (DB)
						values = new Lnhde_ViewModel(UserContext.Current).GetDependant_LnhdeTableLnhpdLine(Selected);
						break;
					case "LNHDE___TPEQ1TIPOEQUI":	// Field (DB)
						values = new Lnhde_ViewModel(UserContext.Current).GetDependant_LnhdeTableTpeq1Tipoequi(Selected);
						break;
					default: break;
				}
		...
		/// <returns></returns>
		[HttpPost]
		public JsonResult RecalculateFormulas_Lnhde([FromBody]Lnhde_ViewModel formData)
		{
			return GenericRecalculateFormulas(formData, "lnhde",
				(primaryKey) => Models.Lnhde.Find(primaryKey, UserContext.Current, "FLNHDE"),
				(model) => formData.MapToModel(model as Models.Lnhde)
			);
		}
					...
					case "LNHDE___TPEQ1TIPOEQUI":	// Field (DB)
						{
							...
							return JsonOK(new { Tree = model.Tree_TableTpeq1Tipoequi });
						}
					default:
						break;
				}
		...
		{
			return base.GetDocumsTickets("LNHDE", requestModel.FieldName, requestModel.KeyValue);
		}
