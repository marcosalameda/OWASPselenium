...
// USE /[MANUAL GQT CONTROLLER_NAVIGATION LNHDF]/
		...
		{
			CSGenio.business.Area area = CSGenio.business.Area.createArea<CSGenioAlnhdf>(UserContext.Current.User, UserContext.Current.User.CurrentModule);
...
// USE /[MANUAL GQT MANUAL_CONTROLLER LNHDF]/
		...
		/// <returns></returns>
		[HttpPost]
		public JsonResult RecalculateFormulas_Lnhdf([FromBody]Lnhdf_ViewModel formData)
		{
			return GenericRecalculateFormulas(formData, "lnhdf",
				(primaryKey) => Models.Lnhdf.Find(primaryKey, UserContext.Current, "FLNHDF"),
				(model) => formData.MapToModel(model as Models.Lnhdf)
			);
		}
		...
		{
			return base.GetDocumsTickets("LNHDF", requestModel.FieldName, requestModel.KeyValue);
		}
