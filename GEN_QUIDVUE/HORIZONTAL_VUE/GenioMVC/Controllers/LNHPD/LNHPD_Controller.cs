...
namespace GenioMVC.Controllers
{
	public partial class LnhpdController : ControllerBase
	{
		...
		public LnhpdController(UserContextService userContext, IChatbotService aiService) : base(userContext)
		{
...
// USE /[MANUAL GQT MANUAL_CONTROLLER LNHPD]/
		...
		public JsonResult ReloadDBEdit([FromBody]RequestReloadDBEditModel requestModel)
		{
			...
			*/
			Models.Lnhpd row = new Models.Lnhpd(UserContext.Current, isEmpty: true);
			...
			try
			{
				switch (string.IsNullOrEmpty(Identifier) ? "" : Identifier)
				{
					case "LNHPD___PEDIDNRPEDIDO":	// Field (DB)
						{
							var model = new Lnhpd_ViewModel(UserContext.Current) { editable = false };
		...
		public JsonResult GetDependants([FromBody]RequestDependantsModel requestModel)
		{
			...
			try
			{
				...
				{
					case "LNHPD___PEDIDNRPEDIDO":	// Field (DB)
						values = new Lnhpd_ViewModel(UserContext.Current).GetDependant_LnhpdTablePedidNrpedido(Selected);
						break;
					case "LNHPD___TPEQUTIPOEQUI":	// Field (DB)
						values = new Lnhpd_ViewModel(UserContext.Current).GetDependant_LnhpdTableTpequTipoequi(Selected);
						break;
					default: break;
				}
		...
		/// <returns></returns>
		[HttpPost]
		public JsonResult RecalculateFormulas_Lnhpd([FromBody]Lnhpd_ViewModel formData)
		{
			return GenericRecalculateFormulas(formData, "lnhpd",
				(primaryKey) => Models.Lnhpd.Find(primaryKey, UserContext.Current, "FLNHPD"),
				(model) => formData.MapToModel(model as Models.Lnhpd)
			);
		}
		...
		{
			return base.GetDocumsTickets("LNHPD", requestModel.FieldName, requestModel.KeyValue);
		}
