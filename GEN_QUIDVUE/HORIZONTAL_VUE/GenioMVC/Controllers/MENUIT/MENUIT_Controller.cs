...
namespace GenioMVC.Controllers
{
	public partial class MenuitController : ControllerBase
	{
		...
		public MenuitController(UserContextService userContext, IChatbotService aiService) : base(userContext)
		{
			_aiService = aiService;
		}

// USE /[MANUAL GQT CONTROLLER_NAVIGATION MENUIT]/



		private List<string> GetActionIds(CriteriaSet crs, CSGenio.persistence.PersistentSupport sp = null)
		{
			CSGenio.business.Area area = CSGenio.business.Area.createArea<CSGenioAmenuit>(UserContext.Current.User, UserContext.Current.User.CurrentModule);
			return base.GetActionIds(crs, sp, area);
		}

// USE /[MANUAL GQT MANUAL_CONTROLLER MENUIT]/
		...
		public JsonResult ReloadDBEdit([FromBody]RequestReloadDBEditModel requestModel)
		{
			...
			*/
			Models.Menuit row = new Models.Menuit(UserContext.Current, isEmpty: true);
			row.klass.QPrimaryKey = Navigation.GetStrValue("menuit");
			...
			try
			{
				...
				{
					case "F_MENUITMENUCMENUCL__":	// Field (DB)
						{
							var model = new F_menuit_ViewModel(UserContext.Current) { editable = false };
							model.MapFromModel(row);
							model.Load_F_menuitmenucmenucl__(qs);
							result = model.TableMenucMenucl;
						}
						break;
					default:
						break;
		...
		public JsonResult GetDependants([FromBody]RequestDependantsModel requestModel)
		{
			...
			try
			{
				...
				{
					case "F_MENUITMENUCMENUCL__":	// Field (DB)
						values = new F_menuit_ViewModel(UserContext.Current).GetDependant_F_menuitTableMenucMenucl(Selected);
						break;
					default: break;
				}
		...
		/// <summary>
		/// Recalculate formulas of the "F_menuit" form. (++, CT, SR, CL and U1)
		/// </summary>
		/// <param name="formData">Current form data</param>
		/// <returns></returns>
		[HttpPost]
		public JsonResult RecalculateFormulas_F_menuit([FromBody]F_menuit_ViewModel formData)
		{
			return GenericRecalculateFormulas(formData, "menuit",
				(primaryKey) => Models.Menuit.Find(primaryKey, UserContext.Current, "FF_MENUIT"),
				(model) => formData.MapToModel(model as Models.Menuit)
			);
		}
		...
		{
			return base.GetDocumsTickets("MENUIT", requestModel.FieldName, requestModel.KeyValue);
		}
