...
namespace GenioMVC.Controllers
{
	public partial class OperacoesController : ControllerBase
	{
		...
		public OperacoesController(UserContextService userContext, IChatbotService aiService) : base(userContext)
		{
			_aiService = aiService;
		}

// USE /[MANUAL GQT CONTROLLER_NAVIGATION OPERACOES]/



		private List<string> GetActionIds(CriteriaSet crs, CSGenio.persistence.PersistentSupport sp = null)
		{
			CSGenio.business.Area area = CSGenio.business.Area.createArea<CSGenioAoperacoes>(UserContext.Current.User, UserContext.Current.User.CurrentModule);
			return base.GetActionIds(crs, sp, area);
		}

// USE /[MANUAL GQT MANUAL_CONTROLLER OPERACOES]/
		...
		public JsonResult ReloadDBEdit([FromBody]RequestReloadDBEditModel requestModel)
		{
			...
			*/
			Models.Operacoes row = new Models.Operacoes(UserContext.Current, isEmpty: true);
			row.klass.QPrimaryKey = Navigation.GetStrValue("operacoes");
			...
			try
			{
				...
				{
					case "OPERACOES__ENTIDADE__ENTIDADE":	// Field (DB)
						{
							var model = new Operacoes_ViewModel(UserContext.Current) { editable = false };
							model.MapFromModel(row);
							model.Load_Operacoes__entidade__entidade(qs);
							result = model.TableEntidadeEntidade;
						}
						break;
					default:
						break;
		...
		public JsonResult GetDependants([FromBody]RequestDependantsModel requestModel)
		{
			...
			try
			{
				...
				{
					case "OPERACOES__ENTIDADE__ENTIDADE":	// Field (DB)
						values = new Operacoes_ViewModel(UserContext.Current).GetDependant_OperacoesTableEntidadeEntidade(Selected);
						break;
					default: break;
				}
		...
		/// <summary>
		/// Recalculate formulas of the "Operacoes" form. (++, CT, SR, CL and U1)
		/// </summary>
		/// <param name="formData">Current form data</param>
		/// <returns></returns>
		[HttpPost]
		public JsonResult RecalculateFormulas_Operacoes([FromBody]Operacoes_ViewModel formData)
		{
			return GenericRecalculateFormulas(formData, "operacoes",
				(primaryKey) => Models.Operacoes.Find(primaryKey, UserContext.Current, "FOPERACOES"),
				(model) => formData.MapToModel(model as Models.Operacoes)
			);
		}
		...
		{
			return base.GetDocumsTickets("OPERACOES", requestModel.FieldName, requestModel.KeyValue);
		}
