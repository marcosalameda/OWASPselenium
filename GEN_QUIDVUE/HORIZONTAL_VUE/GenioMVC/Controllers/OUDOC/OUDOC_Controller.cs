...
namespace GenioMVC.Controllers
{
	public partial class OudocController : ControllerBase
	{
		...
		public OudocController(UserContextService userContext, IChatbotService aiService) : base(userContext)
		{
...
// USE /[MANUAL GQT MANUAL_CONTROLLER OUDOC]/
		...
		/// <returns></returns>
		[HttpPost]
		public JsonResult RecalculateFormulas_Docsd([FromBody]Docsd_ViewModel formData)
		{
			return GenericRecalculateFormulas(formData, "oudoc",
				(primaryKey) => Models.Oudoc.Find(primaryKey, UserContext.Current, "FDOCSD"),
				(model) => formData.MapToModel(model as Models.Oudoc)
			);
		}
		...
		{
			return base.GetDocumsTickets("OUDOC", requestModel.FieldName, requestModel.KeyValue);
		}
