...
namespace GenioMVC.Controllers
{
	public partial class OutptController : ControllerBase
	{
		...
		private static readonly NavigationLocation ACTION_DSAID_CANCEL = new("OUTPUT_DOCUMENT44972", "Dsaid_Cancel", "Outpt") { vueRouteName = "form-DSAID", mode = "CANCEL" };
		private static readonly NavigationLocation ACTION_DSAID_SHOW = new("OUTPUT_DOCUMENT44972", "Dsaid_Show", "Outpt") { vueRouteName = "form-DSAID", mode = "SHOW" };
		private static readonly NavigationLocation ACTION_DSAID_NEW = new("OUTPUT_DOCUMENT44972", "Dsaid_New", "Outpt") { vueRouteName = "form-DSAID", mode = "NEW" };
		private static readonly NavigationLocation ACTION_DSAID_EDIT = new("OUTPUT_DOCUMENT44972", "Dsaid_Edit", "Outpt") { vueRouteName = "form-DSAID", mode = "EDIT" };
		private static readonly NavigationLocation ACTION_DSAID_DUPLICATE = new("OUTPUT_DOCUMENT44972", "Dsaid_Duplicate", "Outpt") { vueRouteName = "form-DSAID", mode = "DUPLICATE" };
		private static readonly NavigationLocation ACTION_DSAID_DELETE = new("OUTPUT_DOCUMENT44972", "Dsaid_Delete", "Outpt") { vueRouteName = "form-DSAID", mode = "DELETE" };
...
// USE /[MANUAL GQT CONTROLLER_SHOW DSAID]/
		...
		public ActionResult Dsaid_Show_GET([FromBody] RequestIdModel requestModel)
		{
			...
			Dsaid_ViewModel model = new(UserContext.Current);
...
// USE /[MANUAL GQT CONTROLLER_DELETE_POST DSAID]/
		...
		{
			string id = requestModel.Id;
			Dsaid_ViewModel model = new(UserContext.Current, id);
			model.MapFromModel();
...
// USE /[MANUAL GQT CONTROLLER_CANCEL_GET DSAID]/
		public ActionResult Dsaid_Cancel()
		{
			if (Navigation.CurrentLevel.FormMode == FormMode.New || Navigation.CurrentLevel.FormMode == FormMode.Duplicate)
			{
				...
				try
				{
					var recordKey = Navigation.GetStrValue("outpt");
					var model = GenioMVC.Models.Outpt.Find(recordKey, UserContext.Current);
...
// USE /[MANUAL GQT AFTER_CANCEL DSAID]/
		...
		public ActionResult Dsaid_Ware1ValWarehdes([FromBody] Dsaid_Ware1ValWarehdesModel requestModel)
		{
			...
			Models.Outpt parentCtx = requestModel.Model == null ? null : new(m_userContext);
			requestModel.Model?.Init(m_userContext);
			requestModel.Model?.MapToModel(parentCtx);
			Dsaid_Ware1ValWarehdes_ViewModel model = new(m_userContext, parentCtx);
		...
		public ActionResult Dsaid_ValSaidas([FromBody] Dsaid_ValSaidasModel requestModel)
		{
			...
			requestModel.Model?.Init(m_userContext);
			requestModel.Model?.MapToModel(parentCtx);
			Dsaid_ValSaidas_ViewModel model = new(m_userContext, parentCtx);
		...
		{
			string id = requestModel.Id;
			string position = requestModel.Position.ToString();
...
// USE /[MANUAL GQT AFTER_APPLY_EDIT DSAID]/
		...
		public class DsaidDocumValidateTickets : RequestDocumValidateTickets
		{
			public Dsaid_ViewModel Model { get; set; }
		}
		...
		public ActionResult UpdateFilesTicketsDsaid([FromBody] DsaidDocumValidateTickets requestModel)
		{
