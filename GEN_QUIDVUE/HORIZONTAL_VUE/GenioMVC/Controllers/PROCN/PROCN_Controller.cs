...
// USE /[MANUAL GQT CONTROLLER_NAVIGATION PROCN]/
		...
		{
			CSGenio.business.Area area = CSGenio.business.Area.createArea<CSGenioAprocn>(UserContext.Current.User, UserContext.Current.User.CurrentModule);
...
// USE /[MANUAL GQT MANUAL_CONTROLLER PROCN]/
		...
		public JsonResult GetDependants([FromBody]RequestDependantsModel requestModel)
		{
			...
			try
			{
				...
				{
					case "CONTAC03PROPETITLE___":	// Field (DB)
						values = new Contac03_ViewModel(UserContext.Current).GetDependant_Contac03TablePropeTitle(Selected);
						break;
					case "CONTAC06PROPETITLE___":	// Field (DB)
						values = new Contac06_ViewModel(UserContext.Current).GetDependant_Contac06TablePropeTitle(Selected);
						break;
					case "CONTAC19PROPETITLE___":	// Field (DB)
						values = new Contac19_ViewModel(UserContext.Current).GetDependant_Contac19TablePropeTitle(Selected);
						break;
					default: break;
				}
		...
		/// <returns></returns>
		[HttpPost]
		public JsonResult RecalculateFormulas_Contac03([FromBody]Contac03_ViewModel formData)
		{
			return GenericRecalculateFormulas(formData, "procn",
				(primaryKey) => Models.Procn.Find(primaryKey, UserContext.Current, "FCONTAC03"),
		...
		/// <returns></returns>
		[HttpPost]
		public JsonResult RecalculateFormulas_Contac06([FromBody]Contac06_ViewModel formData)
		{
			return GenericRecalculateFormulas(formData, "procn",
				(primaryKey) => Models.Procn.Find(primaryKey, UserContext.Current, "FCONTAC06"),
				(model) => formData.MapToModel(model as Models.Procn)
			);
		}
		...
		/// <returns></returns>
		[HttpPost]
		public JsonResult RecalculateFormulas_Contac19([FromBody]Contac19_ViewModel formData)
		{
			return GenericRecalculateFormulas(formData, "procn",
				(primaryKey) => Models.Procn.Find(primaryKey, UserContext.Current, "FCONTAC19"),
				(model) => formData.MapToModel(model as Models.Procn)
			);
		}
		...
		{
			return base.GetDocumsTickets("PROCN", requestModel.FieldName, requestModel.KeyValue);
		}
