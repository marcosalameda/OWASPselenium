...
namespace GenioMVC.Controllers
{
	public partial class ProjeController : ControllerBase
	{
		...
		private static readonly NavigationLocation ACTION_PROJE_CANCEL = new("PROJECTO50142", "Proje_Cancel", "Proje") { vueRouteName = "form-PROJE", mode = "CANCEL" };
		private static readonly NavigationLocation ACTION_PROJE_SHOW = new("PROJECTO50142", "Proje_Show", "Proje") { vueRouteName = "form-PROJE", mode = "SHOW" };
		private static readonly NavigationLocation ACTION_PROJE_NEW = new("PROJECTO50142", "Proje_New", "Proje") { vueRouteName = "form-PROJE", mode = "NEW" };
		private static readonly NavigationLocation ACTION_PROJE_EDIT = new("PROJECTO50142", "Proje_Edit", "Proje") { vueRouteName = "form-PROJE", mode = "EDIT" };
		private static readonly NavigationLocation ACTION_PROJE_DUPLICATE = new("PROJECTO50142", "Proje_Duplicate", "Proje") { vueRouteName = "form-PROJE", mode = "DUPLICATE" };
		private static readonly NavigationLocation ACTION_PROJE_DELETE = new("PROJECTO50142", "Proje_Delete", "Proje") { vueRouteName = "form-PROJE", mode = "DELETE" };
...
// USE /[MANUAL GQT CONTROLLER_SHOW PROJE]/
		...
		public ActionResult Proje_Show_GET([FromBody] RequestIdModel requestModel)
		{
			...
			Proje_ViewModel model = new(UserContext.Current);
...
// USE /[MANUAL GQT CONTROLLER_DELETE_POST PROJE]/
		...
		{
			string id = requestModel.Id;
			Proje_ViewModel model = new(UserContext.Current, id);
			model.MapFromModel();
...
// USE /[MANUAL GQT CONTROLLER_CANCEL_GET PROJE]/
		public ActionResult Proje_Cancel()
		{
			if (Navigation.CurrentLevel.FormMode == FormMode.New || Navigation.CurrentLevel.FormMode == FormMode.Duplicate)
			{
				...
				try
				{
					var recordKey = Navigation.GetStrValue("proje");
					var model = GenioMVC.Models.Proje.Find(recordKey, UserContext.Current);
...
// USE /[MANUAL GQT AFTER_CANCEL PROJE]/
		...
		public ActionResult Proje_Year1ValYear([FromBody] Proje_Year1ValYearModel requestModel)
		{
			...
			Models.Proje parentCtx = requestModel.Model == null ? null : new(m_userContext);
			requestModel.Model?.Init(m_userContext);
			requestModel.Model?.MapToModel(parentCtx);
			Proje_Year1ValYear_ViewModel model = new(m_userContext, parentCtx);
		...
		public ActionResult Proje_ValDespesas([FromBody] Proje_ValDespesasModel requestModel)
		{
			...
			requestModel.Model?.Init(m_userContext);
			requestModel.Model?.MapToModel(parentCtx);
			Proje_ValDespesas_ViewModel model = new(m_userContext, parentCtx);
		...
		public ActionResult Proje_ValAgregado([FromBody] Proje_ValAgregadoModel requestModel)
		{
			...
			requestModel.Model?.Init(m_userContext);
			requestModel.Model?.MapToModel(parentCtx);
			Proje_ValAgregado_ViewModel model = new(m_userContext, parentCtx);
...
// USE /[MANUAL GQT AFTER_APPLY_EDIT PROJE]/
		...
		public class ProjeDocumValidateTickets : RequestDocumValidateTickets
		{
			public Proje_ViewModel Model { get; set; }
		}
