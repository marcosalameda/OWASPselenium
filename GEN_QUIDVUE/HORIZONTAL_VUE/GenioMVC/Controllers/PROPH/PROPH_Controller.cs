...
namespace GenioMVC.Controllers
{
	public partial class ProphController : ControllerBase
	{
		...
		public ProphController(UserContextService userContext, IChatbotService aiService) : base(userContext)
		{
			_aiService = aiService;
		}

// USE /[MANUAL GQT CONTROLLER_NAVIGATION PROPH]/



		private List<string> GetActionIds(CriteriaSet crs, CSGenio.persistence.PersistentSupport sp = null)
		{
			CSGenio.business.Area area = CSGenio.business.Area.createArea<CSGenioAproph>(UserContext.Current.User, UserContext.Current.User.CurrentModule);
			return base.GetActionIds(crs, sp, area);
		}

// USE /[MANUAL GQT MANUAL_CONTROLLER PROPH]/
		...
		public JsonResult ReloadDBEdit([FromBody]RequestReloadDBEditModel requestModel)
		{
			...
			*/
			Models.Proph row = new Models.Proph(UserContext.Current, isEmpty: true);
			row.klass.QPrimaryKey = Navigation.GetStrValue("proph");
			...
			try
			{
				...
				{
					case "PHOTO03_PROPETITLE___":	// Field (DB)
						{
							var model = new Photo03_ViewModel(UserContext.Current) { editable = false };
							model.MapFromModel(row);
							model.Load_Photo03_propetitle___(qs);
		...
		public JsonResult GetDependants([FromBody]RequestDependantsModel requestModel)
		{
			...
			try
			{
				...
				{
					case "PHOTO03_PROPETITLE___":	// Field (DB)
						values = new Photo03_ViewModel(UserContext.Current).GetDependant_Photo03TablePropeTitle(Selected);
						break;
					default: break;
				}
		...
		/// <summary>
		/// Recalculate formulas of the "Photo03" form. (++, CT, SR, CL and U1)
		/// </summary>
		/// <param name="formData">Current form data</param>
		/// <returns></returns>
		[HttpPost]
		public JsonResult RecalculateFormulas_Photo03([FromBody]Photo03_ViewModel formData)
		{
			return GenericRecalculateFormulas(formData, "proph",
				(primaryKey) => Models.Proph.Find(primaryKey, UserContext.Current, "FPHOTO03"),
				(model) => formData.MapToModel(model as Models.Proph)
			);
		}
		...
		{
			return base.GetDocumsTickets("PROPH", requestModel.FieldName, requestModel.KeyValue);
		}
