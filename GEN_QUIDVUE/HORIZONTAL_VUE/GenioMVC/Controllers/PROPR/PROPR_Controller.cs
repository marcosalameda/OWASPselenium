...
// USE /[MANUAL GQT MANUAL_CONTROLLER PROPR]/
		...
		public JsonResult GetDependants([FromBody]RequestDependantsModel requestModel)
		{
			...
			try
			{
				...
				{
					case "PROPR00_TPPROTPPROPRI":	// Field (DB)
						values = new Propr00_ViewModel(UserContext.Current).GetDependant_Propr00TableTpproTppropri(Selected);
						break;
					case "PROPR00_PESSONAME____":	// Field (DB)
						values = new Propr00_ViewModel(UserContext.Current).GetDependant_Propr00TablePessoName(Selected);
						break;
					case "PROPR01_CNTRYCOUNTRY_":	// Field (DB)
						values = new Propr00_ViewModel(UserContext.Current).GetDependant_Propr01TableCntryCountry(Selected);
						break;
					case "PROPR01_REGIOREGIAO__":	// Field (DB)
						values = new Propr00_ViewModel(UserContext.Current).GetDependant_Propr01TableRegioRegiao(Selected);
						break;
					case "PROPRALLTPPROTPPROPRI":	// Field (DB)
						values = new Proprall_ViewModel(UserContext.Current).GetDependant_ProprallTableTpproTppropri(Selected);
						break;
					case "PROPRALLCNTRYCOUNTRY_":	// Field (DB)
						values = new Proprall_ViewModel(UserContext.Current).GetDependant_ProprallTableCntryCountry(Selected);
						break;
					case "PROPRALLREGIOREGIAO__":	// Field (DB)
						values = new Proprall_ViewModel(UserContext.Current).GetDependant_ProprallTableRegioRegiao(Selected);
						break;
					case "PROPRALLPESSONAME____":	// Field (DB)
						values = new Proprall_ViewModel(UserContext.Current).GetDependant_ProprallTablePessoName(Selected);
						break;
					default: break;
				}
		...
		/// <returns></returns>
		[HttpPost]
		public JsonResult RecalculateFormulas_Propr00([FromBody]Propr00_ViewModel formData)
		{
			return GenericRecalculateFormulas(formData, "propr",
				(primaryKey) => Models.Propr.Find(primaryKey, UserContext.Current, "FPROPR00"),
		...
		/// <returns></returns>
		[HttpPost]
		public JsonResult RecalculateFormulas_Proprall([FromBody]Proprall_ViewModel formData)
		{
			return GenericRecalculateFormulas(formData, "propr",
				(primaryKey) => Models.Propr.Find(primaryKey, UserContext.Current, "FPROPRALL"),
				(model) => formData.MapToModel(model as Models.Propr)
			);
		}
		...
		{
			return base.GetDocumsTickets("PROPR", requestModel.FieldName, requestModel.KeyValue);
		}
