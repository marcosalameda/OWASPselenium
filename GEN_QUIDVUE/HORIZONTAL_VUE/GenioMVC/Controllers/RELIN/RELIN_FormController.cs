...
// USE /[MANUAL GQT CONTROLLER_SHOW RELIN]/
		...
		public ActionResult Relin_Show_GET([FromBody] RequestIdModel requestModel)
		{
			...
			Relin_ViewModel model = new(UserContext.Current);
...
// USE /[MANUAL GQT CONTROLLER_DELETE_POST RELIN]/
		...
		{
			string id = requestModel.Id;
			Relin_ViewModel model = new(UserContext.Current, id);
			model.MapFromModel();
...
// USE /[MANUAL GQT CONTROLLER_CANCEL_GET RELIN]/
		public ActionResult Relin_Cancel()
		{
			if (Navigation.CurrentLevel.FormMode == FormMode.New || Navigation.CurrentLevel.FormMode == FormMode.Duplicate)
			{
				...
				try
				{
					var recordKey = Navigation.GetStrValue("relin");
					var model = GenioMVC.Models.Relin.Find(recordKey, UserContext.Current);
...
// USE /[MANUAL GQT AFTER_CANCEL RELIN]/
		...
		public ActionResult Relin_ReceiValNumber([FromBody] Relin_ReceiValNumberModel requestModel)
		{
			...
			Models.Relin parentCtx = requestModel.Model == null ? null : new(m_userContext);
			requestModel.Model?.Init(m_userContext);
			requestModel.Model?.MapToModel(parentCtx);
			Relin_ReceiValNumber_ViewModel model = new(m_userContext, parentCtx);
		...
		public ActionResult Relin_ProduValProduct([FromBody] Relin_ProduValProductModel requestModel)
		{
			...
			requestModel.Model?.Init(m_userContext);
			requestModel.Model?.MapToModel(parentCtx);
			Relin_ProduValProduct_ViewModel model = new(m_userContext, parentCtx);
...
// USE /[MANUAL GQT AFTER_APPLY_EDIT RELIN]/
		...
		public class RelinDocumValidateTickets : RequestDocumValidateTickets
		{
			public Relin_ViewModel Model { get; set; }
		}
		...
		public ActionResult UpdateFilesTicketsRelin([FromBody] RelinDocumValidateTickets requestModel)
		{
