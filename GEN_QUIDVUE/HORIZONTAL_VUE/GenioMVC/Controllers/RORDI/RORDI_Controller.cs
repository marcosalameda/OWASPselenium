...
// USE /[MANUAL GQT CONTROLLER_NAVIGATION RORDI]/
		...
		{
			CSGenio.business.Area area = CSGenio.business.Area.createArea<CSGenioArordi>(UserContext.Current.User, UserContext.Current.User.CurrentModule);
...
// USE /[MANUAL GQT MANUAL_CONTROLLER RORDI]/
		...
		/// <returns></returns>
		[HttpPost]
		public JsonResult RecalculateFormulas_Rordi([FromBody]Rordi_ViewModel formData)
		{
			return GenericRecalculateFormulas(formData, "rordi",
				(primaryKey) => Models.Rordi.Find(primaryKey, UserContext.Current, "FRORDI"),
				(model) => formData.MapToModel(model as Models.Rordi)
			);
		}
		...
		{
			return base.GetDocumsTickets("RORDI", requestModel.FieldName, requestModel.KeyValue);
		}
