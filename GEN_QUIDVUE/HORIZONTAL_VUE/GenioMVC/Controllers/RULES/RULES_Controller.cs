...
namespace GenioMVC.Controllers
{
	public partial class RulesController : ControllerBase
	{
		...
		public RulesController(UserContextService userContext, IChatbotService aiService) : base(userContext)
		{
			_aiService = aiService;
		}

// USE /[MANUAL GQT CONTROLLER_NAVIGATION RULES]/



		private List<string> GetActionIds(CriteriaSet crs, CSGenio.persistence.PersistentSupport sp = null)
		{
			CSGenio.business.Area area = CSGenio.business.Area.createArea<CSGenioArules>(UserContext.Current.User, UserContext.Current.User.CurrentModule);
			return base.GetActionIds(crs, sp, area);
		}

// USE /[MANUAL GQT MANUAL_CONTROLLER RULES]/
		...
		public JsonResult ReloadDBEdit([FromBody]RequestReloadDBEditModel requestModel)
		{
			...
			*/
			Models.Rules row = new Models.Rules(UserContext.Current, isEmpty: true);
			row.klass.QPrimaryKey = Navigation.GetStrValue("rules");
			...
			try
			{
				...
				{
					case "REGRA__UP_RULES__DESCRIPT":	// Field (DB)
						{
							var model = new Regra_ViewModel(UserContext.Current) { editable = false };
							model.MapFromModel(row);
							model.Load_Regra__up_rules__descript(qs);
							result = model.TableUp_rulesDescript;
						}
						break;
					case "REGRA2__UP_RULES__DESCRIPT":	// Field (DB)
						{
							var model = new Regra2_ViewModel(UserContext.Current) { editable = false };
							model.MapFromModel(row);
							model.Load_Regra2__up_rules__descript(qs);
							result = model.TableUp_rulesDescript;
						}
						break;
					default:
		...
		public JsonResult GetDependants([FromBody]RequestDependantsModel requestModel)
		{
			...
			try
			{
				...
				{
					case "REGRA__UP_RULES__DESCRIPT":	// Field (DB)
						values = new Regra_ViewModel(UserContext.Current).GetDependant_RegraTableUp_rulesDescript(Selected);
						break;
					case "REGRA2__UP_RULES__DESCRIPT":	// Field (DB)
						values = new Regra2_ViewModel(UserContext.Current).GetDependant_Regra2TableUp_rulesDescript(Selected);
						break;
					default: break;
				}
		...
		/// <summary>
		/// Recalculate formulas of the "Regra" form. (++, CT, SR, CL and U1)
		/// </summary>
		/// <param name="formData">Current form data</param>
		/// <returns></returns>
		[HttpPost]
		public JsonResult RecalculateFormulas_Regra([FromBody]Regra_ViewModel formData)
		{
			return GenericRecalculateFormulas(formData, "rules",
				(primaryKey) => Models.Rules.Find(primaryKey, UserContext.Current, "FREGRA"),
				(model) => formData.MapToModel(model as Models.Rules)
			);
		}
		...
		/// <summary>
		/// Recalculate formulas of the "Regra2" form. (++, CT, SR, CL and U1)
		/// </summary>
		/// <param name="formData">Current form data</param>
		/// <returns></returns>
		[HttpPost]
		public JsonResult RecalculateFormulas_Regra2([FromBody]Regra2_ViewModel formData)
		{
			return GenericRecalculateFormulas(formData, "rules",
				(primaryKey) => Models.Rules.Find(primaryKey, UserContext.Current, "FREGRA2"),
				(model) => formData.MapToModel(model as Models.Rules)
			);
		}
		...
		{
			return base.GetDocumsTickets("RULES", requestModel.FieldName, requestModel.KeyValue);
		}
