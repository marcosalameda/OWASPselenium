...
// USE /[MANUAL GQT CONTROLLER_NAVIGATION SBCAT]/
		...
		{
			CSGenio.business.Area area = CSGenio.business.Area.createArea<CSGenioAsbcat>(UserContext.Current.User, UserContext.Current.User.CurrentModule);
...
// USE /[MANUAL GQT MANUAL_CONTROLLER SBCAT]/
		...
		/// <summary>
		/// Recalculate formulas of the "Sbcat" form. (++, CT, SR, CL and U1)
		/// </summary>
		/// <param name="formData">Current form data</param>
		/// <returns></returns>
		[HttpPost]
		public JsonResult RecalculateFormulas_Sbcat([FromBody]Sbcat_ViewModel formData)
		{
			return GenericRecalculateFormulas(formData, "sbcat",
				(primaryKey) => Models.Sbcat.Find(primaryKey, UserContext.Current, "FSBCAT"),
				(model) => formData.MapToModel(model as Models.Sbcat)
			);
		}
		...
		{
			return base.GetDocumsTickets("SBCAT", requestModel.FieldName, requestModel.KeyValue);
		}
