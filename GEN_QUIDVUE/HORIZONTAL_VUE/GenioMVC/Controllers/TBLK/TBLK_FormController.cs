...
// USE /[MANUAL GQT CONTROLLER_SHOW TBLK]/
		...
		public ActionResult Tblk_Show_GET([FromBody] RequestIdModel requestModel)
		{
			...
			Tblk_ViewModel model = new(UserContext.Current);
...
// USE /[MANUAL GQT CONTROLLER_DELETE_POST TBLK]/
		...
		{
			string id = requestModel.Id;
			Tblk_ViewModel model = new(UserContext.Current, id);
			model.MapFromModel();
...
// USE /[MANUAL GQT CONTROLLER_CANCEL_GET TBLK]/
		public ActionResult Tblk_Cancel()
		{
			if (Navigation.CurrentLevel.FormMode == FormMode.New || Navigation.CurrentLevel.FormMode == FormMode.Duplicate)
			{
				...
				try
				{
					var recordKey = Navigation.GetStrValue("tblk");
					var model = GenioMVC.Models.Tblk.Find(recordKey, UserContext.Current);
...
// USE /[MANUAL GQT AFTER_CANCEL TBLK]/
		...
		public ActionResult Tblk_GrpbValName([FromBody] Tblk_GrpbValNameModel requestModel)
		{
			...
			Models.Tblk parentCtx = requestModel.Model == null ? null : new(m_userContext);
			requestModel.Model?.Init(m_userContext);
			requestModel.Model?.MapToModel(parentCtx);
			Tblk_GrpbValName_ViewModel model = new(m_userContext, parentCtx);
		...
		public ActionResult Tblk_TrsbValName([FromBody] Tblk_TrsbValNameModel requestModel)
		{
			...
			requestModel.Model?.Init(m_userContext);
			requestModel.Model?.MapToModel(parentCtx);
			Tblk_TrsbValName_ViewModel model = new(m_userContext, parentCtx);
...
// USE /[MANUAL GQT AFTER_APPLY_EDIT TBLK]/
		...
		public class TblkDocumValidateTickets : RequestDocumValidateTickets
		{
			public Tblk_ViewModel Model { get; set; }
		}
		...
		public ActionResult UpdateFilesTicketsTblk([FromBody] TblkDocumValidateTickets requestModel)
		{
