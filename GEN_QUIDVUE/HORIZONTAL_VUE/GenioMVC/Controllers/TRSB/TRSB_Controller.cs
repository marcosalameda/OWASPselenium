...
namespace GenioMVC.Controllers
{
	public partial class TrsbController : ControllerBase
	{
		...
		public TrsbController(UserContextService userContext, IChatbotService aiService) : base(userContext)
		{
			_aiService = aiService;
		}

// USE /[MANUAL GQT CONTROLLER_NAVIGATION TRSB]/



		private List<string> GetActionIds(CriteriaSet crs, CSGenio.persistence.PersistentSupport sp = null)
		{
			CSGenio.business.Area area = CSGenio.business.Area.createArea<CSGenioAtrsb>(UserContext.Current.User, UserContext.Current.User.CurrentModule);
...
// USE /[MANUAL GQT MANUAL_CONTROLLER TRSB]/
		...
		/// <returns></returns>
		[HttpPost]
		public JsonResult RecalculateFormulas_Trsb([FromBody]Trsb_ViewModel formData)
		{
			return GenericRecalculateFormulas(formData, "trsb",
				(primaryKey) => Models.Trsb.Find(primaryKey, UserContext.Current, "FTRSB"),
				(model) => formData.MapToModel(model as Models.Trsb)
			);
		}
		...
		{
			return base.GetDocumsTickets("TRSB", requestModel.FieldName, requestModel.KeyValue);
		}
