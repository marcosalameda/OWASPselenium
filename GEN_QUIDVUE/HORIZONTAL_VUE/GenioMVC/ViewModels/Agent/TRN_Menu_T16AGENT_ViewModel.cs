...
{
	public class TRN_Menu_T16AGENT_ViewModel : ListViewModel
	{
		/// <summary>
		/// Gets or sets the object that represents the table and its elements. List type: "${exposeField.Fajuda}"
		/// </summary>
		[JsonPropertyName("Table")]
		public TablePartial<TRN_Menu_T16AGENT_RowViewModel> Menu { get; set; }

		/// <inheritdoc/>
		public override string TableAlias { get => "agent"; }

		/// <inheritdoc/>
		public override string Uuid { get => "24daf59d-e616-4c6e-8755-3e11a9f6ff85"; }
		...
		public override int GetCount(User user)
		{
			...
			CriteriaSet trn_menu_t16agentConds = CSGenio.business.Listing.CalculateConditionsEphGeneric(areaBase, "MLT16AGENT");
			trn_menu_t16agentConds.Equal(CSGenioAagent.FldZzstate, 0); //valid zzstate only

			//Menu fixed limits and relations:

			

// USE /[MANUAL TRN OVERRQ T16AGENT]/
			...
			SelectQuery qs = sp.getSelectQueryFromListingMVC(trn_menu_t16agentConds, listing);
		...
		/// <summary>
		/// Initializes a new instance of the <see cref="TRN_Menu_T16AGENT_ViewModel" /> class.
		/// </summary>
		/// <param name="userContext">The current user request context</param>
		public TRN_Menu_T16AGENT_ViewModel(UserContext userContext) : base(userContext)
		{
		...
		public override CriteriaSet BuildCriteriaSet(CSGenio.framework.TableConfiguration.TableConfiguration tableConfig, NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			if (Menu == null)
				Menu = new TablePartial<TRN_Menu_T16AGENT_RowViewModel>();
			...
			{
				// EPH
				crs = Models.Agent.AddEPH<CSGenioAagent>(ref u, crs, "MLT16AGENT");
			...
			if (tableReload)
			{
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					crs.Equals(Models.Agent.AddEPH<CSGenioAagent>(ref u, null, "MLT16AGENT"));
			}
		...
		public void Load(CSGenio.framework.TableConfiguration.TableConfiguration tableConfig, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAagent> Qlisting, ref CriteriaSet conditions)
		{
			using (GenioDI.MetricsOtlp.RecordTime("menu_load_time", new List<KeyValuePair<string, object>>() {
				...
				CriteriaSet trn_menu_t16agentConds = CriteriaSet.And();
			...
			//Tooltip for EPHs affecting this viewmodel list
			{
				...
				List<Limit> area_EPH_limits = EPH_Limit_Filler(ref limit, model_limit_area, "MLT16AGENT");
				...
				conditions.SubSets.Add(trn_menu_t16agentConds);
				trn_menu_t16agentConds = BuildCriteriaSet(tableConfig, requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
...
// USE /[MANUAL TRN OVERRQ T16AGENT]/
				...
				if (isToExport)
				{
					...
					Qlisting = Models.ModelBase.Where<CSGenioAagent>(m_userContext, false, trn_menu_t16agentConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "MLT16AGENT", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL TRN OVERRQLSTEXP T16AGENT]/
				...
				if (tableReload)
				{
// USE /[MANUAL TRN OVERRQLIST T16AGENT]/
					...
					if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					{
						var m_iCurPag = m_userContext.PersistentSupport.getPagingPos(CSGenioAagent.GetInformation(), QMVC_POS_RECORD, sorts, trn_menu_t16agentConds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
						if (m_iCurPag != -1)
							pageNumber = ((m_iCurPag - 1) / numberListItems) + 1;
					}

					ListingMVC<CSGenioAagent> listing = Models.ModelBase.Where<CSGenioAagent>(m_userContext, false, trn_menu_t16agentConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "MLT16AGENT", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn, fieldsWithTotalizers, tableConfig.SelectedRows);
					...
					Menu.Elements = MapTRN_Menu_T16AGENT(listing);

					Menu.Identifier = "MLT16AGENT";
					Menu.Slots = new Dictionary<string, List<object>>();

					// Last updated by [CJP] at [2015.02.03]
					// Adds the identifier to each element
					foreach (var element in Menu.Elements)
						element.Identifier = "MLT16AGENT";
		...
		private List<TRN_Menu_T16AGENT_RowViewModel> MapTRN_Menu_T16AGENT(ListingMVC<CSGenioAagent> Qlisting)
		{
			var Elements = new List<TRN_Menu_T16AGENT_RowViewModel>();
			int i = 0;
			...
			if (Qlisting.Rows != null)
			{
				foreach (var row in Qlisting.Rows)
				{
					if (Qlisting.NumRegs > 0 && i >= Qlisting.NumRegs) // Copiado da versão antiga do RowsToViewModels
						break;
					Elements.Add(MapTRN_Menu_T16AGENT(row));
					i++;
				}
			}
		...
		/// to a TRN_Menu_T16AGENT_RowViewModel object.
		/// </summary>
		/// <param name="row">The row.</param>
		private TRN_Menu_T16AGENT_RowViewModel MapTRN_Menu_T16AGENT(CSGenioAagent row)
		{
			var model = new TRN_Menu_T16AGENT_RowViewModel(m_userContext, true, _fieldsToSerialize);
		...
		/// </summary>
		public void CalculateButtonPermissions(TRN_Menu_T16AGENT_RowViewModel model)
		{
			bool canView = true;
			bool canEdit = true;
			bool canDelete = true;
		...
		#region Custom code
// USE /[MANUAL GQT VIEWMODEL_CUSTOM TRN_MENU_T16AGENT]/
		#endregion
