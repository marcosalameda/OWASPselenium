...
{
	public class Asset_ViewModel : FormViewModel<Models.Asset>, IPreparableForSerialization
	{
		[JsonIgnore]
		...
		/// </summary>
		[JsonIgnore]
		public SelectList List_ValCategory { get; set; }
		/// <summary>
		...
		public Asset_ViewModel() : base(null!) { }

		public Asset_ViewModel(UserContext userContext, bool nestedForm = false) : base(userContext, "FASSET", nestedForm) { }

		public Asset_ViewModel(UserContext userContext, Models.Asset row, bool nestedForm = false) : base(userContext, "FASSET", row, nestedForm) { }

		public Asset_ViewModel(UserContext userContext, string id, bool nestedForm = false, string[]? fieldsToLoad = null) : this(userContext, nestedForm)
		{
			this.Navigation.SetValue("asset", id);
			Model = Models.Asset.Find(id, userContext, "FASSET", fieldsToQuery: fieldsToLoad);
			if (Model == null)
		...
		public static StatusMessage InsertConditions(UserContext userContext)
		{
			...
			Models.Asset model = new Models.Asset(userContext) { Identifier = "FASSET" };

			var navigation = m_userContext.CurrentNavigation;
			// The "LoadKeysFromHistory" must be after the "LoadEPH" because the PHE's in the tree mark Foreign Keys to null
			// (since they cannot assign multiple values to a single field) and thus the value that comes from Navigation is lost.
			// And this makes it more like the order of loading the model when opening the form.
			model.LoadEPH("FASSET");
			if (navigation != null)
		...
		public override void MapFromModel(Models.Asset m)
		{
			if (m == null)
			{
				CSGenio.framework.Log.Error("Map Model (Asset) to ViewModel (Asset) - Model is a null reference");
				throw new ModelNotFoundException("Model not found");
			}
			...
			{
				CSGenio.framework.Log.Error("Map Model (Asset) to ViewModel (Asset) - Error during mapping");
				throw;
			}
		}
		...
		public override void MapToModel(Models.Asset m)
		{
			if (m == null)
			{
				CSGenio.framework.Log.Error("Map ViewModel (Asset) to Model (Asset) - Model is a null reference");
				throw new ModelNotFoundException("Model not found");
			}
			...
			{
				CSGenio.framework.Log.Error($"Map ViewModel (Asset) to Model (Asset) - Error during mapping. All user values: {HasDisabledUserValuesSecurity}");
				throw;
			}
		}
			...
			try
			{
				...
				switch (fullFieldName)
				{
					...
					case "asset.codasset":
						...
						break;
					default:
						Log.Error($"SetViewModelValue (Asset) - Unexpected field identifier {fullFieldName}");
						break;
				}
			}
			catch (Exception ex)
			{
				throw new FrameworkException(Resources.Resources.PEDIMOS_DESCULPA__OC63848, "SetViewModelValue (Asset)", "Unexpected error", ex);
			}
		}
		...
		{
			try { Model = Models.Asset.Find(id ?? Navigation.GetStrValue("asset"), m_userContext, "FASSET"); }
			finally { Model ??= new Models.Asset(m_userContext) { Identifier = "FASSET" }; }
		...
		public override void Load(NameValueCollection qs, bool editable, bool ajaxRequest = false, bool lazyLoad = false)
		{
			...
			try
			{
				Model = Models.Asset.Find(Navigation.GetStrValue("asset"), m_userContext, "FASSET");
			}
			finally
			{
				if (Model == null)
		...
		public override void LoadPartial(NameValueCollection qs, bool lazyLoad = false)
		{
			...
			if (Model == null)
			{
				...
				if (Model == null)
				{
					Model = new Models.Asset(m_userContext) { Identifier = "FASSET" };
			...
			Load_Asset___manufname____(qs, lazyLoad);
			Load_Asset___kindedesignat(qs, lazyLoad);
// USE /[MANUAL GQT VIEWMODEL_LOADPARTIAL ASSET]/
		}

// USE /[MANUAL GQT VIEWMODEL_NEW ASSET]/
...
// USE /[MANUAL GQT VIEWMODEL_SAVE ASSET]/
...
// USE /[MANUAL GQT VIEWMODEL_APPLY ASSET]/

// USE /[MANUAL GQT VIEWMODEL_DUPLICATE ASSET]/

// USE /[MANUAL GQT VIEWMODEL_DESTROY ASSET]/
		public override void Destroy(string id)
		{
			Model = Models.Asset.Find(id, m_userContext, "FASSET");
			if (Model == null)
				throw new ModelNotFoundException("Model not found");
		...
		/// <param name="qs"></param>
		/// <param name="lazyLoad">Lazy loading of dropdown items</param>
		public void Load_Asset___manufname____(NameValueCollection qs, bool lazyLoad = false)
		{
			bool asset___manufname____DoLoad = true;
			CriteriaSet asset___manufname____Conds = CriteriaSet.And();
			{
				object hValue = Navigation.GetValue("manuf", true);
				if (hValue != null && !(hValue is Array) && !string.IsNullOrEmpty(Convert.ToString(hValue)))
				{
					asset___manufname____Conds.Equal(CSGenioAmanuf.FldCodentit, hValue);
					this.ValCodmanuf = DBConversion.ToString(hValue);
				}
			}
			// Limits Generation

			object asset___manufname_____flimitmanuf_manufact = "1";
			asset___manufname____Conds.Equal(
				CSGenio.business.CSGenioAmanuf.FldManufact,
				asset___manufname_____flimitmanuf_manufact);
			...
			if (lazyLoad)
			{
				...
				FillDependant_AssetTableManufName(lazyLoad);
				return;
			}

			if (asset___manufname____DoLoad)
			{
				...
				asset___manufname____Conds.SubSet(search_filters);
...
// USE /[MANUAL GQT OVERRQ ASSET_MANUFNAME]/
				...
				*/
				if (Navigation.checkFormMode("manuf", FormMode.New) || Navigation.checkFormMode("manuf", FormMode.Duplicate))
					asset___manufname____Conds.SubSet(CriteriaSet.Or()
						.Equal(CSGenioAmanuf.FldZzstate, 0)
						.Equal(CSGenioAmanuf.FldCodentit, Navigation.GetStrValue("manuf")));
				else
					asset___manufname____Conds.Criterias.Add(new Criteria(new ColumnReference(CSGenioAmanuf.FldZzstate), CriteriaOperator.Equal, 0));

				FieldRef firstVisibleColumn = new FieldRef("manuf", "name");
				ListingMVC<CSGenioAmanuf> listing = Models.ModelBase.Where<CSGenioAmanuf>(m_userContext, false, asset___manufname____Conds, fields, offset, numberItems, sorts, "LED_ASSET___MANUFNAME____", true, false, firstVisibleColumn: firstVisibleColumn);
				...
				TableManufName.Elements = listing.RowsForViewModel((r) => new GenioMVC.Models.Manuf(m_userContext, r, true, _fieldsToSerialize_ASSET___MANUFNAME____));
				...
				FillDependant_AssetTableManufName();
			}
		}
		...
		public ConcurrentDictionary<string, object> GetDependant_AssetTableManufName(string PKey)
		{
		...
		/// </summary>
		/// <param name="lazyLoad">Lazy loading of dropdown items</param>
		public void FillDependant_AssetTableManufName(bool lazyLoad = false)
		{
			var row = GetDependant_AssetTableManufName(this.ValCodmanuf);
			try
			{
		...
		private readonly string[] _fieldsToSerialize_ASSET___MANUFNAME____ = ["Manuf", "Manuf.ValCodentit", "Manuf.ValZzstate", "Manuf.ValName", "Manuf.ValInitials"];
		...
		/// <param name="qs"></param>
		/// <param name="lazyLoad">Lazy loading of dropdown items</param>
		public void Load_Asset___kindedesignat(NameValueCollection qs, bool lazyLoad = false)
		{
			bool asset___kindedesignatDoLoad = true;
			CriteriaSet asset___kindedesignatConds = CriteriaSet.And();
			{
				object hValue = Navigation.GetValue("kinde", true);
				if (hValue != null && !(hValue is Array) && !string.IsNullOrEmpty(Convert.ToString(hValue)))
				{
					asset___kindedesignatConds.Equal(CSGenioAkinde.FldCodkinde, hValue);
			...
			if (lazyLoad)
			{
				...
				FillDependant_AssetTableKindeDesignat(lazyLoad);
				return;
			}

			if (asset___kindedesignatDoLoad)
			{
				List<ColumnSort> sorts = [];
				...
				asset___kindedesignatConds.SubSet(search_filters);
...
// USE /[MANUAL GQT OVERRQ ASSET_KINDEDESIGNAT]/
				...
				*/
				if (Navigation.checkFormMode("kinde", FormMode.New) || Navigation.checkFormMode("kinde", FormMode.Duplicate))
					asset___kindedesignatConds.SubSet(CriteriaSet.Or()
						.Equal(CSGenioAkinde.FldZzstate, 0)
						.Equal(CSGenioAkinde.FldCodkinde, Navigation.GetStrValue("kinde")));
				else
					asset___kindedesignatConds.Criterias.Add(new Criteria(new ColumnReference(CSGenioAkinde.FldZzstate), CriteriaOperator.Equal, 0));

				FieldRef firstVisibleColumn = new FieldRef("kinde", "designat");
				ListingMVC<CSGenioAkinde> listing = Models.ModelBase.Where<CSGenioAkinde>(m_userContext, false, asset___kindedesignatConds, fields, offset, numberItems, sorts, "LED_ASSET___KINDEDESIGNAT", true, false, firstVisibleColumn: firstVisibleColumn);
				...
				TableKindeDesignat.Elements = listing.RowsForViewModel((r) => new GenioMVC.Models.Kinde(m_userContext, r, true, _fieldsToSerialize_ASSET___KINDEDESIGNAT));
				...
				FillDependant_AssetTableKindeDesignat();
			}
		}
		...
		public ConcurrentDictionary<string, object> GetDependant_AssetTableKindeDesignat(string PKey)
		{
		...
		/// </summary>
		/// <param name="lazyLoad">Lazy loading of dropdown items</param>
		public void FillDependant_AssetTableKindeDesignat(bool lazyLoad = false)
		{
			var row = GetDependant_AssetTableKindeDesignat(this.ValCodkinde);
			try
			{
		...
		private readonly string[] _fieldsToSerialize_ASSET___KINDEDESIGNAT = ["Kinde", "Kinde.ValCodkinde", "Kinde.ValZzstate", "Kinde.ValDesignat"];
...
// USE /[MANUAL GQT VIEWMODEL_CUSTOM ASSET]/
