...
// USE /[MANUAL GQT LIST_LIMITS EQUIPM_PSEUDA_TAGS]/
		...
		public void LoadToExport(out ListingMVC<CSGenioAatags> listing, out CriteriaSet conditions, out List<Exports.QColumn> columns, CSGenio.core.framework.table.TableConfiguration tableConfig, NameValueCollection requestValues, bool ajaxRequest = false)
		{
			listing = null;
			conditions = null;
		...
		/// <param name="Qlisting">The rows.</param>
		/// <param name="conditions">The conditions.</param>
		public void Load(CSGenio.core.framework.table.TableConfiguration tableConfig, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAatags> Qlisting, ref CriteriaSet conditions)
		{
			User u = m_userContext.User;
			Menu = new GridTableList<GenioMVC.ViewModels.Atags.Equipm__pseuda_tags___ViewModel>(m_userContext);
			...
			List<ColumnSort> sorts = GetRequestSorts(this.Menu, tableConfig, "atags", allSortOrders);
...
// USE /[MANUAL GQT OVERRQ EQUIPM_PSEUDA_TAGS]/
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.BuildListingForExport<CSGenioAatags>(m_userContext, false, ref equipm__pseuda_tags__Conds, exportFieldRefs, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_EQUIPM__PSEUDA_TAGS__", true, firstVisibleColumn: firstVisibleColumn);
