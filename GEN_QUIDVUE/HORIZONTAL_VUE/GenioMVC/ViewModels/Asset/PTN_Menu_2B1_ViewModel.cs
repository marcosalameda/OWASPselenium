...
{
	public class PTN_Menu_2B1_ViewModel : MenuListViewModel<Models.Asset>
	{
		/// <summary>
		/// Gets or sets the object that represents the table and its elements.
		/// </summary>
		[JsonPropertyName("table")]
		public TablePartial<PTN_Menu_2B1_RowViewModel> Menu { get; set; }
		...
		public override string Uuid => "22ba1123-b517-443c-b7e3-1305c3b39104";
		...
		{
// USE /[MANUAL PTN LIST_LIMITS 2B1]/
		...
		public PTN_Menu_2B1_ViewModel() : base(null!) { }

		/// <summary>
		/// Initializes a new instance of the <see cref="PTN_Menu_2B1_ViewModel" /> class.
		/// </summary>
		/// <param name="userContext">The current user request context</param>
		public PTN_Menu_2B1_ViewModel(UserContext userContext) : base(userContext)
		{
		...
		public PTN_Menu_2B1_ViewModel(UserContext userContext, Models.ModelBase parentCtx) : this(userContext)
		{
			ParentCtx = parentCtx;
		}
		...
		public override CriteriaSet BuildCriteriaSet(CSGenio.core.framework.table.TableConfiguration tableConfig, NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			Menu ??= new TablePartial<PTN_Menu_2B1_RowViewModel>();
			...
			{
				// EPH
				crs = Models.Asset.AddEPH<CSGenioAasset>(ref u, crs, "ML2B1");
			...
			if (tableReload)
			{
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					crs.Equals(Models.Asset.AddEPH<CSGenioAasset>(ref u, null, "ML2B1"));
			}
		...
		public void Load(CSGenio.core.framework.table.TableConfiguration tableConfig, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAasset> Qlisting, ref CriteriaSet conditions)
		{
			...
			CriteriaSet ptn_menu_2b1Conds = CriteriaSet.And();
			...
			conditions.SubSets.Add(ptn_menu_2b1Conds);
			ptn_menu_2b1Conds = BuildCriteriaSet(tableConfig, requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
			tableReload &= hasAllRequiredLimits;

// USE /[MANUAL PTN OVERRQ 2B1]/
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.BuildListingForExport<CSGenioAasset>(m_userContext, false, ref ptn_menu_2b1Conds, exportFieldRefs, (pageNumber - 1) * numberListItems, numberListItems, sorts, "ML2B1", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL PTN OVERRQLSTEXP 2B1]/
...
// USE /[MANUAL PTN OVERRQLIST 2B1]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = m_userContext.PersistentSupport.getPagingPos(CSGenioAasset.GetInformation(), QMVC_POS_RECORD, sorts, ptn_menu_2b1Conds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
						pageNumber = ((m_iCurPag - 1) / numberListItems) + 1;
				}

				ListingMVC<CSGenioAasset> listing = Models.ModelBase.Where<CSGenioAasset>(m_userContext, distinct, ptn_menu_2b1Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "ML2B1", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn, fieldsWithTotalizers, tableConfig.SelectedRows);
				...
				Menu.Elements = MapPTN_Menu_2B1(listing);
		...
		private List<PTN_Menu_2B1_RowViewModel> MapPTN_Menu_2B1(ListingMVC<CSGenioAasset> Qlisting)
		{
			List<PTN_Menu_2B1_RowViewModel> Elements = [];
			int i = 0;
			...
			if (Qlisting.Rows != null)
			{
				foreach (var row in Qlisting.Rows)
				{
					if (Qlisting.NumRegs > 0 && i >= Qlisting.NumRegs) // Copiado da versão antiga do RowsToViewModels
						break;
					Elements.Add(MapPTN_Menu_2B1(row));
					i++;
				}
			}
		...
		/// to a PTN_Menu_2B1_RowViewModel object.
		/// </summary>
		/// <param name="row">The row.</param>
		private PTN_Menu_2B1_RowViewModel MapPTN_Menu_2B1(CSGenioAasset row)
		{
			var model = new PTN_Menu_2B1_RowViewModel(m_userContext, true, _fieldsToSerialize);
			if (row == null)
				return model;
...
// USE /[MANUAL GQT VIEWMODEL_CUSTOM PTN_MENU_2B1]/
