...
{
	public class TBS_Menu_1A1_ViewModel : MenuListViewModel<Models.Asset>
	{
		/// <summary>
		/// Gets or sets the object that represents the table and its elements.
		/// </summary>
		[JsonPropertyName("table")]
		public TablePartial<TBS_Menu_1A1_RowViewModel> Menu { get; set; }
		...
		public override string Uuid => "5ab779f3-1df6-4903-a7a0-c9dc77297b18";
		...
		{
// USE /[MANUAL TBS LIST_LIMITS 1A1]/
		...
		public override int GetCount(User user)
		{
			...
			var areaBase = CSGenio.business.Area.createArea("asset", user, "TBS");

			//gets eph conditions to be applied in listing
			CriteriaSet conditions = CSGenio.business.Listing.CalculateConditionsEphGeneric(areaBase, "ML1A1");
		...
		public TBS_Menu_1A1_ViewModel() : base(null!) { }

		/// <summary>
		/// Initializes a new instance of the <see cref="TBS_Menu_1A1_ViewModel" /> class.
		/// </summary>
		/// <param name="userContext">The current user request context</param>
		public TBS_Menu_1A1_ViewModel(UserContext userContext) : base(userContext)
		{
		...
		public TBS_Menu_1A1_ViewModel(UserContext userContext, Models.ModelBase parentCtx) : this(userContext)
		{
			ParentCtx = parentCtx;
		}
		...
		public override CriteriaSet BuildCriteriaSet(CSGenio.core.framework.table.TableConfiguration tableConfig, NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			Menu ??= new TablePartial<TBS_Menu_1A1_RowViewModel>();
			...
			{
				// EPH
				crs = Models.Asset.AddEPH<CSGenioAasset>(ref u, crs, "ML1A1");
			...
			if (tableReload)
			{
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					crs.Equals(Models.Asset.AddEPH<CSGenioAasset>(ref u, null, "ML1A1"));
			}
		...
		public void Load(CSGenio.core.framework.table.TableConfiguration tableConfig, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAasset> Qlisting, ref CriteriaSet conditions)
		{
			...
			CriteriaSet tbs_menu_1a1Conds = CriteriaSet.And();
			...
			//Tooltip for EPHs affecting this viewmodel list
			{
				...
				List<Limit> area_EPH_limits = EPH_Limit_Filler(ref limit, model_limit_area, "ML1A1");
			...
			conditions.SubSets.Add(tbs_menu_1a1Conds);
			tbs_menu_1a1Conds = BuildCriteriaSet(tableConfig, requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
			tableReload &= hasAllRequiredLimits;

// USE /[MANUAL TBS OVERRQ 1A1]/
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.BuildListingForExport<CSGenioAasset>(m_userContext, false, ref tbs_menu_1a1Conds, exportFieldRefs, (pageNumber - 1) * numberListItems, numberListItems, sorts, "ML1A1", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL TBS OVERRQLSTEXP 1A1]/
...
// USE /[MANUAL TBS OVERRQLIST 1A1]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = m_userContext.PersistentSupport.getPagingPos(CSGenioAasset.GetInformation(), QMVC_POS_RECORD, sorts, tbs_menu_1a1Conds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
						pageNumber = ((m_iCurPag - 1) / numberListItems) + 1;
				}

				ListingMVC<CSGenioAasset> listing = Models.ModelBase.Where<CSGenioAasset>(m_userContext, distinct, tbs_menu_1a1Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "ML1A1", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn, fieldsWithTotalizers, tableConfig.SelectedRows);
				...
				Menu.Elements = MapTBS_Menu_1A1(listing);

				Menu.Identifier = "ML1A1";
				Menu.Slots = new Dictionary<string, List<object>>();

				// Last updated by [CJP] at [2015.02.03]
				// Adds the identifier to each element
				foreach (var element in Menu.Elements)
					element.Identifier = "ML1A1";
		...
		private List<TBS_Menu_1A1_RowViewModel> MapTBS_Menu_1A1(ListingMVC<CSGenioAasset> Qlisting)
		{
			List<TBS_Menu_1A1_RowViewModel> Elements = [];
			int i = 0;
			...
			if (Qlisting.Rows != null)
			{
				foreach (var row in Qlisting.Rows)
				{
					if (Qlisting.NumRegs > 0 && i >= Qlisting.NumRegs) // Copiado da versão antiga do RowsToViewModels
						break;
					Elements.Add(MapTBS_Menu_1A1(row));
					i++;
				}
			}
		...
		/// to a TBS_Menu_1A1_RowViewModel object.
		/// </summary>
		/// <param name="row">The row.</param>
		private TBS_Menu_1A1_RowViewModel MapTBS_Menu_1A1(CSGenioAasset row)
		{
			var model = new TBS_Menu_1A1_RowViewModel(m_userContext, true, _fieldsToSerialize);
			if (row == null)
				return model;
...
// USE /[MANUAL GQT VIEWMODEL_CUSTOM TBS_MENU_1A1]/
