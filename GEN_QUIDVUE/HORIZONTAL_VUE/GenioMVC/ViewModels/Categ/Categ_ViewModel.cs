...
{
	public class Categ_ViewModel : FormViewModel<Models.Categ>, IPreparableForSerialization
	{
		[JsonIgnore]
		...
		/// <summary>
		/// Title: "Professional abbreviation" | Type: "C"
		/// </summary>
		...
		public Categ_ViewModel() : base(null!) { }

		public Categ_ViewModel(UserContext userContext, bool nestedForm = false) : base(userContext, "FCATEG", nestedForm) { }

		public Categ_ViewModel(UserContext userContext, Models.Categ row, bool nestedForm = false) : base(userContext, "FCATEG", row, nestedForm) { }

		public Categ_ViewModel(UserContext userContext, string id, bool nestedForm = false, string[]? fieldsToLoad = null) : this(userContext, nestedForm)
		{
			this.Navigation.SetValue("categ", id);
			Model = Models.Categ.Find(id, userContext, "FCATEG", fieldsToQuery: fieldsToLoad);
			if (Model == null)
		...
		public static StatusMessage InsertConditions(UserContext userContext)
		{
			...
			Models.Categ model = new Models.Categ(userContext) { Identifier = "FCATEG" };

			var navigation = m_userContext.CurrentNavigation;
			// The "LoadKeysFromHistory" must be after the "LoadEPH" because the PHE's in the tree mark Foreign Keys to null
			// (since they cannot assign multiple values to a single field) and thus the value that comes from Navigation is lost.
			// And this makes it more like the order of loading the model when opening the form.
			model.LoadEPH("FCATEG");
			if (navigation != null)
		...
		public override void MapToModel(Models.Categ m)
		{
			...
			{
				CSGenio.framework.Log.Error($"Map ViewModel (Categ) to Model (Categ) - Error during mapping. All user values: {HasDisabledUserValuesSecurity}");
				throw;
			}
		}
		...
		public override void SetViewModelValue(string fullFieldName, object value)
		{
			try
			{
				...
				{
					case "categ.categoria":
						this.ValCategoria = ViewModelConversion.ToString(_value);
						break;
					case "categ.abbreviation":
						this.ValAbbreviation = ViewModelConversion.ToString(_value);
						break;
					case "categ.codcateg":
						this.ValCodcateg = ViewModelConversion.ToString(_value);
						break;
					default:
						Log.Error($"SetViewModelValue (Categ) - Unexpected field identifier {fullFieldName}");
						break;
				}
			}
			catch (Exception ex)
			{
				throw new FrameworkException(Resources.Resources.PEDIMOS_DESCULPA__OC63848, "SetViewModelValue (Categ)", "Unexpected error", ex);
			}
		}
		...
		{
			try { Model = Models.Categ.Find(id ?? Navigation.GetStrValue("categ"), m_userContext, "FCATEG"); }
			finally { Model ??= new Models.Categ(m_userContext) { Identifier = "FCATEG" }; }
		...
		public override void Load(NameValueCollection qs, bool editable, bool ajaxRequest = false, bool lazyLoad = false)
		{
			...
			try
			{
				Model = Models.Categ.Find(Navigation.GetStrValue("categ"), m_userContext, "FCATEG");
			}
			finally
			{
				if (Model == null)
		...
		public override void LoadPartial(NameValueCollection qs, bool lazyLoad = false)
		{
			...
			if (Model == null)
			{
				...
				if (Model == null)
				{
					Model = new Models.Categ(m_userContext) { Identifier = "FCATEG" };
...
// USE /[MANUAL GQT VIEWMODEL_NEW CATEG]/
		...
		public override CrudViewModelValidationResult Validate()
		{
			...
			validator.StringLength("ValAbbreviation", Resources.Resources.PROFESSIONAL_ABBREVI57700, ValAbbreviation, 10);
...
// USE /[MANUAL GQT VIEWMODEL_DESTROY CATEG]/
		...
		{
			Model = Models.Categ.Find(id, m_userContext, "FCATEG");
			if (Model == null)
				throw new ModelNotFoundException("Model not found");
		...
		protected override object GetViewModelValue(string identifier, object modelValue)
		{
			return identifier switch
			{
				...
				"categ.abbreviation" => ViewModelConversion.ToString(modelValue),
