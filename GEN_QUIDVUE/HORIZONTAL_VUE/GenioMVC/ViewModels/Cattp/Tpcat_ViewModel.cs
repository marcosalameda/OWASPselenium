...
{
	public class Tpcat_ViewModel : FormViewModel<Models.Cattp>, IPreparableForSerialization
	{
		[JsonIgnore]
		...
		public override void MapToModel(Models.Cattp m)
		{
			...
			{
				CSGenio.framework.Log.Error($"Map ViewModel (Tpcat) to Model (Cattp) - Error during mapping. All user values: {HasDisabledUserValuesSecurity}");
				throw;
			}
		}
			...
			try
			{
				...
				switch (fullFieldName)
				{
					case "cattp.codsbcat":
						...
						break;
					case "cattp.tpcatego":
						this.ValTpcatego = ViewModelConversion.ToString(_value);
						break;
					case "cattp.codtpcat":
						this.ValCodtpcat = ViewModelConversion.ToString(_value);
						break;
					default:
						Log.Error($"SetViewModelValue (Tpcat) - Unexpected field identifier {fullFieldName}");
						break;
				}
			}
			catch (Exception ex)
			{
				throw new FrameworkException(Resources.Resources.PEDIMOS_DESCULPA__OC63848, "SetViewModelValue (Tpcat)", "Unexpected error", ex);
			}
		}
		...
		{
			try { Model = Models.Cattp.Find(id ?? Navigation.GetStrValue("cattp"), m_userContext, "FTPCAT"); }
			finally { Model ??= new Models.Cattp(m_userContext) { Identifier = "FTPCAT" }; }
...
// USE /[MANUAL GQT VIEWMODEL_DESTROY TPCAT]/
		...
		public void Load_Tpcat___sbcatsubcateg(NameValueCollection qs, bool lazyLoad = false)
		{
			...
			CriteriaSet tpcat___sbcatsubcategConds = CriteriaSet.And();
			{
				...
				{
					tpcat___sbcatsubcategConds.Equal(CSGenioAsbcat.FldCodsbcat, hValue);
					this.ValCodsbcat = DBConversion.ToString(hValue);
				}
			}
			...
			if (tpcat___sbcatsubcategDoLoad)
			{
				...
				FieldRef[] fields = [CSGenioAsbcat.FldCodsbcat, CSGenioAsbcat.FldSubcateg, CSGenioAsbcat.FldZzstate];
...
// USE /[MANUAL GQT OVERRQ TPCAT_SBCATSUBCATEG]/
				...
				*/
				...
				TableSbcatSubcateg.Elements = listing.RowsForViewModel((r) => new GenioMVC.Models.Sbcat(m_userContext, r, true, _fieldsToSerialize_TPCAT___SBCATSUBCATEG));
