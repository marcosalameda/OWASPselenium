...
// USE /[MANUAL GQT LIST_LIMITS COMPTAB_PSEUDBEHAVIOR]/
		...
		public void LoadToExport(out ListingMVC<CSGenioAcompb> listing, out CriteriaSet conditions, out List<Exports.QColumn> columns, CSGenio.framework.TableConfiguration.TableConfiguration tableConfig, NameValueCollection requestValues, bool ajaxRequest = false)
		{
			listing = null;
			conditions = null;
		...
		/// <param name="Qlisting">The rows.</param>
		/// <param name="conditions">The conditions.</param>
		public void Load(CSGenio.framework.TableConfiguration.TableConfiguration tableConfig, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAcompb> Qlisting, ref CriteriaSet conditions)
		{
			using (GenioDI.MetricsOtlp.RecordTime("page_load_time", new System.Diagnostics.TagList([
				new("PageId", "COMPTAB.BEHAVIOR"),
			...
			]), "ms", "Time to load the page."))
			{
				...
				List<ColumnSort> sorts = GetRequestSorts(this.Menu, tableConfig.ColumnOrderBy, "compb", allSortOrders);
...
// USE /[MANUAL GQT OVERRQ COMPTAB_PSEUDBEHAVIOR]/
				...
				if (isToExport)
				{
					...
					Qlisting = Models.ModelBase.Where<CSGenioAcompb>(m_userContext, false, comptab_pseudbehaviorConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_COMPTAB_PSEUDBEHAVIOR", true, firstVisibleColumn: firstVisibleColumn);
