...
namespace GenioMVC.ViewModels.Conta
{
	public class TBS_Menu_141_ViewModel : ListViewModel
	{
		...
		public TablePartial<TBS_Menu_141_RowViewModel> Menu { get; set; }
		...
		public override int GetCount(User user)
		{
			...
			CriteriaSet tbs_menu_141Conds = CSGenio.business.Listing.CalculateConditionsEphGeneric(areaBase, "ML141");
			tbs_menu_141Conds.Equal(CSGenioAconta.FldZzstate, 0); //valid zzstate only
...
// USE /[MANUAL TBS OVERRQ 141]/
			...
			SelectQuery qs = sp.getSelectQueryFromListingMVC(tbs_menu_141Conds, listing);
		...
		public TBS_Menu_141_ViewModel(UserContext userContext) : base(userContext)
		{
		...
		public void Load(CSGenio.framework.TableConfiguration.TableConfiguration tableConfig, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAconta> Qlisting, ref CriteriaSet conditions)
		{
			using (GenioDI.MetricsOtlp.RecordTime("menu_load_time", new List<KeyValuePair<string, object>>() {
				...
				CriteriaSet tbs_menu_141Conds = CriteriaSet.And();
			...
			//Tooltip for EPHs affecting this viewmodel list
			{
				...
				conditions.SubSets.Add(tbs_menu_141Conds);
				tbs_menu_141Conds = BuildCriteriaSet(tableConfig, requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
...
// USE /[MANUAL TBS OVERRQ 141]/
				...
				if (isToExport)
				{
					...
					Qlisting = Models.ModelBase.Where<CSGenioAconta>(m_userContext, false, tbs_menu_141Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "ML141", true, firstVisibleColumn: firstVisibleColumn);
...
// USE /[MANUAL TBS OVERRQLIST 141]/
					...
					if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					{
						var m_iCurPag = m_userContext.PersistentSupport.getPagingPos(CSGenioAconta.GetInformation(), QMVC_POS_RECORD, sorts, tbs_menu_141Conds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
						if (m_iCurPag != -1)
		...
		{
			var Elements = new List<TBS_Menu_141_RowViewModel>();
			int i = 0;
		...
		/// to a TBS_Menu_141_RowViewModel object.
		/// </summary>
		/// <param name="row">The row.</param>
		private TBS_Menu_141_RowViewModel MapTBS_Menu_141(CSGenioAconta row)
		{
			var model = new TBS_Menu_141_RowViewModel(m_userContext, true, _fieldsToSerialize);
		...
		/// </summary>
		public void CalculateButtonPermissions(TBS_Menu_141_RowViewModel model)
		{
			bool canView = true;
			bool canEdit = true;
			bool canDelete = true;
...
// USE /[MANUAL GQT VIEWMODEL_CUSTOM TBS_MENU_141]/
		...
		[
			"Conta", "Conta.ValCodconta", "Conta.ValZzstate", "Pesso", "Pesso.ValName", "Tpcon", "Tpcon.ValTipocont", "Conta.ValContacto", "Conta.ValCodgenre", "Conta.ValCodpesso", "Conta.ValCodtpcon", "BtnPermission"
		];
