...
// USE /[MANUAL GQT LIST_LIMITS ENTIT_FACI2NAME]/
		...
		public void LoadToExport(out ListingMVC<CSGenioAfaci2> listing, out CriteriaSet conditions, out List<Exports.QColumn> columns, CSGenio.core.framework.table.TableConfiguration tableConfig, NameValueCollection requestValues, bool ajaxRequest = false)
		{
			listing = null;
			conditions = null;
		...
		/// <param name="Qlisting">The rows.</param>
		/// <param name="conditions">The conditions.</param>
		public void Load(CSGenio.core.framework.table.TableConfiguration tableConfig, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAfaci2> Qlisting, ref CriteriaSet conditions)
		{
			User u = m_userContext.User;
			Menu = new TablePartial<Entit_Faci2ValName_RowViewModel>();
			...
			List<ColumnSort> sorts = GetRequestSorts(this.Menu, tableConfig, "faci2", allSortOrders);
...
// USE /[MANUAL GQT OVERRQ ENTIT_FACI2NAME]/
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.BuildListingForExport<CSGenioAfaci2>(m_userContext, false, ref entit___faci2name____Conds, exportFieldRefs, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_ENTIT___FACI2NAME____", true, firstVisibleColumn: firstVisibleColumn);
