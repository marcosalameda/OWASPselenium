...
{
	public class Equdocum_ViewModel : FormViewModel<Models.Equip>, IPreparableForSerialization
	{
		[JsonIgnore]
		...
		public Equdocum_ViewModel() : base(null!) { }

		public Equdocum_ViewModel(UserContext userContext, bool nestedForm = false) : base(userContext, "FEQUDOCUM", nestedForm) { }

		public Equdocum_ViewModel(UserContext userContext, Models.Equip row, bool nestedForm = false) : base(userContext, "FEQUDOCUM", row, nestedForm) { }

		public Equdocum_ViewModel(UserContext userContext, string id, bool nestedForm = false, string[]? fieldsToLoad = null) : this(userContext, nestedForm)
		{
			this.Navigation.SetValue("equip", id);
			Model = Models.Equip.Find(id, userContext, "FEQUDOCUM", fieldsToQuery: fieldsToLoad);
			if (Model == null)
		...
		public static StatusMessage InsertConditions(UserContext userContext)
		{
			...
			Models.Equip model = new Models.Equip(userContext) { Identifier = "FEQUDOCUM" };

			var navigation = m_userContext.CurrentNavigation;
			// The "LoadKeysFromHistory" must be after the "LoadEPH" because the PHE's in the tree mark Foreign Keys to null
			// (since they cannot assign multiple values to a single field) and thus the value that comes from Navigation is lost.
			// And this makes it more like the order of loading the model when opening the form.
			model.LoadEPH("FEQUDOCUM");
			if (navigation != null)
		...
		public override void MapFromModel(Models.Equip m)
		{
			if (m == null)
			{
				CSGenio.framework.Log.Error("Map Model (Equip) to ViewModel (Equdocum) - Model is a null reference");
				throw new ModelNotFoundException("Model not found");
			}
			...
			{
				CSGenio.framework.Log.Error("Map Model (Equip) to ViewModel (Equdocum) - Error during mapping");
				throw;
			}
		}
		...
		public override void MapToModel(Models.Equip m)
		{
			if (m == null)
			{
				CSGenio.framework.Log.Error("Map ViewModel (Equdocum) to Model (Equip) - Model is a null reference");
				throw new ModelNotFoundException("Model not found");
			}
			...
			{
				CSGenio.framework.Log.Error($"Map ViewModel (Equdocum) to Model (Equip) - Error during mapping. All user values: {HasDisabledUserValuesSecurity}");
				throw;
			}
		}
			...
			try
			{
				...
				switch (fullFieldName)
				{
					...
					case "equip.codequip":
						...
						break;
					default:
						Log.Error($"SetViewModelValue (Equdocum) - Unexpected field identifier {fullFieldName}");
						break;
				}
			}
			catch (Exception ex)
			{
				throw new FrameworkException(Resources.Resources.PEDIMOS_DESCULPA__OC63848, "SetViewModelValue (Equdocum)", "Unexpected error", ex);
			}
		}
		...
		{
			try { Model = Models.Equip.Find(id ?? Navigation.GetStrValue("equip"), m_userContext, "FEQUDOCUM"); }
			finally { Model ??= new Models.Equip(m_userContext) { Identifier = "FEQUDOCUM" }; }
		...
		public override void Load(NameValueCollection qs, bool editable, bool ajaxRequest = false, bool lazyLoad = false)
		{
			...
			try
			{
				Model = Models.Equip.Find(Navigation.GetStrValue("equip"), m_userContext, "FEQUDOCUM");
			}
			finally
			{
				if (Model == null)
		...
		public override void LoadPartial(NameValueCollection qs, bool lazyLoad = false)
		{
			...
			if (Model == null)
			{
				...
				if (Model == null)
				{
					Model = new Models.Equip(m_userContext) { Identifier = "FEQUDOCUM" };
...
// USE /[MANUAL GQT VIEWMODEL_NEW EQUDOCUM]/
...
// USE /[MANUAL GQT VIEWMODEL_SAVE EQUDOCUM]/
...
// USE /[MANUAL GQT VIEWMODEL_APPLY EQUDOCUM]/

// USE /[MANUAL GQT VIEWMODEL_DUPLICATE EQUDOCUM]/

// USE /[MANUAL GQT VIEWMODEL_DESTROY EQUDOCUM]/
		public override void Destroy(string id)
		{
			Model = Models.Equip.Find(id, m_userContext, "FEQUDOCUM");
			if (Model == null)
				throw new ModelNotFoundException("Model not found");
...
// USE /[MANUAL GQT VIEWMODEL_CUSTOM EQUDOCUM]/
