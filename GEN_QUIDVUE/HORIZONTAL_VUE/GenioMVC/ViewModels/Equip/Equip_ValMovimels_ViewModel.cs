...
// USE /[MANUAL GQT LIST_LIMITS EQUIP_PSEUDMOVIMELS]/
		...
		public void LoadToExport(out ListingMVC<CSGenioAmovim> listing, out CriteriaSet conditions, out List<Exports.QColumn> columns, CSGenio.framework.TableConfiguration.TableConfiguration tableConfig, NameValueCollection requestValues, bool ajaxRequest = false)
		{
			listing = null;
			conditions = null;
		...
		public void Load(CSGenio.framework.TableConfiguration.TableConfiguration tableConfig, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAmovim> Qlisting, ref CriteriaSet conditions)
		{
			...
			]), "ms", "Time to load the page."))
			{
				...
				List<ColumnSort> sorts = GetRequestSorts(this.Menu, tableConfig.ColumnOrderBy, "movim", allSortOrders);
...
// USE /[MANUAL GQT OVERRQ EQUIP_PSEUDMOVIMELS]/
				...
				if (isToExport)
				{
					...
					Qlisting = Models.ModelBase.Where<CSGenioAmovim>(m_userContext, false, equip___pseudmovimelsConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_EQUIP___PSEUDMOVIMELS", true, firstVisibleColumn: firstVisibleColumn);
