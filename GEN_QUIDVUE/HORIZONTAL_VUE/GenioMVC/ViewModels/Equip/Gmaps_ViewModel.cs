...
{
	public class Gmaps_ViewModel : FormViewModel<Models.Equip>, IPreparableForSerialization
	{
		[JsonIgnore]
		...
		public override void MapToModel(Models.Equip m)
		{
			...
			{
				CSGenio.framework.Log.Error($"Map ViewModel (Gmaps) to Model (Equip) - Error during mapping. All user values: {HasDisabledUserValuesSecurity}");
				throw;
			}
		}
			...
			try
			{
				...
				switch (fullFieldName)
				{
					case "equip.codequip":
						...
						break;
					default:
						Log.Error($"SetViewModelValue (Gmaps) - Unexpected field identifier {fullFieldName}");
						break;
				}
			}
			catch (Exception ex)
			{
				throw new FrameworkException(Resources.Resources.PEDIMOS_DESCULPA__OC63848, "SetViewModelValue (Gmaps)", "Unexpected error", ex);
			}
		}
		...
		{
			try { Model = Models.Equip.Find(id ?? Navigation.GetStrValue("equip"), m_userContext, "FGMAPS"); }
			finally { Model ??= new Models.Equip(m_userContext) { Identifier = "FGMAPS" }; }
