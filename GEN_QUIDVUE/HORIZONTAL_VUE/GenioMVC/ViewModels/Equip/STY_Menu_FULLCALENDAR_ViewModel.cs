...
{
	public class STY_Menu_FULLCALENDAR_ViewModel : ListViewModel
	{
		/// <summary>
		/// Gets or sets the object that represents the table and its elements. List type: "${exposeField.Fajuda}"
		/// </summary>
		[JsonPropertyName("Table")]
		public TablePartial<STY_Menu_FULLCALENDAR_RowViewModel> Menu { get; set; }
		...
		/// <inheritdoc/>
		public override string Uuid { get => "cb03d09a-7795-4e74-9423-745e49d6ec5f"; }
		...
		public override int GetCount(User user)
		{
			...
			CriteriaSet sty_menu_fullcalendarConds = CSGenio.business.Listing.CalculateConditionsEphGeneric(areaBase, "MLFULLCALENDAR");
			sty_menu_fullcalendarConds.Equal(CSGenioAequip.FldZzstate, 0); //valid zzstate only

			//Menu fixed limits and relations:

						sty_menu_fullcalendarConds.Equal(CSGenioAequip.FldShowrc, 1);


// USE /[MANUAL STY OVERRQ FULLCALENDAR]/
			...
			SelectQuery qs = sp.getSelectQueryFromListingMVC(sty_menu_fullcalendarConds, listing);
		...
		public STY_Menu_FULLCALENDAR_ViewModel(UserContext userContext) : base(userContext)
		{
		...
		public override CriteriaSet BuildCriteriaSet(CSGenio.framework.TableConfiguration.TableConfiguration tableConfig, NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			if (Menu == null)
				Menu = new TablePartial<STY_Menu_FULLCALENDAR_RowViewModel>();
			...
			{
				// EPH
				crs = Models.Equip.AddEPH<CSGenioAequip>(ref u, crs, "MLFULLCALENDAR");
			...
			if (tableReload)
			{
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					crs.Equals(Models.Equip.AddEPH<CSGenioAequip>(ref u, null, "MLFULLCALENDAR"));
			}
		...
		public void Load(CSGenio.framework.TableConfiguration.TableConfiguration tableConfig, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAequip> Qlisting, ref CriteriaSet conditions)
		{
			using (GenioDI.MetricsOtlp.RecordTime("menu_load_time", new List<KeyValuePair<string, object>>() {
				...
				CriteriaSet sty_menu_fullcalendarConds = CriteriaSet.And();
			...
			//Tooltip for EPHs affecting this viewmodel list
			{
				...
				List<Limit> area_EPH_limits = EPH_Limit_Filler(ref limit, model_limit_area, "MLFULLCALENDAR");
			...
			//1st Area Value: "1"
			{
				...
				conditions.SubSets.Add(sty_menu_fullcalendarConds);
				sty_menu_fullcalendarConds = BuildCriteriaSet(tableConfig, requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
...
// USE /[MANUAL STY OVERRQ FULLCALENDAR]/
				...
				if (isToExport)
				{
					...
					Qlisting = Models.ModelBase.Where<CSGenioAequip>(m_userContext, false, sty_menu_fullcalendarConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "MLFULLCALENDAR", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL STY OVERRQLSTEXP FULLCALENDAR]/
				...
				if (tableReload)
				{
// USE /[MANUAL STY OVERRQLIST FULLCALENDAR]/
					...
					if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					{
						var m_iCurPag = m_userContext.PersistentSupport.getPagingPos(CSGenioAequip.GetInformation(), QMVC_POS_RECORD, sorts, sty_menu_fullcalendarConds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
						if (m_iCurPag != -1)
							pageNumber = ((m_iCurPag - 1) / numberListItems) + 1;
					}

					ListingMVC<CSGenioAequip> listing = Models.ModelBase.Where<CSGenioAequip>(m_userContext, false, sty_menu_fullcalendarConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "MLFULLCALENDAR", true, true, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn, fieldsWithTotalizers, tableConfig.SelectedRows);
					...
					Menu.Elements = MapSTY_Menu_FULLCALENDAR(listing);

					Menu.Identifier = "MLFULLCALENDAR";
					Menu.Slots = new Dictionary<string, List<object>>();

					// Last updated by [CJP] at [2015.02.03]
					// Adds the identifier to each element
					foreach (var element in Menu.Elements)
						element.Identifier = "MLFULLCALENDAR";
		...
		private List<STY_Menu_FULLCALENDAR_RowViewModel> MapSTY_Menu_FULLCALENDAR(ListingMVC<CSGenioAequip> Qlisting)
		{
			var Elements = new List<STY_Menu_FULLCALENDAR_RowViewModel>();
			int i = 0;
			...
			if (Qlisting.Rows != null)
			{
				foreach (var row in Qlisting.Rows)
				{
					if (Qlisting.NumRegs > 0 && i >= Qlisting.NumRegs) // Copiado da versão antiga do RowsToViewModels
						break;
					Elements.Add(MapSTY_Menu_FULLCALENDAR(row));
					i++;
				}
			}
		...
		/// to a STY_Menu_FULLCALENDAR_RowViewModel object.
		/// </summary>
		/// <param name="row">The row.</param>
		private STY_Menu_FULLCALENDAR_RowViewModel MapSTY_Menu_FULLCALENDAR(CSGenioAequip row)
		{
			var model = new STY_Menu_FULLCALENDAR_RowViewModel(m_userContext, true, _fieldsToSerialize);
		...
		/// </summary>
		public void CalculateButtonPermissions(STY_Menu_FULLCALENDAR_RowViewModel model)
		{
			bool canView = true;
			bool canEdit = true;
			bool canDelete = true;
