...
{
	public class Timequip_ViewModel : FormViewModel<Models.Equip>, IPreparableForSerialization
	{
		[JsonIgnore]
		...
		public Timequip_ViewModel() : base(null!) { }

		public Timequip_ViewModel(UserContext userContext, bool nestedForm = false) : base(userContext, "FTIMEQUIP", nestedForm) { }

		public Timequip_ViewModel(UserContext userContext, Models.Equip row, bool nestedForm = false) : base(userContext, "FTIMEQUIP", row, nestedForm) { }

		public Timequip_ViewModel(UserContext userContext, string id, bool nestedForm = false, string[]? fieldsToLoad = null) : this(userContext, nestedForm)
		{
			this.Navigation.SetValue("equip", id);
			Model = Models.Equip.Find(id, userContext, "FTIMEQUIP", fieldsToQuery: fieldsToLoad);
			if (Model == null)
		...
		public static StatusMessage InsertConditions(UserContext userContext)
		{
			...
			Models.Equip model = new Models.Equip(userContext) { Identifier = "FTIMEQUIP" };

			var navigation = m_userContext.CurrentNavigation;
			// The "LoadKeysFromHistory" must be after the "LoadEPH" because the PHE's in the tree mark Foreign Keys to null
			// (since they cannot assign multiple values to a single field) and thus the value that comes from Navigation is lost.
			// And this makes it more like the order of loading the model when opening the form.
			model.LoadEPH("FTIMEQUIP");
			if (navigation != null)
		...
		public override void MapToModel(Models.Equip m)
		{
			...
			{
				CSGenio.framework.Log.Error($"Map ViewModel (Timequip) to Model (Equip) - Error during mapping. All user values: {HasDisabledUserValuesSecurity}");
				throw;
			}
		}
			...
			try
			{
				...
				switch (fullFieldName)
				{
					case "equip.codequip":
						...
						break;
					default:
						Log.Error($"SetViewModelValue (Timequip) - Unexpected field identifier {fullFieldName}");
						break;
				}
			}
			catch (Exception ex)
			{
				throw new FrameworkException(Resources.Resources.PEDIMOS_DESCULPA__OC63848, "SetViewModelValue (Timequip)", "Unexpected error", ex);
			}
		}
		...
		{
			try { Model = Models.Equip.Find(id ?? Navigation.GetStrValue("equip"), m_userContext, "FTIMEQUIP"); }
			finally { Model ??= new Models.Equip(m_userContext) { Identifier = "FTIMEQUIP" }; }
		...
		public override void Load(NameValueCollection qs, bool editable, bool ajaxRequest = false, bool lazyLoad = false)
		{
			...
			try
			{
				Model = Models.Equip.Find(Navigation.GetStrValue("equip"), m_userContext, "FTIMEQUIP");
			}
			finally
			{
				if (Model == null)
		...
		public override void LoadPartial(NameValueCollection qs, bool lazyLoad = false)
		{
			...
			if (Model == null)
			{
				...
				if (Model == null)
				{
					Model = new Models.Equip(m_userContext) { Identifier = "FTIMEQUIP" };
...
// USE /[MANUAL GQT VIEWMODEL_DESTROY TIMEQUIP]/
		...
		{
			Model = Models.Equip.Find(id, m_userContext, "FTIMEQUIP");
			if (Model == null)
				throw new ModelNotFoundException("Model not found");
