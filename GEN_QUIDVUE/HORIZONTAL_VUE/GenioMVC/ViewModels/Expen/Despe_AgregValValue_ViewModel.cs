...
{
	public class Despe_AgregValValue_ViewModel : MenuListViewModel<Models.Agreg>
	{
		/// <summary>
		/// Gets or sets the object that represents the table and its elements.
		/// </summary>
		[JsonPropertyName("Table")]
		public TablePartial<Despe_AgregValValue_RowViewModel> Menu { get; set; }
		...
		/// <inheritdoc/>
		public override string Uuid => "Despe_AgregValValue";
...
// USE /[MANUAL GQT LIST_LIMITS DESPE_AGREGVALUE]/
		...
		public Despe_AgregValValue_ViewModel() : base(null!) { }

		/// <summary>
		/// Initializes a new instance of the <see cref="Despe_AgregValValue_ViewModel" /> class.
		/// </summary>
		/// <param name="userContext">The current user request context</param>
		public Despe_AgregValValue_ViewModel(UserContext userContext) : base(userContext)
		{
		...
		public Despe_AgregValValue_ViewModel(UserContext userContext, Models.ModelBase parentCtx) : this(userContext)
		{
			ParentCtx = parentCtx;
		}
		...
		public void Load(CSGenio.framework.TableConfiguration.TableConfiguration tableConfig, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAagreg> Qlisting, ref CriteriaSet conditions)
		{
			...
			]), "ms", "Time to load the page."))
			{
				...
				conditions.SubSets.Add(despe___agregvalue___Conds);
				despe___agregvalue___Conds = BuildCriteriaSet(tableConfig, requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
...
// USE /[MANUAL GQT OVERRQ DESPE_AGREGVALUE]/
				...
				if (isToExport)
				{
					...
					Qlisting = Models.ModelBase.Where<CSGenioAagreg>(m_userContext, false, despe___agregvalue___Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_DESPE___AGREGVALUE___", true, firstVisibleColumn: firstVisibleColumn);
...
// USE /[MANUAL GQT OVERRQLIST DESPE_AGREGVALUE]/
					...
					if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					{
						var m_iCurPag = m_userContext.PersistentSupport.getPagingPos(CSGenioAagreg.GetInformation(), QMVC_POS_RECORD, sorts, despe___agregvalue___Conds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
						if (m_iCurPag != -1)
							pageNumber = ((m_iCurPag - 1) / numberListItems) + 1;
					}

					ListingMVC<CSGenioAagreg> listing = Models.ModelBase.Where<CSGenioAagreg>(m_userContext, distinct, despe___agregvalue___Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_DESPE___AGREGVALUE___", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn, fieldsWithTotalizers, tableConfig.SelectedRows);
		...
		private List<Despe_AgregValValue_RowViewModel> MapDespe_AgregValValue(ListingMVC<CSGenioAagreg> Qlisting)
		{
			List<Despe_AgregValValue_RowViewModel> Elements = [];
			int i = 0;
		...
		/// to a Despe_AgregValValue_RowViewModel object.
		/// </summary>
		/// <param name="row">The row.</param>
		private Despe_AgregValValue_RowViewModel MapDespe_AgregValValue(CSGenioAagreg row)
		{
			var model = new Despe_AgregValValue_RowViewModel(m_userContext, true, _fieldsToSerialize);
			if (row == null)
				return model;
