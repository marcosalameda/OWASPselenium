...
namespace GenioMVC.ViewModels.Expen
{
	public class Despe_ViewModel : FormViewModel<Models.Expen>, IPreparableForSerialization
	{
		...
		public Despe_ViewModel(UserContext userContext, bool nestedForm = false) : base(userContext, "FDESPE", nestedForm) { }

		public Despe_ViewModel(UserContext userContext, Models.Expen row, bool nestedForm = false) : base(userContext, "FDESPE", row, nestedForm) { }

		public Despe_ViewModel(UserContext userContext, string id, bool nestedForm = false, string[]? fieldsToLoad = null) : this(userContext, nestedForm)
		{
			this.Navigation.SetValue("expen", id);
			Model = Models.Expen.Find(id, userContext, "FDESPE", fieldsToQuery: fieldsToLoad);
			if (Model == null)
		...
		public override void Load(NameValueCollection qs, bool editable, bool ajaxRequest = false, bool lazyLoad = false)
		{
			...
			try
			{
				Model = Models.Expen.Find(Navigation.GetStrValue("expen"), m_userContext, "FDESPE");
			}
			finally
			{
				if (Model == null)
		...
		public override void LoadPartial(NameValueCollection qs, bool lazyLoad = false)
		{
			...
			if (Model == null)
			{
				...
				if (Model == null)
				{
					Model = new Models.Expen(m_userContext) { Identifier = "FDESPE" };
...
// USE /[MANUAL GQT VIEWMODEL_DESTROY DESPE]/
		...
		{
			Model = Models.Expen.Find(id, m_userContext, "FDESPE");
			if (Model == null)
				throw new ModelNotFoundException("Model not found");
...
// USE /[MANUAL GQT OVERRQ DESPE_PROJEPROJECTO]/
				...
				*/
				...
				ListingMVC<CSGenioAproje> listing = Models.ModelBase.Where<CSGenioAproje>(m_userContext, false, despe___projeprojectoConds, fields, offset, numberItems, sorts, "LED_DESPE___PROJEPROJECTO", true, false, firstVisibleColumn: firstVisibleColumn);
		...
		public ConcurrentDictionary<string, object> GetDependant_DespeTableProjeProjecto(string PKey)
		{
		...
		{
			var row = GetDependant_DespeTableProjeProjecto(this.ValCodproje);
			try
			{
...
// USE /[MANUAL GQT OVERRQ DESPE_YEARYEAR]/
				...
				*/
				...
				ListingMVC<CSGenioAyear> listing = Models.ModelBase.Where<CSGenioAyear>(m_userContext, false, despe___year_year____Conds, fields, offset, numberItems, sorts, "LED_DESPE___YEAR_YEAR____", true, false, firstVisibleColumn: firstVisibleColumn);
		...
		public ConcurrentDictionary<string, object> GetDependant_DespeTableYearYear(string PKey)
		{
			FieldRef[] refDependantFields = [CSGenioAyear.FldCodyear, CSGenioAyear.FldYear, CSGenioAyear.FldYearnum];
		...
		{
			var row = GetDependant_DespeTableYearYear(this.ValCodyear);
			try
			{
				this.funcYearValYearnum = () => (decimal?)row["year.yearnum"];
		...
		public void Load_Despe___agregvalue___(NameValueCollection qs, bool lazyLoad = false)
		{
			...
			if (despe___agregvalue___DoLoad)
			{
				...
				FieldRef[] fields = [CSGenioAagreg.FldCodaggre, CSGenioAagreg.FldValue, CSGenioAagreg.FldZzstate];
...
// USE /[MANUAL GQT OVERRQ DESPE_AGREGVALUE]/
				...
				*/
				...
				ListingMVC<CSGenioAagreg> listing = Models.ModelBase.Where<CSGenioAagreg>(m_userContext, false, despe___agregvalue___Conds, fields, offset, numberItems, sorts, "LED_DESPE___AGREGVALUE___", true, false, firstVisibleColumn: firstVisibleColumn);
		...
		public ConcurrentDictionary<string, object> GetDependant_DespeTableAgregValue(string PKey)
		{
			FieldRef[] refDependantFields = [CSGenioAagreg.FldCodaggre, CSGenioAagreg.FldValue];
		...
		{
			var row = GetDependant_DespeTableAgregValue(this.ValCodaggre);
			try
			{
			...
			{
				CSGenio.framework.Log.Error(string.Format("FillDependant_Error (TableAgregValue): {0}; {1}", ex.Message, ex.InnerException != null ? ex.InnerException.Message : ""));
			}
		}
