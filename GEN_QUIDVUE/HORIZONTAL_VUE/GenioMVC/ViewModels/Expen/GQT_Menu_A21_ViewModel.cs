...
{
	public class GQT_Menu_A21_ViewModel : MenuListViewModel<Models.Expen>
	{
		/// <summary>
		/// Gets or sets the object that represents the table and its elements.
		/// </summary>
		[JsonPropertyName("table")]
		public TablePartial<GQT_Menu_A21_RowViewModel> Menu { get; set; }
		...
		public override string Uuid => "cc12db7d-7a4f-45e3-bdcb-e2efc4c5e8a4";
		...
		{
// USE /[MANUAL GQT LIST_LIMITS A21]/
		...
		public override int GetCount(User user)
		{
			...
			var areaBase = CSGenio.business.Area.createArea("expen", user, "GQT");

			//gets eph conditions to be applied in listing
			CriteriaSet conditions = CSGenio.business.Listing.CalculateConditionsEphGeneric(areaBase, "MLA21");
		...
		public GQT_Menu_A21_ViewModel() : base(null!) { }

		/// <summary>
		/// Initializes a new instance of the <see cref="GQT_Menu_A21_ViewModel" /> class.
		/// </summary>
		/// <param name="userContext">The current user request context</param>
		public GQT_Menu_A21_ViewModel(UserContext userContext) : base(userContext)
		{
		...
		public GQT_Menu_A21_ViewModel(UserContext userContext, Models.ModelBase parentCtx) : this(userContext)
		{
			ParentCtx = parentCtx;
		}
		...
		public void LoadToExport(out ListingMVC<CSGenioAexpen> listing, out CriteriaSet conditions, out List<Exports.QColumn> columns, CSGenio.core.framework.table.TableConfiguration tableConfig, NameValueCollection requestValues, bool ajaxRequest = false)
		{
			listing = null;
			conditions = null;
		...
		public override CriteriaSet BuildCriteriaSet(CSGenio.core.framework.table.TableConfiguration tableConfig, NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			Menu ??= new TablePartial<GQT_Menu_A21_RowViewModel>();
			...
			{
				// EPH
				crs = Models.Expen.AddEPH<CSGenioAexpen>(ref u, crs, "MLA21");
			...
			if (tableReload)
			{
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					crs.Equals(Models.Expen.AddEPH<CSGenioAexpen>(ref u, null, "MLA21"));
			}
		...
		/// <param name="Qlisting">The rows.</param>
		/// <param name="conditions">The conditions.</param>
		public void Load(CSGenio.core.framework.table.TableConfiguration tableConfig, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAexpen> Qlisting, ref CriteriaSet conditions)
		{
			User u = m_userContext.User;
			Menu = new TablePartial<GQT_Menu_A21_RowViewModel>();

			CriteriaSet gqt_menu_a21Conds = CriteriaSet.And();
			...
			List<ColumnSort> sorts = GetRequestSorts(this.Menu, tableConfig, "expen", allSortOrders);
			...
			//Tooltip for EPHs affecting this viewmodel list
			{
				...
				List<Limit> area_EPH_limits = EPH_Limit_Filler(ref limit, model_limit_area, "MLA21");
			...
			conditions.SubSets.Add(gqt_menu_a21Conds);
			gqt_menu_a21Conds = BuildCriteriaSet(tableConfig, requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
			tableReload &= hasAllRequiredLimits;

// USE /[MANUAL GQT OVERRQ A21]/
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.BuildListingForExport<CSGenioAexpen>(m_userContext, false, ref gqt_menu_a21Conds, exportFieldRefs, (pageNumber - 1) * numberListItems, numberListItems, sorts, "MLA21", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL GQT OVERRQLSTEXP A21]/
...
// USE /[MANUAL GQT OVERRQLIST A21]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = m_userContext.PersistentSupport.getPagingPos(CSGenioAexpen.GetInformation(), QMVC_POS_RECORD, sorts, gqt_menu_a21Conds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
						pageNumber = ((m_iCurPag - 1) / numberListItems) + 1;
				}

				ListingMVC<CSGenioAexpen> listing = Models.ModelBase.Where<CSGenioAexpen>(m_userContext, distinct, gqt_menu_a21Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "MLA21", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn, fieldsWithTotalizers, tableConfig.SelectedRows);
				...
				Menu.Elements = MapGQT_Menu_A21(listing);

				Menu.Identifier = "MLA21";
				Menu.Slots = new Dictionary<string, List<object>>();

				// Last updated by [CJP] at [2015.02.03]
				// Adds the identifier to each element
				foreach (var element in Menu.Elements)
					element.Identifier = "MLA21";
		...
		private List<GQT_Menu_A21_RowViewModel> MapGQT_Menu_A21(ListingMVC<CSGenioAexpen> Qlisting)
		{
			List<GQT_Menu_A21_RowViewModel> Elements = [];
			int i = 0;
			...
			if (Qlisting.Rows != null)
			{
				foreach (var row in Qlisting.Rows)
				{
					if (Qlisting.NumRegs > 0 && i >= Qlisting.NumRegs) // Copiado da versão antiga do RowsToViewModels
						break;
					Elements.Add(MapGQT_Menu_A21(row));
					i++;
				}
			}
		...
		/// to a GQT_Menu_A21_RowViewModel object.
		/// </summary>
		/// <param name="row">The row.</param>
		private GQT_Menu_A21_RowViewModel MapGQT_Menu_A21(CSGenioAexpen row)
		{
			var model = new GQT_Menu_A21_RowViewModel(m_userContext, true, _fieldsToSerialize);
			if (row == null)
				return model;
...
// USE /[MANUAL GQT VIEWMODEL_CUSTOM GQT_MENU_A21]/
