...
namespace GenioMVC.ViewModels.Facty
{
	public class WMS_Menu_4231_ViewModel : ListViewModel
	{
		...
		public override int GetCount(User user)
		{
			...
			CriteriaSet wms_menu_4231Conds = CSGenio.business.Listing.CalculateConditionsEphGeneric(areaBase, "ML4231");
			wms_menu_4231Conds.Equal(CSGenioAfacty.FldZzstate, 0); //valid zzstate only
...
// USE /[MANUAL WMS OVERRQ 4231]/
			...
			SelectQuery qs = sp.getSelectQueryFromListingMVC(wms_menu_4231Conds, listing);
...
// USE /[MANUAL WMS OVERRQLIST 4231]/
					...
					ListingMVC<CSGenioAfacty> listing = Models.ModelBase.Where<CSGenioAfacty>(m_userContext, false, wms_menu_4231Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "ML4231", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn, fieldsWithTotalizers, tableConfig.SelectedRows);
		...
		{
			var Elements = new List<WMS_Menu_4231_RowViewModel>();
			int i = 0;
		...
		/// </summary>
		public void CalculateButtonPermissions(WMS_Menu_4231_RowViewModel model)
		{
			bool canView = true;
			bool canEdit = true;
			bool canDelete = true;
