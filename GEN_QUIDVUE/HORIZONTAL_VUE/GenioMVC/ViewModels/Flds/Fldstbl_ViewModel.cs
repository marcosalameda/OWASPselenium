...
{
	public class Fldstbl_ViewModel : FormViewModel<Models.Flds>, IPreparableForSerialization
	{
		[JsonIgnore]
		...
		#region Foreign keys
		/// <summary>
		/// Title: "Nome da companhia aérea" | Type: "CE"
		/// </summary>
		...
		/// Title: "Numeric" | Type: "N"
		/// </summary>
		public decimal? ValNrcntry { get; set; }
		/// <summary>
		...
		/// <summary>
		/// Title: "Nome da companhia aérea" | Type: "C"
		/// </summary>
		[ValidateSetAccess]
		...
		public Fldstbl_ViewModel() : base(null!) { }

		public Fldstbl_ViewModel(UserContext userContext, bool nestedForm = false) : base(userContext, "FFLDSTBL", nestedForm) { }

		public Fldstbl_ViewModel(UserContext userContext, Models.Flds row, bool nestedForm = false) : base(userContext, "FFLDSTBL", row, nestedForm) { }

		public Fldstbl_ViewModel(UserContext userContext, string id, bool nestedForm = false, string[]? fieldsToLoad = null) : this(userContext, nestedForm)
		{
			this.Navigation.SetValue("flds", id);
			Model = Models.Flds.Find(id, userContext, "FFLDSTBL", fieldsToQuery: fieldsToLoad);
			if (Model == null)
		...
		public static StatusMessage InsertConditions(UserContext userContext)
		{
			...
			Models.Flds model = new Models.Flds(userContext) { Identifier = "FFLDSTBL" };

			var navigation = m_userContext.CurrentNavigation;
			// The "LoadKeysFromHistory" must be after the "LoadEPH" because the PHE's in the tree mark Foreign Keys to null
			// (since they cannot assign multiple values to a single field) and thus the value that comes from Navigation is lost.
			// And this makes it more like the order of loading the model when opening the form.
			model.LoadEPH("FFLDSTBL");
			if (navigation != null)
		...
		public override void MapFromModel(Models.Flds m)
		{
			if (m == null)
			{
				CSGenio.framework.Log.Error("Map Model (Flds) to ViewModel (Fldstbl) - Model is a null reference");
				throw new ModelNotFoundException("Model not found");
			}
			...
			try
			{
				...
				ValNrcntry = ViewModelConversion.ToNumeric(m.ValNrcntry);
			...
			{
				CSGenio.framework.Log.Error("Map Model (Flds) to ViewModel (Fldstbl) - Error during mapping");
				throw;
			}
		}
		...
		public override void MapToModel(Models.Flds m)
		{
			if (m == null)
			{
				CSGenio.framework.Log.Error("Map ViewModel (Fldstbl) to Model (Flds) - Model is a null reference");
				throw new ModelNotFoundException("Model not found");
			}
			...
			try
			{
				...
				m.ValNrcntry = ViewModelConversion.ToNumeric(ValNrcntry);
			...
			{
				CSGenio.framework.Log.Error($"Map ViewModel (Fldstbl) to Model (Flds) - Error during mapping. All user values: {HasDisabledUserValuesSecurity}");
				throw;
			}
		}
			...
			try
			{
				...
				switch (fullFieldName)
				{
					...
					case "flds.upprtext":
						...
						break;
					case "flds.nrcntry":
						this.ValNrcntry = ViewModelConversion.ToNumeric(_value);
						break;
					case "flds.passfld":
					...
					case "flds.codflds":
						...
						break;
					default:
						Log.Error($"SetViewModelValue (Fldstbl) - Unexpected field identifier {fullFieldName}");
						break;
				}
			}
			catch (Exception ex)
			{
				throw new FrameworkException(Resources.Resources.PEDIMOS_DESCULPA__OC63848, "SetViewModelValue (Fldstbl)", "Unexpected error", ex);
			}
		}
		...
		{
			try { Model = Models.Flds.Find(id ?? Navigation.GetStrValue("flds"), m_userContext, "FFLDSTBL"); }
			finally { Model ??= new Models.Flds(m_userContext) { Identifier = "FFLDSTBL" }; }
		...
		public override void Load(NameValueCollection qs, bool editable, bool ajaxRequest = false, bool lazyLoad = false)
		{
			...
			try
			{
				Model = Models.Flds.Find(Navigation.GetStrValue("flds"), m_userContext, "FFLDSTBL");
			}
			finally
			{
				if (Model == null)
		...
		public override void LoadPartial(NameValueCollection qs, bool lazyLoad = false)
		{
			...
			if (Model == null)
			{
				...
				if (Model == null)
				{
					Model = new Models.Flds(m_userContext) { Identifier = "FFLDSTBL" };
			...
			Load_Fldstbl_aero_name____(qs, lazyLoad);
// USE /[MANUAL GQT VIEWMODEL_LOADPARTIAL FLDSTBL]/
		}

// USE /[MANUAL GQT VIEWMODEL_NEW FLDSTBL]/
...
// USE /[MANUAL GQT VIEWMODEL_SAVE FLDSTBL]/
...
// USE /[MANUAL GQT VIEWMODEL_APPLY FLDSTBL]/

// USE /[MANUAL GQT VIEWMODEL_DUPLICATE FLDSTBL]/

// USE /[MANUAL GQT VIEWMODEL_DESTROY FLDSTBL]/
		public override void Destroy(string id)
		{
			Model = Models.Flds.Find(id, m_userContext, "FFLDSTBL");
			if (Model == null)
				throw new ModelNotFoundException("Model not found");
		...
		/// <param name="qs"></param>
		/// <param name="lazyLoad">Lazy loading of dropdown items</param>
		public void Load_Fldstbl_aero_name____(NameValueCollection qs, bool lazyLoad = false)
		{
			bool fldstbl_aero_name____DoLoad = true;
			CriteriaSet fldstbl_aero_name____Conds = CriteriaSet.And();
			{
				object hValue = Navigation.GetValue("aero", true);
				if (hValue != null && !(hValue is Array) && !string.IsNullOrEmpty(Convert.ToString(hValue)))
				{
					fldstbl_aero_name____Conds.Equal(CSGenioAaero.FldCodaero, hValue);
			...
			if (lazyLoad)
			{
				...
				FillDependant_FldstblTableAeroName(lazyLoad);
				return;
			}

			if (fldstbl_aero_name____DoLoad)
			{
				...
				fldstbl_aero_name____Conds.SubSet(search_filters);
...
// USE /[MANUAL GQT OVERRQ FLDSTBL_AERONAME]/
				...
				*/
				if (Navigation.checkFormMode("aero", FormMode.New) || Navigation.checkFormMode("aero", FormMode.Duplicate))
					fldstbl_aero_name____Conds.SubSet(CriteriaSet.Or()
						.Equal(CSGenioAaero.FldZzstate, 0)
						.Equal(CSGenioAaero.FldCodaero, Navigation.GetStrValue("aero")));
				else
					fldstbl_aero_name____Conds.Criterias.Add(new Criteria(new ColumnReference(CSGenioAaero.FldZzstate), CriteriaOperator.Equal, 0));

				FieldRef firstVisibleColumn = new FieldRef("aero", "name");
				ListingMVC<CSGenioAaero> listing = Models.ModelBase.Where<CSGenioAaero>(m_userContext, false, fldstbl_aero_name____Conds, fields, offset, numberItems, sorts, "LED_FLDSTBL_AERO_NAME____", true, false, firstVisibleColumn: firstVisibleColumn);
				...
				TableAeroName.Elements = listing.RowsForViewModel<GenioMVC.Models.Aero>((r) => new GenioMVC.Models.Aero(m_userContext, r, true, _fieldsToSerialize_FLDSTBL_AERO_NAME____));
				...
				FillDependant_FldstblTableAeroName();
			}
		}
		...
		public ConcurrentDictionary<string, object> GetDependant_FldstblTableAeroName(string PKey)
		{
		...
		/// </summary>
		/// <param name="lazyLoad">Lazy loading of dropdown items</param>
		public void FillDependant_FldstblTableAeroName(bool lazyLoad = false)
		{
			var row = GetDependant_FldstblTableAeroName(this.ValCodaero);
			try
			{
		...
		private readonly string[] _fieldsToSerialize_FLDSTBL_AERO_NAME____ = ["Aero", "Aero.ValCodaero", "Aero.ValZzstate", "Aero.ValName"];
		...
		protected override object GetViewModelValue(string identifier, object modelValue)
		{
			return identifier switch
			{
				...
				"flds.nrcntry" => ViewModelConversion.ToNumeric(modelValue),
...
// USE /[MANUAL GQT VIEWMODEL_CUSTOM FLDSTBL]/
