...
{
	public class Artgl_ViewModel : FormViewModel<Models.Gitem>, IPreparableForSerialization
	{
		[JsonIgnore]
		...
		public Artgl_ViewModel() : base(null!) { }

		public Artgl_ViewModel(UserContext userContext, bool nestedForm = false) : base(userContext, "FARTGL", nestedForm) { }

		public Artgl_ViewModel(UserContext userContext, Models.Gitem row, bool nestedForm = false) : base(userContext, "FARTGL", row, nestedForm) { }

		public Artgl_ViewModel(UserContext userContext, string id, bool nestedForm = false, string[]? fieldsToLoad = null) : this(userContext, nestedForm)
		{
			this.Navigation.SetValue("gitem", id);
			Model = Models.Gitem.Find(id, userContext, "FARTGL", fieldsToQuery: fieldsToLoad);
			if (Model == null)
		...
		public static StatusMessage InsertConditions(UserContext userContext)
		{
			...
			Models.Gitem model = new Models.Gitem(userContext) { Identifier = "FARTGL" };

			var navigation = m_userContext.CurrentNavigation;
			// The "LoadKeysFromHistory" must be after the "LoadEPH" because the PHE's in the tree mark Foreign Keys to null
			// (since they cannot assign multiple values to a single field) and thus the value that comes from Navigation is lost.
			// And this makes it more like the order of loading the model when opening the form.
			model.LoadEPH("FARTGL");
			if (navigation != null)
		...
		public override void MapToModel(Models.Gitem m)
		{
			...
			{
				CSGenio.framework.Log.Error($"Map ViewModel (Artgl) to Model (Gitem) - Error during mapping. All user values: {HasDisabledUserValuesSecurity}");
				throw;
			}
		}
			...
			try
			{
				...
				switch (fullFieldName)
				{
					case "gitem.itemdes":
						...
						break;
					case "gitem.itemgcod":
						this.ValItemgcod = ViewModelConversion.ToString(_value);
						break;
					case "gitem.document":
					...
					case "gitem.codgitem":
						...
						break;
					default:
						Log.Error($"SetViewModelValue (Artgl) - Unexpected field identifier {fullFieldName}");
						break;
				}
			}
			catch (Exception ex)
			{
				throw new FrameworkException(Resources.Resources.PEDIMOS_DESCULPA__OC63848, "SetViewModelValue (Artgl)", "Unexpected error", ex);
			}
		}
		...
		{
			try { Model = Models.Gitem.Find(id ?? Navigation.GetStrValue("gitem"), m_userContext, "FARTGL"); }
			finally { Model ??= new Models.Gitem(m_userContext) { Identifier = "FARTGL" }; }
		...
		public override void Load(NameValueCollection qs, bool editable, bool ajaxRequest = false, bool lazyLoad = false)
		{
			...
			try
			{
				Model = Models.Gitem.Find(Navigation.GetStrValue("gitem"), m_userContext, "FARTGL");
			}
			finally
			{
				if (Model == null)
		...
		public override void LoadPartial(NameValueCollection qs, bool lazyLoad = false)
		{
			...
			if (Model == null)
			{
				...
				if (Model == null)
				{
					Model = new Models.Gitem(m_userContext) { Identifier = "FARTGL" };
...
// USE /[MANUAL GQT VIEWMODEL_DESTROY ARTGL]/
		...
		{
			Model = Models.Gitem.Find(id, m_userContext, "FARTGL");
			if (Model == null)
				throw new ModelNotFoundException("Model not found");
		...
		protected override object GetViewModelValue(string identifier, object modelValue)
		{
			return identifier switch
			{
				...
				"gitem.document" => ViewModelConversion.ToString(modelValue),
