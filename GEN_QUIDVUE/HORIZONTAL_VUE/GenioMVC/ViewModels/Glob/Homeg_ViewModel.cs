...
{
	public class Homeg_ViewModel : FormViewModel<Models.Glob>, IPreparableForSerialization
	{
		[JsonIgnore]
		...
		public Homeg_ViewModel() : base(null!) { }

		public Homeg_ViewModel(UserContext userContext, bool nestedForm = false) : base(userContext, "FHOMEG", nestedForm) { }

		public Homeg_ViewModel(UserContext userContext, Models.Glob row, bool nestedForm = false) : base(userContext, "FHOMEG", row, nestedForm) { }

		public Homeg_ViewModel(UserContext userContext, string id, bool nestedForm = false, string[]? fieldsToLoad = null) : this(userContext, nestedForm)
		{
			this.Navigation.SetValue("glob", id);
			Model = Models.Glob.Find(id, userContext, "FHOMEG", fieldsToQuery: fieldsToLoad);
			if (Model == null)
		...
		public static StatusMessage InsertConditions(UserContext userContext)
		{
			...
			Models.Glob model = new Models.Glob(userContext) { Identifier = "FHOMEG" };

			var navigation = m_userContext.CurrentNavigation;
			// The "LoadKeysFromHistory" must be after the "LoadEPH" because the PHE's in the tree mark Foreign Keys to null
			// (since they cannot assign multiple values to a single field) and thus the value that comes from Navigation is lost.
			// And this makes it more like the order of loading the model when opening the form.
			model.LoadEPH("FHOMEG");
			if (navigation != null)
		...
		public override void MapToModel(Models.Glob m)
		{
			...
			{
				CSGenio.framework.Log.Error($"Map ViewModel (Homeg) to Model (Glob) - Error during mapping. All user values: {HasDisabledUserValuesSecurity}");
				throw;
			}
		}
			...
			try
			{
				...
				switch (fullFieldName)
				{
					...
					case "glob.codglob":
						...
						break;
					default:
						Log.Error($"SetViewModelValue (Homeg) - Unexpected field identifier {fullFieldName}");
						break;
				}
			}
			catch (Exception ex)
			{
				throw new FrameworkException(Resources.Resources.PEDIMOS_DESCULPA__OC63848, "SetViewModelValue (Homeg)", "Unexpected error", ex);
			}
		}
		...
		{
			try { Model = Models.Glob.Find(id ?? Navigation.GetStrValue("glob"), m_userContext, "FHOMEG"); }
			finally { Model ??= new Models.Glob(m_userContext) { Identifier = "FHOMEG" }; }
		...
		public override void Load(NameValueCollection qs, bool editable, bool ajaxRequest = false, bool lazyLoad = false)
		{
			...
			try
			{
				Model = Models.Glob.Find(Navigation.GetStrValue("glob"), m_userContext, "FHOMEG");
			}
			finally
			{
				if (Model == null)
		...
		public override void LoadPartial(NameValueCollection qs, bool lazyLoad = false)
		{
			...
			if (Model == null)
			{
				...
				if (Model == null)
				{
					Model = new Models.Glob(m_userContext) { Identifier = "FHOMEG" };
...
// USE /[MANUAL GQT VIEWMODEL_DESTROY HOMEG]/
		...
		{
			Model = Models.Glob.Find(id, m_userContext, "FHOMEG");
			if (Model == null)
				throw new ModelNotFoundException("Model not found");
