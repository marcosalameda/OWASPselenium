...
// USE /[MANUAL PTN LIST_LIMITS 3M1]/
		...
		public void LoadToExport(out ListingMVC<CSGenioAgrpb> listing, out CriteriaSet conditions, out List<Exports.QColumn> columns, CSGenio.core.framework.table.TableConfiguration tableConfig, NameValueCollection requestValues, bool ajaxRequest = false)
		{
			listing = null;
			conditions = null;
		...
		/// <param name="Qlisting">The rows.</param>
		/// <param name="conditions">The conditions.</param>
		public void Load(CSGenio.core.framework.table.TableConfiguration tableConfig, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAgrpb> Qlisting, ref CriteriaSet conditions)
		{
			User u = m_userContext.User;
			Menu = new TablePartial<PTN_Menu_3M1_RowViewModel>();
			...
			List<ColumnSort> sorts = GetRequestSorts(this.Menu, tableConfig, "grpb", allSortOrders);
...
// USE /[MANUAL PTN OVERRQ 3M1]/
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.BuildListingForExport<CSGenioAgrpb>(m_userContext, false, ref ptn_menu_3m1Conds, exportFieldRefs, (pageNumber - 1) * numberListItems, numberListItems, sorts, "ML3M1", true, firstVisibleColumn: firstVisibleColumn);
...
// USE /[MANUAL PTN OVERRQLIST 3M1]/
		...
		private PTN_Menu_3M1_RowViewModel MapPTN_Menu_3M1(CSGenioAgrpb row, System.Collections.Hashtable tableBelowRows = null)
		{
			...
			if ((bool)tableBelowRows?.ContainsKey("tblb"))
			{
				...
				{
					model.TblbValBool = [.. rows.Where(r => !CSGenio.framework.Field.isEmptyValue(r.ValBool, FieldType.LOGIC.GetFormatting())).Select(r => ViewModelConversion.ToLogic(r.ValBool))];
					model.TblbValCurdec = [.. rows.Where(r => !CSGenio.framework.Field.isEmptyValue(r.ValCurdec, FieldType.CURRENCY.GetFormatting())).Select(r => ViewModelConversion.ToNumeric(r.ValCurdec))];
					model.TblbValCurint = [.. rows.Where(r => !CSGenio.framework.Field.isEmptyValue(r.ValCurint, FieldType.CURRENCY.GetFormatting())).Select(r => ViewModelConversion.ToNumeric(r.ValCurint))];
					model.TblbValDate = [.. rows.Where(r => !CSGenio.framework.Field.isEmptyValue(r.ValDate, FieldType.DATE.GetFormatting())).Select(r => ViewModelConversion.ToDateTime(r.ValDate))];
					model.TblbValDatetm = [.. rows.Where(r => !CSGenio.framework.Field.isEmptyValue(r.ValDatetm, FieldType.DATETIME.GetFormatting())).Select(r => ViewModelConversion.ToDateTime(r.ValDatetm))];
					model.TblbValDatets = [.. rows.Where(r => !CSGenio.framework.Field.isEmptyValue(r.ValDatets, FieldType.DATETIMESECONDS.GetFormatting())).Select(r => ViewModelConversion.ToDateTime(r.ValDatets))];
					model.TblbValEnumn = [.. rows.Where(r => !CSGenio.framework.Field.isEmptyValue(r.ValEnumn, FieldType.ARRAY_NUMERIC.GetFormatting())).Select(r => ViewModelConversion.ToNumeric(r.ValEnumn))];
					model.TblbValEnumt = [.. rows.Where(r => !CSGenio.framework.Field.isEmptyValue(r.ValEnumt, FieldType.ARRAY_TEXT.GetFormatting())).Select(r => ViewModelConversion.ToString(r.ValEnumt))];
					model.TblbValNumdec = [.. rows.Where(r => !CSGenio.framework.Field.isEmptyValue(r.ValNumdec, FieldType.NUMERIC.GetFormatting())).Select(r => ViewModelConversion.ToNumeric(r.ValNumdec))];
					model.TblbValNumint = [.. rows.Where(r => !CSGenio.framework.Field.isEmptyValue(r.ValNumint, FieldType.NUMERIC.GetFormatting())).Select(r => ViewModelConversion.ToNumeric(r.ValNumint))];
					model.TblbValText = [.. rows.Where(r => !CSGenio.framework.Field.isEmptyValue(r.ValText, FieldType.TEXT.GetFormatting())).Select(r => ViewModelConversion.ToString(r.ValText))];
					model.TblbValTextml = [.. rows.Where(r => !CSGenio.framework.Field.isEmptyValue(r.ValTextml, FieldType.MEMO.GetFormatting())).Select(r => ViewModelConversion.ToString(r.ValTextml))];
					model.TblbValTimehm = [.. rows.Where(r => !CSGenio.framework.Field.isEmptyValue(r.ValTimehm, FieldType.TIME_HOURS.GetFormatting())).Select(r => ViewModelConversion.ToString(r.ValTimehm))];
				}
			}
