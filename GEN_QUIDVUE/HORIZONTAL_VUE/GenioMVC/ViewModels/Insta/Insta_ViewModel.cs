...
{
	public class Insta_ViewModel : FormViewModel<Models.Insta>, IPreparableForSerialization
	{
		[JsonIgnore]
		...
		public override void MapToModel(Models.Insta m)
		{
			...
			{
				CSGenio.framework.Log.Error($"Map ViewModel (Insta) to Model (Insta) - Error during mapping. All user values: {HasDisabledUserValuesSecurity}");
				throw;
			}
		}
			...
			try
			{
				...
				switch (fullFieldName)
				{
					...
					case "insta.codinsta":
						...
						break;
					default:
						Log.Error($"SetViewModelValue (Insta) - Unexpected field identifier {fullFieldName}");
						break;
				}
			}
			catch (Exception ex)
			{
				throw new FrameworkException(Resources.Resources.PEDIMOS_DESCULPA__OC63848, "SetViewModelValue (Insta)", "Unexpected error", ex);
			}
		}
		...
		{
			try { Model = Models.Insta.Find(id ?? Navigation.GetStrValue("insta"), m_userContext, "FINSTA"); }
			finally { Model ??= new Models.Insta(m_userContext) { Identifier = "FINSTA" }; }
...
// USE /[MANUAL GQT VIEWMODEL_DESTROY INSTA]/
		...
		public void Load_Insta___tpequtipoequi(NameValueCollection qs, bool lazyLoad = false)
		{
			...
			CriteriaSet insta___tpequtipoequiConds = CriteriaSet.And();
			{
				...
				{
					insta___tpequtipoequiConds.Equal(CSGenioAtpequ.FldCodtpequ, hValue);
...
// USE /[MANUAL GQT OVERRQ INSTA_TPEQUTIPOEQUI]/
				...
				*/
				...
				TableTpequTipoequi.Elements = listing.RowsForViewModel((r) => new GenioMVC.Models.Tpequ(m_userContext, r, true, _fieldsToSerialize_INSTA___TPEQUTIPOEQUI));
...
// USE /[MANUAL GQT OVERRQ INSTA_TPEQUVALTIPOEQUI]/
		...
		public void Load_Insta___equipregistnr(NameValueCollection qs, bool lazyLoad = false)
		{
			...
			CriteriaSet insta___equipregistnrConds = CriteriaSet.And();
			{
				...
				if (hValue != null && !(hValue is Array) && !string.IsNullOrEmpty(Convert.ToString(hValue)))
				{
					insta___equipregistnrConds.Equal(CSGenioAequip.FldCodequip, hValue);
...
// USE /[MANUAL GQT OVERRQ INSTA_EQUIPREGISTNR]/
				...
				*/
				...
				TableEquipRegistnr.Elements = listing.RowsForViewModel((r) => new GenioMVC.Models.Equip(m_userContext, r, true, _fieldsToSerialize_INSTA___EQUIPREGISTNR));
