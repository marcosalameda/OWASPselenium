...
{
	public class Leafletd_ViewModel : FormViewModel<Models.Insta>, IPreparableForSerialization
	{
		[JsonIgnore]
		...
		public Leafletd_ViewModel() : base(null!) { }

		public Leafletd_ViewModel(UserContext userContext, bool nestedForm = false) : base(userContext, "FLEAFLETD", nestedForm) { }

		public Leafletd_ViewModel(UserContext userContext, Models.Insta row, bool nestedForm = false) : base(userContext, "FLEAFLETD", row, nestedForm) { }

		public Leafletd_ViewModel(UserContext userContext, string id, bool nestedForm = false, string[]? fieldsToLoad = null) : this(userContext, nestedForm)
		{
			this.Navigation.SetValue("insta", id);
			Model = Models.Insta.Find(id, userContext, "FLEAFLETD", fieldsToQuery: fieldsToLoad);
			if (Model == null)
		...
		public static StatusMessage InsertConditions(UserContext userContext)
		{
			...
			Models.Insta model = new Models.Insta(userContext) { Identifier = "FLEAFLETD" };

			var navigation = m_userContext.CurrentNavigation;
			// The "LoadKeysFromHistory" must be after the "LoadEPH" because the PHE's in the tree mark Foreign Keys to null
			// (since they cannot assign multiple values to a single field) and thus the value that comes from Navigation is lost.
			// And this makes it more like the order of loading the model when opening the form.
			model.LoadEPH("FLEAFLETD");
			if (navigation != null)
		...
		public override void MapFromModel(Models.Insta m)
		{
			if (m == null)
			{
				CSGenio.framework.Log.Error("Map Model (Insta) to ViewModel (Leafletd) - Model is a null reference");
				throw new ModelNotFoundException("Model not found");
			}
			...
			{
				CSGenio.framework.Log.Error("Map Model (Insta) to ViewModel (Leafletd) - Error during mapping");
				throw;
			}
		}
		...
		public override void MapToModel(Models.Insta m)
		{
			if (m == null)
			{
				CSGenio.framework.Log.Error("Map ViewModel (Leafletd) to Model (Insta) - Model is a null reference");
				throw new ModelNotFoundException("Model not found");
			}
			...
			{
				CSGenio.framework.Log.Error($"Map ViewModel (Leafletd) to Model (Insta) - Error during mapping. All user values: {HasDisabledUserValuesSecurity}");
				throw;
			}
		}
			...
			try
			{
				...
				switch (fullFieldName)
				{
					...
					case "insta.codinsta":
						...
						break;
					default:
						Log.Error($"SetViewModelValue (Leafletd) - Unexpected field identifier {fullFieldName}");
						break;
				}
			}
			catch (Exception ex)
			{
				throw new FrameworkException(Resources.Resources.PEDIMOS_DESCULPA__OC63848, "SetViewModelValue (Leafletd)", "Unexpected error", ex);
			}
		}
		...
		{
			try { Model = Models.Insta.Find(id ?? Navigation.GetStrValue("insta"), m_userContext, "FLEAFLETD"); }
			finally { Model ??= new Models.Insta(m_userContext) { Identifier = "FLEAFLETD" }; }
		...
		public override void Load(NameValueCollection qs, bool editable, bool ajaxRequest = false, bool lazyLoad = false)
		{
			...
			try
			{
				Model = Models.Insta.Find(Navigation.GetStrValue("insta"), m_userContext, "FLEAFLETD");
			}
			finally
			{
				if (Model == null)
		...
		public override void LoadPartial(NameValueCollection qs, bool lazyLoad = false)
		{
			...
			if (Model == null)
			{
				...
				if (Model == null)
				{
					Model = new Models.Insta(m_userContext) { Identifier = "FLEAFLETD" };
			...
			Load_Leafletdequipregistnr(qs, lazyLoad);
// USE /[MANUAL GQT VIEWMODEL_LOADPARTIAL LEAFLETD]/
		}

// USE /[MANUAL GQT VIEWMODEL_NEW LEAFLETD]/
		...
		public override CrudViewModelValidationResult Validate()
		{
			...
			validator.StringLength("TpequValTipoequi", Resources.Resources.TYPE_OF_EQUIPMENT64921, TpequValTipoequi, 50);
...
// USE /[MANUAL GQT VIEWMODEL_SAVE LEAFLETD]/
...
// USE /[MANUAL GQT VIEWMODEL_APPLY LEAFLETD]/

// USE /[MANUAL GQT VIEWMODEL_DUPLICATE LEAFLETD]/

// USE /[MANUAL GQT VIEWMODEL_DESTROY LEAFLETD]/
		public override void Destroy(string id)
		{
			Model = Models.Insta.Find(id, m_userContext, "FLEAFLETD");
			if (Model == null)
				throw new ModelNotFoundException("Model not found");
		...
		/// <param name="qs"></param>
		/// <param name="lazyLoad">Lazy loading of dropdown items</param>
		public void Load_Leafletdequipregistnr(NameValueCollection qs, bool lazyLoad = false)
		{
			bool leafletdequipregistnrDoLoad = true;
			CriteriaSet leafletdequipregistnrConds = CriteriaSet.And();
			{
				object hValue = Navigation.GetValue("equip", true);
				if (hValue != null && !(hValue is Array) && !string.IsNullOrEmpty(Convert.ToString(hValue)))
				{
					leafletdequipregistnrConds.Equal(CSGenioAequip.FldCodequip, hValue);
			...
			// Area limit
			leafletdequipregistnrDoLoad &= AddCriteriaAreaLimit(leafletdequipregistnrConds, CSGenio.business.CSGenioAtpequ.FldCodtpequ, "tpequ", this.ValCodtpequ, true);
			...
			if (lazyLoad)
			{
				...
				FillDependant_LeafletdTableEquipRegistnr(lazyLoad);
				return;
			}

			if (string.IsNullOrEmpty(this.ValCodtpequ))
				leafletdequipregistnrDoLoad = false;

			if (leafletdequipregistnrDoLoad)
			{
				...
				leafletdequipregistnrConds.SubSet(search_filters);
...
// USE /[MANUAL GQT OVERRQ LEAFLETD_EQUIPREGISTNR]/
				...
				*/
				if (Navigation.checkFormMode("equip", FormMode.New) || Navigation.checkFormMode("equip", FormMode.Duplicate))
					leafletdequipregistnrConds.SubSet(CriteriaSet.Or()
						.Equal(CSGenioAequip.FldZzstate, 0)
						.Equal(CSGenioAequip.FldCodequip, Navigation.GetStrValue("equip")));
				else
					leafletdequipregistnrConds.Criterias.Add(new Criteria(new ColumnReference(CSGenioAequip.FldZzstate), CriteriaOperator.Equal, 0));

				FieldRef firstVisibleColumn = new FieldRef("equip", "registnr");
				ListingMVC<CSGenioAequip> listing = Models.ModelBase.Where<CSGenioAequip>(m_userContext, false, leafletdequipregistnrConds, fields, offset, numberItems, sorts, "LED_LEAFLETDEQUIPREGISTNR", true, false, firstVisibleColumn: firstVisibleColumn);
				...
				TableEquipRegistnr.Elements = listing.RowsForViewModel<GenioMVC.Models.Equip>((r) => new GenioMVC.Models.Equip(m_userContext, r, true, _fieldsToSerialize_LEAFLETDEQUIPREGISTNR));
				...
				FillDependant_LeafletdTableEquipRegistnr();
			}
		}
		...
		public ConcurrentDictionary<string, object> GetDependant_LeafletdTableEquipRegistnr(string PKey)
		{
		...
		/// </summary>
		/// <param name="lazyLoad">Lazy loading of dropdown items</param>
		public void FillDependant_LeafletdTableEquipRegistnr(bool lazyLoad = false)
		{
			var row = GetDependant_LeafletdTableEquipRegistnr(this.ValCodequip);
			try
			{
		...
		private readonly string[] _fieldsToSerialize_LEAFLETDEQUIPREGISTNR = ["Equip", "Equip.ValCodequip", "Equip.ValZzstate", "Equip.ValRegistnr"];
...
// USE /[MANUAL GQT VIEWMODEL_CUSTOM LEAFLETD]/
