...
{
	public class Artiginv_ViewModel : FormViewModel<Models.Item>, IPreparableForSerialization
	{
		[JsonIgnore]
		...
		public Artiginv_ViewModel() : base(null!) { }

		public Artiginv_ViewModel(UserContext userContext, bool nestedForm = false) : base(userContext, "FARTIGINV", nestedForm) { }

		public Artiginv_ViewModel(UserContext userContext, Models.Item row, bool nestedForm = false) : base(userContext, "FARTIGINV", row, nestedForm) { }

		public Artiginv_ViewModel(UserContext userContext, string id, bool nestedForm = false, string[]? fieldsToLoad = null) : this(userContext, nestedForm)
		{
			this.Navigation.SetValue("item", id);
			Model = Models.Item.Find(id, userContext, "FARTIGINV", fieldsToQuery: fieldsToLoad);
			if (Model == null)
		...
		public static StatusMessage InsertConditions(UserContext userContext)
		{
			...
			Models.Item model = new Models.Item(userContext) { Identifier = "FARTIGINV" };

			var navigation = m_userContext.CurrentNavigation;
			// The "LoadKeysFromHistory" must be after the "LoadEPH" because the PHE's in the tree mark Foreign Keys to null
			// (since they cannot assign multiple values to a single field) and thus the value that comes from Navigation is lost.
			// And this makes it more like the order of loading the model when opening the form.
			model.LoadEPH("FARTIGINV");
			if (navigation != null)
		...
		public override void MapToModel(Models.Item m)
		{
			...
			{
				CSGenio.framework.Log.Error($"Map ViewModel (Artiginv) to Model (Item) - Error during mapping. All user values: {HasDisabledUserValuesSecurity}");
				throw;
			}
		}
			...
			try
			{
				...
				switch (fullFieldName)
				{
					...
					case "item.coditem":
						...
						break;
					default:
						Log.Error($"SetViewModelValue (Artiginv) - Unexpected field identifier {fullFieldName}");
						break;
				}
			}
			catch (Exception ex)
			{
				throw new FrameworkException(Resources.Resources.PEDIMOS_DESCULPA__OC63848, "SetViewModelValue (Artiginv)", "Unexpected error", ex);
			}
		}
		...
		{
			try { Model = Models.Item.Find(id ?? Navigation.GetStrValue("item"), m_userContext, "FARTIGINV"); }
			finally { Model ??= new Models.Item(m_userContext) { Identifier = "FARTIGINV" }; }
		...
		public override void Load(NameValueCollection qs, bool editable, bool ajaxRequest = false, bool lazyLoad = false)
		{
			...
			try
			{
				Model = Models.Item.Find(Navigation.GetStrValue("item"), m_userContext, "FARTIGINV");
			}
			finally
			{
				if (Model == null)
		...
		public override void LoadPartial(NameValueCollection qs, bool lazyLoad = false)
		{
			...
			if (Model == null)
			{
				...
				if (Model == null)
				{
					Model = new Models.Item(m_userContext) { Identifier = "FARTIGINV" };
...
// USE /[MANUAL GQT VIEWMODEL_DESTROY ARTIGINV]/
		...
		{
			Model = Models.Item.Find(id, m_userContext, "FARTIGINV");
			if (Model == null)
				throw new ModelNotFoundException("Model not found");
		...
		public void Load_Artiginvgitemitemdes_(NameValueCollection qs, bool lazyLoad = false)
		{
			...
			CriteriaSet artiginvgitemitemdes_Conds = CriteriaSet.And();
			{
				...
				{
					artiginvgitemitemdes_Conds.Equal(CSGenioAgitem.FldCodgitem, hValue);
...
// USE /[MANUAL GQT OVERRQ ARTIGINV_GITEMITEMDES]/
				...
				*/
				...
				ListingMVC<CSGenioAgitem> listing = Models.ModelBase.Where<CSGenioAgitem>(m_userContext, false, artiginvgitemitemdes_Conds, fields, offset, numberItems, sorts, "LED_ARTIGINVGITEMITEMDES_", true, false, firstVisibleColumn: firstVisibleColumn);
				...
				TableGitemItemdes.Elements = listing.RowsForViewModel<GenioMVC.Models.Gitem>((r) => new GenioMVC.Models.Gitem(m_userContext, r, true, _fieldsToSerialize_ARTIGINVGITEMITEMDES_));
		...
		public ConcurrentDictionary<string, object> GetDependant_ArtiginvTableGitemItemdes(string PKey)
		{
		...
		{
			var row = GetDependant_ArtiginvTableGitemItemdes(this.ValCodgitem);
			try
			{
		...
		public void Load_Artiginvwarehwarehdes(NameValueCollection qs, bool lazyLoad = false)
		{
			...
			CriteriaSet artiginvwarehwarehdesConds = CriteriaSet.And();
			{
				...
				if (hValue != null && !(hValue is Array) && !string.IsNullOrEmpty(Convert.ToString(hValue)))
				{
					artiginvwarehwarehdesConds.Equal(CSGenioAwareh.FldCodwareh, hValue);
...
// USE /[MANUAL GQT OVERRQ ARTIGINV_WAREHWAREHDES]/
				...
				*/
				...
				ListingMVC<CSGenioAwareh> listing = Models.ModelBase.Where<CSGenioAwareh>(m_userContext, false, artiginvwarehwarehdesConds, fields, offset, numberItems, sorts, "LED_ARTIGINVWAREHWAREHDES", true, false, firstVisibleColumn: firstVisibleColumn);
				...
				TableWarehWarehdes.Elements = listing.RowsForViewModel<GenioMVC.Models.Wareh>((r) => new GenioMVC.Models.Wareh(m_userContext, r, true, _fieldsToSerialize_ARTIGINVWAREHWAREHDES));
		...
		public ConcurrentDictionary<string, object> GetDependant_ArtiginvTableWarehWarehdes(string PKey)
		{
		...
		{
			var row = GetDependant_ArtiginvTableWarehWarehdes(this.ValCodwareh);
			try
			{
