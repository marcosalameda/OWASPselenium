...
{
	public class Item_ViewModel : FormViewModel<Models.Item>, IPreparableForSerialization
	{
		[JsonIgnore]
		...
		/// </summary>
		[Document("ValTechspec", true, false, false, DocumentViewTypeMode.Print)]
		...
		public Item_ViewModel() : base(null!) { }

		public Item_ViewModel(UserContext userContext, bool nestedForm = false) : base(userContext, "FITEM", nestedForm) { }

		public Item_ViewModel(UserContext userContext, Models.Item row, bool nestedForm = false) : base(userContext, "FITEM", row, nestedForm) { }

		public Item_ViewModel(UserContext userContext, string id, bool nestedForm = false, string[]? fieldsToLoad = null) : this(userContext, nestedForm)
		{
			this.Navigation.SetValue("item", id);
			Model = Models.Item.Find(id, userContext, "FITEM", fieldsToQuery: fieldsToLoad);
			if (Model == null)
		...
		public static StatusMessage InsertConditions(UserContext userContext)
		{
			...
			Models.Item model = new Models.Item(userContext) { Identifier = "FITEM" };
		...
		public override void MapFromModel(Models.Item m)
		{
			if (m == null)
			{
				CSGenio.framework.Log.Error("Map Model (Item) to ViewModel (Item) - Model is a null reference");
				throw new ModelNotFoundException("Model not found");
			}
			...
			{
				CSGenio.framework.Log.Error("Map Model (Item) to ViewModel (Item) - Error during mapping");
				throw;
			}
		}
		...
		public override void MapToModel(Models.Item m)
		{
			if (m == null)
			{
				CSGenio.framework.Log.Error("Map ViewModel (Item) to Model (Item) - Model is a null reference");
				throw new ModelNotFoundException("Model not found");
			}
			...
			{
				CSGenio.framework.Log.Error($"Map ViewModel (Item) to Model (Item) - Error during mapping. All user values: {HasDisabledUserValuesSecurity}");
				throw;
			}
		}
			...
			try
			{
				...
				switch (fullFieldName)
				{
					...
					case "item.coditem":
						...
						break;
					default:
						Log.Error($"SetViewModelValue (Item) - Unexpected field identifier {fullFieldName}");
						break;
				}
			}
			catch (Exception ex)
			{
				throw new FrameworkException(Resources.Resources.PEDIMOS_DESCULPA__OC63848, "SetViewModelValue (Item)", "Unexpected error", ex);
			}
		}
		...
		{
			try { Model = Models.Item.Find(id ?? Navigation.GetStrValue("item"), m_userContext, "FITEM"); }
			finally { Model ??= new Models.Item(m_userContext) { Identifier = "FITEM" }; }
		...
		public override void Load(NameValueCollection qs, bool editable, bool ajaxRequest = false, bool lazyLoad = false)
		{
			...
			try
			{
				Model = Models.Item.Find(Navigation.GetStrValue("item"), m_userContext, "FITEM");
			}
			finally
			{
				if (Model == null)
		...
		public override void LoadPartial(NameValueCollection qs, bool lazyLoad = false)
		{
			...
			if (Model == null)
			{
				...
				if (Model == null)
				{
					Model = new Models.Item(m_userContext) { Identifier = "FITEM" };
			...
			Load_Item____gitemitemdes_(qs, lazyLoad);
			Load_Item____warehwarehdes(qs, lazyLoad);
// USE /[MANUAL GQT VIEWMODEL_LOADPARTIAL ITEM]/
		}

// USE /[MANUAL GQT VIEWMODEL_NEW ITEM]/
...
// USE /[MANUAL GQT VIEWMODEL_SAVE ITEM]/
...
// USE /[MANUAL GQT VIEWMODEL_APPLY ITEM]/

// USE /[MANUAL GQT VIEWMODEL_DUPLICATE ITEM]/

// USE /[MANUAL GQT VIEWMODEL_DESTROY ITEM]/
		public override void Destroy(string id)
		{
			Model = Models.Item.Find(id, m_userContext, "FITEM");
			if (Model == null)
				throw new ModelNotFoundException("Model not found");
		...
		/// <param name="qs"></param>
		/// <param name="lazyLoad">Lazy loading of dropdown items</param>
		public void Load_Item____gitemitemdes_(NameValueCollection qs, bool lazyLoad = false)
		{
			bool item____gitemitemdes_DoLoad = true;
			CriteriaSet item____gitemitemdes_Conds = CriteriaSet.And();
			{
				object hValue = Navigation.GetValue("gitem", true);
				if (hValue != null && !(hValue is Array) && !string.IsNullOrEmpty(Convert.ToString(hValue)))
				{
					item____gitemitemdes_Conds.Equal(CSGenioAgitem.FldCodgitem, hValue);
			...
			if (lazyLoad)
			{
				...
				FillDependant_ItemTableGitemItemdes(lazyLoad);
				return;
			}
			...
			if (item____gitemitemdes_DoLoad)
			{
				...
				item____gitemitemdes_Conds.SubSet(search_filters);
...
// USE /[MANUAL GQT OVERRQ ITEM_GITEMITEMDES]/
				...
				*/
				if (Navigation.checkFormMode("gitem", FormMode.New) || Navigation.checkFormMode("gitem", FormMode.Duplicate))
					item____gitemitemdes_Conds.SubSet(CriteriaSet.Or()
						.Equal(CSGenioAgitem.FldZzstate, 0)
						.Equal(CSGenioAgitem.FldCodgitem, Navigation.GetStrValue("gitem")));
				else
					item____gitemitemdes_Conds.Criterias.Add(new Criteria(new ColumnReference(CSGenioAgitem.FldZzstate), CriteriaOperator.Equal, 0));

				FieldRef firstVisibleColumn = new FieldRef("gitem", "itemdes");
				ListingMVC<CSGenioAgitem> listing = Models.ModelBase.Where<CSGenioAgitem>(m_userContext, false, item____gitemitemdes_Conds, fields, offset, numberItems, sorts, "LED_ITEM____GITEMITEMDES_", true, false, firstVisibleColumn: firstVisibleColumn);
				...
				FillDependant_ItemTableGitemItemdes();
			}
		}
		...
		public ConcurrentDictionary<string, object> GetDependant_ItemTableGitemItemdes(string PKey)
		{
		...
		/// </summary>
		/// <param name="lazyLoad">Lazy loading of dropdown items</param>
		public void FillDependant_ItemTableGitemItemdes(bool lazyLoad = false)
		{
			var row = GetDependant_ItemTableGitemItemdes(this.ValCodgitem);
			try
			{
		...
		private readonly string[] _fieldsToSerialize_ITEM____GITEMITEMDES_ = ["Gitem", "Gitem.ValCodgitem", "Gitem.ValZzstate", "Gitem.ValItemdes"];
		...
		/// <param name="qs"></param>
		/// <param name="lazyLoad">Lazy loading of dropdown items</param>
		public void Load_Item____warehwarehdes(NameValueCollection qs, bool lazyLoad = false)
		{
			bool item____warehwarehdesDoLoad = true;
			CriteriaSet item____warehwarehdesConds = CriteriaSet.And();
			{
				object hValue = Navigation.GetValue("wareh", true);
				if (hValue != null && !(hValue is Array) && !string.IsNullOrEmpty(Convert.ToString(hValue)))
				{
					item____warehwarehdesConds.Equal(CSGenioAwareh.FldCodwareh, hValue);
			...
			if (lazyLoad)
			{
				...
				FillDependant_ItemTableWarehWarehdes(lazyLoad);
				return;
			}
			...
			if (item____warehwarehdesDoLoad)
			{
				...
				item____warehwarehdesConds.SubSet(search_filters);
...
// USE /[MANUAL GQT OVERRQ ITEM_WAREHWAREHDES]/
				...
				*/
				if (Navigation.checkFormMode("wareh", FormMode.New) || Navigation.checkFormMode("wareh", FormMode.Duplicate))
					item____warehwarehdesConds.SubSet(CriteriaSet.Or()
						.Equal(CSGenioAwareh.FldZzstate, 0)
						.Equal(CSGenioAwareh.FldCodwareh, Navigation.GetStrValue("wareh")));
				else
					item____warehwarehdesConds.Criterias.Add(new Criteria(new ColumnReference(CSGenioAwareh.FldZzstate), CriteriaOperator.Equal, 0));

				FieldRef firstVisibleColumn = new FieldRef("wareh", "warehdes");
				ListingMVC<CSGenioAwareh> listing = Models.ModelBase.Where<CSGenioAwareh>(m_userContext, false, item____warehwarehdesConds, fields, offset, numberItems, sorts, "LED_ITEM____WAREHWAREHDES", true, false, firstVisibleColumn: firstVisibleColumn);
				...
				FillDependant_ItemTableWarehWarehdes();
			}
		}
		...
		public ConcurrentDictionary<string, object> GetDependant_ItemTableWarehWarehdes(string PKey)
		{
		...
		/// </summary>
		/// <param name="lazyLoad">Lazy loading of dropdown items</param>
		public void FillDependant_ItemTableWarehWarehdes(bool lazyLoad = false)
		{
			var row = GetDependant_ItemTableWarehWarehdes(this.ValCodwareh);
			try
			{
		...
		private readonly string[] _fieldsToSerialize_ITEM____WAREHWAREHDES = ["Wareh", "Wareh.ValCodwareh", "Wareh.ValZzstate", "Wareh.ValWarehdes"];
...
// USE /[MANUAL GQT VIEWMODEL_CUSTOM ITEM]/
