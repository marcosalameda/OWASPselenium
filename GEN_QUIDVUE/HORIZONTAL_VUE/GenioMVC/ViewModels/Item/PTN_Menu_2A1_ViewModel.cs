...
{
	public class PTN_Menu_2A1_ViewModel : MenuListViewModel<Models.Item>
	{
		/// <summary>
		/// Gets or sets the object that represents the table and its elements.
		/// </summary>
		[JsonPropertyName("table")]
		public TablePartial<PTN_Menu_2A1_RowViewModel> Menu { get; set; }
		...
		public override string Uuid => "c436724d-8f81-45af-99b2-732d131332be";
		...
		{
// USE /[MANUAL PTN LIST_LIMITS 2A1]/
		...
		public override int GetCount(User user)
		{
			...
			CriteriaSet conditions = CSGenio.business.Listing.CalculateConditionsEphGeneric(areaBase, "ML2A1");
			...
			FieldRef[] fields = new FieldRef[] { CSGenioAitem.FldCoditem, CSGenioAitem.FldZzstate, CSGenioAitem.FldItemtype, CSGenioAitem.FldImage, CSGenioAitem.FldTechspec, CSGenioAitem.FldValid, CSGenioAitem.FldExistenc, CSGenioAitem.FldCategory, CSGenioAitem.FldCodwareh, CSGenioAwareh.FldCodwareh, CSGenioAwareh.FldWarehdes, CSGenioAitem.FldDisponib, CSGenioAitem.FldItemcod, CSGenioAitem.FldDate, CSGenioAitem.FldExits, CSGenioAitem.FldItemdes, CSGenioAitem.FldEntries, CSGenioAitem.FldCodgitem, CSGenioAgitem.FldCodgitem, CSGenioAgitem.FldItemdes };
		...
		public PTN_Menu_2A1_ViewModel() : base(null!) { }

		/// <summary>
		/// Initializes a new instance of the <see cref="PTN_Menu_2A1_ViewModel" /> class.
		/// </summary>
		/// <param name="userContext">The current user request context</param>
		public PTN_Menu_2A1_ViewModel(UserContext userContext) : base(userContext)
		{
		...
		public PTN_Menu_2A1_ViewModel(UserContext userContext, Models.ModelBase parentCtx) : this(userContext)
		{
			ParentCtx = parentCtx;
		}
		...
		public override List<Exports.QColumn> GetColumnsToExport()
		{
			...
			[
				...
				new Exports.QColumn(CSGenioAitem.FldTechspec, FieldType.DOCUMENT, Resources.Resources.SPECIFICATIONS59226, 30, 0, true),
		...
		public override CriteriaSet BuildCriteriaSet(CSGenio.core.framework.table.TableConfiguration tableConfig, NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			Menu ??= new TablePartial<PTN_Menu_2A1_RowViewModel>();
			...
			{
				// EPH
				crs = Models.Item.AddEPH<CSGenioAitem>(ref u, crs, "ML2A1");
			...
			if (tableReload)
			{
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					crs.Equals(Models.Item.AddEPH<CSGenioAitem>(ref u, null, "ML2A1"));
			}
		...
		public void Load(CSGenio.core.framework.table.TableConfiguration tableConfig, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAitem> Qlisting, ref CriteriaSet conditions)
		{
			...
			CriteriaSet ptn_menu_2a1Conds = CriteriaSet.And();
			...
			FieldRef[] fields = new FieldRef[] { CSGenioAitem.FldCoditem, CSGenioAitem.FldZzstate, CSGenioAitem.FldItemtype, CSGenioAitem.FldImage, CSGenioAitem.FldTechspec, CSGenioAitem.FldTechspecfk, CSGenioAitem.FldValid, CSGenioAitem.FldExistenc, CSGenioAitem.FldCategory, CSGenioAitem.FldCodwareh, CSGenioAwareh.FldCodwareh, CSGenioAwareh.FldWarehdes, CSGenioAitem.FldDisponib, CSGenioAitem.FldItemcod, CSGenioAitem.FldDate, CSGenioAitem.FldExits, CSGenioAitem.FldItemdes, CSGenioAitem.FldEntries, CSGenioAitem.FldCodgitem, CSGenioAgitem.FldCodgitem, CSGenioAgitem.FldItemdes };
			...
			//Tooltip for EPHs affecting this viewmodel list
			{
				...
				List<Limit> area_EPH_limits = EPH_Limit_Filler(ref limit, model_limit_area, "ML2A1");
			...
			conditions.SubSets.Add(ptn_menu_2a1Conds);
			ptn_menu_2a1Conds = BuildCriteriaSet(tableConfig, requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
			tableReload &= hasAllRequiredLimits;

// USE /[MANUAL PTN OVERRQ 2A1]/
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.BuildListingForExport<CSGenioAitem>(m_userContext, false, ref ptn_menu_2a1Conds, exportFieldRefs, (pageNumber - 1) * numberListItems, numberListItems, sorts, "ML2A1", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL PTN OVERRQLSTEXP 2A1]/
...
// USE /[MANUAL PTN OVERRQLIST 2A1]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = m_userContext.PersistentSupport.getPagingPos(CSGenioAitem.GetInformation(), QMVC_POS_RECORD, sorts, ptn_menu_2a1Conds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
						pageNumber = ((m_iCurPag - 1) / numberListItems) + 1;
				}

				ListingMVC<CSGenioAitem> listing = Models.ModelBase.Where<CSGenioAitem>(m_userContext, distinct, ptn_menu_2a1Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "ML2A1", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn, fieldsWithTotalizers, tableConfig.SelectedRows);
				...
				Menu.Elements = MapPTN_Menu_2A1(listing);

				Menu.Identifier = "ML2A1";
				Menu.Slots = new Dictionary<string, List<object>>();

				// Last updated by [CJP] at [2015.02.03]
				// Adds the identifier to each element
				foreach (var element in Menu.Elements)
					element.Identifier = "ML2A1";
		...
		private List<PTN_Menu_2A1_RowViewModel> MapPTN_Menu_2A1(ListingMVC<CSGenioAitem> Qlisting)
		{
			List<PTN_Menu_2A1_RowViewModel> Elements = [];
			int i = 0;
			...
			if (Qlisting.Rows != null)
			{
				foreach (var row in Qlisting.Rows)
				{
					if (Qlisting.NumRegs > 0 && i >= Qlisting.NumRegs) // Copiado da versão antiga do RowsToViewModels
						break;
					Elements.Add(MapPTN_Menu_2A1(row));
					i++;
				}
			}
		...
		/// to a PTN_Menu_2A1_RowViewModel object.
		/// </summary>
		/// <param name="row">The row.</param>
		private PTN_Menu_2A1_RowViewModel MapPTN_Menu_2A1(CSGenioAitem row)
		{
			var model = new PTN_Menu_2A1_RowViewModel(m_userContext, true, _fieldsToSerialize);
			if (row == null)
				return model;
		...
		private void SetDocumentFields(ListingMVC<CSGenioAitem> listing)
		{
			...
			{
				{
					if (!string.IsNullOrEmpty((string)row.returnValueField("item.techspecfk")))
					{
						ResourceQuery resource = new("Item", "ValTechspec", "ValTechspecfk", row.ValCoditem);
						string ticket = QResources.CreateTicketEncryptedBase64(m_userContext.User.Name, m_userContext.User.Location, resource);

						row.insertNameValueField("item.techspec", Newtonsoft.Json.JsonConvert.SerializeObject(new
						{
							fileName = row.returnValueField("item.techspec"),
							ticket
						}));
					}
					else
						row.removeFieldValue("item.techspec");
				}
			}
		}
...
// USE /[MANUAL GQT VIEWMODEL_CUSTOM PTN_MENU_2A1]/

		#endregion

		private static readonly string[] _fieldsToSerialize =
		[
			"Item", "Item.ValCoditem", "Item.ValZzstate", "Item.ValItemtype", "Item.ValImage", "Item.ValTechspec", "Item.ValValid", "Item.ValExistenc", "Item.ValCategory", "Wareh", "Wareh.ValWarehdes", "Item.ValDisponib", "Item.ValItemcod", "Item.ValDate", "Item.ValExits", "Item.ValItemdes", "Item.ValEntries", "Gitem", "Gitem.ValItemdes", "Item.ValCodgitem", "Item.ValCodwareh"
		];

		private static readonly List<TableSearchColumn> _searchableColumns =
		[
			new TableSearchColumn("ValItemtype", CSGenioAitem.FldItemtype, typeof(string), array : "TipoArti"),
			new TableSearchColumn("ValTechspec", CSGenioAitem.FldTechspec, typeof(string)),
