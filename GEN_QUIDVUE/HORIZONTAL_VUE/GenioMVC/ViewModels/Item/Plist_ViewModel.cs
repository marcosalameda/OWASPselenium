...
{
	public class Plist_ViewModel : FormViewModel<Models.Item>, IPreparableForSerialization
	{
		[JsonIgnore]
		...
		/// <summary>
		/// Title: "Property List" | Type: "PSEUD"
		/// </summary>
		[ValidateSetAccess]
		public GenioMVC.ViewModels.Itemp.PropertyList_Plist_ValPlist_ViewModel ValPlist { get; set; }
		...
		public Plist_ViewModel() : base(null!) { }

		public Plist_ViewModel(UserContext userContext, bool nestedForm = false) : base(userContext, "FPLIST", nestedForm) { }

		public Plist_ViewModel(UserContext userContext, Models.Item row, bool nestedForm = false) : base(userContext, "FPLIST", row, nestedForm) { }

		public Plist_ViewModel(UserContext userContext, string id, bool nestedForm = false, string[]? fieldsToLoad = null) : this(userContext, nestedForm)
		{
			this.Navigation.SetValue("item", id);
			Model = Models.Item.Find(id, userContext, "FPLIST", fieldsToQuery: fieldsToLoad);
			if (Model == null)
		...
		public static StatusMessage InsertConditions(UserContext userContext)
		{
			...
			Models.Item model = new Models.Item(userContext) { Identifier = "FPLIST" };

			var navigation = m_userContext.CurrentNavigation;
			// The "LoadKeysFromHistory" must be after the "LoadEPH" because the PHE's in the tree mark Foreign Keys to null
			// (since they cannot assign multiple values to a single field) and thus the value that comes from Navigation is lost.
			// And this makes it more like the order of loading the model when opening the form.
			model.LoadEPH("FPLIST");
			if (navigation != null)
		...
		public override void MapFromModel(Models.Item m)
		{
			if (m == null)
			{
				CSGenio.framework.Log.Error("Map Model (Item) to ViewModel (Plist) - Model is a null reference");
				throw new ModelNotFoundException("Model not found");
			}
			...
			{
				CSGenio.framework.Log.Error("Map Model (Item) to ViewModel (Plist) - Error during mapping");
				throw;
			}
		}
		...
		public override void MapToModel(Models.Item m)
		{
			if (m == null)
			{
				CSGenio.framework.Log.Error("Map ViewModel (Plist) to Model (Item) - Model is a null reference");
				throw new ModelNotFoundException("Model not found");
			}
			...
			{
				CSGenio.framework.Log.Error($"Map ViewModel (Plist) to Model (Item) - Error during mapping. All user values: {HasDisabledUserValuesSecurity}");
				throw;
			}
		}
			...
			try
			{
				...
				switch (fullFieldName)
				{
					...
					case "item.coditem":
						...
						break;
					default:
						Log.Error($"SetViewModelValue (Plist) - Unexpected field identifier {fullFieldName}");
						break;
				}
			}
			catch (Exception ex)
			{
				throw new FrameworkException(Resources.Resources.PEDIMOS_DESCULPA__OC63848, "SetViewModelValue (Plist)", "Unexpected error", ex);
			}
		}
		...
		{
			try { Model = Models.Item.Find(id ?? Navigation.GetStrValue("item"), m_userContext, "FPLIST"); }
			finally { Model ??= new Models.Item(m_userContext) { Identifier = "FPLIST" }; }
		...
		public override void Load(NameValueCollection qs, bool editable, bool ajaxRequest = false, bool lazyLoad = false)
		{
			...
			try
			{
				Model = Models.Item.Find(Navigation.GetStrValue("item"), m_userContext, "FPLIST");
			}
			finally
			{
				if (Model == null)
		...
		public override void LoadPartial(NameValueCollection qs, bool lazyLoad = false)
		{
			...
			if (Model == null)
			{
				...
				if (Model == null)
				{
					Model = new Models.Item(m_userContext) { Identifier = "FPLIST" };
			...
			Load_Plist___warehwarehdes(qs, lazyLoad);
// USE /[MANUAL GQT VIEWMODEL_LOADPARTIAL PLIST]/
		}

// USE /[MANUAL GQT VIEWMODEL_NEW PLIST]/
		...
		public override void Init(UserContext userContext)
		{
			...
			ValPlist?.Init(userContext);
		}
// USE /[MANUAL GQT VIEWMODEL_SAVE PLIST]/
		public override void Save()
		{
			...
			catch (FieldValidationException fvExc)
			{
				...
				foreach (var message in fvExc.StatusMessage.GetErrorList())
					sMsg.MergeStatusMessage(new StatusMessage(message.Status, message.Message, string.Format("ValPlist.{0}", message.Origin)));
...
// USE /[MANUAL GQT VIEWMODEL_APPLY PLIST]/

// USE /[MANUAL GQT VIEWMODEL_DUPLICATE PLIST]/

// USE /[MANUAL GQT VIEWMODEL_DESTROY PLIST]/
		public override void Destroy(string id)
		{
			Model = Models.Item.Find(id, m_userContext, "FPLIST");
			if (Model == null)
				throw new ModelNotFoundException("Model not found");
		...
		/// <param name="qs"></param>
		/// <param name="lazyLoad">Lazy loading of dropdown items</param>
		public void Load_Plist___warehwarehdes(NameValueCollection qs, bool lazyLoad = false)
		{
			bool plist___warehwarehdesDoLoad = true;
			CriteriaSet plist___warehwarehdesConds = CriteriaSet.And();
			{
				object hValue = Navigation.GetValue("wareh", true);
				if (hValue != null && !(hValue is Array) && !string.IsNullOrEmpty(Convert.ToString(hValue)))
				{
					plist___warehwarehdesConds.Equal(CSGenioAwareh.FldCodwareh, hValue);
			...
			if (lazyLoad)
			{
				...
				FillDependant_PlistTableWarehWarehdes(lazyLoad);
				return;
			}

			if (plist___warehwarehdesDoLoad)
			{
				...
				plist___warehwarehdesConds.SubSet(search_filters);
...
// USE /[MANUAL GQT OVERRQ PLIST_WAREHWAREHDES]/
				...
				*/
				if (Navigation.checkFormMode("wareh", FormMode.New) || Navigation.checkFormMode("wareh", FormMode.Duplicate))
					plist___warehwarehdesConds.SubSet(CriteriaSet.Or()
						.Equal(CSGenioAwareh.FldZzstate, 0)
						.Equal(CSGenioAwareh.FldCodwareh, Navigation.GetStrValue("wareh")));
				else
					plist___warehwarehdesConds.Criterias.Add(new Criteria(new ColumnReference(CSGenioAwareh.FldZzstate), CriteriaOperator.Equal, 0));

				FieldRef firstVisibleColumn = new FieldRef("wareh", "warehdes");
				ListingMVC<CSGenioAwareh> listing = Models.ModelBase.Where<CSGenioAwareh>(m_userContext, false, plist___warehwarehdesConds, fields, offset, numberItems, sorts, "LED_PLIST___WAREHWAREHDES", true, false, firstVisibleColumn: firstVisibleColumn);
				...
				TableWarehWarehdes.Elements = listing.RowsForViewModel<GenioMVC.Models.Wareh>((r) => new GenioMVC.Models.Wareh(m_userContext, r, true, _fieldsToSerialize_PLIST___WAREHWAREHDES));
				...
				FillDependant_PlistTableWarehWarehdes();
			}
		}
		...
		public ConcurrentDictionary<string, object> GetDependant_PlistTableWarehWarehdes(string PKey)
		{
		...
		/// </summary>
		/// <param name="lazyLoad">Lazy loading of dropdown items</param>
		public void FillDependant_PlistTableWarehWarehdes(bool lazyLoad = false)
		{
			var row = GetDependant_PlistTableWarehWarehdes(this.ValCodwareh);
			try
			{
		...
		private readonly string[] _fieldsToSerialize_PLIST___WAREHWAREHDES = ["Wareh", "Wareh.ValCodwareh", "Wareh.ValZzstate", "Wareh.ValWarehdes"];
...
// USE /[MANUAL GQT VIEWMODEL_CUSTOM PLIST]/
