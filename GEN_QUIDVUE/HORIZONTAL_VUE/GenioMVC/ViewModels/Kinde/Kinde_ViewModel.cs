...
{
	public class Kinde_ViewModel : FormViewModel<Models.Kinde>, IPreparableForSerialization
	{
		[JsonIgnore]
		...
		public override void MapToModel(Models.Kinde m)
		{
			...
			{
				CSGenio.framework.Log.Error($"Map ViewModel (Kinde) to Model (Kinde) - Error during mapping. All user values: {HasDisabledUserValuesSecurity}");
				throw;
			}
		}
			...
			try
			{
				...
				switch (fullFieldName)
				{
					...
					case "kinde.codkinde":
						...
						break;
					default:
						Log.Error($"SetViewModelValue (Kinde) - Unexpected field identifier {fullFieldName}");
						break;
				}
			}
			catch (Exception ex)
			{
				throw new FrameworkException(Resources.Resources.PEDIMOS_DESCULPA__OC63848, "SetViewModelValue (Kinde)", "Unexpected error", ex);
			}
		}
		...
		{
			try { Model = Models.Kinde.Find(id ?? Navigation.GetStrValue("kinde"), m_userContext, "FKINDE"); }
			finally { Model ??= new Models.Kinde(m_userContext) { Identifier = "FKINDE" }; }
