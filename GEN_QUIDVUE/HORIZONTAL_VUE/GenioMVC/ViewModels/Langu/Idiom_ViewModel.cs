...
{
	public class Idiom_ViewModel : FormViewModel<Models.Langu>, IPreparableForSerialization
	{
		[JsonIgnore]
		...
		public override void MapToModel(Models.Langu m)
		{
			...
			{
				CSGenio.framework.Log.Error($"Map ViewModel (Idiom) to Model (Langu) - Error during mapping. All user values: {HasDisabledUserValuesSecurity}");
				throw;
			}
		}
		...
		public override void SetViewModelValue(string fullFieldName, object value)
		{
			try
			{
				...
				{
					case "langu.langua":
						this.ValLangua = ViewModelConversion.ToString(_value);
						break;
					case "langu.acron":
						this.ValAcron = ViewModelConversion.ToString(_value);
						break;
					case "langu.codlang":
						this.ValCodlang = ViewModelConversion.ToString(_value);
						break;
					default:
						Log.Error($"SetViewModelValue (Idiom) - Unexpected field identifier {fullFieldName}");
						break;
				}
			}
			catch (Exception ex)
			{
				throw new FrameworkException(Resources.Resources.PEDIMOS_DESCULPA__OC63848, "SetViewModelValue (Idiom)", "Unexpected error", ex);
			}
		}
		...
		{
			try { Model = Models.Langu.Find(id ?? Navigation.GetStrValue("langu"), m_userContext, "FIDIOM"); }
			finally { Model ??= new Models.Langu(m_userContext) { Identifier = "FIDIOM" }; }
