...
{
	public class Locat_ViewModel : FormViewModel<Models.Locat>, IPreparableForSerialization
	{
		[JsonIgnore]
		...
		public override void MapToModel(Models.Locat m)
		{
			...
			{
				CSGenio.framework.Log.Error($"Map ViewModel (Locat) to Model (Locat) - Error during mapping. All user values: {HasDisabledUserValuesSecurity}");
				throw;
			}
		}
			...
			try
			{
				...
				switch (fullFieldName)
				{
					...
					case "locat.codfacil":
						...
						break;
					case "locat.gln":
						this.ValGln = ViewModelConversion.ToString(_value);
						break;
					case "locat.codlocat":
						this.ValCodlocat = ViewModelConversion.ToString(_value);
						break;
					default:
						Log.Error($"SetViewModelValue (Locat) - Unexpected field identifier {fullFieldName}");
						break;
				}
			}
			catch (Exception ex)
			{
				throw new FrameworkException(Resources.Resources.PEDIMOS_DESCULPA__OC63848, "SetViewModelValue (Locat)", "Unexpected error", ex);
			}
		}
		...
		{
			try { Model = Models.Locat.Find(id ?? Navigation.GetStrValue("locat"), m_userContext, "FLOCAT"); }
			finally { Model ??= new Models.Locat(m_userContext) { Identifier = "FLOCAT" }; }
...
// USE /[MANUAL GQT VIEWMODEL_DESTROY LOCAT]/
		...
		public void Load_Locat___entitname____(NameValueCollection qs, bool lazyLoad = false)
		{
			...
			CriteriaSet locat___entitname____Conds = CriteriaSet.And();
			{
				...
				{
					locat___entitname____Conds.Equal(CSGenioAentit.FldCodentit, hValue);
...
// USE /[MANUAL GQT OVERRQ LOCAT_ENTITNAME]/
				...
				*/
				...
				TableEntitName.Elements = listing.RowsForViewModel((r) => new GenioMVC.Models.Entit(m_userContext, r, true, _fieldsToSerialize_LOCAT___ENTITNAME____));
		...
		public void Load_Locat___facilname____(NameValueCollection qs, bool lazyLoad = false)
		{
			...
			CriteriaSet locat___facilname____Conds = CriteriaSet.And();
			{
				...
				if (hValue != null && !(hValue is Array) && !string.IsNullOrEmpty(Convert.ToString(hValue)))
				{
					locat___facilname____Conds.Equal(CSGenioAfacil.FldCodfacil, hValue);
					this.ValCodfacil = DBConversion.ToString(hValue);
				}
			}
			// Limits Generation
...
// USE /[MANUAL GQT OVERRQ LOCAT_FACILNAME]/
				...
				*/
				...
				TableFacilName.Elements = listing.RowsForViewModel((r) => new GenioMVC.Models.Facil(m_userContext, r, true, _fieldsToSerialize_LOCAT___FACILNAME____));
