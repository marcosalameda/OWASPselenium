...
{
	public class Movim_ViewModel : FormViewModel<Models.Movim>, IPreparableForSerialization
	{
		[JsonIgnore]
		...
		public Movim_ViewModel() : base(null!) { }

		public Movim_ViewModel(UserContext userContext, bool nestedForm = false) : base(userContext, "FMOVIM", nestedForm) { }

		public Movim_ViewModel(UserContext userContext, Models.Movim row, bool nestedForm = false) : base(userContext, "FMOVIM", row, nestedForm) { }

		public Movim_ViewModel(UserContext userContext, string id, bool nestedForm = false, string[]? fieldsToLoad = null) : this(userContext, nestedForm)
		{
			this.Navigation.SetValue("movim", id);
			Model = Models.Movim.Find(id, userContext, "FMOVIM", fieldsToQuery: fieldsToLoad);
			if (Model == null)
		...
		public static StatusMessage InsertConditions(UserContext userContext)
		{
			...
			Models.Movim model = new Models.Movim(userContext) { Identifier = "FMOVIM" };

			var navigation = m_userContext.CurrentNavigation;
			// The "LoadKeysFromHistory" must be after the "LoadEPH" because the PHE's in the tree mark Foreign Keys to null
			// (since they cannot assign multiple values to a single field) and thus the value that comes from Navigation is lost.
			// And this makes it more like the order of loading the model when opening the form.
			model.LoadEPH("FMOVIM");
			if (navigation != null)
		...
		public override void MapToModel(Models.Movim m)
		{
			...
			{
				CSGenio.framework.Log.Error($"Map ViewModel (Movim) to Model (Movim) - Error during mapping. All user values: {HasDisabledUserValuesSecurity}");
				throw;
			}
		}
			...
			try
			{
				...
				switch (fullFieldName)
				{
					case "movim.codequip":
						...
						break;
					case "movim.codrooms":
						this.ValCodrooms = ViewModelConversion.ToString(_value);
						break;
					case "movim.dhmudanc":
						this.ValDhmudanc = ViewModelConversion.ToDateTime(_value);
						break;
					case "movim.observat":
						this.ValObservat = ViewModelConversion.ToString(_value);
						break;
					case "movim.codmovim":
						this.ValCodmovim = ViewModelConversion.ToString(_value);
						break;
					default:
						Log.Error($"SetViewModelValue (Movim) - Unexpected field identifier {fullFieldName}");
						break;
				}
			}
			catch (Exception ex)
			{
				throw new FrameworkException(Resources.Resources.PEDIMOS_DESCULPA__OC63848, "SetViewModelValue (Movim)", "Unexpected error", ex);
			}
		}
		...
		{
			try { Model = Models.Movim.Find(id ?? Navigation.GetStrValue("movim"), m_userContext, "FMOVIM"); }
			finally { Model ??= new Models.Movim(m_userContext) { Identifier = "FMOVIM" }; }
		...
		public override void Load(NameValueCollection qs, bool editable, bool ajaxRequest = false, bool lazyLoad = false)
		{
			...
			try
			{
				Model = Models.Movim.Find(Navigation.GetStrValue("movim"), m_userContext, "FMOVIM");
			}
			finally
			{
				if (Model == null)
		...
		public override void LoadPartial(NameValueCollection qs, bool lazyLoad = false)
		{
			...
			if (Model == null)
			{
				...
				if (Model == null)
				{
					Model = new Models.Movim(m_userContext) { Identifier = "FMOVIM" };
...
// USE /[MANUAL GQT VIEWMODEL_DESTROY MOVIM]/
		...
		{
			Model = Models.Movim.Find(id, m_userContext, "FMOVIM");
			if (Model == null)
				throw new ModelNotFoundException("Model not found");
		...
		public void Load_Movim___equipregistnr(NameValueCollection qs, bool lazyLoad = false)
		{
			...
			CriteriaSet movim___equipregistnrConds = CriteriaSet.And();
			{
				...
				{
					movim___equipregistnrConds.Equal(CSGenioAequip.FldCodequip, hValue);
...
// USE /[MANUAL GQT OVERRQ MOVIM_EQUIPREGISTNR]/
				...
				*/
				...
				ListingMVC<CSGenioAequip> listing = Models.ModelBase.Where<CSGenioAequip>(m_userContext, false, movim___equipregistnrConds, fields, offset, numberItems, sorts, "LED_MOVIM___EQUIPREGISTNR", true, false, firstVisibleColumn: firstVisibleColumn);
		...
		public ConcurrentDictionary<string, object> GetDependant_MovimTableEquipRegistnr(string PKey)
		{
		...
		{
			var row = GetDependant_MovimTableEquipRegistnr(this.ValCodequip);
			try
			{
		...
		public void Load_Movim___roomsroomnr__(NameValueCollection qs, bool lazyLoad = false)
		{
			...
			CriteriaSet movim___roomsroomnr__Conds = CriteriaSet.And();
			{
				...
				if (hValue != null && !(hValue is Array) && !string.IsNullOrEmpty(Convert.ToString(hValue)))
				{
					movim___roomsroomnr__Conds.Equal(CSGenioArooms.FldCodrooms, hValue);
...
// USE /[MANUAL GQT OVERRQ MOVIM_ROOMSROOMNR]/
				...
				*/
				...
				ListingMVC<CSGenioArooms> listing = Models.ModelBase.Where<CSGenioArooms>(m_userContext, false, movim___roomsroomnr__Conds, fields, offset, numberItems, sorts, "LED_MOVIM___ROOMSROOMNR__", true, false, firstVisibleColumn: firstVisibleColumn);
		...
		public ConcurrentDictionary<string, object> GetDependant_MovimTableRoomsRoomnr(string PKey)
		{
			FieldRef[] refDependantFields = [CSGenioArooms.FldCodrooms, CSGenioArooms.FldRoomnr];
		...
		{
			var row = GetDependant_MovimTableRoomsRoomnr(this.ValCodrooms);
			try
			{
			...
			{
				CSGenio.framework.Log.Error(string.Format("FillDependant_Error (TableRoomsRoomnr): {0}; {1}", ex.Message, ex.InnerException != null ? ex.InnerException.Message : ""));
			}
		}
		...
		protected override object GetViewModelValue(string identifier, object modelValue)
		{
			...
			{
				"movim.codequip" => ViewModelConversion.ToString(modelValue),
				"movim.codrooms" => ViewModelConversion.ToString(modelValue),
				"movim.dhmudanc" => ViewModelConversion.ToDateTime(modelValue),
				"movim.observat" => ViewModelConversion.ToString(modelValue),
				"movim.codmovim" => ViewModelConversion.ToString(modelValue),
				"equip.codequip" => ViewModelConversion.ToString(modelValue),
				"equip.registnr" => ViewModelConversion.ToString(modelValue),
				"rooms.codrooms" => ViewModelConversion.ToString(modelValue),
				"rooms.roomnr" => ViewModelConversion.ToString(modelValue),
				_ => modelValue
			};
		}
