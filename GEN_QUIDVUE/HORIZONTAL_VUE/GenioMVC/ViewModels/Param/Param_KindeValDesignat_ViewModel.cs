...
{
	public class Param_KindeValDesignat_ViewModel : MenuListViewModel<Models.Kinde>
	{
		/// <summary>
		/// Gets or sets the object that represents the table and its elements.
		/// </summary>
		[JsonPropertyName("Table")]
		public TablePartial<Param_KindeValDesignat_RowViewModel> Menu { get; set; }
		...
		/// <inheritdoc/>
		public override string Uuid => "Param_KindeValDesignat";
...
// USE /[MANUAL GQT LIST_LIMITS PARAM_KINDEDESIGNAT]/
		...
		public Param_KindeValDesignat_ViewModel() : base(null!) { }

		/// <summary>
		/// Initializes a new instance of the <see cref="Param_KindeValDesignat_ViewModel" /> class.
		/// </summary>
		/// <param name="userContext">The current user request context</param>
		public Param_KindeValDesignat_ViewModel(UserContext userContext) : base(userContext)
		{
			ValCodparam = userContext.CurrentNavigation.CurrentLevel.GetEntry("param")?.ToString();
		}
		...
		public Param_KindeValDesignat_ViewModel(UserContext userContext, Models.ModelBase parentCtx) : this(userContext)
		{
			ParentCtx = parentCtx;
		}
		...
		public void Load(CSGenio.framework.TableConfiguration.TableConfiguration tableConfig, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAkinde> Qlisting, ref CriteriaSet conditions)
		{
			...
			]), "ms", "Time to load the page."))
			{
				...
				conditions.SubSets.Add(param___kindedesignatConds);
...
// USE /[MANUAL GQT OVERRQLIST PARAM_KINDEDESIGNAT]/
					...
					if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					{
						var m_iCurPag = m_userContext.PersistentSupport.getPagingPos(CSGenioAkinde.GetInformation(), QMVC_POS_RECORD, sorts, param___kindedesignatConds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
						if (m_iCurPag != -1)
							pageNumber = ((m_iCurPag - 1) / numberListItems) + 1;
					}

					ListingMVC<CSGenioAkinde> listing = Models.ModelBase.Where<CSGenioAkinde>(m_userContext, distinct, param___kindedesignatConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_PARAM___KINDEDESIGNAT", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn, fieldsWithTotalizers, tableConfig.SelectedRows);
		...
		private List<Param_KindeValDesignat_RowViewModel> MapParam_KindeValDesignat(ListingMVC<CSGenioAkinde> Qlisting)
		{
			List<Param_KindeValDesignat_RowViewModel> Elements = [];
			int i = 0;
		...
		/// to a Param_KindeValDesignat_RowViewModel object.
		/// </summary>
		/// <param name="row">The row.</param>
		private Param_KindeValDesignat_RowViewModel MapParam_KindeValDesignat(CSGenioAkinde row)
		{
			var model = new Param_KindeValDesignat_RowViewModel(m_userContext, true, _fieldsToSerialize);
			if (row == null)
				return model;
