...
namespace GenioMVC.ViewModels.Pesso
{
	public class TBS_Menu_121_ViewModel : ListViewModel
	{
		...
		public override int GetCount(User user)
		{
			...
			CriteriaSet tbs_menu_121Conds = CSGenio.business.Listing.CalculateConditionsEphGeneric(areaBase, "ML121");
			tbs_menu_121Conds.Equal(CSGenioApesso.FldZzstate, 0); //valid zzstate only
...
// USE /[MANUAL TBS OVERRQ 121]/
			...
			SelectQuery qs = sp.getSelectQueryFromListingMVC(tbs_menu_121Conds, listing);
...
// USE /[MANUAL TBS OVERRQLIST 121]/
					...
					ListingMVC<CSGenioApesso> listing = Models.ModelBase.Where<CSGenioApesso>(m_userContext, false, tbs_menu_121Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "ML121", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn, fieldsWithTotalizers, tableConfig.SelectedRows);
		...
		{
			var Elements = new List<TBS_Menu_121_RowViewModel>();
			int i = 0;
		...
		/// </summary>
		public void CalculateButtonPermissions(TBS_Menu_121_RowViewModel model)
		{
			bool canView = true;
			bool canEdit = true;
			bool canDelete = true;
