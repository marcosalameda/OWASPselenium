...
{
	public class Prope05_ViewModel : FormViewModel<Models.Prope>, IPreparableForSerialization
	{
		[JsonIgnore]
		...
		public Prope05_ViewModel() : base(null!) { }

		public Prope05_ViewModel(UserContext userContext, bool nestedForm = false) : base(userContext, "FPROPE05", nestedForm) { }

		public Prope05_ViewModel(UserContext userContext, Models.Prope row, bool nestedForm = false) : base(userContext, "FPROPE05", row, nestedForm) { }

		public Prope05_ViewModel(UserContext userContext, string id, bool nestedForm = false, string[]? fieldsToLoad = null) : this(userContext, nestedForm)
		{
			this.Navigation.SetValue("prope", id);
			Model = Models.Prope.Find(id, userContext, "FPROPE05", fieldsToQuery: fieldsToLoad);
			if (Model == null)
		...
		public static StatusMessage InsertConditions(UserContext userContext)
		{
			...
			Models.Prope model = new Models.Prope(userContext) { Identifier = "FPROPE05" };

			var navigation = m_userContext.CurrentNavigation;
			// The "LoadKeysFromHistory" must be after the "LoadEPH" because the PHE's in the tree mark Foreign Keys to null
			// (since they cannot assign multiple values to a single field) and thus the value that comes from Navigation is lost.
			// And this makes it more like the order of loading the model when opening the form.
			model.LoadEPH("FPROPE05");
			if (navigation != null)
		...
		public override void MapFromModel(Models.Prope m)
		{
			if (m == null)
			{
				CSGenio.framework.Log.Error("Map Model (Prope) to ViewModel (Prope05) - Model is a null reference");
				throw new ModelNotFoundException("Model not found");
			}
			...
			{
				CSGenio.framework.Log.Error("Map Model (Prope) to ViewModel (Prope05) - Error during mapping");
				throw;
			}
		}
		...
		public override void MapToModel(Models.Prope m)
		{
			if (m == null)
			{
				CSGenio.framework.Log.Error("Map ViewModel (Prope05) to Model (Prope) - Model is a null reference");
				throw new ModelNotFoundException("Model not found");
			}
			...
			{
				CSGenio.framework.Log.Error($"Map ViewModel (Prope05) to Model (Prope) - Error during mapping. All user values: {HasDisabledUserValuesSecurity}");
				throw;
			}
		}
			...
			try
			{
				...
				switch (fullFieldName)
				{
					...
					case "prope.codprope":
						...
						break;
					default:
						Log.Error($"SetViewModelValue (Prope05) - Unexpected field identifier {fullFieldName}");
						break;
				}
			}
			catch (Exception ex)
			{
				throw new FrameworkException(Resources.Resources.PEDIMOS_DESCULPA__OC63848, "SetViewModelValue (Prope05)", "Unexpected error", ex);
			}
		}
		...
		{
			try { Model = Models.Prope.Find(id ?? Navigation.GetStrValue("prope"), m_userContext, "FPROPE05"); }
			finally { Model ??= new Models.Prope(m_userContext) { Identifier = "FPROPE05" }; }
		...
		public override void Load(NameValueCollection qs, bool editable, bool ajaxRequest = false, bool lazyLoad = false)
		{
			...
			try
			{
				Model = Models.Prope.Find(Navigation.GetStrValue("prope"), m_userContext, "FPROPE05");
			}
			finally
			{
				if (Model == null)
		...
		public override void LoadPartial(NameValueCollection qs, bool lazyLoad = false)
		{
			...
			if (Model == null)
			{
				...
				if (Model == null)
				{
					Model = new Models.Prope(m_userContext) { Identifier = "FPROPE05" };
			...
			Load_Prope05_agentname____(qs, lazyLoad);
			Load_Prope05_city_city____(qs, lazyLoad);
// USE /[MANUAL GQT VIEWMODEL_LOADPARTIAL PROPE05]/
		}

// USE /[MANUAL GQT VIEWMODEL_NEW PROPE05]/
...
// USE /[MANUAL GQT VIEWMODEL_SAVE PROPE05]/
...
// USE /[MANUAL GQT VIEWMODEL_APPLY PROPE05]/

// USE /[MANUAL GQT VIEWMODEL_DUPLICATE PROPE05]/

// USE /[MANUAL GQT VIEWMODEL_DESTROY PROPE05]/
		public override void Destroy(string id)
		{
			Model = Models.Prope.Find(id, m_userContext, "FPROPE05");
			if (Model == null)
				throw new ModelNotFoundException("Model not found");
		...
		/// <param name="qs"></param>
		/// <param name="lazyLoad">Lazy loading of dropdown items</param>
		public void Load_Prope05_agentname____(NameValueCollection qs, bool lazyLoad = false)
		{
			bool prope05_agentname____DoLoad = true;
			CriteriaSet prope05_agentname____Conds = CriteriaSet.And();
			{
				object hValue = Navigation.GetValue("agent", true);
				if (hValue != null && !(hValue is Array) && !string.IsNullOrEmpty(Convert.ToString(hValue)))
				{
					prope05_agentname____Conds.Equal(CSGenioAagent.FldCodagent, hValue);
			...
			if (lazyLoad)
			{
				...
				FillDependant_Prope05TableAgentName(lazyLoad);
				return;
			}
			...
			if (prope05_agentname____DoLoad)
			{
				...
				prope05_agentname____Conds.SubSet(search_filters);
...
// USE /[MANUAL GQT OVERRQ PROPE05_AGENTNAME]/
				...
				*/
				if (Navigation.checkFormMode("agent", FormMode.New) || Navigation.checkFormMode("agent", FormMode.Duplicate))
					prope05_agentname____Conds.SubSet(CriteriaSet.Or()
						.Equal(CSGenioAagent.FldZzstate, 0)
						.Equal(CSGenioAagent.FldCodagent, Navigation.GetStrValue("agent")));
				else
					prope05_agentname____Conds.Criterias.Add(new Criteria(new ColumnReference(CSGenioAagent.FldZzstate), CriteriaOperator.Equal, 0));

				FieldRef firstVisibleColumn = new FieldRef("agent", "name");
				ListingMVC<CSGenioAagent> listing = Models.ModelBase.Where<CSGenioAagent>(m_userContext, false, prope05_agentname____Conds, fields, offset, numberItems, sorts, "LED_PROPE05_AGENTNAME____", true, false, firstVisibleColumn: firstVisibleColumn);
				...
				FillDependant_Prope05TableAgentName();
			}
		}
		...
		public ConcurrentDictionary<string, object> GetDependant_Prope05TableAgentName(string PKey)
		{
		...
		/// </summary>
		/// <param name="lazyLoad">Lazy loading of dropdown items</param>
		public void FillDependant_Prope05TableAgentName(bool lazyLoad = false)
		{
			var row = GetDependant_Prope05TableAgentName(this.ValCodagent);
			try
			{
		...
		private readonly string[] _fieldsToSerialize_PROPE05_AGENTNAME____ = ["Agent", "Agent.ValCodagent", "Agent.ValZzstate", "Agent.ValName"];
		...
		/// <param name="qs"></param>
		/// <param name="lazyLoad">Lazy loading of dropdown items</param>
		public void Load_Prope05_city_city____(NameValueCollection qs, bool lazyLoad = false)
		{
			bool prope05_city_city____DoLoad = true;
			CriteriaSet prope05_city_city____Conds = CriteriaSet.And();
			{
				object hValue = Navigation.GetValue("city", true);
				if (hValue != null && !(hValue is Array) && !string.IsNullOrEmpty(Convert.ToString(hValue)))
				{
					prope05_city_city____Conds.Equal(CSGenioAcity.FldCodcity, hValue);
			...
			if (lazyLoad)
			{
				...
				FillDependant_Prope05TableCityCity(lazyLoad);
				return;
			}
			...
			if (prope05_city_city____DoLoad)
			{
				...
				prope05_city_city____Conds.SubSet(search_filters);
...
// USE /[MANUAL GQT OVERRQ PROPE05_CITYCITY]/
				...
				*/
				if (Navigation.checkFormMode("city", FormMode.New) || Navigation.checkFormMode("city", FormMode.Duplicate))
					prope05_city_city____Conds.SubSet(CriteriaSet.Or()
						.Equal(CSGenioAcity.FldZzstate, 0)
						.Equal(CSGenioAcity.FldCodcity, Navigation.GetStrValue("city")));
				else
					prope05_city_city____Conds.Criterias.Add(new Criteria(new ColumnReference(CSGenioAcity.FldZzstate), CriteriaOperator.Equal, 0));

				FieldRef firstVisibleColumn = new FieldRef("city", "city");
				ListingMVC<CSGenioAcity> listing = Models.ModelBase.Where<CSGenioAcity>(m_userContext, false, prope05_city_city____Conds, fields, offset, numberItems, sorts, "LED_PROPE05_CITY_CITY____", true, false, firstVisibleColumn: firstVisibleColumn);
				...
				FillDependant_Prope05TableCityCity();
			}
		}
		...
		public ConcurrentDictionary<string, object> GetDependant_Prope05TableCityCity(string PKey)
		{
		...
		/// </summary>
		/// <param name="lazyLoad">Lazy loading of dropdown items</param>
		public void FillDependant_Prope05TableCityCity(bool lazyLoad = false)
		{
			var row = GetDependant_Prope05TableCityCity(this.ValCodcity);
			try
			{
		...
		private readonly string[] _fieldsToSerialize_PROPE05_CITY_CITY____ = ["City", "City.ValCodcity", "City.ValZzstate", "City.ValCity"];
...
// USE /[MANUAL GQT VIEWMODEL_CUSTOM PROPE05]/
