...
{
	public class Prope11_AgentValName_ViewModel : MenuListViewModel<Models.Agent>
	{
		/// <summary>
		/// Gets or sets the object that represents the table and its elements.
		/// </summary>
		[JsonPropertyName("table")]
		public TablePartial<Prope11_AgentValName_RowViewModel> Menu { get; set; }
		...
		public override string Uuid => "Prope11_AgentValName";
		...
		{
// USE /[MANUAL GQT LIST_LIMITS PROPE11_AGENTNAME]/
		...
		public Prope11_AgentValName_ViewModel() : base(null!) { }

		/// <summary>
		/// Initializes a new instance of the <see cref="Prope11_AgentValName_ViewModel" /> class.
		/// </summary>
		/// <param name="userContext">The current user request context</param>
		public Prope11_AgentValName_ViewModel(UserContext userContext) : base(userContext)
		{
		...
		public Prope11_AgentValName_ViewModel(UserContext userContext, Models.ModelBase parentCtx) : this(userContext)
		{
			ParentCtx = parentCtx;
		}
		...
		public override CriteriaSet BuildCriteriaSet(CSGenio.core.framework.table.TableConfiguration tableConfig, NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			Menu ??= new TablePartial<Prope11_AgentValName_RowViewModel>();
			...
			{
				// EPH
				crs = Models.Agent.AddEPH<CSGenioAagent>(ref u, crs, "IBL_PROPE11_AGENTNAME____");
			...
			if (tableReload)
			{
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					crs.Equals(Models.Agent.AddEPH<CSGenioAagent>(ref u, null, "IBL_PROPE11_AGENTNAME____"));
			}
		...
		public void Load(CSGenio.core.framework.table.TableConfiguration tableConfig, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAagent> Qlisting, ref CriteriaSet conditions)
		{
			...
			conditions.SubSets.Add(prope11_agentname____Conds);
			prope11_agentname____Conds = BuildCriteriaSet(tableConfig, requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
...
// USE /[MANUAL GQT OVERRQ PROPE11_AGENTNAME]/
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.BuildListingForExport<CSGenioAagent>(m_userContext, false, ref prope11_agentname____Conds, exportFieldRefs, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_PROPE11_AGENTNAME____", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL GQT OVERRQLSTEXP PROPE11_AGENTNAME]/
			...
			if (tableReload)
			{
// USE /[MANUAL GQT OVERRQLIST PROPE11_AGENTNAME]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = m_userContext.PersistentSupport.getPagingPos(CSGenioAagent.GetInformation(), QMVC_POS_RECORD, sorts, prope11_agentname____Conds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
				...
				Menu.Elements = MapPrope11_AgentValName(listing);

				Menu.Identifier = "IBL_PROPE11_AGENTNAME____";

				// Last updated by [CJP] at [2015.02.03]
				// Adds the identifier to each element
				foreach (var element in Menu.Elements)
					element.Identifier = "IBL_PROPE11_AGENTNAME____";
		...
		private List<Prope11_AgentValName_RowViewModel> MapPrope11_AgentValName(ListingMVC<CSGenioAagent> Qlisting)
		{
			List<Prope11_AgentValName_RowViewModel> Elements = [];
			int i = 0;
			...
			if (Qlisting.Rows != null)
			{
				foreach (var row in Qlisting.Rows)
				{
					if (Qlisting.NumRegs > 0 && i >= Qlisting.NumRegs) // Copiado da versão antiga do RowsToViewModels
						break;
					Elements.Add(MapPrope11_AgentValName(row));
					i++;
				}
			}
		...
		/// to a Prope11_AgentValName_RowViewModel object.
		/// </summary>
		/// <param name="row">The row.</param>
		private Prope11_AgentValName_RowViewModel MapPrope11_AgentValName(CSGenioAagent row)
		{
			var model = new Prope11_AgentValName_RowViewModel(m_userContext, true, _fieldsToSerialize);
			if (row == null)
				return model;
...
// USE /[MANUAL GQT VIEWMODEL_CUSTOM PROPE11_AGENTVALNAME]/
