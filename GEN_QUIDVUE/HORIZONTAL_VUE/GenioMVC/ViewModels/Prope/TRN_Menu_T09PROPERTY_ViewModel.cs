...
{
	public class TRN_Menu_T09PROPERTY_ViewModel : ListViewModel
	{
		/// <summary>
		/// Gets or sets the object that represents the table and its elements. List type: "${exposeField.Fajuda}"
		/// </summary>
		[JsonPropertyName("Table")]
		public TablePartial<TRN_Menu_T09PROPERTY_RowViewModel> Menu { get; set; }

		/// <inheritdoc/>
		public override string TableAlias { get => "prope"; }

		/// <inheritdoc/>
		public override string Uuid { get => "3e072875-4f42-4a10-adcc-ef7e27f4cb68"; }
		...
		public override int GetCount(User user)
		{
			...
			CriteriaSet trn_menu_t09propertyConds = CSGenio.business.Listing.CalculateConditionsEphGeneric(areaBase, "MLT09PROPERTY");
			trn_menu_t09propertyConds.Equal(CSGenioAprope.FldZzstate, 0); //valid zzstate only

			//Menu fixed limits and relations:

			

// USE /[MANUAL TRN OVERRQ T09PROPERTY]/
			...
			SelectQuery qs = sp.getSelectQueryFromListingMVC(trn_menu_t09propertyConds, listing);
		...
		/// <summary>
		/// Initializes a new instance of the <see cref="TRN_Menu_T09PROPERTY_ViewModel" /> class.
		/// </summary>
		/// <param name="userContext">The current user request context</param>
		public TRN_Menu_T09PROPERTY_ViewModel(UserContext userContext) : base(userContext)
		{
		...
		public override CriteriaSet BuildCriteriaSet(CSGenio.framework.TableConfiguration.TableConfiguration tableConfig, NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			if (Menu == null)
				Menu = new TablePartial<TRN_Menu_T09PROPERTY_RowViewModel>();
			...
			{
				// EPH
				crs = Models.Prope.AddEPH<CSGenioAprope>(ref u, crs, "MLT09PROPERTY");
			...
			if (tableReload)
			{
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					crs.Equals(Models.Prope.AddEPH<CSGenioAprope>(ref u, null, "MLT09PROPERTY"));
			}
		...
		public void Load(CSGenio.framework.TableConfiguration.TableConfiguration tableConfig, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAprope> Qlisting, ref CriteriaSet conditions)
		{
			using (GenioDI.MetricsOtlp.RecordTime("menu_load_time", new List<KeyValuePair<string, object>>() {
				...
				CriteriaSet trn_menu_t09propertyConds = CriteriaSet.And();
			...
			//Tooltip for EPHs affecting this viewmodel list
			{
				...
				List<Limit> area_EPH_limits = EPH_Limit_Filler(ref limit, model_limit_area, "MLT09PROPERTY");
				...
				conditions.SubSets.Add(trn_menu_t09propertyConds);
				trn_menu_t09propertyConds = BuildCriteriaSet(tableConfig, requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
...
// USE /[MANUAL TRN OVERRQ T09PROPERTY]/
				...
				if (isToExport)
				{
					...
					Qlisting = Models.ModelBase.Where<CSGenioAprope>(m_userContext, false, trn_menu_t09propertyConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "MLT09PROPERTY", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL TRN OVERRQLSTEXP T09PROPERTY]/
				...
				if (tableReload)
				{
// USE /[MANUAL TRN OVERRQLIST T09PROPERTY]/
					...
					if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					{
						var m_iCurPag = m_userContext.PersistentSupport.getPagingPos(CSGenioAprope.GetInformation(), QMVC_POS_RECORD, sorts, trn_menu_t09propertyConds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
						if (m_iCurPag != -1)
							pageNumber = ((m_iCurPag - 1) / numberListItems) + 1;
					}

					ListingMVC<CSGenioAprope> listing = Models.ModelBase.Where<CSGenioAprope>(m_userContext, false, trn_menu_t09propertyConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "MLT09PROPERTY", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn, fieldsWithTotalizers, tableConfig.SelectedRows);
					...
					Menu.Elements = MapTRN_Menu_T09PROPERTY(listing);

					Menu.Identifier = "MLT09PROPERTY";
					Menu.Slots = new Dictionary<string, List<object>>();

					// Last updated by [CJP] at [2015.02.03]
					// Adds the identifier to each element
					foreach (var element in Menu.Elements)
						element.Identifier = "MLT09PROPERTY";
		...
		private List<TRN_Menu_T09PROPERTY_RowViewModel> MapTRN_Menu_T09PROPERTY(ListingMVC<CSGenioAprope> Qlisting)
		{
			var Elements = new List<TRN_Menu_T09PROPERTY_RowViewModel>();
			int i = 0;
			...
			if (Qlisting.Rows != null)
			{
				foreach (var row in Qlisting.Rows)
				{
					if (Qlisting.NumRegs > 0 && i >= Qlisting.NumRegs) // Copiado da versão antiga do RowsToViewModels
						break;
					Elements.Add(MapTRN_Menu_T09PROPERTY(row));
					i++;
				}
			}
		...
		/// to a TRN_Menu_T09PROPERTY_RowViewModel object.
		/// </summary>
		/// <param name="row">The row.</param>
		private TRN_Menu_T09PROPERTY_RowViewModel MapTRN_Menu_T09PROPERTY(CSGenioAprope row)
		{
			var model = new TRN_Menu_T09PROPERTY_RowViewModel(m_userContext, true, _fieldsToSerialize);
		...
		/// </summary>
		public void CalculateButtonPermissions(TRN_Menu_T09PROPERTY_RowViewModel model)
		{
			bool canView = true;
			bool canEdit = true;
			bool canDelete = true;
		...
		#region Custom code
// USE /[MANUAL GQT VIEWMODEL_CUSTOM TRN_MENU_T09PROPERTY]/
		#endregion
