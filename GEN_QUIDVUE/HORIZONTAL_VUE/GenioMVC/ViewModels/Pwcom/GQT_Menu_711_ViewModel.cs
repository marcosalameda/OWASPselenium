...
{
	public class GQT_Menu_711_ViewModel : MenuListViewModel<Models.Pwcom>
	{
		/// <summary>
		/// Gets or sets the object that represents the table and its elements.
		/// </summary>
		[JsonPropertyName("table")]
		public TablePartial<GQT_Menu_711_RowViewModel> Menu { get; set; }
		...
		/// <inheritdoc/>
		[JsonIgnore]
		public override string TableAlias => "pwcom";

		/// <inheritdoc/>
		[JsonPropertyName("uuid")]
		public override string Uuid => "b84668d1-565f-459d-a7c5-24dcba182b3d";
...
// USE /[MANUAL GQT LIST_LIMITS 711]/
		...
		public override int GetCount(User user)
		{
			...
			ListingMVC<CSGenioApwcom> listing = new(fields, null, 1, 1, false, user, true, string.Empty, false);
		...
		public GQT_Menu_711_ViewModel() : base(null!) { }

		/// <summary>
		/// Initializes a new instance of the <see cref="GQT_Menu_711_ViewModel" /> class.
		/// </summary>
		/// <param name="userContext">The current user request context</param>
		public GQT_Menu_711_ViewModel(UserContext userContext) : base(userContext)
		{
		...
		public GQT_Menu_711_ViewModel(UserContext userContext, Models.ModelBase parentCtx) : this(userContext)
		{
			ParentCtx = parentCtx;
		}
		...
		public void LoadToExport(out ListingMVC<CSGenioApwcom> listing, out CriteriaSet conditions, out List<Exports.QColumn> columns, CSGenio.core.framework.table.TableConfiguration tableConfig, NameValueCollection requestValues, bool ajaxRequest = false)
		{
			listing = null;
			conditions = null;
		...
		/// <param name="Qlisting">The rows.</param>
		/// <param name="conditions">The conditions.</param>
		public void Load(CSGenio.core.framework.table.TableConfiguration tableConfig, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioApwcom> Qlisting, ref CriteriaSet conditions)
		{
			User u = m_userContext.User;
			Menu = new TablePartial<GQT_Menu_711_RowViewModel>();

			CriteriaSet gqt_menu_711Conds = CriteriaSet.And();
			...
			List<ColumnSort> sorts = GetRequestSorts(this.Menu, tableConfig, "pwcom", allSortOrders);
			...
			//Tooltip for EPHs affecting this viewmodel list
			{
				...
				CSGenioApwcom model_limit_area = new CSGenioApwcom(m_userContext.User);
			...
			conditions.SubSets.Add(gqt_menu_711Conds);
			gqt_menu_711Conds = BuildCriteriaSet(tableConfig, requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
...
// USE /[MANUAL GQT OVERRQ 711]/
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.BuildListingForExport<CSGenioApwcom>(m_userContext, false, ref gqt_menu_711Conds, exportFieldRefs, (pageNumber - 1) * numberListItems, numberListItems, sorts, "ML711", true, firstVisibleColumn: firstVisibleColumn);
...
// USE /[MANUAL GQT OVERRQLIST 711]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = m_userContext.PersistentSupport.getPagingPos(CSGenioApwcom.GetInformation(), QMVC_POS_RECORD, sorts, gqt_menu_711Conds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
		...
		{
			List<GQT_Menu_711_RowViewModel> Elements = [];
			int i = 0;
		...
		/// to a GQT_Menu_711_RowViewModel object.
		/// </summary>
		/// <param name="row">The row.</param>
		private GQT_Menu_711_RowViewModel MapGQT_Menu_711(CSGenioApwcom row)
		{
			var model = new GQT_Menu_711_RowViewModel(m_userContext, true, _fieldsToSerialize);
			if (row == null)
				return model;
		...
		private void SetDocumentFields(ListingMVC<CSGenioApwcom> listing)
		{
		}
...
// USE /[MANUAL GQT VIEWMODEL_CUSTOM GQT_MENU_711]/
		...
		protected void SetTicketToImageFields(Models.Pwcom row)
		{
			...
			row.ValFotoQTicket = Helpers.Helpers.GetFileTicket(m_userContext.User, CSGenio.business.Area.AreaPWCOM, CSGenioApwcom.FldFoto.Field, null, row.ValCodpwcom);
		}
	}
}
