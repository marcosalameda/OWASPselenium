...
{
	public class Pworg_OrganValOrganiza_ViewModel : MenuListViewModel<Models.Organ>
	{
		/// <summary>
		/// Gets or sets the object that represents the table and its elements.
		/// </summary>
		[JsonPropertyName("table")]
		public TablePartial<Pworg_OrganValOrganiza_RowViewModel> Menu { get; set; }
		...
		public override string Uuid => "Pworg_OrganValOrganiza";
...
// USE /[MANUAL GQT LIST_LIMITS PWORG_ORGANORGANIZA]/
		...
		public Pworg_OrganValOrganiza_ViewModel() : base(null!) { }

		/// <summary>
		/// Initializes a new instance of the <see cref="Pworg_OrganValOrganiza_ViewModel" /> class.
		/// </summary>
		/// <param name="userContext">The current user request context</param>
		public Pworg_OrganValOrganiza_ViewModel(UserContext userContext) : base(userContext)
		{
		...
		public Pworg_OrganValOrganiza_ViewModel(UserContext userContext, Models.ModelBase parentCtx) : this(userContext)
		{
			ParentCtx = parentCtx;
		}
		...
		public void Load(CSGenio.core.framework.table.TableConfiguration tableConfig, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAorgan> Qlisting, ref CriteriaSet conditions)
		{
			...
			conditions.SubSets.Add(pworg___organorganizaConds);
...
// USE /[MANUAL GQT OVERRQ PWORG_ORGANORGANIZA]/
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.BuildListingForExport<CSGenioAorgan>(m_userContext, false, ref pworg___organorganizaConds, exportFieldRefs, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_PWORG___ORGANORGANIZA", true, firstVisibleColumn: firstVisibleColumn);
...
// USE /[MANUAL GQT OVERRQLIST PWORG_ORGANORGANIZA]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = m_userContext.PersistentSupport.getPagingPos(CSGenioAorgan.GetInformation(), QMVC_POS_RECORD, sorts, pworg___organorganizaConds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
						pageNumber = ((m_iCurPag - 1) / numberListItems) + 1;
				}

				ListingMVC<CSGenioAorgan> listing = Models.ModelBase.Where<CSGenioAorgan>(m_userContext, distinct, pworg___organorganizaConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_PWORG___ORGANORGANIZA", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn, fieldsWithTotalizers, tableConfig.SelectedRows);
		...
		private List<Pworg_OrganValOrganiza_RowViewModel> MapPworg_OrganValOrganiza(ListingMVC<CSGenioAorgan> Qlisting)
		{
			List<Pworg_OrganValOrganiza_RowViewModel> Elements = [];
			int i = 0;
		...
		/// to a Pworg_OrganValOrganiza_RowViewModel object.
		/// </summary>
		/// <param name="row">The row.</param>
		private Pworg_OrganValOrganiza_RowViewModel MapPworg_OrganValOrganiza(CSGenioAorgan row)
		{
			var model = new Pworg_OrganValOrganiza_RowViewModel(m_userContext, true, _fieldsToSerialize);
			if (row == null)
				return model;
