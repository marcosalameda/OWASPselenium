...
{
	public class WMS_Menu_121_ViewModel : MenuListViewModel<Models.Recei>
	{
		/// <summary>
		/// Gets or sets the object that represents the table and its elements.
		/// </summary>
		[JsonPropertyName("table")]
		public TablePartial<WMS_Menu_121_RowViewModel> Menu { get; set; }
		...
		public override string Uuid => "d5a95839-cfc6-40f7-89d3-b36149d26ab4";
		...
		{
// USE /[MANUAL WMS LIST_LIMITS 121]/
		...
		public override int GetCount(User user)
		{
			...
			ListingMVC<CSGenioArecei> listing = new(fields, null, 1, 1, false, user, true, string.Empty, false);
		...
		public WMS_Menu_121_ViewModel() : base(null!) { }

		/// <summary>
		/// Initializes a new instance of the <see cref="WMS_Menu_121_ViewModel" /> class.
		/// </summary>
		/// <param name="userContext">The current user request context</param>
		public WMS_Menu_121_ViewModel(UserContext userContext) : base(userContext)
		{
		...
		public WMS_Menu_121_ViewModel(UserContext userContext, Models.ModelBase parentCtx) : this(userContext)
		{
			ParentCtx = parentCtx;
		}
		...
		public void LoadToExport(out ListingMVC<CSGenioArecei> listing, out CriteriaSet conditions, out List<Exports.QColumn> columns, CSGenio.core.framework.table.TableConfiguration tableConfig, NameValueCollection requestValues, bool ajaxRequest = false)
		{
			listing = null;
			conditions = null;
		...
		public override CriteriaSet BuildCriteriaSet(CSGenio.core.framework.table.TableConfiguration tableConfig, NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			Menu ??= new TablePartial<WMS_Menu_121_RowViewModel>();
			...
			if (!tableConfig.GroupFilters.ContainsKey("filter_WMS_Menu_121_VERIFICATI"))
			{
				string defaultValue = "1";
				tableConfig.Filters.Add(new GroupFilter { Key = "filter_WMS_Menu_121_VERIFICATI", Value = defaultValue });
			}

			{
				var groupFilters = CriteriaSet.Or();
				bool filter_WMS_Menu_121_VERIFICATI_1 = false;
				if (tableConfig.GroupFilters.ContainsKey("filter_WMS_Menu_121_VERIFICATI"))
					filter_WMS_Menu_121_VERIFICATI_1 = tableConfig.GroupFilters["filter_WMS_Menu_121_VERIFICATI"].Contains("1");
				else if (!tableConfig.GroupFilters.ContainsKey("filter_WMS_Menu_121_VERIFICATI"))
					filter_WMS_Menu_121_VERIFICATI_1 = true;
				if (filter_WMS_Menu_121_VERIFICATI_1)
				{
					groupFilters.Equal(CSGenioArecei.FldTocheck, 1);

				}

				bool filter_WMS_Menu_121_VERIFICATI_2 = false;
				if (tableConfig.GroupFilters.ContainsKey("filter_WMS_Menu_121_VERIFICATI"))
					filter_WMS_Menu_121_VERIFICATI_2 = tableConfig.GroupFilters["filter_WMS_Menu_121_VERIFICATI"].Contains("2");
				if (filter_WMS_Menu_121_VERIFICATI_2)
				{
					groupFilters.Equal(CSGenioArecei.FldChecked, 1);
			...
			{
				// EPH
				crs = Models.Recei.AddEPH<CSGenioArecei>(ref u, crs, "ML121");
			...
			if (tableReload)
			{
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					crs.Equals(Models.Recei.AddEPH<CSGenioArecei>(ref u, null, "ML121"));
			}
		...
		/// <param name="Qlisting">The rows.</param>
		/// <param name="conditions">The conditions.</param>
		public void Load(CSGenio.core.framework.table.TableConfiguration tableConfig, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioArecei> Qlisting, ref CriteriaSet conditions)
		{
			User u = m_userContext.User;
			Menu = new TablePartial<WMS_Menu_121_RowViewModel>();

			CriteriaSet wms_menu_121Conds = CriteriaSet.And();
			...
			List<ColumnSort> sorts = GetRequestSorts(this.Menu, tableConfig, "recei", allSortOrders);
			...
			conditions.SubSets.Add(wms_menu_121Conds);
			wms_menu_121Conds = BuildCriteriaSet(tableConfig, requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
...
// USE /[MANUAL WMS OVERRQ 121]/
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.BuildListingForExport<CSGenioArecei>(m_userContext, false, ref wms_menu_121Conds, exportFieldRefs, (pageNumber - 1) * numberListItems, numberListItems, sorts, "ML121", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL WMS OVERRQLSTEXP 121]/
...
// USE /[MANUAL WMS OVERRQLIST 121]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = m_userContext.PersistentSupport.getPagingPos(CSGenioArecei.GetInformation(), QMVC_POS_RECORD, sorts, wms_menu_121Conds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
				...
				Menu.Elements = MapWMS_Menu_121(listing);
		...
		private List<WMS_Menu_121_RowViewModel> MapWMS_Menu_121(ListingMVC<CSGenioArecei> Qlisting)
		{
			List<WMS_Menu_121_RowViewModel> Elements = [];
			int i = 0;
			...
			if (Qlisting.Rows != null)
			{
				foreach (var row in Qlisting.Rows)
				{
					if (Qlisting.NumRegs > 0 && i >= Qlisting.NumRegs) // Copiado da versão antiga do RowsToViewModels
						break;
					Elements.Add(MapWMS_Menu_121(row));
					i++;
				}
			}
		...
		/// to a WMS_Menu_121_RowViewModel object.
		/// </summary>
		/// <param name="row">The row.</param>
		private WMS_Menu_121_RowViewModel MapWMS_Menu_121(CSGenioArecei row)
		{
			var model = new WMS_Menu_121_RowViewModel(m_userContext, true, _fieldsToSerialize);
			if (row == null)
				return model;
...
// USE /[MANUAL GQT VIEWMODEL_CUSTOM WMS_MENU_121]/
