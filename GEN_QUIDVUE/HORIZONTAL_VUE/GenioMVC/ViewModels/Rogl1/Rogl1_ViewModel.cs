...
{
	public class Rogl1_ViewModel : FormViewModel<Models.Rogl1>, IPreparableForSerialization
	{
		[JsonIgnore]
		...
		public override void MapToModel(Models.Rogl1 m)
		{
			...
			{
				CSGenio.framework.Log.Error($"Map ViewModel (Rogl1) to Model (Rogl1) - Error during mapping. All user values: {HasDisabledUserValuesSecurity}");
				throw;
			}
		}
			...
			try
			{
				...
				switch (fullFieldName)
				{
					...
					case "rogl1.codrogl1":
						...
						break;
					default:
						Log.Error($"SetViewModelValue (Rogl1) - Unexpected field identifier {fullFieldName}");
						break;
				}
			}
			catch (Exception ex)
			{
				throw new FrameworkException(Resources.Resources.PEDIMOS_DESCULPA__OC63848, "SetViewModelValue (Rogl1)", "Unexpected error", ex);
			}
		}
		...
		{
			try { Model = Models.Rogl1.Find(id ?? Navigation.GetStrValue("rogl1"), m_userContext, "FROGL1"); }
			finally { Model ??= new Models.Rogl1(m_userContext) { Identifier = "FROGL1" }; }
