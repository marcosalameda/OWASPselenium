...
namespace GenioMVC.ViewModels.Rules
{
	public class PTN_Menu_131_ViewModel : ListViewModel
	{
		...
		public override int GetCount(User user)
		{
			...
			CriteriaSet ptn_menu_131Conds = CSGenio.business.Listing.CalculateConditionsEphGeneric(areaBase, "ML131");
			ptn_menu_131Conds.Equal(CSGenioArules.FldZzstate, 0); //valid zzstate only
...
// USE /[MANUAL PTN OVERRQ 131]/
			...
			SelectQuery qs = sp.getSelectQueryFromListingMVC(ptn_menu_131Conds, listing);
		...
		public void LoadToExport(out ListingMVC<CSGenioArules> listing, out CriteriaSet conditions, out List<Exports.QColumn> columns, CSGenio.framework.TableConfiguration.TableConfiguration tableConfig, NameValueCollection requestValues, bool ajaxRequest = false)
		{
			listing = null;
			conditions = null;
		...
		public void Load(CSGenio.framework.TableConfiguration.TableConfiguration tableConfig, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioArules> Qlisting, ref CriteriaSet conditions)
		{
			using (GenioDI.MetricsOtlp.RecordTime("menu_load_time", new List<KeyValuePair<string, object>>() {
				...
				List<ColumnSort> sorts = GetRequestSorts(this.Menu, tableConfig.ColumnOrderBy, "rules", allSortOrders);
			...
			//Tooltip for EPHs affecting this viewmodel list
			{
				...
				ptn_menu_131Conds = BuildCriteriaSet(tableConfig, requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
...
// USE /[MANUAL PTN OVERRQ 131]/
				...
				if (isToExport)
				{
					...
					Qlisting = Models.ModelBase.Where<CSGenioArules>(m_userContext, false, ptn_menu_131Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "ML131", true, firstVisibleColumn: firstVisibleColumn);
...
// USE /[MANUAL PTN OVERRQLIST 131]/
					...
					if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					{
						var m_iCurPag = m_userContext.PersistentSupport.getPagingPos(CSGenioArules.GetInformation(), QMVC_POS_RECORD, sorts, ptn_menu_131Conds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
						if (m_iCurPag != -1)
