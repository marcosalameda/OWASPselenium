...
{
	public class Equip_empty_ValEquip_filtrado_ViewModel : MenuListViewModel<Models.Equip>
	{
		/// <summary>
		/// Gets or sets the object that represents the table and its elements.
		/// </summary>
		[JsonPropertyName("table")]
		public TablePartial<Equip_empty_ValEquip_filtrado_RowViewModel> Menu { get; set; }
		...
		public override string Uuid => "Equip_empty_ValEquip_filtrado";
		...
		{
// USE /[MANUAL GQT LIST_LIMITS EQUIP_EMPTY_PSEUDEQUIP_FILTRADO]/
		...
		public Equip_empty_ValEquip_filtrado_ViewModel() : base(null!) { }

		/// <summary>
		/// Initializes a new instance of the <see cref="Equip_empty_ValEquip_filtrado_ViewModel" /> class.
		/// </summary>
		/// <param name="userContext">The current user request context</param>
		public Equip_empty_ValEquip_filtrado_ViewModel(UserContext userContext) : base(userContext)
		{
		}
		...
		public Equip_empty_ValEquip_filtrado_ViewModel(UserContext userContext, Models.ModelBase parentCtx) : this(userContext)
		{
			ParentCtx = parentCtx;
		}
		...
		public override CriteriaSet BuildCriteriaSet(CSGenio.core.framework.table.TableConfiguration tableConfig, NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			Menu ??= new TablePartial<Equip_empty_ValEquip_filtrado_RowViewModel>();
			...
			{
				// EPH
				crs = Models.Equip.AddEPH<CSGenioAequip>(ref u, crs, "IBL_EQUIP_EMPTY__PSEUD__EQUIP_FILTRADO");
			...
			if (tableReload)
			{
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					crs.Equals(Models.Equip.AddEPH<CSGenioAequip>(ref u, null, "IBL_EQUIP_EMPTY__PSEUD__EQUIP_FILTRADO"));
			}
		...
		public void Load(CSGenio.core.framework.table.TableConfiguration tableConfig, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAequip> Qlisting, ref CriteriaSet conditions)
		{
			...
			CriteriaSet equip_empty__pseud__equip_filtradoConds = CriteriaSet.And();
			...
			//Tooltip for EPHs affecting this viewmodel list
			{
				...
				List<Limit> area_EPH_limits = EPH_Limit_Filler(ref limit, model_limit_area, "IBL_EQUIP_EMPTY__PSEUD__EQUIP_FILTRADO");
			...
			conditions.SubSets.Add(equip_empty__pseud__equip_filtradoConds);
			equip_empty__pseud__equip_filtradoConds = BuildCriteriaSet(tableConfig, requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
			tableReload &= hasAllRequiredLimits;

// USE /[MANUAL GQT OVERRQ EQUIP_EMPTY_PSEUDEQUIP_FILTRADO]/
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.BuildListingForExport<CSGenioAequip>(m_userContext, false, ref equip_empty__pseud__equip_filtradoConds, exportFieldRefs, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_EQUIP_EMPTY__PSEUD__EQUIP_FILTRADO", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL GQT OVERRQLSTEXP EQUIP_EMPTY_PSEUDEQUIP_FILTRADO]/
			...
			if (tableReload)
			{
// USE /[MANUAL GQT OVERRQLIST EQUIP_EMPTY_PSEUDEQUIP_FILTRADO]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = m_userContext.PersistentSupport.getPagingPos(CSGenioAequip.GetInformation(), QMVC_POS_RECORD, sorts, equip_empty__pseud__equip_filtradoConds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
						pageNumber = ((m_iCurPag - 1) / numberListItems) + 1;
				}

				ListingMVC<CSGenioAequip> listing = Models.ModelBase.Where<CSGenioAequip>(m_userContext, distinct, equip_empty__pseud__equip_filtradoConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_EQUIP_EMPTY__PSEUD__EQUIP_FILTRADO", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn, fieldsWithTotalizers, tableConfig.SelectedRows);
				...
				Menu.Elements = MapEquip_empty_ValEquip_filtrado(listing);

				Menu.Identifier = "IBL_EQUIP_EMPTY__PSEUD__EQUIP_FILTRADO";

				// Last updated by [CJP] at [2015.02.03]
				// Adds the identifier to each element
				foreach (var element in Menu.Elements)
					element.Identifier = "IBL_EQUIP_EMPTY__PSEUD__EQUIP_FILTRADO";
		...
		private List<Equip_empty_ValEquip_filtrado_RowViewModel> MapEquip_empty_ValEquip_filtrado(ListingMVC<CSGenioAequip> Qlisting)
		{
			List<Equip_empty_ValEquip_filtrado_RowViewModel> Elements = [];
			int i = 0;
			...
			if (Qlisting.Rows != null)
			{
				foreach (var row in Qlisting.Rows)
				{
					if (Qlisting.NumRegs > 0 && i >= Qlisting.NumRegs) // Copiado da versão antiga do RowsToViewModels
						break;
					Elements.Add(MapEquip_empty_ValEquip_filtrado(row));
					i++;
				}
			}
		...
		/// to a Equip_empty_ValEquip_filtrado_RowViewModel object.
		/// </summary>
		/// <param name="row">The row.</param>
		private Equip_empty_ValEquip_filtrado_RowViewModel MapEquip_empty_ValEquip_filtrado(CSGenioAequip row)
		{
			var model = new Equip_empty_ValEquip_filtrado_RowViewModel(m_userContext, true, _fieldsToSerialize);
			if (row == null)
				return model;
...
// USE /[MANUAL GQT VIEWMODEL_CUSTOM EQUIP_EMPTY_VALEQUIP_FILTRADO]/
