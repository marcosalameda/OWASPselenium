...
{
	public class GQT_Menu_2D11_ViewModel : MenuListViewModel<Models.Tpequ>
	{
		/// <summary>
		/// Gets or sets the object that represents the table and its elements.
		/// </summary>
		[JsonPropertyName("table")]
		public TablePartial<GQT_Menu_2D11_RowViewModel> Menu { get; set; }
		...
		public override string Uuid => "8b6bade9-cc0e-443a-a006-75bc94e39e46";
		...
		{
// USE /[MANUAL GQT LIST_LIMITS 2D11]/
		...
		public override int GetCount(User user)
		{
			...
			CriteriaSet conditions = CSGenio.business.Listing.CalculateConditionsEphGeneric(areaBase, "ML2D11");
		...
		public GQT_Menu_2D11_ViewModel() : base(null!) { }

		/// <summary>
		/// Initializes a new instance of the <see cref="GQT_Menu_2D11_ViewModel" /> class.
		/// </summary>
		/// <param name="userContext">The current user request context</param>
		public GQT_Menu_2D11_ViewModel(UserContext userContext) : base(userContext)
		{
		...
		public GQT_Menu_2D11_ViewModel(UserContext userContext, Models.ModelBase parentCtx) : this(userContext)
		{
			ParentCtx = parentCtx;
		}
		...
		public override CriteriaSet BuildCriteriaSet(CSGenio.core.framework.table.TableConfiguration tableConfig, NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			Menu ??= new TablePartial<GQT_Menu_2D11_RowViewModel>();
			...
			{
				// EPH
				crs = Models.Tpequ.AddEPH<CSGenioAtpequ>(ref u, crs, "ML2D11");
			...
			if (tableReload)
			{
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					crs.Equals(Models.Tpequ.AddEPH<CSGenioAtpequ>(ref u, null, "ML2D11"));
			}
		...
		public void Load(CSGenio.core.framework.table.TableConfiguration tableConfig, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAtpequ> Qlisting, ref CriteriaSet conditions)
		{
			...
			CriteriaSet gqt_menu_2d11Conds = CriteriaSet.And();
			...
			//Tooltip for EPHs affecting this viewmodel list
			{
				...
				List<Limit> area_EPH_limits = EPH_Limit_Filler(ref limit, model_limit_area, "ML2D11");
			...
			conditions.SubSets.Add(gqt_menu_2d11Conds);
			gqt_menu_2d11Conds = BuildCriteriaSet(tableConfig, requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
			tableReload &= hasAllRequiredLimits;

// USE /[MANUAL GQT OVERRQ 2D11]/
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.BuildListingForExport<CSGenioAtpequ>(m_userContext, false, ref gqt_menu_2d11Conds, exportFieldRefs, (pageNumber - 1) * numberListItems, numberListItems, sorts, "ML2D11", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL GQT OVERRQLSTEXP 2D11]/
			...
			if (tableReload)
			{
// USE /[MANUAL GQT OVERRQLIST 2D11]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = m_userContext.PersistentSupport.getPagingPos(CSGenioAtpequ.GetInformation(), QMVC_POS_RECORD, sorts, gqt_menu_2d11Conds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
						pageNumber = ((m_iCurPag - 1) / numberListItems) + 1;
				}

				ListingMVC<CSGenioAtpequ> listing = Models.ModelBase.Where<CSGenioAtpequ>(m_userContext, distinct, gqt_menu_2d11Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "ML2D11", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn, fieldsWithTotalizers, tableConfig.SelectedRows);
				...
				Menu.Elements = MapGQT_Menu_2D11(listing);

				Menu.Identifier = "ML2D11";
				Menu.Slots = new Dictionary<string, List<object>>();

				// Last updated by [CJP] at [2015.02.03]
				// Adds the identifier to each element
				foreach (var element in Menu.Elements)
					element.Identifier = "ML2D11";
		...
		private List<GQT_Menu_2D11_RowViewModel> MapGQT_Menu_2D11(ListingMVC<CSGenioAtpequ> Qlisting)
		{
			List<GQT_Menu_2D11_RowViewModel> Elements = [];
			int i = 0;
			...
			if (Qlisting.Rows != null)
			{
				foreach (var row in Qlisting.Rows)
				{
					if (Qlisting.NumRegs > 0 && i >= Qlisting.NumRegs) // Copiado da versão antiga do RowsToViewModels
						break;
					Elements.Add(MapGQT_Menu_2D11(row));
					i++;
				}
			}
		...
		/// to a GQT_Menu_2D11_RowViewModel object.
		/// </summary>
		/// <param name="row">The row.</param>
		private GQT_Menu_2D11_RowViewModel MapGQT_Menu_2D11(CSGenioAtpequ row)
		{
			var model = new GQT_Menu_2D11_RowViewModel(m_userContext, true, _fieldsToSerialize);
			if (row == null)
				return model;
...
// USE /[MANUAL GQT VIEWMODEL_CUSTOM GQT_MENU_2D11]/
