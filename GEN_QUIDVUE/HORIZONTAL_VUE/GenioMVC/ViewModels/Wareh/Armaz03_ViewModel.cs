...
{
	public class Armaz03_ViewModel : FormViewModel<Models.Wareh>, IPreparableForSerialization
	{
		[JsonIgnore]
		...
		public Armaz03_ViewModel() : base(null!) { }

		public Armaz03_ViewModel(UserContext userContext, bool nestedForm = false) : base(userContext, "FARMAZ03", nestedForm) { }

		public Armaz03_ViewModel(UserContext userContext, Models.Wareh row, bool nestedForm = false) : base(userContext, "FARMAZ03", row, nestedForm) { }

		public Armaz03_ViewModel(UserContext userContext, string id, bool nestedForm = false, string[]? fieldsToLoad = null) : this(userContext, nestedForm)
		{
			this.Navigation.SetValue("wareh", id);
			Model = Models.Wareh.Find(id, userContext, "FARMAZ03", fieldsToQuery: fieldsToLoad);
			if (Model == null)
		...
		public static StatusMessage InsertConditions(UserContext userContext)
		{
			...
			Models.Wareh model = new Models.Wareh(userContext) { Identifier = "FARMAZ03" };

			var navigation = m_userContext.CurrentNavigation;
			// The "LoadKeysFromHistory" must be after the "LoadEPH" because the PHE's in the tree mark Foreign Keys to null
			// (since they cannot assign multiple values to a single field) and thus the value that comes from Navigation is lost.
			// And this makes it more like the order of loading the model when opening the form.
			model.LoadEPH("FARMAZ03");
			if (navigation != null)
		...
		public override void MapToModel(Models.Wareh m)
		{
			...
			{
				CSGenio.framework.Log.Error($"Map ViewModel (Armaz03) to Model (Wareh) - Error during mapping. All user values: {HasDisabledUserValuesSecurity}");
				throw;
			}
		}
			...
			try
			{
				...
				switch (fullFieldName)
				{
					case "wareh.codwareh":
						...
						break;
					default:
						Log.Error($"SetViewModelValue (Armaz03) - Unexpected field identifier {fullFieldName}");
						break;
				}
			}
			catch (Exception ex)
			{
				throw new FrameworkException(Resources.Resources.PEDIMOS_DESCULPA__OC63848, "SetViewModelValue (Armaz03)", "Unexpected error", ex);
			}
		}
		...
		{
			try { Model = Models.Wareh.Find(id ?? Navigation.GetStrValue("wareh"), m_userContext, "FARMAZ03"); }
			finally { Model ??= new Models.Wareh(m_userContext) { Identifier = "FARMAZ03" }; }
		...
		public override void Load(NameValueCollection qs, bool editable, bool ajaxRequest = false, bool lazyLoad = false)
		{
			...
			try
			{
				Model = Models.Wareh.Find(Navigation.GetStrValue("wareh"), m_userContext, "FARMAZ03");
			}
			finally
			{
				if (Model == null)
		...
		public override void LoadPartial(NameValueCollection qs, bool lazyLoad = false)
		{
			...
			if (Model == null)
			{
				...
				if (Model == null)
				{
					Model = new Models.Wareh(m_userContext) { Identifier = "FARMAZ03" };
...
// USE /[MANUAL GQT VIEWMODEL_DESTROY ARMAZ03]/
		...
		{
			Model = Models.Wareh.Find(id, m_userContext, "FARMAZ03");
			if (Model == null)
				throw new ModelNotFoundException("Model not found");
