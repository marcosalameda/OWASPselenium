...
{
	public class Armazpop_ViewModel : FormViewModel<Models.Wareh>, IPreparableForSerialization
	{
		[JsonIgnore]
		...
		public override void MapToModel(Models.Wareh m)
		{
			...
			{
				CSGenio.framework.Log.Error($"Map ViewModel (Armazpop) to Model (Wareh) - Error during mapping. All user values: {HasDisabledUserValuesSecurity}");
				throw;
			}
		}
			...
			try
			{
				...
				switch (fullFieldName)
				{
					...
					case "wareh.codwareh":
						...
						break;
					default:
						Log.Error($"SetViewModelValue (Armazpop) - Unexpected field identifier {fullFieldName}");
						break;
				}
			}
			catch (Exception ex)
			{
				throw new FrameworkException(Resources.Resources.PEDIMOS_DESCULPA__OC63848, "SetViewModelValue (Armazpop)", "Unexpected error", ex);
			}
		}
		...
		{
			try { Model = Models.Wareh.Find(id ?? Navigation.GetStrValue("wareh"), m_userContext, "FARMAZPOP"); }
			finally { Model ??= new Models.Wareh(m_userContext) { Identifier = "FARMAZPOP" }; }
