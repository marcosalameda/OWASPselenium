...
namespace GenioMVC.ViewModels.Wareh
{
	public class STY_Menu_TIMELIN_ViewModel : ListViewModel
	{
		...
		public override int GetCount(User user)
		{
			...
			CriteriaSet sty_menu_timelinConds = CSGenio.business.Listing.CalculateConditionsEphGeneric(areaBase, "MLTIMELIN");
			sty_menu_timelinConds.Equal(CSGenioAwareh.FldZzstate, 0); //valid zzstate only

			//Menu fixed limits and relations:

						sty_menu_timelinConds.Equal(CSGenioAwareh.FldShowreco, 1);
...
// USE /[MANUAL STY OVERRQ TIMELIN]/
			...
			SelectQuery qs = sp.getSelectQueryFromListingMVC(sty_menu_timelinConds, listing);
...
// USE /[MANUAL STY OVERRQLIST TIMELIN]/
					...
					ListingMVC<CSGenioAwareh> listing = Models.ModelBase.Where<CSGenioAwareh>(m_userContext, false, sty_menu_timelinConds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "MLTIMELIN", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn, fieldsWithTotalizers, tableConfig.SelectedRows);
		...
		{
			var Elements = new List<STY_Menu_TIMELIN_RowViewModel>();
			int i = 0;
		...
		/// </summary>
		public void CalculateButtonPermissions(STY_Menu_TIMELIN_RowViewModel model)
		{
			bool canView = true;
			bool canEdit = true;
			bool canDelete = true;
