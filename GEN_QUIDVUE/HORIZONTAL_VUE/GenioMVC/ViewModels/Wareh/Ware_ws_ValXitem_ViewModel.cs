...
{
	public class Ware_ws_ValXitem_ViewModel : MenuListViewModel<Models.Item>
	{
		/// <summary>
		/// Gets or sets the object that represents the table and its elements.
		/// </summary>
		[JsonPropertyName("table")]
		public TablePartial<Ware_ws_ValXitem_RowViewModel> Menu { get; set; }
		...
		public override string Uuid => "Ware_ws_ValXitem";
		...
		{
// USE /[MANUAL GQT LIST_LIMITS WARE_WS_PSEUDXITEM]/
		...
		public Ware_ws_ValXitem_ViewModel() : base(null!) { }

		/// <summary>
		/// Initializes a new instance of the <see cref="Ware_ws_ValXitem_ViewModel" /> class.
		/// </summary>
		...
		public Ware_ws_ValXitem_ViewModel(UserContext userContext, Models.ModelBase parentCtx) : this(userContext)
		{
			ParentCtx = parentCtx;
		}
		...
		public override CriteriaSet BuildCriteriaSet(CSGenio.core.framework.table.TableConfiguration tableConfig, NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			Menu ??= new TablePartial<Ware_ws_ValXitem_RowViewModel>();
			...
			{
				// EPH
				crs = Models.Item.AddEPH<CSGenioAitem>(ref u, crs, "IBL_WARE_WS_PSEUDXITEM___");
			...
			if (tableReload)
			{
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					crs.Equals(Models.Item.AddEPH<CSGenioAitem>(ref u, null, "IBL_WARE_WS_PSEUDXITEM___"));
			}
		...
		public void Load(CSGenio.core.framework.table.TableConfiguration tableConfig, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAitem> Qlisting, ref CriteriaSet conditions)
		{
			...
			CriteriaSet ware_ws_pseudxitem___Conds = CriteriaSet.And();
			...
			FieldRef[] fields = new FieldRef[] { CSGenioAitem.FldCoditem, CSGenioAitem.FldZzstate, CSGenioAitem.FldItemtype, CSGenioAitem.FldItemdes, CSGenioAitem.FldItemcod, CSGenioAitem.FldEntries, CSGenioAitem.FldExits, CSGenioAitem.FldExistenc, CSGenioAitem.FldImage, CSGenioAitem.FldCategory, CSGenioAitem.FldValid, CSGenioAitem.FldDisponib, CSGenioAitem.FldDate };
			...
			//Tooltip for EPHs affecting this viewmodel list
			{
				...
				List<Limit> area_EPH_limits = EPH_Limit_Filler(ref limit, model_limit_area, "IBL_WARE_WS_PSEUDXITEM___");
			...
			conditions.SubSets.Add(ware_ws_pseudxitem___Conds);
			ware_ws_pseudxitem___Conds = BuildCriteriaSet(tableConfig, requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
...
// USE /[MANUAL GQT OVERRQ WARE_WS_PSEUDXITEM]/
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.BuildListingForExport<CSGenioAitem>(m_userContext, false, ref ware_ws_pseudxitem___Conds, exportFieldRefs, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_WARE_WS_PSEUDXITEM___", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL GQT OVERRQLSTEXP WARE_WS_PSEUDXITEM]/
			...
			if (tableReload)
			{
// USE /[MANUAL GQT OVERRQLIST WARE_WS_PSEUDXITEM]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = m_userContext.PersistentSupport.getPagingPos(CSGenioAitem.GetInformation(), QMVC_POS_RECORD, sorts, ware_ws_pseudxitem___Conds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
				...
				Menu.Elements = MapWare_ws_ValXitem(listing);

				Menu.Identifier = "IBL_WARE_WS_PSEUDXITEM___";

				// Last updated by [CJP] at [2015.02.03]
				// Adds the identifier to each element
				foreach (var element in Menu.Elements)
					element.Identifier = "IBL_WARE_WS_PSEUDXITEM___";
		...
		private List<Ware_ws_ValXitem_RowViewModel> MapWare_ws_ValXitem(ListingMVC<CSGenioAitem> Qlisting)
		{
			List<Ware_ws_ValXitem_RowViewModel> Elements = [];
			int i = 0;
			...
			if (Qlisting.Rows != null)
			{
				foreach (var row in Qlisting.Rows)
				{
					if (Qlisting.NumRegs > 0 && i >= Qlisting.NumRegs) // Copiado da versão antiga do RowsToViewModels
						break;
					Elements.Add(MapWare_ws_ValXitem(row));
					i++;
				}
			}
		...
		/// to a Ware_ws_ValXitem_RowViewModel object.
		/// </summary>
		/// <param name="row">The row.</param>
		private Ware_ws_ValXitem_RowViewModel MapWare_ws_ValXitem(CSGenioAitem row)
		{
			var model = new Ware_ws_ValXitem_RowViewModel(m_userContext, true, _fieldsToSerialize);
			if (row == null)
				return model;
...
// USE /[MANUAL GQT VIEWMODEL_CUSTOM WARE_WS_VALXITEM]/

		#endregion

		private static readonly string[] _fieldsToSerialize =
		[
			"Item", "Item.ValCoditem", "Item.ValZzstate", "Item.ValItemtype", "Item.ValItemdes", "Item.ValItemcod", "Item.ValEntries", "Item.ValExits", "Item.ValExistenc", "Item.ValImage", "Item.ValCategory", "Item.ValValid", "Item.ValDisponib", "Item.ValDate", "Item.ValCodgitem", "Item.ValCodwareh"
		];
