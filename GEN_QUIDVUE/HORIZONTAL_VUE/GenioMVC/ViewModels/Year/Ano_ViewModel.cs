...
{
	public class Ano_ViewModel : FormViewModel<Models.Year>, IPreparableForSerialization
	{
		[JsonIgnore]
		...
		public override void MapToModel(Models.Year m)
		{
			...
			{
				CSGenio.framework.Log.Error($"Map ViewModel (Ano) to Model (Year) - Error during mapping. All user values: {HasDisabledUserValuesSecurity}");
				throw;
			}
		}
			...
			try
			{
				...
				switch (fullFieldName)
				{
					case "year.year":
						...
						break;
					case "year.yearnum":
						this.ValYearnum = ViewModelConversion.ToNumeric(_value);
						break;
					case "year.codyear":
						this.ValCodyear = ViewModelConversion.ToString(_value);
						break;
					default:
						Log.Error($"SetViewModelValue (Ano) - Unexpected field identifier {fullFieldName}");
						break;
				}
			}
			catch (Exception ex)
			{
				throw new FrameworkException(Resources.Resources.PEDIMOS_DESCULPA__OC63848, "SetViewModelValue (Ano)", "Unexpected error", ex);
			}
		}
		...
		{
			try { Model = Models.Year.Find(id ?? Navigation.GetStrValue("year"), m_userContext, "FANO"); }
			finally { Model ??= new Models.Year(m_userContext) { Identifier = "FANO" }; }
