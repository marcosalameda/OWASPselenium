...
#pragma warning restore CS0168, S1481 // Variable is declared but never used
			...
			//- - - - - - - - - - - - - - - - - - -
			Qfield = new Field(info.Alias, "compicon", FieldType.ARRAY_NUMERIC);
			Qfield.FieldDescription = "Component class";
			Qfield.FieldSize =  1;
			Qfield.MQueue = false;
			Qfield.CavDesignation = "COMPONENT_CLASS57908";

			Qfield.Dupmsg = "";
			argumentsListByArea = new List<ByAreaArguments>();
			argumentsListByArea.Add(new ByAreaArguments(new string[] {"compclas","compclas","compclas","compclas","compclas","compclas","compclas"}, new int[] {0,1,2,3,4,5,6}, "compc", "codcompc"));
			Qfield.Formula = new InternalOperationFormula(argumentsListByArea, 7, delegate(object[] args, User user, string module, PersistentSupport sp) {
				return ((((string)args[0])=="Media")?(1):(((((string)args[1])=="Data Input")?(2):(((((string)args[2])=="Data Grid")?(3):(((((string)args[3])=="Action")?(4):(((((string)args[4])=="Container")?(5):(((((string)args[5])=="Data Display")?(6):(((((string)args[6])=="Interactive")?(7):(8))))))))))))));
			});
			Qfield.ArrayName = "dbo.GetValArrayNcomponenticons";
            Qfield.ArrayClassName = "Componenticons";
			info.RegisterFieldDB(Qfield);
		...
		private static void InicializaRelacoes(AreaInfo info)
		{
			...
			info.ChildTable[0]= new ChildRelation("compb", new String[] {"codcompo"}, DeleteProc.NA);
			info.ChildTable[1]= new ChildRelation("compv", new String[] {"codcompo"}, DeleteProc.NA);
		...
		public static FieldRef FldCompicon { get { return m_fldCompicon; } }
		private static FieldRef m_fldCompicon = new FieldRef("compo", "compicon");

		/// <summary>Field : "Component class" Tipo: "AN" Formula: + "iif ([COMPC->COMPCLAS] == "Media", 1, iif ([COMPC->COMPCLAS] == "Data Input", 2, iif ([COMPC->COMPCLAS] == "Data Grid", 3, iif ([COMPC->COMPCLAS] == "Action", 4, iif ([COMPC->COMPCLAS] == "Container", 5, iif ([COMPC->COMPCLAS] == "Data Display", 6, iif ([COMPC->COMPCLAS] == "Interactive", 7, 8)))))))"</summary>
		public decimal ValCompicon
		{
			get { return (decimal)returnValueField(FldCompicon); }
