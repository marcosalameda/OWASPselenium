...
namespace CSGenio.business
{
	...
	public class CSGenioAyear1 : DbArea
	{
		...
		public CSGenioAyear1(User user, string module)
		{
			fields = new Hashtable();
            this.user = user;
            this.module = module;
			this.KeyType = CodeType.GUID_KEY;
			// USE /[MANUAL GQT CONSTRUTOR YEAR1]/
		}

		public CSGenioAyear1(User user) : this(user, user.CurrentModule)
		{
		}
...
#pragma warning restore CS0168, S1481 // Variable is declared but never used
			//- - - - - - - - - - - - - - - - - - -
			Qfield = new Field("codyear", FieldType.CHAVE_PRIMARIA_GUID);
...
//Actualiza as seguintes réplicas:
			...
			//- - - - - - - - - - - - - - - - - - -
			Qfield = new Field("yearnum", FieldType.NUMERO);
...
//Actualiza as seguintes réplicas:
		...
		private static FieldRef m_fldCodyear = new FieldRef("year1", "codyear");
		...
		private static FieldRef m_fldYear = new FieldRef("year1", "year");
		...
		private static FieldRef m_fldValue = new FieldRef("year1", "value");
		...
		private static FieldRef m_fldYearnum = new FieldRef("year1", "yearnum");
		...
		private static FieldRef m_fldZzstate = new FieldRef("year1", "zzstate");
        ...
        public static CSGenioAyear1 search(PersistentSupport sp, string key, User user, string[] fields = null)
        {
			if (string.IsNullOrEmpty(key))
				return null;

		    CSGenioAyear1 area = new CSGenioAyear1(user, user.CurrentModule);
        ...
        /// <remarks>Persistence operations should not be used on a partially positioned register</remarks>
        public static List<CSGenioAyear1> searchList(PersistentSupport sp, User user, CriteriaSet where, string[] fields = null, bool distinct = false, bool noLock = false)
        {
				return sp.searchListWhere<CSGenioAyear1>(where, user, fields, distinct, noLock);
        }
        ...
        /// <returns>A list of area records with all fields populated</returns>
        /// <remarks>Persistence operations should not be used on a partially positioned register</remarks>
        public static void searchListAdvancedWhere(PersistentSupport sp, User user, CriteriaSet where, ListingMVC<CSGenioAyear1> listing)
        {
			sp.searchListAdvancedWhere<CSGenioAyear1>(where, listing);
        }
